...
usForceNewDeployment =
  lens _usForceNewDeployment (\s a -> s {_usForceNewDeployment = a})
...
instance AWSRequest UpdateService where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateServiceResponse' <$> (x .?> "service") <*> (pure (fromEnum s)))
