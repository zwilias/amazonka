...
rtdExecutionRoleARN =
  lens _rtdExecutionRoleARN (\s a -> s {_rtdExecutionRoleARN = a})
...
rtdRequiresCompatibilities =
  lens _rtdRequiresCompatibilities (\s a -> s {_rtdRequiresCompatibilities = a}) .
  _Default . _Coerce
...
rtdPlacementConstraints ::
     Lens' RegisterTaskDefinition [TaskDefinitionPlacementConstraint]
rtdPlacementConstraints =
  lens _rtdPlacementConstraints (\s a -> s {_rtdPlacementConstraints = a}) .
  _Default . _Coerce
...
instance AWSRequest RegisterTaskDefinition where
  ...
  response =
    receiveJSON
      (\s h x ->
         RegisterTaskDefinitionResponse' <$> (x .?> "taskDefinition") <*>
         (pure (fromEnum s)))
...
instance ToHeaders RegisterTaskDefinition where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerServiceV20141113.RegisterTaskDefinition" :: ByteString)
...
rtdrsTaskDefinition ::
     Lens' RegisterTaskDefinitionResponse (Maybe TaskDefinition)
rtdrsTaskDefinition =
  lens _rtdrsTaskDefinition (\s a -> s {_rtdrsTaskDefinition = a})

-- | -- | The response status code.
rtdrsResponseStatus :: Lens' RegisterTaskDefinitionResponse Int
rtdrsResponseStatus =
  lens _rtdrsResponseStatus (\s a -> s {_rtdrsResponseStatus = a})
