...
instance AWSRequest PutAttributes where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutAttributesResponse' <$> (x .?> "attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON PutAttributes where
  toJSON PutAttributes' {..} =
    object
      (catMaybes
         [("cluster" .=) <$> _paCluster, Just ("attributes" .= _paAttributes)])
...
parsAttributes =
  lens _parsAttributes (\s a -> s {_parsAttributes = a}) . _Default . _Coerce
