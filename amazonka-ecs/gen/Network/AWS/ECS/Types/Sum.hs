...
instance FromText AgentUpdateStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AgentUpdateStatus from value: '" <>
        e <>
...
instance FromText AssignPublicIP where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AssignPublicIP from value: '" <>
...
instance FromText ClusterField where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ClusterField from value: '" <>
        e <> "'. Accepted values: statistics"
...
instance FromText Compatibility where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Compatibility from value: '" <>
        e <> "'. Accepted values: ec2, fargate"
...
instance FromText Connectivity where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Connectivity from value: '" <>
        e <> "'. Accepted values: connected, disconnected"
...
instance FromText ContainerInstanceStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ContainerInstanceStatus from value: '" <>
        e <> "'. Accepted values: active, draining"
...
instance FromText DesiredStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DesiredStatus from value: '" <>
        e <> "'. Accepted values: pending, running, stopped"
...
instance FromText DeviceCgroupPermission where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeviceCgroupPermission from value: '" <>
        e <> "'. Accepted values: mknod, read, write"
...
instance FromText HealthStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HealthStatus from value: '" <>
        e <> "'. Accepted values: healthy, unhealthy, unknown"
...
instance FromText LaunchType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LaunchType from value: '" <>
        e <> "'. Accepted values: ec2, fargate"
...
instance FromText LogDriver where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LogDriver from value: '" <>
        e <>
...
instance FromText NetworkMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NetworkMode from value: '" <>
        e <> "'. Accepted values: awsvpc, bridge, host, none"
...
instance FromText PlacementConstraintType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PlacementConstraintType from value: '" <>
        e <> "'. Accepted values: distinctinstance, memberof"
...
instance FromText PlacementStrategyType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PlacementStrategyType from value: '" <>
        e <> "'. Accepted values: binpack, random, spread"
...
instance FromText TargetType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TargetType from value: '" <>
        e <> "'. Accepted values: container-instance"
...
instance FromText TaskDefinitionFamilyStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TaskDefinitionFamilyStatus from value: '" <>
        e <> "'. Accepted values: active, all, inactive"
...
instance FromText TaskDefinitionPlacementConstraintType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TaskDefinitionPlacementConstraintType from value: '" <>
        e <> "'. Accepted values: memberof"
...
instance FromText TaskDefinitionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TaskDefinitionStatus from value: '" <>
...
instance FromText TransportProtocol where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TransportProtocol from value: '" <>
...
instance FromText UlimitName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UlimitName from value: '" <>
        e <>
