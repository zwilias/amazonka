...
data ClusterSetting = ClusterSetting'{_csValue ::
                                      !(Maybe Text),
...
clusterSetting
    :: ClusterSetting
clusterSetting
  = ClusterSetting'{_csValue = Nothing,
                    _csName = Nothing}
...
instance FromJSON ClusterSetting where
        parseJSON
          = withObject "ClusterSetting"
              (\ x ->
                 ClusterSetting' <$>
                   (x .:? "value") <*> (x .:? "name"))

instance Hashable ClusterSetting where

instance NFData ClusterSetting where

instance ToJSON ClusterSetting where
        toJSON ClusterSetting'{..}
          = object
              (catMaybes
                 [("value" .=) <$> _csValue, ("name" .=) <$> _csName])
