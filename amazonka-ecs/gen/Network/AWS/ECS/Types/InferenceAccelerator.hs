...
data InferenceAccelerator = InferenceAccelerator'{_iaDeviceName
                                                  :: !Text,
                                                  _iaDeviceType :: !Text}
...
inferenceAccelerator pDeviceName_ pDeviceType_
  = InferenceAccelerator'{_iaDeviceName = pDeviceName_,
...
instance FromJSON InferenceAccelerator where
        parseJSON
          = withObject "InferenceAccelerator"
              (\ x ->
                 InferenceAccelerator' <$>
                   (x .: "deviceName") <*> (x .: "deviceType"))

instance Hashable InferenceAccelerator where

instance NFData InferenceAccelerator where
