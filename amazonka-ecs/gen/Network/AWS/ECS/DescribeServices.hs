...
instance AWSRequest DescribeServices where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeServicesResponse' <$> (x .?> "failures" .!@ mempty) <*>
         (x .?> "services" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeServices where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerServiceV20141113.DescribeServices" :: ByteString)
...
instance ToJSON DescribeServices where
  toJSON DescribeServices' {..} =
    object
      (catMaybes [("cluster" .=) <$> _dCluster, Just ("services" .= _dServices)])
...
dssrsFailures =
  lens _dssrsFailures (\s a -> s {_dssrsFailures = a}) . _Default . _Coerce

-- | The list of services described.
dssrsServices :: Lens' DescribeServicesResponse [ContainerService]
dssrsServices =
  lens _dssrsServices (\s a -> s {_dssrsServices = a}) . _Default . _Coerce
