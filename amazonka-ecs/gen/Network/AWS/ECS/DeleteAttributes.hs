...
instance AWSRequest DeleteAttributes where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteAttributesResponse' <$> (x .?> "attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders DeleteAttributes where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerServiceV20141113.DeleteAttributes" :: ByteString)
...
instance ToJSON DeleteAttributes where
  toJSON DeleteAttributes' {..} =
    object
      (catMaybes
         [("cluster" .=) <$> _daCluster, Just ("attributes" .= _daAttributes)])
...
darsAttributes =
  lens _darsAttributes (\s a -> s {_darsAttributes = a}) . _Default . _Coerce
