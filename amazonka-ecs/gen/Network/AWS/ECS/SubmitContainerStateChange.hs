...
instance AWSRequest SubmitContainerStateChange where
  ...
  response =
    receiveJSON
      (\s h x ->
         SubmitContainerStateChangeResponse' <$> (x .?> "acknowledgment") <*>
         (pure (fromEnum s)))
...
instance ToHeaders SubmitContainerStateChange where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerServiceV20141113.SubmitContainerStateChange" :: ByteString)
...
scscrsAcknowledgment =
  lens _scscrsAcknowledgment (\s a -> s {_scscrsAcknowledgment = a})

-- | -- | The response status code.
scscrsResponseStatus :: Lens' SubmitContainerStateChangeResponse Int
scscrsResponseStatus =
  lens _scscrsResponseStatus (\s a -> s {_scscrsResponseStatus = a})
