...
derContainerInstance =
  lens _derContainerInstance (\s a -> s {_derContainerInstance = a})
...
instance AWSRequest DeregisterContainerInstance where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeregisterContainerInstanceResponse' <$> (x .?> "containerInstance") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DeregisterContainerInstance where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerServiceV20141113.DeregisterContainerInstance" :: ByteString)
...
dcirsContainerInstance ::
     Lens' DeregisterContainerInstanceResponse (Maybe ContainerInstance)
dcirsContainerInstance =
  lens _dcirsContainerInstance (\s a -> s {_dcirsContainerInstance = a})

-- | -- | The response status code.
dcirsResponseStatus :: Lens' DeregisterContainerInstanceResponse Int
dcirsResponseStatus =
  lens _dcirsResponseStatus (\s a -> s {_dcirsResponseStatus = a})
