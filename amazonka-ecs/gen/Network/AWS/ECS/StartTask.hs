...
sNetworkConfiguration =
  lens _sNetworkConfiguration (\s a -> s {_sNetworkConfiguration = a})
...
sContainerInstances =
  lens _sContainerInstances (\s a -> s {_sContainerInstances = a}) . _Coerce
...
instance AWSRequest StartTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartTaskResponse' <$> (x .?> "failures" .!@ mempty) <*>
         (x .?> "tasks" .!@ mempty) <*>
         (pure (fromEnum s)))
...
strsFailures =
  lens _strsFailures (\s a -> s {_strsFailures = a}) . _Default . _Coerce
...
strsResponseStatus =
  lens _strsResponseStatus (\s a -> s {_strsResponseStatus = a})
