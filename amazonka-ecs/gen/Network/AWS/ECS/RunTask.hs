...
rtPlacementConstraints =
  lens _rtPlacementConstraints (\s a -> s {_rtPlacementConstraints = a}) .
  _Default . _Coerce

-- | The placement strategy objects to use for the task. You can specify a maximum of five strategy rules per task.
rtPlacementStrategy :: Lens' RunTask [PlacementStrategy]
rtPlacementStrategy =
  lens _rtPlacementStrategy (\s a -> s {_rtPlacementStrategy = a}) .
  _Default . _Coerce
...
rtNetworkConfiguration =
  lens _rtNetworkConfiguration (\s a -> s {_rtNetworkConfiguration = a})
...
instance AWSRequest RunTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         RunTaskResponse' <$> (x .?> "failures" .!@ mempty) <*>
         (x .?> "tasks" .!@ mempty) <*>
         (pure (fromEnum s)))
...
rtrsFailures =
  lens _rtrsFailures (\s a -> s {_rtrsFailures = a}) . _Default . _Coerce
