...
instance AWSRequest DiscoverPollEndpoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         DiscoverPollEndpointResponse' <$> (x .?> "telemetryEndpoint") <*>
         (x .?> "endpoint") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DiscoverPollEndpoint where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerServiceV20141113.DiscoverPollEndpoint" :: ByteString)
...
dpersTelemetryEndpoint =
  lens _dpersTelemetryEndpoint (\s a -> s {_dpersTelemetryEndpoint = a})
...
dpersResponseStatus =
  lens _dpersResponseStatus (\s a -> s {_dpersResponseStatus = a})
