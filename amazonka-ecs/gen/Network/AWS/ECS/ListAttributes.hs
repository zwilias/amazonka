...
instance AWSRequest ListAttributes where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAttributesResponse' <$> (x .?> "nextToken") <*>
         (x .?> "attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListAttributes where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerServiceV20141113.ListAttributes" :: ByteString)
...
larsAttributes =
  lens _larsAttributes (\s a -> s {_larsAttributes = a}) . _Default . _Coerce
