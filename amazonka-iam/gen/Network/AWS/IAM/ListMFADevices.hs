...
instance AWSRequest ListMFADevices where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ListMFADevicesResponse' <$> (x .@? "Marker") <*> (x .@? "IsTruncated") <*>
         (pure (fromEnum s)) <*>
...
lmdrsResponseStatus =
  lens _lmdrsResponseStatus (\s a -> s {_lmdrsResponseStatus = a})

-- | A list of MFA devices.
lmdrsMFADevices :: Lens' ListMFADevicesResponse [MFADevice]
lmdrsMFADevices =
  lens _lmdrsMFADevices (\s a -> s {_lmdrsMFADevices = a}) . _Coerce
