...
clpPasswordResetRequired =
  lens _clpPasswordResetRequired (\s a -> s {_clpPasswordResetRequired = a})
...
instance AWSRequest CreateLoginProfile where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateLoginProfileResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "LoginProfile"))
...
clprsResponseStatus =
  lens _clprsResponseStatus (\s a -> s {_clprsResponseStatus = a})
