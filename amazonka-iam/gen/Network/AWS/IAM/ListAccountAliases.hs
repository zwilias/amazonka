...
instance AWSRequest ListAccountAliases where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ListAccountAliasesResponse' <$> (x .@? "Marker") <*>
         (x .@? "IsTruncated") <*>
         (pure (fromEnum s)) <*>
...
laarsResponseStatus =
  lens _laarsResponseStatus (\s a -> s {_laarsResponseStatus = a})

-- | A list of aliases associated with the account. AWS supports only one alias per account.
laarsAccountAliases :: Lens' ListAccountAliasesResponse [Text]
laarsAccountAliases =
  lens _laarsAccountAliases (\s a -> s {_laarsAccountAliases = a}) . _Coerce
