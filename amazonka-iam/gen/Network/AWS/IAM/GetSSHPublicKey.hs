...
gspkSSHPublicKeyId =
  lens _gspkSSHPublicKeyId (\s a -> s {_gspkSSHPublicKeyId = a})
...
instance AWSRequest GetSSHPublicKey where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         GetSSHPublicKeyResponse' <$> (x .@? "SSHPublicKey") <*>
         (pure (fromEnum s)))
...
gspkrsSSHPublicKey =
  lens _gspkrsSSHPublicKey (\s a -> s {_gspkrsSSHPublicKey = a})

-- | -- | The response status code.
gspkrsResponseStatus :: Lens' GetSSHPublicKeyResponse Int
gspkrsResponseStatus =
  lens _gspkrsResponseStatus (\s a -> s {_gspkrsResponseStatus = a})
