...
instance AWSRequest ListAttachedGroupPolicies where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ListAttachedGroupPoliciesResponse' <$>
         (x .@? "AttachedPolicies" .!@ mempty >>= may (parseXMLList "member")) <*>
         (x .@? "Marker") <*>
         (x .@? "IsTruncated") <*>
         (pure (fromEnum s)))
...
lagprsAttachedPolicies ::
     Lens' ListAttachedGroupPoliciesResponse [AttachedPolicy]
lagprsAttachedPolicies =
  lens _lagprsAttachedPolicies (\s a -> s {_lagprsAttachedPolicies = a}) .
  _Default . _Coerce
...
lagprsResponseStatus =
  lens _lagprsResponseStatus (\s a -> s {_lagprsResponseStatus = a})
