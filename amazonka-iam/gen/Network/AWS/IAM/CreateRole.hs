...
crMaxSessionDuration =
  lens _crMaxSessionDuration (\s a -> s {_crMaxSessionDuration = a}) .
  mapping _Nat
...
crAssumeRolePolicyDocument =
  lens _crAssumeRolePolicyDocument (\s a -> s {_crAssumeRolePolicyDocument = a})
...
instance AWSRequest CreateRole where
  ...
  response =
    receiveXMLWrapper
      "CreateRoleResult"
      (\s h x -> CreateRoleResponse' <$> (pure (fromEnum s)) <*> (x .@ "Role"))
