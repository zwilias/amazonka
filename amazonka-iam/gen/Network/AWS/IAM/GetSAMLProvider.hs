...
instance AWSRequest GetSAMLProvider where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         GetSAMLProviderResponse' <$> (x .@? "CreateDate") <*>
         (x .@? "ValidUntil") <*>
...
gsamlprsCreateDate =
  lens _gsamlprsCreateDate (\s a -> s {_gsamlprsCreateDate = a}) . mapping _Time

-- | The expiration date and time for the SAML provider.
gsamlprsValidUntil :: Lens' GetSAMLProviderResponse (Maybe UTCTime)
gsamlprsValidUntil =
  lens _gsamlprsValidUntil (\s a -> s {_gsamlprsValidUntil = a}) . mapping _Time
...
gsamlprsResponseStatus =
  lens _gsamlprsResponseStatus (\s a -> s {_gsamlprsResponseStatus = a})
