...
instance FromText AssignmentStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AssignmentStatusType from value: '" <>
        e <> "'. Accepted values: any, assigned, unassigned"
...
instance FromText ContextKeyTypeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ContextKeyTypeEnum from value: '" <>
        e <>
...
instance FromText DeletionTaskStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeletionTaskStatusType from value: '" <>
        e <> "'. Accepted values: failed, in_progress, not_started, succeeded"
...
instance FromText EntityType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EntityType from value: '" <>
        e <>
...
instance FromText PolicyEvaluationDecisionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PolicyEvaluationDecisionType from value: '" <>
        e <> "'. Accepted values: allowed, explicitdeny, implicitdeny"
...
instance FromText PolicyScopeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PolicyScopeType from value: '" <>
        e <> "'. Accepted values: aws, all, local"
...
instance FromText PolicySourceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PolicySourceType from value: '" <>
        e <>
...
instance FromText ReportFormatType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReportFormatType from value: '" <>
        e <> "'. Accepted values: text/csv"
...
instance FromText ReportStateType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReportStateType from value: '" <>
        e <> "'. Accepted values: complete, inprogress, started"
...
instance FromText SummaryKeyType where
  parser =
    takeLowerText >>= \case
      ...
      "accountsigningcertificatespresent" ->
        pure AccountSigningCertificatesPresent
      ...
      e ->
        fromTextError $
        "Failure parsing SummaryKeyType from value: '" <>
        e <>
