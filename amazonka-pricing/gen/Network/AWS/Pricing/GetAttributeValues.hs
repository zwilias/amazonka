...
gavMaxResults =
  lens _gavMaxResults (\s a -> s {_gavMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetAttributeValues where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetAttributeValuesResponse' <$> (x .?> "AttributeValues" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
gavrsAttributeValues =
  lens _gavrsAttributeValues (\s a -> s {_gavrsAttributeValues = a}) .
  _Default . _Coerce
...
gavrsResponseStatus =
  lens _gavrsResponseStatus (\s a -> s {_gavrsResponseStatus = a})
