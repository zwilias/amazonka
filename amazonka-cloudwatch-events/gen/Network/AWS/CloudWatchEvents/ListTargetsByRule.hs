...
instance AWSRequest ListTargetsByRule where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListTargetsByRuleResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Targets") <*>
         (pure (fromEnum s)))
...
ltbrrsTargets =
  lens _ltbrrsTargets (\s a -> s {_ltbrrsTargets = a}) . mapping _List1

-- | -- | The response status code.
ltbrrsResponseStatus :: Lens' ListTargetsByRuleResponse Int
ltbrrsResponseStatus =
  lens _ltbrrsResponseStatus (\s a -> s {_ltbrrsResponseStatus = a})
