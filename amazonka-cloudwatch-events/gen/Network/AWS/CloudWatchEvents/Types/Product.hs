...
instance FromJSON BatchParameters where
  parseJSON =
    withObject
      "BatchParameters"
      (\x ->
         BatchParameters' <$> (x .:? "RetryStrategy") <*>
         (x .:? "ArrayProperties") <*>
         (x .: "JobDefinition") <*>
         (x .: "JobName"))
...
epTaskDefinitionARN =
  lens _epTaskDefinitionARN (\s a -> s {_epTaskDefinitionARN = a})
...
instance FromJSON EcsParameters where
  parseJSON =
    withObject
      "EcsParameters"
      (\x ->
         EcsParameters' <$> (x .:? "TaskCount") <*> (x .: "TaskDefinitionArn"))
...
itInputPathsMap =
  lens _itInputPathsMap (\s a -> s {_itInputPathsMap = a}) . _Default . _Map
...
instance FromJSON InputTransformer where
  parseJSON =
    withObject
      "InputTransformer"
      (\x ->
         InputTransformer' <$> (x .:? "InputPathsMap" .!= mempty) <*>
         (x .: "InputTemplate"))
...
kpPartitionKeyPath =
  lens _kpPartitionKeyPath (\s a -> s {_kpPartitionKeyPath = a})
...
instance ToJSON KinesisParameters where
  toJSON KinesisParameters' {..} =
    object (catMaybes [Just ("PartitionKeyPath" .= _kpPartitionKeyPath)])
...
pereResources =
  lens _pereResources (\s a -> s {_pereResources = a}) . _Default . _Coerce
...
instance FromJSON PutEventsResultEntry where
  parseJSON =
    withObject
      "PutEventsResultEntry"
      (\x ->
         PutEventsResultEntry' <$> (x .:? "ErrorCode") <*>
         (x .:? "ErrorMessage") <*>
         (x .:? "EventId"))
...
instance FromJSON PutTargetsResultEntry where
  parseJSON =
    withObject
      "PutTargetsResultEntry"
      (\x ->
         PutTargetsResultEntry' <$> (x .:? "TargetId") <*> (x .:? "ErrorCode") <*>
         (x .:? "ErrorMessage"))
...
instance FromJSON RemoveTargetsResultEntry where
  parseJSON =
    withObject
      "RemoveTargetsResultEntry"
      (\x ->
         RemoveTargetsResultEntry' <$> (x .:? "TargetId") <*>
         (x .:? "ErrorCode") <*>
         (x .:? "ErrorMessage"))
...
rScheduleExpression =
  lens _rScheduleExpression (\s a -> s {_rScheduleExpression = a})
...
instance FromJSON Rule where
  parseJSON =
    withObject
      "Rule"
      (\x ->
         Rule' <$> (x .:? "EventPattern") <*> (x .:? "State") <*> (x .:? "Arn") <*>
...
rcpRunCommandTargets =
  lens _rcpRunCommandTargets (\s a -> s {_rcpRunCommandTargets = a}) . _List1
...
instance ToJSON RunCommandParameters where
  toJSON RunCommandParameters' {..} =
    object (catMaybes [Just ("RunCommandTargets" .= _rcpRunCommandTargets)])
...
instance ToJSON RunCommandTarget where
  toJSON RunCommandTarget' {..} =
    object (catMaybes [Just ("Key" .= _rctKey), Just ("Values" .= _rctValues)])
...
tRunCommandParameters =
  lens _tRunCommandParameters (\s a -> s {_tRunCommandParameters = a})
...
tKinesisParameters =
  lens _tKinesisParameters (\s a -> s {_tKinesisParameters = a})
