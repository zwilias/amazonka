...
instance AWSRequest DescribeRule where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeRuleResponse' <$> (x .?> "EventPattern") <*> (x .?> "State") <*>
         (x .?> "Arn") <*>
...
instance ToJSON DescribeRule where
  toJSON DescribeRule' {..} = object (catMaybes [Just ("Name" .= _desName)])
...
drrsScheduleExpression =
  lens _drrsScheduleExpression (\s a -> s {_drrsScheduleExpression = a})
