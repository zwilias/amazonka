...
instance AWSRequest ListRuleNamesByTarget where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListRuleNamesByTargetResponse' <$> (x .?> "RuleNames" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListRuleNamesByTarget where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSEvents.ListRuleNamesByTarget" :: ByteString)
...
lrnbtrsRuleNames =
  lens _lrnbtrsRuleNames (\s a -> s {_lrnbtrsRuleNames = a}) .
  _Default . _Coerce
...
lrnbtrsResponseStatus =
  lens _lrnbtrsResponseStatus (\s a -> s {_lrnbtrsResponseStatus = a})
