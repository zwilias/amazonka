...
instance AWSRequest GetRateBasedRuleManagedKeys where
  type Rs GetRateBasedRuleManagedKeys = GetRateBasedRuleManagedKeysResponse
  request = postJSON wAFRegional
  response =
    receiveJSON
      (\s h x ->
         GetRateBasedRuleManagedKeysResponse' <$> (x .?> "NextMarker") <*>
...
instance ToHeaders GetRateBasedRuleManagedKeys where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSWAF_Regional_20161128.GetRateBasedRuleManagedKeys" :: ByteString)
...
grbrmkrsNextMarker =
  lens _grbrmkrsNextMarker (\s a -> s {_grbrmkrsNextMarker = a})

-- | An array of IP addresses that currently are blocked by the specified 'RateBasedRule' .
grbrmkrsManagedKeys :: Lens' GetRateBasedRuleManagedKeysResponse [Text]
grbrmkrsManagedKeys =
  lens _grbrmkrsManagedKeys (\s a -> s {_grbrmkrsManagedKeys = a}) .
  _Default . _Coerce

-- | -- | The response status code.
grbrmkrsResponseStatus :: Lens' GetRateBasedRuleManagedKeysResponse Int
grbrmkrsResponseStatus =
  lens _grbrmkrsResponseStatus (\s a -> s {_grbrmkrsResponseStatus = a})
