...
instance AWSRequest ListSqlInjectionMatchSets where
  type Rs ListSqlInjectionMatchSets = ListSqlInjectionMatchSetsResponse
  request = postJSON wAFRegional
  response =
    receiveJSON
      (\s h x ->
         ListSqlInjectionMatchSetsResponse' <$> (x .?> "NextMarker") <*>
...
instance ToHeaders ListSqlInjectionMatchSets where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSWAF_Regional_20161128.ListSqlInjectionMatchSets" :: ByteString)
...
instance ToJSON ListSqlInjectionMatchSets where
  toJSON ListSqlInjectionMatchSets' {..} =
    object
      (catMaybes
         [("NextMarker" .=) <$> _lsimsNextMarker, ("Limit" .=) <$> _lsimsLimit])
...
lsimsrsSqlInjectionMatchSets ::
     Lens' ListSqlInjectionMatchSetsResponse [SqlInjectionMatchSetSummary]
lsimsrsSqlInjectionMatchSets =
  lens
    _lsimsrsSqlInjectionMatchSets
    (\s a -> s {_lsimsrsSqlInjectionMatchSets = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lsimsrsResponseStatus :: Lens' ListSqlInjectionMatchSetsResponse Int
lsimsrsResponseStatus =
  lens _lsimsrsResponseStatus (\s a -> s {_lsimsrsResponseStatus = a})
