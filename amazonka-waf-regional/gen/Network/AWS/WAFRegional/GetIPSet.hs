...
instance AWSRequest GetIPSet where
  ...
  response =
    receiveJSON
      (\s h x -> GetIPSetResponse' <$> (x .?> "IPSet") <*> (pure (fromEnum s)))
...
instance ToHeaders GetIPSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_Regional_20161128.GetIPSet" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON GetIPSet where
  toJSON GetIPSet' {..} = object (catMaybes [Just ("IPSetId" .= _gisIPSetId)])
