...
grpsRegexPatternSetId =
  lens _grpsRegexPatternSetId (\s a -> s {_grpsRegexPatternSetId = a})
...
instance AWSRequest GetRegexPatternSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetRegexPatternSetResponse' <$> (x .?> "RegexPatternSet") <*>
         (pure (fromEnum s)))
...
instance ToJSON GetRegexPatternSet where
  toJSON GetRegexPatternSet' {..} =
    object (catMaybes [Just ("RegexPatternSetId" .= _grpsRegexPatternSetId)])
...
grpsrsRegexPatternSet ::
     Lens' GetRegexPatternSetResponse (Maybe RegexPatternSet)
grpsrsRegexPatternSet =
  lens _grpsrsRegexPatternSet (\s a -> s {_grpsrsRegexPatternSet = a})

-- | -- | The response status code.
grpsrsResponseStatus :: Lens' GetRegexPatternSetResponse Int
grpsrsResponseStatus =
  lens _grpsrsResponseStatus (\s a -> s {_grpsrsResponseStatus = a})
