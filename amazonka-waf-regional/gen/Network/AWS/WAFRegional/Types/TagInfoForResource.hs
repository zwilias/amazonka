...
data TagInfoForResource = TagInfoForResource'{_tifrTagList
                                              :: !(Maybe (List1 Tag)),
...
tagInfoForResource
    :: TagInfoForResource
tagInfoForResource
  = TagInfoForResource'{_tifrTagList = Nothing,
...
instance FromJSON TagInfoForResource where
        parseJSON
          = withObject "TagInfoForResource"
              (\ x ->
                 TagInfoForResource' <$>
                   (x .:? "TagList") <*> (x .:? "ResourceARN"))

instance Hashable TagInfoForResource where

instance NFData TagInfoForResource where
