...
instance AWSRequest GetRule where
  ...
  response =
    receiveJSON
      (\s h x -> GetRuleResponse' <$> (x .?> "Rule") <*> (pure (fromEnum s)))
...
instance ToHeaders GetRule where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_Regional_20161128.GetRule" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON GetRule where
  toJSON GetRule' {..} = object (catMaybes [Just ("RuleId" .= _grRuleId)])
