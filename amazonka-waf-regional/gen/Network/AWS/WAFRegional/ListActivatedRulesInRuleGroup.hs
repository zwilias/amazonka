...
instance AWSRequest ListActivatedRulesInRuleGroup where
  type Rs ListActivatedRulesInRuleGroup = ListActivatedRulesInRuleGroupResponse
  request = postJSON wAFRegional
  response =
    receiveJSON
      (\s h x ->
         ListActivatedRulesInRuleGroupResponse' <$> (x .?> "NextMarker") <*>
...
instance ToHeaders ListActivatedRulesInRuleGroup where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSWAF_Regional_20161128.ListActivatedRulesInRuleGroup" :: ByteString)
...
larirgrsNextMarker =
  lens _larirgrsNextMarker (\s a -> s {_larirgrsNextMarker = a})

-- | An array of @ActivatedRules@ objects.
larirgrsActivatedRules ::
     Lens' ListActivatedRulesInRuleGroupResponse [ActivatedRule]
larirgrsActivatedRules =
  lens _larirgrsActivatedRules (\s a -> s {_larirgrsActivatedRules = a}) .
  _Default . _Coerce

-- | -- | The response status code.
larirgrsResponseStatus :: Lens' ListActivatedRulesInRuleGroupResponse Int
larirgrsResponseStatus =
  lens _larirgrsResponseStatus (\s a -> s {_larirgrsResponseStatus = a})
