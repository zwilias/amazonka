...
instance AWSRequest ListSubscribedRuleGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListSubscribedRuleGroupsResponse' <$> (x .?> "RuleGroups" .!@ mempty) <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListSubscribedRuleGroups where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSWAF_Regional_20161128.ListSubscribedRuleGroups" :: ByteString)
...
instance ToJSON ListSubscribedRuleGroups where
  toJSON ListSubscribedRuleGroups' {..} =
    object
      (catMaybes
         [("NextMarker" .=) <$> _lsrgNextMarker, ("Limit" .=) <$> _lsrgLimit])
...
lsrgrsRuleGroups ::
     Lens' ListSubscribedRuleGroupsResponse [SubscribedRuleGroupSummary]
lsrgrsRuleGroups =
  lens _lsrgrsRuleGroups (\s a -> s {_lsrgrsRuleGroups = a}) .
  _Default . _Coerce
...
lsrgrsResponseStatus =
  lens _lsrgrsResponseStatus (\s a -> s {_lsrgrsResponseStatus = a})
