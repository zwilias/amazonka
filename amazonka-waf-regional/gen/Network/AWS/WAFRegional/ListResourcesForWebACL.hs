...
instance AWSRequest ListResourcesForWebACL where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListResourcesForWebACLResponse' <$> (x .?> "ResourceArns" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON ListResourcesForWebACL where
  toJSON ListResourcesForWebACL' {..} =
    object (catMaybes [Just ("WebACLId" .= _lrfwaWebACLId)])
...
lrfwarsResourceARNs =
  lens _lrfwarsResourceARNs (\s a -> s {_lrfwarsResourceARNs = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lrfwarsResponseStatus :: Lens' ListResourcesForWebACLResponse Int
lrfwarsResponseStatus =
  lens _lrfwarsResponseStatus (\s a -> s {_lrfwarsResponseStatus = a})
