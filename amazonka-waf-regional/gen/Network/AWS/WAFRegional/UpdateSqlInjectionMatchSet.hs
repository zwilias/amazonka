...
usimsSqlInjectionMatchSetId =
  lens
    ...
    (\s a -> s {_usimsSqlInjectionMatchSetId = a})
...
usimsUpdates ::
     Lens' UpdateSqlInjectionMatchSet (NonEmpty SqlInjectionMatchSetUpdate)
usimsUpdates = lens _usimsUpdates (\s a -> s {_usimsUpdates = a}) . _List1

instance AWSRequest UpdateSqlInjectionMatchSet where
  type Rs UpdateSqlInjectionMatchSet = UpdateSqlInjectionMatchSetResponse
  request = postJSON wAFRegional
  response =
    receiveJSON
      (\s h x ->
         UpdateSqlInjectionMatchSetResponse' <$> (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateSqlInjectionMatchSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSWAF_Regional_20161128.UpdateSqlInjectionMatchSet" :: ByteString)
...
usimsrsChangeToken =
  lens _usimsrsChangeToken (\s a -> s {_usimsrsChangeToken = a})

-- | -- | The response status code.
usimsrsResponseStatus :: Lens' UpdateSqlInjectionMatchSetResponse Int
usimsrsResponseStatus =
  lens _usimsrsResponseStatus (\s a -> s {_usimsrsResponseStatus = a})
