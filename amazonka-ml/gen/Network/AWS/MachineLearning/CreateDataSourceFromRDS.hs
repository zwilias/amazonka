...
cdsfrdsComputeStatistics =
  lens _cdsfrdsComputeStatistics (\s a -> s {_cdsfrdsComputeStatistics = a})
...
instance AWSRequest CreateDataSourceFromRDS where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateDataSourceFromRDSResponse' <$> (x .?> "DataSourceId") <*>
         (pure (fromEnum s)))
...
cdsfrdsrsDataSourceId =
  lens _cdsfrdsrsDataSourceId (\s a -> s {_cdsfrdsrsDataSourceId = a})

-- | -- | The response status code.
cdsfrdsrsResponseStatus :: Lens' CreateDataSourceFromRDSResponse Int
cdsfrdsrsResponseStatus =
  lens _cdsfrdsrsResponseStatus (\s a -> s {_cdsfrdsrsResponseStatus = a})
