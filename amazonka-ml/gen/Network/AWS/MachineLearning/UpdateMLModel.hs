...
instance AWSRequest UpdateMLModel where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateMLModelResponse' <$> (x .?> "MLModelId") <*> (pure (fromEnum s)))
...
instance ToHeaders UpdateMLModel where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonML_20141212.UpdateMLModel" :: ByteString)
...
umlmrsResponseStatus =
  lens _umlmrsResponseStatus (\s a -> s {_umlmrsResponseStatus = a})
