...
instance AWSRequest DescribeDataSources where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeDataSourcesResponse' <$> (x .?> "Results" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
ddssrsResults =
  lens _ddssrsResults (\s a -> s {_ddssrsResults = a}) . _Default . _Coerce
...
ddssrsResponseStatus =
  lens _ddssrsResponseStatus (\s a -> s {_ddssrsResponseStatus = a})
