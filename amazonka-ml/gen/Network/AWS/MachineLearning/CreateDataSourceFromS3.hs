...
instance AWSRequest CreateDataSourceFromS3 where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateDataSourceFromS3Response' <$> (x .?> "DataSourceId") <*>
         (pure (fromEnum s)))
...
cdsfsrsDataSourceId =
  lens _cdsfsrsDataSourceId (\s a -> s {_cdsfsrsDataSourceId = a})

-- | -- | The response status code.
cdsfsrsResponseStatus :: Lens' CreateDataSourceFromS3Response Int
cdsfsrsResponseStatus =
  lens _cdsfsrsResponseStatus (\s a -> s {_cdsfsrsResponseStatus = a})
