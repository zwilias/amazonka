...
instance AWSRequest GetEvaluation where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetEvaluationResponse' <$> (x .?> "Status") <*>
...
instance ToHeaders GetEvaluation where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonML_20141212.GetEvaluation" :: ByteString)
...
gersPerformanceMetrics =
  lens _gersPerformanceMetrics (\s a -> s {_gersPerformanceMetrics = a})

-- | The time of the most recent edit to the @Evaluation@ . The time is expressed in epoch time.
gersLastUpdatedAt :: Lens' GetEvaluationResponse (Maybe UTCTime)
gersLastUpdatedAt =
  lens _gersLastUpdatedAt (\s a -> s {_gersLastUpdatedAt = a}) . mapping _Time

-- | The time that the @Evaluation@ was created. The time is expressed in epoch time.
gersCreatedAt :: Lens' GetEvaluationResponse (Maybe UTCTime)
gersCreatedAt =
  lens _gersCreatedAt (\s a -> s {_gersCreatedAt = a}) . mapping _Time
...
gersInputDataLocationS3 =
  lens _gersInputDataLocationS3 (\s a -> s {_gersInputDataLocationS3 = a})
...
gersEvaluationDataSourceId =
  lens _gersEvaluationDataSourceId (\s a -> s {_gersEvaluationDataSourceId = a})
