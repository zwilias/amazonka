...
cmlmParameters =
  lens _cmlmParameters (\s a -> s {_cmlmParameters = a}) . _Default . _Map
...
cmlmTrainingDataSourceId =
  lens _cmlmTrainingDataSourceId (\s a -> s {_cmlmTrainingDataSourceId = a})
...
instance AWSRequest CreateMLModel where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateMLModelResponse' <$> (x .?> "MLModelId") <*> (pure (fromEnum s)))
...
instance ToHeaders CreateMLModel where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonML_20141212.CreateMLModel" :: ByteString)
