...
instance AWSRequest DescribeMLModels where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeMLModelsResponse' <$> (x .?> "Results" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
dmlmsrsResults =
  lens _dmlmsrsResults (\s a -> s {_dmlmsrsResults = a}) . _Default . _Coerce
...
dmlmsrsResponseStatus =
  lens _dmlmsrsResponseStatus (\s a -> s {_dmlmsrsResponseStatus = a})
