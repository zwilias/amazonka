...
instance AWSRequest GetBatchPrediction where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBatchPredictionResponse' <$> (x .?> "Status") <*>
         (x .?> "LastUpdatedAt") <*>
         (x .?> "CreatedAt") <*>
         (x .?> "ComputeTime") <*>
...
instance ToJSON GetBatchPrediction where
  toJSON GetBatchPrediction' {..} =
    object (catMaybes [Just ("BatchPredictionId" .= _gbpBatchPredictionId)])
...
gbprsLastUpdatedAt =
  lens _gbprsLastUpdatedAt (\s a -> s {_gbprsLastUpdatedAt = a}) . mapping _Time

-- | The time when the @BatchPrediction@ was created. The time is expressed in epoch time.
gbprsCreatedAt :: Lens' GetBatchPredictionResponse (Maybe UTCTime)
gbprsCreatedAt =
  lens _gbprsCreatedAt (\s a -> s {_gbprsCreatedAt = a}) . mapping _Time
...
gbprsInputDataLocationS3 =
  lens _gbprsInputDataLocationS3 (\s a -> s {_gbprsInputDataLocationS3 = a})
