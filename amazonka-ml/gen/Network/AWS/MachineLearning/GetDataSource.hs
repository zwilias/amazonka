...
instance AWSRequest GetDataSource where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDataSourceResponse' <$> (x .?> "Status") <*> (x .?> "NumberOfFiles") <*>
         (x .?> "LastUpdatedAt") <*>
         (x .?> "CreatedAt") <*>
         (x .?> "ComputeTime") <*>
         (x .?> "DataSourceId") <*>
...
instance ToHeaders GetDataSource where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonML_20141212.GetDataSource" :: ByteString)
...
gdsrsNumberOfFiles =
  lens _gdsrsNumberOfFiles (\s a -> s {_gdsrsNumberOfFiles = a})

-- | The time of the most recent edit to the @DataSource@ . The time is expressed in epoch time.
gdsrsLastUpdatedAt :: Lens' GetDataSourceResponse (Maybe UTCTime)
gdsrsLastUpdatedAt =
  lens _gdsrsLastUpdatedAt (\s a -> s {_gdsrsLastUpdatedAt = a}) . mapping _Time

-- | The time that the @DataSource@ was created. The time is expressed in epoch time.
gdsrsCreatedAt :: Lens' GetDataSourceResponse (Maybe UTCTime)
gdsrsCreatedAt =
  lens _gdsrsCreatedAt (\s a -> s {_gdsrsCreatedAt = a}) . mapping _Time
...
gdsrsDataLocationS3 =
  lens _gdsrsDataLocationS3 (\s a -> s {_gdsrsDataLocationS3 = a})

-- | The parameter is @true@ if statistics need to be generated from the observation data.
gdsrsComputeStatistics :: Lens' GetDataSourceResponse (Maybe Bool)
gdsrsComputeStatistics =
  lens _gdsrsComputeStatistics (\s a -> s {_gdsrsComputeStatistics = a})
...
gdsrsRedshiftMetadata =
  lens _gdsrsRedshiftMetadata (\s a -> s {_gdsrsRedshiftMetadata = a})

-- | A JSON string that represents the splitting and rearrangement requirement used when this @DataSource@ was created.
gdsrsDataRearrangement :: Lens' GetDataSourceResponse (Maybe Text)
gdsrsDataRearrangement =
  lens _gdsrsDataRearrangement (\s a -> s {_gdsrsDataRearrangement = a})
