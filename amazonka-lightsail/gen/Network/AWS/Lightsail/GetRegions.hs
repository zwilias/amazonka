...
grIncludeAvailabilityZones =
  lens _grIncludeAvailabilityZones (\s a -> s {_grIncludeAvailabilityZones = a})
...
instance AWSRequest GetRegions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetRegionsResponse' <$> (x .?> "regions" .!@ mempty) <*>
         (pure (fromEnum s)))
...
grrsRegions =
  lens _grrsRegions (\s a -> s {_grrsRegions = a}) . _Default . _Coerce
