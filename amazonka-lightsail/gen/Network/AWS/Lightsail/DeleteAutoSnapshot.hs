...
data DeleteAutoSnapshot = DeleteAutoSnapshot'{_dasResourceName
                                              :: !Text,
                                              _dasDate :: !Text}
...
deleteAutoSnapshot
    :: Text -- ^ 'dasResourceName'
    -> Text -- ^ 'dasDate'
    -> DeleteAutoSnapshot
deleteAutoSnapshot pResourceName_ pDate_
  = DeleteAutoSnapshot'{_dasResourceName =
                          pResourceName_,
                        _dasDate = pDate_}
...
instance Hashable DeleteAutoSnapshot where

instance NFData DeleteAutoSnapshot where
...
data DeleteAutoSnapshotResponse = DeleteAutoSnapshotResponse'{_dasrsOperations
                                                              ::
                                                              !(Maybe
                                                                  [Operation]),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'dasrsOperations' - An array of objects that describe the result of the action, such as the status of the request, the time stamp of the request, and the resources affected by the request.
--
...
deleteAutoSnapshotResponse pResponseStatus_
  = DeleteAutoSnapshotResponse'{_dasrsOperations =
                                  Nothing,
...
instance NFData DeleteAutoSnapshotResponse where
