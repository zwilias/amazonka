...
data GetAlarms = GetAlarms'{_gaAlarmName ::
                            !(Maybe Text),
...
getAlarms
    :: GetAlarms
getAlarms
  = GetAlarms'{_gaAlarmName = Nothing,
...
instance Hashable GetAlarms where

instance NFData GetAlarms where
...
data GetAlarmsResponse = GetAlarmsResponse'{_garsNextPageToken
                                            :: !(Maybe Text),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'garsNextPageToken' - The token to advance to the next page of resutls from your request. A next page token is not returned if there are no more results to display. To get the next page of results, perform another @GetAlarms@ request and specify the next page token using the @pageToken@ parameter.
--
...
getAlarmsResponse pResponseStatus_
  = GetAlarmsResponse'{_garsNextPageToken = Nothing,
                       _garsAlarms = Nothing,
...
instance NFData GetAlarmsResponse where
