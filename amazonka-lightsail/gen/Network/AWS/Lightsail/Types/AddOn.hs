...
data AddOn = AddOn'{_aoStatus :: !(Maybe Text),
...
addOn
    :: AddOn
addOn
  = AddOn'{_aoStatus = Nothing,
           _aoNextSnapshotTimeOfDay = Nothing,
           _aoSnapshotTimeOfDay = Nothing, _aoName = Nothing}
...
instance FromJSON AddOn where
        parseJSON
          = withObject "AddOn"
              (\ x ->
                 AddOn' <$>
                   (x .:? "status") <*> (x .:? "nextSnapshotTimeOfDay")
                     <*> (x .:? "snapshotTimeOfDay")
                     <*> (x .:? "name"))

instance Hashable AddOn where

instance NFData AddOn where
