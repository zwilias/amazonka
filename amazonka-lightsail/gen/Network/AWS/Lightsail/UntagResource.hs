...
untagResource pResourceName_
  = UntagResource'{_urResourceARN = Nothing,
...
data UntagResourceResponse = UntagResourceResponse'{_urrsOperations
                                                    :: !(Maybe [Operation]),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'urrsOperations' - An array of objects that describe the result of the action, such as the status of the request, the time stamp of the request, and the resources affected by the request.
--
...
untagResourceResponse pResponseStatus_
  = UntagResourceResponse'{_urrsOperations = Nothing,
