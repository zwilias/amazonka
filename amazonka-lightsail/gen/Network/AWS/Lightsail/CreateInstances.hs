...
ciInstanceNames =
  lens _ciInstanceNames (\s a -> s {_ciInstanceNames = a}) . _Coerce
...
instance AWSRequest CreateInstances where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateInstancesResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
cirsOperations =
  lens _cirsOperations (\s a -> s {_cirsOperations = a}) . _Default . _Coerce
