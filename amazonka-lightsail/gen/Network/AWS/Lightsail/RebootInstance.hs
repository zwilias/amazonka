...
instance AWSRequest RebootInstance where
  ...
  response =
    receiveJSON
      (\s h x ->
         RebootInstanceResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders RebootInstance where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Lightsail_20161128.RebootInstance" :: ByteString)
...
rirsOperations =
  lens _rirsOperations (\s a -> s {_rirsOperations = a}) . _Default . _Coerce
