...
gisInstanceSnapshotName =
  lens _gisInstanceSnapshotName (\s a -> s {_gisInstanceSnapshotName = a})
...
instance AWSRequest GetInstanceSnapshot where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetInstanceSnapshotResponse' <$> (x .?> "instanceSnapshot") <*>
         (pure (fromEnum s)))
...
instance ToJSON GetInstanceSnapshot where
  toJSON GetInstanceSnapshot' {..} =
    object
      (catMaybes [Just ("instanceSnapshotName" .= _gisInstanceSnapshotName)])
...
gisrsInstanceSnapshot ::
     Lens' GetInstanceSnapshotResponse (Maybe InstanceSnapshot)
gisrsInstanceSnapshot =
  lens _gisrsInstanceSnapshot (\s a -> s {_gisrsInstanceSnapshot = a})

-- | -- | The response status code.
gisrsResponseStatus :: Lens' GetInstanceSnapshotResponse Int
gisrsResponseStatus =
  lens _gisrsResponseStatus (\s a -> s {_gisrsResponseStatus = a})
