...
instance AWSRequest DeleteLoadBalancer where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteLoadBalancerResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteLoadBalancer where
  toJSON DeleteLoadBalancer' {..} =
    object (catMaybes [Just ("loadBalancerName" .= _dlbLoadBalancerName)])
...
dlbrsOperations =
  lens _dlbrsOperations (\s a -> s {_dlbrsOperations = a}) . _Default . _Coerce

-- | -- | The response status code.
dlbrsResponseStatus :: Lens' DeleteLoadBalancerResponse Int
dlbrsResponseStatus =
  lens _dlbrsResponseStatus (\s a -> s {_dlbrsResponseStatus = a})
