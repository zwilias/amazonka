...
instance AWSRequest DeleteDisk where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteDiskResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteDisk where
  toJSON DeleteDisk' {..} = object (catMaybes [Just ("diskName" .= _dDiskName)])
...
drsOperations =
  lens _drsOperations (\s a -> s {_drsOperations = a}) . _Default . _Coerce
