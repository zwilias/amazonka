...
instance AWSRequest GetBlueprints where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBlueprintsResponse' <$> (x .?> "blueprints" .!@ mempty) <*>
         (x .?> "nextPageToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetBlueprints where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Lightsail_20161128.GetBlueprints" :: ByteString)
...
gbsrsBlueprints =
  lens _gbsrsBlueprints (\s a -> s {_gbsrsBlueprints = a}) . _Default . _Coerce

-- | A token used for advancing to the next page of results from your get blueprints request.
gbsrsNextPageToken :: Lens' GetBlueprintsResponse (Maybe Text)
gbsrsNextPageToken =
  lens _gbsrsNextPageToken (\s a -> s {_gbsrsNextPageToken = a})

-- | -- | The response status code.
gbsrsResponseStatus :: Lens' GetBlueprintsResponse Int
gbsrsResponseStatus =
  lens _gbsrsResponseStatus (\s a -> s {_gbsrsResponseStatus = a})
