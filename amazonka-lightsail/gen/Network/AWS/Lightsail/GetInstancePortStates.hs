...
instance AWSRequest GetInstancePortStates where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetInstancePortStatesResponse' <$> (x .?> "portStates" .!@ mempty) <*>
         (pure (fromEnum s)))
...
gipsrsPortStates =
  lens _gipsrsPortStates (\s a -> s {_gipsrsPortStates = a}) .
  _Default . _Coerce

-- | -- | The response status code.
gipsrsResponseStatus :: Lens' GetInstancePortStatesResponse Int
gipsrsResponseStatus =
  lens _gipsrsResponseStatus (\s a -> s {_gipsrsResponseStatus = a})
