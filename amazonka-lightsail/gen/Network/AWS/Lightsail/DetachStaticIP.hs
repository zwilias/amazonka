...
instance AWSRequest DetachStaticIP where
  ...
  response =
    receiveJSON
      (\s h x ->
         DetachStaticIPResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders DetachStaticIP where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Lightsail_20161128.DetachStaticIp" :: ByteString)
...
dsirsOperations =
  lens _dsirsOperations (\s a -> s {_dsirsOperations = a}) . _Default . _Coerce
