...
newtype DeleteAlarm = DeleteAlarm'{_daAlarmName ::
                                   Text}
...
instance Hashable DeleteAlarm where

instance NFData DeleteAlarm where
...
data DeleteAlarmResponse = DeleteAlarmResponse'{_darsOperations
                                                :: !(Maybe [Operation]),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'darsOperations' - An array of objects that describe the result of the action, such as the status of the request, the time stamp of the request, and the resources affected by the request.
--
...
deleteAlarmResponse pResponseStatus_
  = DeleteAlarmResponse'{_darsOperations = Nothing,
...
instance NFData DeleteAlarmResponse where
