...
instance AWSRequest AllocateStaticIP where
  ...
  response =
    receiveJSON
      (\s h x ->
         AllocateStaticIPResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
asirsOperations =
  lens _asirsOperations (\s a -> s {_asirsOperations = a}) . _Default . _Coerce

-- | -- | The response status code.
asirsResponseStatus :: Lens' AllocateStaticIPResponse Int
asirsResponseStatus =
  lens _asirsResponseStatus (\s a -> s {_asirsResponseStatus = a})
