...
glbLoadBalancerName =
  lens _glbLoadBalancerName (\s a -> s {_glbLoadBalancerName = a})
...
instance AWSRequest GetLoadBalancer where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetLoadBalancerResponse' <$> (x .?> "loadBalancer") <*>
         (pure (fromEnum s)))
...
instance ToJSON GetLoadBalancer where
  toJSON GetLoadBalancer' {..} =
    object (catMaybes [Just ("loadBalancerName" .= _glbLoadBalancerName)])
...
glbrsResponseStatus =
  lens _glbrsResponseStatus (\s a -> s {_glbrsResponseStatus = a})
