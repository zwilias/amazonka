...
data TestAlarm = TestAlarm'{_taAlarmName :: !Text,
                            _taState :: !AlarmState}
...
testAlarm pAlarmName_ pState_
  = TestAlarm'{_taAlarmName = pAlarmName_,
               _taState = pState_}
...
instance Hashable TestAlarm where

instance NFData TestAlarm where
...
data TestAlarmResponse = TestAlarmResponse'{_tarsOperations
                                            :: !(Maybe [Operation]),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'tarsOperations' - An array of objects that describe the result of the action, such as the status of the request, the time stamp of the request, and the resources affected by the request.
--
...
testAlarmResponse pResponseStatus_
  = TestAlarmResponse'{_tarsOperations = Nothing,
...
instance NFData TestAlarmResponse where
