...
disInstanceSnapshotName =
  lens _disInstanceSnapshotName (\s a -> s {_disInstanceSnapshotName = a})
...
instance AWSRequest DeleteInstanceSnapshot where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteInstanceSnapshotResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteInstanceSnapshot where
  toJSON DeleteInstanceSnapshot' {..} =
    object
      (catMaybes [Just ("instanceSnapshotName" .= _disInstanceSnapshotName)])
...
disrsOperations =
  lens _disrsOperations (\s a -> s {_disrsOperations = a}) . _Default . _Coerce
