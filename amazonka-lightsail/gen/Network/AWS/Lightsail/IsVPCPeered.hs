...
instance AWSRequest IsVPCPeered where
  ...
  response =
    receiveJSON
      (\s h x ->
         IsVPCPeeredResponse' <$> (x .?> "isPeered") <*> (pure (fromEnum s)))
...
instance ToHeaders IsVPCPeered where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Lightsail_20161128.IsVpcPeered" :: ByteString)
...
ivprsResponseStatus =
  lens _ivprsResponseStatus (\s a -> s {_ivprsResponseStatus = a})
