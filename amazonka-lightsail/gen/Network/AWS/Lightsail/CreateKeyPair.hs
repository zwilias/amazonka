...
instance AWSRequest CreateKeyPair where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateKeyPairResponse' <$> (x .?> "keyPair") <*> (x .?> "operation") <*>
         (x .?> "publicKeyBase64") <*>
         (x .?> "privateKeyBase64") <*>
         (pure (fromEnum s)))
...
instance ToHeaders CreateKeyPair where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Lightsail_20161128.CreateKeyPair" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON CreateKeyPair where
  toJSON CreateKeyPair' {..} =
    object (catMaybes [Just ("keyPairName" .= _ckpKeyPairName)])
...
ckprsPublicKeyBase64 =
  lens _ckprsPublicKeyBase64 (\s a -> s {_ckprsPublicKeyBase64 = a})

-- | A base64-encoded RSA private key.
ckprsPrivateKeyBase64 :: Lens' CreateKeyPairResponse (Maybe Text)
ckprsPrivateKeyBase64 =
  lens _ckprsPrivateKeyBase64 (\s a -> s {_ckprsPrivateKeyBase64 = a})
