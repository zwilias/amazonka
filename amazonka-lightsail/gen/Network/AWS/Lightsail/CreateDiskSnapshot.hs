...
cdsDiskSnapshotName =
  lens _cdsDiskSnapshotName (\s a -> s {_cdsDiskSnapshotName = a})
...
instance AWSRequest CreateDiskSnapshot where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateDiskSnapshotResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
cdsrsOperations =
  lens _cdsrsOperations (\s a -> s {_cdsrsOperations = a}) . _Default . _Coerce
