...
instance AWSPager GetDomains where
  page rq rs
    ...
    | otherwise = Just $ rq & gdPageToken .~ rs ^. ggrsNextPageToken
...
instance AWSRequest GetDomains where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDomainsResponse' <$> (x .?> "nextPageToken") <*>
         (x .?> "domains" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON GetDomains where
  toJSON GetDomains' {..} =
    object (catMaybes [("pageToken" .=) <$> _gdPageToken])
...
ggrsDomains =
  lens _ggrsDomains (\s a -> s {_ggrsDomains = a}) . _Default . _Coerce
