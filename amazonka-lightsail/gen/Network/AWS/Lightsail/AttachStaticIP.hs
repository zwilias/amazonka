...
instance AWSRequest AttachStaticIP where
  ...
  response =
    receiveJSON
      (\s h x ->
         AttachStaticIPResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders AttachStaticIP where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Lightsail_20161128.AttachStaticIp" :: ByteString)
...
asiprsOperations =
  lens _asiprsOperations (\s a -> s {_asiprsOperations = a}) .
  _Default . _Coerce

-- | -- | The response status code.
asiprsResponseStatus :: Lens' AttachStaticIPResponse Int
asiprsResponseStatus =
  lens _asiprsResponseStatus (\s a -> s {_asiprsResponseStatus = a})
