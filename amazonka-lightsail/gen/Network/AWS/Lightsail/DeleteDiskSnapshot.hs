...
ddsDiskSnapshotName =
  lens _ddsDiskSnapshotName (\s a -> s {_ddsDiskSnapshotName = a})
...
instance AWSRequest DeleteDiskSnapshot where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteDiskSnapshotResponse' <$> (x .?> "operations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteDiskSnapshot where
  toJSON DeleteDiskSnapshot' {..} =
    object (catMaybes [Just ("diskSnapshotName" .= _ddsDiskSnapshotName)])
...
ddsrsOperations =
  lens _ddsrsOperations (\s a -> s {_ddsrsOperations = a}) . _Default . _Coerce
