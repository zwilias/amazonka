...
data EnableAddOn = EnableAddOn'{_eaoResourceName ::
                                !Text,
...
enableAddOn
    :: Text -- ^ 'eaoResourceName'
    -> AddOnRequest -- ^ 'eaoAddOnRequest'
    -> EnableAddOn
enableAddOn pResourceName_ pAddOnRequest_
  = EnableAddOn'{_eaoResourceName = pResourceName_,
...
instance Hashable EnableAddOn where

instance NFData EnableAddOn where
...
data EnableAddOnResponse = EnableAddOnResponse'{_eaorsOperations
                                                :: !(Maybe [Operation]),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'eaorsOperations' - An array of objects that describe the result of the action, such as the status of the request, the time stamp of the request, and the resources affected by the request.
--
-- * 'eaorsResponseStatus' - -- | The response status code.
enableAddOnResponse
    :: Int -- ^ 'eaorsResponseStatus'
    -> EnableAddOnResponse
enableAddOnResponse pResponseStatus_
  = EnableAddOnResponse'{_eaorsOperations = Nothing,
...
instance NFData EnableAddOnResponse where
