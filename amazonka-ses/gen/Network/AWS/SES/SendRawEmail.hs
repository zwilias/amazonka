...
sreConfigurationSetName =
  lens _sreConfigurationSetName (\s a -> s {_sreConfigurationSetName = a})
...
sreDestinations =
  lens _sreDestinations (\s a -> s {_sreDestinations = a}) . _Default . _Coerce
...
instance AWSRequest SendRawEmail where
  ...
  response =
    receiveXMLWrapper
      "SendRawEmailResult"
      (\s h x ->
         SendRawEmailResponse' <$> (pure (fromEnum s)) <*> (x .@ "MessageId"))
...
instance ToQuery SendRawEmail where
  toQuery SendRawEmail' {..} =
    mconcat
      ...
      , "Destinations" =: toQuery (toQueryList "member" <$> _sreDestinations)
...
srersResponseStatus =
  lens _srersResponseStatus (\s a -> s {_srersResponseStatus = a})
