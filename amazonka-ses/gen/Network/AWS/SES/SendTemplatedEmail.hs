...
steConfigurationSetName =
  lens _steConfigurationSetName (\s a -> s {_steConfigurationSetName = a})
...
steReplyToAddresses =
  lens _steReplyToAddresses (\s a -> s {_steReplyToAddresses = a}) .
  _Default . _Coerce
...
instance AWSRequest SendTemplatedEmail where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         SendTemplatedEmailResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "MessageId"))
...
stersResponseStatus =
  lens _stersResponseStatus (\s a -> s {_stersResponseStatus = a})
