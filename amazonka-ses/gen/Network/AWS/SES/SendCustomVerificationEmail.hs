...
scveConfigurationSetName =
  lens _scveConfigurationSetName (\s a -> s {_scveConfigurationSetName = a})
...
instance AWSRequest SendCustomVerificationEmail where
  type Rs SendCustomVerificationEmail = SendCustomVerificationEmailResponse
  request = postQuery ses
  response =
    receiveXMLWrapper
      "SendCustomVerificationEmailResult"
      (\s h x ->
         SendCustomVerificationEmailResponse' <$> (x .@? "MessageId") <*>
         (pure (fromEnum s)))
...
scversResponseStatus =
  lens _scversResponseStatus (\s a -> s {_scversResponseStatus = a})
