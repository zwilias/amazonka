...
instance FromXML AddHeaderAction where
  parseXML x = AddHeaderAction' <$> (x .@ "HeaderName") <*> (x .@ "HeaderValue")
...
instance ToQuery AddHeaderAction where
  toQuery AddHeaderAction' {..} =
    mconcat ["HeaderName" =: _ahaHeaderName, "HeaderValue" =: _ahaHeaderValue]
...
instance FromXML BounceAction where
  parseXML x =
    BounceAction' <$> (x .@? "TopicArn") <*> (x .@? "StatusCode") <*>
    (x .@ "SmtpReplyCode") <*>
    (x .@ "Message") <*>
    (x .@ "Sender")
...
briRecipientDsnFields =
  lens _briRecipientDsnFields (\s a -> s {_briRecipientDsnFields = a})
...
bedReplacementTemplateData =
  lens _bedReplacementTemplateData (\s a -> s {_bedReplacementTemplateData = a})

-- | A list of tags, in the form of name/value pairs, to apply to an email that you send using @SendBulkTemplatedEmail@ . Tags correspond to characteristics of the email that you define, so that you can publish email sending events.
bedReplacementTags :: Lens' BulkEmailDestination [MessageTag]
bedReplacementTags =
  lens _bedReplacementTags (\s a -> s {_bedReplacementTags = a}) .
  _Default . _Coerce
...
instance FromXML BulkEmailDestinationStatus where
  parseXML x =
    BulkEmailDestinationStatus' <$> (x .@? "Status") <*> (x .@? "Error") <*>
    (x .@? "MessageId")
...
cwdDimensionConfigurations ::
     Lens' CloudWatchDestination [CloudWatchDimensionConfiguration]
cwdDimensionConfigurations =
  lens _cwdDimensionConfigurations (\s a -> s {_cwdDimensionConfigurations = a}) .
  _Coerce

instance FromXML CloudWatchDestination where
  parseXML x =
    CloudWatchDestination' <$>
    (x .@? "DimensionConfigurations" .!@ mempty >>= parseXMLList "member")
...
cwdcDimensionValueSource =
  lens _cwdcDimensionValueSource (\s a -> s {_cwdcDimensionValueSource = a})
...
cwdcDefaultDimensionValue =
  lens _cwdcDefaultDimensionValue (\s a -> s {_cwdcDefaultDimensionValue = a})

instance FromXML CloudWatchDimensionConfiguration where
  parseXML x =
    CloudWatchDimensionConfiguration' <$> (x .@ "DimensionName") <*>
...
instance ToQuery Content where
  toQuery Content' {..} = mconcat ["Charset" =: _cCharset, "Data" =: _cData]
...
cvetFromEmailAddress =
  lens _cvetFromEmailAddress (\s a -> s {_cvetFromEmailAddress = a})
...
dBCCAddresses =
  lens _dBCCAddresses (\s a -> s {_dBCCAddresses = a}) . _Default . _Coerce

-- | The CC: field(s) of the message.
dCCAddresses :: Lens' Destination [Text]
dCCAddresses =
  lens _dCCAddresses (\s a -> s {_dCCAddresses = a}) . _Default . _Coerce

-- | The To: field(s) of the message.
dToAddresses :: Lens' Destination [Text]
dToAddresses =
  lens _dToAddresses (\s a -> s {_dToAddresses = a}) . _Default . _Coerce
...
instance ToQuery Destination where
  toQuery Destination' {..} =
    mconcat
      [ "BccAddresses" =: toQuery (toQueryList "member" <$> _dBCCAddresses)
...
edKinesisFirehoseDestination ::
     Lens' EventDestination (Maybe KinesisFirehoseDestination)
edKinesisFirehoseDestination =
  lens
    _edKinesisFirehoseDestination
    (\s a -> s {_edKinesisFirehoseDestination = a})
...
edCloudWatchDestination =
  lens _edCloudWatchDestination (\s a -> s {_edCloudWatchDestination = a})
...
edMatchingEventTypes =
  lens _edMatchingEventTypes (\s a -> s {_edMatchingEventTypes = a}) . _Coerce

instance FromXML EventDestination where
  parseXML x =
    EventDestination' <$> (x .@? "Enabled") <*>
...
idaDkimTokens =
  lens _idaDkimTokens (\s a -> s {_idaDkimTokens = a}) . _Default . _Coerce
...
idaDkimVerificationStatus =
  lens _idaDkimVerificationStatus (\s a -> s {_idaDkimVerificationStatus = a})
...
imfdaMailFromDomain =
  lens _imfdaMailFromDomain (\s a -> s {_imfdaMailFromDomain = a})

-- | The state that indicates whether Amazon SES has successfully read the MX record required for custom MAIL FROM domain setup. If the state is @Success@ , Amazon SES uses the specified custom MAIL FROM domain when the verified identity sends an email. All other states indicate that Amazon SES takes the action described by @BehaviorOnMXFailure@ .
imfdaMailFromDomainStatus ::
     Lens' IdentityMailFromDomainAttributes CustomMailFromStatus
imfdaMailFromDomainStatus =
  lens _imfdaMailFromDomainStatus (\s a -> s {_imfdaMailFromDomainStatus = a})

-- | The action that Amazon SES takes if it cannot successfully read the required MX record when you send an email. A value of @UseDefaultValue@ indicates that if Amazon SES cannot read the required MX record, it uses amazonses.com (or a subdomain of that) as the MAIL FROM domain. A value of @RejectMessage@ indicates that if Amazon SES cannot read the required MX record, Amazon SES returns a @MailFromDomainNotVerified@ error and does not send the email. The custom MAIL FROM setup states that result in this behavior are @Pending@ , @Failed@ , and @TemporaryFailure@ .
imfdaBehaviorOnMXFailure ::
     Lens' IdentityMailFromDomainAttributes BehaviorOnMXFailure
imfdaBehaviorOnMXFailure =
  lens _imfdaBehaviorOnMXFailure (\s a -> s {_imfdaBehaviorOnMXFailure = a})

instance FromXML IdentityMailFromDomainAttributes where
  parseXML x =
    IdentityMailFromDomainAttributes' <$> (x .@ "MailFromDomain") <*>
...
inaHeadersInDeliveryNotificationsEnabled ::
     Lens' IdentityNotificationAttributes (Maybe Bool)
inaHeadersInDeliveryNotificationsEnabled =
  lens
    _inaHeadersInDeliveryNotificationsEnabled
    (\s a -> s {_inaHeadersInDeliveryNotificationsEnabled = a})
...
inaHeadersInComplaintNotificationsEnabled =
  lens
    ...
    (\s a -> s {_inaHeadersInComplaintNotificationsEnabled = a})
...
inaHeadersInBounceNotificationsEnabled =
  lens
    ...
    (\s a -> s {_inaHeadersInBounceNotificationsEnabled = a})
...
instance FromXML IdentityVerificationAttributes where
  parseXML x =
    IdentityVerificationAttributes' <$> (x .@? "VerificationToken") <*>
...
kfdDeliveryStreamARN =
  lens _kfdDeliveryStreamARN (\s a -> s {_kfdDeliveryStreamARN = a})

instance FromXML KinesisFirehoseDestination where
  parseXML x =
    KinesisFirehoseDestination' <$> (x .@ "IAMRoleARN") <*>
    (x .@ "DeliveryStreamARN")
...
instance FromXML LambdaAction where
  parseXML x =
    LambdaAction' <$> (x .@? "InvocationType") <*> (x .@? "TopicArn") <*>
    (x .@ "FunctionArn")
...
instance ToQuery Message where
  toQuery Message' {..} = mconcat ["Subject" =: _mSubject, "Body" =: _mBody]
...
mdArrivalDate =
  lens _mdArrivalDate (\s a -> s {_mdArrivalDate = a}) . mapping _Time

-- | Additional X-headers to include in the DSN.
mdExtensionFields :: Lens' MessageDsn [ExtensionField]
mdExtensionFields =
  lens _mdExtensionFields (\s a -> s {_mdExtensionFields = a}) .
  _Default . _Coerce
...
instance ToQuery MessageTag where
  toQuery MessageTag' {..} = mconcat ["Name" =: _mtName, "Value" =: _mtValue]
...
instance FromXML ReceiptAction where
  parseXML x =
    ReceiptAction' <$> (x .@? "AddHeaderAction") <*> (x .@? "SNSAction") <*>
    (x .@? "WorkmailAction") <*>
    (x .@? "BounceAction") <*>
    (x .@? "LambdaAction") <*>
    (x .@? "StopAction") <*>
...
rrRecipients =
  lens _rrRecipients (\s a -> s {_rrRecipients = a}) . _Default . _Coerce
...
instance FromXML ReceiptRule where
  parseXML x =
    ReceiptRule' <$> (x .@? "ScanEnabled") <*> (x .@? "Enabled") <*>
...
rrsmCreatedTimestamp =
  lens _rrsmCreatedTimestamp (\s a -> s {_rrsmCreatedTimestamp = a}) .
  mapping _Time

instance FromXML ReceiptRuleSetMetadata where
  parseXML x =
    ReceiptRuleSetMetadata' <$> (x .@? "Name") <*> (x .@? "CreatedTimestamp")
...
rdfExtensionFields =
  lens _rdfExtensionFields (\s a -> s {_rdfExtensionFields = a}) .
  _Default . _Coerce
...
rdfLastAttemptDate =
  lens _rdfLastAttemptDate (\s a -> s {_rdfLastAttemptDate = a}) . mapping _Time
...
roLastFreshStart =
  lens _roLastFreshStart (\s a -> s {_roLastFreshStart = a}) . mapping _Time
...
roReputationMetricsEnabled =
  lens _roReputationMetricsEnabled (\s a -> s {_roReputationMetricsEnabled = a})
...
instance FromXML ReputationOptions where
  parseXML x =
    ReputationOptions' <$> (x .@? "LastFreshStart") <*>
...
instance FromXML S3Action where
  parseXML x =
    S3Action' <$> (x .@? "KmsKeyArn") <*> (x .@? "TopicArn") <*>
    (x .@? "ObjectKeyPrefix") <*>
    (x .@ "BucketName")
...
instance FromXML SNSAction where
  parseXML x = SNSAction' <$> (x .@? "Encoding") <*> (x .@ "TopicArn")
...
sdpTimestamp =
  lens _sdpTimestamp (\s a -> s {_sdpTimestamp = a}) . mapping _Time

instance FromXML SendDataPoint where
  parseXML x =
    SendDataPoint' <$> (x .@? "Rejects") <*> (x .@? "Complaints") <*>
    (x .@? "DeliveryAttempts") <*>
    (x .@? "Bounces") <*>
    (x .@? "Timestamp")
...
instance FromXML Template where
  parseXML x =
    Template' <$> (x .@? "TextPart") <*> (x .@? "SubjectPart") <*>
    (x .@? "HtmlPart") <*>
    (x .@ "TemplateName")
...
tmCreatedTimestamp =
  lens _tmCreatedTimestamp (\s a -> s {_tmCreatedTimestamp = a}) . mapping _Time

instance FromXML TemplateMetadata where
  parseXML x =
    TemplateMetadata' <$> (x .@? "Name") <*> (x .@? "CreatedTimestamp")
...
toCustomRedirectDomain =
  lens _toCustomRedirectDomain (\s a -> s {_toCustomRedirectDomain = a})
...
instance FromXML WorkmailAction where
  parseXML x =
    WorkmailAction' <$> (x .@? "TopicArn") <*> (x .@ "OrganizationArn")
...
instance ToQuery WorkmailAction where
  toQuery WorkmailAction' {..} =
    mconcat ["TopicArn" =: _waTopicARN, "OrganizationArn" =: _waOrganizationARN]
