...
instance FromText BehaviorOnMXFailure where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BehaviorOnMXFailure from value: '" <>
        e <> "'. Accepted values: rejectmessage, usedefaultvalue"
...
instance FromText BounceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BounceType from value: '" <>
        e <>
...
instance FromText BulkEmailStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BulkEmailStatus from value: '" <>
        e <>
...
instance FromText ConfigurationSetAttribute where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConfigurationSetAttribute from value: '" <>
        e <>
...
instance FromText CustomMailFromStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CustomMailFromStatus from value: '" <>
        e <> "'. Accepted values: failed, pending, success, temporaryfailure"
...
instance FromText DimensionValueSource where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DimensionValueSource from value: '" <>
        e <> "'. Accepted values: emailheader, linktag, messagetag"
...
instance FromText DsnAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DsnAction from value: '" <>
        e <> "'. Accepted values: delayed, delivered, expanded, failed, relayed"
...
instance FromText IdentityType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IdentityType from value: '" <>
        e <> "'. Accepted values: domain, emailaddress"
...
instance FromText InvocationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing InvocationType from value: '" <>
        e <> "'. Accepted values: event, requestresponse"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: bounce, complaint, delivery"
...
instance FromText ReceiptFilterPolicy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReceiptFilterPolicy from value: '" <>
        e <> "'. Accepted values: allow, block"
...
instance FromText SNSActionEncoding where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SNSActionEncoding from value: '" <>
        e <> "'. Accepted values: base64, utf-8"
...
instance FromText StopScope where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StopScope from value: '" <>
        e <> "'. Accepted values: ruleset"
...
instance FromText TLSPolicy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TLSPolicy from value: '" <>
        e <> "'. Accepted values: optional, require"
...
instance FromText VerificationStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VerificationStatus from value: '" <>
        e <>
