...
sbOriginalMessageId =
  lens _sbOriginalMessageId (\s a -> s {_sbOriginalMessageId = a})
...
sbBouncedRecipientInfoList =
  lens _sbBouncedRecipientInfoList (\s a -> s {_sbBouncedRecipientInfoList = a}) .
  _Coerce
...
instance AWSRequest SendBounce where
  ...
  response =
    receiveXMLWrapper
      "SendBounceResult"
      (\s h x ->
         SendBounceResponse' <$> (x .@? "MessageId") <*> (pure (fromEnum s)))
...
sbrsResponseStatus =
  lens _sbrsResponseStatus (\s a -> s {_sbrsResponseStatus = a})
