...
sbteConfigurationSetName =
  lens _sbteConfigurationSetName (\s a -> s {_sbteConfigurationSetName = a})
...
sbteDefaultTags =
  lens _sbteDefaultTags (\s a -> s {_sbteDefaultTags = a}) . _Default . _Coerce
...
sbteDefaultTemplateData =
  lens _sbteDefaultTemplateData (\s a -> s {_sbteDefaultTemplateData = a})

-- | The reply-to email address(es) for the message. If the recipient replies to the message, each reply-to address will receive the reply.
sbteReplyToAddresses :: Lens' SendBulkTemplatedEmail [Text]
sbteReplyToAddresses =
  lens _sbteReplyToAddresses (\s a -> s {_sbteReplyToAddresses = a}) .
  _Default . _Coerce
...
sbteDestinations =
  lens _sbteDestinations (\s a -> s {_sbteDestinations = a}) . _Coerce
...
instance AWSRequest SendBulkTemplatedEmail where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         SendBulkTemplatedEmailResponse' <$> (pure (fromEnum s)) <*>
...
instance ToQuery SendBulkTemplatedEmail where
  toQuery SendBulkTemplatedEmail' {..} =
    mconcat
      ...
      , "DefaultTags" =: toQuery (toQueryList "member" <$> _sbteDefaultTags)
...
sbtersResponseStatus =
  lens _sbtersResponseStatus (\s a -> s {_sbtersResponseStatus = a})

-- | The unique message identifier returned from the @SendBulkTemplatedEmail@ action.
sbtersStatus ::
     Lens' SendBulkTemplatedEmailResponse [BulkEmailDestinationStatus]
