...
gidaIdentities =
  lens _gidaIdentities (\s a -> s {_gidaIdentities = a}) . _Coerce
...
instance AWSRequest GetIdentityDkimAttributes where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         GetIdentityDkimAttributesResponse' <$> (pure (fromEnum s)) <*>
...
gidarsResponseStatus =
  lens _gidarsResponseStatus (\s a -> s {_gidarsResponseStatus = a})

-- | The DKIM attributes for an email address or a domain.
gidarsDkimAttributes ::
     Lens' GetIdentityDkimAttributesResponse (HashMap Text IdentityDkimAttributes)
gidarsDkimAttributes =
  lens _gidarsDkimAttributes (\s a -> s {_gidarsDkimAttributes = a}) . _Map
