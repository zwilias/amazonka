...
instance AWSRequest SetIdentityFeedbackForwardingEnabled where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         SetIdentityFeedbackForwardingEnabledResponse' <$> (pure (fromEnum s)))
...
instance ToQuery SetIdentityFeedbackForwardingEnabled where
  toQuery SetIdentityFeedbackForwardingEnabled' {..} =
    mconcat
      [ "Action" =: ("SetIdentityFeedbackForwardingEnabled" :: ByteString)
...
siffersResponseStatus =
  lens _siffersResponseStatus (\s a -> s {_siffersResponseStatus = a})
