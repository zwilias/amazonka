...
instance AWSRequest DeregisterWebhookWithThirdParty where
  type Rs DeregisterWebhookWithThirdParty = DeregisterWebhookWithThirdPartyResponse
...
instance ToHeaders DeregisterWebhookWithThirdParty where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("CodePipeline_20150709.DeregisterWebhookWithThirdParty" :: ByteString)
...
dwwtprsResponseStatus =
  lens _dwwtprsResponseStatus (\s a -> s {_dwwtprsResponseStatus = a})
