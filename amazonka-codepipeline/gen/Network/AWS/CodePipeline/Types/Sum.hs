...
instance FromText ActionCategory where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionCategory from value: '" <>
        e <> "'. Accepted values: approval, build, deploy, invoke, source, test"
...
instance FromText ActionConfigurationPropertyType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionConfigurationPropertyType from value: '" <>
        e <> "'. Accepted values: boolean, number, string"
...
instance FromText ActionExecutionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionExecutionStatus from value: '" <>
        e <> "'. Accepted values: failed, inprogress, succeeded"
...
instance FromText ActionOwner where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionOwner from value: '" <>
        e <> "'. Accepted values: aws, custom, thirdparty"
...
instance FromText ApprovalStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ApprovalStatus from value: '" <>
        e <> "'. Accepted values: approved, rejected"
...
instance FromText ArtifactLocationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ArtifactLocationType from value: '" <>
        e <> "'. Accepted values: s3"
...
instance FromText ArtifactStoreType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ArtifactStoreType from value: '" <>
        e <> "'. Accepted values: s3"
...
instance FromText BlockerType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BlockerType from value: '" <>
        e <> "'. Accepted values: schedule"
...
instance FromText EncryptionKeyType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EncryptionKeyType from value: '" <>
        e <> "'. Accepted values: kms"
...
instance FromText PipelineExecutionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PipelineExecutionStatus from value: '" <>
        e <> "'. Accepted values: failed, inprogress, succeeded, superseded"
...
instance FromText StageExecutionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StageExecutionStatus from value: '" <>
        e <> "'. Accepted values: failed, inprogress, succeeded"
...
instance FromText StageRetryMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StageRetryMode from value: '" <>
        e <> "'. Accepted values: failed_actions"
...
instance FromText StageTransitionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StageTransitionType from value: '" <>
...
instance FromText WebhookAuthenticationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing WebhookAuthenticationType from value: '" <>
        e <> "'. Accepted values: github_hmac, ip, unauthenticated"
