...
pftpjMaxBatchSize =
  lens _pftpjMaxBatchSize (\s a -> s {_pftpjMaxBatchSize = a}) . mapping _Nat
...
instance AWSRequest PollForThirdPartyJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         PollForThirdPartyJobsResponse' <$> (x .?> "jobs" .!@ mempty) <*>
         (pure (fromEnum s)))
...
pftpjrsJobs =
  lens _pftpjrsJobs (\s a -> s {_pftpjrsJobs = a}) . _Default . _Coerce

-- | -- | The response status code.
pftpjrsResponseStatus :: Lens' PollForThirdPartyJobsResponse Int
pftpjrsResponseStatus =
  lens _pftpjrsResponseStatus (\s a -> s {_pftpjrsResponseStatus = a})
