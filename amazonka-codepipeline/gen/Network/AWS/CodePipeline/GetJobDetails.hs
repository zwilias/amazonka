...
instance AWSRequest GetJobDetails where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetJobDetailsResponse' <$> (x .?> "jobDetails") <*> (pure (fromEnum s)))
...
instance ToJSON GetJobDetails where
  toJSON GetJobDetails' {..} = object (catMaybes [Just ("jobId" .= _gjdJobId)])
...
gjdrsResponseStatus =
  lens _gjdrsResponseStatus (\s a -> s {_gjdrsResponseStatus = a})
