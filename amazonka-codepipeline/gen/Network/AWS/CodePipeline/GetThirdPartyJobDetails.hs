...
instance AWSRequest GetThirdPartyJobDetails where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetThirdPartyJobDetailsResponse' <$> (x .?> "jobDetails") <*>
         (pure (fromEnum s)))
...
gtpjdrsJobDetails ::
     Lens' GetThirdPartyJobDetailsResponse (Maybe ThirdPartyJobDetails)
...
gtpjdrsResponseStatus =
  lens _gtpjdrsResponseStatus (\s a -> s {_gtpjdrsResponseStatus = a})
