...
instance AWSRequest ListDeadLetterSourceQueues where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ListDeadLetterSourceQueuesResponse' <$> (pure (fromEnum s)) <*>
...
ldlsqrsResponseStatus =
  lens _ldlsqrsResponseStatus (\s a -> s {_ldlsqrsResponseStatus = a})

-- | A list of source queue URLs that have the @RedrivePolicy@ queue attribute configured with a dead-letter queue.
ldlsqrsQueueURLs :: Lens' ListDeadLetterSourceQueuesResponse [Text]
ldlsqrsQueueURLs =
  lens _ldlsqrsQueueURLs (\s a -> s {_ldlsqrsQueueURLs = a}) . _Coerce
