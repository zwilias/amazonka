...
cqAttributes =
  lens _cqAttributes (\s a -> s {_cqAttributes = a}) . _Default . _Map
...
instance AWSRequest CreateQueue where
  ...
  response =
    receiveXMLWrapper
      "CreateQueueResult"
      (\s h x ->
         CreateQueueResponse' <$> (x .@? "QueueUrl") <*> (pure (fromEnum s)))
...
cqrsResponseStatus =
  lens _cqrsResponseStatus (\s a -> s {_cqrsResponseStatus = a})
