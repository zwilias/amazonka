...
rmReceiveRequestAttemptId =
  lens _rmReceiveRequestAttemptId (\s a -> s {_rmReceiveRequestAttemptId = a})
...
instance AWSRequest ReceiveMessage where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ReceiveMessageResponse' <$> (may (parseXMLList "Message") x) <*>
         (pure (fromEnum s)))
...
rmrsMessages =
  lens _rmrsMessages (\s a -> s {_rmrsMessages = a}) . _Default . _Coerce

-- | -- | The response status code.
rmrsResponseStatus :: Lens' ReceiveMessageResponse Int
rmrsResponseStatus =
  lens _rmrsResponseStatus (\s a -> s {_rmrsResponseStatus = a})
