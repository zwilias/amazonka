...
smMessageDeduplicationId =
  lens _smMessageDeduplicationId (\s a -> s {_smMessageDeduplicationId = a})
...
instance AWSRequest SendMessage where
  ...
  response =
    receiveXMLWrapper
      "SendMessageResult"
      (\s h x ->
         SendMessageResponse' <$> (x .@? "SequenceNumber") <*>
         (x .@? "MessageId") <*>
         (x .@? "MD5OfMessageBody") <*>
...
instance ToQuery SendMessage where
  toQuery SendMessage' {..} =
    mconcat
      ...
      , toQuery
          (toQueryMap "MessageAttribute" "Name" "Value" <$> _smMessageAttributes)
...
smrsSequenceNumber =
  lens _smrsSequenceNumber (\s a -> s {_smrsSequenceNumber = a})
...
smrsMD5OfMessageAttributes =
  lens _smrsMD5OfMessageAttributes (\s a -> s {_smrsMD5OfMessageAttributes = a})
