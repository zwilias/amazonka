...
sqeQueryExecutionId =
  lens _sqeQueryExecutionId (\s a -> s {_sqeQueryExecutionId = a})

instance AWSRequest StopQueryExecution where
  type Rs StopQueryExecution = StopQueryExecutionResponse
  request = postJSON athena
  response =
    receiveEmpty (\s h x -> StopQueryExecutionResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders StopQueryExecution where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonAthena.StopQueryExecution" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON StopQueryExecution where
  toJSON StopQueryExecution' {..} =
    object (catMaybes [Just ("QueryExecutionId" .= _sqeQueryExecutionId)])
