...
gqeQueryExecutionId =
  lens _gqeQueryExecutionId (\s a -> s {_gqeQueryExecutionId = a})
...
instance AWSRequest GetQueryExecution where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetQueryExecutionResponse' <$> (x .?> "QueryExecution") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetQueryExecution where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonAthena.GetQueryExecution" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON GetQueryExecution where
  toJSON GetQueryExecution' {..} =
    object (catMaybes [Just ("QueryExecutionId" .= _gqeQueryExecutionId)])
...
gqersQueryExecution =
  lens _gqersQueryExecution (\s a -> s {_gqersQueryExecution = a})

-- | -- | The response status code.
gqersResponseStatus :: Lens' GetQueryExecutionResponse Int
gqersResponseStatus =
  lens _gqersResponseStatus (\s a -> s {_gqersResponseStatus = a})
