...
bgnqNamedQueryIds =
  lens _bgnqNamedQueryIds (\s a -> s {_bgnqNamedQueryIds = a}) . _List1
...
instance AWSRequest BatchGetNamedQuery where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchGetNamedQueryResponse' <$> (x .?> "NamedQueries" .!@ mempty) <*>
...
instance ToHeaders BatchGetNamedQuery where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonAthena.BatchGetNamedQuery" :: ByteString)
...
bgnqrsNamedQueries =
  lens _bgnqrsNamedQueries (\s a -> s {_bgnqrsNamedQueries = a}) .
  _Default . _Coerce

-- | Information about provided query IDs.
bgnqrsUnprocessedNamedQueryIds ::
     Lens' BatchGetNamedQueryResponse [UnprocessedNamedQueryId]
bgnqrsUnprocessedNamedQueryIds =
  lens
    _bgnqrsUnprocessedNamedQueryIds
    (\s a -> s {_bgnqrsUnprocessedNamedQueryIds = a}) .
  _Default . _Coerce

-- | -- | The response status code.
bgnqrsResponseStatus :: Lens' BatchGetNamedQueryResponse Int
bgnqrsResponseStatus =
  lens _bgnqrsResponseStatus (\s a -> s {_bgnqrsResponseStatus = a})
