...
cnqClientRequestToken =
  lens _cnqClientRequestToken (\s a -> s {_cnqClientRequestToken = a})
...
instance AWSRequest CreateNamedQuery where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateNamedQueryResponse' <$> (x .?> "NamedQueryId") <*>
         (pure (fromEnum s)))
...
cnqrsResponseStatus =
  lens _cnqrsResponseStatus (\s a -> s {_cnqrsResponseStatus = a})
