...
data SentimentResponse = SentimentResponse'{_sSentimentScore
                                            :: !(Maybe Text),
...
sentimentResponse
    :: SentimentResponse
sentimentResponse
  = SentimentResponse'{_sSentimentScore = Nothing,
...
instance FromJSON SentimentResponse where
        parseJSON
          = withObject "SentimentResponse"
              (\ x ->
                 SentimentResponse' <$>
                   (x .:? "sentimentScore") <*>
                     (x .:? "sentimentLabel"))

instance Hashable SentimentResponse where

instance NFData SentimentResponse where
