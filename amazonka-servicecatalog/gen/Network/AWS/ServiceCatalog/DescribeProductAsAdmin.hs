...
dpaaAcceptLanguage =
  lens _dpaaAcceptLanguage (\s a -> s {_dpaaAcceptLanguage = a})
...
instance AWSRequest DescribeProductAsAdmin where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeProductAsAdminResponse' <$> (x .?> "ProductViewDetail") <*>
         (x .?> "TagOptions" .!@ mempty) <*>
...
instance ToHeaders DescribeProductAsAdmin where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.DescribeProductAsAdmin" :: ByteString)
...
instance ToJSON DescribeProductAsAdmin where
  toJSON DescribeProductAsAdmin' {..} =
    object
      (catMaybes
         [("AcceptLanguage" .=) <$> _dpaaAcceptLanguage, Just ("Id" .= _dpaaId)])
...
dpaarsProductViewDetail ::
     Lens' DescribeProductAsAdminResponse (Maybe ProductViewDetail)
dpaarsProductViewDetail =
  lens _dpaarsProductViewDetail (\s a -> s {_dpaarsProductViewDetail = a})

-- | Information about the TagOptions associated with the product.
dpaarsTagOptions :: Lens' DescribeProductAsAdminResponse [TagOptionDetail]
dpaarsTagOptions =
  lens _dpaarsTagOptions (\s a -> s {_dpaarsTagOptions = a}) .
  _Default . _Coerce

-- | Information about the provisioning artifacts (also known as versions) for the specified product.
dpaarsProvisioningArtifactSummaries ::
     Lens' DescribeProductAsAdminResponse [ProvisioningArtifactSummary]
dpaarsProvisioningArtifactSummaries =
  lens
    _dpaarsProvisioningArtifactSummaries
    (\s a -> s {_dpaarsProvisioningArtifactSummaries = a}) .
  _Default . _Coerce
...
dpaarsResponseStatus =
  lens _dpaarsResponseStatus (\s a -> s {_dpaarsResponseStatus = a})
