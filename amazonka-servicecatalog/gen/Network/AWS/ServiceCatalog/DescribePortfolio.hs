...
instance AWSRequest DescribePortfolio where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribePortfolioResponse' <$> (x .?> "PortfolioDetail") <*>
         (x .?> "TagOptions" .!@ mempty) <*>
         (x .?> "Tags" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DescribePortfolio where
  toJSON DescribePortfolio' {..} =
    object
      (catMaybes
         [("AcceptLanguage" .=) <$> _desAcceptLanguage, Just ("Id" .= _desId)])
...
dprsPortfolioDetail =
  lens _dprsPortfolioDetail (\s a -> s {_dprsPortfolioDetail = a})

-- | Information about the TagOptions associated with the portfolio.
dprsTagOptions :: Lens' DescribePortfolioResponse [TagOptionDetail]
dprsTagOptions =
  lens _dprsTagOptions (\s a -> s {_dprsTagOptions = a}) . _Default . _Coerce
