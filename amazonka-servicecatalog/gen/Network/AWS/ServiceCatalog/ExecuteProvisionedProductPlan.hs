...
epppAcceptLanguage =
  lens _epppAcceptLanguage (\s a -> s {_epppAcceptLanguage = a})
...
epppIdempotencyToken =
  lens _epppIdempotencyToken (\s a -> s {_epppIdempotencyToken = a})

instance AWSRequest ExecuteProvisionedProductPlan where
  type Rs ExecuteProvisionedProductPlan = ExecuteProvisionedProductPlanResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         ExecuteProvisionedProductPlanResponse' <$> (x .?> "RecordDetail") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ExecuteProvisionedProductPlan where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.ExecuteProvisionedProductPlan" :: ByteString)
...
eppprsRecordDetail ::
     Lens' ExecuteProvisionedProductPlanResponse (Maybe RecordDetail)
eppprsRecordDetail =
  lens _eppprsRecordDetail (\s a -> s {_eppprsRecordDetail = a})

-- | -- | The response status code.
eppprsResponseStatus :: Lens' ExecuteProvisionedProductPlanResponse Int
eppprsResponseStatus =
  lens _eppprsResponseStatus (\s a -> s {_eppprsResponseStatus = a})
