...
instance AWSRequest ListLaunchPaths where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListLaunchPathsResponse' <$> (x .?> "NextPageToken") <*>
...
llprsNextPageToken =
  lens _llprsNextPageToken (\s a -> s {_llprsNextPageToken = a})

-- | Information about the launch path.
llprsLaunchPathSummaries :: Lens' ListLaunchPathsResponse [LaunchPathSummary]
llprsLaunchPathSummaries =
  lens _llprsLaunchPathSummaries (\s a -> s {_llprsLaunchPathSummaries = a}) .
  _Default . _Coerce
