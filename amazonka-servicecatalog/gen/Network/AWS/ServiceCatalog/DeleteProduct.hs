...
instance AWSRequest DeleteProduct where
  ...
  response =
    receiveEmpty (\s h x -> DeleteProductResponse' <$> (pure (fromEnum s)))
...
instance ToJSON DeleteProduct where
  toJSON DeleteProduct' {..} =
    object
      (catMaybes
         [("AcceptLanguage" .=) <$> _dppAcceptLanguage, Just ("Id" .= _dppId)])
...
delersResponseStatus =
  lens _delersResponseStatus (\s a -> s {_delersResponseStatus = a})
