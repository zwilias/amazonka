...
lapsAcceptLanguage =
  lens _lapsAcceptLanguage (\s a -> s {_lapsAcceptLanguage = a})
...
instance AWSPager ListAcceptedPortfolioShares where
  page rq rs
    ...
    | otherwise = Just $ rq & lapsPageToken .~ rs ^. lapsrsNextPageToken

instance AWSRequest ListAcceptedPortfolioShares where
  type Rs ListAcceptedPortfolioShares = ListAcceptedPortfolioSharesResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         ListAcceptedPortfolioSharesResponse' <$> (x .?> "NextPageToken") <*>
...
instance ToHeaders ListAcceptedPortfolioShares where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.ListAcceptedPortfolioShares" :: ByteString)
...
lapsrsNextPageToken =
  lens _lapsrsNextPageToken (\s a -> s {_lapsrsNextPageToken = a})

-- | Information about the portfolios.
lapsrsPortfolioDetails ::
     Lens' ListAcceptedPortfolioSharesResponse [PortfolioDetail]
lapsrsPortfolioDetails =
  lens _lapsrsPortfolioDetails (\s a -> s {_lapsrsPortfolioDetails = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lapsrsResponseStatus :: Lens' ListAcceptedPortfolioSharesResponse Int
lapsrsResponseStatus =
  lens _lapsrsResponseStatus (\s a -> s {_lapsrsResponseStatus = a})
