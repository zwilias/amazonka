...
dpaProvisioningArtifactId =
  lens _dpaProvisioningArtifactId (\s a -> s {_dpaProvisioningArtifactId = a})
...
instance AWSRequest DescribeProvisioningArtifact where
  type Rs DescribeProvisioningArtifact = DescribeProvisioningArtifactResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         DescribeProvisioningArtifactResponse' <$> (x .?> "Status") <*>
         (x .?> "Info" .!@ mempty) <*>
...
instance ToHeaders DescribeProvisioningArtifact where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.DescribeProvisioningArtifact" :: ByteString)
  ...
  toJSON DescribeProvisioningArtifact' {..} =
    object
      (catMaybes
         ...
         , Just ("ProvisioningArtifactId" .= _dpaProvisioningArtifactId)
...
dpaprsProvisioningArtifactDetail ::
     Lens' DescribeProvisioningArtifactResponse (Maybe ProvisioningArtifactDetail)
dpaprsProvisioningArtifactDetail =
  lens
    _dpaprsProvisioningArtifactDetail
    (\s a -> s {_dpaprsProvisioningArtifactDetail = a})

-- | -- | The response status code.
dpaprsResponseStatus :: Lens' DescribeProvisioningArtifactResponse Int
dpaprsResponseStatus =
  lens _dpaprsResponseStatus (\s a -> s {_dpaprsResponseStatus = a})
