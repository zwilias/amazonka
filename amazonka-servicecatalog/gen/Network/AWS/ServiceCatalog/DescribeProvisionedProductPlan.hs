...
dpppAcceptLanguage =
  lens _dpppAcceptLanguage (\s a -> s {_dpppAcceptLanguage = a})
...
instance AWSRequest DescribeProvisionedProductPlan where
  type Rs DescribeProvisionedProductPlan = DescribeProvisionedProductPlanResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         DescribeProvisionedProductPlanResponse' <$> (x .?> "NextPageToken") <*>
...
instance ToHeaders DescribeProvisionedProductPlan where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.DescribeProvisionedProductPlan" :: ByteString)
...
dpppprsNextPageToken ::
     Lens' DescribeProvisionedProductPlanResponse (Maybe Text)
dpppprsNextPageToken =
  lens _dpppprsNextPageToken (\s a -> s {_dpppprsNextPageToken = a})

-- | Information about the plan.
dpppprsProvisionedProductPlanDetails ::
     Lens' DescribeProvisionedProductPlanResponse (Maybe ProvisionedProductPlanDetails)
dpppprsProvisionedProductPlanDetails =
  lens
    _dpppprsProvisionedProductPlanDetails
    (\s a -> s {_dpppprsProvisionedProductPlanDetails = a})

-- | Information about the resource changes that will occur when the plan is executed.
dpppprsResourceChanges ::
     Lens' DescribeProvisionedProductPlanResponse [ResourceChange]
dpppprsResourceChanges =
  lens _dpppprsResourceChanges (\s a -> s {_dpppprsResourceChanges = a}) .
  _Default . _Coerce

-- | -- | The response status code.
dpppprsResponseStatus :: Lens' DescribeProvisionedProductPlanResponse Int
dpppprsResponseStatus =
  lens _dpppprsResponseStatus (\s a -> s {_dpppprsResponseStatus = a})
