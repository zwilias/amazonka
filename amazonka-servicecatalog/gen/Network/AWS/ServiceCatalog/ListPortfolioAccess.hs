...
instance AWSRequest ListPortfolioAccess where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPortfolioAccessResponse' <$> (x .?> "NextPageToken") <*>
         (x .?> "AccountIds" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lparsNextPageToken =
  lens _lparsNextPageToken (\s a -> s {_lparsNextPageToken = a})

-- | Information about the AWS accounts with access to the portfolio.
lparsAccountIds :: Lens' ListPortfolioAccessResponse [Text]
lparsAccountIds =
  lens _lparsAccountIds (\s a -> s {_lparsAccountIds = a}) . _Default . _Coerce
