...
spaaAcceptLanguage =
  lens _spaaAcceptLanguage (\s a -> s {_spaaAcceptLanguage = a})
...
instance AWSPager SearchProductsAsAdmin where
  page rq rs
    ...
    | otherwise = Just $ rq & spaaPageToken .~ rs ^. spaarsNextPageToken
...
instance AWSRequest SearchProductsAsAdmin where
  ...
  response =
    receiveJSON
      (\s h x ->
         SearchProductsAsAdminResponse' <$> (x .?> "NextPageToken") <*>
...
instance ToHeaders SearchProductsAsAdmin where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.SearchProductsAsAdmin" :: ByteString)
...
spaarsNextPageToken =
  lens _spaarsNextPageToken (\s a -> s {_spaarsNextPageToken = a})

-- | Information about the product views.
spaarsProductViewDetails ::
     Lens' SearchProductsAsAdminResponse [ProductViewDetail]
spaarsProductViewDetails =
  lens _spaarsProductViewDetails (\s a -> s {_spaarsProductViewDetails = a}) .
  _Default . _Coerce

-- | -- | The response status code.
spaarsResponseStatus :: Lens' SearchProductsAsAdminResponse Int
spaarsResponseStatus =
  lens _spaarsResponseStatus (\s a -> s {_spaarsResponseStatus = a})
