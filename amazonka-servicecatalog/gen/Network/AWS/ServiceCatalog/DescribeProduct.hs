...
instance AWSRequest DescribeProduct where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeProductResponse' <$> (x .?> "ProductViewSummary") <*>
...
instance ToJSON DescribeProduct where
  toJSON DescribeProduct' {..} =
    object
      (catMaybes
         [("AcceptLanguage" .=) <$> _dpAcceptLanguage, Just ("Id" .= _dpId)])
...
ddrsProductViewSummary ::
     Lens' DescribeProductResponse (Maybe ProductViewSummary)
ddrsProductViewSummary =
  lens _ddrsProductViewSummary (\s a -> s {_ddrsProductViewSummary = a})

-- | Information about the provisioning artifacts for the specified product.
ddrsProvisioningArtifacts ::
     Lens' DescribeProductResponse [ProvisioningArtifact]
ddrsProvisioningArtifacts =
  lens _ddrsProvisioningArtifacts (\s a -> s {_ddrsProvisioningArtifacts = a}) .
  _Default . _Coerce
