...
lpfpAcceptLanguage =
  lens _lpfpAcceptLanguage (\s a -> s {_lpfpAcceptLanguage = a})
...
instance AWSRequest ListPrincipalsForPortfolio where
  type Rs ListPrincipalsForPortfolio = ListPrincipalsForPortfolioResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         ListPrincipalsForPortfolioResponse' <$> (x .?> "NextPageToken") <*>
         (x .?> "Principals" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListPrincipalsForPortfolio where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.ListPrincipalsForPortfolio" :: ByteString)
...
lisrsNextPageToken =
  lens _lisrsNextPageToken (\s a -> s {_lisrsNextPageToken = a})

-- | The IAM principals (users or roles) associated with the portfolio.
lisrsPrincipals :: Lens' ListPrincipalsForPortfolioResponse [Principal]
lisrsPrincipals =
  lens _lisrsPrincipals (\s a -> s {_lisrsPrincipals = a}) . _Default . _Coerce

-- | -- | The response status code.
lisrsResponseStatus :: Lens' ListPrincipalsForPortfolioResponse Int
lisrsResponseStatus =
  lens _lisrsResponseStatus (\s a -> s {_lisrsResponseStatus = a})
