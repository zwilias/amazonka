...
instance AWSRequest ListPortfoliosForProduct where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPortfoliosForProductResponse' <$> (x .?> "NextPageToken") <*>
...
instance ToHeaders ListPortfoliosForProduct where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.ListPortfoliosForProduct" :: ByteString)
...
lpfprsNextPageToken =
  lens _lpfprsNextPageToken (\s a -> s {_lpfprsNextPageToken = a})

-- | Information about the portfolios.
lpfprsPortfolioDetails ::
     Lens' ListPortfoliosForProductResponse [PortfolioDetail]
lpfprsPortfolioDetails =
  lens _lpfprsPortfolioDetails (\s a -> s {_lpfprsPortfolioDetails = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lpfprsResponseStatus :: Lens' ListPortfoliosForProductResponse Int
lpfprsResponseStatus =
  lens _lpfprsResponseStatus (\s a -> s {_lpfprsResponseStatus = a})
