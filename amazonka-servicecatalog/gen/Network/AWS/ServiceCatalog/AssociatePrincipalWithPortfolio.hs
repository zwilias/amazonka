...
instance AWSRequest AssociatePrincipalWithPortfolio where
  type Rs AssociatePrincipalWithPortfolio = AssociatePrincipalWithPortfolioResponse
...
instance ToHeaders AssociatePrincipalWithPortfolio where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.AssociatePrincipalWithPortfolio" :: ByteString)
...
apwprsResponseStatus =
  lens _apwprsResponseStatus (\s a -> s {_apwprsResponseStatus = a})
