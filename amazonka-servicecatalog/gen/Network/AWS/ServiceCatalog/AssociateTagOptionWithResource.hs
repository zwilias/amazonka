...
instance AWSRequest AssociateTagOptionWithResource where
  type Rs AssociateTagOptionWithResource = AssociateTagOptionWithResourceResponse
  request = postJSON serviceCatalog
  response =
    receiveEmpty
      (\s h x -> AssociateTagOptionWithResourceResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AssociateTagOptionWithResource where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.AssociateTagOptionWithResource" :: ByteString)
...
atowrrsResponseStatus =
  lens _atowrrsResponseStatus (\s a -> s {_atowrrsResponseStatus = a})
