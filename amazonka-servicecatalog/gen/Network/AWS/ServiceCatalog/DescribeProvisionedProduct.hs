...
deseAcceptLanguage =
  lens _deseAcceptLanguage (\s a -> s {_deseAcceptLanguage = a})
...
instance AWSRequest DescribeProvisionedProduct where
  type Rs DescribeProvisionedProduct = DescribeProvisionedProductResponse
...
instance ToHeaders DescribeProvisionedProduct where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.DescribeProvisionedProduct" :: ByteString)
...
instance ToJSON DescribeProvisionedProduct where
  toJSON DescribeProvisionedProduct' {..} =
    object
      (catMaybes
         [("AcceptLanguage" .=) <$> _deseAcceptLanguage, Just ("Id" .= _deseId)])
...
drsProvisionedProductDetail ::
     Lens' DescribeProvisionedProductResponse (Maybe ProvisionedProductDetail)
drsProvisionedProductDetail =
  lens
    _drsProvisionedProductDetail
    (\s a -> s {_drsProvisionedProductDetail = a})

-- | Any CloudWatch dashboards that were created when provisioning the product.
drsCloudWatchDashboards ::
     Lens' DescribeProvisionedProductResponse [CloudWatchDashboard]
drsCloudWatchDashboards =
  lens _drsCloudWatchDashboards (\s a -> s {_drsCloudWatchDashboards = a}) .
  _Default . _Coerce
