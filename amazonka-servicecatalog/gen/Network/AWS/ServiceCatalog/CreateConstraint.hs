...
ccIdempotencyToken =
  lens _ccIdempotencyToken (\s a -> s {_ccIdempotencyToken = a})
...
instance AWSRequest CreateConstraint where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateConstraintResponse' <$> (x .?> "Status") <*>
         (x .?> "ConstraintDetail") <*>
...
ccrsConstraintDetail =
  lens _ccrsConstraintDetail (\s a -> s {_ccrsConstraintDetail = a})

-- | The constraint parameters.
ccrsConstraintParameters :: Lens' CreateConstraintResponse (Maybe Text)
ccrsConstraintParameters =
  lens _ccrsConstraintParameters (\s a -> s {_ccrsConstraintParameters = a})
