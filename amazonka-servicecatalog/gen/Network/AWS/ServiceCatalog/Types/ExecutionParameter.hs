...
data ExecutionParameter = ExecutionParameter'{_epDefaultValues
                                              :: !(Maybe [Text]),
                                              _epName :: !(Maybe Text),
                                              _epType :: !(Maybe Text)}
...
executionParameter
    :: ExecutionParameter
executionParameter
  = ExecutionParameter'{_epDefaultValues = Nothing,
                        _epName = Nothing, _epType = Nothing}
...
instance FromJSON ExecutionParameter where
        parseJSON
          = withObject "ExecutionParameter"
              (\ x ->
                 ExecutionParameter' <$>
                   (x .:? "DefaultValues" .!= mempty) <*> (x .:? "Name")
                     <*> (x .:? "Type"))

instance Hashable ExecutionParameter where

instance NFData ExecutionParameter where
