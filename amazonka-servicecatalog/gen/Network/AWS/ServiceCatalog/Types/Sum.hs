...
instance FromText AccessLevelFilterKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AccessLevelFilterKey from value: '" <>
        e <> "'. Accepted values: account, role, user"
...
instance FromText CopyOption where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CopyOption from value: '" <>
        e <> "'. Accepted values: copytags"
...
instance FromText CopyProductStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CopyProductStatus from value: '" <>
        e <> "'. Accepted values: failed, in_progress, succeeded"
...
instance FromText ProductSource where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProductSource from value: '" <>
        e <> "'. Accepted values: account"
...
instance FromText ProductType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProductType from value: '" <>
        e <> "'. Accepted values: cloud_formation_template, marketplace"
...
instance FromText ProductViewFilterBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProductViewFilterBy from value: '" <>
        e <>
...
instance FromText ProductViewSortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProductViewSortBy from value: '" <>
        e <> "'. Accepted values: creationdate, title, versioncount"
...
instance FromText ProvisionedProductPlanStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProvisionedProductPlanStatus from value: '" <>
        e <>
...
instance FromText ProvisionedProductPlanType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProvisionedProductPlanType from value: '" <>
        e <> "'. Accepted values: cloudformation"
...
instance FromText ProvisionedProductStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProvisionedProductStatus from value: '" <>
        e <>
...
instance FromText ProvisionedProductViewFilterBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProvisionedProductViewFilterBy from value: '" <>
        e <> "'. Accepted values: searchquery"
...
instance FromText ProvisioningArtifactPropertyName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProvisioningArtifactPropertyName from value: '" <>
        e <> "'. Accepted values: id"
...
instance FromText ProvisioningArtifactType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProvisioningArtifactType from value: '" <>
        e <>
...
instance FromText RecordStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RecordStatus from value: '" <>
        e <>
...
instance FromText RequestStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RequestStatus from value: '" <>
        e <> "'. Accepted values: available, creating, failed"
