...
ppNotificationARNs =
  lens _ppNotificationARNs (\s a -> s {_ppNotificationARNs = a}) .
  _Default . _Coerce
...
ppProvisioningParameters =
  lens _ppProvisioningParameters (\s a -> s {_ppProvisioningParameters = a}) .
  _Default . _Coerce
...
ppProvisioningArtifactId =
  lens _ppProvisioningArtifactId (\s a -> s {_ppProvisioningArtifactId = a})

-- | A user-friendly name for the provisioned product. This value must be unique for the AWS account and cannot be updated after the product is provisioned.
ppProvisionedProductName :: Lens' ProvisionProduct Text
ppProvisionedProductName =
  lens _ppProvisionedProductName (\s a -> s {_ppProvisionedProductName = a})
...
instance AWSRequest ProvisionProduct where
  ...
  response =
    receiveJSON
      (\s h x ->
         ProvisionProductResponse' <$> (x .?> "RecordDetail") <*>
         (pure (fromEnum s)))
  ...
  toJSON ProvisionProduct' {..} =
    object
      (catMaybes
         ...
         , Just ("ProvisioningArtifactId" .= _ppProvisioningArtifactId)
