...
tppProvisionedProductName =
  lens _tppProvisionedProductName (\s a -> s {_tppProvisionedProductName = a})
...
tppProvisionedProductId =
  lens _tppProvisionedProductId (\s a -> s {_tppProvisionedProductId = a})
...
instance AWSRequest TerminateProvisionedProduct where
  type Rs TerminateProvisionedProduct = TerminateProvisionedProductResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         TerminateProvisionedProductResponse' <$> (x .?> "RecordDetail") <*>
         (pure (fromEnum s)))
...
instance ToHeaders TerminateProvisionedProduct where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.TerminateProvisionedProduct" :: ByteString)
...
tpprsRecordDetail ::
     Lens' TerminateProvisionedProductResponse (Maybe RecordDetail)
...
tpprsResponseStatus =
  lens _tpprsResponseStatus (\s a -> s {_tpprsResponseStatus = a})
