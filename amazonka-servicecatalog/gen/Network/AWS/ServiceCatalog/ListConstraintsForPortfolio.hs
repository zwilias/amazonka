...
lcfpAcceptLanguage =
  lens _lcfpAcceptLanguage (\s a -> s {_lcfpAcceptLanguage = a})
...
instance AWSPager ListConstraintsForPortfolio where
  page rq rs
    ...
    | otherwise = Just $ rq & lcfpPageToken .~ rs ^. lcfprsNextPageToken

instance AWSRequest ListConstraintsForPortfolio where
  type Rs ListConstraintsForPortfolio = ListConstraintsForPortfolioResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         ListConstraintsForPortfolioResponse' <$> (x .?> "NextPageToken") <*>
...
instance ToHeaders ListConstraintsForPortfolio where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.ListConstraintsForPortfolio" :: ByteString)
...
lcfprsNextPageToken =
  lens _lcfprsNextPageToken (\s a -> s {_lcfprsNextPageToken = a})

-- | Information about the constraints.
lcfprsConstraintDetails ::
     Lens' ListConstraintsForPortfolioResponse [ConstraintDetail]
lcfprsConstraintDetails =
  lens _lcfprsConstraintDetails (\s a -> s {_lcfprsConstraintDetails = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lcfprsResponseStatus :: Lens' ListConstraintsForPortfolioResponse Int
lcfprsResponseStatus =
  lens _lcfprsResponseStatus (\s a -> s {_lcfprsResponseStatus = a})
