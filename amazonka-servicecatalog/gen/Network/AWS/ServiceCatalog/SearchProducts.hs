...
instance AWSRequest SearchProducts where
  ...
  response =
    receiveJSON
      (\s h x ->
         SearchProductsResponse' <$> (x .?> "NextPageToken") <*>
...
sprsProductViewAggregations ::
     Lens' SearchProductsResponse (HashMap Text [ProductViewAggregationValue])
sprsProductViewAggregations =
  lens
...
sprsProductViewSummaries =
  lens _sprsProductViewSummaries (\s a -> s {_sprsProductViewSummaries = a}) .
  _Default . _Coerce
