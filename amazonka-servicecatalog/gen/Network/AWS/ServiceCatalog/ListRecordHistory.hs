...
lrhAccessLevelFilter =
  lens _lrhAccessLevelFilter (\s a -> s {_lrhAccessLevelFilter = a})
...
instance AWSRequest ListRecordHistory where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListRecordHistoryResponse' <$> (x .?> "NextPageToken") <*>
...
lrhrsNextPageToken =
  lens _lrhrsNextPageToken (\s a -> s {_lrhrsNextPageToken = a})

-- | The records, in reverse chronological order.
lrhrsRecordDetails :: Lens' ListRecordHistoryResponse [RecordDetail]
lrhrsRecordDetails =
  lens _lrhrsRecordDetails (\s a -> s {_lrhrsRecordDetails = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lrhrsResponseStatus :: Lens' ListRecordHistoryResponse Int
lrhrsResponseStatus =
  lens _lrhrsResponseStatus (\s a -> s {_lrhrsResponseStatus = a})
