...
instance AWSRequest DescribeTagOption where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeTagOptionResponse' <$> (x .?> "TagOptionDetail") <*>
         (pure (fromEnum s)))
...
instance ToJSON DescribeTagOption where
  toJSON DescribeTagOption' {..} = object (catMaybes [Just ("Id" .= _dtoId)])
...
dtorsTagOptionDetail =
  lens _dtorsTagOptionDetail (\s a -> s {_dtorsTagOptionDetail = a})

-- | -- | The response status code.
dtorsResponseStatus :: Lens' DescribeTagOptionResponse Int
dtorsResponseStatus =
  lens _dtorsResponseStatus (\s a -> s {_dtorsResponseStatus = a})
