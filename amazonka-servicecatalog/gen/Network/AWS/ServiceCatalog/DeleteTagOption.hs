...
instance AWSRequest DeleteTagOption where
  ...
  response =
    receiveEmpty (\s h x -> DeleteTagOptionResponse' <$> (pure (fromEnum s)))
...
instance ToJSON DeleteTagOption where
  toJSON DeleteTagOption' {..} = object (catMaybes [Just ("Id" .= _delId)])
...
dtotrsResponseStatus =
  lens _dtotrsResponseStatus (\s a -> s {_dtotrsResponseStatus = a})
