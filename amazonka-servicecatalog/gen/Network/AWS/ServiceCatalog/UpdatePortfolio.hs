...
uRemoveTags =
  lens _uRemoveTags (\s a -> s {_uRemoveTags = a}) . _Default . _Coerce
...
instance AWSRequest UpdatePortfolio where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdatePortfolioResponse' <$> (x .?> "PortfolioDetail") <*>
         (x .?> "Tags" .!@ mempty) <*>
         (pure (fromEnum s)))
...
uprsPortfolioDetail =
  lens _uprsPortfolioDetail (\s a -> s {_uprsPortfolioDetail = a})
