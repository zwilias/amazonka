...
sppFilters ::
     Lens' SearchProvisionedProducts (HashMap ProvisionedProductViewFilterBy [Text])
...
sppAccessLevelFilter ::
     Lens' SearchProvisionedProducts (Maybe AccessLevelFilter)
sppAccessLevelFilter =
  lens _sppAccessLevelFilter (\s a -> s {_sppAccessLevelFilter = a})
...
instance AWSRequest SearchProvisionedProducts where
  type Rs SearchProvisionedProducts = SearchProvisionedProductsResponse
  request = postJSON serviceCatalog
  response =
    receiveJSON
      (\s h x ->
         SearchProvisionedProductsResponse' <$> (x .?> "NextPageToken") <*>
...
instance ToHeaders SearchProvisionedProducts where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.SearchProvisionedProducts" :: ByteString)
...
srsProvisionedProducts ::
     Lens' SearchProvisionedProductsResponse [ProvisionedProductAttribute]
srsProvisionedProducts =
  lens _srsProvisionedProducts (\s a -> s {_srsProvisionedProducts = a}) .
  _Default . _Coerce

-- | The number of provisioned products found.
srsTotalResultsCount :: Lens' SearchProvisionedProductsResponse (Maybe Int)
srsTotalResultsCount =
  lens _srsTotalResultsCount (\s a -> s {_srsTotalResultsCount = a})
