...
dpapAcceptLanguage =
  lens _dpapAcceptLanguage (\s a -> s {_dpapAcceptLanguage = a})
...
dpapProvisioningArtifactId =
  lens _dpapProvisioningArtifactId (\s a -> s {_dpapProvisioningArtifactId = a})

instance AWSRequest DeleteProvisioningArtifact where
  type Rs DeleteProvisioningArtifact = DeleteProvisioningArtifactResponse
  request = postJSON serviceCatalog
  response =
    receiveEmpty
      (\s h x -> DeleteProvisioningArtifactResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteProvisioningArtifact where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.DeleteProvisioningArtifact" :: ByteString)
...
dparsResponseStatus =
  lens _dparsResponseStatus (\s a -> s {_dparsResponseStatus = a})
