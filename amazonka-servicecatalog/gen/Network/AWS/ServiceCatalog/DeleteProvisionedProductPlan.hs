...
instance AWSRequest DeleteProvisionedProductPlan where
  type Rs DeleteProvisionedProductPlan = DeleteProvisionedProductPlanResponse
  request = postJSON serviceCatalog
  response =
    receiveEmpty
      (\s h x -> DeleteProvisionedProductPlanResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteProvisionedProductPlan where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.DeleteProvisionedProductPlan" :: ByteString)
...
newtype DeleteProvisionedProductPlanResponse =
  DeleteProvisionedProductPlanResponse'
...
dppprsResponseStatus =
  lens _dppprsResponseStatus (\s a -> s {_dppprsResponseStatus = a})
