...
instance AWSRequest DisassociateTagOptionFromResource where
  type Rs DisassociateTagOptionFromResource = DisassociateTagOptionFromResourceResponse
...
instance ToHeaders DisassociateTagOptionFromResource where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWS242ServiceCatalogService.DisassociateTagOptionFromResource" :: ByteString)
...
dtofrrsResponseStatus =
  lens _dtofrrsResponseStatus (\s a -> s {_dtofrrsResponseStatus = a})
