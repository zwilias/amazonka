...
cpSupportDescription =
  lens _cpSupportDescription (\s a -> s {_cpSupportDescription = a})
...
cpProvisioningArtifactParameters ::
     Lens' CreateProduct ProvisioningArtifactProperties
cpProvisioningArtifactParameters =
  lens
    _cpProvisioningArtifactParameters
    (\s a -> s {_cpProvisioningArtifactParameters = a})

-- | A unique identifier that you provide to ensure idempotency. If multiple requests differ only by the idempotency token, the same response is returned for each repeated request.
cpIdempotencyToken :: Lens' CreateProduct Text
cpIdempotencyToken =
  lens _cpIdempotencyToken (\s a -> s {_cpIdempotencyToken = a})
...
instance AWSRequest CreateProduct where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateProductResponse' <$> (x .?> "ProductViewDetail") <*>
...
cprsProductViewDetail =
  lens _cprsProductViewDetail (\s a -> s {_cprsProductViewDetail = a})

-- | Information about the provisioning artifact.
cprsProvisioningArtifactDetail ::
     Lens' CreateProductResponse (Maybe ProvisioningArtifactDetail)
cprsProvisioningArtifactDetail =
  lens
    _cprsProvisioningArtifactDetail
    (\s a -> s {_cprsProvisioningArtifactDetail = a})
