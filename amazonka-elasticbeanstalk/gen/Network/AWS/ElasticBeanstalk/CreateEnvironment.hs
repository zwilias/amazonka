...
cSolutionStackName =
  lens _cSolutionStackName (\s a -> s {_cSolutionStackName = a})
...
instance AWSRequest CreateEnvironment where
  ...
  response = receiveXMLWrapper "CreateEnvironmentResult" (\s h x -> parseXML x)
...
instance ToQuery CreateEnvironment where
  toQuery CreateEnvironment' {..} =
    mconcat
      ...
      , "OptionSettings" =: toQuery (toQueryList "member" <$> _cOptionSettings)
