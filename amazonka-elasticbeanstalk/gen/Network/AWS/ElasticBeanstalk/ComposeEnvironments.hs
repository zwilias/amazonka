...
ceVersionLabels =
  lens _ceVersionLabels (\s a -> s {_ceVersionLabels = a}) . _Default . _Coerce
...
instance AWSRequest ComposeEnvironments where
  ...
  response =
    receiveXMLWrapper "ComposeEnvironmentsResult" (\s h x -> parseXML x)
...
instance ToQuery ComposeEnvironments where
  toQuery ComposeEnvironments' {..} =
    mconcat
      ...
      , "VersionLabels" =: toQuery (toQueryList "member" <$> _ceVersionLabels)
