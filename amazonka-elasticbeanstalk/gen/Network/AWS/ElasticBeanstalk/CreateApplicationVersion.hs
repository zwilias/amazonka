...
cavAutoCreateApplication =
  lens _cavAutoCreateApplication (\s a -> s {_cavAutoCreateApplication = a})

-- | Specify a commit in an AWS CodeCommit Git repository to use as the source code for the application version.
cavSourceBuildInformation ::
     Lens' CreateApplicationVersion (Maybe SourceBuildInformation)
cavSourceBuildInformation =
  lens _cavSourceBuildInformation (\s a -> s {_cavSourceBuildInformation = a})
...
cavBuildConfiguration ::
     Lens' CreateApplicationVersion (Maybe BuildConfiguration)
cavBuildConfiguration =
  lens _cavBuildConfiguration (\s a -> s {_cavBuildConfiguration = a})
...
instance AWSRequest CreateApplicationVersion where
  type Rs CreateApplicationVersion = ApplicationVersionDescriptionMessage
  request = postQuery elasticBeanstalk
  response =
    receiveXMLWrapper "CreateApplicationVersionResult" (\s h x -> parseXML x)
