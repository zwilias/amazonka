...
instance AWSRequest RetrieveEnvironmentInfo where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         RetrieveEnvironmentInfoResponse' <$>
         (x .@? "EnvironmentInfo" .!@ mempty >>= may (parseXMLList "member")) <*>
         (pure (fromEnum s)))
...
reirsEnvironmentInfo ::
     Lens' RetrieveEnvironmentInfoResponse [EnvironmentInfoDescription]
reirsEnvironmentInfo =
...
reirsResponseStatus =
  lens _reirsResponseStatus (\s a -> s {_reirsResponseStatus = a})
