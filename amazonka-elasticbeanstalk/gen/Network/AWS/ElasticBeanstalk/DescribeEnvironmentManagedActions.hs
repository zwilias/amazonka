...
demaEnvironmentName =
  lens _demaEnvironmentName (\s a -> s {_demaEnvironmentName = a})
...
instance AWSRequest DescribeEnvironmentManagedActions where
  type Rs DescribeEnvironmentManagedActions = DescribeEnvironmentManagedActionsResponse
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeEnvironmentManagedActionsResponse' <$>
         (x .@? "ManagedActions" .!@ mempty >>= may (parseXMLList1 "member")) <*>
         (pure (fromEnum s)))
...
demarsManagedActions ::
     Lens' DescribeEnvironmentManagedActionsResponse (Maybe (NonEmpty ManagedAction))
demarsManagedActions =
  lens _demarsManagedActions (\s a -> s {_demarsManagedActions = a}) .
  mapping _List1

-- | -- | The response status code.
demarsResponseStatus :: Lens' DescribeEnvironmentManagedActionsResponse Int
demarsResponseStatus =
  lens _demarsResponseStatus (\s a -> s {_demarsResponseStatus = a})
