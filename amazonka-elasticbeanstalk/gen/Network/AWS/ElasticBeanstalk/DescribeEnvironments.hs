...
desEnvironmentIds =
  lens _desEnvironmentIds (\s a -> s {_desEnvironmentIds = a}) .
  _Default . _Coerce

-- | If specified, AWS Elastic Beanstalk restricts the returned descriptions to include only those that have the specified names.
desEnvironmentNames :: Lens' DescribeEnvironments [Text]
desEnvironmentNames =
  lens _desEnvironmentNames (\s a -> s {_desEnvironmentNames = a}) .
  _Default . _Coerce
...
desMaxRecords =
  lens _desMaxRecords (\s a -> s {_desMaxRecords = a}) . mapping _Nat

-- | If specified, AWS Elastic Beanstalk restricts the returned descriptions to include only those that are associated with this application.
desApplicationName :: Lens' DescribeEnvironments (Maybe Text)
desApplicationName =
  lens _desApplicationName (\s a -> s {_desApplicationName = a})

-- | If specified when @IncludeDeleted@ is set to @true@ , then environments deleted after this date are displayed.
desIncludedDeletedBackTo :: Lens' DescribeEnvironments (Maybe UTCTime)
desIncludedDeletedBackTo =
  lens _desIncludedDeletedBackTo (\s a -> s {_desIncludedDeletedBackTo = a}) .
  mapping _Time
...
instance AWSRequest DescribeEnvironments where
  ...
  response =
    receiveXMLWrapper "DescribeEnvironmentsResult" (\s h x -> parseXML x)
