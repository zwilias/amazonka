...
cpvOptionSettings =
  lens _cpvOptionSettings (\s a -> s {_cpvOptionSettings = a}) .
  _Default . _Coerce

-- | The name of the builder environment.
cpvEnvironmentName :: Lens' CreatePlatformVersion (Maybe Text)
cpvEnvironmentName =
  lens _cpvEnvironmentName (\s a -> s {_cpvEnvironmentName = a})
...
cpvPlatformVersion =
  lens _cpvPlatformVersion (\s a -> s {_cpvPlatformVersion = a})
...
cpvPlatformDefinitionBundle =
  lens
    ...
    (\s a -> s {_cpvPlatformDefinitionBundle = a})
...
instance AWSRequest CreatePlatformVersion where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreatePlatformVersionResponse' <$> (x .@? "Builder") <*>
         (x .@? "PlatformSummary") <*>
         (pure (fromEnum s)))
...
cpvrsPlatformSummary ::
     Lens' CreatePlatformVersionResponse (Maybe PlatformSummary)
cpvrsPlatformSummary =
  lens _cpvrsPlatformSummary (\s a -> s {_cpvrsPlatformSummary = a})

-- | -- | The response status code.
cpvrsResponseStatus :: Lens' CreatePlatformVersionResponse Int
cpvrsResponseStatus =
  lens _cpvrsResponseStatus (\s a -> s {_cpvrsResponseStatus = a})
