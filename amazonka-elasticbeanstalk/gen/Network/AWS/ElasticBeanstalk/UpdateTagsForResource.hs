...
utfrTagsToRemove =
  lens _utfrTagsToRemove (\s a -> s {_utfrTagsToRemove = a}) .
  _Default . _Coerce

-- | A list of tags to add or update. If a key of an existing tag is added, the tag's value is updated.
utfrTagsToAdd :: Lens' UpdateTagsForResource [Tag]
utfrTagsToAdd =
  lens _utfrTagsToAdd (\s a -> s {_utfrTagsToAdd = a}) . _Default . _Coerce
...
instance ToQuery UpdateTagsForResource where
  toQuery UpdateTagsForResource' {..} =
    mconcat
      ...
      , "TagsToRemove" =: toQuery (toQueryList "member" <$> _utfrTagsToRemove)
