...
instance AWSRequest DeletePlatformVersion where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DeletePlatformVersionResponse' <$> (x .@? "PlatformSummary") <*>
         (pure (fromEnum s)))
...
dpvrsPlatformSummary ::
     Lens' DeletePlatformVersionResponse (Maybe PlatformSummary)
dpvrsPlatformSummary =
  lens _dpvrsPlatformSummary (\s a -> s {_dpvrsPlatformSummary = a})
