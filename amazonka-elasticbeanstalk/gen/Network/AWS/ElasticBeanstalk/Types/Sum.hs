...
instance FromText ActionHistoryStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionHistoryStatus from value: '" <>
        e <> "'. Accepted values: completed, failed, unknown"
...
instance FromText ActionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionStatus from value: '" <>
        e <> "'. Accepted values: pending, running, scheduled, unknown"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: instancerefresh, platformupdate, unknown"
...
instance FromText ApplicationVersionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ApplicationVersionStatus from value: '" <>
        e <>
...
instance FromText ConfigurationDeploymentStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConfigurationDeploymentStatus from value: '" <>
        e <> "'. Accepted values: deployed, failed, pending"
...
instance FromText ConfigurationOptionValueType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConfigurationOptionValueType from value: '" <>
        e <> "'. Accepted values: list, scalar"
...
instance FromText EnvironmentHealth where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EnvironmentHealth from value: '" <>
        e <> "'. Accepted values: green, grey, red, yellow"
...
instance FromText EnvironmentHealthAttribute where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EnvironmentHealthAttribute from value: '" <>
        e <>
...
instance FromText EnvironmentHealthStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EnvironmentHealthStatus from value: '" <>
        e <>
...
instance FromText EnvironmentInfoType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EnvironmentInfoType from value: '" <>
        e <> "'. Accepted values: bundle, tail"
...
instance FromText EnvironmentStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EnvironmentStatus from value: '" <>
        e <>
...
instance FromText EventSeverity where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventSeverity from value: '" <>
        e <> "'. Accepted values: debug, error, fatal, info, trace, warn"
...
instance FromText FailureType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FailureType from value: '" <>
        e <>
...
instance FromText InstancesHealthAttribute where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing InstancesHealthAttribute from value: '" <>
        e <>
...
instance FromText PlatformStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PlatformStatus from value: '" <>
        e <> "'. Accepted values: creating, deleted, deleting, failed, ready"
...
instance FromText SourceRepository where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SourceRepository from value: '" <>
        e <> "'. Accepted values: codecommit, s3"
...
instance FromText ValidationSeverity where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ValidationSeverity from value: '" <>
        e <> "'. Accepted values: error, warning"
