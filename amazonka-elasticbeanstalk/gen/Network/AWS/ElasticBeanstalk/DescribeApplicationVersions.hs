...
dVersionLabels =
  lens _dVersionLabels (\s a -> s {_dVersionLabels = a}) . _Default . _Coerce
...
instance AWSRequest DescribeApplicationVersions where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeApplicationVersionsResponse' <$>
         (x .@? "ApplicationVersions" .!@ mempty >>= may (parseXMLList "member")) <*>
         (x .@? "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery DescribeApplicationVersions where
  toQuery DescribeApplicationVersions' {..} =
    mconcat
      ...
      , "VersionLabels" =: toQuery (toQueryList "member" <$> _dVersionLabels)
...
davrsApplicationVersions ::
     Lens' DescribeApplicationVersionsResponse [ApplicationVersionDescription]
davrsApplicationVersions =
  lens _davrsApplicationVersions (\s a -> s {_davrsApplicationVersions = a}) .
  _Default . _Coerce
...
davrsResponseStatus =
  lens _davrsResponseStatus (\s a -> s {_davrsResponseStatus = a})
