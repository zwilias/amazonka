...
instance AWSRequest CheckDNSAvailability where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CheckDNSAvailabilityResponse' <$> (x .@? "FullyQualifiedCNAME") <*>
         (x .@? "Available") <*>
         (pure (fromEnum s)))
...
cdarsFullyQualifiedCNAME =
  lens _cdarsFullyQualifiedCNAME (\s a -> s {_cdarsFullyQualifiedCNAME = a})
...
cdarsResponseStatus =
  lens _cdarsResponseStatus (\s a -> s {_cdarsResponseStatus = a})
