...
lpvMaxRecords =
  lens _lpvMaxRecords (\s a -> s {_lpvMaxRecords = a}) . mapping _Nat
...
instance AWSRequest ListPlatformVersions where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ListPlatformVersionsResponse' <$> (x .@? "NextToken") <*>
         (x .@? "PlatformSummaryList" .!@ mempty >>= may (parseXMLList "member")) <*>
         (pure (fromEnum s)))
...
lpvrsPlatformSummaryList =
  lens _lpvrsPlatformSummaryList (\s a -> s {_lpvrsPlatformSummaryList = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lpvrsResponseStatus :: Lens' ListPlatformVersionsResponse Int
lpvrsResponseStatus =
  lens _lpvrsResponseStatus (\s a -> s {_lpvrsResponseStatus = a})
