...
instance AWSRequest DescribePlatformVersion where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribePlatformVersionResponse' <$> (x .@? "PlatformDescription") <*>
         (pure (fromEnum s)))
...
drsPlatformDescription ::
     Lens' DescribePlatformVersionResponse (Maybe PlatformDescription)
drsPlatformDescription =
  lens _drsPlatformDescription (\s a -> s {_drsPlatformDescription = a})
