...
lsExclusiveStartStreamName =
  lens _lsExclusiveStartStreamName (\s a -> s {_lsExclusiveStartStreamName = a})
...
instance AWSPager ListStreams where
  page rq rs
    ...
    | otherwise =
      Just $ rq & lsExclusiveStartStreamName .~ rs ^? lsrsStreamNames . _last
...
instance AWSRequest ListStreams where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListStreamsResponse' <$> (pure (fromEnum s)) <*>
...
lsrsStreamNames =
  lens _lsrsStreamNames (\s a -> s {_lsrsStreamNames = a}) . _Coerce
