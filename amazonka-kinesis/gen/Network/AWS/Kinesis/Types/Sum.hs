...
instance FromText MetricsName where
  parser =
    takeLowerText >>= \case
      ...
      "writeprovisionedthroughputexceeded" ->
        pure WriteProvisionedThroughputExceeded
      e ->
        fromTextError $
        "Failure parsing MetricsName from value: '" <>
        e <>
...
instance FromText ScalingType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ScalingType from value: '" <>
        e <> "'. Accepted values: uniform_scaling"
...
instance FromText ShardIteratorType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ShardIteratorType from value: '" <>
        e <>
...
instance FromText StreamStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StreamStatus from value: '" <>
