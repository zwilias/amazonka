...
dExclusiveStartShardId =
  lens _dExclusiveStartShardId (\s a -> s {_dExclusiveStartShardId = a})
...
instance AWSPager DescribeStream where
  page rq rs
    ...
    | isNothing (rs ^? dsrsStreamDescription . sdShards . _last . sShardId) =
      Nothing
    | otherwise =
      Just $ rq & dExclusiveStartShardId .~ rs ^? dsrsStreamDescription .
      sdShards .
      _last .
      sShardId
...
instance AWSRequest DescribeStream where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeStreamResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "StreamDescription"))
...
instance ToHeaders DescribeStream where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Kinesis_20131202.DescribeStream" :: ByteString)
