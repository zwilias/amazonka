...
instance FromText AlgorithmicStemming where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AlgorithmicStemming from value: '" <>
        e <> "'. Accepted values: full, light, minimal, none"
...
instance FromText AnalysisSchemeLanguage where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AnalysisSchemeLanguage from value: '" <>
        e <>
...
instance FromText IndexFieldType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IndexFieldType from value: '" <>
        e <>
...
instance FromText PartitionInstanceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PartitionInstanceType from value: '" <>
        e <>
...
instance FromText SuggesterFuzzyMatching where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SuggesterFuzzyMatching from value: '" <>
        e <> "'. Accepted values: high, low, none"
