...
dasaAnalysisScheme =
  lens _dasaAnalysisScheme (\s a -> s {_dasaAnalysisScheme = a})
...
instance AWSRequest DefineAnalysisScheme where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DefineAnalysisSchemeResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "AnalysisScheme"))
...
defersResponseStatus =
  lens _defersResponseStatus (\s a -> s {_defersResponseStatus = a})
