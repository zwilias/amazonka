...
instance AWSRequest CreateUserPoolDomain where
  ...
  response =
    receiveEmpty
      (\s h x -> CreateUserPoolDomainResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders CreateUserPoolDomain where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.CreateUserPoolDomain" :: ByteString)
...
cupdrsResponseStatus =
  lens _cupdrsResponseStatus (\s a -> s {_cupdrsResponseStatus = a})
