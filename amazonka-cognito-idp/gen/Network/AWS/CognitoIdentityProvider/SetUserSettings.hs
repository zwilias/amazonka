...
susAccessToken =
  lens _susAccessToken (\s a -> s {_susAccessToken = a}) . _Sensitive
...
instance AWSRequest SetUserSettings where
  ...
  response =
    receiveEmpty (\s h x -> SetUserSettingsResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders SetUserSettings where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.SetUserSettings" :: ByteString)
...
susrsResponseStatus =
  lens _susrsResponseStatus (\s a -> s {_susrsResponseStatus = a})
