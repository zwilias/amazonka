...
audsDeviceRememberedStatus ::
     Lens' AdminUpdateDeviceStatus (Maybe DeviceRememberedStatusType)
audsDeviceRememberedStatus =
  lens _audsDeviceRememberedStatus (\s a -> s {_audsDeviceRememberedStatus = a})
...
instance AWSRequest AdminUpdateDeviceStatus where
  ...
  response =
    receiveEmpty
      (\s h x -> AdminUpdateDeviceStatusResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminUpdateDeviceStatus where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminUpdateDeviceStatus" :: ByteString)
...
audsrsResponseStatus =
  lens _audsrsResponseStatus (\s a -> s {_audsrsResponseStatus = a})
