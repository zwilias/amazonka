...
instance AWSRequest AdminConfirmSignUp where
  ...
  response =
    receiveEmpty (\s h x -> AdminConfirmSignUpResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminConfirmSignUp where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminConfirmSignUp" :: ByteString)
...
acsursResponseStatus =
  lens _acsursResponseStatus (\s a -> s {_acsursResponseStatus = a})
