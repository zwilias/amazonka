...
rccAnalyticsMetadata ::
     Lens' ResendConfirmationCode (Maybe AnalyticsMetadataType)
rccAnalyticsMetadata =
  lens _rccAnalyticsMetadata (\s a -> s {_rccAnalyticsMetadata = a})

-- | Contextual data such as the user's device fingerprint, IP address, or location used for evaluating the risk of an unexpected event by Amazon Cognito advanced security.
rccUserContextData :: Lens' ResendConfirmationCode (Maybe UserContextDataType)
rccUserContextData =
  lens _rccUserContextData (\s a -> s {_rccUserContextData = a})

-- | A keyed-hash message authentication code (HMAC) calculated using the secret key of a user pool client and username plus the client ID in the message.
rccSecretHash :: Lens' ResendConfirmationCode (Maybe Text)
rccSecretHash =
  lens _rccSecretHash (\s a -> s {_rccSecretHash = a}) . mapping _Sensitive
...
instance AWSRequest ResendConfirmationCode where
  ...
  response =
    receiveJSON
      (\s h x ->
         ResendConfirmationCodeResponse' <$> (x .?> "CodeDeliveryDetails") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ResendConfirmationCode where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.ResendConfirmationCode" :: ByteString)
...
rccrsCodeDeliveryDetails ::
     Lens' ResendConfirmationCodeResponse (Maybe CodeDeliveryDetailsType)
rccrsCodeDeliveryDetails =
  lens _rccrsCodeDeliveryDetails (\s a -> s {_rccrsCodeDeliveryDetails = a})
