...
iaClientMetadata =
  lens _iaClientMetadata (\s a -> s {_iaClientMetadata = a}) . _Default . _Map

-- | The Amazon Pinpoint analytics metadata for collecting metrics for @InitiateAuth@ calls.
iaAnalyticsMetadata :: Lens' InitiateAuth (Maybe AnalyticsMetadataType)
iaAnalyticsMetadata =
  lens _iaAnalyticsMetadata (\s a -> s {_iaAnalyticsMetadata = a})
...
iaAuthParameters =
  lens _iaAuthParameters (\s a -> s {_iaAuthParameters = a}) . _Default . _Map
...
instance AWSRequest InitiateAuth where
  ...
  response =
    receiveJSON
      (\s h x ->
         InitiateAuthResponse' <$> (x .?> "ChallengeName") <*>
...
iarsChallengeParameters =
  lens _iarsChallengeParameters (\s a -> s {_iarsChallengeParameters = a}) .
  _Default . _Map
...
iarsAuthenticationResult =
  lens _iarsAuthenticationResult (\s a -> s {_iarsAuthenticationResult = a})
...
iarsResponseStatus =
  lens _iarsResponseStatus (\s a -> s {_iarsResponseStatus = a})
