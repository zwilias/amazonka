...
lrsMaxResults =
  lens _lrsMaxResults (\s a -> s {_lrsMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListResourceServers where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListResourceServersResponse' <$> (x .?> "NextToken") <*>
         (pure (fromEnum s)) <*>
...
instance ToHeaders ListResourceServers where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.ListResourceServers" :: ByteString)
...
lrsrsResponseStatus =
  lens _lrsrsResponseStatus (\s a -> s {_lrsrsResponseStatus = a})

-- | The resource servers.
lrsrsResourceServers :: Lens' ListResourceServersResponse [ResourceServerType]
lrsrsResourceServers =
  lens _lrsrsResourceServers (\s a -> s {_lrsrsResourceServers = a}) . _Coerce
