...
csuConfirmationCode =
  lens _csuConfirmationCode (\s a -> s {_csuConfirmationCode = a})

instance AWSRequest ConfirmSignUp where
  type Rs ConfirmSignUp = ConfirmSignUpResponse
  request = postJSON cognitoIdentityProvider
  response =
    receiveEmpty (\s h x -> ConfirmSignUpResponse' <$> (pure (fromEnum s)))
...
csursResponseStatus =
  lens _csursResponseStatus (\s a -> s {_csursResponseStatus = a})
