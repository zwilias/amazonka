...
gsoAccessToken =
  lens _gsoAccessToken (\s a -> s {_gsoAccessToken = a}) . _Sensitive

instance AWSRequest GlobalSignOut where
  type Rs GlobalSignOut = GlobalSignOutResponse
  request = postJSON cognitoIdentityProvider
  response =
    receiveEmpty (\s h x -> GlobalSignOutResponse' <$> (pure (fromEnum s)))
...
gsorsResponseStatus =
  lens _gsorsResponseStatus (\s a -> s {_gsorsResponseStatus = a})
