...
aldPaginationToken =
  lens _aldPaginationToken (\s a -> s {_aldPaginationToken = a})
...
instance AWSRequest AdminListDevices where
  ...
  response =
    receiveJSON
      (\s h x ->
         AdminListDevicesResponse' <$> (x .?> "PaginationToken") <*>
         (x .?> "Devices" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders AdminListDevices where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminListDevices" :: ByteString)
...
aldrsPaginationToken =
  lens _aldrsPaginationToken (\s a -> s {_aldrsPaginationToken = a})

-- | The devices in the list of devices response.
aldrsDevices :: Lens' AdminListDevicesResponse [DeviceType]
aldrsDevices =
  lens _aldrsDevices (\s a -> s {_aldrsDevices = a}) . _Default . _Coerce

-- | -- | The response status code.
aldrsResponseStatus :: Lens' AdminListDevicesResponse Int
aldrsResponseStatus =
  lens _aldrsResponseStatus (\s a -> s {_aldrsResponseStatus = a})
