...
aduaUserAttributeNames =
  lens _aduaUserAttributeNames (\s a -> s {_aduaUserAttributeNames = a}) .
  _Coerce

instance AWSRequest AdminDeleteUserAttributes where
  type Rs AdminDeleteUserAttributes = AdminDeleteUserAttributesResponse
  request = postJSON cognitoIdentityProvider
  response =
    receiveEmpty
      (\s h x -> AdminDeleteUserAttributesResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminDeleteUserAttributes where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminDeleteUserAttributes" :: ByteString)
...
aduarsResponseStatus =
  lens _aduarsResponseStatus (\s a -> s {_aduarsResponseStatus = a})
