...
instance AWSRequest AdminEnableUser where
  ...
  response =
    receiveEmpty (\s h x -> AdminEnableUserResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminEnableUser where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminEnableUser" :: ByteString)
...
aeursResponseStatus =
  lens _aeursResponseStatus (\s a -> s {_aeursResponseStatus = a})
