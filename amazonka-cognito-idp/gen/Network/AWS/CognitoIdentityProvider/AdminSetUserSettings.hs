...
asusMFAOptions =
  lens _asusMFAOptions (\s a -> s {_asusMFAOptions = a}) . _Coerce
...
instance ToHeaders AdminSetUserSettings where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminSetUserSettings" :: ByteString)
...
asusrsResponseStatus =
  lens _asusrsResponseStatus (\s a -> s {_asusrsResponseStatus = a})
