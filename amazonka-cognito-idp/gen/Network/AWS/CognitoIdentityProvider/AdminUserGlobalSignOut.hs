...
augsoUsername =
  lens _augsoUsername (\s a -> s {_augsoUsername = a}) . _Sensitive
...
instance AWSRequest AdminUserGlobalSignOut where
  ...
  response =
    receiveEmpty
      (\s h x -> AdminUserGlobalSignOutResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminUserGlobalSignOut where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminUserGlobalSignOut" :: ByteString)
...
augsorsResponseStatus =
  lens _augsorsResponseStatus (\s a -> s {_augsorsResponseStatus = a})
