...
instance AWSRequest DescribeUserImportJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeUserImportJobResponse' <$> (x .?> "UserImportJob") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeUserImportJob where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.DescribeUserImportJob" :: ByteString)
...
instance ToJSON DescribeUserImportJob where
  toJSON DescribeUserImportJob' {..} =
    object
      (catMaybes
         [Just ("UserPoolId" .= _duijUserPoolId), Just ("JobId" .= _duijJobId)])
...
duijrsUserImportJob ::
     Lens' DescribeUserImportJobResponse (Maybe UserImportJobType)
duijrsUserImportJob =
  lens _duijrsUserImportJob (\s a -> s {_duijrsUserImportJob = a})

-- | -- | The response status code.
duijrsResponseStatus :: Lens' DescribeUserImportJobResponse Int
duijrsResponseStatus =
  lens _duijrsResponseStatus (\s a -> s {_duijrsResponseStatus = a})
