...
lipMaxResults =
  lens _lipMaxResults (\s a -> s {_lipMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListIdentityProviders where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListIdentityProvidersResponse' <$> (x .?> "NextToken") <*>
         (pure (fromEnum s)) <*>
         (x .?> "Providers" .!@ mempty))
...
instance ToHeaders ListIdentityProviders where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.ListIdentityProviders" :: ByteString)
...
liprsProviders =
  lens _liprsProviders (\s a -> s {_liprsProviders = a}) . _Coerce
