...
instance FromText AccountTakeoverEventActionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AccountTakeoverEventActionType from value: '" <>
        e <>
...
instance FromText AdvancedSecurityModeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AdvancedSecurityModeType from value: '" <>
        e <> "'. Accepted values: audit, enforced, off"
...
instance FromText AliasAttributeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AliasAttributeType from value: '" <>
        e <> "'. Accepted values: email, phone_number, preferred_username"
...
instance FromText AttributeDataType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AttributeDataType from value: '" <>
        e <> "'. Accepted values: boolean, datetime, number, string"
...
instance FromText AuthFlowType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuthFlowType from value: '" <>
        e <>
...
instance FromText ChallengeName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChallengeName from value: '" <>
        e <> "'. Accepted values: mfa, password"
...
instance FromText ChallengeNameType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChallengeNameType from value: '" <>
        e <>
...
instance FromText ChallengeResponse where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChallengeResponse from value: '" <>
        e <> "'. Accepted values: failure, success"
...
instance FromText CompromisedCredentialsEventActionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CompromisedCredentialsEventActionType from value: '" <>
        e <> "'. Accepted values: block, no_action"
...
instance FromText DefaultEmailOptionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DefaultEmailOptionType from value: '" <>
        e <> "'. Accepted values: confirm_with_code, confirm_with_link"
...
instance FromText DeliveryMediumType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeliveryMediumType from value: '" <>
        e <> "'. Accepted values: email, sms"
...
instance FromText DeviceRememberedStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeviceRememberedStatusType from value: '" <>
        e <> "'. Accepted values: not_remembered, remembered"
...
instance FromText DomainStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DomainStatusType from value: '" <>
        e <> "'. Accepted values: active, creating, deleting, failed, updating"
...
instance FromText EventFilterType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventFilterType from value: '" <>
        e <> "'. Accepted values: password_change, sign_in, sign_up"
...
instance FromText EventResponseType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventResponseType from value: '" <>
        e <> "'. Accepted values: failure, success"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: forgotpassword, signin, signup"
...
instance FromText ExplicitAuthFlowsType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExplicitAuthFlowsType from value: '" <>
        e <>
...
instance FromText FeedbackValueType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FeedbackValueType from value: '" <>
        e <> "'. Accepted values: invalid, valid"
...
instance FromText IdentityProviderTypeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IdentityProviderTypeType from value: '" <>
        e <> "'. Accepted values: facebook, google, loginwithamazon, saml"
...
instance FromText MessageActionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MessageActionType from value: '" <>
        e <> "'. Accepted values: resend, suppress"
...
instance FromText OAuthFlowType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OAuthFlowType from value: '" <>
        e <> "'. Accepted values: client_credentials, code, implicit"
...
instance FromText RiskDecisionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RiskDecisionType from value: '" <>
        e <> "'. Accepted values: accounttakeover, block, norisk"
...
instance FromText RiskLevelType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RiskLevelType from value: '" <>
...
instance FromText UserImportJobStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UserImportJobStatusType from value: '" <>
        e <>
...
instance FromText UserPoolMFAType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UserPoolMFAType from value: '" <>
        e <> "'. Accepted values: on, off, optional"
...
instance FromText UserStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UserStatusType from value: '" <>
        e <>
...
instance FromText UsernameAttributeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UsernameAttributeType from value: '" <>
        e <> "'. Accepted values: email, phone_number"
...
instance FromText VerifiedAttributeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VerifiedAttributeType from value: '" <>
        e <> "'. Accepted values: email, phone_number"
...
instance FromText VerifySoftwareTokenResponseType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VerifySoftwareTokenResponseType from value: '" <>
        e <> "'. Accepted values: error, success"
