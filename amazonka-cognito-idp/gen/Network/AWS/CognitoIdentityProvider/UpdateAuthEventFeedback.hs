...
uaefFeedbackToken =
  lens _uaefFeedbackToken (\s a -> s {_uaefFeedbackToken = a}) . _Sensitive
...
instance AWSRequest UpdateAuthEventFeedback where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateAuthEventFeedbackResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateAuthEventFeedback where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.UpdateAuthEventFeedback" :: ByteString)
...
uaefrsResponseStatus =
  lens _uaefrsResponseStatus (\s a -> s {_uaefrsResponseStatus = a})
