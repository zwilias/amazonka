...
asumpSMSMFASettings ::
     Lens' AdminSetUserMFAPreference (Maybe SMSMFASettingsType)
asumpSMSMFASettings =
  lens _asumpSMSMFASettings (\s a -> s {_asumpSMSMFASettings = a})

-- | The time-based one-time password software token MFA settings.
asumpSoftwareTokenMFASettings ::
     Lens' AdminSetUserMFAPreference (Maybe SoftwareTokenMFASettingsType)
asumpSoftwareTokenMFASettings =
  lens
    _asumpSoftwareTokenMFASettings
    (\s a -> s {_asumpSoftwareTokenMFASettings = a})

-- | The user pool username or alias.
asumpUsername :: Lens' AdminSetUserMFAPreference Text
asumpUsername =
  lens _asumpUsername (\s a -> s {_asumpUsername = a}) . _Sensitive
...
instance AWSRequest AdminSetUserMFAPreference where
  type Rs AdminSetUserMFAPreference = AdminSetUserMFAPreferenceResponse
  request = postJSON cognitoIdentityProvider
  response =
    receiveEmpty
      (\s h x -> AdminSetUserMFAPreferenceResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminSetUserMFAPreference where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminSetUserMFAPreference" :: ByteString)
...
asumprsResponseStatus =
  lens _asumprsResponseStatus (\s a -> s {_asumprsResponseStatus = a})
