...
drcClientId =
  lens _drcClientId (\s a -> s {_drcClientId = a}) . mapping _Sensitive
...
instance AWSRequest DescribeRiskConfiguration where
  type Rs DescribeRiskConfiguration = DescribeRiskConfigurationResponse
  request = postJSON cognitoIdentityProvider
  response =
    receiveJSON
      (\s h x ->
         DescribeRiskConfigurationResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "RiskConfiguration"))
...
instance ToHeaders DescribeRiskConfiguration where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.DescribeRiskConfiguration" :: ByteString)
...
drcrsRiskConfiguration ::
     Lens' DescribeRiskConfigurationResponse RiskConfigurationType
drcrsRiskConfiguration =
  lens _drcrsRiskConfiguration (\s a -> s {_drcrsRiskConfiguration = a})
