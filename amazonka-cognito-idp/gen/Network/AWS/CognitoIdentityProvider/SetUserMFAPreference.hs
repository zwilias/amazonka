...
sumpSMSMFASettings =
  lens _sumpSMSMFASettings (\s a -> s {_sumpSMSMFASettings = a})

-- | The time-based one-time password software token MFA settings.
sumpSoftwareTokenMFASettings ::
     Lens' SetUserMFAPreference (Maybe SoftwareTokenMFASettingsType)
sumpSoftwareTokenMFASettings =
  lens
    _sumpSoftwareTokenMFASettings
    (\s a -> s {_sumpSoftwareTokenMFASettings = a})

-- | The access token.
sumpAccessToken :: Lens' SetUserMFAPreference Text
sumpAccessToken =
  lens _sumpAccessToken (\s a -> s {_sumpAccessToken = a}) . _Sensitive
...
instance ToHeaders SetUserMFAPreference where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.SetUserMFAPreference" :: ByteString)
...
sumprsResponseStatus =
  lens _sumprsResponseStatus (\s a -> s {_sumprsResponseStatus = a})
