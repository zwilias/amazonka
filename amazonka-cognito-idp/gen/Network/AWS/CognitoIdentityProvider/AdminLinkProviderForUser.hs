...
alpfuDestinationUser =
  lens _alpfuDestinationUser (\s a -> s {_alpfuDestinationUser = a})
...
instance AWSRequest AdminLinkProviderForUser where
  ...
  response =
    receiveEmpty
      (\s h x -> AdminLinkProviderForUserResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminLinkProviderForUser where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminLinkProviderForUser" :: ByteString)
...
alpfursResponseStatus =
  lens _alpfursResponseStatus (\s a -> s {_alpfursResponseStatus = a})
