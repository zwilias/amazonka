...
guicClientId =
  lens _guicClientId (\s a -> s {_guicClientId = a}) . mapping _Sensitive
...
instance AWSRequest GetUICustomization where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetUICustomizationResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "UICustomization"))
...
instance ToHeaders GetUICustomization where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.GetUICustomization" :: ByteString)
...
guicrsResponseStatus =
  lens _guicrsResponseStatus (\s a -> s {_guicrsResponseStatus = a})

-- | The UI customization information.
guicrsUICustomization :: Lens' GetUICustomizationResponse UICustomizationType
guicrsUICustomization =
  lens _guicrsUICustomization (\s a -> s {_guicrsUICustomization = a})
