...
gipbiIdpIdentifier =
  lens _gipbiIdpIdentifier (\s a -> s {_gipbiIdpIdentifier = a})

instance AWSRequest GetIdentityProviderByIdentifier where
  type Rs GetIdentityProviderByIdentifier = GetIdentityProviderByIdentifierResponse
  request = postJSON cognitoIdentityProvider
  response =
    receiveJSON
      (\s h x ->
         GetIdentityProviderByIdentifierResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "IdentityProvider"))
...
instance ToHeaders GetIdentityProviderByIdentifier where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.GetIdentityProviderByIdentifier" :: ByteString)
...
gipbirsResponseStatus =
  lens _gipbirsResponseStatus (\s a -> s {_gipbirsResponseStatus = a})

-- | The identity provider object.
gipbirsIdentityProvider ::
     Lens' GetIdentityProviderByIdentifierResponse IdentityProviderType
gipbirsIdentityProvider =
  lens _gipbirsIdentityProvider (\s a -> s {_gipbirsIdentityProvider = a})
