...
duaUserAttributeNames =
  lens _duaUserAttributeNames (\s a -> s {_duaUserAttributeNames = a}) . _Coerce

-- | The access token used in the request to delete user attributes.
duaAccessToken :: Lens' DeleteUserAttributes Text
duaAccessToken =
  lens _duaAccessToken (\s a -> s {_duaAccessToken = a}) . _Sensitive
...
instance ToHeaders DeleteUserAttributes where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.DeleteUserAttributes" :: ByteString)
...
duarsResponseStatus =
  lens _duarsResponseStatus (\s a -> s {_duarsResponseStatus = a})
