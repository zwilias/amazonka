...
suicClientId =
  lens _suicClientId (\s a -> s {_suicClientId = a}) . mapping _Sensitive
...
suicImageFile =
  lens _suicImageFile (\s a -> s {_suicImageFile = a}) . mapping _Base64
...
instance AWSRequest SetUICustomization where
  ...
  response =
    receiveJSON
      (\s h x ->
         SetUICustomizationResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "UICustomization"))
...
instance ToHeaders SetUICustomization where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.SetUICustomization" :: ByteString)
...
suicrsResponseStatus =
  lens _suicrsResponseStatus (\s a -> s {_suicrsResponseStatus = a})

-- | The UI customization information.
suicrsUICustomization :: Lens' SetUICustomizationResponse UICustomizationType
suicrsUICustomization =
  lens _suicrsUICustomization (\s a -> s {_suicrsUICustomization = a})
