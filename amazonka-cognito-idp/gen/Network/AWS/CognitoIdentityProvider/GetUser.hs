...
guAccessToken =
  lens _guAccessToken (\s a -> s {_guAccessToken = a}) . _Sensitive
...
instance AWSRequest GetUser where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetUserResponse' <$> (x .?> "UserMFASettingList" .!@ mempty) <*>
         (x .?> "MFAOptions" .!@ mempty) <*>
...
gursUserMFASettingList =
  lens _gursUserMFASettingList (\s a -> s {_gursUserMFASettingList = a}) .
  _Default . _Coerce

-- | Specifies the options for MFA (e.g., email or phone number).
gursMFAOptions :: Lens' GetUserResponse [MFAOptionType]
gursMFAOptions =
  lens _gursMFAOptions (\s a -> s {_gursMFAOptions = a}) . _Default . _Coerce

-- | The user's preferred MFA setting.
gursPreferredMFASetting :: Lens' GetUserResponse (Maybe Text)
gursPreferredMFASetting =
  lens _gursPreferredMFASetting (\s a -> s {_gursPreferredMFASetting = a})

-- | -- | The response status code.
gursResponseStatus :: Lens' GetUserResponse Int
gursResponseStatus =
  lens _gursResponseStatus (\s a -> s {_gursResponseStatus = a})
...
gursUserAttributes =
  lens _gursUserAttributes (\s a -> s {_gursUserAttributes = a}) . _Coerce
