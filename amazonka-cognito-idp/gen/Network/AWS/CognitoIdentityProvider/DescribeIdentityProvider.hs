...
instance AWSRequest DescribeIdentityProvider where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeIdentityProviderResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "IdentityProvider"))
...
instance ToHeaders DescribeIdentityProvider where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.DescribeIdentityProvider" :: ByteString)
...
diprsResponseStatus =
  lens _diprsResponseStatus (\s a -> s {_diprsResponseStatus = a})

-- | The identity provider that was deleted.
diprsIdentityProvider ::
     Lens' DescribeIdentityProviderResponse IdentityProviderType
diprsIdentityProvider =
  lens _diprsIdentityProvider (\s a -> s {_diprsIdentityProvider = a})
