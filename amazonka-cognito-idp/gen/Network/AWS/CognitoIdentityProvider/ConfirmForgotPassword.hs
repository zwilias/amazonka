...
cfpAnalyticsMetadata ::
     Lens' ConfirmForgotPassword (Maybe AnalyticsMetadataType)
cfpAnalyticsMetadata =
  lens _cfpAnalyticsMetadata (\s a -> s {_cfpAnalyticsMetadata = a})

-- | Contextual data such as the user's device fingerprint, IP address, or location used for evaluating the risk of an unexpected event by Amazon Cognito advanced security.
cfpUserContextData :: Lens' ConfirmForgotPassword (Maybe UserContextDataType)
cfpUserContextData =
  lens _cfpUserContextData (\s a -> s {_cfpUserContextData = a})

-- | A keyed-hash message authentication code (HMAC) calculated using the secret key of a user pool client and username plus the client ID in the message.
cfpSecretHash :: Lens' ConfirmForgotPassword (Maybe Text)
cfpSecretHash =
  lens _cfpSecretHash (\s a -> s {_cfpSecretHash = a}) . mapping _Sensitive
...
cfpConfirmationCode =
  lens _cfpConfirmationCode (\s a -> s {_cfpConfirmationCode = a})
...
instance ToHeaders ConfirmForgotPassword where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.ConfirmForgotPassword" :: ByteString)
...
cfprsResponseStatus =
  lens _cfprsResponseStatus (\s a -> s {_cfprsResponseStatus = a})
