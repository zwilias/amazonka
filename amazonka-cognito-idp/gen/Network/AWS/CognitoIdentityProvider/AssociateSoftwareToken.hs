...
astAccessToken =
  lens _astAccessToken (\s a -> s {_astAccessToken = a}) . mapping _Sensitive
...
instance AWSRequest AssociateSoftwareToken where
  ...
  response =
    receiveJSON
      (\s h x ->
         AssociateSoftwareTokenResponse' <$> (x .?> "SecretCode") <*>
         (x .?> "Session") <*>
         (pure (fromEnum s)))
...
instance ToHeaders AssociateSoftwareToken where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AssociateSoftwareToken" :: ByteString)
...
astrsSecretCode =
  lens _astrsSecretCode (\s a -> s {_astrsSecretCode = a}) . mapping _Sensitive
...
astrsResponseStatus =
  lens _astrsResponseStatus (\s a -> s {_astrsResponseStatus = a})
