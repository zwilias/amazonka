...
cuijCloudWatchLogsRoleARN =
  lens _cuijCloudWatchLogsRoleARN (\s a -> s {_cuijCloudWatchLogsRoleARN = a})
...
instance AWSRequest CreateUserImportJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateUserImportJobResponse' <$> (x .?> "UserImportJob") <*>
         (pure (fromEnum s)))
...
instance ToHeaders CreateUserImportJob where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.CreateUserImportJob" :: ByteString)
...
cuijrsUserImportJob ::
     Lens' CreateUserImportJobResponse (Maybe UserImportJobType)
cuijrsUserImportJob =
  lens _cuijrsUserImportJob (\s a -> s {_cuijrsUserImportJob = a})

-- | -- | The response status code.
cuijrsResponseStatus :: Lens' CreateUserImportJobResponse Int
cuijrsResponseStatus =
  lens _cuijrsResponseStatus (\s a -> s {_cuijrsResponseStatus = a})
