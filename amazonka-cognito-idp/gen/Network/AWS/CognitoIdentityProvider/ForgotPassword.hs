...
fpAnalyticsMetadata =
  lens _fpAnalyticsMetadata (\s a -> s {_fpAnalyticsMetadata = a})
...
fpSecretHash =
  lens _fpSecretHash (\s a -> s {_fpSecretHash = a}) . mapping _Sensitive
...
instance AWSRequest ForgotPassword where
  ...
  response =
    receiveJSON
      (\s h x ->
         ForgotPasswordResponse' <$> (x .?> "CodeDeliveryDetails") <*>
         (pure (fromEnum s)))
...
fprsCodeDeliveryDetails ::
     Lens' ForgotPasswordResponse (Maybe CodeDeliveryDetailsType)
fprsCodeDeliveryDetails =
  lens _fprsCodeDeliveryDetails (\s a -> s {_fprsCodeDeliveryDetails = a})

-- | -- | The response status code.
fprsResponseStatus :: Lens' ForgotPasswordResponse Int
fprsResponseStatus =
  lens _fprsResponseStatus (\s a -> s {_fprsResponseStatus = a})
