...
instance AWSRequest AdminResetUserPassword where
  ...
  response =
    receiveEmpty
      (\s h x -> AdminResetUserPasswordResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AdminResetUserPassword where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityProviderService.AdminResetUserPassword" :: ByteString)
...
aruprsResponseStatus =
  lens _aruprsResponseStatus (\s a -> s {_aruprsResponseStatus = a})
