...
papStatements =
  lens _papStatements (\s a -> s {_papStatements = a}) . _Default . _Coerce
...
instance AWSRequest PutApplicationPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutApplicationPolicyResponse' <$> (x .?> "statements" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON PutApplicationPolicy where
  toJSON PutApplicationPolicy' {..} =
    object (catMaybes [("statements" .=) <$> _papStatements])
...
-- | Array of policy statements applied to the application.
paprsStatements ::
     Lens' PutApplicationPolicyResponse [ApplicationPolicyStatement]
paprsStatements =
  lens _paprsStatements (\s a -> s {_paprsStatements = a}) . _Default . _Coerce

-- | -- | The response status code.
paprsResponseStatus :: Lens' PutApplicationPolicyResponse Int
paprsResponseStatus =
  lens _paprsResponseStatus (\s a -> s {_paprsResponseStatus = a})
