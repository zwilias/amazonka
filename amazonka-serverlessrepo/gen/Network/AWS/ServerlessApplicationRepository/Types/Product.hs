...
instance FromJSON ApplicationPolicyStatement where
  parseJSON =
    withObject
      "ApplicationPolicyStatement"
      (\x ->
         ApplicationPolicyStatement' <$> (x .:? "statementId") <*>
         (x .:? "principals" .!= mempty) <*>
         (x .:? "actions" .!= mempty))
...
instance FromJSON ApplicationSummary where
  parseJSON =
    withObject
      "ApplicationSummary"
      (\x ->
         ApplicationSummary' <$> (x .:? "creationTime") <*>
         (x .:? "homePageUrl") <*>
         (x .:? "labels" .!= mempty) <*>
         (x .:? "spdxLicenseId") <*>
         (x .: "description") <*>
...
pdConstraintDescription =
  lens _pdConstraintDescription (\s a -> s {_pdConstraintDescription = a})
...
pdAllowedValues =
  lens _pdAllowedValues (\s a -> s {_pdAllowedValues = a}) . _Default . _Coerce
...
pdReferencedByResources =
  lens _pdReferencedByResources (\s a -> s {_pdReferencedByResources = a}) .
  _Coerce
...
instance FromJSON ParameterDefinition where
  parseJSON =
    withObject
      "ParameterDefinition"
      (\x ->
         ParameterDefinition' <$> (x .:? "maxValue") <*> (x .:? "maxLength") <*>
...
instance ToJSON ParameterValue where
  toJSON ParameterValue' {..} =
    object (catMaybes [Just ("value" .= _pvValue), Just ("name" .= _pvName)])
...
vParameterDefinitions =
  lens _vParameterDefinitions (\s a -> s {_vParameterDefinitions = a}) . _Coerce
...
instance FromJSON Version where
  parseJSON =
    withObject
      "Version"
      (\x ->
         Version' <$> (x .:? "sourceCodeUrl") <*> (x .: "templateUrl") <*>
...
instance FromJSON VersionSummary where
  parseJSON =
    withObject
      "VersionSummary"
      (\x ->
         VersionSummary' <$> (x .:? "sourceCodeUrl") <*> (x .: "creationTime") <*>
         (x .: "applicationId") <*>
         (x .: "semanticVersion"))
