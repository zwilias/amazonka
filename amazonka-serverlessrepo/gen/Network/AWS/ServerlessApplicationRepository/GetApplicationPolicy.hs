...
instance AWSRequest GetApplicationPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetApplicationPolicyResponse' <$> (x .?> "statements" .!@ mempty) <*>
         (pure (fromEnum s)))
...
gaprsStatements ::
     Lens' GetApplicationPolicyResponse [ApplicationPolicyStatement]
gaprsStatements =
  lens _gaprsStatements (\s a -> s {_gaprsStatements = a}) . _Default . _Coerce

-- | -- | The response status code.
gaprsResponseStatus :: Lens' GetApplicationPolicyResponse Int
gaprsResponseStatus =
  lens _gaprsResponseStatus (\s a -> s {_gaprsResponseStatus = a})
