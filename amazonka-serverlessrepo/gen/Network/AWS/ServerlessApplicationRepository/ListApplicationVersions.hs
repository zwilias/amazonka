...
instance AWSRequest ListApplicationVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListApplicationVersionsResponse' <$> (x .?> "versions" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListApplicationVersions where
  toQuery ListApplicationVersions' {..} =
    mconcat ["nextToken" =: _lavNextToken, "maxItems" =: _lavMaxItems]
...
lavrsVersions =
  lens _lavrsVersions (\s a -> s {_lavrsVersions = a}) . _Default . _Coerce
