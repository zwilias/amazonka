...
instance AWSRequest GetSecretValue where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetSecretValueResponse' <$> (x .?> "VersionId") <*> (x .?> "ARN") <*>
         (x .?> "VersionStages") <*>
         (x .?> "SecretBinary") <*>
         (x .?> "CreatedDate") <*>
         (x .?> "Name") <*>
...
gsvrsVersionStages =
  lens _gsvrsVersionStages (\s a -> s {_gsvrsVersionStages = a}) .
  mapping _List1
...
gsvrsSecretBinary =
  lens _gsvrsSecretBinary (\s a -> s {_gsvrsSecretBinary = a}) .
  mapping (_Sensitive . _Base64)

-- | The date and time that this version of the secret was created.
gsvrsCreatedDate :: Lens' GetSecretValueResponse (Maybe UTCTime)
gsvrsCreatedDate =
  lens _gsvrsCreatedDate (\s a -> s {_gsvrsCreatedDate = a}) . mapping _Time
...
gsvrsSecretString =
  lens _gsvrsSecretString (\s a -> s {_gsvrsSecretString = a}) .
  mapping _Sensitive

-- | -- | The response status code.
gsvrsResponseStatus :: Lens' GetSecretValueResponse Int
gsvrsResponseStatus =
  lens _gsvrsResponseStatus (\s a -> s {_gsvrsResponseStatus = a})
