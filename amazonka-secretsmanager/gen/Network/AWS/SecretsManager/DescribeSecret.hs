...
instance AWSRequest DescribeSecret where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeSecretResponse' <$> (x .?> "LastChangedDate") <*> (x .?> "ARN") <*>
         (x .?> "RotationRules") <*>
         (x .?> "DeletedDate") <*>
         (x .?> "RotationEnabled") <*>
         (x .?> "KmsKeyId") <*>
...
drsLastChangedDate =
  lens _drsLastChangedDate (\s a -> s {_drsLastChangedDate = a}) . mapping _Time
...
drsDeletedDate =
  lens _drsDeletedDate (\s a -> s {_drsDeletedDate = a}) . mapping _Time
...
drsRotationEnabled =
  lens _drsRotationEnabled (\s a -> s {_drsRotationEnabled = a})
...
drsVersionIdsToStages ::
     Lens' DescribeSecretResponse (HashMap Text (NonEmpty Text))
drsVersionIdsToStages =
  lens _drsVersionIdsToStages (\s a -> s {_drsVersionIdsToStages = a}) .
  _Default . _Map
...
drsLastRotatedDate =
  lens _drsLastRotatedDate (\s a -> s {_drsLastRotatedDate = a}) . mapping _Time

-- | The last date that this secret was accessed. This value is truncated to midnight of the date and therefore shows only the date, not the time.
drsLastAccessedDate :: Lens' DescribeSecretResponse (Maybe UTCTime)
drsLastAccessedDate =
  lens _drsLastAccessedDate (\s a -> s {_drsLastAccessedDate = a}) .
  mapping _Time
...
drsRotationLambdaARN =
  lens _drsRotationLambdaARN (\s a -> s {_drsRotationLambdaARN = a})
