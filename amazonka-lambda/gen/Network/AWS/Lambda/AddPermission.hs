...
apEventSourceToken =
  lens _apEventSourceToken (\s a -> s {_apEventSourceToken = a})
...
instance AWSRequest AddPermission where
  ...
  response =
    receiveJSON
      (\s h x ->
         AddPermissionResponse' <$> (x .?> "Statement") <*> (pure (fromEnum s)))
...
instance ToPath AddPermission where
  toPath AddPermission' {..} =
    mconcat ["/2015-03-31/functions/", toBS _apFunctionName, "/policy"]
...
aprsResponseStatus =
  lens _aprsResponseStatus (\s a -> s {_aprsResponseStatus = a})
