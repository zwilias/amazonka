...
instance AWSRequest ListVersionsByFunction where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListVersionsByFunctionResponse' <$> (x .?> "Versions" .!@ mempty) <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
instance ToPath ListVersionsByFunction where
  toPath ListVersionsByFunction' {..} =
    mconcat ["/2015-03-31/functions/", toBS _lvbfFunctionName, "/versions"]
...
lvbfrsVersions =
  lens _lvbfrsVersions (\s a -> s {_lvbfrsVersions = a}) . _Default . _Coerce
...
lvbfrsResponseStatus =
  lens _lvbfrsResponseStatus (\s a -> s {_lvbfrsResponseStatus = a})
