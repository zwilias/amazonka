...
instance FromText EventSourcePosition where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventSourcePosition from value: '" <>
        e <> "'. Accepted values: at_timestamp, latest, trim_horizon"
...
instance FromText FunctionVersion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FunctionVersion from value: '" <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: dryrun, event, requestresponse"
...
instance FromText Runtime where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Runtime from value: '" <>
        e <>
...
instance FromText TracingMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TracingMode from value: '" <>
        e <> "'. Accepted values: active, passthrough"
