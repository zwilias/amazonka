...
alConcurrentExecutions =
  lens _alConcurrentExecutions (\s a -> s {_alConcurrentExecutions = a})
...
alUnreservedConcurrentExecutions =
  lens
    ...
    (\s a -> s {_alUnreservedConcurrentExecutions = a}) .
  mapping _Nat

-- | Size, in bytes, of code/dependencies that you can zip into a deployment package (uncompressed zip/jar size) for uploading. The default limit is 250 MB.
alCodeSizeUnzipped :: Lens' AccountLimit (Maybe Integer)
alCodeSizeUnzipped =
  lens _alCodeSizeUnzipped (\s a -> s {_alCodeSizeUnzipped = a})
...
instance FromJSON AccountLimit where
  parseJSON =
    withObject
      "AccountLimit"
      (\x ->
         AccountLimit' <$> (x .:? "ConcurrentExecutions") <*>
         (x .:? "TotalCodeSize") <*>
...
instance FromJSON AccountUsage where
  parseJSON =
    withObject
      "AccountUsage"
      (\x ->
         AccountUsage' <$> (x .:? "TotalCodeSize") <*> (x .:? "FunctionCount"))
...
instance FromJSON AliasConfiguration where
  parseJSON =
    withObject
      "AliasConfiguration"
      (\x ->
         AliasConfiguration' <$> (x .:? "RoutingConfig") <*> (x .:? "Name") <*>
         (x .:? "FunctionVersion") <*>
         (x .:? "AliasArn") <*>
         (x .:? "Description") <*>
         (x .:? "RevisionId"))
...
arcAdditionalVersionWeights ::
     Lens' AliasRoutingConfiguration (HashMap Text Double)
arcAdditionalVersionWeights =
  lens
    _arcAdditionalVersionWeights
    (\s a -> s {_arcAdditionalVersionWeights = a}) .
  _Default . _Map
...
cReservedConcurrentExecutions =
  lens
    ...
    (\s a -> s {_cReservedConcurrentExecutions = a}) .
  mapping _Nat
...
eVariables =
  lens _eVariables (\s a -> s {_eVariables = a}) . mapping (_Sensitive . _Map)
...
instance FromJSON EnvironmentError where
  parseJSON =
    withObject
      "EnvironmentError"
      (\x -> EnvironmentError' <$> (x .:? "ErrorCode") <*> (x .:? "Message"))
...
envVariables =
  lens _envVariables (\s a -> s {_envVariables = a}) .
...
instance FromJSON EnvironmentResponse where
  parseJSON =
    withObject
      "EnvironmentResponse"
      (\x ->
         EnvironmentResponse' <$> (x .:? "Variables" .!= mempty) <*>
         (x .:? "Error"))
...
instance FromJSON EventSourceMappingConfiguration where
  parseJSON =
    withObject
      "EventSourceMappingConfiguration"
      (\x ->
         EventSourceMappingConfiguration' <$> (x .:? "EventSourceArn") <*>
         (x .:? "State") <*>
         (x .:? "FunctionArn") <*>
         (x .:? "UUID") <*>
...
fcZipFile =
  lens _fcZipFile (\s a -> s {_fcZipFile = a}) . mapping (_Sensitive . _Base64)
...
instance FromJSON FunctionCodeLocation where
  parseJSON =
    withObject
      "FunctionCodeLocation"
      (\x ->
         FunctionCodeLocation' <$> (x .:? "Location") <*>
         (x .:? "RepositoryType"))
...
fcDeadLetterConfig =
  lens _fcDeadLetterConfig (\s a -> s {_fcDeadLetterConfig = a})
...
instance FromJSON FunctionConfiguration where
  parseJSON =
    withObject
      "FunctionConfiguration"
      (\x ->
         FunctionConfiguration' <$> (x .:? "MemorySize") <*> (x .:? "Runtime") <*>
         (x .:? "FunctionArn") <*>
         (x .:? "KMSKeyArn") <*>
         (x .:? "Environment") <*>
         (x .:? "DeadLetterConfig") <*>
...
instance ToJSON TracingConfig where
  toJSON TracingConfig' {..} = object (catMaybes [("Mode" .=) <$> _tMode])
...
vpccSecurityGroupIds =
  lens _vpccSecurityGroupIds (\s a -> s {_vpccSecurityGroupIds = a}) .
  _Default . _Coerce

-- | A list of one or more subnet IDs in your VPC.
vpccSubnetIds :: Lens' VPCConfig [Text]
vpccSubnetIds =
  lens _vpccSubnetIds (\s a -> s {_vpccSubnetIds = a}) . _Default . _Coerce
...
instance FromJSON VPCConfigResponse where
  parseJSON =
    withObject
      "VPCConfigResponse"
      (\x ->
         VPCConfigResponse' <$> (x .:? "SecurityGroupIds" .!= mempty) <*>
         (x .:? "SubnetIds" .!= mempty) <*>
         (x .:? "VpcId"))
