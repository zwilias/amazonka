...
ufcMemorySize =
  lens _ufcMemorySize (\s a -> s {_ufcMemorySize = a}) . mapping _Nat
...
ufcDeadLetterConfig ::
     Lens' UpdateFunctionConfiguration (Maybe DeadLetterConfig)
ufcDeadLetterConfig =
  lens _ufcDeadLetterConfig (\s a -> s {_ufcDeadLetterConfig = a})
...
instance ToPath UpdateFunctionConfiguration where
  toPath UpdateFunctionConfiguration' {..} =
    mconcat ["/2015-03-31/functions/", toBS _ufcFunctionName, "/configuration"]
