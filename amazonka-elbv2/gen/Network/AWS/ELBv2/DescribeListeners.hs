...
dlListenerARNs =
  lens _dlListenerARNs (\s a -> s {_dlListenerARNs = a}) . _Default . _Coerce
...
instance AWSRequest DescribeListeners where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeListenersResponse' <$> (x .@? "NextMarker") <*>
...
instance ToQuery DescribeListeners where
  toQuery DescribeListeners' {..} =
    mconcat
      ...
      , "ListenerArns" =: toQuery (toQueryList "member" <$> _dlListenerARNs)
...
dlsrsListeners =
  lens _dlsrsListeners (\s a -> s {_dlsrsListeners = a}) . _Default . _Coerce
