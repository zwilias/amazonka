...
rlcCertificates =
  lens _rlcCertificates (\s a -> s {_rlcCertificates = a}) . _Coerce

instance AWSRequest RemoveListenerCertificates where
  type Rs RemoveListenerCertificates = RemoveListenerCertificatesResponse
  request = postQuery eLBv2
  response =
    receiveXMLWrapper
      "RemoveListenerCertificatesResult"
      (\s h x -> RemoveListenerCertificatesResponse' <$> (pure (fromEnum s)))
...
rlcrsResponseStatus =
  lens _rlcrsResponseStatus (\s a -> s {_rlcrsResponseStatus = a})
