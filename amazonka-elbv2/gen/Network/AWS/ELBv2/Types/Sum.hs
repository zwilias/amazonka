...
instance FromText ActionTypeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionTypeEnum from value: '" <>
        e <> "'. Accepted values: forward"
...
instance FromText IPAddressType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IPAddressType from value: '" <>
        e <> "'. Accepted values: dualstack, ipv4"
...
instance FromText LoadBalancerSchemeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LoadBalancerSchemeEnum from value: '" <>
        e <> "'. Accepted values: internal, internet-facing"
...
instance FromText LoadBalancerStateEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LoadBalancerStateEnum from value: '" <>
        e <> "'. Accepted values: active, active_impaired, failed, provisioning"
...
instance FromText LoadBalancerTypeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LoadBalancerTypeEnum from value: '" <>
        e <> "'. Accepted values: application, network"
...
instance FromText ProtocolEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProtocolEnum from value: '" <>
...
instance FromText TargetHealthReasonEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TargetHealthReasonEnum from value: '" <>
        e <>
...
instance FromText TargetHealthStateEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TargetHealthStateEnum from value: '" <>
        e <>
...
instance FromText TargetTypeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TargetTypeEnum from value: '" <>
        e <> "'. Accepted values: ip, instance"
