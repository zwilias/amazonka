...
instance FromXML Action where
  parseXML x = Action' <$> (x .@ "Type") <*> (x .@ "TargetGroupArn")
...
instance ToQuery Action where
  toQuery Action' {..} =
    mconcat ["Type" =: _aType, "TargetGroupArn" =: _aTargetGroupARN]
...
azLoadBalancerAddresses =
  lens _azLoadBalancerAddresses (\s a -> s {_azLoadBalancerAddresses = a}) .
  _Default . _Coerce

instance FromXML AvailabilityZone where
  parseXML x =
    AvailabilityZone' <$> (x .@? "SubnetId") <*> (x .@? "ZoneName") <*>
    (x .@? "LoadBalancerAddresses" .!@ mempty >>= may (parseXMLList "member"))
...
instance FromXML Certificate where
  parseXML x = Certificate' <$> (x .@? "CertificateArn") <*> (x .@? "IsDefault")
...
instance ToQuery Certificate where
  toQuery Certificate' {..} =
    mconcat ["CertificateArn" =: _cCertificateARN, "IsDefault" =: _cIsDefault]
...
lDefaultActions =
  lens _lDefaultActions (\s a -> s {_lDefaultActions = a}) . _Default . _Coerce

-- | The SSL server certificate. You must provide a certificate if the protocol is HTTPS.
lCertificates :: Lens' Listener [Certificate]
lCertificates =
  lens _lCertificates (\s a -> s {_lCertificates = a}) . _Default . _Coerce
...
instance FromXML Listener where
  parseXML x =
    Listener' <$> (x .@? "SslPolicy") <*> (x .@? "ListenerArn") <*>
    (x .@? "Protocol") <*>
...
lbSecurityGroups =
  lens _lbSecurityGroups (\s a -> s {_lbSecurityGroups = a}) .
  _Default . _Coerce
...
lbCreatedTime =
  lens _lbCreatedTime (\s a -> s {_lbCreatedTime = a}) . mapping _Time
...
lbCanonicalHostedZoneId =
  lens _lbCanonicalHostedZoneId (\s a -> s {_lbCanonicalHostedZoneId = a})

-- | The Availability Zones for the load balancer.
lbAvailabilityZones :: Lens' LoadBalancer [AvailabilityZone]
lbAvailabilityZones =
  lens _lbAvailabilityZones (\s a -> s {_lbAvailabilityZones = a}) .
  _Default . _Coerce
...
instance FromXML LoadBalancer where
  parseXML x =
    LoadBalancer' <$> (x .@? "State") <*>
    (x .@? "SecurityGroups" .!@ mempty >>= may (parseXMLList "member")) <*>
    (x .@? "LoadBalancerName") <*>
    (x .@? "CreatedTime") <*>
    (x .@? "VpcId") <*>
    (x .@? "CanonicalHostedZoneId") <*>
    (x .@? "AvailabilityZones" .!@ mempty >>= may (parseXMLList "member")) <*>
    (x .@? "LoadBalancerArn") <*>
    (x .@? "IpAddressType") <*>
    (x .@? "Scheme") <*>
    (x .@? "Type") <*>
...
instance FromXML LoadBalancerAddress where
  parseXML x =
    LoadBalancerAddress' <$> (x .@? "IpAddress") <*> (x .@? "AllocationId")
...
instance FromXML LoadBalancerAttribute where
  parseXML x = LoadBalancerAttribute' <$> (x .@? "Value") <*> (x .@? "Key")
...
instance FromXML LoadBalancerState where
  parseXML x = LoadBalancerState' <$> (x .@? "Reason") <*> (x .@? "Code")
...
instance ToQuery Matcher where
  toQuery Matcher' {..} = mconcat ["HttpCode" =: _mHTTPCode]
...
rConditions =
  lens _rConditions (\s a -> s {_rConditions = a}) . _Default . _Coerce
...
spSSLProtocols =
  lens _spSSLProtocols (\s a -> s {_spSSLProtocols = a}) . _Default . _Coerce

instance FromXML SSLPolicy where
  parseXML x =
    SSLPolicy' <$> (x .@? "Ciphers" .!@ mempty >>= may (parseXMLList "member")) <*>
    (x .@? "Name") <*>
...
instance ToQuery SubnetMapping where
  toQuery SubnetMapping' {..} =
    mconcat ["AllocationId" =: _smAllocationId, "SubnetId" =: _smSubnetId]
...
tdAvailabilityZone =
  lens _tdAvailabilityZone (\s a -> s {_tdAvailabilityZone = a})
...
instance FromXML TargetDescription where
  parseXML x =
    TargetDescription' <$> (x .@? "AvailabilityZone") <*> (x .@? "Port") <*>
    (x .@ "Id")
...
tgUnhealthyThresholdCount =
  lens _tgUnhealthyThresholdCount (\s a -> s {_tgUnhealthyThresholdCount = a}) .
  mapping _Nat
...
tgHealthCheckIntervalSeconds =
  lens
    _tgHealthCheckIntervalSeconds
    (\s a -> s {_tgHealthCheckIntervalSeconds = a}) .
  mapping _Nat
...
tgHealthyThresholdCount =
  lens _tgHealthyThresholdCount (\s a -> s {_tgHealthyThresholdCount = a}) .
  mapping _Nat

-- | The protocol to use to connect with the target.
tgHealthCheckProtocol :: Lens' TargetGroup (Maybe ProtocolEnum)
tgHealthCheckProtocol =
  lens _tgHealthCheckProtocol (\s a -> s {_tgHealthCheckProtocol = a})

-- | The Amazon Resource Names (ARN) of the load balancers that route traffic to this target group.
tgLoadBalancerARNs :: Lens' TargetGroup [Text]
tgLoadBalancerARNs =
  lens _tgLoadBalancerARNs (\s a -> s {_tgLoadBalancerARNs = a}) .
  _Default . _Coerce
...
tgHealthCheckTimeoutSeconds =
  lens
    _tgHealthCheckTimeoutSeconds
    (\s a -> s {_tgHealthCheckTimeoutSeconds = a}) .
  mapping _Nat
...
instance FromXML TargetGroup where
  parseXML x =
    TargetGroup' <$> (x .@? "Matcher") <*> (x .@? "HealthCheckPath") <*>
    ...
    (x .@? "LoadBalancerArns" .!@ mempty >>= may (parseXMLList "member")) <*>
...
instance FromXML TargetGroupAttribute where
  parseXML x = TargetGroupAttribute' <$> (x .@? "Value") <*> (x .@? "Key")
...
thdHealthCheckPort =
  lens _thdHealthCheckPort (\s a -> s {_thdHealthCheckPort = a})
...
instance FromXML TargetHealthDescription where
  parseXML x =
    TargetHealthDescription' <$> (x .@? "TargetHealth") <*>
    (x .@? "HealthCheckPort") <*>
    (x .@? "Target")
