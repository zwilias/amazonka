...
ctgHealthCheckPath =
  lens _ctgHealthCheckPath (\s a -> s {_ctgHealthCheckPath = a})
...
ctgUnhealthyThresholdCount =
  lens _ctgUnhealthyThresholdCount (\s a -> s {_ctgUnhealthyThresholdCount = a}) .
  mapping _Nat
...
ctgHealthCheckIntervalSeconds =
  lens
    ...
    (\s a -> s {_ctgHealthCheckIntervalSeconds = a}) .
  mapping _Nat
...
ctgHealthyThresholdCount =
  lens _ctgHealthyThresholdCount (\s a -> s {_ctgHealthyThresholdCount = a}) .
  mapping _Nat
...
ctgHealthCheckProtocol =
  lens _ctgHealthCheckProtocol (\s a -> s {_ctgHealthCheckProtocol = a})
...
ctgHealthCheckTimeoutSeconds =
  lens
    ...
    (\s a -> s {_ctgHealthCheckTimeoutSeconds = a}) .
  mapping _Nat

-- | The port the load balancer uses when performing health checks on targets. The default is @traffic-port@ , which is the port on which each target receives traffic from the load balancer.
ctgHealthCheckPort :: Lens' CreateTargetGroup (Maybe Text)
ctgHealthCheckPort =
  lens _ctgHealthCheckPort (\s a -> s {_ctgHealthCheckPort = a})
...
ctgrsTargetGroups =
  lens _ctgrsTargetGroups (\s a -> s {_ctgrsTargetGroups = a}) .
  _Default . _Coerce
