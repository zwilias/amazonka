...
ssgLoadBalancerARN =
  lens _ssgLoadBalancerARN (\s a -> s {_ssgLoadBalancerARN = a})

-- | The IDs of the security groups.
ssgSecurityGroups :: Lens' SetSecurityGroups [Text]
ssgSecurityGroups =
  lens _ssgSecurityGroups (\s a -> s {_ssgSecurityGroups = a}) . _Coerce
...
instance AWSRequest SetSecurityGroups where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         SetSecurityGroupsResponse' <$>
         (x .@? "SecurityGroupIds" .!@ mempty >>= may (parseXMLList "member")) <*>
         (pure (fromEnum s)))
...
ssgrsSecurityGroupIds =
  lens _ssgrsSecurityGroupIds (\s a -> s {_ssgrsSecurityGroupIds = a}) .
  _Default . _Coerce

-- | -- | The response status code.
ssgrsResponseStatus :: Lens' SetSecurityGroupsResponse Int
ssgrsResponseStatus =
  lens _ssgrsResponseStatus (\s a -> s {_ssgrsResponseStatus = a})
