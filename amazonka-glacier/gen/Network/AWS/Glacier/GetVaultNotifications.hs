...
instance AWSRequest GetVaultNotifications where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetVaultNotificationsResponse' <$> (eitherParseJSON x) <*>
         (pure (fromEnum s)))
...
gvnrsVaultNotificationConfig ::
     Lens' GetVaultNotificationsResponse (Maybe VaultNotificationConfig)
gvnrsVaultNotificationConfig =
  lens
    _gvnrsVaultNotificationConfig
    (\s a -> s {_gvnrsVaultNotificationConfig = a})

-- | -- | The response status code.
gvnrsResponseStatus :: Lens' GetVaultNotificationsResponse Int
gvnrsResponseStatus =
  lens _gvnrsResponseStatus (\s a -> s {_gvnrsResponseStatus = a})
