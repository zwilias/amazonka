...
instance AWSRequest ListParts where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPartsResponse' <$> (x .?> "Parts" .!@ mempty) <*>
         (x .?> "MultipartUploadId") <*>
         (x .?> "PartSizeInBytes") <*>
...
instance ToQuery ListParts where
  toQuery ListParts' {..} = mconcat ["marker" =: _lpMarker, "limit" =: _lpLimit]
...
lprsMultipartUploadId =
  lens _lprsMultipartUploadId (\s a -> s {_lprsMultipartUploadId = a})

-- | The part size in bytes. This is the same value that you specified in the Initiate Multipart Upload request.
lprsPartSizeInBytes :: Lens' ListPartsResponse (Maybe Integer)
lprsPartSizeInBytes =
  lens _lprsPartSizeInBytes (\s a -> s {_lprsPartSizeInBytes = a})

-- | The description of the archive that was specified in the Initiate Multipart Upload request.
lprsArchiveDescription :: Lens' ListPartsResponse (Maybe Text)
lprsArchiveDescription =
  lens _lprsArchiveDescription (\s a -> s {_lprsArchiveDescription = a})
