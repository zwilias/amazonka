...
instance AWSRequest InitiateVaultLock where
  ...
  response =
    receiveEmpty
      (\s h x ->
         InitiateVaultLockResponse' <$> (h .#? "x-amz-lock-id") <*>
         (pure (fromEnum s)))
...
instance ToPath InitiateVaultLock where
  toPath InitiateVaultLock' {..} =
    mconcat
      ["/", toBS _ivlAccountId, "/vaults/", toBS _ivlVaultName, "/lock-policy"]
...
ivlrsResponseStatus =
  lens _ivlrsResponseStatus (\s a -> s {_ivlrsResponseStatus = a})
