...
data CreateWorkteam = CreateWorkteam'{_cwNotificationConfiguration
                                      :: !(Maybe NotificationConfiguration),
                                      _cwTags :: !(Maybe [Tag]),
                                      _cwWorkteamName :: !Text,
                                      _cwMemberDefinitions ::
...
--
-- * 'cwMemberDefinitions' - A list of @MemberDefinition@ objects that contains objects that identify the Amazon Cognito user pool that makes up the work team. For more information, see <https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-identity-pools.html Amazon Cognito User Pools> . All of the @CognitoMemberDefinition@ objects that make up the member definition must have the same @ClientId@ and @UserPool@ values.
--
...
createWorkteam
    ...
    -> Text -- ^ 'cwDescription'
    -> CreateWorkteam
createWorkteam pWorkteamName_ pMemberDefinitions_
  pDescription_
  = CreateWorkteam'{_cwNotificationConfiguration =
                      Nothing,
                    _cwTags = Nothing, _cwWorkteamName = pWorkteamName_,
...
-- | A list of @MemberDefinition@ objects that contains objects that identify the Amazon Cognito user pool that makes up the work team. For more information, see <https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-identity-pools.html Amazon Cognito User Pools> . All of the @CognitoMemberDefinition@ objects that make up the member definition must have the same @ClientId@ and @UserPool@ values.
...
data CreateWorkteamResponse = CreateWorkteamResponse'{_cwrsWorkteamARN
                                                      :: !(Maybe Text),
                                                      _cwrsResponseStatus ::
                                                      !Int}
...
createWorkteamResponse pResponseStatus_
  = CreateWorkteamResponse'{_cwrsWorkteamARN = Nothing,
