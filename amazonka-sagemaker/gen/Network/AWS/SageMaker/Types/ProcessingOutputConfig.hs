...
data ProcessingOutputConfig = ProcessingOutputConfig'{_pocKMSKeyId
                                                      :: !(Maybe Text),
                                                      _pocOutputs ::
                                                      ![ProcessingOutput]}
...
processingOutputConfig
    :: ProcessingOutputConfig
processingOutputConfig
  = ProcessingOutputConfig'{_pocKMSKeyId = Nothing,
                            _pocOutputs = mempty}
...
instance FromJSON ProcessingOutputConfig where
        parseJSON
          = withObject "ProcessingOutputConfig"
              (\ x ->
                 ProcessingOutputConfig' <$>
                   (x .:? "KmsKeyId") <*> (x .:? "Outputs" .!= mempty))

instance Hashable ProcessingOutputConfig where

instance NFData ProcessingOutputConfig where
