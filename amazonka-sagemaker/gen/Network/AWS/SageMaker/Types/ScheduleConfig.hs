...
newtype ScheduleConfig = ScheduleConfig'{_scScheduleExpression
                                         :: Text}
...
scheduleConfig
    :: Text -- ^ 'scScheduleExpression'
    -> ScheduleConfig
scheduleConfig pScheduleExpression_
  = ScheduleConfig'{_scScheduleExpression =
...
instance FromJSON ScheduleConfig where
        parseJSON
          = withObject "ScheduleConfig"
              (\ x ->
                 ScheduleConfig' <$> (x .: "ScheduleExpression"))

instance Hashable ScheduleConfig where

instance NFData ScheduleConfig where
