...
data AutoMLContainerDefinition = AutoMLContainerDefinition'{_amlcdEnvironment
                                                            ::
                                                            !(Maybe
                                                                (Map Text
                                                                   Text)),
...
autoMLContainerDefinition
    :: Text -- ^ 'amlcdImage'
    -> Text -- ^ 'amlcdModelDataURL'
    -> AutoMLContainerDefinition
autoMLContainerDefinition pImage_ pModelDataURL_
  = AutoMLContainerDefinition'{_amlcdEnvironment =
                                 Nothing,
                               _amlcdImage = pImage_,
...
instance FromJSON AutoMLContainerDefinition where
        parseJSON
          = withObject "AutoMLContainerDefinition"
              (\ x ->
                 AutoMLContainerDefinition' <$>
                   (x .:? "Environment" .!= mempty) <*> (x .: "Image")
                     <*> (x .: "ModelDataUrl"))

instance Hashable AutoMLContainerDefinition where

instance NFData AutoMLContainerDefinition where
