...
data ProcessingClusterConfig = ProcessingClusterConfig'{_pccVolumeKMSKeyId
                                                        :: !(Maybe Text),
                                                        _pccInstanceCount ::
                                                        !Nat,
                                                        _pccInstanceType ::
...
processingClusterConfig
    :: Natural -- ^ 'pccInstanceCount'
    -> ProcessingInstanceType -- ^ 'pccInstanceType'
    -> Natural -- ^ 'pccVolumeSizeInGB'
    -> ProcessingClusterConfig
processingClusterConfig pInstanceCount_
  pInstanceType_ pVolumeSizeInGB_
  = ProcessingClusterConfig'{_pccVolumeKMSKeyId =
                               Nothing,
...
instance FromJSON ProcessingClusterConfig where
        parseJSON
          = withObject "ProcessingClusterConfig"
              (\ x ->
                 ProcessingClusterConfig' <$>
                   (x .:? "VolumeKmsKeyId") <*> (x .: "InstanceCount")
                     <*> (x .: "InstanceType")
                     <*> (x .: "VolumeSizeInGB"))

instance Hashable ProcessingClusterConfig where

instance NFData ProcessingClusterConfig where
