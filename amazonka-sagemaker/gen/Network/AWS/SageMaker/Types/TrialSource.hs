...
data TrialSource = TrialSource'{_tsSourceType ::
                                !(Maybe Text),
                                _tsSourceARN :: !Text}
...
trialSource pSourceARN_
  = TrialSource'{_tsSourceType = Nothing,
...
instance FromJSON TrialSource where
        parseJSON
          = withObject "TrialSource"
              (\ x ->
                 TrialSource' <$>
                   (x .:? "SourceType") <*> (x .: "SourceArn"))

instance Hashable TrialSource where

instance NFData TrialSource where
