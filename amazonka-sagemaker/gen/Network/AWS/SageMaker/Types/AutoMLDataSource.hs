...
newtype AutoMLDataSource = AutoMLDataSource'{_amldsS3DataSource
                                             :: AutoMLS3DataSource}
...
autoMLDataSource
    :: AutoMLS3DataSource -- ^ 'amldsS3DataSource'
    -> AutoMLDataSource
autoMLDataSource pS3DataSource_
  = AutoMLDataSource'{_amldsS3DataSource =
                        pS3DataSource_}
...
instance FromJSON AutoMLDataSource where
        parseJSON
          = withObject "AutoMLDataSource"
              (\ x -> AutoMLDataSource' <$> (x .: "S3DataSource"))

instance Hashable AutoMLDataSource where

instance NFData AutoMLDataSource where
