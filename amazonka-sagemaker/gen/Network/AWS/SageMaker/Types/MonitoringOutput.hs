...
newtype MonitoringOutput = MonitoringOutput'{_moS3Output
                                             :: MonitoringS3Output}
...
instance FromJSON MonitoringOutput where
        parseJSON
          = withObject "MonitoringOutput"
              (\ x -> MonitoringOutput' <$> (x .: "S3Output"))

instance Hashable MonitoringOutput where

instance NFData MonitoringOutput where
