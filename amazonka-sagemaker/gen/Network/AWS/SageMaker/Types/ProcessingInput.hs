...
data ProcessingInput = ProcessingInput'{_piInputName
                                        :: !Text,
...
processingInput
    :: Text -- ^ 'piInputName'
    -> ProcessingS3Input -- ^ 'piS3Input'
    -> ProcessingInput
processingInput pInputName_ pS3Input_
  = ProcessingInput'{_piInputName = pInputName_,
...
instance FromJSON ProcessingInput where
        parseJSON
          = withObject "ProcessingInput"
              (\ x ->
                 ProcessingInput' <$>
                   (x .: "InputName") <*> (x .: "S3Input"))

instance Hashable ProcessingInput where

instance NFData ProcessingInput where
