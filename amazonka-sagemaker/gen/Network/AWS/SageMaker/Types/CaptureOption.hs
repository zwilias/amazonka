...
newtype CaptureOption = CaptureOption'{_coCaptureMode
                                       :: CaptureMode}
...
instance FromJSON CaptureOption where
        parseJSON
          = withObject "CaptureOption"
              (\ x -> CaptureOption' <$> (x .: "CaptureMode"))

instance Hashable CaptureOption where

instance NFData CaptureOption where
