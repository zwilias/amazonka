...
data AutoMLJobSummary = AutoMLJobSummary'{_amljsFailureReason
                                          :: !(Maybe Text),
...
--
-- * 'amljsFailureReason' - The failure reason.
--
-- * 'amljsEndTime' - The end time.
--
...
autoMLJobSummary
    :: Text -- ^ 'amljsAutoMLJobName'
    -> Text -- ^ 'amljsAutoMLJobARN'
    -> AutoMLJobStatus -- ^ 'amljsAutoMLJobStatus'
    -> AutoMLJobSecondaryStatus -- ^ 'amljsAutoMLJobSecondaryStatus'
    -> UTCTime -- ^ 'amljsCreationTime'
    -> UTCTime -- ^ 'amljsLastModifiedTime'
    -> AutoMLJobSummary
autoMLJobSummary pAutoMLJobName_ pAutoMLJobARN_
  pAutoMLJobStatus_ pAutoMLJobSecondaryStatus_
  pCreationTime_ pLastModifiedTime_
  = AutoMLJobSummary'{_amljsFailureReason = Nothing,
...
instance FromJSON AutoMLJobSummary where
        parseJSON
          = withObject "AutoMLJobSummary"
              (\ x ->
                 AutoMLJobSummary' <$>
                   (x .:? "FailureReason") <*> (x .:? "EndTime") <*>
                     (x .: "AutoMLJobName")
                     <*> (x .: "AutoMLJobArn")
                     <*> (x .: "AutoMLJobStatus")
...
instance Hashable AutoMLJobSummary where

instance NFData AutoMLJobSummary where
