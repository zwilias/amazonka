...
data CheckpointConfig = CheckpointConfig'{_ccLocalPath
                                          :: !(Maybe Text),
                                          _ccS3URI :: !Text}
...
checkpointConfig pS3URI_
  = CheckpointConfig'{_ccLocalPath = Nothing,
                      _ccS3URI = pS3URI_}
...
instance FromJSON CheckpointConfig where
        parseJSON
          = withObject "CheckpointConfig"
              (\ x ->
                 CheckpointConfig' <$>
                   (x .:? "LocalPath") <*> (x .: "S3Uri"))

instance Hashable CheckpointConfig where

instance NFData CheckpointConfig where
