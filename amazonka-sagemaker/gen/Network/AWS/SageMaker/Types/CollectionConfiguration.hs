...
data CollectionConfiguration = CollectionConfiguration'{_ccCollectionParameters
                                                        ::
                                                        !(Maybe
                                                            (Map Text Text)),
                                                        _ccCollectionName ::
...
collectionConfiguration
    :: CollectionConfiguration
collectionConfiguration
  = CollectionConfiguration'{_ccCollectionParameters =
                               Nothing,
...
instance FromJSON CollectionConfiguration where
        parseJSON
          = withObject "CollectionConfiguration"
              (\ x ->
                 CollectionConfiguration' <$>
                   (x .:? "CollectionParameters" .!= mempty) <*>
                     (x .:? "CollectionName"))

instance Hashable CollectionConfiguration where

instance NFData CollectionConfiguration where
