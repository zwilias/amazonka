...
data ProcessingOutput = ProcessingOutput'{_poOutputName
                                          :: !Text,
...
processingOutput
    :: Text -- ^ 'poOutputName'
    -> ProcessingS3Output -- ^ 'poS3Output'
    -> ProcessingOutput
processingOutput pOutputName_ pS3Output_
  = ProcessingOutput'{_poOutputName = pOutputName_,
...
instance FromJSON ProcessingOutput where
        parseJSON
          = withObject "ProcessingOutput"
              (\ x ->
                 ProcessingOutput' <$>
                   (x .: "OutputName") <*> (x .: "S3Output"))

instance Hashable ProcessingOutput where

instance NFData ProcessingOutput where
