...
data AutoMLJobConfig = AutoMLJobConfig'{_amljcSecurityConfig
...
autoMLJobConfig
    :: AutoMLJobConfig
autoMLJobConfig
  = AutoMLJobConfig'{_amljcSecurityConfig = Nothing,
...
instance FromJSON AutoMLJobConfig where
        parseJSON
          = withObject "AutoMLJobConfig"
              (\ x ->
                 AutoMLJobConfig' <$>
                   (x .:? "SecurityConfig") <*>
                     (x .:? "CompletionCriteria"))

instance Hashable AutoMLJobConfig where

instance NFData AutoMLJobConfig where
