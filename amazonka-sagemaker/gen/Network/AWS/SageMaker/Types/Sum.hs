...
instance FromText DirectInternetAccess where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DirectInternetAccess from value: '" <>
...
instance FromText EndpointConfigSortKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EndpointConfigSortKey from value: '" <>
        e <> "'. Accepted values: creationtime, name"
...
instance FromText EndpointSortKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EndpointSortKey from value: '" <>
        e <> "'. Accepted values: creationtime, name, status"
...
instance FromText EndpointStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EndpointStatus from value: '" <>
        e <>
...
instance FromText ModelSortKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ModelSortKey from value: '" <>
        e <> "'. Accepted values: creationtime, name"
...
instance FromText NotebookInstanceLifecycleConfigSortKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NotebookInstanceLifecycleConfigSortKey from value: '" <>
        e <> "'. Accepted values: creationtime, lastmodifiedtime, name"
...
instance FromText NotebookInstanceLifecycleConfigSortOrder where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NotebookInstanceLifecycleConfigSortOrder from value: '" <>
...
instance FromText NotebookInstanceSortKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NotebookInstanceSortKey from value: '" <>
        e <> "'. Accepted values: creationtime, name, status"
...
instance FromText NotebookInstanceSortOrder where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NotebookInstanceSortOrder from value: '" <>
        e <> "'. Accepted values: ascending, descending"
...
instance FromText NotebookInstanceStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NotebookInstanceStatus from value: '" <>
        e <>
...
instance FromText OrderKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrderKey from value: '" <>
        e <> "'. Accepted values: ascending, descending"
...
instance FromText ProductionVariantInstanceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProductionVariantInstanceType from value: '" <>
        e <>
...
instance FromText RecordWrapper where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RecordWrapper from value: '" <>
        e <> "'. Accepted values: none, recordio"
...
instance FromText S3DataDistribution where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing S3DataDistribution from value: '" <>
        e <> "'. Accepted values: fullyreplicated, shardedbys3key"
...
instance FromText S3DataType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing S3DataType from value: '" <>
        e <> "'. Accepted values: manifestfile, s3prefix"
...
instance FromText SecondaryStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SecondaryStatus from value: '" <>
        e <>
...
instance FromText SortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SortBy from value: '" <>
        e <> "'. Accepted values: creationtime, name, status"
...
instance FromText TrainingInputMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TrainingInputMode from value: '" <>
        e <> "'. Accepted values: file, pipe"
...
instance FromText TrainingInstanceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TrainingInstanceType from value: '" <>
        e <>
...
instance FromText TrainingJobStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TrainingJobStatus from value: '" <>
        e <>
