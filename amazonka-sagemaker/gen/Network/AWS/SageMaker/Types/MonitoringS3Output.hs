...
data MonitoringS3Output = MonitoringS3Output'{_msoS3UploadMode
                                              ::
...
monitoringS3Output pS3URI_ pLocalPath_
  = MonitoringS3Output'{_msoS3UploadMode = Nothing,
                        _msoS3URI = pS3URI_, _msoLocalPath = pLocalPath_}
...
instance FromJSON MonitoringS3Output where
        parseJSON
          = withObject "MonitoringS3Output"
              (\ x ->
                 MonitoringS3Output' <$>
                   (x .:? "S3UploadMode") <*> (x .: "S3Uri") <*>
                     (x .: "LocalPath"))

instance Hashable MonitoringS3Output where

instance NFData MonitoringS3Output where
