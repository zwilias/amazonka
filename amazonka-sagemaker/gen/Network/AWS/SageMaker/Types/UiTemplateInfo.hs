...
data UiTemplateInfo = UiTemplateInfo'{_utiURL ::
                                      !(Maybe Text),
...
uiTemplateInfo
    :: UiTemplateInfo
uiTemplateInfo
  = UiTemplateInfo'{_utiURL = Nothing,
...
instance FromJSON UiTemplateInfo where
        parseJSON
          = withObject "UiTemplateInfo"
              (\ x ->
                 UiTemplateInfo' <$>
                   (x .:? "Url") <*> (x .:? "ContentSha256"))

instance Hashable UiTemplateInfo where

instance NFData UiTemplateInfo where
