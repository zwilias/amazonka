...
newtype MonitoringStatisticsResource = MonitoringStatisticsResource'{_msrS3URI
                                                                     ::
                                                                     Maybe Text}
...
instance FromJSON MonitoringStatisticsResource where
        parseJSON
          = withObject "MonitoringStatisticsResource"
              (\ x ->
                 MonitoringStatisticsResource' <$> (x .:? "S3Uri"))

instance Hashable MonitoringStatisticsResource where

instance NFData MonitoringStatisticsResource where
