...
data DataCaptureConfigSummary = DataCaptureConfigSummary'{_dccsEnableCapture
                                                          :: !Bool,
                                                          _dccsCaptureStatus ::
                                                          !CaptureStatus,
...
dataCaptureConfigSummary
    :: Bool -- ^ 'dccsEnableCapture'
    -> CaptureStatus -- ^ 'dccsCaptureStatus'
    -> Natural -- ^ 'dccsCurrentSamplingPercentage'
    -> Text -- ^ 'dccsDestinationS3URI'
    -> Text -- ^ 'dccsKMSKeyId'
    -> DataCaptureConfigSummary
dataCaptureConfigSummary pEnableCapture_
  pCaptureStatus_ pCurrentSamplingPercentage_
  pDestinationS3URI_ pKMSKeyId_
  = DataCaptureConfigSummary'{_dccsEnableCapture =
                                pEnableCapture_,
...
instance FromJSON DataCaptureConfigSummary where
        parseJSON
          = withObject "DataCaptureConfigSummary"
              (\ x ->
                 DataCaptureConfigSummary' <$>
                   (x .: "EnableCapture") <*> (x .: "CaptureStatus") <*>
                     (x .: "CurrentSamplingPercentage")
                     <*> (x .: "DestinationS3Uri")
                     <*> (x .: "KmsKeyId"))

instance Hashable DataCaptureConfigSummary where

instance NFData DataCaptureConfigSummary where
