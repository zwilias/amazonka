...
data ExperimentSource = ExperimentSource'{_esSourceType
                                          :: !(Maybe Text),
                                          _esSourceARN :: !Text}
...
experimentSource pSourceARN_
  = ExperimentSource'{_esSourceType = Nothing,
...
instance FromJSON ExperimentSource where
        parseJSON
          = withObject "ExperimentSource"
              (\ x ->
                 ExperimentSource' <$>
                   (x .:? "SourceType") <*> (x .: "SourceArn"))

instance Hashable ExperimentSource where

instance NFData ExperimentSource where
