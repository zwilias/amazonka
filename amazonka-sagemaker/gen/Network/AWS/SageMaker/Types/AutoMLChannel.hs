...
data AutoMLChannel = AutoMLChannel'{_amlcCompressionType
...
autoMLChannel
    :: AutoMLDataSource -- ^ 'amlcDataSource'
    -> Text -- ^ 'amlcTargetAttributeName'
    -> AutoMLChannel
autoMLChannel pDataSource_ pTargetAttributeName_
  = AutoMLChannel'{_amlcCompressionType = Nothing,
...
instance FromJSON AutoMLChannel where
        parseJSON
          = withObject "AutoMLChannel"
              (\ x ->
                 AutoMLChannel' <$>
                   (x .:? "CompressionType") <*> (x .: "DataSource") <*>
                     (x .: "TargetAttributeName"))

instance Hashable AutoMLChannel where

instance NFData AutoMLChannel where
