...
data HumanLoopConfig = HumanLoopConfig'{_hlcTaskKeywords
                                        :: !(Maybe (List1 Text)),
...
--
-- * 'hlcTaskTimeLimitInSeconds' - The amount of time that a worker has to complete a task.
--
-- * 'hlcTaskAvailabilityLifetimeInSeconds' - The length of time that a task remains available for labeling by human workers.
--
...
humanLoopConfig
    :: Text -- ^ 'hlcWorkteamARN'
    -> Text -- ^ 'hlcHumanTaskUiARN'
    -> Text -- ^ 'hlcTaskTitle'
    -> Text -- ^ 'hlcTaskDescription'
    -> Natural -- ^ 'hlcTaskCount'
    -> HumanLoopConfig
humanLoopConfig pWorkteamARN_ pHumanTaskUiARN_
  pTaskTitle_ pTaskDescription_ pTaskCount_
  = HumanLoopConfig'{_hlcTaskKeywords = Nothing,
