...
data TensorBoardOutputConfig = TensorBoardOutputConfig'{_tbocLocalPath
                                                        :: !(Maybe Text),
                                                        _tbocS3OutputPath ::
                                                        !Text}
...
tensorBoardOutputConfig
    :: Text -- ^ 'tbocS3OutputPath'
    -> TensorBoardOutputConfig
tensorBoardOutputConfig pS3OutputPath_
  = TensorBoardOutputConfig'{_tbocLocalPath = Nothing,
...
instance FromJSON TensorBoardOutputConfig where
        parseJSON
          = withObject "TensorBoardOutputConfig"
              (\ x ->
                 TensorBoardOutputConfig' <$>
                   (x .:? "LocalPath") <*> (x .: "S3OutputPath"))

instance Hashable TensorBoardOutputConfig where

instance NFData TensorBoardOutputConfig where
