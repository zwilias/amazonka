...
uewacDesiredWeightsAndCapacities ::
     Lens' UpdateEndpointWeightsAndCapacities (NonEmpty DesiredWeightAndCapacity)
uewacDesiredWeightsAndCapacities =
  lens
    _uewacDesiredWeightsAndCapacities
    (\s a -> s {_uewacDesiredWeightsAndCapacities = a}) .
  _List1

instance AWSRequest UpdateEndpointWeightsAndCapacities where
  type Rs UpdateEndpointWeightsAndCapacities = UpdateEndpointWeightsAndCapacitiesResponse
  request = postJSON sageMaker
  response =
    receiveJSON
      (\s h x ->
         UpdateEndpointWeightsAndCapacitiesResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "EndpointArn"))
  ...
  toJSON UpdateEndpointWeightsAndCapacities' {..} =
    object
      (catMaybes
         ...
         , Just
             ("DesiredWeightsAndCapacities" .= _uewacDesiredWeightsAndCapacities)
         ])
...
uewacrsResponseStatus =
  lens _uewacrsResponseStatus (\s a -> s {_uewacrsResponseStatus = a})

-- | The Amazon Resource Name (ARN) of the updated endpoint.
uewacrsEndpointARN :: Lens' UpdateEndpointWeightsAndCapacitiesResponse Text
uewacrsEndpointARN =
  lens _uewacrsEndpointARN (\s a -> s {_uewacrsEndpointARN = a})
