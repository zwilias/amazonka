...
lecCreationTimeAfter =
  lens _lecCreationTimeAfter (\s a -> s {_lecCreationTimeAfter = a}) .
  mapping _Time
...
lecCreationTimeBefore =
  lens _lecCreationTimeBefore (\s a -> s {_lecCreationTimeBefore = a}) .
  mapping _Time

-- | The maximum number of training jobs to return in the response.
lecMaxResults :: Lens' ListEndpointConfigs (Maybe Natural)
lecMaxResults =
  lens _lecMaxResults (\s a -> s {_lecMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListEndpointConfigs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListEndpointConfigsResponse' <$> (x .?> "NextToken") <*>
         (pure (fromEnum s)) <*>
...
lecrsResponseStatus =
  lens _lecrsResponseStatus (\s a -> s {_lecrsResponseStatus = a})

-- | An array of endpoint configurations.
lecrsEndpointConfigs ::
     Lens' ListEndpointConfigsResponse [EndpointConfigSummary]
lecrsEndpointConfigs =
  lens _lecrsEndpointConfigs (\s a -> s {_lecrsEndpointConfigs = a}) . _Coerce
