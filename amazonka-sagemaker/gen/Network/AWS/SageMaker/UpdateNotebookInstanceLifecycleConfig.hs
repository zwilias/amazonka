...
unilcOnCreate ::
     Lens' UpdateNotebookInstanceLifecycleConfig [NotebookInstanceLifecycleHook]
unilcOnCreate =
  lens _unilcOnCreate (\s a -> s {_unilcOnCreate = a}) . _Default . _Coerce

-- | The shell script that runs every time you start a notebook instance, including when you create the notebook instance.
unilcOnStart ::
     Lens' UpdateNotebookInstanceLifecycleConfig [NotebookInstanceLifecycleHook]
unilcOnStart =
  lens _unilcOnStart (\s a -> s {_unilcOnStart = a}) . _Default . _Coerce

-- | The name of the lifecycle configuration.
unilcNotebookInstanceLifecycleConfigName ::
     Lens' UpdateNotebookInstanceLifecycleConfig Text
unilcNotebookInstanceLifecycleConfigName =
  lens
    _unilcNotebookInstanceLifecycleConfigName
    (\s a -> s {_unilcNotebookInstanceLifecycleConfigName = a})

instance AWSRequest UpdateNotebookInstanceLifecycleConfig where
  type Rs UpdateNotebookInstanceLifecycleConfig = UpdateNotebookInstanceLifecycleConfigResponse
  request = postJSON sageMaker
  response =
    receiveEmpty
      (\s h x ->
         UpdateNotebookInstanceLifecycleConfigResponse' <$> (pure (fromEnum s)))
...
unilcrsResponseStatus =
  lens _unilcrsResponseStatus (\s a -> s {_unilcrsResponseStatus = a})
