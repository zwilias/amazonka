...
decEndpointConfigName =
  lens _decEndpointConfigName (\s a -> s {_decEndpointConfigName = a})
...
instance AWSRequest DescribeEndpointConfig where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeEndpointConfigResponse' <$> (x .?> "KmsKeyId") <*>
         (pure (fromEnum s)) <*>
...
instance ToHeaders DescribeEndpointConfig where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("SageMaker.DescribeEndpointConfig" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON DescribeEndpointConfig where
  toJSON DescribeEndpointConfig' {..} =
    object (catMaybes [Just ("EndpointConfigName" .= _decEndpointConfigName)])
...
decrsEndpointConfigName =
  lens _decrsEndpointConfigName (\s a -> s {_decrsEndpointConfigName = a})

-- | The Amazon Resource Name (ARN) of the endpoint configuration.
decrsEndpointConfigARN :: Lens' DescribeEndpointConfigResponse Text
decrsEndpointConfigARN =
  lens _decrsEndpointConfigARN (\s a -> s {_decrsEndpointConfigARN = a})

-- | An array of @ProductionVariant@ objects, one for each model that you want to host at this endpoint.
decrsProductionVariants ::
     Lens' DescribeEndpointConfigResponse (NonEmpty ProductionVariant)
decrsProductionVariants =
  lens _decrsProductionVariants (\s a -> s {_decrsProductionVariants = a}) .
  _List1

-- | A timestamp that shows when the endpoint configuration was created.
decrsCreationTime :: Lens' DescribeEndpointConfigResponse UTCTime
decrsCreationTime =
  lens _decrsCreationTime (\s a -> s {_decrsCreationTime = a}) . _Time
