...
ltjLastModifiedTimeBefore =
  lens _ltjLastModifiedTimeBefore (\s a -> s {_ltjLastModifiedTimeBefore = a}) .
  mapping _Time
...
ltjCreationTimeAfter =
  lens _ltjCreationTimeAfter (\s a -> s {_ltjCreationTimeAfter = a}) .
  mapping _Time
...
ltjLastModifiedTimeAfter =
  lens _ltjLastModifiedTimeAfter (\s a -> s {_ltjLastModifiedTimeAfter = a}) .
  mapping _Time

-- | A filter that returns only training jobs created before the specified time (timestamp).
ltjCreationTimeBefore :: Lens' ListTrainingJobs (Maybe UTCTime)
ltjCreationTimeBefore =
  lens _ltjCreationTimeBefore (\s a -> s {_ltjCreationTimeBefore = a}) .
  mapping _Time
...
ltjMaxResults =
  lens _ltjMaxResults (\s a -> s {_ltjMaxResults = a}) . mapping _Nat
...
instance AWSPager ListTrainingJobs where
  page rq rs
    ...
    | otherwise = Just $ rq & ltjNextToken .~ rs ^. ltjrsNextToken
...
instance AWSRequest ListTrainingJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListTrainingJobsResponse' <$> (x .?> "NextToken") <*>
         (pure (fromEnum s)) <*>
...
ltjrsResponseStatus =
  lens _ltjrsResponseStatus (\s a -> s {_ltjrsResponseStatus = a})

-- | An array of @TrainingJobSummary@ objects, each listing a training job.
ltjrsTrainingJobSummaries :: Lens' ListTrainingJobsResponse [TrainingJobSummary]
ltjrsTrainingJobSummaries =
  lens _ltjrsTrainingJobSummaries (\s a -> s {_ltjrsTrainingJobSummaries = a}) .
  _Coerce
