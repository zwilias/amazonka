...
leLastModifiedTimeBefore =
  lens _leLastModifiedTimeBefore (\s a -> s {_leLastModifiedTimeBefore = a}) .
  mapping _Time
...
leCreationTimeAfter =
  lens _leCreationTimeAfter (\s a -> s {_leCreationTimeAfter = a}) .
  mapping _Time
...
leLastModifiedTimeAfter =
  lens _leLastModifiedTimeAfter (\s a -> s {_leLastModifiedTimeAfter = a}) .
  mapping _Time

-- | A filter that returns only endpoints that were created before the specified time (timestamp).
leCreationTimeBefore :: Lens' ListEndpoints (Maybe UTCTime)
leCreationTimeBefore =
  lens _leCreationTimeBefore (\s a -> s {_leCreationTimeBefore = a}) .
  mapping _Time
...
instance AWSRequest ListEndpoints where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListEndpointsResponse' <$> (x .?> "NextToken") <*> (pure (fromEnum s)) <*>
         (x .?> "Endpoints" .!@ mempty))
  ...
  toJSON ListEndpoints' {..} =
    object
      (catMaybes
         ...
         , ("LastModifiedTimeBefore" .=) <$> _leLastModifiedTimeBefore
         , ("CreationTimeAfter" .=) <$> _leCreationTimeAfter
         , ("NextToken" .=) <$> _leNextToken
         , ("SortOrder" .=) <$> _leSortOrder
         , ("LastModifiedTimeAfter" .=) <$> _leLastModifiedTimeAfter
