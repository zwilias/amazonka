...
--
--
-- After you run an AutoPilot job, you can find the best performing model by calling , and then deploy that model by following the steps described in <https://docs.aws.amazon.com/sagemaker/latest/dg/ex1-deploy-model.html Step 6.1: Deploy the Model to Amazon SageMaker Hosting Services> .
--
-- For information about how to use AutoPilot, see <https://docs.aws.amazon.com/sagemaker/latest/dg/autopilot-automate-model-development.html Use AutoPilot to Automate Model Development> .
--
module Network.AWS.SageMaker.CreateAutoMLJob
    (
    -- * Creating a Request
...
data CreateAutoMLJob = CreateAutoMLJob'{_camljGenerateCandidateDefinitionsOnly
                                        :: !(Maybe Bool),
                                        _camljProblemType ::
                                        !(Maybe ProblemType),
...
--
-- * 'camljGenerateCandidateDefinitionsOnly' - This will generate possible candidates without training a model. A candidate is a combination of data preprocessors, algorithms, and algorithm parameter settings.
--
...
--
-- * 'camljAutoMLJobObjective' - Defines the job's objective. You provide a MetricName and AutoML will infer minimize or maximize. If this is not provided, the most commonly used ObjectiveMetric for problem type will be selected.
--
...
--
-- * 'camljInputDataConfig' - Similar to InputDataConfig supported by Tuning. Format(s) supported: CSV. Minimum of 1000 rows.
--
-- * 'camljOutputDataConfig' - Similar to OutputDataConfig supported by Tuning. Format(s) supported: CSV.
--
-- * 'camljRoleARN' - The ARN of the role that will be used to access the data.
createAutoMLJob
    :: Text -- ^ 'camljAutoMLJobName'
    -> NonEmpty AutoMLChannel -- ^ 'camljInputDataConfig'
    -> AutoMLOutputDataConfig -- ^ 'camljOutputDataConfig'
    -> Text -- ^ 'camljRoleARN'
    -> CreateAutoMLJob
createAutoMLJob pAutoMLJobName_ pInputDataConfig_
  pOutputDataConfig_ pRoleARN_
  = CreateAutoMLJob'{_camljGenerateCandidateDefinitionsOnly
                       = Nothing,
...
instance Hashable CreateAutoMLJob where

instance NFData CreateAutoMLJob where
...
data CreateAutoMLJobResponse = CreateAutoMLJobResponse'{_camljrsResponseStatus
                                                        :: !Int,
                                                        _camljrsAutoMLJobARN ::
                                                        !Text}
...
createAutoMLJobResponse
    :: Int -- ^ 'camljrsResponseStatus'
    -> Text -- ^ 'camljrsAutoMLJobARN'
    -> CreateAutoMLJobResponse
createAutoMLJobResponse pResponseStatus_
  pAutoMLJobARN_
  = CreateAutoMLJobResponse'{_camljrsResponseStatus =
                               pResponseStatus_,
...
instance NFData CreateAutoMLJobResponse where
