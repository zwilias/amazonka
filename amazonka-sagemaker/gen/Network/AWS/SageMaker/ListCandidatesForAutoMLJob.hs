...
data ListCandidatesForAutoMLJob = ListCandidatesForAutoMLJob'{_lcfamljCandidateNameEquals
                                                              :: !(Maybe Text),
                                                              _lcfamljNextToken
                                                              :: !(Maybe Text),
                                                              _lcfamljSortOrder
...
--
-- * 'lcfamljNextToken' - If the previous response was truncated, you will receive this token. Use it in your next request to receive the next set of results.
--
...
listCandidatesForAutoMLJob
    :: Text -- ^ 'lcfamljAutoMLJobName'
    -> ListCandidatesForAutoMLJob
listCandidatesForAutoMLJob pAutoMLJobName_
  = ListCandidatesForAutoMLJob'{_lcfamljCandidateNameEquals
                                  = Nothing,
...
instance AWSPager ListCandidatesForAutoMLJob where
        page rq rs
          ...
          | otherwise =
            Just $ rq &
              lcfamljNextToken .~ rs ^. lcfamljrsNextToken
...
instance Hashable ListCandidatesForAutoMLJob where

instance NFData ListCandidatesForAutoMLJob where
...
data ListCandidatesForAutoMLJobResponse = ListCandidatesForAutoMLJobResponse'{_lcfamljrsNextToken
                                                                              ::
                                                                              !(Maybe
                                                                                  Text),
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lcfamljrsNextToken' - If the previous response was truncated, you will receive this token. Use it in your next request to receive the next set of results.
--
-- * 'lcfamljrsResponseStatus' - -- | The response status code.
--
-- * 'lcfamljrsCandidates' - Summaries about the Candidates.
listCandidatesForAutoMLJobResponse
    :: Int -- ^ 'lcfamljrsResponseStatus'
    -> ListCandidatesForAutoMLJobResponse
listCandidatesForAutoMLJobResponse pResponseStatus_
  = ListCandidatesForAutoMLJobResponse'{_lcfamljrsNextToken
                                          = Nothing,
                                        _lcfamljrsResponseStatus =
                                          pResponseStatus_,
