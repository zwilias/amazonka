...
lniLastModifiedTimeBefore =
  lens _lniLastModifiedTimeBefore (\s a -> s {_lniLastModifiedTimeBefore = a}) .
  mapping _Time

-- | A string in the name of a notebook instances lifecycle configuration associated with this notebook instance. This filter returns only notebook instances associated with a lifecycle configuration with a name that contains the specified string.
lniNotebookInstanceLifecycleConfigNameContains ::
     Lens' ListNotebookInstances (Maybe Text)
lniNotebookInstanceLifecycleConfigNameContains =
  lens
    _lniNotebookInstanceLifecycleConfigNameContains
    (\s a -> s {_lniNotebookInstanceLifecycleConfigNameContains = a})

-- | A filter that returns only notebook instances that were created after the specified time (timestamp).
lniCreationTimeAfter :: Lens' ListNotebookInstances (Maybe UTCTime)
lniCreationTimeAfter =
  lens _lniCreationTimeAfter (\s a -> s {_lniCreationTimeAfter = a}) .
  mapping _Time
...
lniLastModifiedTimeAfter =
  lens _lniLastModifiedTimeAfter (\s a -> s {_lniLastModifiedTimeAfter = a}) .
  mapping _Time

-- | A filter that returns only notebook instances that were created before the specified time (timestamp).
lniCreationTimeBefore :: Lens' ListNotebookInstances (Maybe UTCTime)
lniCreationTimeBefore =
  lens _lniCreationTimeBefore (\s a -> s {_lniCreationTimeBefore = a}) .
  mapping _Time
...
lniMaxResults =
  lens _lniMaxResults (\s a -> s {_lniMaxResults = a}) . mapping _Nat
...
instance ToHeaders ListNotebookInstances where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("SageMaker.ListNotebookInstances" :: ByteString)
...
lnirsNotebookInstances ::
     Lens' ListNotebookInstancesResponse [NotebookInstanceSummary]
lnirsNotebookInstances =
  lens _lnirsNotebookInstances (\s a -> s {_lnirsNotebookInstances = a}) .
  _Default . _Coerce
...
lnirsResponseStatus =
  lens _lnirsResponseStatus (\s a -> s {_lnirsResponseStatus = a})
