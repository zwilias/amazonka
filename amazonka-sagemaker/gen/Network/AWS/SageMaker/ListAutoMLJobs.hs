...
data ListAutoMLJobs = ListAutoMLJobs'{_lamljNameContains
                                      :: !(Maybe Text),
...
--
-- * 'lamljNextToken' - If the previous response was truncated, you will receive this token. Use it in your next request to receive the next set of results.
--
...
instance AWSPager ListAutoMLJobs where
        page rq rs
          ...
          | otherwise =
            Just $ rq & lamljNextToken .~ rs ^. lamljrsNextToken
...
instance Hashable ListAutoMLJobs where

instance NFData ListAutoMLJobs where
...
data ListAutoMLJobsResponse = ListAutoMLJobsResponse'{_lamljrsNextToken
                                                      :: !(Maybe Text),
                                                      _lamljrsResponseStatus ::
                                                      !Int,
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lamljrsNextToken' - If the previous response was truncated, you will receive this token. Use it in your next request to receive the next set of results.
--
-- * 'lamljrsResponseStatus' - -- | The response status code.
--
-- * 'lamljrsAutoMLJobSummaries' - Returns a summary list of jobs.
listAutoMLJobsResponse
    :: Int -- ^ 'lamljrsResponseStatus'
    -> ListAutoMLJobsResponse
listAutoMLJobsResponse pResponseStatus_
  = ListAutoMLJobsResponse'{_lamljrsNextToken =
                              Nothing,
...
instance NFData ListAutoMLJobsResponse where
