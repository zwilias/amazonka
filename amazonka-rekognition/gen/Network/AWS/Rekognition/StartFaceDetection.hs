...
sfdNotificationChannel =
  lens _sfdNotificationChannel (\s a -> s {_sfdNotificationChannel = a})

-- | Idempotent token used to identify the start request. If you use the same token with multiple @StartFaceDetection@ requests, the same @JobId@ is returned. Use @ClientRequestToken@ to prevent the same job from being accidently started more than once.
sfdClientRequestToken :: Lens' StartFaceDetection (Maybe Text)
sfdClientRequestToken =
  lens _sfdClientRequestToken (\s a -> s {_sfdClientRequestToken = a})
...
instance AWSRequest StartFaceDetection where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartFaceDetectionResponse' <$> (x .?> "JobId") <*> (pure (fromEnum s)))
...
sfdrsResponseStatus =
  lens _sfdrsResponseStatus (\s a -> s {_sfdrsResponseStatus = a})
