...
gptMaxResults =
  lens _gptMaxResults (\s a -> s {_gptMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetPersonTracking where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetPersonTrackingResponse' <$> (x .?> "NextToken") <*>
         (x .?> "VideoMetadata") <*>
         (x .?> "StatusMessage") <*>
         (x .?> "JobStatus") <*>
         (x .?> "Persons" .!@ mempty) <*>
...
gptrsVideoMetadata =
  lens _gptrsVideoMetadata (\s a -> s {_gptrsVideoMetadata = a})

-- | If the job fails, @StatusMessage@ provides a descriptive error message.
gptrsStatusMessage :: Lens' GetPersonTrackingResponse (Maybe Text)
gptrsStatusMessage =
  lens _gptrsStatusMessage (\s a -> s {_gptrsStatusMessage = a})
...
gptrsPersons =
  lens _gptrsPersons (\s a -> s {_gptrsPersons = a}) . _Default . _Coerce

-- | -- | The response status code.
gptrsResponseStatus :: Lens' GetPersonTrackingResponse Int
gptrsResponseStatus =
  lens _gptrsResponseStatus (\s a -> s {_gptrsResponseStatus = a})
