...
gldMaxResults =
  lens _gldMaxResults (\s a -> s {_gldMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetLabelDetection where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetLabelDetectionResponse' <$> (x .?> "NextToken") <*>
         (x .?> "VideoMetadata") <*>
         (x .?> "StatusMessage") <*>
         (x .?> "Labels" .!@ mempty) <*>
         (x .?> "JobStatus") <*>
