...
instance AWSRequest DescribeStreamProcessor where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeStreamProcessorResponse' <$> (x .?> "Status") <*>
         (x .?> "Settings") <*>
         (x .?> "Input") <*>
         (x .?> "Output") <*>
...
dsprsStatus ::
     Lens' DescribeStreamProcessorResponse (Maybe StreamProcessorStatus)
dsprsStatus = lens _dsprsStatus (\s a -> s {_dsprsStatus = a})

-- | Face recognition input parameters that are being used by the stream processor. Includes the collection to use for face recognition and the face attributes to detect.
dsprsSettings ::
     Lens' DescribeStreamProcessorResponse (Maybe StreamProcessorSettings)
...
dsprsOutput ::
     Lens' DescribeStreamProcessorResponse (Maybe StreamProcessorOutput)
...
dsprsStreamProcessorARN =
  lens _dsprsStreamProcessorARN (\s a -> s {_dsprsStreamProcessorARN = a})

-- | Detailed status message about the stream processor.
dsprsStatusMessage :: Lens' DescribeStreamProcessorResponse (Maybe Text)
dsprsStatusMessage =
  lens _dsprsStatusMessage (\s a -> s {_dsprsStatusMessage = a})
...
dsprsCreationTimestamp =
  lens _dsprsCreationTimestamp (\s a -> s {_dsprsCreationTimestamp = a}) .
  mapping _Time

-- | The time, in Unix format, the stream processor was last updated. For example, when the stream processor moves from a running state to a failed state, or when the user starts or stops the stream processor.
dsprsLastUpdateTimestamp ::
     Lens' DescribeStreamProcessorResponse (Maybe UTCTime)
dsprsLastUpdateTimestamp =
  lens _dsprsLastUpdateTimestamp (\s a -> s {_dsprsLastUpdateTimestamp = a}) .
  mapping _Time
