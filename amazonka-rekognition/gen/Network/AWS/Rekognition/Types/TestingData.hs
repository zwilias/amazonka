...
data TestingData = TestingData'{_tdAssets ::
                                !(Maybe [Asset]),
...
instance FromJSON TestingData where
        parseJSON
          = withObject "TestingData"
              (\ x ->
                 TestingData' <$>
                   (x .:? "Assets" .!= mempty) <*> (x .:? "AutoCreate"))

instance Hashable TestingData where

instance NFData TestingData where
