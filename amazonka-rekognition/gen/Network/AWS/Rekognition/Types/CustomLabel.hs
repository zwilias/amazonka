...
data CustomLabel = CustomLabel'{_clConfidence ::
                                !(Maybe Double),
                                _clName :: !(Maybe Text),
...
customLabel
    :: CustomLabel
customLabel
  = CustomLabel'{_clConfidence = Nothing,
                 _clName = Nothing, _clGeometry = Nothing}
...
instance FromJSON CustomLabel where
        parseJSON
          = withObject "CustomLabel"
              (\ x ->
                 CustomLabel' <$>
                   (x .:? "Confidence") <*> (x .:? "Name") <*>
                     (x .:? "Geometry"))

instance Hashable CustomLabel where

instance NFData CustomLabel where
