...
instance FromText Attribute where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Attribute from value: '" <>
        e <> "'. Accepted values: all, default"
...
instance FromText CelebrityRecognitionSortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CelebrityRecognitionSortBy from value: '" <>
        e <> "'. Accepted values: id, timestamp"
...
instance FromText ContentModerationSortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ContentModerationSortBy from value: '" <>
        e <> "'. Accepted values: name, timestamp"
...
instance FromText EmotionName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EmotionName from value: '" <>
        e <>
...
instance FromText FaceAttributes where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FaceAttributes from value: '" <>
        e <> "'. Accepted values: all, default"
...
instance FromText FaceSearchSortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FaceSearchSortBy from value: '" <>
        e <> "'. Accepted values: index, timestamp"
...
instance FromText GenderType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing GenderType from value: '" <>
        e <> "'. Accepted values: female, male"
...
instance FromText LabelDetectionSortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LabelDetectionSortBy from value: '" <>
        e <> "'. Accepted values: name, timestamp"
...
instance FromText LandmarkType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LandmarkType from value: '" <>
        e <>
...
instance FromText OrientationCorrection where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrientationCorrection from value: '" <>
        e <> "'. Accepted values: rotate_0, rotate_180, rotate_270, rotate_90"
...
instance FromText PersonTrackingSortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PersonTrackingSortBy from value: '" <>
        e <> "'. Accepted values: index, timestamp"
...
instance FromText StreamProcessorStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StreamProcessorStatus from value: '" <>
        e <> "'. Accepted values: failed, running, starting, stopped, stopping"
...
instance FromText TextTypes where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TextTypes from value: '" <>
        e <> "'. Accepted values: line, word"
...
instance FromText VideoJobStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VideoJobStatus from value: '" <>
