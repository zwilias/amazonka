...
instance AWSRequest DeleteFaces where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteFacesResponse' <$> (x .?> "DeletedFaces") <*> (pure (fromEnum s)))
...
instance ToHeaders DeleteFaces where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("RekognitionService.DeleteFaces" :: ByteString)
...
dfsrsDeletedFaces =
  lens _dfsrsDeletedFaces (\s a -> s {_dfsrsDeletedFaces = a}) . mapping _List1
