...
sptNotificationChannel =
  lens _sptNotificationChannel (\s a -> s {_sptNotificationChannel = a})

-- | Idempotent token used to identify the start request. If you use the same token with multiple @StartPersonTracking@ requests, the same @JobId@ is returned. Use @ClientRequestToken@ to prevent the same job from being accidently started more than once.
sptClientRequestToken :: Lens' StartPersonTracking (Maybe Text)
sptClientRequestToken =
  lens _sptClientRequestToken (\s a -> s {_sptClientRequestToken = a})
...
instance AWSRequest StartPersonTracking where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartPersonTrackingResponse' <$> (x .?> "JobId") <*>
         (pure (fromEnum s)))
...
sptrsResponseStatus =
  lens _sptrsResponseStatus (\s a -> s {_sptrsResponseStatus = a})
