...
sfFaceMatchThreshold =
  lens _sfFaceMatchThreshold (\s a -> s {_sfFaceMatchThreshold = a})
...
instance AWSRequest SearchFaces where
  ...
  response =
    receiveJSON
      (\s h x ->
         SearchFacesResponse' <$> (x .?> "FaceMatches" .!@ mempty) <*>
         (x .?> "FaceModelVersion") <*>
         (x .?> "SearchedFaceId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders SearchFaces where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("RekognitionService.SearchFaces" :: ByteString)
...
sfrsFaceMatches =
  lens _sfrsFaceMatches (\s a -> s {_sfrsFaceMatches = a}) . _Default . _Coerce

-- | Version number of the face detection model associated with the input collection (@CollectionId@ ).
sfrsFaceModelVersion :: Lens' SearchFacesResponse (Maybe Text)
sfrsFaceModelVersion =
  lens _sfrsFaceModelVersion (\s a -> s {_sfrsFaceModelVersion = a})

-- | ID of the face that was searched for matches in a collection.
sfrsSearchedFaceId :: Lens' SearchFacesResponse (Maybe Text)
sfrsSearchedFaceId =
  lens _sfrsSearchedFaceId (\s a -> s {_sfrsSearchedFaceId = a})

-- | -- | The response status code.
sfrsResponseStatus :: Lens' SearchFacesResponse Int
sfrsResponseStatus =
  lens _sfrsResponseStatus (\s a -> s {_sfrsResponseStatus = a})
