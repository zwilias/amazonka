...
lspMaxResults =
  lens _lspMaxResults (\s a -> s {_lspMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListStreamProcessors where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListStreamProcessorsResponse' <$> (x .?> "StreamProcessors" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
lsprsStreamProcessors =
  lens _lsprsStreamProcessors (\s a -> s {_lsprsStreamProcessors = a}) .
  _Default . _Coerce
...
lsprsResponseStatus =
  lens _lsprsResponseStatus (\s a -> s {_lsprsResponseStatus = a})
