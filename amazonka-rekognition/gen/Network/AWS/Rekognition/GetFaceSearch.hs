...
gfsMaxResults =
  lens _gfsMaxResults (\s a -> s {_gfsMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetFaceSearch where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetFaceSearchResponse' <$> (x .?> "NextToken") <*>
         (x .?> "VideoMetadata") <*>
         (x .?> "StatusMessage") <*>
         (x .?> "JobStatus") <*>
         (x .?> "Persons" .!@ mempty) <*>
...
instance ToHeaders GetFaceSearch where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("RekognitionService.GetFaceSearch" :: ByteString)
...
gfsrsVideoMetadata =
  lens _gfsrsVideoMetadata (\s a -> s {_gfsrsVideoMetadata = a})

-- | If the job fails, @StatusMessage@ provides a descriptive error message.
gfsrsStatusMessage :: Lens' GetFaceSearchResponse (Maybe Text)
gfsrsStatusMessage =
  lens _gfsrsStatusMessage (\s a -> s {_gfsrsStatusMessage = a})
...
gfsrsPersons =
  lens _gfsrsPersons (\s a -> s {_gfsrsPersons = a}) . _Default . _Coerce
