...
gfdMaxResults =
  lens _gfdMaxResults (\s a -> s {_gfdMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetFaceDetection where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetFaceDetectionResponse' <$> (x .?> "NextToken") <*>
         (x .?> "VideoMetadata") <*>
         (x .?> "StatusMessage") <*>
         (x .?> "Faces" .!@ mempty) <*>
         (x .?> "JobStatus") <*>
...
gfdrsVideoMetadata =
  lens _gfdrsVideoMetadata (\s a -> s {_gfdrsVideoMetadata = a})

-- | If the job fails, @StatusMessage@ provides a descriptive error message.
gfdrsStatusMessage :: Lens' GetFaceDetectionResponse (Maybe Text)
gfdrsStatusMessage =
  lens _gfdrsStatusMessage (\s a -> s {_gfdrsStatusMessage = a})
