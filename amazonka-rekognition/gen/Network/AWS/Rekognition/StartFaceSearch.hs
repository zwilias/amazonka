...
sfsFaceMatchThreshold =
  lens _sfsFaceMatchThreshold (\s a -> s {_sfsFaceMatchThreshold = a})
...
sfsNotificationChannel =
  lens _sfsNotificationChannel (\s a -> s {_sfsNotificationChannel = a})

-- | Idempotent token used to identify the start request. If you use the same token with multiple @StartFaceSearch@ requests, the same @JobId@ is returned. Use @ClientRequestToken@ to prevent the same job from being accidently started more than once.
sfsClientRequestToken :: Lens' StartFaceSearch (Maybe Text)
sfsClientRequestToken =
  lens _sfsClientRequestToken (\s a -> s {_sfsClientRequestToken = a})
...
instance AWSRequest StartFaceSearch where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartFaceSearchResponse' <$> (x .?> "JobId") <*> (pure (fromEnum s)))
...
sfsrsResponseStatus =
  lens _sfsrsResponseStatus (\s a -> s {_sfsrsResponseStatus = a})
