...
cfSimilarityThreshold =
  lens _cfSimilarityThreshold (\s a -> s {_cfSimilarityThreshold = a})
...
instance AWSRequest CompareFaces where
  ...
  response =
    receiveJSON
      (\s h x ->
         CompareFacesResponse' <$> (x .?> "FaceMatches" .!@ mempty) <*>
...
instance ToHeaders CompareFaces where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("RekognitionService.CompareFaces" :: ByteString)
...
cfrsFaceMatches =
  lens _cfrsFaceMatches (\s a -> s {_cfrsFaceMatches = a}) . _Default . _Coerce

-- | An array of faces in the target image that did not match the source image face.
cfrsUnmatchedFaces :: Lens' CompareFacesResponse [ComparedFace]
cfrsUnmatchedFaces =
  lens _cfrsUnmatchedFaces (\s a -> s {_cfrsUnmatchedFaces = a}) .
  _Default . _Coerce
...
cfrsTargetImageOrientationCorrection ::
     Lens' CompareFacesResponse (Maybe OrientationCorrection)
cfrsTargetImageOrientationCorrection =
  lens
    _cfrsTargetImageOrientationCorrection
    (\s a -> s {_cfrsTargetImageOrientationCorrection = a})
...
cfrsSourceImageOrientationCorrection =
  lens
    ...
    (\s a -> s {_cfrsSourceImageOrientationCorrection = a})

-- | The face in the source image that was used for comparison.
cfrsSourceImageFace ::
     Lens' CompareFacesResponse (Maybe ComparedSourceImageFace)
cfrsSourceImageFace =
  lens _cfrsSourceImageFace (\s a -> s {_cfrsSourceImageFace = a})
