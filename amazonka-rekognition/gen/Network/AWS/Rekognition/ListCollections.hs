...
instance AWSRequest ListCollections where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListCollectionsResponse' <$> (x .?> "CollectionIds" .!@ mempty) <*>
         (x .?> "NextToken") <*>
...
lcrsCollectionIds =
  lens _lcrsCollectionIds (\s a -> s {_lcrsCollectionIds = a}) .
  _Default . _Coerce
...
lcrsFaceModelVersions =
  lens _lcrsFaceModelVersions (\s a -> s {_lcrsFaceModelVersions = a}) .
  _Default . _Coerce
