...
coeWhitelist =
  lens _coeWhitelist (\s a -> s {_coeWhitelist = a}) . _Default . _Coerce
...
coeStartoverWindowSeconds =
  lens _coeStartoverWindowSeconds (\s a -> s {_coeStartoverWindowSeconds = a})
...
coeTimeDelaySeconds =
  lens _coeTimeDelaySeconds (\s a -> s {_coeTimeDelaySeconds = a})

-- | Undocumented member.
coeCmafPackage ::
     Lens' CreateOriginEndpoint (Maybe CmafPackageCreateOrUpdateParameters)
...
instance AWSRequest CreateOriginEndpoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateOriginEndpointResponse' <$> (x .?> "whitelist" .!@ mempty) <*>
         (x .?> "hlsPackage") <*>
         (x .?> "arn") <*>
         (x .?> "manifestName") <*>
         (x .?> "url") <*>
...
coersWhitelist =
  lens _coersWhitelist (\s a -> s {_coersWhitelist = a}) . _Default . _Coerce
...
coersStartoverWindowSeconds =
  lens
    ...
    (\s a -> s {_coersStartoverWindowSeconds = a})
...
coersTimeDelaySeconds =
  lens _coersTimeDelaySeconds (\s a -> s {_coersTimeDelaySeconds = a})
...
coersResponseStatus =
  lens _coersResponseStatus (\s a -> s {_coersResponseStatus = a})
