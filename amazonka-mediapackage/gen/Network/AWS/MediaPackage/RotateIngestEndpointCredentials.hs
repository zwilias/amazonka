...
data RotateIngestEndpointCredentials = RotateIngestEndpointCredentials'{_riecIngestEndpointId
                                                                        ::
                                                                        !Text,
                                                                        _riecId
                                                                        ::
...
rotateIngestEndpointCredentials
    :: Text -- ^ 'riecIngestEndpointId'
    -> Text -- ^ 'riecId'
    -> RotateIngestEndpointCredentials
rotateIngestEndpointCredentials pIngestEndpointId_
  pId_
  = RotateIngestEndpointCredentials'{_riecIngestEndpointId
                                       = pIngestEndpointId_,
                                     _riecId = pId_}
...
instance AWSRequest RotateIngestEndpointCredentials
         where
...
instance NFData RotateIngestEndpointCredentials where

instance ToHeaders RotateIngestEndpointCredentials
         where
        toHeaders
          = const
              (mconcat
...
instance ToPath RotateIngestEndpointCredentials where
        toPath RotateIngestEndpointCredentials'{..}
          = mconcat
              ["/channels/", toBS _riecId, "/ingest_endpoints/",
               toBS _riecIngestEndpointId, "/credentials"]

instance ToQuery RotateIngestEndpointCredentials
         where
        toQuery = const mempty

-- | /See:/ 'rotateIngestEndpointCredentialsResponse' smart constructor.
data RotateIngestEndpointCredentialsResponse = RotateIngestEndpointCredentialsResponse'{_riecrsHlsIngest
                                                                                        ::
                                                                                        !(Maybe
                                                                                            HlsIngest),
                                                                                        _riecrsARN
...
rotateIngestEndpointCredentialsResponse
    :: Int -- ^ 'riecrsResponseStatus'
    -> RotateIngestEndpointCredentialsResponse
rotateIngestEndpointCredentialsResponse
  pResponseStatus_
  = RotateIngestEndpointCredentialsResponse'{_riecrsHlsIngest
                                               = Nothing,
                                             _riecrsARN = Nothing,
                                             _riecrsId = Nothing,
