...
data ListHarvestJobs = ListHarvestJobs'{_lhjIncludeStatus
                                        :: !(Maybe Text),
...
listHarvestJobs
    :: ListHarvestJobs
listHarvestJobs
  = ListHarvestJobs'{_lhjIncludeStatus = Nothing,
...
instance AWSPager ListHarvestJobs where
        page rq rs
          ...
          | otherwise =
            Just $ rq & lhjNextToken .~ rs ^. lhjrsNextToken
...
instance Hashable ListHarvestJobs where

instance NFData ListHarvestJobs where
...
data ListHarvestJobsResponse = ListHarvestJobsResponse'{_lhjrsHarvestJobs
                                                        ::
                                                        !(Maybe [HarvestJob]),
                                                        _lhjrsNextToken ::
                                                        !(Maybe Text),
                                                        _lhjrsResponseStatus ::
                                                        !Int}
...
listHarvestJobsResponse
    :: Int -- ^ 'lhjrsResponseStatus'
    -> ListHarvestJobsResponse
listHarvestJobsResponse pResponseStatus_
  = ListHarvestJobsResponse'{_lhjrsHarvestJobs =
                               Nothing,
...
instance NFData ListHarvestJobsResponse where
