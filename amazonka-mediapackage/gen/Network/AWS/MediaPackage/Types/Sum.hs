...
instance FromText AdMarkers where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AdMarkers from value: '" <>
        e <> "'. Accepted values: none, passthrough, scte35_enhanced"
...
instance FromText EncryptionMethod where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EncryptionMethod from value: '" <>
        e <> "'. Accepted values: aes_128, sample_aes"
...
instance FromText PlaylistType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PlaylistType from value: '" <>
        e <> "'. Accepted values: event, none, vod"
...
instance FromText Profile where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Profile from value: '" <>
...
instance FromText StreamOrder where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StreamOrder from value: '" <>
        e <>
