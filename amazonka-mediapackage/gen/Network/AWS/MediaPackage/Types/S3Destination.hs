...
data S3Destination = S3Destination'{_sdManifestKey ::
                                    !Text,
                                    _sdBucketName :: !Text, _sdRoleARN :: !Text}
...
s3Destination pManifestKey_ pBucketName_ pRoleARN_
  = S3Destination'{_sdManifestKey = pManifestKey_,
                   _sdBucketName = pBucketName_, _sdRoleARN = pRoleARN_}
...
instance FromJSON S3Destination where
        parseJSON
          = withObject "S3Destination"
              (\ x ->
                 S3Destination' <$>
                   (x .: "manifestKey") <*> (x .: "bucketName") <*>
                     (x .: "roleArn"))

instance Hashable S3Destination where

instance NFData S3Destination where
