...
instance AWSRequest DescribeOriginEndpoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeOriginEndpointResponse' <$> (x .?> "whitelist" .!@ mempty) <*>
         (x .?> "hlsPackage") <*>
         (x .?> "arn") <*>
         (x .?> "manifestName") <*>
         (x .?> "url") <*>
...
desrsWhitelist =
  lens _desrsWhitelist (\s a -> s {_desrsWhitelist = a}) . _Default . _Coerce
...
desrsStartoverWindowSeconds =
  lens
    ...
    (\s a -> s {_desrsStartoverWindowSeconds = a})
...
desrsTimeDelaySeconds =
  lens _desrsTimeDelaySeconds (\s a -> s {_desrsTimeDelaySeconds = a})
