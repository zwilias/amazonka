...
ctSSESpecification =
  lens _ctSSESpecification (\s a -> s {_ctSSESpecification = a})
...
ctGlobalSecondaryIndexes =
  lens _ctGlobalSecondaryIndexes (\s a -> s {_ctGlobalSecondaryIndexes = a}) .
  _Default . _Coerce
...
ctStreamSpecification =
  lens _ctStreamSpecification (\s a -> s {_ctStreamSpecification = a})
  ...
  toJSON CreateTable' {..} =
    object
      (catMaybes
         ...
         , Just ("ProvisionedThroughput" .= _ctProvisionedThroughput)
         ])
