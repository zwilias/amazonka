...
instance AWSRequest DeleteBackup where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteBackupResponse' <$> (x .?> "BackupDescription") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DeleteBackup where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DynamoDB_20120810.DeleteBackup" :: ByteString)
...
dbrsBackupDescription =
  lens _dbrsBackupDescription (\s a -> s {_dbrsBackupDescription = a})
