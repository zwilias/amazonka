...
dgtGlobalTableName =
  lens _dgtGlobalTableName (\s a -> s {_dgtGlobalTableName = a})
...
instance AWSRequest DescribeGlobalTable where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeGlobalTableResponse' <$> (x .?> "GlobalTableDescription") <*>
         (pure (fromEnum s)))
...
instance ToJSON DescribeGlobalTable where
  toJSON DescribeGlobalTable' {..} =
    object (catMaybes [Just ("GlobalTableName" .= _dgtGlobalTableName)])
...
dgtrsGlobalTableDescription ::
     Lens' DescribeGlobalTableResponse (Maybe GlobalTableDescription)
dgtrsGlobalTableDescription =
  lens
    _dgtrsGlobalTableDescription
    (\s a -> s {_dgtrsGlobalTableDescription = a})

-- | -- | The response status code.
dgtrsResponseStatus :: Lens' DescribeGlobalTableResponse Int
dgtrsResponseStatus =
  lens _dgtrsResponseStatus (\s a -> s {_dgtrsResponseStatus = a})
