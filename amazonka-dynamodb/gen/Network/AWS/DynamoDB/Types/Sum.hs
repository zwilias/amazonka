...
instance FromText AttributeAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AttributeAction from value: '" <>
        e <> "'. Accepted values: add, delete, put"
...
instance FromText BackupStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BackupStatus from value: '" <>
        e <> "'. Accepted values: available, creating, deleted"
...
instance FromText ConditionalOperator where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConditionalOperator from value: '" <>
        e <> "'. Accepted values: and, or"
...
instance FromText ContinuousBackupsStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ContinuousBackupsStatus from value: '" <>
...
instance FromText GlobalTableStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing GlobalTableStatus from value: '" <>
...
instance FromText KeyType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing KeyType from value: '" <>
        e <> "'. Accepted values: hash, range"
...
instance FromText PointInTimeRecoveryStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PointInTimeRecoveryStatus from value: '" <>
        e <> "'. Accepted values: disabled, enabled"
...
instance FromText ProjectionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProjectionType from value: '" <>
        e <> "'. Accepted values: all, include, keys_only"
...
instance FromText ReplicaStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReplicaStatus from value: '" <>
        e <> "'. Accepted values: active, creating, deleting, updating"
...
instance FromText ReturnConsumedCapacity where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReturnConsumedCapacity from value: '" <>
        e <> "'. Accepted values: indexes, none, total"
...
instance FromText ReturnItemCollectionMetrics where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReturnItemCollectionMetrics from value: '" <>
        e <> "'. Accepted values: none, size"
...
instance FromText ReturnValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReturnValue from value: '" <>
        e <>
...
instance FromText SSEStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SSEStatus from value: '" <>
        e <> "'. Accepted values: disabled, disabling, enabled, enabling"
...
instance FromText ScalarAttributeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ScalarAttributeType from value: '" <>
        e <> "'. Accepted values: b, n, s"
...
instance FromText Select where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Select from value: '" <>
        e <>
...
instance FromText StreamViewType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StreamViewType from value: '" <>
        e <>
...
instance FromText TableStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TableStatus from value: '" <>
        e <> "'. Accepted values: active, creating, deleting, updating"
...
instance FromText TimeToLiveStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TimeToLiveStatus from value: '" <>
        e <> "'. Accepted values: disabled, disabling, enabled, enabling"
