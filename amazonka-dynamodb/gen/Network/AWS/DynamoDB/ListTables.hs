...
ltExclusiveStartTableName =
  lens _ltExclusiveStartTableName (\s a -> s {_ltExclusiveStartTableName = a})
...
instance AWSPager ListTables where
  page rq rs
    ...
    | otherwise =
      Just $ rq & ltExclusiveStartTableName .~ rs ^. ltrsLastEvaluatedTableName
...
instance AWSRequest ListTables where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListTablesResponse' <$> (x .?> "LastEvaluatedTableName") <*>
         (x .?> "TableNames" .!@ mempty) <*>
         (pure (fromEnum s)))
...
ltrsLastEvaluatedTableName =
  lens _ltrsLastEvaluatedTableName (\s a -> s {_ltrsLastEvaluatedTableName = a})
...
ltrsTableNames =
  lens _ltrsTableNames (\s a -> s {_ltrsTableNames = a}) . _Default . _Coerce
