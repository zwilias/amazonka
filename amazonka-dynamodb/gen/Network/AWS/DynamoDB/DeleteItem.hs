...
diExpressionAttributeNames =
  lens _diExpressionAttributeNames (\s a -> s {_diExpressionAttributeNames = a}) .
  _Default . _Map
...
diReturnConsumedCapacity =
  lens _diReturnConsumedCapacity (\s a -> s {_diReturnConsumedCapacity = a})

-- | Determines whether item collection metrics are returned. If set to @SIZE@ , the response includes statistics about item collections, if any, that were modified during the operation are returned in the response. If set to @NONE@ (the default), no statistics are returned.
diReturnItemCollectionMetrics ::
     Lens' DeleteItem (Maybe ReturnItemCollectionMetrics)
diReturnItemCollectionMetrics =
  lens
...
instance AWSRequest DeleteItem where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteItemResponse' <$> (x .?> "ItemCollectionMetrics") <*>
         (x .?> "ConsumedCapacity") <*>
         (x .?> "Attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
dirsItemCollectionMetrics =
  lens _dirsItemCollectionMetrics (\s a -> s {_dirsItemCollectionMetrics = a})
...
dirsAttributes =
  lens _dirsAttributes (\s a -> s {_dirsAttributes = a}) . _Default . _Map
