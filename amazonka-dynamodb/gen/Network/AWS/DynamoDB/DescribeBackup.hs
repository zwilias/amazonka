...
instance AWSRequest DescribeBackup where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeBackupResponse' <$> (x .?> "BackupDescription") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeBackup where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DynamoDB_20120810.DescribeBackup" :: ByteString)
...
desrsBackupDescription =
  lens _desrsBackupDescription (\s a -> s {_desrsBackupDescription = a})
