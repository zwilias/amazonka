...
lbTimeRangeUpperBound =
  lens _lbTimeRangeUpperBound (\s a -> s {_lbTimeRangeUpperBound = a}) .
  mapping _Time

-- | Only backups created after this time are listed. @TimeRangeLowerBound@ is inclusive.
lbTimeRangeLowerBound :: Lens' ListBackups (Maybe UTCTime)
lbTimeRangeLowerBound =
  lens _lbTimeRangeLowerBound (\s a -> s {_lbTimeRangeLowerBound = a}) .
  mapping _Time
...
lbExclusiveStartBackupARN =
  lens _lbExclusiveStartBackupARN (\s a -> s {_lbExclusiveStartBackupARN = a})
...
instance AWSPager ListBackups where
  page rq rs
    ...
    | otherwise =
      Just $ rq & lbExclusiveStartBackupARN .~ rs ^. lbrsLastEvaluatedBackupARN
...
instance AWSRequest ListBackups where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListBackupsResponse' <$> (x .?> "BackupSummaries" .!@ mempty) <*>
...
lbrsBackupSummaries =
  lens _lbrsBackupSummaries (\s a -> s {_lbrsBackupSummaries = a}) .
  _Default . _Coerce

-- | Last evaluated BackupARN.
lbrsLastEvaluatedBackupARN :: Lens' ListBackupsResponse (Maybe Text)
lbrsLastEvaluatedBackupARN =
  lens _lbrsLastEvaluatedBackupARN (\s a -> s {_lbrsLastEvaluatedBackupARN = a})
