...
data ContentRedaction = ContentRedaction'{_crRedactionType
                                          :: !RedactionType,
                                          _crRedactionOutput ::
                                          !RedactionOutput}
...
contentRedaction
    :: RedactionType -- ^ 'crRedactionType'
    -> RedactionOutput -- ^ 'crRedactionOutput'
    -> ContentRedaction
contentRedaction pRedactionType_ pRedactionOutput_
  = ContentRedaction'{_crRedactionType =
                        pRedactionType_,
...
instance FromJSON ContentRedaction where
        parseJSON
          = withObject "ContentRedaction"
              (\ x ->
                 ContentRedaction' <$>
                   (x .: "RedactionType") <*> (x .: "RedactionOutput"))

instance Hashable ContentRedaction where

instance NFData ContentRedaction where
