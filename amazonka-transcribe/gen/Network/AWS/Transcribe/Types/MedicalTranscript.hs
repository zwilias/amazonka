...
newtype MedicalTranscript = MedicalTranscript'{_mtTranscriptFileURI
                                               :: Maybe Text}
...
instance FromJSON MedicalTranscript where
        parseJSON
          = withObject "MedicalTranscript"
              (\ x ->
                 MedicalTranscript' <$> (x .:? "TranscriptFileUri"))

instance Hashable MedicalTranscript where

instance NFData MedicalTranscript where
