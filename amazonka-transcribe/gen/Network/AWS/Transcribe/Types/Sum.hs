...
instance FromText MediaFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MediaFormat from value: '" <>
        e <> "'. Accepted values: flac, mp3, mp4, wav"
...
instance FromText TranscriptionJobStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TranscriptionJobStatus from value: '" <>
        e <> "'. Accepted values: completed, failed, in_progress"
...
instance FromText VocabularyState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VocabularyState from value: '" <>
        e <> "'. Accepted values: failed, pending, ready"
