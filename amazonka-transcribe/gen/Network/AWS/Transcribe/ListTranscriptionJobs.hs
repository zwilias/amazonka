...
ltjJobNameContains =
  lens _ltjJobNameContains (\s a -> s {_ltjJobNameContains = a})
...
instance AWSRequest ListTranscriptionJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListTranscriptionJobsResponse' <$> (x .?> "Status") <*>
         (x .?> "NextToken") <*>
...
instance ToHeaders ListTranscriptionJobs where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Transcribe.ListTranscriptionJobs" :: ByteString)
...
ltjrsStatus ::
     Lens' ListTranscriptionJobsResponse (Maybe TranscriptionJobStatus)
...
ltjrsTranscriptionJobSummaries ::
     Lens' ListTranscriptionJobsResponse [TranscriptionJobSummary]
ltjrsTranscriptionJobSummaries =
  lens
    _ltjrsTranscriptionJobSummaries
    (\s a -> s {_ltjrsTranscriptionJobSummaries = a}) .
  _Default . _Coerce
