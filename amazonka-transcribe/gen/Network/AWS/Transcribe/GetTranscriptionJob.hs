...
gtjTranscriptionJobName =
  lens _gtjTranscriptionJobName (\s a -> s {_gtjTranscriptionJobName = a})
...
instance AWSRequest GetTranscriptionJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetTranscriptionJobResponse' <$> (x .?> "TranscriptionJob") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetTranscriptionJob where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Transcribe.GetTranscriptionJob" :: ByteString)
...
instance ToJSON GetTranscriptionJob where
  toJSON GetTranscriptionJob' {..} =
    object
      (catMaybes [Just ("TranscriptionJobName" .= _gtjTranscriptionJobName)])
...
gtjrsTranscriptionJob ::
     Lens' GetTranscriptionJobResponse (Maybe TranscriptionJob)
gtjrsTranscriptionJob =
  lens _gtjrsTranscriptionJob (\s a -> s {_gtjrsTranscriptionJob = a})

-- | -- | The response status code.
gtjrsResponseStatus :: Lens' GetTranscriptionJobResponse Int
gtjrsResponseStatus =
  lens _gtjrsResponseStatus (\s a -> s {_gtjrsResponseStatus = a})
