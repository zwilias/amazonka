...
ucSecurityGroupIds =
  lens _ucSecurityGroupIds (\s a -> s {_ucSecurityGroupIds = a}) .
  _Default . _Coerce
...
ucPreferredMaintenanceWindow =
  lens
    ...
    (\s a -> s {_ucPreferredMaintenanceWindow = a})

-- | The current state of the topic.
ucNotificationTopicStatus :: Lens' UpdateCluster (Maybe Text)
ucNotificationTopicStatus =
  lens _ucNotificationTopicStatus (\s a -> s {_ucNotificationTopicStatus = a})
...
ucNotificationTopicARN =
  lens _ucNotificationTopicARN (\s a -> s {_ucNotificationTopicARN = a})

-- | The name of a parameter group for this cluster.
ucParameterGroupName :: Lens' UpdateCluster (Maybe Text)
ucParameterGroupName =
  lens _ucParameterGroupName (\s a -> s {_ucParameterGroupName = a})
...
instance AWSRequest UpdateCluster where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateClusterResponse' <$> (x .?> "Cluster") <*> (pure (fromEnum s)))
  ...
  toJSON UpdateCluster' {..} =
    object
      (catMaybes
         ...
         , ("PreferredMaintenanceWindow" .=) <$> _ucPreferredMaintenanceWindow
