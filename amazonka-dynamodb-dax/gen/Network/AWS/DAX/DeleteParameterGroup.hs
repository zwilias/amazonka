...
dpgParameterGroupName =
  lens _dpgParameterGroupName (\s a -> s {_dpgParameterGroupName = a})
...
instance AWSRequest DeleteParameterGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteParameterGroupResponse' <$> (x .?> "DeletionMessage") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DeleteParameterGroup where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDAXV3.DeleteParameterGroup" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON DeleteParameterGroup where
  toJSON DeleteParameterGroup' {..} =
    object (catMaybes [Just ("ParameterGroupName" .= _dpgParameterGroupName)])
...
dpgrsDeletionMessage =
  lens _dpgrsDeletionMessage (\s a -> s {_dpgrsDeletionMessage = a})

-- | -- | The response status code.
dpgrsResponseStatus :: Lens' DeleteParameterGroupResponse Int
dpgrsResponseStatus =
  lens _dpgrsResponseStatus (\s a -> s {_dpgrsResponseStatus = a})
