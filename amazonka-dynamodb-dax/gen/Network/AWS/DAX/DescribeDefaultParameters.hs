...
instance AWSRequest DescribeDefaultParameters where
  type Rs DescribeDefaultParameters = DescribeDefaultParametersResponse
  request = postJSON dax
  response =
    receiveJSON
      (\s h x ->
         DescribeDefaultParametersResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Parameters" .!@ mempty) <*>
         (pure (fromEnum s)))
...
ddprsParameters =
  lens _ddprsParameters (\s a -> s {_ddprsParameters = a}) . _Default . _Coerce
