...
dsgSubnetGroupNames =
  lens _dsgSubnetGroupNames (\s a -> s {_dsgSubnetGroupNames = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeSubnetGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeSubnetGroupsResponse' <$> (x .?> "SubnetGroups" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeSubnetGroups where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDAXV3.DescribeSubnetGroups" :: ByteString)
...
dsgsrsSubnetGroups =
  lens _dsgsrsSubnetGroups (\s a -> s {_dsgsrsSubnetGroups = a}) .
  _Default . _Coerce
...
dsgsrsResponseStatus =
  lens _dsgsrsResponseStatus (\s a -> s {_dsgsrsResponseStatus = a})
