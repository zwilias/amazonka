...
upgParameterGroupName =
  lens _upgParameterGroupName (\s a -> s {_upgParameterGroupName = a})

-- | An array of name-value pairs for the parameters in the group. Each element in the array represents a single parameter.
upgParameterNameValues :: Lens' UpdateParameterGroup [ParameterNameValue]
upgParameterNameValues =
  lens _upgParameterNameValues (\s a -> s {_upgParameterNameValues = a}) .
  _Coerce
...
instance AWSRequest UpdateParameterGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateParameterGroupResponse' <$> (x .?> "ParameterGroup") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateParameterGroup where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDAXV3.UpdateParameterGroup" :: ByteString)
...
upgrsParameterGroup =
  lens _upgrsParameterGroup (\s a -> s {_upgrsParameterGroup = a})

-- | -- | The response status code.
upgrsResponseStatus :: Lens' UpdateParameterGroupResponse Int
upgrsResponseStatus =
  lens _upgrsResponseStatus (\s a -> s {_upgrsResponseStatus = a})
