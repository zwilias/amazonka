...
irfAvailabilityZones =
  lens _irfAvailabilityZones (\s a -> s {_irfAvailabilityZones = a}) .
  _Default . _Coerce
...
irfNewReplicationFactor =
  lens _irfNewReplicationFactor (\s a -> s {_irfNewReplicationFactor = a})

instance AWSRequest IncreaseReplicationFactor where
  type Rs IncreaseReplicationFactor = IncreaseReplicationFactorResponse
  request = postJSON dax
  response =
    receiveJSON
      (\s h x ->
         IncreaseReplicationFactorResponse' <$> (x .?> "Cluster") <*>
         (pure (fromEnum s)))
...
irfrsResponseStatus =
  lens _irfrsResponseStatus (\s a -> s {_irfrsResponseStatus = a})
