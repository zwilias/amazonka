...
ccSecurityGroupIds =
  lens _ccSecurityGroupIds (\s a -> s {_ccSecurityGroupIds = a}) .
  _Default . _Coerce
...
ccAvailabilityZones =
  lens _ccAvailabilityZones (\s a -> s {_ccAvailabilityZones = a}) .
  _Default . _Coerce
...
ccNotificationTopicARN =
  lens _ccNotificationTopicARN (\s a -> s {_ccNotificationTopicARN = a})
...
ccParameterGroupName =
  lens _ccParameterGroupName (\s a -> s {_ccParameterGroupName = a})
...
ccReplicationFactor =
  lens _ccReplicationFactor (\s a -> s {_ccReplicationFactor = a})
...
instance AWSRequest CreateCluster where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateClusterResponse' <$> (x .?> "Cluster") <*> (pure (fromEnum s)))
