...
ssdeCustomerDefinedValues =
  lens _ssdeCustomerDefinedValues (\s a -> s {_ssdeCustomerDefinedValues = a}) .
  _Default . _Map

-- | (Optional) The desired S3 prefix for the published data set, similar to a directory path in standard file systems. For example, if given the bucket name "mybucket" and the prefix "myprefix/mydatasets", the output file "outputfile" would be published to "s3://mybucket/myprefix/mydatasets/outputfile". If the prefix directory structure does not exist, it will be created. If no prefix is provided, the data set will be published to the S3 bucket root.
ssdeDestinationS3Prefix :: Lens' StartSupportDataExport (Maybe Text)
ssdeDestinationS3Prefix =
  lens _ssdeDestinationS3Prefix (\s a -> s {_ssdeDestinationS3Prefix = a})
...
ssdeDestinationS3BucketName =
  lens
    ...
    (\s a -> s {_ssdeDestinationS3BucketName = a})
...
instance AWSRequest StartSupportDataExport where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartSupportDataExportResponse' <$> (x .?> "dataSetRequestId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders StartSupportDataExport where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MarketplaceCommerceAnalytics20150701.StartSupportDataExport" :: ByteString)
...
ssdersDataSetRequestId =
  lens _ssdersDataSetRequestId (\s a -> s {_ssdersDataSetRequestId = a})

-- | -- | The response status code.
ssdersResponseStatus :: Lens' StartSupportDataExportResponse Int
ssdersResponseStatus =
  lens _ssdersResponseStatus (\s a -> s {_ssdersResponseStatus = a})
