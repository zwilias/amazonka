...
gdsCustomerDefinedValues =
  lens _gdsCustomerDefinedValues (\s a -> s {_gdsCustomerDefinedValues = a}) .
  _Default . _Map

-- | (Optional) The desired S3 prefix for the published data set, similar to a directory path in standard file systems. For example, if given the bucket name "mybucket" and the prefix "myprefix/mydatasets", the output file "outputfile" would be published to "s3://mybucket/myprefix/mydatasets/outputfile". If the prefix directory structure does not exist, it will be created. If no prefix is provided, the data set will be published to the S3 bucket root.
gdsDestinationS3Prefix :: Lens' GenerateDataSet (Maybe Text)
gdsDestinationS3Prefix =
  lens _gdsDestinationS3Prefix (\s a -> s {_gdsDestinationS3Prefix = a})
...
gdsDataSetPublicationDate =
  lens _gdsDataSetPublicationDate (\s a -> s {_gdsDataSetPublicationDate = a}) .
  _Time
...
gdsDestinationS3BucketName =
  lens _gdsDestinationS3BucketName (\s a -> s {_gdsDestinationS3BucketName = a})
...
instance AWSRequest GenerateDataSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         GenerateDataSetResponse' <$> (x .?> "dataSetRequestId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GenerateDataSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MarketplaceCommerceAnalytics20150701.GenerateDataSet" :: ByteString)
...
gdsrsDataSetRequestId =
  lens _gdsrsDataSetRequestId (\s a -> s {_gdsrsDataSetRequestId = a})
