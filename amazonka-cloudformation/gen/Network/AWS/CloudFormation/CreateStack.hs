...
csEnableTerminationProtection =
  lens
    ...
    (\s a -> s {_csEnableTerminationProtection = a})
...
csParameters =
  lens _csParameters (\s a -> s {_csParameters = a}) . _Default . _Coerce
...
csClientRequestToken =
  lens _csClientRequestToken (\s a -> s {_csClientRequestToken = a})
...
csCapabilities =
  lens _csCapabilities (\s a -> s {_csCapabilities = a}) . _Default . _Coerce

-- | The rollback triggers for AWS CloudFormation to monitor during stack creation and updating operations, and for the specified monitoring period afterwards.
csRollbackConfiguration :: Lens' CreateStack (Maybe RollbackConfiguration)
csRollbackConfiguration =
  lens _csRollbackConfiguration (\s a -> s {_csRollbackConfiguration = a})
...
csResourceTypes =
  lens _csResourceTypes (\s a -> s {_csResourceTypes = a}) . _Default . _Coerce
...
csTimeoutInMinutes =
  lens _csTimeoutInMinutes (\s a -> s {_csTimeoutInMinutes = a}) . mapping _Nat
...
instance ToQuery CreateStack where
  toQuery CreateStack' {..} =
    mconcat
      ...
      , "Capabilities" =: toQuery (toQueryList "member" <$> _csCapabilities)
      , "RollbackConfiguration" =: _csRollbackConfiguration
      , "OnFailure" =: _csOnFailure
      , "ResourceTypes" =: toQuery (toQueryList "member" <$> _csResourceTypes)
