...
instance AWSRequest DescribeStackSetOperation where
  type Rs DescribeStackSetOperation = DescribeStackSetOperationResponse
  request = postQuery cloudFormation
  response =
    receiveXMLWrapper
      "DescribeStackSetOperationResult"
      (\s h x ->
         DescribeStackSetOperationResponse' <$> (x .@? "StackSetOperation") <*>
         (pure (fromEnum s)))
...
dssorsStackSetOperation ::
     Lens' DescribeStackSetOperationResponse (Maybe StackSetOperation)
dssorsStackSetOperation =
  lens _dssorsStackSetOperation (\s a -> s {_dssorsStackSetOperation = a})

-- | -- | The response status code.
dssorsResponseStatus :: Lens' DescribeStackSetOperationResponse Int
dssorsResponseStatus =
  lens _dssorsResponseStatus (\s a -> s {_dssorsResponseStatus = a})
