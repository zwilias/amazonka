...
etcParameters =
  lens _etcParameters (\s a -> s {_etcParameters = a}) . _Default . _Coerce
...
instance AWSRequest EstimateTemplateCost where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         EstimateTemplateCostResponse' <$> (x .@? "Url") <*> (pure (fromEnum s)))
...
etcrsResponseStatus =
  lens _etcrsResponseStatus (\s a -> s {_etcrsResponseStatus = a})
