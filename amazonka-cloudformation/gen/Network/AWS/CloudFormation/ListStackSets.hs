...
lssMaxResults =
  lens _lssMaxResults (\s a -> s {_lssMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListStackSets where
  ...
  response =
    receiveXMLWrapper
      "ListStackSetsResult"
      (\s h x ->
         ListStackSetsResponse' <$> (x .@? "NextToken") <*>
...
lssrsSummaries =
  lens _lssrsSummaries (\s a -> s {_lssrsSummaries = a}) . _Default . _Coerce

-- | -- | The response status code.
lssrsResponseStatus :: Lens' ListStackSetsResponse Int
lssrsResponseStatus =
  lens _lssrsResponseStatus (\s a -> s {_lssrsResponseStatus = a})
