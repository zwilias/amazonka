...
curResourcesToSkip =
  lens _curResourcesToSkip (\s a -> s {_curResourcesToSkip = a}) .
  _Default . _Coerce
...
curClientRequestToken =
  lens _curClientRequestToken (\s a -> s {_curClientRequestToken = a})
...
instance AWSRequest ContinueUpdateRollback where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x -> ContinueUpdateRollbackResponse' <$> (pure (fromEnum s)))
