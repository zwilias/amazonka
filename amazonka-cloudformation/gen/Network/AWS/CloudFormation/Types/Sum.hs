...
instance FromText AccountGateStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AccountGateStatus from value: '" <>
        e <> "'. Accepted values: failed, skipped, succeeded"
...
instance FromText Capability where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Capability from value: '" <>
        e <> "'. Accepted values: capability_iam, capability_named_iam"
...
instance FromText ChangeAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChangeAction from value: '" <>
        e <> "'. Accepted values: add, modify, remove"
...
instance FromText ChangeSetStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChangeSetStatus from value: '" <>
        e <>
...
instance FromText ChangeSetType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChangeSetType from value: '" <>
        e <> "'. Accepted values: create, update"
...
instance FromText ChangeSource where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChangeSource from value: '" <>
        e <>
...
instance FromText ChangeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChangeType from value: '" <>
        e <> "'. Accepted values: resource"
...
instance FromText EvaluationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EvaluationType from value: '" <>
...
instance FromText OnFailure where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OnFailure from value: '" <>
        e <> "'. Accepted values: delete, do_nothing, rollback"
...
instance FromText Replacement where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Replacement from value: '" <>
        e <> "'. Accepted values: conditional, false, true"
...
instance FromText RequiresRecreation where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RequiresRecreation from value: '" <>
        e <> "'. Accepted values: always, conditionally, never"
...
instance FromText ResourceAttribute where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ResourceAttribute from value: '" <>
        e <>
...
instance FromText ResourceSignalStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ResourceSignalStatus from value: '" <>
...
instance FromText ResourceStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ResourceStatus from value: '" <>
        e <>
...
instance FromText StackInstanceStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StackInstanceStatus from value: '" <>
        e <> "'. Accepted values: current, inoperable, outdated"
...
instance FromText StackSetOperationAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StackSetOperationAction from value: '" <>
        e <> "'. Accepted values: create, delete, update"
...
instance FromText StackSetOperationResultStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StackSetOperationResultStatus from value: '" <>
        e <>
...
instance FromText StackSetOperationStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StackSetOperationStatus from value: '" <>
        e <> "'. Accepted values: failed, running, stopped, stopping, succeeded"
...
instance FromText StackSetStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StackSetStatus from value: '" <>
        e <> "'. Accepted values: active, deleted"
...
instance FromText TemplateStage where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TemplateStage from value: '" <>
        e <> "'. Accepted values: original, processed"
