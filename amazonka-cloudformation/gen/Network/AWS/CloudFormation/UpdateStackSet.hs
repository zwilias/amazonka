...
ussAdministrationRoleARN =
  lens _ussAdministrationRoleARN (\s a -> s {_ussAdministrationRoleARN = a})
...
ussUsePreviousTemplate =
  lens _ussUsePreviousTemplate (\s a -> s {_ussUsePreviousTemplate = a})

-- | A list of input parameters for the stack set template.
ussParameters :: Lens' UpdateStackSet [Parameter]
ussParameters =
  lens _ussParameters (\s a -> s {_ussParameters = a}) . _Default . _Coerce

-- | Preferences for how AWS CloudFormation performs this stack set operation.
ussOperationPreferences ::
     Lens' UpdateStackSet (Maybe StackSetOperationPreferences)
ussOperationPreferences =
  lens _ussOperationPreferences (\s a -> s {_ussOperationPreferences = a})
...
ussCapabilities =
  lens _ussCapabilities (\s a -> s {_ussCapabilities = a}) . _Default . _Coerce
...
instance AWSRequest UpdateStackSet where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         UpdateStackSetResponse' <$> (x .@? "OperationId") <*>
         (pure (fromEnum s)))
...
instance ToQuery UpdateStackSet where
  toQuery UpdateStackSet' {..} =
    mconcat
      ...
      , "Capabilities" =: toQuery (toQueryList "member" <$> _ussCapabilities)
