...
csiOperationPreferences ::
     Lens' CreateStackInstances (Maybe StackSetOperationPreferences)
csiOperationPreferences =
  lens _csiOperationPreferences (\s a -> s {_csiOperationPreferences = a})
...
csiParameterOverrides =
  lens _csiParameterOverrides (\s a -> s {_csiParameterOverrides = a}) .
  _Default . _Coerce
...
instance AWSRequest CreateStackInstances where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateStackInstancesResponse' <$> (x .@? "OperationId") <*>
         (pure (fromEnum s)))
...
csirsResponseStatus =
  lens _csirsResponseStatus (\s a -> s {_csirsResponseStatus = a})
