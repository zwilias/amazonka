...
lssoMaxResults =
  lens _lssoMaxResults (\s a -> s {_lssoMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListStackSetOperations where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ListStackSetOperationsResponse' <$> (x .@? "NextToken") <*>
...
lssorsSummaries ::
     Lens' ListStackSetOperationsResponse [StackSetOperationSummary]
lssorsSummaries =
  lens _lssorsSummaries (\s a -> s {_lssorsSummaries = a}) . _Default . _Coerce

-- | -- | The response status code.
lssorsResponseStatus :: Lens' ListStackSetOperationsResponse Int
lssorsResponseStatus =
  lens _lssorsResponseStatus (\s a -> s {_lssorsResponseStatus = a})
