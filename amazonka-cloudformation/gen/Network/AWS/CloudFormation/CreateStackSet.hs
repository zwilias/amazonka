...
cssAdministrationRoleARN =
  lens _cssAdministrationRoleARN (\s a -> s {_cssAdministrationRoleARN = a})

-- | The input parameters for the stack set template.
cssParameters :: Lens' CreateStackSet [Parameter]
cssParameters =
  lens _cssParameters (\s a -> s {_cssParameters = a}) . _Default . _Coerce
...
cssClientRequestToken =
  lens _cssClientRequestToken (\s a -> s {_cssClientRequestToken = a})
...
cssCapabilities =
  lens _cssCapabilities (\s a -> s {_cssCapabilities = a}) . _Default . _Coerce
...
instance AWSRequest CreateStackSet where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateStackSetResponse' <$> (x .@? "StackSetId") <*>
         (pure (fromEnum s)))
...
instance ToQuery CreateStackSet where
  toQuery CreateStackSet' {..} =
    mconcat
      ...
      , "Capabilities" =: toQuery (toQueryList "member" <$> _cssCapabilities)
...
cssrsResponseStatus =
  lens _cssrsResponseStatus (\s a -> s {_cssrsResponseStatus = a})
