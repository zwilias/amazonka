...
dsiOperationPreferences ::
     Lens' DeleteStackInstances (Maybe StackSetOperationPreferences)
dsiOperationPreferences =
  lens _dsiOperationPreferences (\s a -> s {_dsiOperationPreferences = a})
...
instance AWSRequest DeleteStackInstances where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DeleteStackInstancesResponse' <$> (x .@? "OperationId") <*>
         (pure (fromEnum s)))
