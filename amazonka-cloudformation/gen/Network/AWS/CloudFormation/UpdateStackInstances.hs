...
usiOperationPreferences ::
     Lens' UpdateStackInstances (Maybe StackSetOperationPreferences)
usiOperationPreferences =
  lens _usiOperationPreferences (\s a -> s {_usiOperationPreferences = a})
...
usiParameterOverrides =
  lens _usiParameterOverrides (\s a -> s {_usiParameterOverrides = a}) .
  _Default . _Coerce
...
instance AWSRequest UpdateStackInstances where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         UpdateStackInstancesResponse' <$> (x .@? "OperationId") <*>
         (pure (fromEnum s)))
...
usirsResponseStatus =
  lens _usirsResponseStatus (\s a -> s {_usirsResponseStatus = a})
