...
instance AWSRequest GetTemplateSummary where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         GetTemplateSummaryResponse' <$>
         (x .@? "DeclaredTransforms" .!@ mempty >>= may (parseXMLList "member")) <*>
         (x .@? "Version") <*>
...
gtsrsDeclaredTransforms =
  lens _gtsrsDeclaredTransforms (\s a -> s {_gtsrsDeclaredTransforms = a}) .
  _Default . _Coerce
...
gtsrsCapabilitiesReason =
  lens _gtsrsCapabilitiesReason (\s a -> s {_gtsrsCapabilitiesReason = a})

-- | A list of parameter declarations that describe various properties for each parameter.
gtsrsParameters :: Lens' GetTemplateSummaryResponse [ParameterDeclaration]
gtsrsParameters =
  lens _gtsrsParameters (\s a -> s {_gtsrsParameters = a}) . _Default . _Coerce
...
gtsrsCapabilities =
  lens _gtsrsCapabilities (\s a -> s {_gtsrsCapabilities = a}) .
  _Default . _Coerce

-- | A list of all the template resource types that are defined in the template, such as @AWS::EC2::Instance@ , @AWS::Dynamo::Table@ , and @Custom::MyCustomInstance@ .
gtsrsResourceTypes :: Lens' GetTemplateSummaryResponse [Text]
gtsrsResourceTypes =
  lens _gtsrsResourceTypes (\s a -> s {_gtsrsResourceTypes = a}) .
  _Default . _Coerce
