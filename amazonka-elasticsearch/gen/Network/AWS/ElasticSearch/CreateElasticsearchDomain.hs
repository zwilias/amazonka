...
cedLogPublishingOptions ::
     Lens' CreateElasticsearchDomain (HashMap LogType LogPublishingOption)
cedLogPublishingOptions =
  lens _cedLogPublishingOptions (\s a -> s {_cedLogPublishingOptions = a}) .
  _Default . _Map

-- | Configuration options for an Elasticsearch domain. Specifies the instance type and number of instances in the domain cluster.
cedElasticsearchClusterConfig ::
     Lens' CreateElasticsearchDomain (Maybe ElasticsearchClusterConfig)
cedElasticsearchClusterConfig =
  lens
    _cedElasticsearchClusterConfig
    (\s a -> s {_cedElasticsearchClusterConfig = a})

-- | Option to set time, in UTC format, of the daily automated snapshot. Default value is 0 hours.
cedSnapshotOptions :: Lens' CreateElasticsearchDomain (Maybe SnapshotOptions)
cedSnapshotOptions =
  lens _cedSnapshotOptions (\s a -> s {_cedSnapshotOptions = a})
...
cedEncryptionAtRestOptions ::
     Lens' CreateElasticsearchDomain (Maybe EncryptionAtRestOptions)
cedEncryptionAtRestOptions =
  lens _cedEncryptionAtRestOptions (\s a -> s {_cedEncryptionAtRestOptions = a})
...
cedAdvancedOptions =
  lens _cedAdvancedOptions (\s a -> s {_cedAdvancedOptions = a}) .
  _Default . _Map
...
cedElasticsearchVersion =
  lens _cedElasticsearchVersion (\s a -> s {_cedElasticsearchVersion = a})
...
instance AWSRequest CreateElasticsearchDomain where
  type Rs CreateElasticsearchDomain = CreateElasticsearchDomainResponse
  request = postJSON elasticSearch
  response =
    receiveJSON
      (\s h x ->
         CreateElasticsearchDomainResponse' <$> (x .?> "DomainStatus") <*>
         (pure (fromEnum s)))
  ...
  toJSON CreateElasticsearchDomain' {..} =
    object
      (catMaybes
         ...
         , ("ElasticsearchClusterConfig" .=) <$> _cedElasticsearchClusterConfig
...
cedrsDomainStatus ::
     Lens' CreateElasticsearchDomainResponse (Maybe ElasticsearchDomainStatus)
...
cedrsResponseStatus =
  lens _cedrsResponseStatus (\s a -> s {_cedrsResponseStatus = a})
