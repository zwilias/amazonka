...
instance AWSRequest StartChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartChannelResponse' <$> (x .?> "state") <*> (x .?> "arn") <*>
...
scrsEgressEndpoints =
  lens _scrsEgressEndpoints (\s a -> s {_scrsEgressEndpoints = a}) .
  _Default . _Coerce

-- | Undocumented member.
scrsEncoderSettings :: Lens' StartChannelResponse (Maybe EncoderSettings)
scrsEncoderSettings =
  lens _scrsEncoderSettings (\s a -> s {_scrsEncoderSettings = a})
