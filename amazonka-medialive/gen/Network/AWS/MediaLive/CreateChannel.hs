...
ccInputSpecification =
  lens _ccInputSpecification (\s a -> s {_ccInputSpecification = a})

-- | List of input attachments for channel.
ccInputAttachments :: Lens' CreateChannel [InputAttachment]
ccInputAttachments =
  lens _ccInputAttachments (\s a -> s {_ccInputAttachments = a}) .
  _Default . _Coerce
...
ccDestinations =
  lens _ccDestinations (\s a -> s {_ccDestinations = a}) . _Default . _Coerce
...
instance AWSRequest CreateChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateChannelResponse' <$> (x .?> "channel") <*> (pure (fromEnum s)))
