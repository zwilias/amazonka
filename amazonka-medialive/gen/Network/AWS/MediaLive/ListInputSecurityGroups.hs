...
lisgMaxResults =
  lens _lisgMaxResults (\s a -> s {_lisgMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListInputSecurityGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListInputSecurityGroupsResponse' <$> (x .?> "nextToken") <*>
...
instance ToQuery ListInputSecurityGroups where
  toQuery ListInputSecurityGroups' {..} =
    mconcat ["nextToken" =: _lisgNextToken, "maxResults" =: _lisgMaxResults]
...
lisgrsInputSecurityGroups ::
     Lens' ListInputSecurityGroupsResponse [InputSecurityGroup]
lisgrsInputSecurityGroups =
  lens _lisgrsInputSecurityGroups (\s a -> s {_lisgrsInputSecurityGroups = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lisgrsResponseStatus :: Lens' ListInputSecurityGroupsResponse Int
lisgrsResponseStatus =
  lens _lisgrsResponseStatus (\s a -> s {_lisgrsResponseStatus = a})
