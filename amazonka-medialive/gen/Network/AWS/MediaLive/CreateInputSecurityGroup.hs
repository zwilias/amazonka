...
cisgWhitelistRules =
  lens _cisgWhitelistRules (\s a -> s {_cisgWhitelistRules = a}) .
  _Default . _Coerce
...
instance AWSRequest CreateInputSecurityGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateInputSecurityGroupResponse' <$> (x .?> "securityGroup") <*>
         (pure (fromEnum s)))
...
instance ToJSON CreateInputSecurityGroup where
  toJSON CreateInputSecurityGroup' {..} =
    object (catMaybes [("whitelistRules" .=) <$> _cisgWhitelistRules])
...
-- | Undocumented member.
cisgrsSecurityGroup ::
     Lens' CreateInputSecurityGroupResponse (Maybe InputSecurityGroup)
cisgrsSecurityGroup =
  lens _cisgrsSecurityGroup (\s a -> s {_cisgrsSecurityGroup = a})

-- | -- | The response status code.
cisgrsResponseStatus :: Lens' CreateInputSecurityGroupResponse Int
cisgrsResponseStatus =
  lens _cisgrsResponseStatus (\s a -> s {_cisgrsResponseStatus = a})
