...
ucInputSpecification =
  lens _ucInputSpecification (\s a -> s {_ucInputSpecification = a})

-- | Undocumented member.
ucInputAttachments :: Lens' UpdateChannel [InputAttachment]
ucInputAttachments =
  lens _ucInputAttachments (\s a -> s {_ucInputAttachments = a}) .
  _Default . _Coerce

-- | A list of output destinations for this channel.
ucDestinations :: Lens' UpdateChannel [OutputDestination]
ucDestinations =
  lens _ucDestinations (\s a -> s {_ucDestinations = a}) . _Default . _Coerce
...
instance AWSRequest UpdateChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateChannelResponse' <$> (x .?> "channel") <*> (pure (fromEnum s)))
...
instance ToPath UpdateChannel where
  toPath UpdateChannel' {..} = mconcat ["/prod/channels/", toBS _ucChannelId]
