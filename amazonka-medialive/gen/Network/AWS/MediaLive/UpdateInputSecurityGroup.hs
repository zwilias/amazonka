...
uisgWhitelistRules =
  lens _uisgWhitelistRules (\s a -> s {_uisgWhitelistRules = a}) .
  _Default . _Coerce

-- | The id of the Input Security Group to update.
uisgInputSecurityGroupId :: Lens' UpdateInputSecurityGroup Text
uisgInputSecurityGroupId =
  lens _uisgInputSecurityGroupId (\s a -> s {_uisgInputSecurityGroupId = a})
...
instance AWSRequest UpdateInputSecurityGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateInputSecurityGroupResponse' <$> (x .?> "securityGroup") <*>
         (pure (fromEnum s)))
...
instance ToJSON UpdateInputSecurityGroup where
  toJSON UpdateInputSecurityGroup' {..} =
    object (catMaybes [("whitelistRules" .=) <$> _uisgWhitelistRules])

instance ToPath UpdateInputSecurityGroup where
  toPath UpdateInputSecurityGroup' {..} =
    mconcat ["/prod/inputSecurityGroups/", toBS _uisgInputSecurityGroupId]
...
-- | Undocumented member.
uisgrsSecurityGroup ::
     Lens' UpdateInputSecurityGroupResponse (Maybe InputSecurityGroup)
uisgrsSecurityGroup =
  lens _uisgrsSecurityGroup (\s a -> s {_uisgrsSecurityGroup = a})

-- | -- | The response status code.
uisgrsResponseStatus :: Lens' UpdateInputSecurityGroupResponse Int
uisgrsResponseStatus =
  lens _uisgrsResponseStatus (\s a -> s {_uisgrsResponseStatus = a})
