...
instance ToPath DescribeInput where
  toPath DescribeInput' {..} = mconcat ["/prod/inputs/", toBS _dInputId]
...
diirsSecurityGroups =
  lens _diirsSecurityGroups (\s a -> s {_diirsSecurityGroups = a}) .
  _Default . _Coerce
...
diirsSources =
  lens _diirsSources (\s a -> s {_diirsSources = a}) . _Default . _Coerce

-- | A list of the destinations of the input (PUSH-type).
diirsDestinations :: Lens' DescribeInputResponse [InputDestination]
diirsDestinations =
  lens _diirsDestinations (\s a -> s {_diirsDestinations = a}) .
  _Default . _Coerce
...
diirsAttachedChannels =
  lens _diirsAttachedChannels (\s a -> s {_diirsAttachedChannels = a}) .
  _Default . _Coerce
