...
disgInputSecurityGroupId =
  lens _disgInputSecurityGroupId (\s a -> s {_disgInputSecurityGroupId = a})

instance AWSRequest DescribeInputSecurityGroup where
  type Rs DescribeInputSecurityGroup = DescribeInputSecurityGroupResponse
  request = get mediaLive
  response =
    receiveJSON
      (\s h x ->
         DescribeInputSecurityGroupResponse' <$> (x .?> "state") <*>
         (x .?> "arn") <*>
         (x .?> "inputs" .!@ mempty) <*>
         (x .?> "id") <*>
...
instance ToPath DescribeInputSecurityGroup where
  toPath DescribeInputSecurityGroup' {..} =
    mconcat ["/prod/inputSecurityGroups/", toBS _disgInputSecurityGroupId]
...
desrsState ::
     Lens' DescribeInputSecurityGroupResponse (Maybe InputSecurityGroupState)
...
desrsInputs =
  lens _desrsInputs (\s a -> s {_desrsInputs = a}) . _Default . _Coerce
...
desrsWhitelistRules ::
     Lens' DescribeInputSecurityGroupResponse [InputWhitelistRule]
desrsWhitelistRules =
  lens _desrsWhitelistRules (\s a -> s {_desrsWhitelistRules = a}) .
  _Default . _Coerce
