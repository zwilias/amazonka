...
dInputSecurityGroupId =
  lens _dInputSecurityGroupId (\s a -> s {_dInputSecurityGroupId = a})
...
instance AWSRequest DeleteInputSecurityGroup where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteInputSecurityGroupResponse' <$> (pure (fromEnum s)))
...
instance ToPath DeleteInputSecurityGroup where
  toPath DeleteInputSecurityGroup' {..} =
    mconcat ["/prod/inputSecurityGroups/", toBS _dInputSecurityGroupId]
...
disgrsResponseStatus =
  lens _disgrsResponseStatus (\s a -> s {_disgrsResponseStatus = a})
