...
geFilter ::
     Lens' GetEntitlements (HashMap GetEntitlementFilterName (NonEmpty Text))
...
instance AWSRequest GetEntitlements where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetEntitlementsResponse' <$> (x .?> "NextToken") <*>
...
gersEntitlements =
  lens _gersEntitlements (\s a -> s {_gersEntitlements = a}) .
  _Default . _Coerce
