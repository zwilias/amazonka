...
gmdMetricDataQueries =
  lens _gmdMetricDataQueries (\s a -> s {_gmdMetricDataQueries = a}) . _Coerce
...
instance AWSRequest GetMetricData where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         GetMetricDataResponse' <$>
         (x .@? "MetricDataResults" .!@ mempty >>= may (parseXMLList "member")) <*>
         (x .@? "NextToken") <*>
         (pure (fromEnum s)))
...
gmdrsMetricDataResults =
  lens _gmdrsMetricDataResults (\s a -> s {_gmdrsMetricDataResults = a}) .
  _Default . _Coerce
...
gmdrsResponseStatus =
  lens _gmdrsResponseStatus (\s a -> s {_gmdrsResponseStatus = a})
