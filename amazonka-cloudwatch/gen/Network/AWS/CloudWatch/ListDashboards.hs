...
ldDashboardNamePrefix =
  lens _ldDashboardNamePrefix (\s a -> s {_ldDashboardNamePrefix = a})
...
instance AWSRequest ListDashboards where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ListDashboardsResponse' <$>
         (x .@? "DashboardEntries" .!@ mempty >>= may (parseXMLList "member")) <*>
         (x .@? "NextToken") <*>
         (pure (fromEnum s)))
...
ldrsDashboardEntries =
  lens _ldrsDashboardEntries (\s a -> s {_ldrsDashboardEntries = a}) .
  _Default . _Coerce
