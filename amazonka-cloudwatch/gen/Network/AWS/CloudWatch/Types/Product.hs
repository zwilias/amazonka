...
ahiTimestamp =
  lens _ahiTimestamp (\s a -> s {_ahiTimestamp = a}) . mapping _Time

instance FromXML AlarmHistoryItem where
  parseXML x =
    AlarmHistoryItem' <$> (x .@? "AlarmName") <*> (x .@? "HistoryItemType") <*>
    (x .@? "HistoryData") <*>
    (x .@? "HistorySummary") <*>
    (x .@? "Timestamp")
...
deLastModified =
  lens _deLastModified (\s a -> s {_deLastModified = a}) . mapping _Time
...
instance FromXML DashboardEntry where
  parseXML x =
    DashboardEntry' <$> (x .@? "Size") <*> (x .@? "DashboardName") <*>
    (x .@? "LastModified") <*>
    (x .@? "DashboardArn")
...
instance FromXML DashboardValidationMessage where
  parseXML x =
    DashboardValidationMessage' <$> (x .@? "DataPath") <*> (x .@? "Message")
...
dExtendedStatistics =
  lens _dExtendedStatistics (\s a -> s {_dExtendedStatistics = a}) .
  _Default . _Map
...
instance FromXML Datapoint where
  parseXML x =
    Datapoint' <$> (x .@? "SampleCount") <*> (x .@? "Maximum") <*>
    (x .@? "Average") <*>
    (x .@? "Minimum") <*>
...
instance ToQuery Dimension where
  toQuery Dimension' {..} = mconcat ["Name" =: _dName, "Value" =: _dValue]
...
mDimensions =
  lens _mDimensions (\s a -> s {_mDimensions = a}) . _Default . _Coerce

instance FromXML Metric where
  parseXML x =
    Metric' <$> (x .@? "MetricName") <*> (x .@? "Namespace") <*>
...
maTreatMissingData =
  lens _maTreatMissingData (\s a -> s {_maTreatMissingData = a})
...
maOKActions =
  lens _maOKActions (\s a -> s {_maOKActions = a}) . _Default . _Coerce
...
maEvaluateLowSampleCountPercentile =
  lens
    ...
    (\s a -> s {_maEvaluateLowSampleCountPercentile = a})
...
maDatapointsToAlarm =
  lens _maDatapointsToAlarm (\s a -> s {_maDatapointsToAlarm = a}) .
  mapping _Nat
...
maInsufficientDataActions =
  lens _maInsufficientDataActions (\s a -> s {_maInsufficientDataActions = a}) .
  _Default . _Coerce
...
maDimensions =
  lens _maDimensions (\s a -> s {_maDimensions = a}) . _Default . _Coerce
...
maAlarmActions =
  lens _maAlarmActions (\s a -> s {_maAlarmActions = a}) . _Default . _Coerce
...
maExtendedStatistic =
  lens _maExtendedStatistic (\s a -> s {_maExtendedStatistic = a})

instance FromXML MetricAlarm where
  parseXML x =
    MetricAlarm' <$> (x .@? "AlarmName") <*> (x .@? "StateUpdatedTimestamp") <*>
    (x .@? "TreatMissingData") <*>
    (x .@? "Period") <*>
    (x .@? "AlarmDescription") <*>
    (x .@? "EvaluationPeriods") <*>
    ...
    (x .@? "ActionsEnabled") <*>
    (x .@? "InsufficientDataActions" .!@ mempty >>= may (parseXMLList "member")) <*>
    (x .@? "StateReason") <*>
    (x .@? "StateReasonData") <*>
    (x .@? "Dimensions" .!@ mempty >>= may (parseXMLList "member")) <*>
    (x .@? "AlarmArn") <*>
...
mdrTimestamps =
  lens _mdrTimestamps (\s a -> s {_mdrTimestamps = a}) . _Default . _Coerce

-- | A list of messages with additional information about the data returned.
mdrMessages :: Lens' MetricDataResult [MessageData]
mdrMessages =
  lens _mdrMessages (\s a -> s {_mdrMessages = a}) . _Default . _Coerce
...
mdStorageResolution =
  lens _mdStorageResolution (\s a -> s {_mdStorageResolution = a}) .
  mapping _Nat

-- | The dimensions associated with the metric.
mdDimensions :: Lens' MetricDatum [Dimension]
mdDimensions =
  lens _mdDimensions (\s a -> s {_mdDimensions = a}) . _Default . _Coerce
