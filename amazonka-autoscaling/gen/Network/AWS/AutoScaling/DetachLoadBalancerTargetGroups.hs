...
dlbtgAutoScalingGroupName =
  lens _dlbtgAutoScalingGroupName (\s a -> s {_dlbtgAutoScalingGroupName = a})

-- | The Amazon Resource Names (ARN) of the target groups. You can specify up to 10 target groups.
dlbtgTargetGroupARNs :: Lens' DetachLoadBalancerTargetGroups [Text]
dlbtgTargetGroupARNs =
  lens _dlbtgTargetGroupARNs (\s a -> s {_dlbtgTargetGroupARNs = a}) . _Coerce

instance AWSRequest DetachLoadBalancerTargetGroups where
  type Rs DetachLoadBalancerTargetGroups = DetachLoadBalancerTargetGroupsResponse
  request = postQuery autoScaling
  response =
    receiveXMLWrapper
      "DetachLoadBalancerTargetGroupsResult"
      (\s h x -> DetachLoadBalancerTargetGroupsResponse' <$> (pure (fromEnum s)))
...
dlbtgrsResponseStatus =
  lens _dlbtgrsResponseStatus (\s a -> s {_dlbtgrsResponseStatus = a})
