...
dasgAutoScalingGroupNames =
  lens _dasgAutoScalingGroupNames (\s a -> s {_dasgAutoScalingGroupNames = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeAutoScalingGroups where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeAutoScalingGroupsResponse' <$> (x .@? "NextToken") <*>
         (pure (fromEnum s)) <*>
...
dasgrsAutoScalingGroups =
  lens _dasgrsAutoScalingGroups (\s a -> s {_dasgrsAutoScalingGroups = a}) .
  _Coerce
