...
desAutoScalingGroupName =
  lens _desAutoScalingGroupName (\s a -> s {_desAutoScalingGroupName = a})
...
desActivityIds =
  lens _desActivityIds (\s a -> s {_desActivityIds = a}) . _Default . _Coerce
...
instance AWSRequest DescribeScalingActivities where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeScalingActivitiesResponse' <$> (x .@? "NextToken") <*>
         (pure (fromEnum s)) <*>
...
instance ToQuery DescribeScalingActivities where
  toQuery DescribeScalingActivities' {..} =
    mconcat
      ...
      , "ActivityIds" =: toQuery (toQueryList "member" <$> _desActivityIds)
      ]
...
dsasrsActivities =
  lens _dsasrsActivities (\s a -> s {_dsasrsActivities = a}) . _Coerce
