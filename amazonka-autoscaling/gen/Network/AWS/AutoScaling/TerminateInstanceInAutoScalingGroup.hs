...
tiiasgShouldDecrementDesiredCapacity ::
     Lens' TerminateInstanceInAutoScalingGroup Bool
tiiasgShouldDecrementDesiredCapacity =
  lens
...
instance AWSRequest TerminateInstanceInAutoScalingGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         TerminateInstanceInAutoScalingGroupResponse' <$> (x .@? "Activity") <*>
         (pure (fromEnum s)))
...
-- | A scaling activity.
tiiasgrsActivity ::
     Lens' TerminateInstanceInAutoScalingGroupResponse (Maybe Activity)
...
tiiasgrsResponseStatus =
  lens _tiiasgrsResponseStatus (\s a -> s {_tiiasgrsResponseStatus = a})
