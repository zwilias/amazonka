...
dncAutoScalingGroupNames =
  lens _dncAutoScalingGroupNames (\s a -> s {_dncAutoScalingGroupNames = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeNotificationConfigurations where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeNotificationConfigurationsResponse' <$> (x .@? "NextToken") <*>
         (pure (fromEnum s)) <*>
...
dncrsResponseStatus =
  lens _dncrsResponseStatus (\s a -> s {_dncrsResponseStatus = a})
