...
dpsPolicyNames =
  lens _dpsPolicyNames (\s a -> s {_dpsPolicyNames = a}) . _Default . _Coerce
...
dpsAutoScalingGroupName =
  lens _dpsAutoScalingGroupName (\s a -> s {_dpsAutoScalingGroupName = a})
...
dpsPolicyTypes =
  lens _dpsPolicyTypes (\s a -> s {_dpsPolicyTypes = a}) . _Default . _Coerce
...
instance AWSRequest DescribePolicies where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribePoliciesResponse' <$> (x .@? "NextToken") <*>
         (x .@? "ScalingPolicies" .!@ mempty >>= may (parseXMLList "member")) <*>
         (pure (fromEnum s)))
...
instance ToQuery DescribePolicies where
  toQuery DescribePolicies' {..} =
    mconcat
      ...
      , "PolicyNames" =: toQuery (toQueryList "member" <$> _dpsPolicyNames)
      , "NextToken" =: _dpsNextToken
      , "AutoScalingGroupName" =: _dpsAutoScalingGroupName
      , "MaxRecords" =: _dpsMaxRecords
      , "PolicyTypes" =: toQuery (toQueryList "member" <$> _dpsPolicyTypes)
      ]
