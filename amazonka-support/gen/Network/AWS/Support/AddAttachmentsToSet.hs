...
aatsAttachments =
  lens _aatsAttachments (\s a -> s {_aatsAttachments = a}) . _Coerce
...
instance AWSRequest AddAttachmentsToSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         AddAttachmentsToSetResponse' <$> (x .?> "expiryTime") <*>
         (x .?> "attachmentSetId") <*>
         (pure (fromEnum s)))
...
aatsrsResponseStatus =
  lens _aatsrsResponseStatus (\s a -> s {_aatsrsResponseStatus = a})
