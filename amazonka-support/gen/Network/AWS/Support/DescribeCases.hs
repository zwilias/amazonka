...
dcCaseIdList =
  lens _dcCaseIdList (\s a -> s {_dcCaseIdList = a}) . _Default . _Coerce
...
instance AWSRequest DescribeCases where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeCasesResponse' <$> (x .?> "cases" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeCases where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSSupport_20130415.DescribeCases" :: ByteString)
