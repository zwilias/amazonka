...
instance AWSRequest ListOrganizations where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListOrganizationsResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders ListOrganizations where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("WorkMailService.ListOrganizations" :: ByteString)
...
lorsOrganizationSummaries ::
     Lens' ListOrganizationsResponse [OrganizationSummary]
lorsOrganizationSummaries =
  lens _lorsOrganizationSummaries (\s a -> s {_lorsOrganizationSummaries = a}) .
  _Default . _Coerce
