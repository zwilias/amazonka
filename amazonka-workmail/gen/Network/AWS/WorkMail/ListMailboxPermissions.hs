...
lmpMaxResults =
  lens _lmpMaxResults (\s a -> s {_lmpMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListMailboxPermissions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListMailboxPermissionsResponse' <$> (x .?> "NextToken") <*>
...
lmprsPermissions =
  lens _lmprsPermissions (\s a -> s {_lmprsPermissions = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lmprsResponseStatus :: Lens' ListMailboxPermissionsResponse Int
lmprsResponseStatus =
  lens _lmprsResponseStatus (\s a -> s {_lmprsResponseStatus = a})
