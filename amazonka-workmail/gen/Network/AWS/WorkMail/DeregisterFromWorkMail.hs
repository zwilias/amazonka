...
dfwmOrganizationId =
  lens _dfwmOrganizationId (\s a -> s {_dfwmOrganizationId = a})
...
instance AWSRequest DeregisterFromWorkMail where
  ...
  response =
    receiveEmpty
      (\s h x -> DeregisterFromWorkMailResponse' <$> (pure (fromEnum s)))
...
dfwmrsResponseStatus =
  lens _dfwmrsResponseStatus (\s a -> s {_dfwmrsResponseStatus = a})
