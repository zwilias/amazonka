...
amtgOrganizationId =
  lens _amtgOrganizationId (\s a -> s {_amtgOrganizationId = a})
...
instance AWSRequest AssociateMemberToGroup where
  ...
  response =
    receiveEmpty
      (\s h x -> AssociateMemberToGroupResponse' <$> (pure (fromEnum s)))
...
amtgrsResponseStatus =
  lens _amtgrsResponseStatus (\s a -> s {_amtgrsResponseStatus = a})
