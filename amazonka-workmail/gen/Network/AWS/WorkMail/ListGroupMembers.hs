...
lgmMaxResults =
  lens _lgmMaxResults (\s a -> s {_lgmMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListGroupMembers where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListGroupMembersResponse' <$> (x .?> "Members" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListGroupMembers where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("WorkMailService.ListGroupMembers" :: ByteString)
...
lgmrsMembers =
  lens _lgmrsMembers (\s a -> s {_lgmrsMembers = a}) . _Default . _Coerce
...
lgmrsResponseStatus =
  lens _lgmrsResponseStatus (\s a -> s {_lgmrsResponseStatus = a})
