...
rtwmOrganizationId =
  lens _rtwmOrganizationId (\s a -> s {_rtwmOrganizationId = a})
...
instance AWSRequest RegisterToWorkMail where
  ...
  response =
    receiveEmpty (\s h x -> RegisterToWorkMailResponse' <$> (pure (fromEnum s)))
...
rtwmrsResponseStatus =
  lens _rtwmrsResponseStatus (\s a -> s {_rtwmrsResponseStatus = a})
