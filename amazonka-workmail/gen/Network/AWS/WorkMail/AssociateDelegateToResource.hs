...
adtrOrganizationId =
  lens _adtrOrganizationId (\s a -> s {_adtrOrganizationId = a})
...
instance AWSRequest AssociateDelegateToResource where
  type Rs AssociateDelegateToResource = AssociateDelegateToResourceResponse
  request = postJSON workMail
  response =
    receiveEmpty
      (\s h x -> AssociateDelegateToResourceResponse' <$> (pure (fromEnum s)))
...
adtrrsResponseStatus =
  lens _adtrrsResponseStatus (\s a -> s {_adtrrsResponseStatus = a})
