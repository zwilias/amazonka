...
instance AWSRequest DescribeResource where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeResourceResponse' <$> (x .?> "Email") <*> (x .?> "State") <*>
         (x .?> "ResourceId") <*>
         (x .?> "DisabledDate") <*>
         (x .?> "Name") <*>
         (x .?> "Type") <*>
...
instance ToHeaders DescribeResource where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("WorkMailService.DescribeResource" :: ByteString)
...
drrsDisabledDate =
  lens _drrsDisabledDate (\s a -> s {_drrsDisabledDate = a}) . mapping _Time
...
drrsEnabledDate =
  lens _drrsEnabledDate (\s a -> s {_drrsEnabledDate = a}) . mapping _Time

-- | The booking options for the described resource.
drrsBookingOptions :: Lens' DescribeResourceResponse (Maybe BookingOptions)
drrsBookingOptions =
  lens _drrsBookingOptions (\s a -> s {_drrsBookingOptions = a})
