...
instance AWSRequest UpdateBroker where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateBrokerResponse' <$> (x .?> "configuration") <*>
         (x .?> "brokerId") <*>
         (pure (fromEnum s)))
...
instance ToJSON UpdateBroker where
  toJSON UpdateBroker' {..} =
    object (catMaybes [("configuration" .=) <$> _ubConfiguration])

instance ToPath UpdateBroker where
  toPath UpdateBroker' {..} = mconcat ["/v1/brokers/", toBS _ubBrokerId]
...
ubrsResponseStatus =
  lens _ubrsResponseStatus (\s a -> s {_ubrsResponseStatus = a})
