...
lcrMaxResults =
  lens _lcrMaxResults (\s a -> s {_lcrMaxResults = a}) . mapping _Nat

-- | The unique ID that Amazon MQ generates for the configuration.
lcrConfigurationId :: Lens' ListConfigurationRevisions Text
lcrConfigurationId =
  lens _lcrConfigurationId (\s a -> s {_lcrConfigurationId = a})

instance AWSRequest ListConfigurationRevisions where
  type Rs ListConfigurationRevisions = ListConfigurationRevisionsResponse
  request = get mq
  response =
    receiveJSON
      (\s h x ->
         ListConfigurationRevisionsResponse' <$> (x .?> "configurationId") <*>
         (x .?> "nextToken") <*>
         (x .?> "revisions" .!@ mempty) <*>
         (x .?> "maxResults") <*>
         (pure (fromEnum s)))
...
instance ToPath ListConfigurationRevisions where
  toPath ListConfigurationRevisions' {..} =
    mconcat ["/v1/configurations/", toBS _lcrConfigurationId, "/revisions"]

instance ToQuery ListConfigurationRevisions where
  toQuery ListConfigurationRevisions' {..} =
    mconcat ["nextToken" =: _lcrNextToken, "maxResults" =: _lcrMaxResults]
...
lcrrsConfigurationId =
  lens _lcrrsConfigurationId (\s a -> s {_lcrrsConfigurationId = a})
...
lcrrsRevisions ::
     Lens' ListConfigurationRevisionsResponse [ConfigurationRevision]
lcrrsRevisions =
  lens _lcrrsRevisions (\s a -> s {_lcrrsRevisions = a}) . _Default . _Coerce
...
lcrrsResponseStatus =
  lens _lcrrsResponseStatus (\s a -> s {_lcrrsResponseStatus = a})
