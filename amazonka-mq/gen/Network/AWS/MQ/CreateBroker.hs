...
cbPubliclyAccessible =
  lens _cbPubliclyAccessible (\s a -> s {_cbPubliclyAccessible = a})

-- | Required. Enables automatic upgrades to new minor versions for brokers, as Apache releases the versions. The automatic upgrades occur during the maintenance window of the broker or after a manual broker reboot.
cbAutoMinorVersionUpgrade :: Lens' CreateBroker (Maybe Bool)
cbAutoMinorVersionUpgrade =
  lens _cbAutoMinorVersionUpgrade (\s a -> s {_cbAutoMinorVersionUpgrade = a})
...
cbSecurityGroups =
  lens _cbSecurityGroups (\s a -> s {_cbSecurityGroups = a}) .
  _Default . _Coerce
...
cbSubnetIds =
  lens _cbSubnetIds (\s a -> s {_cbSubnetIds = a}) . _Default . _Coerce

-- | The unique ID that the requester receives for the created broker. Amazon MQ passes your ID with the API action. Note: We recommend using a Universally Unique Identifier (UUID) for the creatorRequestId. You may omit the creatorRequestId if your application doesn't require idempotency.
cbCreatorRequestId :: Lens' CreateBroker (Maybe Text)
cbCreatorRequestId =
  lens _cbCreatorRequestId (\s a -> s {_cbCreatorRequestId = a})
...
cbMaintenanceWindowStartTime =
  lens
    ...
    (\s a -> s {_cbMaintenanceWindowStartTime = a})
...
cbHostInstanceType =
  lens _cbHostInstanceType (\s a -> s {_cbHostInstanceType = a})
...
instance AWSRequest CreateBroker where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateBrokerResponse' <$> (x .?> "brokerId") <*> (x .?> "brokerArn") <*>
         (pure (fromEnum s)))
  ...
  toJSON CreateBroker' {..} =
    object
      (catMaybes
         ...
         , ("maintenanceWindowStartTime" .=) <$> _cbMaintenanceWindowStartTime
