...
instance AWSRequest DescribeBroker where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeBrokerResponse' <$> (x .?> "brokerName") <*>
         (x .?> "engineVersion") <*>
         (x .?> "brokerState") <*>
...
instance ToPath DescribeBroker where
  toPath DescribeBroker' {..} = mconcat ["/v1/brokers/", toBS _desBrokerId]
...
dbrsPubliclyAccessible =
  lens _dbrsPubliclyAccessible (\s a -> s {_dbrsPubliclyAccessible = a})
...
dbrsAutoMinorVersionUpgrade =
  lens
    ...
    (\s a -> s {_dbrsAutoMinorVersionUpgrade = a})
...
dbrsSecurityGroups =
  lens _dbrsSecurityGroups (\s a -> s {_dbrsSecurityGroups = a}) .
  _Default . _Coerce
...
dbrsSubnetIds =
  lens _dbrsSubnetIds (\s a -> s {_dbrsSubnetIds = a}) . _Default . _Coerce

-- | The list of all revisions for the specified configuration.
dbrsConfigurations :: Lens' DescribeBrokerResponse (Maybe Configurations)
dbrsConfigurations =
  lens _dbrsConfigurations (\s a -> s {_dbrsConfigurations = a})

-- | The parameters that determine the WeeklyStartTime.
dbrsMaintenanceWindowStartTime ::
     Lens' DescribeBrokerResponse (Maybe WeeklyStartTime)
dbrsMaintenanceWindowStartTime =
  lens
    _dbrsMaintenanceWindowStartTime
    (\s a -> s {_dbrsMaintenanceWindowStartTime = a})
...
dbrsDeploymentMode =
  lens _dbrsDeploymentMode (\s a -> s {_dbrsDeploymentMode = a})
...
dbrsBrokerInstances =
  lens _dbrsBrokerInstances (\s a -> s {_dbrsBrokerInstances = a}) .
  _Default . _Coerce
...
dbrsHostInstanceType =
  lens _dbrsHostInstanceType (\s a -> s {_dbrsHostInstanceType = a})
