...
instance FromText BrokerState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BrokerState from value: '" <>
        e <>
...
instance FromText DayOfWeek where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DayOfWeek from value: '" <>
        e <>
...
instance FromText DeploymentMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeploymentMode from value: '" <>
        e <> "'. Accepted values: active_standby_multi_az, single_instance"
...
instance FromText EngineType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EngineType from value: '" <>
        e <> "'. Accepted values: activemq"
...
instance FromText SanitizationWarningReason where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SanitizationWarningReason from value: '" <>
        e <>
