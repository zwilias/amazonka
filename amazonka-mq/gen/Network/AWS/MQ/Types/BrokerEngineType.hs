...
data BrokerEngineType = BrokerEngineType'{_betEngineVersions
                                          :: !(Maybe [EngineVersion]),
...
brokerEngineType
    :: BrokerEngineType
brokerEngineType
  = BrokerEngineType'{_betEngineVersions = Nothing,
...
instance FromJSON BrokerEngineType where
        parseJSON
          = withObject "BrokerEngineType"
              (\ x ->
                 BrokerEngineType' <$>
                   (x .:? "engineVersions" .!= mempty) <*>
                     (x .:? "engineType"))

instance Hashable BrokerEngineType where

instance NFData BrokerEngineType where
