...
fleLogStreamNames =
  lens _fleLogStreamNames (\s a -> s {_fleLogStreamNames = a}) . mapping _List1
...
instance AWSRequest FilterLogEvents where
  ...
  response =
    receiveJSON
      (\s h x ->
         FilterLogEventsResponse' <$> (x .?> "searchedLogStreams" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (x .?> "events" .!@ mempty) <*>
         (pure (fromEnum s)))
...
flersSearchedLogStreams =
  lens _flersSearchedLogStreams (\s a -> s {_flersSearchedLogStreams = a}) .
  _Default . _Coerce
...
flersEvents =
  lens _flersEvents (\s a -> s {_flersEvents = a}) . _Default . _Coerce

-- | -- | The response status code.
flersResponseStatus :: Lens' FilterLogEventsResponse Int
flersResponseStatus =
  lens _flersResponseStatus (\s a -> s {_flersResponseStatus = a})
