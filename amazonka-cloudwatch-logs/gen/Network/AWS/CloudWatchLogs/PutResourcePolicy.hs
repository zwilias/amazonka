...
instance AWSRequest PutResourcePolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutResourcePolicyResponse' <$> (x .?> "resourcePolicy") <*>
         (pure (fromEnum s)))
...
instance ToHeaders PutResourcePolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Logs_20140328.PutResourcePolicy" :: ByteString)
...
prprsResourcePolicy =
  lens _prprsResourcePolicy (\s a -> s {_prprsResourcePolicy = a})

-- | -- | The response status code.
prprsResponseStatus :: Lens' PutResourcePolicyResponse Int
prprsResponseStatus =
  lens _prprsResponseStatus (\s a -> s {_prprsResponseStatus = a})
