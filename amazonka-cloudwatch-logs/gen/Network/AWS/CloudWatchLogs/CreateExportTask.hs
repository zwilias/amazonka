...
cetDestinationPrefix =
  lens _cetDestinationPrefix (\s a -> s {_cetDestinationPrefix = a})
...
cetLogStreamNamePrefix =
  lens _cetLogStreamNamePrefix (\s a -> s {_cetLogStreamNamePrefix = a})
...
instance AWSRequest CreateExportTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateExportTaskResponse' <$> (x .?> "taskId") <*> (pure (fromEnum s)))
...
instance ToHeaders CreateExportTask where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Logs_20140328.CreateExportTask" :: ByteString)
...
cetrsResponseStatus =
  lens _cetrsResponseStatus (\s a -> s {_cetrsResponseStatus = a})
