...
instance AWSRequest DescribeLogGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeLogGroupsResponse' <$> (x .?> "logGroups" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeLogGroups where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Logs_20140328.DescribeLogGroups" :: ByteString)
...
dlgrsLogGroups =
  lens _dlgrsLogGroups (\s a -> s {_dlgrsLogGroups = a}) . _Default . _Coerce
...
dlgrsResponseStatus =
  lens _dlgrsResponseStatus (\s a -> s {_dlgrsResponseStatus = a})
