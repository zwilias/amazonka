...
instance FromText Distribution where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Distribution from value: '" <>
        e <> "'. Accepted values: bylogstream, random"
...
instance FromText ExportTaskStatusCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExportTaskStatusCode from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: lasteventtime, logstreamname"
