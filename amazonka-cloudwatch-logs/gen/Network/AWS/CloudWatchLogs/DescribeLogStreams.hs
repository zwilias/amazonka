...
dlssLogStreamNamePrefix =
  lens _dlssLogStreamNamePrefix (\s a -> s {_dlssLogStreamNamePrefix = a})
...
instance AWSRequest DescribeLogStreams where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeLogStreamsResponse' <$> (x .?> "nextToken") <*>
         (x .?> "logStreams" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeLogStreams where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("Logs_20140328.DescribeLogStreams" :: ByteString)
...
dlsrsLogStreams =
  lens _dlsrsLogStreams (\s a -> s {_dlsrsLogStreams = a}) . _Default . _Coerce

-- | -- | The response status code.
dlsrsResponseStatus :: Lens' DescribeLogStreamsResponse Int
dlsrsResponseStatus =
  lens _dlsrsResponseStatus (\s a -> s {_dlsrsResponseStatus = a})
