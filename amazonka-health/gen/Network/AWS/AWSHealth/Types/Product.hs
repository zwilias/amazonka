...
aeLastUpdatedTime =
  lens _aeLastUpdatedTime (\s a -> s {_aeLastUpdatedTime = a}) . mapping _Time
...
instance FromJSON AffectedEntity where
  parseJSON =
    withObject
      "AffectedEntity"
      (\x ->
         AffectedEntity' <$> (x .:? "lastUpdatedTime") <*> (x .:? "entityValue") <*>
         (x .:? "awsAccountId") <*>
         (x .:? "eventArn") <*>
         (x .:? "entityArn") <*>
         (x .:? "tags" .!= mempty) <*>
...
instance ToJSON DateTimeRange where
  toJSON DateTimeRange' {..} =
    object (catMaybes [("to" .=) <$> _dtrTo, ("from" .=) <$> _dtrFrom])
...
instance FromJSON EntityAggregate where
  parseJSON =
    withObject
      "EntityAggregate"
      (\x -> EntityAggregate' <$> (x .:? "count") <*> (x .:? "eventArn"))
...
eStatusCodes =
  lens _eStatusCodes (\s a -> s {_eStatusCodes = a}) . mapping _List1
...
eEntityValues =
  lens _eEntityValues (\s a -> s {_eEntityValues = a}) . mapping _List1
...
eLastUpdatedTimes =
  lens _eLastUpdatedTimes (\s a -> s {_eLastUpdatedTimes = a}) . mapping _List1
...
eLastUpdatedTime =
  lens _eLastUpdatedTime (\s a -> s {_eLastUpdatedTime = a}) . mapping _Time
...
eEventTypeCategory =
  lens _eEventTypeCategory (\s a -> s {_eEventTypeCategory = a})
...
instance FromJSON EventAggregate where
  parseJSON =
    withObject
      "EventAggregate"
      (\x -> EventAggregate' <$> (x .:? "count") <*> (x .:? "aggregateValue"))
...
edLatestDescription =
  lens _edLatestDescription (\s a -> s {_edLatestDescription = a})
...
edEventDescription =
  lens _edEventDescription (\s a -> s {_edEventDescription = a})

-- | Additional metadata about the event.
edEventMetadata :: Lens' EventDetails (HashMap Text Text)
edEventMetadata =
  lens _edEventMetadata (\s a -> s {_edEventMetadata = a}) . _Default . _Map
...
instance FromJSON EventDetails where
  parseJSON =
    withObject
      "EventDetails"
      (\x ->
         EventDetails' <$> (x .:? "event") <*> (x .:? "eventDescription") <*>
...
instance FromJSON EventDetailsErrorItem where
  parseJSON =
    withObject
      "EventDetailsErrorItem"
      (\x ->
         EventDetailsErrorItem' <$> (x .:? "eventArn") <*> (x .:? "errorName") <*>
         (x .:? "errorMessage"))
...
efEventTypeCategories =
  lens _efEventTypeCategories (\s a -> s {_efEventTypeCategories = a}) .
  mapping _List1

-- | A list of unique identifiers for event types. For example, @"AWS_EC2_SYSTEM_MAINTENANCE_EVENT","AWS_RDS_MAINTENANCE_SCHEDULED"@
efEventTypeCodes :: Lens' EventFilter (Maybe (NonEmpty Text))
efEventTypeCodes =
  lens _efEventTypeCodes (\s a -> s {_efEventTypeCodes = a}) . mapping _List1
...
efEventStatusCodes =
  lens _efEventStatusCodes (\s a -> s {_efEventStatusCodes = a}) .
  mapping _List1
...
efAvailabilityZones =
  lens _efAvailabilityZones (\s a -> s {_efAvailabilityZones = a}) .
  _Default . _Coerce

-- | A list of entity ARNs (unique identifiers).
efEntityARNs :: Lens' EventFilter (Maybe (NonEmpty Text))
efEntityARNs =
  lens _efEntityARNs (\s a -> s {_efEntityARNs = a}) . mapping _List1

-- | A list of entity identifiers, such as EC2 instance IDs (@i-34ab692e@ ) or EBS volumes (@vol-426ab23e@ ).
efEntityValues :: Lens' EventFilter (Maybe (NonEmpty Text))
efEntityValues =
  lens _efEntityValues (\s a -> s {_efEntityValues = a}) . mapping _List1

-- | A list of dates and times that the event began.
efStartTimes :: Lens' EventFilter (Maybe (NonEmpty DateTimeRange))
efStartTimes =
  lens _efStartTimes (\s a -> s {_efStartTimes = a}) . mapping _List1
...
efLastUpdatedTimes =
  lens _efLastUpdatedTimes (\s a -> s {_efLastUpdatedTimes = a}) .
  mapping _List1
...
instance FromJSON EventType where
  parseJSON =
    withObject
      "EventType"
      (\x ->
         EventType' <$> (x .:? "service") <*> (x .:? "category") <*>
         (x .:? "code"))
...
etfEventTypeCategories =
  lens _etfEventTypeCategories (\s a -> s {_etfEventTypeCategories = a}) .
  mapping _List1

-- | A list of event type codes.
etfEventTypeCodes :: Lens' EventTypeFilter (Maybe (NonEmpty Text))
etfEventTypeCodes =
  lens _etfEventTypeCodes (\s a -> s {_etfEventTypeCodes = a}) . mapping _List1
