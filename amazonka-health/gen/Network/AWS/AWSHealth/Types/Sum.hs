...
instance FromText EntityStatusCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EntityStatusCode from value: '" <>
        e <> "'. Accepted values: impaired, unimpaired, unknown"
...
instance FromText EventAggregateField where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventAggregateField from value: '" <>
        e <> "'. Accepted values: eventtypecategory"
...
instance FromText EventStatusCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventStatusCode from value: '" <>
        e <> "'. Accepted values: closed, open, upcoming"
...
instance FromText EventTypeCategory where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventTypeCategory from value: '" <>
        e <> "'. Accepted values: accountnotification, issue, scheduledchange"
