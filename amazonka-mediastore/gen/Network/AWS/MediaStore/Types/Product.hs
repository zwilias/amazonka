...
crAllowedMethods =
  lens _crAllowedMethods (\s a -> s {_crAllowedMethods = a}) .
  _Default . _Coerce
...
crMaxAgeSeconds =
  lens _crMaxAgeSeconds (\s a -> s {_crMaxAgeSeconds = a}) . mapping _Nat
...
crAllowedOrigins =
  lens _crAllowedOrigins (\s a -> s {_crAllowedOrigins = a}) .
  _Default . _Coerce
...
instance FromJSON CORSRule where
  parseJSON =
    withObject
      "CORSRule"
      (\x ->
         CORSRule' <$> (x .:? "AllowedMethods" .!= mempty) <*>
         (x .:? "MaxAgeSeconds") <*>
...
instance FromJSON Container where
  parseJSON =
    withObject
      "Container"
      (\x ->
         Container' <$> (x .:? "CreationTime") <*> (x .:? "Status") <*>
         (x .:? "ARN") <*>
         (x .:? "Name") <*>
         (x .:? "Endpoint"))
