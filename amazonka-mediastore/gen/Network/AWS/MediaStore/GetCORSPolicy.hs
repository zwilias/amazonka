...
instance AWSRequest GetCORSPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCORSPolicyResponse' <$> (pure (fromEnum s)) <*> (x .:> "CorsPolicy"))
...
instance ToHeaders GetCORSPolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("MediaStore_20170901.GetCorsPolicy" :: ByteString)
...
gcorsprsResponseStatus =
  lens _gcorsprsResponseStatus (\s a -> s {_gcorsprsResponseStatus = a})

-- | Undocumented member.
gcorsprsCORSPolicy :: Lens' GetCORSPolicyResponse (NonEmpty CORSRule)
gcorsprsCORSPolicy =
  lens _gcorsprsCORSPolicy (\s a -> s {_gcorsprsCORSPolicy = a}) . _List1
