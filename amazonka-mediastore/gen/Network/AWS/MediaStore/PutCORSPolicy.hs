...
instance AWSRequest PutCORSPolicy where
  ...
  response =
    receiveEmpty (\s h x -> PutCORSPolicyResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders PutCORSPolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("MediaStore_20170901.PutCorsPolicy" :: ByteString)
...
pcorsprsResponseStatus =
  lens _pcorsprsResponseStatus (\s a -> s {_pcorsprsResponseStatus = a})
