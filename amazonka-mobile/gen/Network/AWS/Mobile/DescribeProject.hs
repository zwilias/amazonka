...
dSyncFromResources =
  lens _dSyncFromResources (\s a -> s {_dSyncFromResources = a})
...
instance AWSRequest DescribeProject where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeProjectResponse' <$> (x .?> "details") <*> (pure (fromEnum s)))
...
instance ToQuery DescribeProject where
  toQuery DescribeProject' {..} =
    mconcat
      ["syncFromResources" =: _dSyncFromResources, "projectId" =: _dProjectId]
