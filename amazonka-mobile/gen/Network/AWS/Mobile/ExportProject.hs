...
instance AWSRequest ExportProject where
  ...
  response =
    receiveJSON
      (\s h x ->
         ExportProjectResponse' <$> (x .?> "shareUrl") <*> (x .?> "downloadUrl") <*>
         (x .?> "snapshotId") <*>
         (pure (fromEnum s)))
...
instance ToPath ExportProject where
  toPath ExportProject' {..} = mconcat ["/exports/", toBS _epProjectId]
...
eprsResponseStatus =
  lens _eprsResponseStatus (\s a -> s {_eprsResponseStatus = a})
