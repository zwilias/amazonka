...
instance AWSRequest ListBundles where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListBundlesResponse' <$> (x .?> "bundleList" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListBundles where
  toQuery ListBundles' {..} =
    mconcat ["nextToken" =: _lbNextToken, "maxResults" =: _lbMaxResults]
...
lbrsBundleList =
  lens _lbrsBundleList (\s a -> s {_lbrsBundleList = a}) . _Default . _Coerce
