...
instance FromText LifeCycleState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LifeCycleState from value: '" <>
        e <> "'. Accepted values: available, creating, deleted, deleting"
...
instance FromText PerformanceMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PerformanceMode from value: '" <>
        e <> "'. Accepted values: generalpurpose, maxio"
