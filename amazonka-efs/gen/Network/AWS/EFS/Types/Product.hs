...
fsdCreationTime =
  lens _fsdCreationTime (\s a -> s {_fsdCreationTime = a}) . _Time
...
fsdNumberOfMountTargets =
  lens _fsdNumberOfMountTargets (\s a -> s {_fsdNumberOfMountTargets = a}) .
  _Nat
...
fsdPerformanceMode =
  lens _fsdPerformanceMode (\s a -> s {_fsdPerformanceMode = a})
...
instance FromJSON FileSystemDescription where
  parseJSON =
    withObject
      "FileSystemDescription"
      (\x ->
         FileSystemDescription' <$> (x .:? "Encrypted") <*> (x .:? "KmsKeyId") <*>
         (x .:? "Name") <*>
         (x .: "OwnerId") <*>
         (x .: "CreationToken") <*>
         (x .: "FileSystemId") <*>
...
fssTimestamp =
  lens _fssTimestamp (\s a -> s {_fssTimestamp = a}) . mapping _Time
...
instance FromJSON FileSystemSize where
  parseJSON =
    withObject
      "FileSystemSize"
      (\x -> FileSystemSize' <$> (x .:? "Timestamp") <*> (x .: "Value"))
...
mtdNetworkInterfaceId =
  lens _mtdNetworkInterfaceId (\s a -> s {_mtdNetworkInterfaceId = a})
...
instance FromJSON MountTargetDescription where
  parseJSON =
    withObject
      "MountTargetDescription"
      (\x ->
         MountTargetDescription' <$> (x .:? "IpAddress") <*>
