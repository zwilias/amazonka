...
data PosixUser = PosixUser'{_puSecondaryGids ::
                            !(Maybe [Nat]),
                            _puUid :: !Nat, _puGid :: !Nat}
...
posixUser pUid_ pGid_
  = PosixUser'{_puSecondaryGids = Nothing,
               _puUid = _Nat # pUid_, _puGid = _Nat # pGid_}
...
instance FromJSON PosixUser where
        parseJSON
          = withObject "PosixUser"
              (\ x ->
                 PosixUser' <$>
                   (x .:? "SecondaryGids" .!= mempty) <*> (x .: "Uid")
                     <*> (x .: "Gid"))

instance Hashable PosixUser where

instance NFData PosixUser where
          ...
          = object
              (catMaybes
                 [("SecondaryGids" .=) <$> _puSecondaryGids,
                  Just ("Uid" .= _puUid), Just ("Gid" .= _puGid)])
