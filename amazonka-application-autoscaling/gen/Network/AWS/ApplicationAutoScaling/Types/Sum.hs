...
instance FromText MetricAggregationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MetricAggregationType from value: '" <>
        e <> "'. Accepted values: average, maximum, minimum"
...
instance FromText MetricType where
  parser =
    takeLowerText >>= \case
      ...
      "dynamodbwritecapacityutilization" ->
        pure DynamoDBWriteCapacityUtilization
      "ec2spotfleetrequestaveragecpuutilization" ->
        pure EC2SpotFleetRequestAverageCPUUtilization
      "ec2spotfleetrequestaveragenetworkin" ->
        pure EC2SpotFleetRequestAverageNetworkIn
      "ec2spotfleetrequestaveragenetworkout" ->
        pure EC2SpotFleetRequestAverageNetworkOut
      "ecsserviceaveragecpuutilization" -> pure ECSServiceAverageCPUUtilization
      "ecsserviceaveragememoryutilization" ->
        pure ECSServiceAverageMemoryUtilization
      "rdsreaderaveragecpuutilization" -> pure RDSReaderAverageCPUUtilization
      "rdsreaderaveragedatabaseconnections" ->
        pure RDSReaderAverageDatabaseConnections
      "sagemakervariantinvocationsperinstance" ->
        pure SageMakerVariantInvocationsPerInstance
      e ->
        fromTextError $
        "Failure parsing MetricType from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: stepscaling, targettrackingscaling"
  ...
  parser =
    takeLowerText >>= \case
      ...
      "dynamodb:index:writecapacityunits" ->
        pure DynamodbIndexWriteCapacityUnits
      "dynamodb:table:readcapacityunits" -> pure DynamodbTableReadCapacityUnits
      "dynamodb:table:writecapacityunits" ->
        pure DynamodbTableWriteCapacityUnits
      "ec2:spot-fleet-request:targetcapacity" ->
        pure EC2SpotFleetRequestTargetCapacity
      "ecs:service:desiredcount" -> pure EcsServiceDesiredCount
      "elasticmapreduce:instancegroup:instancecount" ->
        pure ElasticmapreduceInstancegroupInstanceCount
      "rds:cluster:readreplicacount" -> pure RDSClusterReadReplicaCount
      "sagemaker:variant:desiredinstancecount" ->
        pure SagemakerVariantDesiredInstanceCount
      e ->
        fromTextError $
        "Failure parsing ScalableDimension from value: '" <>
        e <>
...
instance FromText ScalingActivityStatusCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ScalingActivityStatusCode from value: '" <>
        e <>
...
instance FromText ServiceNamespace where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ServiceNamespace from value: '" <>
        e <>
