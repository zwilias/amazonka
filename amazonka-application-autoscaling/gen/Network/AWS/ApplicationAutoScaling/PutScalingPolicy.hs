...
pspTargetTrackingScalingPolicyConfiguration ::
     Lens' PutScalingPolicy (Maybe TargetTrackingScalingPolicyConfiguration)
pspTargetTrackingScalingPolicyConfiguration =
  lens
    _pspTargetTrackingScalingPolicyConfiguration
    (\s a -> s {_pspTargetTrackingScalingPolicyConfiguration = a})

-- | A step scaling policy. This parameter is required if you are creating a policy and the policy type is @StepScaling@ .
pspStepScalingPolicyConfiguration ::
     Lens' PutScalingPolicy (Maybe StepScalingPolicyConfiguration)
pspStepScalingPolicyConfiguration =
  lens
    _pspStepScalingPolicyConfiguration
    (\s a -> s {_pspStepScalingPolicyConfiguration = a})
...
pspServiceNamespace =
  lens _pspServiceNamespace (\s a -> s {_pspServiceNamespace = a})
...
pspScalableDimension =
  lens _pspScalableDimension (\s a -> s {_pspScalableDimension = a})
...
instance AWSRequest PutScalingPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutScalingPolicyResponse' <$> (x .?> "Alarms" .!@ mempty) <*>
         (pure (fromEnum s)) <*>
         (x .:> "PolicyARN"))
...
psprsAlarms =
  lens _psprsAlarms (\s a -> s {_psprsAlarms = a}) . _Default . _Coerce
