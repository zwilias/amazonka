...
rstServiceNamespace =
  lens _rstServiceNamespace (\s a -> s {_rstServiceNamespace = a})
...
rstScalableDimension =
  lens _rstScalableDimension (\s a -> s {_rstScalableDimension = a})
...
instance AWSRequest RegisterScalableTarget where
  ...
  response =
    receiveEmpty
      (\s h x -> RegisterScalableTargetResponse' <$> (pure (fromEnum s)))
...
rstrsResponseStatus =
  lens _rstrsResponseStatus (\s a -> s {_rstrsResponseStatus = a})
