...
derServiceNamespace =
  lens _derServiceNamespace (\s a -> s {_derServiceNamespace = a})
...
derScalableDimension =
  lens _derScalableDimension (\s a -> s {_derScalableDimension = a})
...
instance AWSRequest DeregisterScalableTarget where
  ...
  response =
    receiveEmpty
      (\s h x -> DeregisterScalableTargetResponse' <$> (pure (fromEnum s)))
...
dstrsResponseStatus =
  lens _dstrsResponseStatus (\s a -> s {_dstrsResponseStatus = a})
