...
cjUserMetadata =
  lens _cjUserMetadata (\s a -> s {_cjUserMetadata = a}) . _Default . _Map
...
cjClientRequestToken =
  lens _cjClientRequestToken (\s a -> s {_cjClientRequestToken = a})
...
instance AWSRequest CreateJob where
  ...
  response =
    receiveJSON
      (\s h x -> CreateJobResponse' <$> (x .?> "job") <*> (pure (fromEnum s)))
