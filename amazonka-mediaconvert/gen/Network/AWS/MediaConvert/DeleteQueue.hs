...
instance AWSRequest DeleteQueue where
  ...
  response =
    receiveEmpty (\s h x -> DeleteQueueResponse' <$> (pure (fromEnum s)))
...
instance ToPath DeleteQueue where
  toPath DeleteQueue' {..} = mconcat ["/2017-08-29/queues/", toBS _dqName]
...
dqrsResponseStatus =
  lens _dqrsResponseStatus (\s a -> s {_dqrsResponseStatus = a})
