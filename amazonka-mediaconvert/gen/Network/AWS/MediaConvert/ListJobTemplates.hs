...
instance AWSRequest ListJobTemplates where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListJobTemplatesResponse' <$> (x .?> "jobTemplates" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
ljtrsJobTemplates =
  lens _ljtrsJobTemplates (\s a -> s {_ljtrsJobTemplates = a}) .
  _Default . _Coerce
...
ljtrsResponseStatus =
  lens _ljtrsResponseStatus (\s a -> s {_ljtrsResponseStatus = a})
