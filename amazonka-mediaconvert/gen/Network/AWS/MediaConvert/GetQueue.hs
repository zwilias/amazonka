...
instance AWSRequest GetQueue where
  ...
  response =
    receiveJSON
      (\s h x -> GetQueueResponse' <$> (x .?> "queue") <*> (pure (fromEnum s)))
...
instance ToPath GetQueue where
  toPath GetQueue' {..} = mconcat ["/2017-08-29/queues/", toBS _gqName]
...
gqrsResponseStatus =
  lens _gqrsResponseStatus (\s a -> s {_gqrsResponseStatus = a})
