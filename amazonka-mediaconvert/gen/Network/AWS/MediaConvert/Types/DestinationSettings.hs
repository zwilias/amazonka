...
newtype DestinationSettings = DestinationSettings'{_dsS3Settings
                                                   ::
...
instance FromJSON DestinationSettings where
        parseJSON
          = withObject "DestinationSettings"
              (\ x ->
                 DestinationSettings' <$> (x .:? "s3Settings"))

instance Hashable DestinationSettings where

instance NFData DestinationSettings where
