...
data QueueTransition = QueueTransition'{_qtSourceQueue
                                        :: !(Maybe Text),
...
queueTransition
    :: QueueTransition
queueTransition
  = QueueTransition'{_qtSourceQueue = Nothing,
...
instance FromJSON QueueTransition where
        parseJSON
          = withObject "QueueTransition"
              (\ x ->
                 QueueTransition' <$>
                   (x .:? "sourceQueue") <*> (x .:? "destinationQueue")
                     <*> (x .:? "timestamp"))

instance Hashable QueueTransition where

instance NFData QueueTransition where
