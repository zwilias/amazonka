...
data S3DestinationSettings = S3DestinationSettings'{_sdsAccessControl
                                                    ::
                                                    !(Maybe
...
s3DestinationSettings
    :: S3DestinationSettings
s3DestinationSettings
  = S3DestinationSettings'{_sdsAccessControl = Nothing,
...
instance FromJSON S3DestinationSettings where
        parseJSON
          = withObject "S3DestinationSettings"
              (\ x ->
                 S3DestinationSettings' <$>
                   (x .:? "accessControl") <*> (x .:? "encryption"))

instance Hashable S3DestinationSettings where

instance NFData S3DestinationSettings where
