...
newtype S3DestinationAccessControl = S3DestinationAccessControl'{_sdacCannedACL
                                                                 ::
                                                                 Maybe
                                                                   S3ObjectCannedACL}
...
s3DestinationAccessControl
    :: S3DestinationAccessControl
s3DestinationAccessControl
  = S3DestinationAccessControl'{_sdacCannedACL =
                                  Nothing}
...
instance FromJSON S3DestinationAccessControl where
        parseJSON
          = withObject "S3DestinationAccessControl"
              (\ x ->
                 S3DestinationAccessControl' <$> (x .:? "cannedAcl"))

instance Hashable S3DestinationAccessControl where

instance NFData S3DestinationAccessControl where
