...
data JobMessages = JobMessages'{_jmWarning ::
                                !(Maybe [Text]),
                                _jmInfo :: !(Maybe [Text])}
...
jobMessages
    :: JobMessages
jobMessages
  = JobMessages'{_jmWarning = Nothing,
                 _jmInfo = Nothing}
...
instance FromJSON JobMessages where
        parseJSON
          = withObject "JobMessages"
              (\ x ->
                 JobMessages' <$>
                   (x .:? "warning" .!= mempty) <*>
                     (x .:? "info" .!= mempty))

instance Hashable JobMessages where

instance NFData JobMessages where
