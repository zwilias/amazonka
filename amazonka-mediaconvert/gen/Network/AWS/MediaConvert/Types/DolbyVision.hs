...
data DolbyVision = DolbyVision'{_dvProfile ::
...
dolbyVision
    :: DolbyVision
dolbyVision
  = DolbyVision'{_dvProfile = Nothing,
                 _dvL6Mode = Nothing, _dvL6Metadata = Nothing}
...
instance FromJSON DolbyVision where
        parseJSON
          = withObject "DolbyVision"
              (\ x ->
                 DolbyVision' <$>
                   (x .:? "profile") <*> (x .:? "l6Mode") <*>
                     (x .:? "l6Metadata"))

instance Hashable DolbyVision where

instance NFData DolbyVision where
