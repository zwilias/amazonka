...
dtaTapeARNs =
  lens _dtaTapeARNs (\s a -> s {_dtaTapeARNs = a}) . _Default . _Coerce
...
instance AWSRequest DescribeTapeArchives where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeTapeArchivesResponse' <$> (x .?> "TapeArchives" .!@ mempty) <*>
         (x .?> "Marker") <*>
         (pure (fromEnum s)))
...
dtarsTapeArchives =
  lens _dtarsTapeArchives (\s a -> s {_dtarsTapeArchives = a}) .
  _Default . _Coerce
...
dtarsResponseStatus =
  lens _dtarsResponseStatus (\s a -> s {_dtarsResponseStatus = a})
