...
instance AWSRequest DescribeCache where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeCacheResponse' <$> (x .?> "GatewayARN") <*>
         (x .?> "DiskIds" .!@ mempty) <*>
...
dcrsDiskIds =
  lens _dcrsDiskIds (\s a -> s {_dcrsDiskIds = a}) . _Default . _Coerce

-- | Undocumented member.
dcrsCacheUsedPercentage :: Lens' DescribeCacheResponse (Maybe Double)
dcrsCacheUsedPercentage =
  lens _dcrsCacheUsedPercentage (\s a -> s {_dcrsCacheUsedPercentage = a})
...
dcrsCacheMissPercentage =
  lens _dcrsCacheMissPercentage (\s a -> s {_dcrsCacheMissPercentage = a})

-- | Undocumented member.
dcrsCacheAllocatedInBytes :: Lens' DescribeCacheResponse (Maybe Integer)
dcrsCacheAllocatedInBytes =
  lens _dcrsCacheAllocatedInBytes (\s a -> s {_dcrsCacheAllocatedInBytes = a})

-- | Undocumented member.
dcrsCacheDirtyPercentage :: Lens' DescribeCacheResponse (Maybe Double)
dcrsCacheDirtyPercentage =
  lens _dcrsCacheDirtyPercentage (\s a -> s {_dcrsCacheDirtyPercentage = a})
