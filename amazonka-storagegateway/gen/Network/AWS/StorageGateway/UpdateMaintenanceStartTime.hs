...
umstMinuteOfHour =
  lens _umstMinuteOfHour (\s a -> s {_umstMinuteOfHour = a}) . _Nat
...
instance AWSRequest UpdateMaintenanceStartTime where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateMaintenanceStartTimeResponse' <$> (x .?> "GatewayARN") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateMaintenanceStartTime where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StorageGateway_20130630.UpdateMaintenanceStartTime" :: ByteString)
...
umstrsResponseStatus =
  lens _umstrsResponseStatus (\s a -> s {_umstrsResponseStatus = a})
