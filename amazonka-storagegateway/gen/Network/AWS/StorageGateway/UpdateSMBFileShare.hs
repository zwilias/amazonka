...
data UpdateSMBFileShare = UpdateSMBFileShare'{_usmbfsAdminUserList
                                              :: !(Maybe [Text]),
...
--
-- * 'usmbfsAdminUserList' - A list of users in the Active Directory that have administrator rights to the file share. A group must be prefixed with the @ character. For example @@group1@ . Can only be set if Authentication is set to @ActiveDirectory@ .
--
...
--
-- * 'usmbfsSMBACLEnabled' - Set this value to "true to enable ACL (access control list) on the SMB file share. Set it to "false" to map file and directory permissions to the POSIX permissions. For more information, see https://docs.aws.amazon.com/storagegateway/latest/userguide/smb-acl.htmlin the Storage Gateway User Guide.
--
-- * 'usmbfsRequesterPays' - A value that sets who pays the cost of the request and the cost associated with data download from the S3 bucket. If this value is set to true, the requester pays the costs. Otherwise the S3 bucket owner pays. However, the S3 bucket owner always pays the cost of storing data.
--
...
updateSMBFileShare pFileShareARN_
  = UpdateSMBFileShare'{_usmbfsAdminUserList = Nothing,
