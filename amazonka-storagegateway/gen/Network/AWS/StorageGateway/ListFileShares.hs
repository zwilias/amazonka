...
instance AWSRequest ListFileShares where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListFileSharesResponse' <$> (x .?> "FileShareInfoList" .!@ mempty) <*>
         (x .?> "Marker") <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
lfsrsFileShareInfoList =
  lens _lfsrsFileShareInfoList (\s a -> s {_lfsrsFileShareInfoList = a}) .
  _Default . _Coerce
...
lfsrsResponseStatus =
  lens _lfsrsResponseStatus (\s a -> s {_lfsrsResponseStatus = a})
