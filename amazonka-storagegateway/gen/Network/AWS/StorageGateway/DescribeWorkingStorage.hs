...
instance AWSRequest DescribeWorkingStorage where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeWorkingStorageResponse' <$> (x .?> "GatewayARN") <*>
         (x .?> "DiskIds" .!@ mempty) <*>
...
dwsrsDiskIds =
  lens _dwsrsDiskIds (\s a -> s {_dwsrsDiskIds = a}) . _Default . _Coerce

-- | The total working storage in bytes allocated for the gateway. If no working storage is configured for the gateway, this field returns 0.
dwsrsWorkingStorageAllocatedInBytes ::
     Lens' DescribeWorkingStorageResponse (Maybe Integer)
dwsrsWorkingStorageAllocatedInBytes =
  lens
...
dwsrsResponseStatus =
  lens _dwsrsResponseStatus (\s a -> s {_dwsrsResponseStatus = a})
