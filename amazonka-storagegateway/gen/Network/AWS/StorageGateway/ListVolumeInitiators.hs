...
instance AWSRequest ListVolumeInitiators where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListVolumeInitiatorsResponse' <$> (x .?> "Initiators" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lvirsInitiators =
  lens _lvirsInitiators (\s a -> s {_lvirsInitiators = a}) . _Default . _Coerce

-- | -- | The response status code.
lvirsResponseStatus :: Lens' ListVolumeInitiatorsResponse Int
lvirsResponseStatus =
  lens _lvirsResponseStatus (\s a -> s {_lvirsResponseStatus = a})
