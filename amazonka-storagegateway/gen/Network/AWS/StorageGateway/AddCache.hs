...
instance ToHeaders AddCache where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("StorageGateway_20130630.AddCache" :: ByteString)
...
instance ToJSON AddCache where
  toJSON AddCache' {..} =
    object
      (catMaybes
         [Just ("GatewayARN" .= _acGatewayARN), Just ("DiskIds" .= _acDiskIds)])
...
acrsResponseStatus =
  lens _acrsResponseStatus (\s a -> s {_acrsResponseStatus = a})
