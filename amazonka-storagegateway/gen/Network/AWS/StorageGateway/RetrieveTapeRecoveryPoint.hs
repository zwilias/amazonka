...
instance AWSRequest RetrieveTapeRecoveryPoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         RetrieveTapeRecoveryPointResponse' <$> (x .?> "TapeARN") <*>
         (pure (fromEnum s)))
...
instance ToHeaders RetrieveTapeRecoveryPoint where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StorageGateway_20130630.RetrieveTapeRecoveryPoint" :: ByteString)
...
rtrprsResponseStatus =
  lens _rtrprsResponseStatus (\s a -> s {_rtrprsResponseStatus = a})
