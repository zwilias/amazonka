...
-- | Undocumented member.
dubrsUploadBufferAllocatedInBytes ::
     Lens' DescribeUploadBufferResponse (Maybe Integer)
dubrsUploadBufferAllocatedInBytes =
  lens
...
dubrsDiskIds =
  lens _dubrsDiskIds (\s a -> s {_dubrsDiskIds = a}) . _Default . _Coerce
...
dubrsResponseStatus =
  lens _dubrsResponseStatus (\s a -> s {_dubrsResponseStatus = a})
