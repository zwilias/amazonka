...
instance AWSRequest DescribeBandwidthRateLimit where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeBandwidthRateLimitResponse' <$> (x .?> "GatewayARN") <*>
...
instance ToHeaders DescribeBandwidthRateLimit where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StorageGateway_20130630.DescribeBandwidthRateLimit" :: ByteString)
...
dbrlrsAverageUploadRateLimitInBitsPerSec ::
     Lens' DescribeBandwidthRateLimitResponse (Maybe Natural)
dbrlrsAverageUploadRateLimitInBitsPerSec =
  lens
    _dbrlrsAverageUploadRateLimitInBitsPerSec
    (\s a -> s {_dbrlrsAverageUploadRateLimitInBitsPerSec = a}) .
  mapping _Nat
...
dbrlrsAverageDownloadRateLimitInBitsPerSec =
  lens
    ...
    (\s a -> s {_dbrlrsAverageDownloadRateLimitInBitsPerSec = a}) .
  mapping _Nat

-- | -- | The response status code.
dbrlrsResponseStatus :: Lens' DescribeBandwidthRateLimitResponse Int
dbrlrsResponseStatus =
  lens _dbrlrsResponseStatus (\s a -> s {_dbrlrsResponseStatus = a})
