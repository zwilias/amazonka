...
slcpLocalConsolePassword =
  lens _slcpLocalConsolePassword (\s a -> s {_slcpLocalConsolePassword = a}) .
  _Sensitive
...
instance AWSRequest SetLocalConsolePassword where
  ...
  response =
    receiveJSON
      (\s h x ->
         SetLocalConsolePasswordResponse' <$> (x .?> "GatewayARN") <*>
         (pure (fromEnum s)))
...
slcprsResponseStatus =
  lens _slcprsResponseStatus (\s a -> s {_slcprsResponseStatus = a})
