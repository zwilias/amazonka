...
data CreateSMBFileShare = CreateSMBFileShare'{_csmbfsAdminUserList
                                              :: !(Maybe [Text]),
...
--
-- * 'csmbfsSMBACLEnabled' - Set this value to "true to enable ACL (access control list) on the SMB file share. Set it to "false" to map file and directory permissions to the POSIX permissions. For more information, see https://docs.aws.amazon.com/storagegateway/latest/userguide/smb-acl.html in the Storage Gateway User Guide.
--
-- * 'csmbfsRequesterPays' - A value that sets who pays the cost of the request and the cost associated with data download from the S3 bucket. If this value is set to true, the requester pays the costs. Otherwise the S3 bucket owner pays. However, the S3 bucket owner always pays the cost of storing data.
--
...
createSMBFileShare pClientToken_ pGatewayARN_ pRole_
  pLocationARN_
  = CreateSMBFileShare'{_csmbfsAdminUserList = Nothing,
                        ...
                        _csmbfsReadOnly = Nothing, _csmbfsTags = Nothing,
