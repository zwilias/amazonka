...
instance AWSRequest UpdateVTLDeviceType where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateVTLDeviceTypeResponse' <$> (x .?> "VTLDeviceARN") <*>
         (pure (fromEnum s)))
...
uvtldtrsVTLDeviceARN =
  lens _uvtldtrsVTLDeviceARN (\s a -> s {_uvtldtrsVTLDeviceARN = a})

-- | -- | The response status code.
uvtldtrsResponseStatus :: Lens' UpdateVTLDeviceTypeResponse Int
uvtldtrsResponseStatus =
  lens _uvtldtrsResponseStatus (\s a -> s {_uvtldtrsResponseStatus = a})
