...
cnfsfsKMSEncrypted =
  lens _cnfsfsKMSEncrypted (\s a -> s {_cnfsfsKMSEncrypted = a})
...
cnfsfsDefaultStorageClass =
  lens _cnfsfsDefaultStorageClass (\s a -> s {_cnfsfsDefaultStorageClass = a})
...
cnfsfsRequesterPays =
  lens _cnfsfsRequesterPays (\s a -> s {_cnfsfsRequesterPays = a})

-- | File share default values. Optional.
cnfsfsNFSFileShareDefaults ::
     Lens' CreateNFSFileShare (Maybe NFSFileShareDefaults)
cnfsfsNFSFileShareDefaults =
  lens _cnfsfsNFSFileShareDefaults (\s a -> s {_cnfsfsNFSFileShareDefaults = a})

-- | The list of clients that are allowed to access the file gateway. The list must contain either valid IP addresses or valid CIDR blocks.
cnfsfsClientList :: Lens' CreateNFSFileShare (Maybe (NonEmpty Text))
cnfsfsClientList =
  lens _cnfsfsClientList (\s a -> s {_cnfsfsClientList = a}) . mapping _List1
...
cnfsfsGuessMIMETypeEnabled =
  lens _cnfsfsGuessMIMETypeEnabled (\s a -> s {_cnfsfsGuessMIMETypeEnabled = a})
...
instance AWSRequest CreateNFSFileShare where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateNFSFileShareResponse' <$> (x .?> "FileShareARN") <*>
         (pure (fromEnum s)))
...
cnfsfsrsFileShareARN =
  lens _cnfsfsrsFileShareARN (\s a -> s {_cnfsfsrsFileShareARN = a})

-- | -- | The response status code.
cnfsfsrsResponseStatus :: Lens' CreateNFSFileShareResponse Int
cnfsfsrsResponseStatus =
  lens _cnfsfsrsResponseStatus (\s a -> s {_cnfsfsrsResponseStatus = a})
