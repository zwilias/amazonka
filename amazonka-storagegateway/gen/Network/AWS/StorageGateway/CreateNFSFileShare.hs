...
--
-- * 'cnfsfsSquash' - A value that maps a user to anonymous user. Valid options are the following:      * @RootSquash@ - Only root is mapped to anonymous user.     * @NoSquash@ - No one is mapped to anonymous user     * @AllSquash@ - Everyone is mapped to anonymous user.
--
-- * 'cnfsfsRequesterPays' - A value that sets who pays the cost of the request and the cost associated with data download from the S3 bucket. If this value is set to true, the requester pays the costs. Otherwise the S3 bucket owner pays. However, the S3 bucket owner always pays the cost of storing data.
--
