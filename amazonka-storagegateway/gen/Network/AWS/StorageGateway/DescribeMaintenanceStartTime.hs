...
instance AWSRequest DescribeMaintenanceStartTime where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeMaintenanceStartTimeResponse' <$> (x .?> "GatewayARN") <*>
         (x .?> "MinuteOfHour") <*>
         (x .?> "HourOfDay") <*>
         (x .?> "Timezone") <*>
         (x .?> "DayOfWeek") <*>
...
instance ToHeaders DescribeMaintenanceStartTime where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StorageGateway_20130630.DescribeMaintenanceStartTime" :: ByteString)
...
dmstrsMinuteOfHour =
  lens _dmstrsMinuteOfHour (\s a -> s {_dmstrsMinuteOfHour = a}) . mapping _Nat

-- | The hour component of the maintenance start time represented as /hh/ , where /hh/ is the hour (0 to 23). The hour of the day is in the time zone of the gateway.
dmstrsHourOfDay :: Lens' DescribeMaintenanceStartTimeResponse (Maybe Natural)
dmstrsHourOfDay =
  lens _dmstrsHourOfDay (\s a -> s {_dmstrsHourOfDay = a}) . mapping _Nat
...
dmstrsDayOfWeek =
  lens _dmstrsDayOfWeek (\s a -> s {_dmstrsDayOfWeek = a}) . mapping _Nat

-- | -- | The response status code.
dmstrsResponseStatus :: Lens' DescribeMaintenanceStartTimeResponse Int
dmstrsResponseStatus =
  lens _dmstrsResponseStatus (\s a -> s {_dmstrsResponseStatus = a})
