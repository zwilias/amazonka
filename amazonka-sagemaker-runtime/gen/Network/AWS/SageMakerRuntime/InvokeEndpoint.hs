...
instance AWSRequest InvokeEndpoint where
  ...
  response =
    receiveBytes
      (\s h x ->
         InvokeEndpointResponse' <$> (h .#? "x-Amzn-Invoked-Production-Variant") <*>
         (h .#? "Content-Type") <*>
         (pure (fromEnum s)) <*>
         (pure x))
...
instance ToHeaders InvokeEndpoint where
  toHeaders InvokeEndpoint' {..} =
    mconcat ["Accept" =# _ieAccept, "Content-Type" =# _ieContentType]
...
iersInvokedProductionVariant =
  lens
    ...
    (\s a -> s {_iersInvokedProductionVariant = a})
...
iersResponseStatus =
  lens _iersResponseStatus (\s a -> s {_iersResponseStatus = a})
