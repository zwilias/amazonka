...
gcfccBeforeCommitId =
  lens _gcfccBeforeCommitId (\s a -> s {_gcfccBeforeCommitId = a})
...
gcfccRepositoryName =
  lens _gcfccRepositoryName (\s a -> s {_gcfccRepositoryName = a})

-- | To establish the directionality of the comparison, the full commit ID of the 'after' commit.
gcfccAfterCommitId :: Lens' GetCommentsForComparedCommit Text
gcfccAfterCommitId =
  lens _gcfccAfterCommitId (\s a -> s {_gcfccAfterCommitId = a})
...
instance AWSRequest GetCommentsForComparedCommit where
  type Rs GetCommentsForComparedCommit = GetCommentsForComparedCommitResponse
...
gcfccrsCommentsForComparedCommitData ::
     Lens' GetCommentsForComparedCommitResponse [CommentsForComparedCommit]
gcfccrsCommentsForComparedCommitData =
  lens
    _gcfccrsCommentsForComparedCommitData
    (\s a -> s {_gcfccrsCommentsForComparedCommitData = a}) .
  _Default . _Coerce
...
gcfccrsResponseStatus =
  lens _gcfccrsResponseStatus (\s a -> s {_gcfccrsResponseStatus = a})
