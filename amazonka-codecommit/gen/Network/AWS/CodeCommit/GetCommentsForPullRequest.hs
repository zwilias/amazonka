...
gcfprAfterCommitId =
  lens _gcfprAfterCommitId (\s a -> s {_gcfprAfterCommitId = a})
...
gcfprBeforeCommitId =
  lens _gcfprBeforeCommitId (\s a -> s {_gcfprBeforeCommitId = a})

-- | The name of the repository that contains the pull request.
gcfprRepositoryName :: Lens' GetCommentsForPullRequest (Maybe Text)
gcfprRepositoryName =
  lens _gcfprRepositoryName (\s a -> s {_gcfprRepositoryName = a})
...
gcfprPullRequestId =
  lens _gcfprPullRequestId (\s a -> s {_gcfprPullRequestId = a})
...
instance AWSRequest GetCommentsForPullRequest where
  type Rs GetCommentsForPullRequest = GetCommentsForPullRequestResponse
...
gcfprrsCommentsForPullRequestData ::
     Lens' GetCommentsForPullRequestResponse [CommentsForPullRequest]
gcfprrsCommentsForPullRequestData =
  lens
    _gcfprrsCommentsForPullRequestData
    (\s a -> s {_gcfprrsCommentsForPullRequestData = a}) .
  _Default . _Coerce
...
gcfprrsResponseStatus =
  lens _gcfprrsResponseStatus (\s a -> s {_gcfprrsResponseStatus = a})
