...
instance AWSRequest UpdatePullRequestDescription where
  type Rs UpdatePullRequestDescription = UpdatePullRequestDescriptionResponse
  request = postJSON codeCommit
  response =
    receiveJSON
      (\s h x ->
         UpdatePullRequestDescriptionResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "pullRequest"))
...
uprdrsResponseStatus =
  lens _uprdrsResponseStatus (\s a -> s {_uprdrsResponseStatus = a})
