...
instance AWSRequest PostCommentForPullRequest where
  ...
  response =
    receiveJSON
      (\s h x ->
         PostCommentForPullRequestResponse' <$> (x .?> "beforeBlobId") <*>
         (x .?> "location") <*>
         (x .?> "afterCommitId") <*>
         (x .?> "pullRequestId") <*>
         (x .?> "afterBlobId") <*>
...
pcfprrsBeforeBlobId =
  lens _pcfprrsBeforeBlobId (\s a -> s {_pcfprrsBeforeBlobId = a})
...
pcfprrsResponseStatus =
  lens _pcfprrsResponseStatus (\s a -> s {_pcfprrsResponseStatus = a})
