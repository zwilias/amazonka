...
instance AWSRequest UpdateComment where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateCommentResponse' <$> (x .?> "comment") <*> (pure (fromEnum s)))
...
instance ToHeaders UpdateComment where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeCommit_20150413.UpdateComment" :: ByteString)
...
instance ToJSON UpdateComment where
  toJSON UpdateComment' {..} =
    object
      (catMaybes
         [Just ("commentId" .= _ucCommentId), Just ("content" .= _ucContent)])
