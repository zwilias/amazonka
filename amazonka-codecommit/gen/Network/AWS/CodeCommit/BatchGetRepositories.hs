...
bgrRepositoryNames =
  lens _bgrRepositoryNames (\s a -> s {_bgrRepositoryNames = a}) . _Coerce
...
instance AWSRequest BatchGetRepositories where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchGetRepositoriesResponse' <$> (x .?> "repositories" .!@ mempty) <*>
...
instance ToJSON BatchGetRepositories where
  toJSON BatchGetRepositories' {..} =
    object (catMaybes [Just ("repositoryNames" .= _bgrRepositoryNames)])
...
bgrrsRepositories =
  lens _bgrrsRepositories (\s a -> s {_bgrrsRepositories = a}) .
  _Default . _Coerce

-- | Returns a list of repository names for which information could not be found.
bgrrsRepositoriesNotFound :: Lens' BatchGetRepositoriesResponse [Text]
bgrrsRepositoriesNotFound =
  lens _bgrrsRepositoriesNotFound (\s a -> s {_bgrrsRepositoriesNotFound = a}) .
  _Default . _Coerce
