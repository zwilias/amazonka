...
gmcDestinationCommitSpecifier =
  lens
    ...
    (\s a -> s {_gmcDestinationCommitSpecifier = a})

-- | The branch, tag, HEAD, or other fully qualified reference used to identify a commit. For example, a branch name or a full commit ID.
gmcSourceCommitSpecifier :: Lens' GetMergeConflicts Text
gmcSourceCommitSpecifier =
  lens _gmcSourceCommitSpecifier (\s a -> s {_gmcSourceCommitSpecifier = a})
...
instance AWSRequest GetMergeConflicts where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetMergeConflictsResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "mergeable") <*>
  ...
  toJSON GetMergeConflicts' {..} =
    object
      (catMaybes
         ...
         , Just ("destinationCommitSpecifier" .= _gmcDestinationCommitSpecifier)
...
gmcrsResponseStatus =
  lens _gmcrsResponseStatus (\s a -> s {_gmcrsResponseStatus = a})
...
gmcrsDestinationCommitId =
  lens _gmcrsDestinationCommitId (\s a -> s {_gmcrsDestinationCommitId = a})

-- | The commit ID of the source commit specifier that was used in the merge evaluation.
gmcrsSourceCommitId :: Lens' GetMergeConflictsResponse Text
gmcrsSourceCommitId =
  lens _gmcrsSourceCommitId (\s a -> s {_gmcrsSourceCommitId = a})
