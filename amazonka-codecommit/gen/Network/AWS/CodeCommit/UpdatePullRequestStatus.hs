...
uprsPullRequestStatus =
  lens _uprsPullRequestStatus (\s a -> s {_uprsPullRequestStatus = a})
...
instance AWSRequest UpdatePullRequestStatus where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdatePullRequestStatusResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "pullRequest"))
...
uprsrsResponseStatus =
  lens _uprsrsResponseStatus (\s a -> s {_uprsrsResponseStatus = a})
