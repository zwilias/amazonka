...
pcfccBeforeCommitId =
  lens _pcfccBeforeCommitId (\s a -> s {_pcfccBeforeCommitId = a})
...
pcfccClientRequestToken =
  lens _pcfccClientRequestToken (\s a -> s {_pcfccClientRequestToken = a})

-- | The name of the repository where you want to post a comment on the comparison between commits.
pcfccRepositoryName :: Lens' PostCommentForComparedCommit Text
pcfccRepositoryName =
  lens _pcfccRepositoryName (\s a -> s {_pcfccRepositoryName = a})

-- | To establish the directionality of the comparison, the full commit ID of the 'after' commit.
pcfccAfterCommitId :: Lens' PostCommentForComparedCommit Text
pcfccAfterCommitId =
  lens _pcfccAfterCommitId (\s a -> s {_pcfccAfterCommitId = a})
...
instance AWSRequest PostCommentForComparedCommit where
  type Rs PostCommentForComparedCommit = PostCommentForComparedCommitResponse
  request = postJSON codeCommit
  response =
    receiveJSON
      (\s h x ->
         PostCommentForComparedCommitResponse' <$> (x .?> "beforeBlobId") <*>
         (x .?> "location") <*>
         (x .?> "afterCommitId") <*>
         (x .?> "afterBlobId") <*>
         (x .?> "beforeCommitId") <*>
...
pcfccrsBeforeBlobId =
  lens _pcfccrsBeforeBlobId (\s a -> s {_pcfccrsBeforeBlobId = a})
...
pcfccrsResponseStatus =
  lens _pcfccrsResponseStatus (\s a -> s {_pcfccrsResponseStatus = a})
