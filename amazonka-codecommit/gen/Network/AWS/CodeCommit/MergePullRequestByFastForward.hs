...
mprbffSourceCommitId =
  lens _mprbffSourceCommitId (\s a -> s {_mprbffSourceCommitId = a})

-- | The system-generated ID of the pull request. To get this ID, use 'ListPullRequests' .
mprbffPullRequestId :: Lens' MergePullRequestByFastForward Text
mprbffPullRequestId =
  lens _mprbffPullRequestId (\s a -> s {_mprbffPullRequestId = a})

-- | The name of the repository where the pull request was created.
mprbffRepositoryName :: Lens' MergePullRequestByFastForward Text
mprbffRepositoryName =
  lens _mprbffRepositoryName (\s a -> s {_mprbffRepositoryName = a})

instance AWSRequest MergePullRequestByFastForward where
  type Rs MergePullRequestByFastForward = MergePullRequestByFastForwardResponse
  request = postJSON codeCommit
  response =
    receiveJSON
      (\s h x ->
         MergePullRequestByFastForwardResponse' <$> (x .?> "pullRequest") <*>
         (pure (fromEnum s)))
...
instance ToHeaders MergePullRequestByFastForward where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("CodeCommit_20150413.MergePullRequestByFastForward" :: ByteString)
...
mprbffrsPullRequest ::
     Lens' MergePullRequestByFastForwardResponse (Maybe PullRequest)
mprbffrsPullRequest =
  lens _mprbffrsPullRequest (\s a -> s {_mprbffrsPullRequest = a})

-- | -- | The response status code.
mprbffrsResponseStatus :: Lens' MergePullRequestByFastForwardResponse Int
mprbffrsResponseStatus =
  lens _mprbffrsResponseStatus (\s a -> s {_mprbffrsResponseStatus = a})
