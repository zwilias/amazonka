...
pcrClientRequestToken =
  lens _pcrClientRequestToken (\s a -> s {_pcrClientRequestToken = a})
...
instance AWSRequest PostCommentReply where
  ...
  response =
    receiveJSON
      (\s h x ->
         PostCommentReplyResponse' <$> (x .?> "comment") <*> (pure (fromEnum s)))
...
pcrrsResponseStatus =
  lens _pcrrsResponseStatus (\s a -> s {_pcrrsResponseStatus = a})
