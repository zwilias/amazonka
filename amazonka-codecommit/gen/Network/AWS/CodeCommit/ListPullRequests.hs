...
lprPullRequestStatus =
  lens _lprPullRequestStatus (\s a -> s {_lprPullRequestStatus = a})
...
instance AWSRequest ListPullRequests where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPullRequestsResponse' <$> (x .?> "nextToken") <*>
         (pure (fromEnum s)) <*>
...
lprrsResponseStatus =
  lens _lprrsResponseStatus (\s a -> s {_lprrsResponseStatus = a})

-- | The system-generated IDs of the pull requests.
lprrsPullRequestIds :: Lens' ListPullRequestsResponse [Text]
lprrsPullRequestIds =
  lens _lprrsPullRequestIds (\s a -> s {_lprrsPullRequestIds = a}) . _Coerce
