...
instance AWSRequest PutRepositoryTriggers where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutRepositoryTriggersResponse' <$> (x .?> "configurationId") <*>
         (pure (fromEnum s)))
...
prtrsConfigurationId =
  lens _prtrsConfigurationId (\s a -> s {_prtrsConfigurationId = a})

-- | -- | The response status code.
prtrsResponseStatus :: Lens' PutRepositoryTriggersResponse Int
prtrsResponseStatus =
  lens _prtrsResponseStatus (\s a -> s {_prtrsResponseStatus = a})
