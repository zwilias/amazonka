...
dprePullRequestEventType ::
     Lens' DescribePullRequestEvents (Maybe PullRequestEventType)
dprePullRequestEventType =
  lens _dprePullRequestEventType (\s a -> s {_dprePullRequestEventType = a})
...
instance AWSRequest DescribePullRequestEvents where
  type Rs DescribePullRequestEvents = DescribePullRequestEventsResponse
  request = postJSON codeCommit
  response =
    receiveJSON
      (\s h x ->
         DescribePullRequestEventsResponse' <$> (x .?> "nextToken") <*>
         (pure (fromEnum s)) <*>
...
dprersResponseStatus =
  lens _dprersResponseStatus (\s a -> s {_dprersResponseStatus = a})

-- | Information about the pull request events.
dprersPullRequestEvents ::
     Lens' DescribePullRequestEventsResponse [PullRequestEvent]
dprersPullRequestEvents =
  lens _dprersPullRequestEvents (\s a -> s {_dprersPullRequestEvents = a}) .
  _Coerce
