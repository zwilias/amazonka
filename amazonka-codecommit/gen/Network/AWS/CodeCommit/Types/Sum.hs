...
instance FromText ChangeTypeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChangeTypeEnum from value: '" <>
        e <> "'. Accepted values: a, d, m"
...
instance FromText FileModeTypeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FileModeTypeEnum from value: '" <>
        e <> "'. Accepted values: executable, normal, symlink"
...
instance FromText MergeOptionTypeEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MergeOptionTypeEnum from value: '" <>
        e <> "'. Accepted values: fast_forward_merge"
...
instance FromText OrderEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrderEnum from value: '" <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      "pull_request_source_reference_updated" ->
        pure PullRequestSourceReferenceUpdated
      "pull_request_status_changed" -> pure PullRequestStatusChanged
      e ->
        fromTextError $
        "Failure parsing PullRequestEventType from value: '" <>
        e <>
...
instance FromText PullRequestStatusEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PullRequestStatusEnum from value: '" <>
...
instance FromText RelativeFileVersionEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RelativeFileVersionEnum from value: '" <>
        e <> "'. Accepted values: after, before"
...
instance FromText RepositoryTriggerEventEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RepositoryTriggerEventEnum from value: '" <>
        e <>
...
instance FromText SortByEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SortByEnum from value: '" <>
        e <> "'. Accepted values: lastmodifieddate, repositoryname"
