...
gdBeforeCommitSpecifier =
  lens _gdBeforeCommitSpecifier (\s a -> s {_gdBeforeCommitSpecifier = a})
...
gdAfterCommitSpecifier =
  lens _gdAfterCommitSpecifier (\s a -> s {_gdAfterCommitSpecifier = a})
...
instance AWSRequest GetDifferences where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDifferencesResponse' <$> (x .?> "NextToken") <*>
...
gdrsDifferences =
  lens _gdrsDifferences (\s a -> s {_gdrsDifferences = a}) . _Default . _Coerce
