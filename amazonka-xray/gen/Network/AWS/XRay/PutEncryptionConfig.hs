...
instance AWSRequest PutEncryptionConfig where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutEncryptionConfigResponse' <$> (x .?> "EncryptionConfig") <*>
         (pure (fromEnum s)))
...
instance ToJSON PutEncryptionConfig where
  toJSON PutEncryptionConfig' {..} =
    object (catMaybes [("KeyId" .=) <$> _pecKeyId, Just ("Type" .= _pecType)])
...
pecrsEncryptionConfig ::
     Lens' PutEncryptionConfigResponse (Maybe EncryptionConfig)
pecrsEncryptionConfig =
  lens _pecrsEncryptionConfig (\s a -> s {_pecrsEncryptionConfig = a})

-- | -- | The response status code.
pecrsResponseStatus :: Lens' PutEncryptionConfigResponse Int
pecrsResponseStatus =
  lens _pecrsResponseStatus (\s a -> s {_pecrsResponseStatus = a})
