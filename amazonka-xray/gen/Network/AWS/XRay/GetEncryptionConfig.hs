...
instance AWSRequest GetEncryptionConfig where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetEncryptionConfigResponse' <$> (x .?> "EncryptionConfig") <*>
         (pure (fromEnum s)))
...
gecrsEncryptionConfig ::
     Lens' GetEncryptionConfigResponse (Maybe EncryptionConfig)
gecrsEncryptionConfig =
  lens _gecrsEncryptionConfig (\s a -> s {_gecrsEncryptionConfig = a})

-- | -- | The response status code.
gecrsResponseStatus :: Lens' GetEncryptionConfigResponse Int
gecrsResponseStatus =
  lens _gecrsResponseStatus (\s a -> s {_gecrsResponseStatus = a})
