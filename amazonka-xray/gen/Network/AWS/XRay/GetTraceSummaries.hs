...
gtsFilterExpression =
  lens _gtsFilterExpression (\s a -> s {_gtsFilterExpression = a})
...
instance AWSRequest GetTraceSummaries where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetTraceSummariesResponse' <$> (x .?> "TracesProcessedCount") <*>
         (x .?> "NextToken") <*>
         (x .?> "ApproximateTime") <*>
...
gtsrsTracesProcessedCount =
  lens _gtsrsTracesProcessedCount (\s a -> s {_gtsrsTracesProcessedCount = a})
...
gtsrsApproximateTime =
  lens _gtsrsApproximateTime (\s a -> s {_gtsrsApproximateTime = a}) .
  mapping _Time

-- | Trace IDs and metadata for traces that were found in the specified time frame.
gtsrsTraceSummaries :: Lens' GetTraceSummariesResponse [TraceSummary]
gtsrsTraceSummaries =
  lens _gtsrsTraceSummaries (\s a -> s {_gtsrsTraceSummaries = a}) .
  _Default . _Coerce
