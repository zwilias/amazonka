...
instance AWSRequest BatchGetTraces where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchGetTracesResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Traces" .!@ mempty) <*>
...
instance ToJSON BatchGetTraces where
  toJSON BatchGetTraces' {..} =
    object
      (catMaybes
         [("NextToken" .=) <$> _bgtNextToken, Just ("TraceIds" .= _bgtTraceIds)])
...
bgtrsTraces =
  lens _bgtrsTraces (\s a -> s {_bgtrsTraces = a}) . _Default . _Coerce

-- | Trace IDs of requests that haven't been processed.
bgtrsUnprocessedTraceIds :: Lens' BatchGetTracesResponse [Text]
bgtrsUnprocessedTraceIds =
  lens _bgtrsUnprocessedTraceIds (\s a -> s {_bgtrsUnprocessedTraceIds = a}) .
  _Default . _Coerce

-- | -- | The response status code.
bgtrsResponseStatus :: Lens' BatchGetTracesResponse Int
bgtrsResponseStatus =
  lens _bgtrsResponseStatus (\s a -> s {_bgtrsResponseStatus = a})
