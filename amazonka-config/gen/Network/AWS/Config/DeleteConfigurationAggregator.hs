...
dcaConfigurationAggregatorName =
  lens
    ...
    (\s a -> s {_dcaConfigurationAggregatorName = a})

instance AWSRequest DeleteConfigurationAggregator where
  type Rs DeleteConfigurationAggregator = DeleteConfigurationAggregatorResponse
...
instance ToHeaders DeleteConfigurationAggregator where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StarlingDoveService.DeleteConfigurationAggregator" :: ByteString)
...
instance ToJSON DeleteConfigurationAggregator where
  toJSON DeleteConfigurationAggregator' {..} =
    object
      (catMaybes
         [ Just
             ("ConfigurationAggregatorName" .= _dcaConfigurationAggregatorName)
         ])
