...
paaAuthorizedAccountId =
  lens _paaAuthorizedAccountId (\s a -> s {_paaAuthorizedAccountId = a})

-- | The region authorized to collect aggregated data.
paaAuthorizedAWSRegion :: Lens' PutAggregationAuthorization Text
paaAuthorizedAWSRegion =
  lens _paaAuthorizedAWSRegion (\s a -> s {_paaAuthorizedAWSRegion = a})

instance AWSRequest PutAggregationAuthorization where
  type Rs PutAggregationAuthorization = PutAggregationAuthorizationResponse
...
paarsAggregationAuthorization ::
     Lens' PutAggregationAuthorizationResponse (Maybe AggregationAuthorization)
paarsAggregationAuthorization =
  lens
    _paarsAggregationAuthorization
    (\s a -> s {_paarsAggregationAuthorization = a})

-- | -- | The response status code.
paarsResponseStatus :: Lens' PutAggregationAuthorizationResponse Int
paarsResponseStatus =
  lens _paarsResponseStatus (\s a -> s {_paarsResponseStatus = a})
