...
gcdbrComplianceTypes =
  lens _gcdbrComplianceTypes (\s a -> s {_gcdbrComplianceTypes = a}) .
  _Default . _Coerce
...
instance AWSRequest GetComplianceDetailsByResource where
  type Rs GetComplianceDetailsByResource = GetComplianceDetailsByResourceResponse
...
instance ToHeaders GetComplianceDetailsByResource where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StarlingDoveService.GetComplianceDetailsByResource" :: ByteString)
...
gcdbrrsEvaluationResults ::
     Lens' GetComplianceDetailsByResourceResponse [EvaluationResult]
gcdbrrsEvaluationResults =
  lens _gcdbrrsEvaluationResults (\s a -> s {_gcdbrrsEvaluationResults = a}) .
  _Default . _Coerce
...
gcdbrrsResponseStatus =
  lens _gcdbrrsResponseStatus (\s a -> s {_gcdbrrsResponseStatus = a})
