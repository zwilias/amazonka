...
gcdbcrComplianceTypes =
  lens _gcdbcrComplianceTypes (\s a -> s {_gcdbcrComplianceTypes = a}) .
  _Default . _Coerce
...
gcdbcrConfigRuleName =
  lens _gcdbcrConfigRuleName (\s a -> s {_gcdbcrConfigRuleName = a})
...
instance AWSPager GetComplianceDetailsByConfigRule where
  page rq rs
    ...
    | otherwise = Just $ rq & gcdbcrNextToken .~ rs ^. gcdbcrrsNextToken

instance AWSRequest GetComplianceDetailsByConfigRule where
  type Rs GetComplianceDetailsByConfigRule = GetComplianceDetailsByConfigRuleResponse
...
instance ToHeaders GetComplianceDetailsByConfigRule where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StarlingDoveService.GetComplianceDetailsByConfigRule" :: ByteString)
...
gcdbcrrsEvaluationResults ::
     Lens' GetComplianceDetailsByConfigRuleResponse [EvaluationResult]
gcdbcrrsEvaluationResults =
  lens _gcdbcrrsEvaluationResults (\s a -> s {_gcdbcrrsEvaluationResults = a}) .
  _Default . _Coerce
...
gcdbcrrsResponseStatus =
  lens _gcdbcrrsResponseStatus (\s a -> s {_gcdbcrrsResponseStatus = a})
