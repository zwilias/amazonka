...
daaAuthorizedAccountId =
  lens _daaAuthorizedAccountId (\s a -> s {_daaAuthorizedAccountId = a})

-- | The region authorized to collect aggregated data.
daaAuthorizedAWSRegion :: Lens' DeleteAggregationAuthorization Text
daaAuthorizedAWSRegion =
  lens _daaAuthorizedAWSRegion (\s a -> s {_daaAuthorizedAWSRegion = a})

instance AWSRequest DeleteAggregationAuthorization where
  type Rs DeleteAggregationAuthorization = DeleteAggregationAuthorizationResponse
...
instance ToHeaders DeleteAggregationAuthorization where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("StarlingDoveService.DeleteAggregationAuthorization" :: ByteString)
