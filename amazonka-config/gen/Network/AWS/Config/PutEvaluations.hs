...
peEvaluations =
  lens _peEvaluations (\s a -> s {_peEvaluations = a}) . _Default . _Coerce
...
instance AWSRequest PutEvaluations where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutEvaluationsResponse' <$> (x .?> "FailedEvaluations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
persFailedEvaluations =
  lens _persFailedEvaluations (\s a -> s {_persFailedEvaluations = a}) .
  _Default . _Coerce
