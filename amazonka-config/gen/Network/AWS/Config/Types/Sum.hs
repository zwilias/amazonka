...
instance FromText AggregatedSourceStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AggregatedSourceStatusType from value: '" <>
        e <> "'. Accepted values: failed, outdated, succeeded"
...
instance FromText AggregatedSourceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AggregatedSourceType from value: '" <>
        e <> "'. Accepted values: account, organization"
...
instance FromText ChronologicalOrder where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChronologicalOrder from value: '" <>
        e <> "'. Accepted values: forward, reverse"
...
instance FromText ComplianceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ComplianceType from value: '" <>
        e <>
...
instance FromText ConfigRuleComplianceSummaryGroupKey where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConfigRuleComplianceSummaryGroupKey from value: '" <>
        e <> "'. Accepted values: aws_region, account_id"
...
instance FromText ConfigRuleState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConfigRuleState from value: '" <>
        e <>
...
instance FromText ConfigurationItemStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConfigurationItemStatus from value: '" <>
        e <>
...
instance FromText DeliveryStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeliveryStatus from value: '" <>
        e <> "'. Accepted values: failure, not_applicable, success"
...
instance FromText EventSource where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EventSource from value: '" <>
        e <> "'. Accepted values: aws.config"
...
instance FromText MaximumExecutionFrequency where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MaximumExecutionFrequency from value: '" <>
        e <>
...
instance FromText MessageType where
  parser =
    takeLowerText >>= \case
      "configurationitemchangenotification" ->
        pure ConfigurationItemChangeNotification
      "configurationsnapshotdeliverycompleted" ->
        pure ConfigurationSnapshotDeliveryCompleted
      "oversizedconfigurationitemchangenotification" ->
        pure OversizedConfigurationItemChangeNotification
      "schedulednotification" -> pure ScheduledNotification
      e ->
        fromTextError $
        "Failure parsing MessageType from value: '" <>
        e <>
...
instance FromText Owner where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Owner from value: '" <>
        e <> "'. Accepted values: aws, custom_lambda"
...
instance FromText RecorderStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RecorderStatus from value: '" <>
        e <> "'. Accepted values: failure, pending, success"
  ...
  parser =
    takeLowerText >>= \case
      ...
      "aws::autoscaling::autoscalinggroup" ->
        pure AWSAutoScalingAutoScalingGroup
      "aws::autoscaling::launchconfiguration" ->
        pure AWSAutoScalingLaunchConfiguration
      ...
      "aws::cloudfront::streamingdistribution" ->
        pure AWSCloudFrontStreamingDistribution
      ...
      "aws::elasticloadbalancingv2::loadbalancer" ->
        pure AWSELASTICLOADBALANCINGV2LoadBalancer
      "aws::elasticbeanstalk::application" ->
        pure AWSElasticBeanstalkApplication
      "aws::elasticbeanstalk::applicationversion" ->
        pure AWSElasticBeanstalkApplicationVersion
      "aws::elasticbeanstalk::environment" ->
        pure AWSElasticBeanstalkEnvironment
      "aws::elasticloadbalancing::loadbalancer" ->
        pure AWSElasticLoadBalancingLoadBalancer
      ...
      "aws::redshift::clusterparametergroup" ->
        pure AWSRedshiftClusterParameterGroup
      "aws::redshift::clustersecuritygroup" ->
        pure AWSRedshiftClusterSecurityGroup
      ...
      "aws::ssm::managedinstanceinventory" ->
        pure AWSSsmManagedInstanceInventory
