...
grchChronologicalOrder ::
     Lens' GetResourceConfigHistory (Maybe ChronologicalOrder)
grchChronologicalOrder =
...
grchLaterTime =
  lens _grchLaterTime (\s a -> s {_grchLaterTime = a}) . mapping _Time

-- | The time stamp that indicates an earlier time. If not specified, the action returns paginated results that contain configuration items that start when the first configuration item was recorded.
grchEarlierTime :: Lens' GetResourceConfigHistory (Maybe UTCTime)
grchEarlierTime =
  lens _grchEarlierTime (\s a -> s {_grchEarlierTime = a}) . mapping _Time
...
instance AWSRequest GetResourceConfigHistory where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetResourceConfigHistoryResponse' <$> (x .?> "nextToken") <*>
...
grchrsConfigurationItems ::
     Lens' GetResourceConfigHistoryResponse [ConfigurationItem]
grchrsConfigurationItems =
  lens _grchrsConfigurationItems (\s a -> s {_grchrsConfigurationItems = a}) .
  _Default . _Coerce

-- | -- | The response status code.
grchrsResponseStatus :: Lens' GetResourceConfigHistoryResponse Int
grchrsResponseStatus =
  lens _grchrsResponseStatus (\s a -> s {_grchrsResponseStatus = a})
