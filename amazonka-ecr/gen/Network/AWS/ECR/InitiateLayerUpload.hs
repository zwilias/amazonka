...
instance AWSRequest InitiateLayerUpload where
  ...
  response =
    receiveJSON
      (\s h x ->
         InitiateLayerUploadResponse' <$> (x .?> "partSize") <*>
         (x .?> "uploadId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders InitiateLayerUpload where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.InitiateLayerUpload" :: ByteString)
...
ilursPartSize =
  lens _ilursPartSize (\s a -> s {_ilursPartSize = a}) . mapping _Nat
...
ilursResponseStatus =
  lens _ilursResponseStatus (\s a -> s {_ilursResponseStatus = a})
