...
drRepositoryNames =
  lens _drRepositoryNames (\s a -> s {_drRepositoryNames = a}) . mapping _List1
...
instance AWSRequest DescribeRepositories where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeRepositoriesResponse' <$> (x .?> "repositories" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeRepositories where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.DescribeRepositories" :: ByteString)
...
drrsRepositories =
  lens _drrsRepositories (\s a -> s {_drrsRepositories = a}) .
  _Default . _Coerce
