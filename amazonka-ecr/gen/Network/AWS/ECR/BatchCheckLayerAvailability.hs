...
bclaRepositoryName =
  lens _bclaRepositoryName (\s a -> s {_bclaRepositoryName = a})

-- | The digests of the image layers to check.
bclaLayerDigests :: Lens' BatchCheckLayerAvailability (NonEmpty Text)
bclaLayerDigests =
  lens _bclaLayerDigests (\s a -> s {_bclaLayerDigests = a}) . _List1

instance AWSRequest BatchCheckLayerAvailability where
  type Rs BatchCheckLayerAvailability = BatchCheckLayerAvailabilityResponse
  request = postJSON ecr
  response =
    receiveJSON
      (\s h x ->
         BatchCheckLayerAvailabilityResponse' <$> (x .?> "failures" .!@ mempty) <*>
         (x .?> "layers" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders BatchCheckLayerAvailability where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.BatchCheckLayerAvailability" :: ByteString)
...
bclarsFailures =
  lens _bclarsFailures (\s a -> s {_bclarsFailures = a}) . _Default . _Coerce

-- | A list of image layer objects corresponding to the image layer references in the request.
bclarsLayers :: Lens' BatchCheckLayerAvailabilityResponse [Layer]
bclarsLayers =
  lens _bclarsLayers (\s a -> s {_bclarsLayers = a}) . _Default . _Coerce

-- | -- | The response status code.
bclarsResponseStatus :: Lens' BatchCheckLayerAvailabilityResponse Int
bclarsResponseStatus =
  lens _bclarsResponseStatus (\s a -> s {_bclarsResponseStatus = a})
