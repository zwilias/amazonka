...
instance AWSRequest GetLifecyclePolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetLifecyclePolicyResponse' <$> (x .?> "registryId") <*>
         (x .?> "lastEvaluatedAt") <*>
...
instance ToHeaders GetLifecyclePolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.GetLifecyclePolicy" :: ByteString)
...
glprsLastEvaluatedAt =
  lens _glprsLastEvaluatedAt (\s a -> s {_glprsLastEvaluatedAt = a}) .
  mapping _Time

-- | The JSON lifecycle policy text.
glprsLifecyclePolicyText :: Lens' GetLifecyclePolicyResponse (Maybe Text)
glprsLifecyclePolicyText =
  lens _glprsLifecyclePolicyText (\s a -> s {_glprsLifecyclePolicyText = a})

-- | The repository name associated with the request.
glprsRepositoryName :: Lens' GetLifecyclePolicyResponse (Maybe Text)
glprsRepositoryName =
  lens _glprsRepositoryName (\s a -> s {_glprsRepositoryName = a})

-- | -- | The response status code.
glprsResponseStatus :: Lens' GetLifecyclePolicyResponse Int
glprsResponseStatus =
  lens _glprsResponseStatus (\s a -> s {_glprsResponseStatus = a})
