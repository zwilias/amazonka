...
ulpPartFirstByte =
  lens _ulpPartFirstByte (\s a -> s {_ulpPartFirstByte = a}) . _Nat

-- | The integer value of the last byte of the layer part.
ulpPartLastByte :: Lens' UploadLayerPart Natural
ulpPartLastByte =
  lens _ulpPartLastByte (\s a -> s {_ulpPartLastByte = a}) . _Nat
...
ulpLayerPartBlob =
  lens _ulpLayerPartBlob (\s a -> s {_ulpLayerPartBlob = a}) . _Base64
...
instance AWSRequest UploadLayerPart where
  ...
  response =
    receiveJSON
      (\s h x ->
         UploadLayerPartResponse' <$> (x .?> "registryId") <*>
         (x .?> "lastByteReceived") <*>
         (x .?> "repositoryName") <*>
         (x .?> "uploadId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UploadLayerPart where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.UploadLayerPart" :: ByteString)
...
ulprsLastByteReceived =
  lens _ulprsLastByteReceived (\s a -> s {_ulprsLastByteReceived = a}) .
  mapping _Nat

-- | The repository name associated with the request.
ulprsRepositoryName :: Lens' UploadLayerPartResponse (Maybe Text)
ulprsRepositoryName =
  lens _ulprsRepositoryName (\s a -> s {_ulprsRepositoryName = a})
...
ulprsResponseStatus =
  lens _ulprsResponseStatus (\s a -> s {_ulprsResponseStatus = a})
