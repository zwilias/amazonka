...
instance AWSRequest DeleteRepositoryPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteRepositoryPolicyResponse' <$> (x .?> "registryId") <*>
         (x .?> "repositoryName") <*>
         (x .?> "policyText") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DeleteRepositoryPolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.DeleteRepositoryPolicy" :: ByteString)
...
drprsRepositoryName =
  lens _drprsRepositoryName (\s a -> s {_drprsRepositoryName = a})
