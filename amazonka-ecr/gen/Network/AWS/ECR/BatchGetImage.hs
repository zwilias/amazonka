...
bgiAcceptedMediaTypes =
  lens _bgiAcceptedMediaTypes (\s a -> s {_bgiAcceptedMediaTypes = a}) .
  mapping _List1
...
instance AWSRequest BatchGetImage where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchGetImageResponse' <$> (x .?> "images" .!@ mempty) <*>
         (x .?> "failures" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders BatchGetImage where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.BatchGetImage" :: ByteString)
...
bgirsImages =
  lens _bgirsImages (\s a -> s {_bgirsImages = a}) . _Default . _Coerce

-- | Any failures associated with the call.
bgirsFailures :: Lens' BatchGetImageResponse [ImageFailure]
bgirsFailures =
  lens _bgirsFailures (\s a -> s {_bgirsFailures = a}) . _Default . _Coerce
