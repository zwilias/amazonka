...
data ImageScanStatus = ImageScanStatus'{_issStatus ::
                                        !(Maybe ScanStatus),
...
imageScanStatus
    :: ImageScanStatus
imageScanStatus
  = ImageScanStatus'{_issStatus = Nothing,
...
instance FromJSON ImageScanStatus where
        parseJSON
          = withObject "ImageScanStatus"
              (\ x ->
                 ImageScanStatus' <$>
                   (x .:? "status") <*> (x .:? "description"))

instance Hashable ImageScanStatus where

instance NFData ImageScanStatus where
