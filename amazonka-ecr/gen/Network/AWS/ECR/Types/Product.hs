...
adAuthorizationToken =
  lens _adAuthorizationToken (\s a -> s {_adAuthorizationToken = a})
...
instance FromJSON AuthorizationData where
  parseJSON =
    withObject
      "AuthorizationData"
      (\x ->
         AuthorizationData' <$> (x .:? "expiresAt") <*> (x .:? "proxyEndpoint") <*>
...
idImageTags =
  lens _idImageTags (\s a -> s {_idImageTags = a}) . _Default . _Coerce

-- | The size, in bytes, of the image in the repository.
idImageSizeInBytes :: Lens' ImageDetail (Maybe Integer)
idImageSizeInBytes =
  lens _idImageSizeInBytes (\s a -> s {_idImageSizeInBytes = a})
...
idImagePushedAt =
  lens _idImagePushedAt (\s a -> s {_idImagePushedAt = a}) . mapping _Time
...
instance FromJSON ImageFailure where
  parseJSON =
    withObject
      "ImageFailure"
      (\x ->
         ImageFailure' <$> (x .:? "failureReason") <*> (x .:? "failureCode") <*>
         (x .:? "imageId"))
...
instance FromJSON ImageIdentifier where
  parseJSON =
    withObject
      "ImageIdentifier"
      (\x -> ImageIdentifier' <$> (x .:? "imageDigest") <*> (x .:? "imageTag"))
...
lLayerAvailability =
  lens _lLayerAvailability (\s a -> s {_lLayerAvailability = a})
...
instance FromJSON Layer where
  parseJSON =
    withObject
      "Layer"
      (\x ->
         Layer' <$> (x .:? "mediaType") <*> (x .:? "layerDigest") <*>
         (x .:? "layerSize") <*>
         (x .:? "layerAvailability"))
...
instance FromJSON LayerFailure where
  parseJSON =
    withObject
      "LayerFailure"
      (\x ->
         LayerFailure' <$> (x .:? "failureReason") <*> (x .:? "failureCode") <*>
         (x .:? "layerDigest"))
...
lpprImageTags =
  lens _lpprImageTags (\s a -> s {_lpprImageTags = a}) . _Default . _Coerce

-- | The type of action to be taken.
lpprAction ::
     Lens' LifecyclePolicyPreviewResult (Maybe LifecyclePolicyRuleAction)
...
lpprImagePushedAt =
  lens _lpprImagePushedAt (\s a -> s {_lpprImagePushedAt = a}) . mapping _Time

-- | The priority of the applied rule.
lpprAppliedRulePriority :: Lens' LifecyclePolicyPreviewResult (Maybe Natural)
lpprAppliedRulePriority =
  lens _lpprAppliedRulePriority (\s a -> s {_lpprAppliedRulePriority = a}) .
  mapping _Nat
...
instance FromJSON LifecyclePolicyPreviewResult where
  parseJSON =
    withObject
      "LifecyclePolicyPreviewResult"
      (\x ->
         LifecyclePolicyPreviewResult' <$> (x .:? "imageTags" .!= mempty) <*>
         (x .:? "action") <*>
         (x .:? "imageDigest") <*>
         (x .:? "imagePushedAt") <*>
...
lppsExpiringImageTotalCount ::
     Lens' LifecyclePolicyPreviewSummary (Maybe Natural)
lppsExpiringImageTotalCount =
  lens
    _lppsExpiringImageTotalCount
    (\s a -> s {_lppsExpiringImageTotalCount = a}) .
  mapping _Nat
...
instance FromJSON LifecyclePolicyPreviewSummary where
  parseJSON =
    withObject
      "LifecyclePolicyPreviewSummary"
      (\x ->
         LifecyclePolicyPreviewSummary' <$> (x .:? "expiringImageTotalCount"))
...
instance FromJSON Repository where
  parseJSON =
    withObject
      "Repository"
      (\x ->
         Repository' <$> (x .:? "repositoryArn") <*> (x .:? "createdAt") <*>
         (x .:? "registryId") <*>
         (x .:? "repositoryUri") <*>
         (x .:? "repositoryName"))
