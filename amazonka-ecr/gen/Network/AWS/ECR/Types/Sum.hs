...
instance FromText ImageActionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ImageActionType from value: '" <>
        e <> "'. Accepted values: expire"
...
instance FromText ImageFailureCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ImageFailureCode from value: '" <>
        e <>
...
instance FromText LayerAvailability where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LayerAvailability from value: '" <>
        e <> "'. Accepted values: available, unavailable"
...
instance FromText LayerFailureCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LayerFailureCode from value: '" <>
        e <> "'. Accepted values: invalidlayerdigest, missinglayerdigest"
...
instance FromText LifecyclePolicyPreviewStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LifecyclePolicyPreviewStatus from value: '" <>
        e <> "'. Accepted values: complete, expired, failed, in_progress"
...
instance FromText TagStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TagStatus from value: '" <>
        e <> "'. Accepted values: tagged, untagged"
