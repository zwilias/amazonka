...
instance AWSRequest GetRepositoryPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetRepositoryPolicyResponse' <$> (x .?> "registryId") <*>
         (x .?> "repositoryName") <*>
         (x .?> "policyText") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetRepositoryPolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.GetRepositoryPolicy" :: ByteString)
...
grprsRepositoryName =
  lens _grprsRepositoryName (\s a -> s {_grprsRepositoryName = a})
