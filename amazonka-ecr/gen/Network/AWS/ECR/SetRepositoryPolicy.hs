...
instance AWSRequest SetRepositoryPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         SetRepositoryPolicyResponse' <$> (x .?> "registryId") <*>
         (x .?> "repositoryName") <*>
         (x .?> "policyText") <*>
         (pure (fromEnum s)))
...
instance ToHeaders SetRepositoryPolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.SetRepositoryPolicy" :: ByteString)
...
srprsRepositoryName =
  lens _srprsRepositoryName (\s a -> s {_srprsRepositoryName = a})
...
srprsResponseStatus =
  lens _srprsResponseStatus (\s a -> s {_srprsResponseStatus = a})
