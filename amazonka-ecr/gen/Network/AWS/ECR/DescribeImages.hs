...
instance AWSRequest DescribeImages where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeImagesResponse' <$> (x .?> "imageDetails" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeImages where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.DescribeImages" :: ByteString)
...
dirsImageDetails =
  lens _dirsImageDetails (\s a -> s {_dirsImageDetails = a}) .
  _Default . _Coerce
