...
instance AWSRequest DeleteLifecyclePolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteLifecyclePolicyResponse' <$> (x .?> "registryId") <*>
         (x .?> "lastEvaluatedAt") <*>
...
instance ToHeaders DeleteLifecyclePolicy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonEC2ContainerRegistry_V20150921.DeleteLifecyclePolicy" :: ByteString)
...
dlprsLastEvaluatedAt =
  lens _dlprsLastEvaluatedAt (\s a -> s {_dlprsLastEvaluatedAt = a}) .
  mapping _Time

-- | The JSON lifecycle policy text.
dlprsLifecyclePolicyText :: Lens' DeleteLifecyclePolicyResponse (Maybe Text)
dlprsLifecyclePolicyText =
  lens _dlprsLifecyclePolicyText (\s a -> s {_dlprsLifecyclePolicyText = a})

-- | The repository name associated with the request.
dlprsRepositoryName :: Lens' DeleteLifecyclePolicyResponse (Maybe Text)
dlprsRepositoryName =
  lens _dlprsRepositoryName (\s a -> s {_dlprsRepositoryName = a})

-- | -- | The response status code.
dlprsResponseStatus :: Lens' DeleteLifecyclePolicyResponse Int
dlprsResponseStatus =
  lens _dlprsResponseStatus (\s a -> s {_dlprsResponseStatus = a})
