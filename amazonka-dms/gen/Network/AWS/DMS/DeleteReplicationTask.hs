...
drtReplicationTaskARN =
  lens _drtReplicationTaskARN (\s a -> s {_drtReplicationTaskARN = a})
...
instance AWSRequest DeleteReplicationTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteReplicationTaskResponse' <$> (x .?> "ReplicationTask") <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteReplicationTask where
  toJSON DeleteReplicationTask' {..} =
    object (catMaybes [Just ("ReplicationTaskArn" .= _drtReplicationTaskARN)])
...
drtrsReplicationTask ::
     Lens' DeleteReplicationTaskResponse (Maybe ReplicationTask)
drtrsReplicationTask =
  lens _drtrsReplicationTask (\s a -> s {_drtrsReplicationTask = a})

-- | -- | The response status code.
drtrsResponseStatus :: Lens' DeleteReplicationTaskResponse Int
drtrsResponseStatus =
  lens _drtrsResponseStatus (\s a -> s {_drtrsResponseStatus = a})
