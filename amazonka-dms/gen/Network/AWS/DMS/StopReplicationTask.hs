...
sReplicationTaskARN =
  lens _sReplicationTaskARN (\s a -> s {_sReplicationTaskARN = a})
...
instance AWSRequest StopReplicationTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         StopReplicationTaskResponse' <$> (x .?> "ReplicationTask") <*>
         (pure (fromEnum s)))
...
instance ToJSON StopReplicationTask where
  toJSON StopReplicationTask' {..} =
    object (catMaybes [Just ("ReplicationTaskArn" .= _sReplicationTaskARN)])
...
srsReplicationTask =
  lens _srsReplicationTask (\s a -> s {_srsReplicationTask = a})
