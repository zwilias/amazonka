...
drsgFilters =
  lens _drsgFilters (\s a -> s {_drsgFilters = a}) . _Default . _Coerce
...
instance AWSRequest DescribeReplicationSubnetGroups where
  type Rs DescribeReplicationSubnetGroups = DescribeReplicationSubnetGroupsResponse
  request = postJSON dms
  response =
    receiveJSON
      (\s h x ->
         DescribeReplicationSubnetGroupsResponse' <$> (x .?> "Marker") <*>
...
instance ToHeaders DescribeReplicationSubnetGroups where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AmazonDMSv20160101.DescribeReplicationSubnetGroups" :: ByteString)
...
drsgsrsReplicationSubnetGroups ::
     Lens' DescribeReplicationSubnetGroupsResponse [ReplicationSubnetGroup]
drsgsrsReplicationSubnetGroups =
  lens
    _drsgsrsReplicationSubnetGroups
    (\s a -> s {_drsgsrsReplicationSubnetGroups = a}) .
  _Default . _Coerce

-- | -- | The response status code.
drsgsrsResponseStatus :: Lens' DescribeReplicationSubnetGroupsResponse Int
drsgsrsResponseStatus =
  lens _drsgsrsResponseStatus (\s a -> s {_drsgsrsResponseStatus = a})
