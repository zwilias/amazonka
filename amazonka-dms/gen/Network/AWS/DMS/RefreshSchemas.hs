...
rsReplicationInstanceARN =
  lens _rsReplicationInstanceARN (\s a -> s {_rsReplicationInstanceARN = a})
...
instance AWSRequest RefreshSchemas where
  ...
  response =
    receiveJSON
      (\s h x ->
         RefreshSchemasResponse' <$> (x .?> "RefreshSchemasStatus") <*>
         (pure (fromEnum s)))
...
instance ToHeaders RefreshSchemas where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDMSv20160101.RefreshSchemas" :: ByteString)
...
rsrsRefreshSchemasStatus ::
     Lens' RefreshSchemasResponse (Maybe RefreshSchemasStatus)
rsrsRefreshSchemasStatus =
  lens _rsrsRefreshSchemasStatus (\s a -> s {_rsrsRefreshSchemasStatus = a})

-- | -- | The response status code.
rsrsResponseStatus :: Lens' RefreshSchemasResponse Int
rsrsResponseStatus =
  lens _rsrsResponseStatus (\s a -> s {_rsrsResponseStatus = a})
