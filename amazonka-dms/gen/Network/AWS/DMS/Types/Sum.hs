...
instance FromText AuthMechanismValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuthMechanismValue from value: '" <>
        e <> "'. Accepted values: default, mongodb_cr, scram_sha_1"
...
instance FromText AuthTypeValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuthTypeValue from value: '" <>
        e <> "'. Accepted values: no, password"
...
instance FromText CompressionTypeValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CompressionTypeValue from value: '" <>
...
instance FromText DmsSSLModeValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DmsSSLModeValue from value: '" <>
        e <> "'. Accepted values: none, require, verify-ca, verify-full"
...
instance FromText MigrationTypeValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MigrationTypeValue from value: '" <>
        e <> "'. Accepted values: cdc, full-load, full-load-and-cdc"
...
instance FromText NestingLevelValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NestingLevelValue from value: '" <>
        e <> "'. Accepted values: none, one"
...
instance FromText RefreshSchemasStatusTypeValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RefreshSchemasStatusTypeValue from value: '" <>
        e <> "'. Accepted values: failed, refreshing, successful"
...
instance FromText ReplicationEndpointTypeValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReplicationEndpointTypeValue from value: '" <>
        e <> "'. Accepted values: source, target"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: replication-instance"
...
instance FromText StartReplicationTaskTypeValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StartReplicationTaskTypeValue from value: '" <>
        e <>
