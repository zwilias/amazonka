...
tcReplicationInstanceARN =
  lens _tcReplicationInstanceARN (\s a -> s {_tcReplicationInstanceARN = a})
...
instance AWSRequest TestConnection where
  ...
  response =
    receiveJSON
      (\s h x ->
         TestConnectionResponse' <$> (x .?> "Connection") <*>
         (pure (fromEnum s)))
...
instance ToHeaders TestConnection where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDMSv20160101.TestConnection" :: ByteString)
