...
instance AWSRequest ModifyEventSubscription where
  ...
  response =
    receiveJSON
      (\s h x ->
         ModifyEventSubscriptionResponse' <$> (x .?> "EventSubscription") <*>
         (pure (fromEnum s)))
...
mesrsEventSubscription ::
     Lens' ModifyEventSubscriptionResponse (Maybe EventSubscription)
mesrsEventSubscription =
...
mesrsResponseStatus =
  lens _mesrsResponseStatus (\s a -> s {_mesrsResponseStatus = a})
