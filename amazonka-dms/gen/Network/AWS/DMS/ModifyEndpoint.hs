...
meServiceAccessRoleARN =
  lens _meServiceAccessRoleARN (\s a -> s {_meServiceAccessRoleARN = a})
...
meExtraConnectionAttributes =
  lens
    ...
    (\s a -> s {_meExtraConnectionAttributes = a})
...
meExternalTableDefinition =
  lens _meExternalTableDefinition (\s a -> s {_meExternalTableDefinition = a})
...
meEndpointIdentifier =
  lens _meEndpointIdentifier (\s a -> s {_meEndpointIdentifier = a})
...
meDynamoDBSettings =
  lens _meDynamoDBSettings (\s a -> s {_meDynamoDBSettings = a})
...
instance AWSRequest ModifyEndpoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         ModifyEndpointResponse' <$> (x .?> "Endpoint") <*> (pure (fromEnum s)))
...
instance ToHeaders ModifyEndpoint where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDMSv20160101.ModifyEndpoint" :: ByteString)
...
mersResponseStatus =
  lens _mersResponseStatus (\s a -> s {_mersResponseStatus = a})
