...
dtsReplicationTaskARN =
  lens _dtsReplicationTaskARN (\s a -> s {_dtsReplicationTaskARN = a})
...
instance AWSRequest DescribeTableStatistics where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeTableStatisticsResponse' <$> (x .?> "ReplicationTaskArn") <*>
         (x .?> "Marker") <*>
...
dtsrsReplicationTaskARN =
  lens _dtsrsReplicationTaskARN (\s a -> s {_dtsrsReplicationTaskARN = a})
...
dtsrsTableStatistics =
  lens _dtsrsTableStatistics (\s a -> s {_dtsrsTableStatistics = a}) .
  _Default . _Coerce
