...
ceServiceAccessRoleARN =
  lens _ceServiceAccessRoleARN (\s a -> s {_ceServiceAccessRoleARN = a})
...
ceExtraConnectionAttributes =
  lens
    ...
    (\s a -> s {_ceExtraConnectionAttributes = a})
...
ceExternalTableDefinition =
  lens _ceExternalTableDefinition (\s a -> s {_ceExternalTableDefinition = a})
...
ceDynamoDBSettings =
  lens _ceDynamoDBSettings (\s a -> s {_ceDynamoDBSettings = a})
...
ceEndpointIdentifier =
  lens _ceEndpointIdentifier (\s a -> s {_ceEndpointIdentifier = a})
...
instance AWSRequest CreateEndpoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateEndpointResponse' <$> (x .?> "Endpoint") <*> (pure (fromEnum s)))
...
instance ToHeaders CreateEndpoint where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDMSv20160101.CreateEndpoint" :: ByteString)
...
cersResponseStatus =
  lens _cersResponseStatus (\s a -> s {_cersResponseStatus = a})
