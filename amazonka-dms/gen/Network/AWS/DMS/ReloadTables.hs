...
rtReplicationTaskARN =
  lens _rtReplicationTaskARN (\s a -> s {_rtReplicationTaskARN = a})

-- | The name and schema of the table to be reloaded.
rtTablesToReload :: Lens' ReloadTables [TableToReload]
rtTablesToReload =
  lens _rtTablesToReload (\s a -> s {_rtTablesToReload = a}) . _Coerce
...
instance AWSRequest ReloadTables where
  ...
  response =
    receiveJSON
      (\s h x ->
         ReloadTablesResponse' <$> (x .?> "ReplicationTaskArn") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ReloadTables where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonDMSv20160101.ReloadTables" :: ByteString)
...
rtrsReplicationTaskARN =
  lens _rtrsReplicationTaskARN (\s a -> s {_rtrsReplicationTaskARN = a})
