...
instance AWSRequest DescribeReplicationInstances where
  type Rs DescribeReplicationInstances = DescribeReplicationInstancesResponse
  request = postJSON dms
  response =
    receiveJSON
      (\s h x ->
         DescribeReplicationInstancesResponse' <$> (x .?> "Marker") <*>
...
drisrsReplicationInstances =
  lens _drisrsReplicationInstances (\s a -> s {_drisrsReplicationInstances = a}) .
  _Default . _Coerce

-- | -- | The response status code.
drisrsResponseStatus :: Lens' DescribeReplicationInstancesResponse Int
drisrsResponseStatus =
  lens _drisrsResponseStatus (\s a -> s {_drisrsResponseStatus = a})
