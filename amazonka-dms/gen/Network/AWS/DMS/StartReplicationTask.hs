...
srtCdcStartPosition =
  lens _srtCdcStartPosition (\s a -> s {_srtCdcStartPosition = a})
...
srtCdcStopPosition =
  lens _srtCdcStopPosition (\s a -> s {_srtCdcStopPosition = a})

-- | Indicates the start time for a change data capture (CDC) operation. Use either CdcStartTime or CdcStartPosition to specify when you want a CDC operation to start. Specifying both values results in an error.
srtCdcStartTime :: Lens' StartReplicationTask (Maybe UTCTime)
srtCdcStartTime =
  lens _srtCdcStartTime (\s a -> s {_srtCdcStartTime = a}) . mapping _Time

-- | The Amazon Resource Name (ARN) of the replication task to be started.
srtReplicationTaskARN :: Lens' StartReplicationTask Text
srtReplicationTaskARN =
  lens _srtReplicationTaskARN (\s a -> s {_srtReplicationTaskARN = a})

-- | The type of replication task.
srtStartReplicationTaskType ::
     Lens' StartReplicationTask StartReplicationTaskTypeValue
srtStartReplicationTaskType =
  lens
    _srtStartReplicationTaskType
    (\s a -> s {_srtStartReplicationTaskType = a})
...
instance AWSRequest StartReplicationTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartReplicationTaskResponse' <$> (x .?> "ReplicationTask") <*>
         (pure (fromEnum s)))
...
srtrsReplicationTask ::
     Lens' StartReplicationTaskResponse (Maybe ReplicationTask)
srtrsReplicationTask =
  lens _srtrsReplicationTask (\s a -> s {_srtrsReplicationTask = a})

-- | -- | The response status code.
srtrsResponseStatus :: Lens' StartReplicationTaskResponse Int
srtrsResponseStatus =
  lens _srtrsResponseStatus (\s a -> s {_srtrsResponseStatus = a})
