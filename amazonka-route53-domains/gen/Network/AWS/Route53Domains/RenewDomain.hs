...
rdDurationInYears =
  lens _rdDurationInYears (\s a -> s {_rdDurationInYears = a}) . mapping _Nat
...
rdCurrentExpiryYear =
  lens _rdCurrentExpiryYear (\s a -> s {_rdCurrentExpiryYear = a})
...
instance AWSRequest RenewDomain where
  ...
  response =
    receiveJSON
      (\s h x ->
         RenewDomainResponse' <$> (pure (fromEnum s)) <*> (x .:> "OperationId"))
