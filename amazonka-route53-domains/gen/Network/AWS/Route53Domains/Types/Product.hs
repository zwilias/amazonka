...
instance FromJSON BillingRecord where
  parseJSON =
    withObject
      "BillingRecord"
      (\x ->
         BillingRecord' <$> (x .:? "Operation") <*> (x .:? "InvoiceId") <*>
         (x .:? "DomainName") <*>
         (x .:? "BillDate") <*>
         (x .:? "Price"))
...
cdExtraParams =
  lens _cdExtraParams (\s a -> s {_cdExtraParams = a}) . _Default . _Coerce
...
instance FromJSON ContactDetail where
  parseJSON =
    withObject
      "ContactDetail"
      (\x ->
         ContactDetail' <$> (x .:? "OrganizationName") <*> (x .:? "Email") <*>
         (x .:? "State") <*>
         (x .:? "Fax") <*>
         (x .:? "LastName") <*>
...
instance FromJSON DomainSuggestion where
  parseJSON =
    withObject
      "DomainSuggestion"
      (\x ->
         DomainSuggestion' <$> (x .:? "Availability") <*> (x .:? "DomainName"))
...
instance FromJSON DomainSummary where
  parseJSON =
    withObject
      "DomainSummary"
      (\x ->
         DomainSummary' <$> (x .:? "Expiry") <*> (x .:? "TransferLock") <*>
         (x .:? "AutoRenew") <*>
         (x .: "DomainName"))
...
instance ToJSON ExtraParam where
  toJSON ExtraParam' {..} =
    object (catMaybes [Just ("Name" .= _epName), Just ("Value" .= _epValue)])
...
instance FromJSON Nameserver where
  parseJSON =
    withObject
      "Nameserver"
      (\x -> Nameserver' <$> (x .:? "GlueIps" .!= mempty) <*> (x .: "Name"))
...
instance ToJSON Nameserver where
  toJSON Nameserver' {..} =
    object (catMaybes [("GlueIps" .=) <$> _nGlueIPs, Just ("Name" .= _nName)])
...
osSubmittedDate =
  lens _osSubmittedDate (\s a -> s {_osSubmittedDate = a}) . _Time
...
instance FromJSON OperationSummary where
  parseJSON =
    withObject
      "OperationSummary"
      (\x ->
         OperationSummary' <$> (x .: "OperationId") <*> (x .: "Status") <*>
         (x .: "Type") <*>
         (x .: "SubmittedDate"))
