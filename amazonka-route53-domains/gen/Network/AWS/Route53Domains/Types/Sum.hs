...
instance FromText ContactType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ContactType from value: '" <>
        e <>
...
instance FromText CountryCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CountryCode from value: '" <>
        e <>
...
instance FromText DomainAvailability where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DomainAvailability from value: '" <>
        e <>
...
instance FromText ExtraParamName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExtraParamName from value: '" <>
        e <>
...
instance FromText ReachabilityStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReachabilityStatus from value: '" <>
        e <> "'. Accepted values: done, expired, pending"
...
instance FromText Transferable where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Transferable from value: '" <>
        e <> "'. Accepted values: dont_know, transferable, untransferable"
