...
loSubmittedSince =
  lens _loSubmittedSince (\s a -> s {_loSubmittedSince = a}) . mapping _Time
...
instance AWSRequest ListOperations where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListOperationsResponse' <$> (x .?> "NextPageMarker") <*>
         (pure (fromEnum s)) <*>
         (x .?> "Operations" .!@ mempty))
...
lorsOperations =
  lens _lorsOperations (\s a -> s {_lorsOperations = a}) . _Coerce
