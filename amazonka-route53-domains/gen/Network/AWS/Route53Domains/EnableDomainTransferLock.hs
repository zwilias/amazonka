...
instance AWSRequest EnableDomainTransferLock where
  ...
  response =
    receiveJSON
      (\s h x ->
         EnableDomainTransferLockResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "OperationId"))
...
instance ToHeaders EnableDomainTransferLock where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("Route53Domains_v20140515.EnableDomainTransferLock" :: ByteString)
...
edtlrsResponseStatus =
  lens _edtlrsResponseStatus (\s a -> s {_edtlrsResponseStatus = a})
