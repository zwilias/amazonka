...
cdtAuthCode =
  lens _cdtAuthCode (\s a -> s {_cdtAuthCode = a}) . mapping _Sensitive
...
instance AWSRequest CheckDomainTransferability where
  type Rs CheckDomainTransferability = CheckDomainTransferabilityResponse
  request = postJSON route53Domains
  response =
    receiveJSON
      (\s h x ->
         CheckDomainTransferabilityResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "Transferability"))
...
instance ToHeaders CheckDomainTransferability where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("Route53Domains_v20140515.CheckDomainTransferability" :: ByteString)
...
cdtrsResponseStatus =
  lens _cdtrsResponseStatus (\s a -> s {_cdtrsResponseStatus = a})

-- | A complex type that contains information about whether the specified domain can be transferred to Amazon Route 53.
cdtrsTransferability ::
     Lens' CheckDomainTransferabilityResponse DomainTransferability
cdtrsTransferability =
  lens _cdtrsTransferability (\s a -> s {_cdtrsTransferability = a})
