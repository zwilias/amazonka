...
instance AWSRequest ResendContactReachabilityEmail where
  type Rs ResendContactReachabilityEmail = ResendContactReachabilityEmailResponse
  request = postJSON route53Domains
  response =
    receiveJSON
      (\s h x ->
         ResendContactReachabilityEmailResponse' <$> (x .?> "domainName") <*>
         (x .?> "emailAddress") <*>
         (x .?> "isAlreadyVerified") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ResendContactReachabilityEmail where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("Route53Domains_v20140515.ResendContactReachabilityEmail" :: ByteString)
...
rcrersEmailAddress =
  lens _rcrersEmailAddress (\s a -> s {_rcrersEmailAddress = a})

-- | @True@ if the email address for the registrant contact has already been verified, and @false@ otherwise. If the email address has already been verified, we don't send another confirmation email.
rcrersIsAlreadyVerified ::
     Lens' ResendContactReachabilityEmailResponse (Maybe Bool)
rcrersIsAlreadyVerified =
  lens _rcrersIsAlreadyVerified (\s a -> s {_rcrersIsAlreadyVerified = a})

-- | -- | The response status code.
rcrersResponseStatus :: Lens' ResendContactReachabilityEmailResponse Int
rcrersResponseStatus =
  lens _rcrersResponseStatus (\s a -> s {_rcrersResponseStatus = a})
