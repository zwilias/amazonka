...
instance AWSRequest GetOperationDetail where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetOperationDetailResponse' <$> (x .?> "Status") <*>
         (x .?> "SubmittedDate") <*>
         (x .?> "DomainName") <*>
         (x .?> "OperationId") <*>
         (x .?> "Type") <*>
...
godrsSubmittedDate =
  lens _godrsSubmittedDate (\s a -> s {_godrsSubmittedDate = a}) . mapping _Time
...
godrsResponseStatus =
  lens _godrsResponseStatus (\s a -> s {_godrsResponseStatus = a})
