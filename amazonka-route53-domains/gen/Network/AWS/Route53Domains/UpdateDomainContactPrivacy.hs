...
instance AWSRequest UpdateDomainContactPrivacy where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateDomainContactPrivacyResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "OperationId"))
...
instance ToHeaders UpdateDomainContactPrivacy where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("Route53Domains_v20140515.UpdateDomainContactPrivacy" :: ByteString)
...
udcprsResponseStatus =
  lens _udcprsResponseStatus (\s a -> s {_udcprsResponseStatus = a})
