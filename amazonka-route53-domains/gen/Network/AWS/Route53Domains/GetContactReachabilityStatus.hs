...
instance AWSRequest GetContactReachabilityStatus where
  type Rs GetContactReachabilityStatus = GetContactReachabilityStatusResponse
  request = postJSON route53Domains
  response =
    receiveJSON
      (\s h x ->
         GetContactReachabilityStatusResponse' <$> (x .?> "status") <*>
         (x .?> "domainName") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetContactReachabilityStatus where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("Route53Domains_v20140515.GetContactReachabilityStatus" :: ByteString)
...
gcrsrsStatus ::
     Lens' GetContactReachabilityStatusResponse (Maybe ReachabilityStatus)
...
gcrsrsResponseStatus =
  lens _gcrsrsResponseStatus (\s a -> s {_gcrsrsResponseStatus = a})
