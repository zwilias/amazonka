...
instance FromText AttackLayer where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AttackLayer from value: '" <>
...
instance FromText AttackPropertyIdentifier where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AttackPropertyIdentifier from value: '" <>
        e <>
...
instance FromText SubResourceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SubResourceType from value: '" <>
        e <> "'. Accepted values: ip, url"
...
instance FromText SubscriptionState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SubscriptionState from value: '" <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: bits, bytes, packets, requests"
