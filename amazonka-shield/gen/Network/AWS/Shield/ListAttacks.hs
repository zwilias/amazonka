...
laResourceARNs =
  lens _laResourceARNs (\s a -> s {_laResourceARNs = a}) . _Default . _Coerce
...
instance AWSRequest ListAttacks where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAttacksResponse' <$> (x .?> "AttackSummaries" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListAttacks where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSShield_20160616.ListAttacks" :: ByteString)
...
larsAttackSummaries =
  lens _larsAttackSummaries (\s a -> s {_larsAttackSummaries = a}) .
  _Default . _Coerce
