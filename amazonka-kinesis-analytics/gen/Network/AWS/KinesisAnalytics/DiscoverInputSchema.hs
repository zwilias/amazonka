...
disInputStartingPositionConfiguration ::
     Lens' DiscoverInputSchema (Maybe InputStartingPositionConfiguration)
disInputStartingPositionConfiguration =
  lens
    _disInputStartingPositionConfiguration
    (\s a -> s {_disInputStartingPositionConfiguration = a})

-- | The 'InputProcessingConfiguration' to use to preprocess the records before discovering the schema of the records.
disInputProcessingConfiguration ::
     Lens' DiscoverInputSchema (Maybe InputProcessingConfiguration)
disInputProcessingConfiguration =
  lens
    _disInputProcessingConfiguration
    (\s a -> s {_disInputProcessingConfiguration = a})
...
disS3Configuration =
  lens _disS3Configuration (\s a -> s {_disS3Configuration = a})
...
instance AWSRequest DiscoverInputSchema where
  ...
  response =
    receiveJSON
      (\s h x ->
         DiscoverInputSchemaResponse' <$> (x .?> "RawInputRecords" .!@ mempty) <*>
         (x .?> "InputSchema") <*>
...
disrsRawInputRecords =
  lens _disrsRawInputRecords (\s a -> s {_disrsRawInputRecords = a}) .
  _Default . _Coerce
...
disrsProcessedInputRecords =
  lens _disrsProcessedInputRecords (\s a -> s {_disrsProcessedInputRecords = a}) .
  _Default . _Coerce

-- | An array of elements, where each element corresponds to a row in a stream record (a stream record can have more than one row).
disrsParsedInputRecords :: Lens' DiscoverInputSchemaResponse [[Text]]
disrsParsedInputRecords =
  lens _disrsParsedInputRecords (\s a -> s {_disrsParsedInputRecords = a}) .
  _Default . _Coerce

-- | -- | The response status code.
disrsResponseStatus :: Lens' DiscoverInputSchemaResponse Int
disrsResponseStatus =
  lens _disrsResponseStatus (\s a -> s {_disrsResponseStatus = a})
