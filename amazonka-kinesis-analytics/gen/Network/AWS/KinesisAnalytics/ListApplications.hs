...
laExclusiveStartApplicationName =
  lens
    ...
    (\s a -> s {_laExclusiveStartApplicationName = a})
...
instance AWSRequest ListApplications where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListApplicationsResponse' <$> (pure (fromEnum s)) <*>
...
larsApplicationSummaries =
  lens _larsApplicationSummaries (\s a -> s {_larsApplicationSummaries = a}) .
  _Coerce

-- | Returns true if there are more applications to retrieve.
larsHasMoreApplications :: Lens' ListApplicationsResponse Bool
larsHasMoreApplications =
  lens _larsHasMoreApplications (\s a -> s {_larsHasMoreApplications = a})
