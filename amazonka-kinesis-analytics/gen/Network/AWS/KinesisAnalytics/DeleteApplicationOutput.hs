...
daoApplicationName =
  lens _daoApplicationName (\s a -> s {_daoApplicationName = a})
...
daoCurrentApplicationVersionId =
  lens
    ...
    (\s a -> s {_daoCurrentApplicationVersionId = a}) .
  _Nat
...
instance AWSRequest DeleteApplicationOutput where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteApplicationOutputResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteApplicationOutput where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("KinesisAnalytics_20150814.DeleteApplicationOutput" :: ByteString)
...
daorsResponseStatus =
  lens _daorsResponseStatus (\s a -> s {_daorsResponseStatus = a})
