...
uaCurrentApplicationVersionId =
  lens
    ...
    (\s a -> s {_uaCurrentApplicationVersionId = a}) .
  _Nat

-- | Describes application updates.
uaApplicationUpdate :: Lens' UpdateApplication ApplicationUpdate
uaApplicationUpdate =
  lens _uaApplicationUpdate (\s a -> s {_uaApplicationUpdate = a})

instance AWSRequest UpdateApplication where
  type Rs UpdateApplication = UpdateApplicationResponse
  request = postJSON kinesisAnalytics
  response =
    receiveEmpty (\s h x -> UpdateApplicationResponse' <$> (pure (fromEnum s)))
