...
dardsApplicationName =
  lens _dardsApplicationName (\s a -> s {_dardsApplicationName = a})
...
dardsCurrentApplicationVersionId ::
     Lens' DeleteApplicationReferenceDataSource Natural
dardsCurrentApplicationVersionId =
  lens
    _dardsCurrentApplicationVersionId
    (\s a -> s {_dardsCurrentApplicationVersionId = a}) .
  _Nat
...
instance AWSRequest DeleteApplicationReferenceDataSource where
  type Rs DeleteApplicationReferenceDataSource = DeleteApplicationReferenceDataSourceResponse
  request = postJSON kinesisAnalytics
  response =
    receiveEmpty
      (\s h x ->
         DeleteApplicationReferenceDataSourceResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteApplicationReferenceDataSource where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("KinesisAnalytics_20150814.DeleteApplicationReferenceDataSource" :: ByteString)
  ...
  toJSON DeleteApplicationReferenceDataSource' {..} =
    object
      (catMaybes
         ...
         , Just
             ("CurrentApplicationVersionId" .= _dardsCurrentApplicationVersionId)
...
dardsrsResponseStatus =
  lens _dardsrsResponseStatus (\s a -> s {_dardsrsResponseStatus = a})
