...
saInputConfigurations =
  lens _saInputConfigurations (\s a -> s {_saInputConfigurations = a}) . _Coerce
...
instance AWSRequest StartApplication where
  ...
  response =
    receiveEmpty (\s h x -> StartApplicationResponse' <$> (pure (fromEnum s)))
...
sarsResponseStatus =
  lens _sarsResponseStatus (\s a -> s {_sarsResponseStatus = a})
