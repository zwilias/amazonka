...
caApplicationDescription =
  lens _caApplicationDescription (\s a -> s {_caApplicationDescription = a})
...
caCloudWatchLoggingOptions =
  lens _caCloudWatchLoggingOptions (\s a -> s {_caCloudWatchLoggingOptions = a}) .
  _Default . _Coerce
...
instance AWSRequest CreateApplication where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateApplicationResponse' <$> (pure (fromEnum s)) <*>
...
carsApplicationSummary =
  lens _carsApplicationSummary (\s a -> s {_carsApplicationSummary = a})
