...
aacwloApplicationName =
  lens _aacwloApplicationName (\s a -> s {_aacwloApplicationName = a})

-- | The version ID of the Kinesis Analytics application.
aacwloCurrentApplicationVersionId ::
     Lens' AddApplicationCloudWatchLoggingOption Natural
aacwloCurrentApplicationVersionId =
  lens
    _aacwloCurrentApplicationVersionId
    (\s a -> s {_aacwloCurrentApplicationVersionId = a}) .
  _Nat

-- | Provides the CloudWatch log stream Amazon Resource Name (ARN) and the IAM role ARN. Note: To write application messages to CloudWatch, the IAM role that is used must have the @PutLogEvents@ policy action enabled.
aacwloCloudWatchLoggingOption ::
     Lens' AddApplicationCloudWatchLoggingOption CloudWatchLoggingOption
aacwloCloudWatchLoggingOption =
  lens
    _aacwloCloudWatchLoggingOption
    (\s a -> s {_aacwloCloudWatchLoggingOption = a})

instance AWSRequest AddApplicationCloudWatchLoggingOption where
  type Rs AddApplicationCloudWatchLoggingOption = AddApplicationCloudWatchLoggingOptionResponse
  request = postJSON kinesisAnalytics
  response =
    receiveEmpty
      (\s h x ->
         AddApplicationCloudWatchLoggingOptionResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AddApplicationCloudWatchLoggingOption where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("KinesisAnalytics_20150814.AddApplicationCloudWatchLoggingOption" :: ByteString)
      ...
      (catMaybes
         ...
         , Just
             ("CurrentApplicationVersionId" .=
              _aacwloCurrentApplicationVersionId)
         , Just ("CloudWatchLoggingOption" .= _aacwloCloudWatchLoggingOption)
         ])
...
aacwlorsResponseStatus ::
     Lens' AddApplicationCloudWatchLoggingOptionResponse Int
aacwlorsResponseStatus =
  lens _aacwlorsResponseStatus (\s a -> s {_aacwlorsResponseStatus = a})
