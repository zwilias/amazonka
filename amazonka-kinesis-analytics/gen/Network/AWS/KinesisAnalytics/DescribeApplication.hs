...
instance AWSRequest DescribeApplication where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeApplicationResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "ApplicationDetail"))
...
instance ToJSON DescribeApplication where
  toJSON DescribeApplication' {..} =
    object (catMaybes [Just ("ApplicationName" .= _daApplicationName)])
...
darsApplicationDetail =
  lens _darsApplicationDetail (\s a -> s {_darsApplicationDetail = a})
