...
aardsApplicationName =
  lens _aardsApplicationName (\s a -> s {_aardsApplicationName = a})

-- | Version of the application for which you are adding the reference data source. You can use the 'DescribeApplication' operation to get the current application version. If the version specified is not the current version, the @ConcurrentModificationException@ is returned.
aardsCurrentApplicationVersionId ::
     Lens' AddApplicationReferenceDataSource Natural
aardsCurrentApplicationVersionId =
  lens
    _aardsCurrentApplicationVersionId
    (\s a -> s {_aardsCurrentApplicationVersionId = a}) .
  _Nat

-- | The reference data source can be an object in your Amazon S3 bucket. Amazon Kinesis Analytics reads the object and copies the data into the in-application table that is created. You provide an S3 bucket, object key name, and the resulting in-application table that is created. You must also provide an IAM role with the necessary permissions that Amazon Kinesis Analytics can assume to read the object from your S3 bucket on your behalf.
aardsReferenceDataSource ::
     Lens' AddApplicationReferenceDataSource ReferenceDataSource
aardsReferenceDataSource =
  lens _aardsReferenceDataSource (\s a -> s {_aardsReferenceDataSource = a})

instance AWSRequest AddApplicationReferenceDataSource where
  type Rs AddApplicationReferenceDataSource = AddApplicationReferenceDataSourceResponse
...
instance ToHeaders AddApplicationReferenceDataSource where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("KinesisAnalytics_20150814.AddApplicationReferenceDataSource" :: ByteString)
  ...
  toJSON AddApplicationReferenceDataSource' {..} =
    object
      (catMaybes
         ...
         , Just
             ("CurrentApplicationVersionId" .= _aardsCurrentApplicationVersionId)
...
aardsrsResponseStatus =
  lens _aardsrsResponseStatus (\s a -> s {_aardsrsResponseStatus = a})
