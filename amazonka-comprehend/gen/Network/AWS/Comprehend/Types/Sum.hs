...
instance FromText InputFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing InputFormat from value: '" <>
        e <> "'. Accepted values: one_doc_per_file, one_doc_per_line"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: completed, failed, in_progress, submitted"
...
instance FromText SentimentType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SentimentType from value: '" <>
        e <> "'. Accepted values: mixed, negative, neutral, positive"
