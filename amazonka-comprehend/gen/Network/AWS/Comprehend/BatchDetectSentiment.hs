...
instance AWSRequest BatchDetectSentiment where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchDetectSentimentResponse' <$> (pure (fromEnum s)) <*>
         (x .?> "ResultList" .!@ mempty) <*>
         (x .?> "ErrorList" .!@ mempty))
...
bdsrsResponseStatus =
  lens _bdsrsResponseStatus (\s a -> s {_bdsrsResponseStatus = a})

-- | A list of objects containing the results of the operation. The results are sorted in ascending order by the @Index@ field and match the order of the documents in the input list. If all of the documents contain an error, the @ResultList@ is empty.
bdsrsResultList ::
     Lens' BatchDetectSentimentResponse [BatchDetectSentimentItemResult]
bdsrsResultList =
  lens _bdsrsResultList (\s a -> s {_bdsrsResultList = a}) . _Coerce

-- | A list containing one object for each document that contained an error. The results are sorted in ascending order by the @Index@ field and match the order of the documents in the input list. If there are no errors in the batch, the @ErrorList@ is empty.
bdsrsErrorList :: Lens' BatchDetectSentimentResponse [BatchItemError]
bdsrsErrorList =
  lens _bdsrsErrorList (\s a -> s {_bdsrsErrorList = a}) . _Coerce
