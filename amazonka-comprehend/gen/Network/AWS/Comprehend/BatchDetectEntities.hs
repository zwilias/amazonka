...
instance AWSRequest BatchDetectEntities where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchDetectEntitiesResponse' <$> (pure (fromEnum s)) <*>
         (x .?> "ResultList" .!@ mempty) <*>
         (x .?> "ErrorList" .!@ mempty))
...
bdersResponseStatus =
  lens _bdersResponseStatus (\s a -> s {_bdersResponseStatus = a})

-- | A list of objects containing the results of the operation. The results are sorted in ascending order by the @Index@ field and match the order of the documents in the input list. If all of the documents contain an error, the @ResultList@ is empty.
bdersResultList ::
     Lens' BatchDetectEntitiesResponse [BatchDetectEntitiesItemResult]
bdersResultList =
  lens _bdersResultList (\s a -> s {_bdersResultList = a}) . _Coerce

-- | A list containing one object for each document that contained an error. The results are sorted in ascending order by the @Index@ field and match the order of the documents in the input list. If there are no errors in the batch, the @ErrorList@ is empty.
bdersErrorList :: Lens' BatchDetectEntitiesResponse [BatchItemError]
bdersErrorList =
  lens _bdersErrorList (\s a -> s {_bdersErrorList = a}) . _Coerce
