...
instance AWSRequest DetectKeyPhrases where
  ...
  response =
    receiveJSON
      (\s h x ->
         DetectKeyPhrasesResponse' <$> (x .?> "KeyPhrases" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DetectKeyPhrases where
  toJSON DetectKeyPhrases' {..} =
    object
      (catMaybes
         [Just ("Text" .= _dkpText), Just ("LanguageCode" .= _dkpLanguageCode)])
...
dkprsKeyPhrases =
  lens _dkprsKeyPhrases (\s a -> s {_dkprsKeyPhrases = a}) . _Default . _Coerce
