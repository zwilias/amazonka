...
instance AWSRequest StartTopicsDetectionJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartTopicsDetectionJobResponse' <$> (x .?> "JobId") <*>
         (x .?> "JobStatus") <*>
         (pure (fromEnum s)))
...
stdjrsResponseStatus =
  lens _stdjrsResponseStatus (\s a -> s {_stdjrsResponseStatus = a})
