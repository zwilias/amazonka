...
instance AWSRequest DetectDominantLanguage where
  ...
  response =
    receiveJSON
      (\s h x ->
         DetectDominantLanguageResponse' <$> (x .?> "Languages" .!@ mempty) <*>
         (pure (fromEnum s)))
...
ddlrsLanguages =
  lens _ddlrsLanguages (\s a -> s {_ddlrsLanguages = a}) . _Default . _Coerce

-- | -- | The response status code.
ddlrsResponseStatus :: Lens' DetectDominantLanguageResponse Int
ddlrsResponseStatus =
  lens _ddlrsResponseStatus (\s a -> s {_ddlrsResponseStatus = a})
