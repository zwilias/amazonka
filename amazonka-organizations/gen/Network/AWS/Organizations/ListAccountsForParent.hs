...
lafpMaxResults =
  lens _lafpMaxResults (\s a -> s {_lafpMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListAccountsForParent where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAccountsForParentResponse' <$> (x .?> "Accounts" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
lafprsAccounts =
  lens _lafprsAccounts (\s a -> s {_lafprsAccounts = a}) . _Default . _Coerce
...
lafprsResponseStatus =
  lens _lafprsResponseStatus (\s a -> s {_lafprsResponseStatus = a})
