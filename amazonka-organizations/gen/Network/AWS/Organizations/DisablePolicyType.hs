...
instance AWSRequest DisablePolicyType where
  ...
  response =
    receiveJSON
      (\s h x ->
         DisablePolicyTypeResponse' <$> (x .?> "Root") <*> (pure (fromEnum s)))
...
instance ToJSON DisablePolicyType where
  toJSON DisablePolicyType' {..} =
    object
      (catMaybes
         [Just ("RootId" .= _dptRootId), Just ("PolicyType" .= _dptPolicyType)])
...
dptrsResponseStatus =
  lens _dptrsResponseStatus (\s a -> s {_dptrsResponseStatus = a})
