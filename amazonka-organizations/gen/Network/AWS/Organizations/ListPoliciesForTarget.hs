...
lpftMaxResults =
  lens _lpftMaxResults (\s a -> s {_lpftMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListPoliciesForTarget where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPoliciesForTargetResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Policies" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lpftrsPolicies =
  lens _lpftrsPolicies (\s a -> s {_lpftrsPolicies = a}) . _Default . _Coerce

-- | -- | The response status code.
lpftrsResponseStatus :: Lens' ListPoliciesForTargetResponse Int
lpftrsResponseStatus =
  lens _lpftrsResponseStatus (\s a -> s {_lpftrsResponseStatus = a})
