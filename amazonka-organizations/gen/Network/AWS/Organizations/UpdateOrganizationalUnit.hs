...
uouOrganizationalUnitId =
  lens _uouOrganizationalUnitId (\s a -> s {_uouOrganizationalUnitId = a})
...
instance AWSRequest UpdateOrganizationalUnit where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateOrganizationalUnitResponse' <$> (x .?> "OrganizationalUnit") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateOrganizationalUnit where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.UpdateOrganizationalUnit" :: ByteString)
...
uoursOrganizationalUnit ::
     Lens' UpdateOrganizationalUnitResponse (Maybe OrganizationalUnit)
uoursOrganizationalUnit =
  lens _uoursOrganizationalUnit (\s a -> s {_uoursOrganizationalUnit = a})

-- | -- | The response status code.
uoursResponseStatus :: Lens' UpdateOrganizationalUnitResponse Int
uoursResponseStatus =
  lens _uoursResponseStatus (\s a -> s {_uoursResponseStatus = a})
