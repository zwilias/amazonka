...
dOrganizationalUnitId =
  lens _dOrganizationalUnitId (\s a -> s {_dOrganizationalUnitId = a})
...
instance ToHeaders DeleteOrganizationalUnit where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.DeleteOrganizationalUnit" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON DeleteOrganizationalUnit where
  toJSON DeleteOrganizationalUnit' {..} =
    object (catMaybes [Just ("OrganizationalUnitId" .= _dOrganizationalUnitId)])
