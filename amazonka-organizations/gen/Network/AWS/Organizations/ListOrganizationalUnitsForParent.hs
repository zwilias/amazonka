...
loufpMaxResults =
  lens _loufpMaxResults (\s a -> s {_loufpMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListOrganizationalUnitsForParent where
  type Rs ListOrganizationalUnitsForParent = ListOrganizationalUnitsForParentResponse
  request = postJSON organizations
  response =
    receiveJSON
      (\s h x ->
         ListOrganizationalUnitsForParentResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders ListOrganizationalUnitsForParent where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.ListOrganizationalUnitsForParent" :: ByteString)
...
loufprsOrganizationalUnits ::
     Lens' ListOrganizationalUnitsForParentResponse [OrganizationalUnit]
loufprsOrganizationalUnits =
  lens _loufprsOrganizationalUnits (\s a -> s {_loufprsOrganizationalUnits = a}) .
  _Default . _Coerce

-- | -- | The response status code.
loufprsResponseStatus :: Lens' ListOrganizationalUnitsForParentResponse Int
loufprsResponseStatus =
  lens _loufprsResponseStatus (\s a -> s {_loufprsResponseStatus = a})
