...
lasafoMaxResults =
  lens _lasafoMaxResults (\s a -> s {_lasafoMaxResults = a}) . mapping _Nat
...
instance AWSPager ListAWSServiceAccessForOrganization where
  page rq rs
    ...
    | otherwise = Just $ rq & lasafoNextToken .~ rs ^. lasaforsNextToken

instance AWSRequest ListAWSServiceAccessForOrganization where
  type Rs ListAWSServiceAccessForOrganization = ListAWSServiceAccessForOrganizationResponse
  request = postJSON organizations
  response =
    receiveJSON
      (\s h x ->
         ListAWSServiceAccessForOrganizationResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders ListAWSServiceAccessForOrganization where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.ListAWSServiceAccessForOrganization" :: ByteString)
...
lasaforsEnabledServicePrincipals ::
     Lens' ListAWSServiceAccessForOrganizationResponse [EnabledServicePrincipal]
lasaforsEnabledServicePrincipals =
  lens
    _lasaforsEnabledServicePrincipals
    (\s a -> s {_lasaforsEnabledServicePrincipals = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lasaforsResponseStatus :: Lens' ListAWSServiceAccessForOrganizationResponse Int
lasaforsResponseStatus =
  lens _lasaforsResponseStatus (\s a -> s {_lasaforsResponseStatus = a})
