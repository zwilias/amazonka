...
instance AWSRequest EnablePolicyType where
  ...
  response =
    receiveJSON
      (\s h x ->
         EnablePolicyTypeResponse' <$> (x .?> "Root") <*> (pure (fromEnum s)))
...
instance ToJSON EnablePolicyType where
  toJSON EnablePolicyType' {..} =
    object
      (catMaybes
         [Just ("RootId" .= _eptRootId), Just ("PolicyType" .= _eptPolicyType)])
...
eptrsResponseStatus =
  lens _eptrsResponseStatus (\s a -> s {_eptrsResponseStatus = a})
