...
lcasMaxResults =
  lens _lcasMaxResults (\s a -> s {_lcasMaxResults = a}) . mapping _Nat
...
instance ToHeaders ListCreateAccountStatus where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.ListCreateAccountStatus" :: ByteString)
...
lcasrsCreateAccountStatuses ::
     Lens' ListCreateAccountStatusResponse [CreateAccountStatus]
lcasrsCreateAccountStatuses =
  lens
    _lcasrsCreateAccountStatuses
    (\s a -> s {_lcasrsCreateAccountStatuses = a}) .
  _Default . _Coerce
...
lcasrsResponseStatus =
  lens _lcasrsResponseStatus (\s a -> s {_lcasrsResponseStatus = a})
