...
lhfaMaxResults =
  lens _lhfaMaxResults (\s a -> s {_lhfaMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListHandshakesForAccount where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListHandshakesForAccountResponse' <$> (x .?> "Handshakes" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListHandshakesForAccount where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.ListHandshakesForAccount" :: ByteString)
...
lhfarsHandshakes =
  lens _lhfarsHandshakes (\s a -> s {_lhfarsHandshakes = a}) .
  _Default . _Coerce
...
lhfarsResponseStatus =
  lens _lhfarsResponseStatus (\s a -> s {_lhfarsResponseStatus = a})
