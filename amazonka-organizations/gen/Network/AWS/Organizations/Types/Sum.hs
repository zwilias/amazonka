...
instance FromText AccountJoinedMethod where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AccountJoinedMethod from value: '" <>
        e <> "'. Accepted values: created, invited"
...
instance FromText AccountStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AccountStatus from value: '" <>
        e <> "'. Accepted values: active, suspended"
  ...
  parser =
    takeLowerText >>= \case
      "add_organizations_service_linked_role" ->
        pure AddOrganizationsServiceLinkedRole
...
instance FromText ChildType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChildType from value: '" <>
        e <> "'. Accepted values: account, organizational_unit"
...
instance FromText CreateAccountFailureReason where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CreateAccountFailureReason from value: '" <>
        e <>
...
instance FromText CreateAccountState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CreateAccountState from value: '" <>
...
instance FromText HandshakePartyType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HandshakePartyType from value: '" <>
        e <> "'. Accepted values: account, email, organization"
...
instance FromText HandshakeResourceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HandshakeResourceType from value: '" <>
        e <>
...
instance FromText HandshakeState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HandshakeState from value: '" <>
        e <>
...
instance FromText IAMUserAccessToBilling where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IAMUserAccessToBilling from value: '" <>
...
instance FromText OrganizationFeatureSet where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrganizationFeatureSet from value: '" <>
        e <> "'. Accepted values: all, consolidated_billing"
...
instance FromText ParentType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ParentType from value: '" <>
        e <> "'. Accepted values: organizational_unit, root"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: service_control_policy"
...
instance FromText PolicyTypeStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PolicyTypeStatus from value: '" <>
        e <> "'. Accepted values: enabled, pending_disable, pending_enable"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: account, organizational_unit, root"
