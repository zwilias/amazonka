...
data DelegatedService = DelegatedService'{_dsServicePrincipal
                                          :: !(Maybe Text),
...
delegatedService
    :: DelegatedService
delegatedService
  = DelegatedService'{_dsServicePrincipal = Nothing,
...
instance FromJSON DelegatedService where
        parseJSON
          = withObject "DelegatedService"
              (\ x ->
                 DelegatedService' <$>
                   (x .:? "ServicePrincipal") <*>
                     (x .:? "DelegationEnabledDate"))

instance Hashable DelegatedService where

instance NFData DelegatedService where
