...
instance AWSRequest InviteAccountToOrganization where
  type Rs InviteAccountToOrganization = InviteAccountToOrganizationResponse
  request = postJSON organizations
  response =
    receiveJSON
      (\s h x ->
         InviteAccountToOrganizationResponse' <$> (x .?> "Handshake") <*>
         (pure (fromEnum s)))
...
instance ToHeaders InviteAccountToOrganization where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.InviteAccountToOrganization" :: ByteString)
...
instance ToJSON InviteAccountToOrganization where
  toJSON InviteAccountToOrganization' {..} =
    object
      (catMaybes [("Notes" .=) <$> _iatoNotes, Just ("Target" .= _iatoTarget)])
...
iatorsResponseStatus =
  lens _iatorsResponseStatus (\s a -> s {_iatorsResponseStatus = a})
