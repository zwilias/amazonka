...
douOrganizationalUnitId =
  lens _douOrganizationalUnitId (\s a -> s {_douOrganizationalUnitId = a})

instance AWSRequest DescribeOrganizationalUnit where
  type Rs DescribeOrganizationalUnit = DescribeOrganizationalUnitResponse
  request = postJSON organizations
  response =
    receiveJSON
      (\s h x ->
         DescribeOrganizationalUnitResponse' <$> (x .?> "OrganizationalUnit") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeOrganizationalUnit where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSOrganizationsV20161128.DescribeOrganizationalUnit" :: ByteString)
...
instance ToJSON DescribeOrganizationalUnit where
  toJSON DescribeOrganizationalUnit' {..} =
    object
      (catMaybes [Just ("OrganizationalUnitId" .= _douOrganizationalUnitId)])
...
doursOrganizationalUnit ::
     Lens' DescribeOrganizationalUnitResponse (Maybe OrganizationalUnit)
doursOrganizationalUnit =
  lens _doursOrganizationalUnit (\s a -> s {_doursOrganizationalUnit = a})

-- | -- | The response status code.
doursResponseStatus :: Lens' DescribeOrganizationalUnitResponse Int
doursResponseStatus =
  lens _doursResponseStatus (\s a -> s {_doursResponseStatus = a})
