...
instance AWSRequest RenewOffering where
  ...
  response =
    receiveJSON
      (\s h x ->
         RenewOfferingResponse' <$> (x .?> "offeringTransaction") <*>
         (pure (fromEnum s)))
...
instance ToHeaders RenewOffering where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DeviceFarm_20150623.RenewOffering" :: ByteString)
...
instance ToJSON RenewOffering where
  toJSON RenewOffering' {..} =
    object
      (catMaybes
         [("quantity" .=) <$> _roQuantity, ("offeringId" .=) <$> _roOfferingId])
...
rorsOfferingTransaction ::
     Lens' RenewOfferingResponse (Maybe OfferingTransaction)
rorsOfferingTransaction =
  lens _rorsOfferingTransaction (\s a -> s {_rorsOfferingTransaction = a})
