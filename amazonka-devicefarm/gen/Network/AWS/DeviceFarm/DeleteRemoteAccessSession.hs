...
instance AWSRequest DeleteRemoteAccessSession where
  type Rs DeleteRemoteAccessSession = DeleteRemoteAccessSessionResponse
  request = postJSON deviceFarm
  response =
    receiveEmpty
      (\s h x -> DeleteRemoteAccessSessionResponse' <$> (pure (fromEnum s)))
...
drasrsResponseStatus =
  lens _drasrsResponseStatus (\s a -> s {_drasrsResponseStatus = a})
