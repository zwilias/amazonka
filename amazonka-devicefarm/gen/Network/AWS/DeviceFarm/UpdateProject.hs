...
upDefaultJobTimeoutMinutes =
  lens _upDefaultJobTimeoutMinutes (\s a -> s {_upDefaultJobTimeoutMinutes = a})
...
instance AWSRequest UpdateProject where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateProjectResponse' <$> (x .?> "project") <*> (pure (fromEnum s)))
...
instance ToHeaders UpdateProject where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DeviceFarm_20150623.UpdateProject" :: ByteString)
