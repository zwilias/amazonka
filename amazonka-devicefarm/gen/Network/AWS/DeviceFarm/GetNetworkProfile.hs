...
instance AWSRequest GetNetworkProfile where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetNetworkProfileResponse' <$> (x .?> "networkProfile") <*>
         (pure (fromEnum s)))
...
instance ToJSON GetNetworkProfile where
  toJSON GetNetworkProfile' {..} = object (catMaybes [Just ("arn" .= _gnpArn)])
...
gnprsNetworkProfile =
  lens _gnprsNetworkProfile (\s a -> s {_gnprsNetworkProfile = a})

-- | -- | The response status code.
gnprsResponseStatus :: Lens' GetNetworkProfileResponse Int
gnprsResponseStatus =
  lens _gnprsResponseStatus (\s a -> s {_gnprsResponseStatus = a})
