...
crasRemoteRecordEnabled =
  lens _crasRemoteRecordEnabled (\s a -> s {_crasRemoteRecordEnabled = a})

-- | The Amazon Resource Name (ARN) for the app to be recorded in the remote access session.
crasRemoteRecordAppARN :: Lens' CreateRemoteAccessSession (Maybe Text)
crasRemoteRecordAppARN =
  lens _crasRemoteRecordAppARN (\s a -> s {_crasRemoteRecordAppARN = a})
...
crasRemoteDebugEnabled =
  lens _crasRemoteDebugEnabled (\s a -> s {_crasRemoteDebugEnabled = a})

-- | The configuration information for the remote access session request.
crasConfiguration ::
     Lens' CreateRemoteAccessSession (Maybe CreateRemoteAccessSessionConfiguration)
...
crasInteractionMode =
  lens _crasInteractionMode (\s a -> s {_crasInteractionMode = a})
...
instance AWSRequest CreateRemoteAccessSession where
  type Rs CreateRemoteAccessSession = CreateRemoteAccessSessionResponse
  request = postJSON deviceFarm
  response =
    receiveJSON
      (\s h x ->
         CreateRemoteAccessSessionResponse' <$> (x .?> "remoteAccessSession") <*>
         (pure (fromEnum s)))
...
crasrsRemoteAccessSession ::
     Lens' CreateRemoteAccessSessionResponse (Maybe RemoteAccessSession)
crasrsRemoteAccessSession =
  lens _crasrsRemoteAccessSession (\s a -> s {_crasrsRemoteAccessSession = a})

-- | -- | The response status code.
crasrsResponseStatus :: Lens' CreateRemoteAccessSessionResponse Int
crasrsResponseStatus =
  lens _crasrsResponseStatus (\s a -> s {_crasrsResponseStatus = a})
