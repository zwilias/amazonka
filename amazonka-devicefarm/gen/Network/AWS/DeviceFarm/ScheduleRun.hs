...
srExecutionConfiguration =
  lens _srExecutionConfiguration (\s a -> s {_srExecutionConfiguration = a})
...
instance AWSRequest ScheduleRun where
  ...
  response =
    receiveJSON
      (\s h x -> ScheduleRunResponse' <$> (x .?> "run") <*> (pure (fromEnum s)))
...
instance ToHeaders ScheduleRun where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DeviceFarm_20150623.ScheduleRun" :: ByteString)
...
srrsResponseStatus =
  lens _srrsResponseStatus (\s a -> s {_srrsResponseStatus = a})
