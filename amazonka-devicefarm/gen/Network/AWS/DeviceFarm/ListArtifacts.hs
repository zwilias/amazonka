...
instance AWSRequest ListArtifacts where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListArtifactsResponse' <$> (x .?> "artifacts" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListArtifacts where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DeviceFarm_20150623.ListArtifacts" :: ByteString)
...
larsArtifacts =
  lens _larsArtifacts (\s a -> s {_larsArtifacts = a}) . _Default . _Coerce
