...
instance AWSRequest ListUploads where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListUploadsResponse' <$> (x .?> "nextToken") <*>
         (x .?> "uploads" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListUploads where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DeviceFarm_20150623.ListUploads" :: ByteString)
...
instance ToJSON ListUploads where
  toJSON ListUploads' {..} =
    object
      (catMaybes [("nextToken" .=) <$> _luNextToken, Just ("arn" .= _luArn)])
...
lursUploads =
  lens _lursUploads (\s a -> s {_lursUploads = a}) . _Default . _Coerce

-- | -- | The response status code.
lursResponseStatus :: Lens' ListUploadsResponse Int
lursResponseStatus =
  lens _lursResponseStatus (\s a -> s {_lursResponseStatus = a})
