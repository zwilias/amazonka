...
instance AWSRequest GetDeviceInstance where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDeviceInstanceResponse' <$> (x .?> "deviceInstance") <*>
         (pure (fromEnum s)))
...
instance ToJSON GetDeviceInstance where
  toJSON GetDeviceInstance' {..} = object (catMaybes [Just ("arn" .= _gdiArn)])
...
gdirsDeviceInstance =
  lens _gdirsDeviceInstance (\s a -> s {_gdirsDeviceInstance = a})
