...
instance AWSRequest ListRemoteAccessSessions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListRemoteAccessSessionsResponse' <$> (x .?> "nextToken") <*>
...
instance ToJSON ListRemoteAccessSessions where
  toJSON ListRemoteAccessSessions' {..} =
    object
      (catMaybes [("nextToken" .=) <$> _lrasNextToken, Just ("arn" .= _lrasArn)])
...
lrasrsRemoteAccessSessions ::
     Lens' ListRemoteAccessSessionsResponse [RemoteAccessSession]
lrasrsRemoteAccessSessions =
  lens _lrasrsRemoteAccessSessions (\s a -> s {_lrasrsRemoteAccessSessions = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lrasrsResponseStatus :: Lens' ListRemoteAccessSessionsResponse Int
lrasrsResponseStatus =
  lens _lrasrsResponseStatus (\s a -> s {_lrasrsResponseStatus = a})
