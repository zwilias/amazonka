...
instance AWSRequest ListNetworkProfiles where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListNetworkProfilesResponse' <$> (x .?> "networkProfiles" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
lnprsNetworkProfiles =
  lens _lnprsNetworkProfiles (\s a -> s {_lnprsNetworkProfiles = a}) .
  _Default . _Coerce
...
lnprsResponseStatus =
  lens _lnprsResponseStatus (\s a -> s {_lnprsResponseStatus = a})
