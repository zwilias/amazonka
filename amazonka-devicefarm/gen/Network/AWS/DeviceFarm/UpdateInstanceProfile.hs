...
uipExcludeAppPackagesFromCleanup =
  lens
    ...
    (\s a -> s {_uipExcludeAppPackagesFromCleanup = a}) .
  _Default . _Coerce
...
instance AWSRequest UpdateInstanceProfile where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateInstanceProfileResponse' <$> (x .?> "instanceProfile") <*>
         (pure (fromEnum s)))
...
uiprsInstanceProfile ::
     Lens' UpdateInstanceProfileResponse (Maybe InstanceProfile)
uiprsInstanceProfile =
  lens _uiprsInstanceProfile (\s a -> s {_uiprsInstanceProfile = a})
