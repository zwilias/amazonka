...
poOfferingPromotionId =
  lens _poOfferingPromotionId (\s a -> s {_poOfferingPromotionId = a})
...
instance AWSRequest PurchaseOffering where
  ...
  response =
    receiveJSON
      (\s h x ->
         PurchaseOfferingResponse' <$> (x .?> "offeringTransaction") <*>
         (pure (fromEnum s)))
...
porsOfferingTransaction ::
     Lens' PurchaseOfferingResponse (Maybe OfferingTransaction)
porsOfferingTransaction =
  lens _porsOfferingTransaction (\s a -> s {_porsOfferingTransaction = a})
