...
gtvsMaxResults =
  lens _gtvsMaxResults (\s a -> s {_gtvsMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetTableVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetTableVersionsResponse' <$> (x .?> "TableVersions" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
gtvsrsTableVersions =
  lens _gtvsrsTableVersions (\s a -> s {_gtvsrsTableVersions = a}) .
  _Default . _Coerce
...
gtvsrsResponseStatus =
  lens _gtvsrsResponseStatus (\s a -> s {_gtvsrsResponseStatus = a})
