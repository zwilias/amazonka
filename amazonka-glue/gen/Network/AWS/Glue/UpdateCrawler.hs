...
uSchemaChangePolicy =
  lens _uSchemaChangePolicy (\s a -> s {_uSchemaChangePolicy = a})
...
uClassifiers =
  lens _uClassifiers (\s a -> s {_uClassifiers = a}) . _Default . _Coerce
...
instance AWSRequest UpdateCrawler where
  ...
  response =
    receiveEmpty (\s h x -> UpdateCrawlerResponse' <$> (pure (fromEnum s)))
...
uccrsResponseStatus =
  lens _uccrsResponseStatus (\s a -> s {_uccrsResponseStatus = a})
