...
gcmCrawlerNameList =
  lens _gcmCrawlerNameList (\s a -> s {_gcmCrawlerNameList = a}) .
  _Default . _Coerce
...
instance AWSRequest GetCrawlerMetrics where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCrawlerMetricsResponse' <$> (x .?> "CrawlerMetricsList" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
gcmrsCrawlerMetricsList =
  lens _gcmrsCrawlerMetricsList (\s a -> s {_gcmrsCrawlerMetricsList = a}) .
  _Default . _Coerce
