...
ccSchemaChangePolicy =
  lens _ccSchemaChangePolicy (\s a -> s {_ccSchemaChangePolicy = a})
...
ccClassifiers =
  lens _ccClassifiers (\s a -> s {_ccClassifiers = a}) . _Default . _Coerce
...
instance AWSRequest CreateCrawler where
  ...
  response =
    receiveEmpty (\s h x -> CreateCrawlerResponse' <$> (pure (fromEnum s)))
