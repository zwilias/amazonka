...
bdcConnectionNameList =
  lens _bdcConnectionNameList (\s a -> s {_bdcConnectionNameList = a}) . _Coerce
...
instance AWSRequest BatchDeleteConnection where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchDeleteConnectionResponse' <$> (x .?> "Succeeded" .!@ mempty) <*>
         (x .?> "Errors" .!@ mempty) <*>
         (pure (fromEnum s)))
...
bdcrsSucceeded =
  lens _bdcrsSucceeded (\s a -> s {_bdcrsSucceeded = a}) . _Default . _Coerce
...
bdcrsResponseStatus =
  lens _bdcrsResponseStatus (\s a -> s {_bdcrsResponseStatus = a})
