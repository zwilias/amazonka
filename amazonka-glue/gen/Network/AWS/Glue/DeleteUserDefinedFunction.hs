...
instance AWSRequest DeleteUserDefinedFunction where
  type Rs DeleteUserDefinedFunction = DeleteUserDefinedFunctionResponse
  request = postJSON glue
  response =
    receiveEmpty
      (\s h x -> DeleteUserDefinedFunctionResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteUserDefinedFunction where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSGlue.DeleteUserDefinedFunction" :: ByteString)
...
dudfrsResponseStatus =
  lens _dudfrsResponseStatus (\s a -> s {_dudfrsResponseStatus = a})
