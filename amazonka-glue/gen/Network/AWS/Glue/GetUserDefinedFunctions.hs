...
gudfMaxResults =
  lens _gudfMaxResults (\s a -> s {_gudfMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetUserDefinedFunctions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetUserDefinedFunctionsResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders GetUserDefinedFunctions where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSGlue.GetUserDefinedFunctions" :: ByteString)
...
gudfrsUserDefinedFunctions ::
     Lens' GetUserDefinedFunctionsResponse [UserDefinedFunction]
gudfrsUserDefinedFunctions =
  lens _gudfrsUserDefinedFunctions (\s a -> s {_gudfrsUserDefinedFunctions = a}) .
  _Default . _Coerce

-- | -- | The response status code.
gudfrsResponseStatus :: Lens' GetUserDefinedFunctionsResponse Int
gudfrsResponseStatus =
  lens _gudfrsResponseStatus (\s a -> s {_gudfrsResponseStatus = a})
