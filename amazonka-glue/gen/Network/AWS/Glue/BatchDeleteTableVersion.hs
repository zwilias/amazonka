...
bdtvVersionIds =
  lens _bdtvVersionIds (\s a -> s {_bdtvVersionIds = a}) . _Coerce
...
instance AWSRequest BatchDeleteTableVersion where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchDeleteTableVersionResponse' <$> (x .?> "Errors" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders BatchDeleteTableVersion where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSGlue.BatchDeleteTableVersion" :: ByteString)
...
bdtvrsErrors =
  lens _bdtvrsErrors (\s a -> s {_bdtvrsErrors = a}) . _Default . _Coerce

-- | -- | The response status code.
bdtvrsResponseStatus :: Lens' BatchDeleteTableVersionResponse Int
bdtvrsResponseStatus =
  lens _bdtvrsResponseStatus (\s a -> s {_bdtvrsResponseStatus = a})
