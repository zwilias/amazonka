...
instance AWSRequest BatchStopJobRun where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchStopJobRunResponse' <$> (x .?> "SuccessfulSubmissions" .!@ mempty) <*>
         (x .?> "Errors" .!@ mempty) <*>
         (pure (fromEnum s)))
...
bsjrrsSuccessfulSubmissions ::
     Lens' BatchStopJobRunResponse [BatchStopJobRunSuccessfulSubmission]
bsjrrsSuccessfulSubmissions =
  lens
    _bsjrrsSuccessfulSubmissions
    (\s a -> s {_bsjrrsSuccessfulSubmissions = a}) .
  _Default . _Coerce
...
bsjrrsErrors =
  lens _bsjrrsErrors (\s a -> s {_bsjrrsErrors = a}) . _Default . _Coerce

-- | -- | The response status code.
bsjrrsResponseStatus :: Lens' BatchStopJobRunResponse Int
bsjrrsResponseStatus =
  lens _bsjrrsResponseStatus (\s a -> s {_bsjrrsResponseStatus = a})
