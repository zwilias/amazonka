...
instance AWSRequest UpdateUserDefinedFunction where
  type Rs UpdateUserDefinedFunction = UpdateUserDefinedFunctionResponse
  request = postJSON glue
  response =
    receiveEmpty
      (\s h x -> UpdateUserDefinedFunctionResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateUserDefinedFunction where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSGlue.UpdateUserDefinedFunction" :: ByteString)
...
uudfrsResponseStatus =
  lens _uudfrsResponseStatus (\s a -> s {_uudfrsResponseStatus = a})
