...
data ListWorkflows = ListWorkflows'{_lwNextToken ::
                                    !(Maybe Text),
...
listWorkflows
    :: ListWorkflows
listWorkflows
  = ListWorkflows'{_lwNextToken = Nothing,
...
instance AWSRequest ListWorkflows where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 ListWorkflowsResponse' <$>
                   (x .?> "NextToken") <*> (x .?> "Workflows") <*>
                     (pure (fromEnum s)))

instance Hashable ListWorkflows where

instance NFData ListWorkflows where
...
data ListWorkflowsResponse = ListWorkflowsResponse'{_lwrsNextToken
                                                    :: !(Maybe Text),
                                                    _lwrsWorkflows ::
                                                    !(Maybe (List1 Text)),
...
listWorkflowsResponse pResponseStatus_
  = ListWorkflowsResponse'{_lwrsNextToken = Nothing,
...
instance NFData ListWorkflowsResponse where
