...
bcpPartitionInputList =
  lens _bcpPartitionInputList (\s a -> s {_bcpPartitionInputList = a}) . _Coerce
...
instance AWSRequest BatchCreatePartition where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchCreatePartitionResponse' <$> (x .?> "Errors" .!@ mempty) <*>
         (pure (fromEnum s)))
...
bcprsErrors =
  lens _bcprsErrors (\s a -> s {_bcprsErrors = a}) . _Default . _Coerce

-- | -- | The response status code.
bcprsResponseStatus :: Lens' BatchCreatePartitionResponse Int
bcprsResponseStatus =
  lens _bcprsResponseStatus (\s a -> s {_bcprsResponseStatus = a})
