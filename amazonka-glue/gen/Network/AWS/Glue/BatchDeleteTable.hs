...
bdtTablesToDelete =
  lens _bdtTablesToDelete (\s a -> s {_bdtTablesToDelete = a}) . _Coerce
...
instance AWSRequest BatchDeleteTable where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchDeleteTableResponse' <$> (x .?> "Errors" .!@ mempty) <*>
         (pure (fromEnum s)))
...
bdtrsErrors =
  lens _bdtrsErrors (\s a -> s {_bdtrsErrors = a}) . _Default . _Coerce

-- | -- | The response status code.
bdtrsResponseStatus :: Lens' BatchDeleteTableResponse Int
bdtrsResponseStatus =
  lens _bdtrsResponseStatus (\s a -> s {_bdtrsResponseStatus = a})
