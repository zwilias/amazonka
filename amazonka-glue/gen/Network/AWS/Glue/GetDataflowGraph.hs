...
instance AWSRequest GetDataflowGraph where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDataflowGraphResponse' <$> (x .?> "DagEdges" .!@ mempty) <*>
         (x .?> "DagNodes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
gdgrsDagEdges =
  lens _gdgrsDagEdges (\s a -> s {_gdgrsDagEdges = a}) . _Default . _Coerce

-- | A list of the nodes in the resulting DAG.
gdgrsDagNodes :: Lens' GetDataflowGraphResponse [CodeGenNode]
gdgrsDagNodes =
  lens _gdgrsDagNodes (\s a -> s {_gdgrsDagNodes = a}) . _Default . _Coerce
