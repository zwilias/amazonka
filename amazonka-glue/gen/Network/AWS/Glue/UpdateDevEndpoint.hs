...
udeCustomLibraries =
  lens _udeCustomLibraries (\s a -> s {_udeCustomLibraries = a})
...
udeUpdateEtlLibraries =
  lens _udeUpdateEtlLibraries (\s a -> s {_udeUpdateEtlLibraries = a})
...
instance AWSRequest UpdateDevEndpoint where
  ...
  response =
    receiveEmpty (\s h x -> UpdateDevEndpointResponse' <$> (pure (fromEnum s)))
...
udersResponseStatus =
  lens _udersResponseStatus (\s a -> s {_udersResponseStatus = a})
