...
instance AWSRequest GetUserDefinedFunction where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetUserDefinedFunctionResponse' <$> (x .?> "UserDefinedFunction") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetUserDefinedFunction where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSGlue.GetUserDefinedFunction" :: ByteString)
...
gudfursUserDefinedFunction ::
     Lens' GetUserDefinedFunctionResponse (Maybe UserDefinedFunction)
gudfursUserDefinedFunction =
  lens _gudfursUserDefinedFunction (\s a -> s {_gudfursUserDefinedFunction = a})

-- | -- | The response status code.
gudfursResponseStatus :: Lens' GetUserDefinedFunctionResponse Int
gudfursResponseStatus =
  lens _gudfursResponseStatus (\s a -> s {_gudfursResponseStatus = a})
