...
instance AWSRequest UpdateTrigger where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateTriggerResponse' <$> (x .?> "Trigger") <*> (pure (fromEnum s)))
...
instance ToJSON UpdateTrigger where
  toJSON UpdateTrigger' {..} =
    object
      (catMaybes
         [Just ("Name" .= _utName), Just ("TriggerUpdate" .= _utTriggerUpdate)])
...
updrsResponseStatus =
  lens _updrsResponseStatus (\s a -> s {_updrsResponseStatus = a})
