...
data WorkflowRunStatistics = WorkflowRunStatistics'{_wrsRunningActions
                                                    :: !(Maybe Int),
                                                    _wrsStoppedActions ::
                                                    !(Maybe Int),
                                                    _wrsTotalActions ::
...
--
-- * 'wrsStoppedActions' - Total number of Actions which have stopped.
--
-- * 'wrsTotalActions' - Total number of Actions in the workflow run.
--
-- * 'wrsFailedActions' - Total number of Actions which have failed.
--
-- * 'wrsTimeoutActions' - Total number of Actions which timed out.
--
-- * 'wrsSucceededActions' - Total number of Actions which have succeeded.
workflowRunStatistics
    :: WorkflowRunStatistics
workflowRunStatistics
  = WorkflowRunStatistics'{_wrsRunningActions =
                             Nothing,
...
instance FromJSON WorkflowRunStatistics where
        parseJSON
          = withObject "WorkflowRunStatistics"
              (\ x ->
                 WorkflowRunStatistics' <$>
                   (x .:? "RunningActions") <*> (x .:? "StoppedActions")
                     <*> (x .:? "TotalActions")
                     <*> (x .:? "FailedActions")
                     <*> (x .:? "TimeoutActions")
                     <*> (x .:? "SucceededActions"))

instance Hashable WorkflowRunStatistics where

instance NFData WorkflowRunStatistics where
