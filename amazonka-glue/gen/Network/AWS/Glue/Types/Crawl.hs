...
data Crawl = Crawl'{_craCompletedOn ::
                    !(Maybe POSIX),
...
crawl
    :: Crawl
crawl
  = Crawl'{_craCompletedOn = Nothing,
           _craState = Nothing, _craStartedOn = Nothing,
           _craLogStream = Nothing, _craLogGroup = Nothing,
...
instance FromJSON Crawl where
        parseJSON
          = withObject "Crawl"
              (\ x ->
                 Crawl' <$>
                   (x .:? "CompletedOn") <*> (x .:? "State") <*>
                     (x .:? "StartedOn")
                     <*> (x .:? "LogStream")
                     <*> (x .:? "LogGroup")
                     <*> (x .:? "ErrorMessage"))

instance Hashable Crawl where

instance NFData Crawl where
