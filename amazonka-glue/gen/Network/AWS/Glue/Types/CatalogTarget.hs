...
data CatalogTarget = CatalogTarget'{_ctDatabaseName
                                    :: !Text,
...
catalogTarget
    :: Text -- ^ 'ctDatabaseName'
    -> NonEmpty Text -- ^ 'ctTables'
    -> CatalogTarget
catalogTarget pDatabaseName_ pTables_
  = CatalogTarget'{_ctDatabaseName = pDatabaseName_,
...
instance FromJSON CatalogTarget where
        parseJSON
          = withObject "CatalogTarget"
              (\ x ->
                 CatalogTarget' <$>
                   (x .: "DatabaseName") <*> (x .: "Tables"))

instance Hashable CatalogTarget where

instance NFData CatalogTarget where
