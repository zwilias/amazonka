...
data GlueTable = GlueTable'{_gtCatalogId ::
                            !(Maybe Text),
                            _gtConnectionName :: !(Maybe Text),
                            _gtDatabaseName :: !Text, _gtTableName :: !Text}
...
glueTable pDatabaseName_ pTableName_
  = GlueTable'{_gtCatalogId = Nothing,
...
instance FromJSON GlueTable where
        parseJSON
          = withObject "GlueTable"
              (\ x ->
                 GlueTable' <$>
                   (x .:? "CatalogId") <*> (x .:? "ConnectionName") <*>
                     (x .: "DatabaseName")
                     <*> (x .: "TableName"))

instance Hashable GlueTable where

instance NFData GlueTable where
