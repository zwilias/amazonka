...
newtype TriggerNodeDetails = TriggerNodeDetails'{_tndTrigger
                                                 :: Maybe Trigger}
...
instance FromJSON TriggerNodeDetails where
        parseJSON
          = withObject "TriggerNodeDetails"
              (\ x -> TriggerNodeDetails' <$> (x .:? "Trigger"))

instance Hashable TriggerNodeDetails where

instance NFData TriggerNodeDetails where
