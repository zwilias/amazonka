...
newtype CrawlerNodeDetails = CrawlerNodeDetails'{_cndCrawls
                                                 :: Maybe [Crawl]}
...
instance FromJSON CrawlerNodeDetails where
        parseJSON
          = withObject "CrawlerNodeDetails"
              (\ x ->
                 CrawlerNodeDetails' <$> (x .:? "Crawls" .!= mempty))

instance Hashable CrawlerNodeDetails where

instance NFData CrawlerNodeDetails where
