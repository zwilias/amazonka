...
data Workflow = Workflow'{_wGraph ::
                          !(Maybe WorkflowGraph),
...
workflow
    :: Workflow
workflow
  = Workflow'{_wGraph = Nothing,
              _wLastModifiedOn = Nothing,
              _wDefaultRunProperties = Nothing, _wName = Nothing,
              _wLastRun = Nothing, _wDescription = Nothing,
              _wCreatedOn = Nothing}
...
instance NFData Workflow where
