...
data SchemaColumn = SchemaColumn'{_sName ::
                                  !(Maybe Text),
...
instance FromJSON SchemaColumn where
        parseJSON
          = withObject "SchemaColumn"
              (\ x ->
                 SchemaColumn' <$>
                   (x .:? "Name") <*> (x .:? "DataType"))

instance Hashable SchemaColumn where

instance NFData SchemaColumn where
