...
gpsMaxResults =
  lens _gpsMaxResults (\s a -> s {_gpsMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetPartitions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetPartitionsResponse' <$> (x .?> "Partitions" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
gpsrsPartitions =
  lens _gpsrsPartitions (\s a -> s {_gpsrsPartitions = a}) . _Default . _Coerce
