...
instance AWSRequest ResetJobBookmark where
  ...
  response =
    receiveJSON
      (\s h x ->
         ResetJobBookmarkResponse' <$> (x .?> "JobBookmarkEntry") <*>
         (pure (fromEnum s)))
...
instance ToJSON ResetJobBookmark where
  toJSON ResetJobBookmark' {..} =
    object (catMaybes [Just ("JobName" .= _rjbJobName)])
...
rjbrsJobBookmarkEntry =
  lens _rjbrsJobBookmarkEntry (\s a -> s {_rjbrsJobBookmarkEntry = a})

-- | -- | The response status code.
rjbrsResponseStatus :: Lens' ResetJobBookmarkResponse Int
rjbrsResponseStatus =
  lens _rjbrsResponseStatus (\s a -> s {_rjbrsResponseStatus = a})
