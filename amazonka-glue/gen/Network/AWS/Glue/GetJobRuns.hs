...
gjrMaxResults =
  lens _gjrMaxResults (\s a -> s {_gjrMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetJobRuns where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetJobRunsResponse' <$> (x .?> "NextToken") <*>
         (x .?> "JobRuns" .!@ mempty) <*>
         (pure (fromEnum s)))
...
gjrrsJobRuns =
  lens _gjrrsJobRuns (\s a -> s {_gjrrsJobRuns = a}) . _Default . _Coerce

-- | -- | The response status code.
gjrrsResponseStatus :: Lens' GetJobRunsResponse Int
gjrrsResponseStatus =
  lens _gjrrsResponseStatus (\s a -> s {_gjrrsResponseStatus = a})
