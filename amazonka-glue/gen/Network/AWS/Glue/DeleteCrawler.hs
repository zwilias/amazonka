...
instance AWSRequest DeleteCrawler where
  ...
  response =
    receiveEmpty (\s h x -> DeleteCrawlerResponse' <$> (pure (fromEnum s)))
...
instance ToJSON DeleteCrawler where
  toJSON DeleteCrawler' {..} = object (catMaybes [Just ("Name" .= _dcName)])
...
dccrsResponseStatus =
  lens _dccrsResponseStatus (\s a -> s {_dccrsResponseStatus = a})
