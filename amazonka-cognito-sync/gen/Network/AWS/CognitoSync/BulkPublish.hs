...
instance ToPath BulkPublish where
  toPath BulkPublish' {..} =
    mconcat ["/identitypools/", toBS _bpIdentityPoolId, "/bulkpublish"]
...
bprsIdentityPoolId =
  lens _bprsIdentityPoolId (\s a -> s {_bprsIdentityPoolId = a})

-- | -- | The response status code.
bprsResponseStatus :: Lens' BulkPublishResponse Int
bprsResponseStatus =
  lens _bprsResponseStatus (\s a -> s {_bprsResponseStatus = a})
