...
urRecordPatches =
  lens _urRecordPatches (\s a -> s {_urRecordPatches = a}) . _Default . _Coerce
...
instance AWSRequest UpdateRecords where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateRecordsResponse' <$> (x .?> "Records" .!@ mempty) <*>
         (pure (fromEnum s)))
...
urrsRecords =
  lens _urrsRecords (\s a -> s {_urrsRecords = a}) . _Default . _Coerce
