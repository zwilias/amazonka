...
instance AWSRequest SetIdentityPoolConfiguration where
  ...
  response =
    receiveJSON
      (\s h x ->
         SetIdentityPoolConfigurationResponse' <$> (x .?> "IdentityPoolId") <*>
         (x .?> "CognitoStreams") <*>
         (x .?> "PushSync") <*>
         (pure (fromEnum s)))
...
instance ToPath SetIdentityPoolConfiguration where
  toPath SetIdentityPoolConfiguration' {..} =
    mconcat ["/identitypools/", toBS _sipcIdentityPoolId, "/configuration"]
...
sipcrsIdentityPoolId =
  lens _sipcrsIdentityPoolId (\s a -> s {_sipcrsIdentityPoolId = a})

-- | Options to apply to this identity pool for Amazon Cognito streams.
sipcrsCognitoStreams ::
     Lens' SetIdentityPoolConfigurationResponse (Maybe CognitoStreams)
sipcrsCognitoStreams =
  lens _sipcrsCognitoStreams (\s a -> s {_sipcrsCognitoStreams = a})
