...
instance FromText BulkPublishStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BulkPublishStatus from value: '" <>
...
instance FromText Operation where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Operation from value: '" <>
        e <> "'. Accepted values: remove, replace"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: adm, apns, apns_sandbox, gcm"
...
instance FromText StreamingStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StreamingStatus from value: '" <>
