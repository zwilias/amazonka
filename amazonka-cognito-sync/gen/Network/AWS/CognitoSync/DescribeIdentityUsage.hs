...
instance AWSRequest DescribeIdentityUsage where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeIdentityUsageResponse' <$> (x .?> "IdentityUsage") <*>
         (pure (fromEnum s)))
...
diursIdentityUsage =
  lens _diursIdentityUsage (\s a -> s {_diursIdentityUsage = a})

-- | -- | The response status code.
diursResponseStatus :: Lens' DescribeIdentityUsageResponse Int
diursResponseStatus =
  lens _diursResponseStatus (\s a -> s {_diursResponseStatus = a})
