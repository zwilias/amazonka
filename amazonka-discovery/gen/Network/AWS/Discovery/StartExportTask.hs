...
setExportDataFormat =
  lens _setExportDataFormat (\s a -> s {_setExportDataFormat = a}) .
  _Default . _Coerce

-- | The start timestamp for exported data from the single Application Discovery Agent selected in the filters. If no value is specified, data is exported starting from the first data collected by the agent.
setStartTime :: Lens' StartExportTask (Maybe UTCTime)
setStartTime =
  lens _setStartTime (\s a -> s {_setStartTime = a}) . mapping _Time
...
instance AWSRequest StartExportTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartExportTaskResponse' <$> (x .?> "exportId") <*> (pure (fromEnum s)))
...
setrsResponseStatus =
  lens _setrsResponseStatus (\s a -> s {_setrsResponseStatus = a})
