...
lsnPortInformationNeeded =
  lens _lsnPortInformationNeeded (\s a -> s {_lsnPortInformationNeeded = a})
...
lsnNeighborConfigurationIds =
  lens
    ...
    (\s a -> s {_lsnNeighborConfigurationIds = a}) .
  _Default . _Coerce
...
lsnConfigurationId =
  lens _lsnConfigurationId (\s a -> s {_lsnConfigurationId = a})
...
instance AWSRequest ListServerNeighbors where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListServerNeighborsResponse' <$> (x .?> "nextToken") <*>
...
instance ToHeaders ListServerNeighbors where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSPoseidonService_V2015_11_01.ListServerNeighbors" :: ByteString)
...
lsnrsKnownDependencyCount =
  lens _lsnrsKnownDependencyCount (\s a -> s {_lsnrsKnownDependencyCount = a})

-- | -- | The response status code.
lsnrsResponseStatus :: Lens' ListServerNeighborsResponse Int
lsnrsResponseStatus =
  lens _lsnrsResponseStatus (\s a -> s {_lsnrsResponseStatus = a})

-- | List of distinct servers that are one hop away from the given server.
lsnrsNeighbors :: Lens' ListServerNeighborsResponse [NeighborConnectionDetail]
lsnrsNeighbors =
  lens _lsnrsNeighbors (\s a -> s {_lsnrsNeighbors = a}) . _Coerce
