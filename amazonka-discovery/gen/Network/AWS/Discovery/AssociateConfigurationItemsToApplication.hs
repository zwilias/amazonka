...
acitaApplicationConfigurationId ::
     Lens' AssociateConfigurationItemsToApplication Text
acitaApplicationConfigurationId =
  lens
    _acitaApplicationConfigurationId
    (\s a -> s {_acitaApplicationConfigurationId = a})

-- | The ID of each configuration item to be associated with an application.
acitaConfigurationIds :: Lens' AssociateConfigurationItemsToApplication [Text]
acitaConfigurationIds =
  lens _acitaConfigurationIds (\s a -> s {_acitaConfigurationIds = a}) . _Coerce

instance AWSRequest AssociateConfigurationItemsToApplication where
  type Rs AssociateConfigurationItemsToApplication = AssociateConfigurationItemsToApplicationResponse
...
instance ToHeaders AssociateConfigurationItemsToApplication where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSPoseidonService_V2015_11_01.AssociateConfigurationItemsToApplication" :: ByteString)
...
acitarsResponseStatus ::
     Lens' AssociateConfigurationItemsToApplicationResponse Int
acitarsResponseStatus =
  lens _acitarsResponseStatus (\s a -> s {_acitarsResponseStatus = a})
