...
instance FromText AgentStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AgentStatus from value: '" <>
        e <>
...
instance FromText ConfigurationItemType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConfigurationItemType from value: '" <>
        e <> "'. Accepted values: application, connection, process, server"
...
instance FromText ExportDataFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExportDataFormat from value: '" <>
        e <> "'. Accepted values: csv, graphml"
...
instance FromText OrderString where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrderString from value: '" <>
