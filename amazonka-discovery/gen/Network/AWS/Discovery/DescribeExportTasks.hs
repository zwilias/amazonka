...
detExportIds =
  lens _detExportIds (\s a -> s {_detExportIds = a}) . _Default . _Coerce
...
instance AWSRequest DescribeExportTasks where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeExportTasksResponse' <$> (x .?> "nextToken") <*>
...
instance ToHeaders DescribeExportTasks where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSPoseidonService_V2015_11_01.DescribeExportTasks" :: ByteString)
...
detrsExportsInfo =
  lens _detrsExportsInfo (\s a -> s {_detrsExportsInfo = a}) .
  _Default . _Coerce
