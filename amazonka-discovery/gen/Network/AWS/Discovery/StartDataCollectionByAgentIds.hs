...
instance AWSRequest StartDataCollectionByAgentIds where
  type Rs StartDataCollectionByAgentIds = StartDataCollectionByAgentIdsResponse
...
instance ToHeaders StartDataCollectionByAgentIds where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSPoseidonService_V2015_11_01.StartDataCollectionByAgentIds" :: ByteString)
...
srsAgentsConfigurationStatus ::
     Lens' StartDataCollectionByAgentIdsResponse [AgentConfigurationStatus]
srsAgentsConfigurationStatus =
  lens
    _srsAgentsConfigurationStatus
    (\s a -> s {_srsAgentsConfigurationStatus = a}) .
  _Default . _Coerce
