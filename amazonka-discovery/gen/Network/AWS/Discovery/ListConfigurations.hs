...
lcConfigurationType =
  lens _lcConfigurationType (\s a -> s {_lcConfigurationType = a})
...
instance AWSRequest ListConfigurations where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListConfigurationsResponse' <$> (x .?> "configurations" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListConfigurations where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSPoseidonService_V2015_11_01.ListConfigurations" :: ByteString)
...
lcrsConfigurations =
  lens _lcrsConfigurations (\s a -> s {_lcrsConfigurations = a}) .
  _Default . _Coerce
