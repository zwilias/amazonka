...
diInstanceIds =
  lens _diInstanceIds (\s a -> s {_diInstanceIds = a}) . _Default . _Coerce
...
instance AWSRequest DescribeInstances where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeInstancesResponse' <$> (x .?> "Instances" .!@ mempty) <*>
         (pure (fromEnum s)))
...
dirsInstances =
  lens _dirsInstances (\s a -> s {_dirsInstances = a}) . _Default . _Coerce
