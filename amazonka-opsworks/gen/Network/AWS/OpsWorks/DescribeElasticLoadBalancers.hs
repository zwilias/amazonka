...
delbLayerIds =
  lens _delbLayerIds (\s a -> s {_delbLayerIds = a}) . _Default . _Coerce
...
instance ToJSON DescribeElasticLoadBalancers where
  toJSON DescribeElasticLoadBalancers' {..} =
    object
      (catMaybes
         [("LayerIds" .=) <$> _delbLayerIds, ("StackId" .=) <$> _delbStackId])
...
delbrsElasticLoadBalancers ::
     Lens' DescribeElasticLoadBalancersResponse [ElasticLoadBalancer]
delbrsElasticLoadBalancers =
  lens _delbrsElasticLoadBalancers (\s a -> s {_delbrsElasticLoadBalancers = a}) .
  _Default . _Coerce
