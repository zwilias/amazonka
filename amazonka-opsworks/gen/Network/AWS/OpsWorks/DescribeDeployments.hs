...
ddDeploymentIds =
  lens _ddDeploymentIds (\s a -> s {_ddDeploymentIds = a}) . _Default . _Coerce
...
instance AWSRequest DescribeDeployments where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeDeploymentsResponse' <$> (x .?> "Deployments" .!@ mempty) <*>
         (pure (fromEnum s)))
...
ddrsDeployments =
  lens _ddrsDeployments (\s a -> s {_ddrsDeployments = a}) . _Default . _Coerce
