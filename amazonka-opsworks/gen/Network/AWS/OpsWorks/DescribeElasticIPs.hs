...
instance AWSRequest DescribeElasticIPs where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeElasticIPsResponse' <$> (x .?> "ElasticIps" .!@ mempty) <*>
         (pure (fromEnum s)))
...
deirsElasticIPs =
  lens _deirsElasticIPs (\s a -> s {_deirsElasticIPs = a}) . _Default . _Coerce

-- | -- | The response status code.
deirsResponseStatus :: Lens' DescribeElasticIPsResponse Int
deirsResponseStatus =
  lens _deirsResponseStatus (\s a -> s {_deirsResponseStatus = a})
