...
caEnvironment =
  lens _caEnvironment (\s a -> s {_caEnvironment = a}) . _Default . _Coerce
...
caDataSources =
  lens _caDataSources (\s a -> s {_caDataSources = a}) . _Default . _Coerce
...
caAttributes =
  lens _caAttributes (\s a -> s {_caAttributes = a}) . _Default . _Map
...
instance AWSRequest CreateApp where
  ...
  response =
    receiveJSON
      (\s h x -> CreateAppResponse' <$> (x .?> "AppId") <*> (pure (fromEnum s)))
