...
dvVolumeIds =
  lens _dvVolumeIds (\s a -> s {_dvVolumeIds = a}) . _Default . _Coerce
...
instance AWSRequest DescribeVolumes where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeVolumesResponse' <$> (x .?> "Volumes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeVolumes where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("OpsWorks_20130218.DescribeVolumes" :: ByteString)
...
dvrsVolumes =
  lens _dvrsVolumes (\s a -> s {_dvrsVolumes = a}) . _Default . _Coerce
