...
csDefaultAvailabilityZone =
  lens _csDefaultAvailabilityZone (\s a -> s {_csDefaultAvailabilityZone = a})

-- | One or more user-defined key-value pairs to be added to the stack attributes.
csAttributes :: Lens' CreateStack (HashMap StackAttributesKeys (Maybe Text))
csAttributes =
  lens _csAttributes (\s a -> s {_csAttributes = a}) . _Default . _Map
  ...
  toJSON CreateStack' {..} =
    object
      (catMaybes
         ...
         , Just ("DefaultInstanceProfileArn" .= _csDefaultInstanceProfileARN)
         ])
