...
instance AWSRequest DescribePermissions where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribePermissionsResponse' <$> (x .?> "Permissions" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DescribePermissions where
  toJSON DescribePermissions' {..} =
    object
      (catMaybes
         [("IamUserArn" .=) <$> _dpIAMUserARN, ("StackId" .=) <$> _dpStackId])
...
dprsPermissions =
  lens _dprsPermissions (\s a -> s {_dprsPermissions = a}) . _Default . _Coerce
