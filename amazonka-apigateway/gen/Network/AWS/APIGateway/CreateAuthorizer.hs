...
caIdentityValidationExpression =
  lens
    ...
    (\s a -> s {_caIdentityValidationExpression = a})

-- | A list of the Amazon Cognito user pool ARNs for the @COGNITO_USER_POOLS@ authorizer. Each element is of this format: @arn:aws:cognito-idp:{region}:{account_id}:userpool/{user_pool_id}@ . For a @TOKEN@ or @REQUEST@ authorizer, this is not defined.
caProviderARNs :: Lens' CreateAuthorizer [Text]
caProviderARNs =
  lens _caProviderARNs (\s a -> s {_caProviderARNs = a}) . _Default . _Coerce
...
caAuthorizerResultTtlInSeconds =
  lens
    ...
    (\s a -> s {_caAuthorizerResultTtlInSeconds = a})
...
caAuthorizerCredentials =
  lens _caAuthorizerCredentials (\s a -> s {_caAuthorizerCredentials = a})
