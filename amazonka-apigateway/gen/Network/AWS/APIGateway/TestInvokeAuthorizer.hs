...
tiaPathWithQueryString =
  lens _tiaPathWithQueryString (\s a -> s {_tiaPathWithQueryString = a})
...
tiaAdditionalContext =
  lens _tiaAdditionalContext (\s a -> s {_tiaAdditionalContext = a}) .
  _Default . _Map

-- | A key-value map of stage variables to simulate an invocation on a deployed 'Stage' .
tiaStageVariables :: Lens' TestInvokeAuthorizer (HashMap Text Text)
tiaStageVariables =
  lens _tiaStageVariables (\s a -> s {_tiaStageVariables = a}) . _Default . _Map
...
instance AWSRequest TestInvokeAuthorizer where
  ...
  response =
    receiveJSON
      (\s h x ->
         TestInvokeAuthorizerResponse' <$> (x .?> "log") <*>
         (x .?> "principalId") <*>
         (x .?> "latency") <*>
...
instance ToPath TestInvokeAuthorizer where
  toPath TestInvokeAuthorizer' {..} =
    mconcat
      ["/restapis/", toBS _tiaRestAPIId, "/authorizers/", toBS _tiaAuthorizerId]
...
tiarsAuthorization =
  lens _tiarsAuthorization (\s a -> s {_tiarsAuthorization = a}) .
  _Default . _Map
