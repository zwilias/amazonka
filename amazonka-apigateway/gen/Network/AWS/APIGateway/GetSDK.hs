...
gsdkParameters =
  lens _gsdkParameters (\s a -> s {_gsdkParameters = a}) . _Default . _Map
...
instance AWSRequest GetSDK where
  ...
  response =
    receiveBytes
      (\s h x ->
         GetSDKResponse' <$> (pure (Just x)) <*> (h .#? "Content-Disposition") <*>
         (h .#? "Content-Type") <*>
         (pure (fromEnum s)))
...
gsdkrsContentDisposition =
  lens _gsdkrsContentDisposition (\s a -> s {_gsdkrsContentDisposition = a})
...
gsdkrsResponseStatus =
  lens _gsdkrsResponseStatus (\s a -> s {_gsdkrsResponseStatus = a})
