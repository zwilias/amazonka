...
instance FromText APIKeySourceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing APIKeySourceType from value: '" <>
        e <> "'. Accepted values: authorizer, header"
...
instance FromText APIKeysFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing APIKeysFormat from value: '" <>
        e <> "'. Accepted values: csv"
...
instance FromText AuthorizerType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuthorizerType from value: '" <>
        e <> "'. Accepted values: cognito_user_pools, request, token"
...
instance FromText CacheClusterSize where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CacheClusterSize from value: '" <>
        e <> "'. Accepted values: 0.5, 118, 13.5, 1.6, 237, 28.4, 58.2, 6.1"
...
instance FromText CacheClusterStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CacheClusterStatus from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: internet, vpc_link"
...
instance FromText ContentHandlingStrategy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ContentHandlingStrategy from value: '" <>
        e <> "'. Accepted values: convert_to_binary, convert_to_text"
...
instance FromText DocumentationPartType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DocumentationPartType from value: '" <>
        e <>
...
instance FromText EndpointType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EndpointType from value: '" <>
        e <> "'. Accepted values: edge, regional"
...
instance FromText GatewayResponseType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing GatewayResponseType from value: '" <>
        e <>
...
instance FromText IntegrationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IntegrationType from value: '" <>
        e <> "'. Accepted values: aws, aws_proxy, http, http_proxy, mock"
...
instance FromText LocationStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LocationStatusType from value: '" <>
        e <> "'. Accepted values: documented, undocumented"
...
instance FromText Op where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Op from value: '" <>
        e <> "'. Accepted values: add, copy, move, remove, replace, test"
...
instance FromText PutMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PutMode from value: '" <>
        e <> "'. Accepted values: merge, overwrite"
...
instance FromText QuotaPeriodType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing QuotaPeriodType from value: '" <>
        e <> "'. Accepted values: day, month, week"
...
instance FromText UnauthorizedCacheControlHeaderStrategy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UnauthorizedCacheControlHeaderStrategy from value: '" <>
        e <>
...
instance FromText VPCLinkStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VPCLinkStatus from value: '" <>
        e <> "'. Accepted values: available, deleting, failed, pending"
