...
geParameters =
  lens _geParameters (\s a -> s {_geParameters = a}) . _Default . _Map
...
instance ToHeaders GetExport where
  toHeaders GetExport' {..} =
    mconcat
      ["Accept" =# _geAccepts, "Accept" =# ("application/json" :: ByteString)]
...
gersContentDisposition =
  lens _gersContentDisposition (\s a -> s {_gersContentDisposition = a})
