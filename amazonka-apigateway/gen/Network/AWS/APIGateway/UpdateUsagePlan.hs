...
uupPatchOperations =
  lens _uupPatchOperations (\s a -> s {_uupPatchOperations = a}) .
  _Default . _Coerce
...
instance ToJSON UpdateUsagePlan where
  toJSON UpdateUsagePlan' {..} =
    object (catMaybes [("patchOperations" .=) <$> _uupPatchOperations])

instance ToPath UpdateUsagePlan where
  toPath UpdateUsagePlan' {..} = mconcat ["/usageplans/", toBS _uupUsagePlanId]
