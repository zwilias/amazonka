...
urPatchOperations =
  lens _urPatchOperations (\s a -> s {_urPatchOperations = a}) .
  _Default . _Coerce
...
instance ToJSON UpdateResource where
  toJSON UpdateResource' {..} =
    object (catMaybes [("patchOperations" .=) <$> _urPatchOperations])

instance ToPath UpdateResource where
  toPath UpdateResource' {..} =
    mconcat ["/restapis/", toBS _urRestAPIId, "/resources/", toBS _urResourceId]
