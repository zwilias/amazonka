...
uuPatchOperations =
  lens _uuPatchOperations (\s a -> s {_uuPatchOperations = a}) .
  _Default . _Coerce
...
instance ToJSON UpdateUsage where
  toJSON UpdateUsage' {..} =
    object (catMaybes [("patchOperations" .=) <$> _uuPatchOperations])

instance ToPath UpdateUsage where
  toPath UpdateUsage' {..} =
    mconcat
      ["/usageplans/", toBS _uuUsagePlanId, "/keys/", toBS _uuKeyId, "/usage"]
