...
gtvPaginationToken =
  lens _gtvPaginationToken (\s a -> s {_gtvPaginationToken = a})
...
instance AWSPager GetTagValues where
  page rq rs
    ...
    | otherwise = Just $ rq & gtvPaginationToken .~ rs ^. gtvrsPaginationToken
...
instance AWSRequest GetTagValues where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetTagValuesResponse' <$> (x .?> "PaginationToken") <*>
         (x .?> "TagValues" .!@ mempty) <*>
         (pure (fromEnum s)))
...
gtvrsPaginationToken =
  lens _gtvrsPaginationToken (\s a -> s {_gtvrsPaginationToken = a})

-- | A list of all tag values for the specified key in the AWS account.
gtvrsTagValues :: Lens' GetTagValuesResponse [Text]
gtvrsTagValues =
  lens _gtvrsTagValues (\s a -> s {_gtvrsTagValues = a}) . _Default . _Coerce

-- | -- | The response status code.
gtvrsResponseStatus :: Lens' GetTagValuesResponse Int
gtvrsResponseStatus =
  lens _gtvrsResponseStatus (\s a -> s {_gtvrsResponseStatus = a})
