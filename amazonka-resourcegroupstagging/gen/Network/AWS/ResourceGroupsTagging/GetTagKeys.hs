...
gtkPaginationToken =
  lens _gtkPaginationToken (\s a -> s {_gtkPaginationToken = a})

instance AWSPager GetTagKeys where
  page rq rs
    | stop (rs ^. gtkrsPaginationToken) = Nothing
    | stop (rs ^. gtkrsTagKeys) = Nothing
    | otherwise = Just $ rq & gtkPaginationToken .~ rs ^. gtkrsPaginationToken
...
instance AWSRequest GetTagKeys where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetTagKeysResponse' <$> (x .?> "PaginationToken") <*>
         (x .?> "TagKeys" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON GetTagKeys where
  toJSON GetTagKeys' {..} =
    object (catMaybes [("PaginationToken" .=) <$> _gtkPaginationToken])
...
gtkrsPaginationToken =
  lens _gtkrsPaginationToken (\s a -> s {_gtkrsPaginationToken = a})

-- | A list of all tag keys in the AWS account.
gtkrsTagKeys :: Lens' GetTagKeysResponse [Text]
gtkrsTagKeys =
  lens _gtkrsTagKeys (\s a -> s {_gtkrsTagKeys = a}) . _Default . _Coerce

-- | -- | The response status code.
gtkrsResponseStatus :: Lens' GetTagKeysResponse Int
gtkrsResponseStatus =
  lens _gtkrsResponseStatus (\s a -> s {_gtkrsResponseStatus = a})
