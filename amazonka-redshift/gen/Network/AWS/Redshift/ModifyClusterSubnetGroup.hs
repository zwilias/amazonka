...
mcsgClusterSubnetGroupName =
  lens _mcsgClusterSubnetGroupName (\s a -> s {_mcsgClusterSubnetGroupName = a})
...
instance AWSRequest ModifyClusterSubnetGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ModifyClusterSubnetGroupResponse' <$> (x .@? "ClusterSubnetGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
mcsgrsClusterSubnetGroup ::
     Lens' ModifyClusterSubnetGroupResponse (Maybe ClusterSubnetGroup)
mcsgrsClusterSubnetGroup =
  lens _mcsgrsClusterSubnetGroup (\s a -> s {_mcsgrsClusterSubnetGroup = a})
