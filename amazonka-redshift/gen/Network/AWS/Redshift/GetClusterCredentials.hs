...
gccDBGroups =
  lens _gccDBGroups (\s a -> s {_gccDBGroups = a}) . _Default . _Coerce
...
gccDurationSeconds =
  lens _gccDurationSeconds (\s a -> s {_gccDurationSeconds = a})
...
gccClusterIdentifier =
  lens _gccClusterIdentifier (\s a -> s {_gccClusterIdentifier = a})
...
instance AWSRequest GetClusterCredentials where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         GetClusterCredentialsResponse' <$> (x .@? "DbUser") <*>
         (x .@? "Expiration") <*>
         (x .@? "DbPassword") <*>
         (pure (fromEnum s)))
...
gccrsExpiration =
  lens _gccrsExpiration (\s a -> s {_gccrsExpiration = a}) . mapping _Time

-- | A temporary password that authorizes the user name returned by @DbUser@ to log on to the database @DbName@ .
gccrsDBPassword :: Lens' GetClusterCredentialsResponse (Maybe Text)
gccrsDBPassword =
  lens _gccrsDBPassword (\s a -> s {_gccrsDBPassword = a}) . mapping _Sensitive

-- | -- | The response status code.
gccrsResponseStatus :: Lens' GetClusterCredentialsResponse Int
gccrsResponseStatus =
  lens _gccrsResponseStatus (\s a -> s {_gccrsResponseStatus = a})
