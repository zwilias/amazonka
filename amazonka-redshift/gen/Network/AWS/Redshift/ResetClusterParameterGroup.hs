...
rcpgParameters =
  lens _rcpgParameters (\s a -> s {_rcpgParameters = a}) . _Default . _Coerce
...
instance AWSRequest ResetClusterParameterGroup where
  type Rs ResetClusterParameterGroup = ClusterParameterGroupNameMessage
  request = postQuery redshift
  response =
    receiveXMLWrapper "ResetClusterParameterGroupResult" (\s h x -> parseXML x)
...
instance ToQuery ResetClusterParameterGroup where
  toQuery ResetClusterParameterGroup' {..} =
    mconcat
      ...
      , "Parameters" =: toQuery (toQueryList "Parameter" <$> _rcpgParameters)
