...
prnoReservedNodeOfferingId =
  lens _prnoReservedNodeOfferingId (\s a -> s {_prnoReservedNodeOfferingId = a})
...
instance AWSRequest PurchaseReservedNodeOffering where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         PurchaseReservedNodeOfferingResponse' <$> (x .@? "ReservedNode") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
prnorsReservedNode ::
     Lens' PurchaseReservedNodeOfferingResponse (Maybe ReservedNode)
prnorsReservedNode =
...
prnorsResponseStatus =
  lens _prnorsResponseStatus (\s a -> s {_prnorsResponseStatus = a})
