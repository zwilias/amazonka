...
creClusterSecurityGroupName =
  lens
    ...
    (\s a -> s {_creClusterSecurityGroupName = a})
...
instance AWSRequest CreateClusterSecurityGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateClusterSecurityGroupResponse' <$> (x .@? "ClusterSecurityGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
crsClusterSecurityGroup ::
     Lens' CreateClusterSecurityGroupResponse (Maybe ClusterSecurityGroup)
crsClusterSecurityGroup =
  lens _crsClusterSecurityGroup (\s a -> s {_crsClusterSecurityGroup = a})
