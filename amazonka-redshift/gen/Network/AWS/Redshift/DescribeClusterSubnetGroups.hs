...
dcsgsTagValues =
  lens _dcsgsTagValues (\s a -> s {_dcsgsTagValues = a}) . _Default . _Coerce

-- | A tag key or keys for which you want to return all matching cluster subnet groups that are associated with the specified key or keys. For example, suppose that you have subnet groups that are tagged with keys called @owner@ and @environment@ . If you specify both of these tag keys in the request, Amazon Redshift returns a response with the subnet groups that have either or both of these tag keys associated with them.
dcsgsTagKeys :: Lens' DescribeClusterSubnetGroups [Text]
dcsgsTagKeys =
  lens _dcsgsTagKeys (\s a -> s {_dcsgsTagKeys = a}) . _Default . _Coerce
...
dcsgsClusterSubnetGroupName =
  lens
    ...
    (\s a -> s {_dcsgsClusterSubnetGroupName = a})
...
instance ToQuery DescribeClusterSubnetGroups where
  toQuery DescribeClusterSubnetGroups' {..} =
    mconcat
      ...
      , "TagValues" =: toQuery (toQueryList "TagValue" <$> _dcsgsTagValues)
...
dcsgrsClusterSubnetGroups ::
     Lens' DescribeClusterSubnetGroupsResponse [ClusterSubnetGroup]
dcsgrsClusterSubnetGroups =
  lens _dcsgrsClusterSubnetGroups (\s a -> s {_dcsgrsClusterSubnetGroups = a}) .
  _Default . _Coerce
...
dcsgrsResponseStatus =
  lens _dcsgrsResponseStatus (\s a -> s {_dcsgrsResponseStatus = a})
