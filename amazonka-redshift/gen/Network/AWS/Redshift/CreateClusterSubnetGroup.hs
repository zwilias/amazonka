...
ccsgClusterSubnetGroupName =
  lens _ccsgClusterSubnetGroupName (\s a -> s {_ccsgClusterSubnetGroupName = a})
...
instance AWSRequest CreateClusterSubnetGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateClusterSubnetGroupResponse' <$> (x .@? "ClusterSubnetGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
ccsgrsClusterSubnetGroup ::
     Lens' CreateClusterSubnetGroupResponse (Maybe ClusterSubnetGroup)
ccsgrsClusterSubnetGroup =
  lens _ccsgrsClusterSubnetGroup (\s a -> s {_ccsgrsClusterSubnetGroup = a})
