...
mcEnhancedVPCRouting =
  lens _mcEnhancedVPCRouting (\s a -> s {_mcEnhancedVPCRouting = a})
...
mcPubliclyAccessible =
  lens _mcPubliclyAccessible (\s a -> s {_mcPubliclyAccessible = a})
...
instance AWSRequest ModifyCluster where
  ...
  response =
    receiveXMLWrapper
      "ModifyClusterResult"
      (\s h x ->
         ModifyClusterResponse' <$> (x .@? "Cluster") <*> (pure (fromEnum s)))
  ...
  toQuery ModifyCluster' {..} =
    mconcat
      ...
      , "ClusterSecurityGroups" =:
        toQuery
          (toQueryList "ClusterSecurityGroupName" <$> _mcClusterSecurityGroups)
      ...
      , "VpcSecurityGroupIds" =:
        toQuery (toQueryList "VpcSecurityGroupId" <$> _mcVPCSecurityGroupIds)
...
mcrsResponseStatus =
  lens _mcrsResponseStatus (\s a -> s {_mcrsResponseStatus = a})
