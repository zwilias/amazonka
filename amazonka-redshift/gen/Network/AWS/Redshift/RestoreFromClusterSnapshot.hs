...
rfcsEnhancedVPCRouting =
  lens _rfcsEnhancedVPCRouting (\s a -> s {_rfcsEnhancedVPCRouting = a})

-- | Reserved.
rfcsAdditionalInfo :: Lens' RestoreFromClusterSnapshot (Maybe Text)
rfcsAdditionalInfo =
  lens _rfcsAdditionalInfo (\s a -> s {_rfcsAdditionalInfo = a})
...
rfcsClusterSecurityGroups =
  lens _rfcsClusterSecurityGroups (\s a -> s {_rfcsClusterSecurityGroups = a}) .
  _Default . _Coerce

-- | The number of days that automated snapshots are retained. If the value is 0, automated snapshots are disabled. Even if automated snapshots are disabled, you can still create manual snapshots when you want with 'CreateClusterSnapshot' .  Default: The value selected for the cluster from which the snapshot was taken. Constraints: Must be a value from 0 to 35.
rfcsAutomatedSnapshotRetentionPeriod ::
     Lens' RestoreFromClusterSnapshot (Maybe Int)
rfcsAutomatedSnapshotRetentionPeriod =
  lens
...
rfcsClusterSubnetGroupName =
  lens _rfcsClusterSubnetGroupName (\s a -> s {_rfcsClusterSubnetGroupName = a})

-- | Specifies the name of the HSM client certificate the Amazon Redshift cluster uses to retrieve the data encryption keys stored in an HSM.
rfcsHSMClientCertificateIdentifier ::
     Lens' RestoreFromClusterSnapshot (Maybe Text)
rfcsHSMClientCertificateIdentifier =
  lens
...
rfcsIAMRoles =
  lens _rfcsIAMRoles (\s a -> s {_rfcsIAMRoles = a}) . _Default . _Coerce
...
instance AWSRequest RestoreFromClusterSnapshot where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         RestoreFromClusterSnapshotResponse' <$> (x .@? "Cluster") <*>
         (pure (fromEnum s)))
  ...
  toQuery RestoreFromClusterSnapshot' {..} =
    mconcat
      ...
      , "ClusterSecurityGroups" =:
        toQuery
          (toQueryList "ClusterSecurityGroupName" <$> _rfcsClusterSecurityGroups)
      ...
      , "VpcSecurityGroupIds" =:
        toQuery (toQueryList "VpcSecurityGroupId" <$> _rfcsVPCSecurityGroupIds)
...
rfcsrsResponseStatus =
  lens _rfcsrsResponseStatus (\s a -> s {_rfcsrsResponseStatus = a})
