...
dscgsTagValues =
  lens _dscgsTagValues (\s a -> s {_dscgsTagValues = a}) . _Default . _Coerce

-- | A tag key or keys for which you want to return all matching resources that are associated with the specified key or keys. For example, suppose that you have resources tagged with keys called @owner@ and @environment@ . If you specify both of these tag keys in the request, Amazon Redshift returns a response with all resources that have either or both of these tag keys associated with them.
dscgsTagKeys :: Lens' DescribeSnapshotCopyGrants [Text]
dscgsTagKeys =
  lens _dscgsTagKeys (\s a -> s {_dscgsTagKeys = a}) . _Default . _Coerce
...
dscgsSnapshotCopyGrantName =
  lens _dscgsSnapshotCopyGrantName (\s a -> s {_dscgsSnapshotCopyGrantName = a})

instance AWSRequest DescribeSnapshotCopyGrants where
  type Rs DescribeSnapshotCopyGrants = DescribeSnapshotCopyGrantsResponse
...
instance ToQuery DescribeSnapshotCopyGrants where
  toQuery DescribeSnapshotCopyGrants' {..} =
    mconcat
      ...
      , "TagValues" =: toQuery (toQueryList "TagValue" <$> _dscgsTagValues)
...
dscgrsSnapshotCopyGrants ::
     Lens' DescribeSnapshotCopyGrantsResponse [SnapshotCopyGrant]
dscgrsSnapshotCopyGrants =
  lens _dscgrsSnapshotCopyGrants (\s a -> s {_dscgrsSnapshotCopyGrants = a}) .
  _Default . _Coerce
...
dscgrsResponseStatus =
  lens _dscgrsResponseStatus (\s a -> s {_dscgrsResponseStatus = a})
