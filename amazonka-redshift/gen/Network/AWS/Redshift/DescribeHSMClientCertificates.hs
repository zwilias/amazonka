...
dhccTagValues =
  lens _dhccTagValues (\s a -> s {_dhccTagValues = a}) . _Default . _Coerce

-- | A tag key or keys for which you want to return all matching HSM client certificates that are associated with the specified key or keys. For example, suppose that you have HSM client certificates that are tagged with keys called @owner@ and @environment@ . If you specify both of these tag keys in the request, Amazon Redshift returns a response with the HSM client certificates that have either or both of these tag keys associated with them.
dhccTagKeys :: Lens' DescribeHSMClientCertificates [Text]
dhccTagKeys =
  lens _dhccTagKeys (\s a -> s {_dhccTagKeys = a}) . _Default . _Coerce

-- | The identifier of a specific HSM client certificate for which you want information. If no identifier is specified, information is returned for all HSM client certificates owned by your AWS customer account.
dhccHSMClientCertificateIdentifier ::
     Lens' DescribeHSMClientCertificates (Maybe Text)
dhccHSMClientCertificateIdentifier =
  lens
...
instance AWSRequest DescribeHSMClientCertificates where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeHSMClientCertificatesResponse' <$> (x .@? "Marker") <*>
...
dhccrsHSMClientCertificates ::
     Lens' DescribeHSMClientCertificatesResponse [HSMClientCertificate]
dhccrsHSMClientCertificates =
  lens
