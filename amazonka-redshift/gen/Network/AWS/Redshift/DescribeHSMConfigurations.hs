...
dhsmcTagValues =
  lens _dhsmcTagValues (\s a -> s {_dhsmcTagValues = a}) . _Default . _Coerce
...
dhsmcHSMConfigurationIdentifier =
  lens
    ...
    (\s a -> s {_dhsmcHSMConfigurationIdentifier = a})

-- | A tag key or keys for which you want to return all matching HSM configurations that are associated with the specified key or keys. For example, suppose that you have HSM configurations that are tagged with keys called @owner@ and @environment@ . If you specify both of these tag keys in the request, Amazon Redshift returns a response with the HSM configurations that have either or both of these tag keys associated with them.
dhsmcTagKeys :: Lens' DescribeHSMConfigurations [Text]
dhsmcTagKeys =
  lens _dhsmcTagKeys (\s a -> s {_dhsmcTagKeys = a}) . _Default . _Coerce
...
instance AWSRequest DescribeHSMConfigurations where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeHSMConfigurationsResponse' <$> (x .@? "Marker") <*>
...
instance ToQuery DescribeHSMConfigurations where
  toQuery DescribeHSMConfigurations' {..} =
    mconcat
      ...
      , "TagValues" =: toQuery (toQueryList "TagValue" <$> _dhsmcTagValues)
...
dhcrsHSMConfigurations ::
     Lens' DescribeHSMConfigurationsResponse [HSMConfiguration]
dhcrsHSMConfigurations =
...
dhcrsResponseStatus =
  lens _dhcrsResponseStatus (\s a -> s {_dhcrsResponseStatus = a})
