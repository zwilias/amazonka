...
mcirRemoveIAMRoles =
  lens _mcirRemoveIAMRoles (\s a -> s {_mcirRemoveIAMRoles = a}) .
  _Default . _Coerce

-- | Zero or more IAM roles to associate with the cluster. The roles must be in their Amazon Resource Name (ARN) format. You can associate up to 10 IAM roles with a single cluster in a single request.
mcirAddIAMRoles :: Lens' ModifyClusterIAMRoles [Text]
mcirAddIAMRoles =
  lens _mcirAddIAMRoles (\s a -> s {_mcirAddIAMRoles = a}) . _Default . _Coerce

-- | The unique identifier of the cluster for which you want to associate or disassociate IAM roles.
mcirClusterIdentifier :: Lens' ModifyClusterIAMRoles Text
mcirClusterIdentifier =
  lens _mcirClusterIdentifier (\s a -> s {_mcirClusterIdentifier = a})
...
instance AWSRequest ModifyClusterIAMRoles where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ModifyClusterIAMRolesResponse' <$> (x .@? "Cluster") <*>
         (pure (fromEnum s)))
...
instance ToQuery ModifyClusterIAMRoles where
  toQuery ModifyClusterIAMRoles' {..} =
    mconcat
      ...
      , "AddIamRoles" =: toQuery (toQueryList "IamRoleArn" <$> _mcirAddIAMRoles)
...
mcirrsResponseStatus =
  lens _mcirrsResponseStatus (\s a -> s {_mcirrsResponseStatus = a})
