...
instance AWSRequest ModifySnapshotCopyRetentionPeriod where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ModifySnapshotCopyRetentionPeriodResponse' <$> (x .@? "Cluster") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
mscrprsCluster ::
     Lens' ModifySnapshotCopyRetentionPeriodResponse (Maybe Cluster)
...
mscrprsResponseStatus =
  lens _mscrprsResponseStatus (\s a -> s {_mscrprsResponseStatus = a})
