...
instance AWSRequest DescribeObject where
  ...
  response =
    receiveEmpty
      (\s h x ->
         DescribeObjectResponse' <$> (h .#? "ETag") <*> (h .#? "Content-Length") <*>
         (h .#? "Cache-Control") <*>
         (h .#? "Last-Modified") <*>
         (h .#? "Content-Type") <*>
         (pure (fromEnum s)))
...
drsContentLength =
  lens _drsContentLength (\s a -> s {_drsContentLength = a}) . mapping _Nat
...
drsLastModified =
  lens _drsLastModified (\s a -> s {_drsLastModified = a}) . mapping _Time
