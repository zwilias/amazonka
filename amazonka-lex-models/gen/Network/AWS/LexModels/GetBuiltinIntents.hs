...
gbiSignatureContains =
  lens _gbiSignatureContains (\s a -> s {_gbiSignatureContains = a})

-- | The maximum number of intents to return in the response. The default is 10.
gbiMaxResults :: Lens' GetBuiltinIntents (Maybe Natural)
gbiMaxResults =
  lens _gbiMaxResults (\s a -> s {_gbiMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetBuiltinIntents where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBuiltinIntentsResponse' <$> (x .?> "intents" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
