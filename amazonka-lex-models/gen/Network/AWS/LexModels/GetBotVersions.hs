...
gbvMaxResults =
  lens _gbvMaxResults (\s a -> s {_gbvMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetBotVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBotVersionsResponse' <$> (x .?> "bots" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToPath GetBotVersions where
  toPath GetBotVersions' {..} = mconcat ["/bots/", toBS _gbvName, "/versions/"]

instance ToQuery GetBotVersions where
  toQuery GetBotVersions' {..} =
    mconcat ["nextToken" =: _gbvNextToken, "maxResults" =: _gbvMaxResults]
