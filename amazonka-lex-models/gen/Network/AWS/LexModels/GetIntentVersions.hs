...
givMaxResults =
  lens _givMaxResults (\s a -> s {_givMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetIntentVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetIntentVersionsResponse' <$> (x .?> "intents" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery GetIntentVersions where
  toQuery GetIntentVersions' {..} =
    mconcat ["nextToken" =: _givNextToken, "maxResults" =: _givMaxResults]
...
givrsIntents =
  lens _givrsIntents (\s a -> s {_givrsIntents = a}) . _Default . _Coerce
...
givrsResponseStatus =
  lens _givrsResponseStatus (\s a -> s {_givrsResponseStatus = a})
