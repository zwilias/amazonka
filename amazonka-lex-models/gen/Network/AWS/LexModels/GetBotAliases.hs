...
gbaMaxResults =
  lens _gbaMaxResults (\s a -> s {_gbaMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetBotAliases where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBotAliasesResponse' <$> (x .?> "nextToken") <*>
         (x .?> "BotAliases" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToPath GetBotAliases where
  toPath GetBotAliases' {..} = mconcat ["/bots/", toBS _gbaBotName, "/aliases/"]
...
gbarsBotAliases =
  lens _gbarsBotAliases (\s a -> s {_gbarsBotAliases = a}) . _Default . _Coerce
