...
gbstSignatureContains =
  lens _gbstSignatureContains (\s a -> s {_gbstSignatureContains = a})

-- | The maximum number of slot types to return in the response. The default is 10.
gbstMaxResults :: Lens' GetBuiltinSlotTypes (Maybe Natural)
gbstMaxResults =
  lens _gbstMaxResults (\s a -> s {_gbstMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetBuiltinSlotTypes where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBuiltinSlotTypesResponse' <$> (x .?> "nextToken") <*>
         (x .?> "slotTypes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
gbstrsSlotTypes =
  lens _gbstrsSlotTypes (\s a -> s {_gbstrsSlotTypes = a}) . _Default . _Coerce

-- | -- | The response status code.
gbstrsResponseStatus :: Lens' GetBuiltinSlotTypesResponse Int
gbstrsResponseStatus =
  lens _gbstrsResponseStatus (\s a -> s {_gbstrsResponseStatus = a})
