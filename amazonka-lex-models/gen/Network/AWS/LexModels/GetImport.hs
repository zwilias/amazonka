...
instance AWSRequest GetImport where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetImportResponse' <$> (x .?> "failureReason" .!@ mempty) <*>
         (x .?> "resourceType") <*>
         (x .?> "importId") <*>
         (x .?> "createdDate") <*>
         (x .?> "name") <*>
...
girsFailureReason =
  lens _girsFailureReason (\s a -> s {_girsFailureReason = a}) .
  _Default . _Coerce
...
girsCreatedDate =
  lens _girsCreatedDate (\s a -> s {_girsCreatedDate = a}) . mapping _Time
