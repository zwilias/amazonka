...
instance AWSRequest GetBot where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBotResponse' <$> (x .?> "failureReason") <*> (x .?> "status") <*>
         (x .?> "abortStatement") <*>
         (x .?> "intents" .!@ mempty) <*>
         (x .?> "checksum") <*>
         (x .?> "locale") <*>
...
instance ToPath GetBot where
  toPath GetBot' {..} =
    mconcat ["/bots/", toBS _gbName, "/versions/", toBS _gbVersionOrAlias]
...
gbrsAbortStatement =
  lens _gbrsAbortStatement (\s a -> s {_gbrsAbortStatement = a})

-- | An array of @intent@ objects. For more information, see 'PutBot' .
gbrsIntents :: Lens' GetBotResponse [Intent]
gbrsIntents =
  lens _gbrsIntents (\s a -> s {_gbrsIntents = a}) . _Default . _Coerce
...
gbrsCreatedDate =
  lens _gbrsCreatedDate (\s a -> s {_gbrsCreatedDate = a}) . mapping _Time
...
gbrsIdleSessionTTLInSeconds =
  lens
    ...
    (\s a -> s {_gbrsIdleSessionTTLInSeconds = a}) .
  mapping _Nat

-- | The message Amazon Lex uses when it doesn't understand the user's request. For more information, see 'PutBot' .
gbrsClarificationPrompt :: Lens' GetBotResponse (Maybe Prompt)
gbrsClarificationPrompt =
  lens _gbrsClarificationPrompt (\s a -> s {_gbrsClarificationPrompt = a})
...
gbrsLastUpdatedDate =
  lens _gbrsLastUpdatedDate (\s a -> s {_gbrsLastUpdatedDate = a}) .
  mapping _Time
...
gbrsResponseStatus =
  lens _gbrsResponseStatus (\s a -> s {_gbrsResponseStatus = a})
