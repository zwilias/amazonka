...
data LogSettingsResponse = LogSettingsResponse'{_lsDestination
                                                :: !(Maybe Destination),
...
logSettingsResponse
    :: LogSettingsResponse
logSettingsResponse
  = LogSettingsResponse'{_lsDestination = Nothing,
                         _lsKmsKeyARN = Nothing, _lsLogType = Nothing,
...
instance FromJSON LogSettingsResponse where
        parseJSON
          = withObject "LogSettingsResponse"
              (\ x ->
                 LogSettingsResponse' <$>
                   (x .:? "destination") <*> (x .:? "kmsKeyArn") <*>
                     (x .:? "logType")
                     <*> (x .:? "resourceArn")
                     <*> (x .:? "resourcePrefix"))

instance Hashable LogSettingsResponse where

instance NFData LogSettingsResponse where
