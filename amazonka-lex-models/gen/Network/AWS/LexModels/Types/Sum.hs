...
instance FromText ChannelStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChannelStatus from value: '" <>
        e <> "'. Accepted values: created, failed, in_progress"
...
instance FromText ChannelType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChannelType from value: '" <>
        e <> "'. Accepted values: facebook, kik, slack, twilio-sms"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: custompayload, plaintext, ssml"
...
instance FromText ExportStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExportStatus from value: '" <>
        e <> "'. Accepted values: failed, in_progress, ready"
...
instance FromText ExportType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExportType from value: '" <>
        e <> "'. Accepted values: alexa_skills_kit, lex"
...
instance FromText FulfillmentActivityType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FulfillmentActivityType from value: '" <>
        e <> "'. Accepted values: codehook, returnintent"
...
instance FromText ImportStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ImportStatus from value: '" <>
        e <> "'. Accepted values: complete, failed, in_progress"
...
instance FromText LexStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LexStatus from value: '" <>
        e <> "'. Accepted values: building, failed, not_built, ready"
...
instance FromText Locale where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Locale from value: '" <>
        e <> "'. Accepted values: de-de, en-gb, en-us"
...
instance FromText MergeStrategy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MergeStrategy from value: '" <>
        e <> "'. Accepted values: fail_on_conflict, overwrite_latest"
...
instance FromText ProcessBehavior where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProcessBehavior from value: '" <>
        e <> "'. Accepted values: build, save"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: bot, intent, slot_type"
...
instance FromText SlotConstraint where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SlotConstraint from value: '" <>
        e <> "'. Accepted values: optional, required"
...
instance FromText SlotValueSelectionStrategy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SlotValueSelectionStrategy from value: '" <>
        e <> "'. Accepted values: original_value, top_resolution"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: detected, missed"
