...
data ConversationLogsResponse = ConversationLogsResponse'{_clIamRoleARN
                                                          :: !(Maybe Text),
                                                          _clLogSettings ::
                                                          !(Maybe
                                                              [LogSettingsResponse])}
...
conversationLogsResponse
    :: ConversationLogsResponse
conversationLogsResponse
  = ConversationLogsResponse'{_clIamRoleARN = Nothing,
...
instance FromJSON ConversationLogsResponse where
        parseJSON
          = withObject "ConversationLogsResponse"
              (\ x ->
                 ConversationLogsResponse' <$>
                   (x .:? "iamRoleArn") <*>
                     (x .:? "logSettings" .!= mempty))

instance Hashable ConversationLogsResponse where

instance NFData ConversationLogsResponse where
