...
instance AWSRequest GetBotAlias where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetBotAliasResponse' <$> (x .?> "checksum") <*> (x .?> "botVersion") <*>
         (x .?> "botName") <*>
         (x .?> "createdDate") <*>
         (x .?> "name") <*>
         (x .?> "lastUpdatedDate") <*>
...
instance ToPath GetBotAlias where
  toPath GetBotAlias' {..} =
    mconcat ["/bots/", toBS _gbasBotName, "/aliases/", toBS _gbasName]
...
gbasrsCreatedDate =
  lens _gbasrsCreatedDate (\s a -> s {_gbasrsCreatedDate = a}) . mapping _Time
...
gbasrsLastUpdatedDate =
  lens _gbasrsLastUpdatedDate (\s a -> s {_gbasrsLastUpdatedDate = a}) .
  mapping _Time
...
gbasrsResponseStatus =
  lens _gbasrsResponseStatus (\s a -> s {_gbasrsResponseStatus = a})
