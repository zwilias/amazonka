...
instance AWSRequest ListProjects where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListProjectsResponse' <$> (x .?> "nextToken") <*> (x .?> "projects") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListProjects where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeBuild_20161006.ListProjects" :: ByteString)
...
lprsProjects =
  lens _lprsProjects (\s a -> s {_lprsProjects = a}) . mapping _List1
