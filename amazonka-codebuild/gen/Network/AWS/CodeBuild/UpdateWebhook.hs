...
instance AWSRequest UpdateWebhook where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateWebhookResponse' <$> (x .?> "webhook") <*> (pure (fromEnum s)))
...
instance ToHeaders UpdateWebhook where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeBuild_20161006.UpdateWebhook" :: ByteString)
...
uwrsResponseStatus =
  lens _uwrsResponseStatus (\s a -> s {_uwrsResponseStatus = a})
