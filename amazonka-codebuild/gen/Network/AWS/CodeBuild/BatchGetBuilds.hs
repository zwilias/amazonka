...
instance AWSRequest BatchGetBuilds where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchGetBuildsResponse' <$> (x .?> "builds" .!@ mempty) <*>
         (x .?> "buildsNotFound") <*>
         (pure (fromEnum s)))
...
instance ToHeaders BatchGetBuilds where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeBuild_20161006.BatchGetBuilds" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON BatchGetBuilds where
  toJSON BatchGetBuilds' {..} = object (catMaybes [Just ("ids" .= _bgbIds)])
...
bgbrsBuilds =
  lens _bgbrsBuilds (\s a -> s {_bgbrsBuilds = a}) . _Default . _Coerce

-- | The IDs of builds for which information could not be found.
bgbrsBuildsNotFound :: Lens' BatchGetBuildsResponse (Maybe (NonEmpty Text))
bgbrsBuildsNotFound =
  lens _bgbrsBuildsNotFound (\s a -> s {_bgbrsBuildsNotFound = a}) .
  mapping _List1

-- | -- | The response status code.
bgbrsResponseStatus :: Lens' BatchGetBuildsResponse Int
bgbrsResponseStatus =
  lens _bgbrsResponseStatus (\s a -> s {_bgbrsResponseStatus = a})
