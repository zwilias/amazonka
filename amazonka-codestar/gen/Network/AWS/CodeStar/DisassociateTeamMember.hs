...
instance AWSRequest DisassociateTeamMember where
  ...
  response =
    receiveEmpty
      (\s h x -> DisassociateTeamMemberResponse' <$> (pure (fromEnum s)))
...
instance ToJSON DisassociateTeamMember where
  toJSON DisassociateTeamMember' {..} =
    object
      (catMaybes
         [Just ("projectId" .= _dtmProjectId), Just ("userArn" .= _dtmUserARN)])
...
dtmrsResponseStatus =
  lens _dtmrsResponseStatus (\s a -> s {_dtmrsResponseStatus = a})
