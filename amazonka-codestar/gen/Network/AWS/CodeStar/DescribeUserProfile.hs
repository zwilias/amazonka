...
instance AWSRequest DescribeUserProfile where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeUserProfileResponse' <$> (x .?> "sshPublicKey") <*>
         (x .?> "emailAddress") <*>
         (x .?> "displayName") <*>
         (pure (fromEnum s)) <*>
         (x .:> "userArn") <*>
...
duprsEmailAddress =
  lens _duprsEmailAddress (\s a -> s {_duprsEmailAddress = a}) .
  mapping _Sensitive
...
duprsResponseStatus =
  lens _duprsResponseStatus (\s a -> s {_duprsResponseStatus = a})
...
duprsCreatedTimestamp =
  lens _duprsCreatedTimestamp (\s a -> s {_duprsCreatedTimestamp = a}) . _Time

-- | The date and time when the user profile was last modified, in timestamp format.
duprsLastModifiedTimestamp :: Lens' DescribeUserProfileResponse UTCTime
duprsLastModifiedTimestamp =
  lens _duprsLastModifiedTimestamp (\s a -> s {_duprsLastModifiedTimestamp = a}) .
  _Time
