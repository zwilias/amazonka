...
cpClientRequestToken =
  lens _cpClientRequestToken (\s a -> s {_cpClientRequestToken = a})

-- | Reserved for future use.
cpDescription :: Lens' CreateProject (Maybe Text)
cpDescription =
  lens _cpDescription (\s a -> s {_cpDescription = a}) . mapping _Sensitive
...
instance AWSRequest CreateProject where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateProjectResponse' <$> (x .?> "projectTemplateId") <*>
...
instance ToHeaders CreateProject where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeStar_20170419.CreateProject" :: ByteString)
...
cprsProjectTemplateId =
  lens _cprsProjectTemplateId (\s a -> s {_cprsProjectTemplateId = a})

-- | Reserved for future use.
cprsClientRequestToken :: Lens' CreateProjectResponse (Maybe Text)
cprsClientRequestToken =
  lens _cprsClientRequestToken (\s a -> s {_cprsClientRequestToken = a})
