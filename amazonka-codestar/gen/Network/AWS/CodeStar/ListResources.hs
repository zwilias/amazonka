...
instance AWSRequest ListResources where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListResourcesResponse' <$> (x .?> "resources" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListResources where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeStar_20170419.ListResources" :: ByteString)
...
lrrsResources =
  lens _lrrsResources (\s a -> s {_lrrsResources = a}) . _Default . _Coerce
