...
utmRemoteAccessAllowed =
  lens _utmRemoteAccessAllowed (\s a -> s {_utmRemoteAccessAllowed = a})
...
instance AWSRequest UpdateTeamMember where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateTeamMemberResponse' <$> (x .?> "userArn") <*>
...
utmrsRemoteAccessAllowed =
  lens _utmrsRemoteAccessAllowed (\s a -> s {_utmrsRemoteAccessAllowed = a})
...
utmrsResponseStatus =
  lens _utmrsResponseStatus (\s a -> s {_utmrsResponseStatus = a})
