...
cupEmailAddress =
  lens _cupEmailAddress (\s a -> s {_cupEmailAddress = a}) . _Sensitive
...
instance AWSRequest CreateUserProfile where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateUserProfileResponse' <$> (x .?> "lastModifiedTimestamp") <*>
         (x .?> "sshPublicKey") <*>
         (x .?> "emailAddress") <*>
         (x .?> "displayName") <*>
         (x .?> "createdTimestamp") <*>
...
cuprsLastModifiedTimestamp =
  lens _cuprsLastModifiedTimestamp (\s a -> s {_cuprsLastModifiedTimestamp = a}) .
  mapping _Time
...
cuprsEmailAddress =
  lens _cuprsEmailAddress (\s a -> s {_cuprsEmailAddress = a}) .
  mapping _Sensitive
...
cuprsCreatedTimestamp =
  lens _cuprsCreatedTimestamp (\s a -> s {_cuprsCreatedTimestamp = a}) .
  mapping _Time
