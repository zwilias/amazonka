...
instance FromJSON ProjectSummary where
  parseJSON =
    withObject
      "ProjectSummary"
      (\x -> ProjectSummary' <$> (x .:? "projectArn") <*> (x .:? "projectId"))
...
instance FromJSON Resource where
  parseJSON = withObject "Resource" (\x -> Resource' <$> (x .: "id"))
...
tmRemoteAccessAllowed =
  lens _tmRemoteAccessAllowed (\s a -> s {_tmRemoteAccessAllowed = a})
...
instance FromJSON TeamMember where
  parseJSON =
    withObject
      "TeamMember"
      (\x ->
         TeamMember' <$> (x .:? "remoteAccessAllowed") <*> (x .: "userArn") <*>
         (x .: "projectRole"))
...
upsEmailAddress =
  lens _upsEmailAddress (\s a -> s {_upsEmailAddress = a}) . mapping _Sensitive
...
instance FromJSON UserProfileSummary where
  parseJSON =
    withObject
      "UserProfileSummary"
      (\x ->
         UserProfileSummary' <$> (x .:? "sshPublicKey") <*> (x .:? "userArn") <*>
         (x .:? "emailAddress") <*>
         (x .:? "displayName"))
