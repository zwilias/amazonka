...
uupEmailAddress =
  lens _uupEmailAddress (\s a -> s {_uupEmailAddress = a}) . mapping _Sensitive
...
instance AWSRequest UpdateUserProfile where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateUserProfileResponse' <$> (x .?> "lastModifiedTimestamp") <*>
         (x .?> "sshPublicKey") <*>
         (x .?> "emailAddress") <*>
         (x .?> "displayName") <*>
         (x .?> "createdTimestamp") <*>
...
uuprsLastModifiedTimestamp =
  lens _uuprsLastModifiedTimestamp (\s a -> s {_uuprsLastModifiedTimestamp = a}) .
  mapping _Time
...
uuprsEmailAddress =
  lens _uuprsEmailAddress (\s a -> s {_uuprsEmailAddress = a}) .
  mapping _Sensitive
...
uuprsCreatedTimestamp =
  lens _uuprsCreatedTimestamp (\s a -> s {_uuprsCreatedTimestamp = a}) .
  mapping _Time
