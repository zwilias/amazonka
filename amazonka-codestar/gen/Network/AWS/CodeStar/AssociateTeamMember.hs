...
atmRemoteAccessAllowed =
  lens _atmRemoteAccessAllowed (\s a -> s {_atmRemoteAccessAllowed = a})

-- | A user- or system-generated token that identifies the entity that requested the team member association to the project. This token can be used to repeat the request.
atmClientRequestToken :: Lens' AssociateTeamMember (Maybe Text)
atmClientRequestToken =
  lens _atmClientRequestToken (\s a -> s {_atmClientRequestToken = a})
...
instance AWSRequest AssociateTeamMember where
  ...
  response =
    receiveJSON
      (\s h x ->
         AssociateTeamMemberResponse' <$> (x .?> "clientRequestToken") <*>
         (pure (fromEnum s)))
...
atmrsClientRequestToken =
  lens _atmrsClientRequestToken (\s a -> s {_atmrsClientRequestToken = a})

-- | -- | The response status code.
atmrsResponseStatus :: Lens' AssociateTeamMemberResponse Int
atmrsResponseStatus =
  lens _atmrsResponseStatus (\s a -> s {_atmrsResponseStatus = a})
