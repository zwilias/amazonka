...
ltmMaxResults =
  lens _ltmMaxResults (\s a -> s {_ltmMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListTeamMembers where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListTeamMembersResponse' <$> (x .?> "nextToken") <*>
         (pure (fromEnum s)) <*>
...
instance ToHeaders ListTeamMembers where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeStar_20170419.ListTeamMembers" :: ByteString)
...
ltmrsResponseStatus =
  lens _ltmrsResponseStatus (\s a -> s {_ltmrsResponseStatus = a})

-- | A list of team member objects for the project.
ltmrsTeamMembers :: Lens' ListTeamMembersResponse [TeamMember]
ltmrsTeamMembers =
  lens _ltmrsTeamMembers (\s a -> s {_ltmrsTeamMembers = a}) . _Coerce
