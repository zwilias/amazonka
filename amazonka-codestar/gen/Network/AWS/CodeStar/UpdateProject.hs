...
upDescription =
  lens _upDescription (\s a -> s {_upDescription = a}) . mapping _Sensitive
...
instance AWSRequest UpdateProject where
  ...
  response =
    receiveEmpty (\s h x -> UpdateProjectResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateProject where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CodeStar_20170419.UpdateProject" :: ByteString)
