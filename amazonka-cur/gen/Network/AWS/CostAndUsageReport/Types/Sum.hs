...
instance FromText AWSRegion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AWSRegion from value: '" <>
        e <>
...
instance FromText AdditionalArtifact where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AdditionalArtifact from value: '" <>
        e <> "'. Accepted values: quicksight, redshift"
...
instance FromText ReportFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReportFormat from value: '" <>
        e <> "'. Accepted values: textorcsv"
...
instance FromText SchemaElement where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SchemaElement from value: '" <>
        e <> "'. Accepted values: resources"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: daily, hourly"
