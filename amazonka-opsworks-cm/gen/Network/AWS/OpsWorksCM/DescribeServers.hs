...
dssMaxResults =
  lens _dssMaxResults (\s a -> s {_dssMaxResults = a}) . mapping _Nat
...
instance AWSRequest DescribeServers where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeServersResponse' <$> (x .?> "Servers" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
dssrsServers =
  lens _dssrsServers (\s a -> s {_dssrsServers = a}) . _Default . _Coerce
