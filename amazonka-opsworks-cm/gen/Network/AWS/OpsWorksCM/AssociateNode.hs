...
anEngineAttributes =
  lens _anEngineAttributes (\s a -> s {_anEngineAttributes = a}) . _Coerce
...
instance AWSRequest AssociateNode where
  ...
  response =
    receiveJSON
      (\s h x ->
         AssociateNodeResponse' <$> (x .?> "NodeAssociationStatusToken") <*>
         (pure (fromEnum s)))
...
anrsNodeAssociationStatusToken =
  lens
    ...
    (\s a -> s {_anrsNodeAssociationStatusToken = a})

-- | -- | The response status code.
anrsResponseStatus :: Lens' AssociateNodeResponse Int
anrsResponseStatus =
  lens _anrsResponseStatus (\s a -> s {_anrsResponseStatus = a})
