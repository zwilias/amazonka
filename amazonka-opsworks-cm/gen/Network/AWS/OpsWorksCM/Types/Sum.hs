  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: deleting, failed, in_progress, ok"
...
instance FromText BackupType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BackupType from value: '" <>
        e <> "'. Accepted values: automated, manual"
...
instance FromText MaintenanceStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MaintenanceStatus from value: '" <>
        e <> "'. Accepted values: failed, success"
...
instance FromText NodeAssociationStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NodeAssociationStatus from value: '" <>
        e <> "'. Accepted values: failed, in_progress, success"
...
instance FromText ServerStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ServerStatus from value: '" <>
        e <>
