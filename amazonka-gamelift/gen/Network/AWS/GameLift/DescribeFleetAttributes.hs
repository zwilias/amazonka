...
instance AWSRequest DescribeFleetAttributes where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeFleetAttributesResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders DescribeFleetAttributes where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("GameLift.DescribeFleetAttributes" :: ByteString)
...
dfarsFleetAttributes =
  lens _dfarsFleetAttributes (\s a -> s {_dfarsFleetAttributes = a}) .
  _Default . _Coerce

-- | -- | The response status code.
dfarsResponseStatus :: Lens' DescribeFleetAttributesResponse Int
dfarsResponseStatus =
  lens _dfarsResponseStatus (\s a -> s {_dfarsResponseStatus = a})
