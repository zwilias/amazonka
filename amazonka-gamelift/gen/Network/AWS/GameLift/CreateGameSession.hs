...
cgsIdempotencyToken =
  lens _cgsIdempotencyToken (\s a -> s {_cgsIdempotencyToken = a})

-- | Set of custom properties for a game session, formatted as key:value pairs. These properties are passed to a game server process in the 'GameSession' object with a request to start a new game session (see <http://docs.aws.amazon.com/gamelift/latest/developerguide/gamelift-sdk-server-api.html#gamelift-sdk-server-startsession Start a Game Session> ).
cgsGameProperties :: Lens' CreateGameSession [GameProperty]
cgsGameProperties =
  lens _cgsGameProperties (\s a -> s {_cgsGameProperties = a}) .
  _Default . _Coerce
...
cgsGameSessionData =
  lens _cgsGameSessionData (\s a -> s {_cgsGameSessionData = a})
...
cgsMaximumPlayerSessionCount =
  lens
    ...
    (\s a -> s {_cgsMaximumPlayerSessionCount = a}) .
  _Nat
...
instance AWSRequest CreateGameSession where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateGameSessionResponse' <$> (x .?> "GameSession") <*>
         (pure (fromEnum s)))
  ...
  toJSON CreateGameSession' {..} =
    object
      (catMaybes
         ...
         , Just ("MaximumPlayerSessionCount" .= _cgsMaximumPlayerSessionCount)
         ])
...
cgsrsResponseStatus =
  lens _cgsrsResponseStatus (\s a -> s {_cgsrsResponseStatus = a})
