...
dfeStartTime =
  lens _dfeStartTime (\s a -> s {_dfeStartTime = a}) . mapping _Time
...
instance AWSRequest DescribeFleetEvents where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeFleetEventsResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Events" .!@ mempty) <*>
         (pure (fromEnum s)))
...
dfersEvents =
  lens _dfersEvents (\s a -> s {_dfersEvents = a}) . _Default . _Coerce

-- | -- | The response status code.
dfersResponseStatus :: Lens' DescribeFleetEventsResponse Int
dfersResponseStatus =
  lens _dfersResponseStatus (\s a -> s {_dfersResponseStatus = a})
