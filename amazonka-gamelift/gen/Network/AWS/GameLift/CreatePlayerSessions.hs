...
cpsPlayerDataMap =
  lens _cpsPlayerDataMap (\s a -> s {_cpsPlayerDataMap = a}) . _Default . _Map
...
instance AWSRequest CreatePlayerSessions where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreatePlayerSessionsResponse' <$> (x .?> "PlayerSessions" .!@ mempty) <*>
         (pure (fromEnum s)))
...
crsPlayerSessions =
  lens _crsPlayerSessions (\s a -> s {_crsPlayerSessions = a}) .
  _Default . _Coerce
