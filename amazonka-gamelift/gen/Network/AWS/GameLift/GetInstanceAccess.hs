...
instance AWSRequest GetInstanceAccess where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetInstanceAccessResponse' <$> (x .?> "InstanceAccess") <*>
         (pure (fromEnum s)))
...
giarsInstanceAccess =
  lens _giarsInstanceAccess (\s a -> s {_giarsInstanceAccess = a})

-- | -- | The response status code.
giarsResponseStatus :: Lens' GetInstanceAccessResponse Int
giarsResponseStatus =
  lens _giarsResponseStatus (\s a -> s {_giarsResponseStatus = a})
