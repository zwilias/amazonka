...
instance AWSRequest StopFleetActions where
  ...
  response =
    receiveEmpty (\s h x -> StopFleetActionsResponse' <$> (pure (fromEnum s)))
...
instance ToJSON StopFleetActions where
  toJSON StopFleetActions' {..} =
    object
      (catMaybes [Just ("FleetId" .= _sFleetId), Just ("Actions" .= _sActions)])
...
newtype StopFleetActionsResponse =
  StopFleetActionsResponse'
...
sfasrsResponseStatus =
  lens _sfasrsResponseStatus (\s a -> s {_sfasrsResponseStatus = a})
