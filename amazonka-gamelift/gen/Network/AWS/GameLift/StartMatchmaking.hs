...
sConfigurationName =
  lens _sConfigurationName (\s a -> s {_sConfigurationName = a})
...
instance AWSRequest StartMatchmaking where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartMatchmakingResponse' <$> (x .?> "MatchmakingTicket") <*>
         (pure (fromEnum s)))
...
srsMatchmakingTicket =
  lens _srsMatchmakingTicket (\s a -> s {_srsMatchmakingTicket = a})
