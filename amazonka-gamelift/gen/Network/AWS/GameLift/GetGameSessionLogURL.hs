...
ggsluGameSessionId =
  lens _ggsluGameSessionId (\s a -> s {_ggsluGameSessionId = a})
...
instance AWSRequest GetGameSessionLogURL where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetGameSessionLogURLResponse' <$> (x .?> "PreSignedUrl") <*>
         (pure (fromEnum s)))
...
ggslursPreSignedURL =
  lens _ggslursPreSignedURL (\s a -> s {_ggslursPreSignedURL = a})

-- | -- | The response status code.
ggslursResponseStatus :: Lens' GetGameSessionLogURLResponse Int
ggslursResponseStatus =
  lens _ggslursResponseStatus (\s a -> s {_ggslursResponseStatus = a})
