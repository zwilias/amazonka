...
smbConfigurationName =
  lens _smbConfigurationName (\s a -> s {_smbConfigurationName = a})
...
instance AWSRequest StartMatchBackfill where
  ...
  response =
    receiveJSON
      (\s h x ->
         StartMatchBackfillResponse' <$> (x .?> "MatchmakingTicket") <*>
         (pure (fromEnum s)))
...
smbrsMatchmakingTicket ::
     Lens' StartMatchBackfillResponse (Maybe MatchmakingTicket)
smbrsMatchmakingTicket =
  lens _smbrsMatchmakingTicket (\s a -> s {_smbrsMatchmakingTicket = a})

-- | -- | The response status code.
smbrsResponseStatus :: Lens' StartMatchBackfillResponse Int
smbrsResponseStatus =
  lens _smbrsResponseStatus (\s a -> s {_smbrsResponseStatus = a})
