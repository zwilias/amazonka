...
dvpaGameLiftAWSAccountId =
  lens _dvpaGameLiftAWSAccountId (\s a -> s {_dvpaGameLiftAWSAccountId = a})
...
instance AWSRequest DeleteVPCPeeringAuthorization where
  type Rs DeleteVPCPeeringAuthorization = DeleteVPCPeeringAuthorizationResponse
  request = postJSON gameLift
  response =
    receiveEmpty
      (\s h x -> DeleteVPCPeeringAuthorizationResponse' <$> (pure (fromEnum s)))
...
dvparsResponseStatus =
  lens _dvparsResponseStatus (\s a -> s {_dvparsResponseStatus = a})
