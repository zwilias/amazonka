...
umcGameProperties =
  lens _umcGameProperties (\s a -> s {_umcGameProperties = a}) .
  _Default . _Coerce
...
umcAcceptanceTimeoutSeconds ::
     Lens' UpdateMatchmakingConfiguration (Maybe Natural)
umcAcceptanceTimeoutSeconds =
  lens
    _umcAcceptanceTimeoutSeconds
    (\s a -> s {_umcAcceptanceTimeoutSeconds = a}) .
  mapping _Nat

-- | Maximum duration, in seconds, that a matchmaking ticket can remain in process before timing out. Requests that time out can be resubmitted as needed.
umcRequestTimeoutSeconds :: Lens' UpdateMatchmakingConfiguration (Maybe Natural)
umcRequestTimeoutSeconds =
  lens _umcRequestTimeoutSeconds (\s a -> s {_umcRequestTimeoutSeconds = a}) .
  mapping _Nat
...
umcNotificationTarget =
  lens _umcNotificationTarget (\s a -> s {_umcNotificationTarget = a})
...
umcAdditionalPlayerCount =
  lens _umcAdditionalPlayerCount (\s a -> s {_umcAdditionalPlayerCount = a}) .
  mapping _Nat
...
instance AWSRequest UpdateMatchmakingConfiguration where
  type Rs UpdateMatchmakingConfiguration = UpdateMatchmakingConfigurationResponse
  request = postJSON gameLift
  response =
    receiveJSON
      (\s h x ->
         UpdateMatchmakingConfigurationResponse' <$> (x .?> "Configuration") <*>
         (pure (fromEnum s)))
...
umcrsConfiguration ::
     Lens' UpdateMatchmakingConfigurationResponse (Maybe MatchmakingConfiguration)
umcrsConfiguration =
  lens _umcrsConfiguration (\s a -> s {_umcrsConfiguration = a})

-- | -- | The response status code.
umcrsResponseStatus :: Lens' UpdateMatchmakingConfigurationResponse Int
umcrsResponseStatus =
  lens _umcrsResponseStatus (\s a -> s {_umcrsResponseStatus = a})
