...
cmcAdditionalPlayerCount =
  lens _cmcAdditionalPlayerCount (\s a -> s {_cmcAdditionalPlayerCount = a}) .
  mapping _Nat
...
instance AWSRequest CreateMatchmakingConfiguration where
  type Rs CreateMatchmakingConfiguration = CreateMatchmakingConfigurationResponse
  request = postJSON gameLift
  response =
    receiveJSON
      (\s h x ->
         CreateMatchmakingConfigurationResponse' <$> (x .?> "Configuration") <*>
         (pure (fromEnum s)))
  ...
  toJSON CreateMatchmakingConfiguration' {..} =
    object
      (catMaybes
         ...
         , Just ("Name" .= _cmcName)
         , Just ("GameSessionQueueArns" .= _cmcGameSessionQueueARNs)
...
cmcrsConfiguration ::
     Lens' CreateMatchmakingConfigurationResponse (Maybe MatchmakingConfiguration)
cmcrsConfiguration =
  lens _cmcrsConfiguration (\s a -> s {_cmcrsConfiguration = a})

-- | -- | The response status code.
cmcrsResponseStatus :: Lens' CreateMatchmakingConfigurationResponse Int
cmcrsResponseStatus =
  lens _cmcrsResponseStatus (\s a -> s {_cmcrsResponseStatus = a})
