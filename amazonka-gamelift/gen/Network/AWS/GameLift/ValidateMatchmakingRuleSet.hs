...
instance AWSRequest ValidateMatchmakingRuleSet where
  type Rs ValidateMatchmakingRuleSet = ValidateMatchmakingRuleSetResponse
  request = postJSON gameLift
  response =
    receiveJSON
      (\s h x ->
         ValidateMatchmakingRuleSetResponse' <$> (x .?> "Valid") <*>
         (pure (fromEnum s)))
...
vmrsrsResponseStatus =
  lens _vmrsrsResponseStatus (\s a -> s {_vmrsrsResponseStatus = a})
