...
instance AWSRequest DeleteMatchmakingConfiguration where
  type Rs DeleteMatchmakingConfiguration = DeleteMatchmakingConfigurationResponse
  request = postJSON gameLift
  response =
    receiveEmpty
      (\s h x -> DeleteMatchmakingConfigurationResponse' <$> (pure (fromEnum s)))
...
dmcrsResponseStatus =
  lens _dmcrsResponseStatus (\s a -> s {_dmcrsResponseStatus = a})
