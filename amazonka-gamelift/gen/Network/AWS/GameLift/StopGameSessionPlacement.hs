...
instance AWSRequest StopGameSessionPlacement where
  ...
  response =
    receiveJSON
      (\s h x ->
         StopGameSessionPlacementResponse' <$> (x .?> "GameSessionPlacement") <*>
         (pure (fromEnum s)))
...
instance ToHeaders StopGameSessionPlacement where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("GameLift.StopGameSessionPlacement" :: ByteString)
...
storsGameSessionPlacement ::
     Lens' StopGameSessionPlacementResponse (Maybe GameSessionPlacement)
storsGameSessionPlacement =
  lens _storsGameSessionPlacement (\s a -> s {_storsGameSessionPlacement = a})
