...
instance AWSRequest DeleteGameSessionQueue where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteGameSessionQueueResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteGameSessionQueue where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("GameLift.DeleteGameSessionQueue" :: ByteString)
...
dgsqrsResponseStatus =
  lens _dgsqrsResponseStatus (\s a -> s {_dgsqrsResponseStatus = a})
