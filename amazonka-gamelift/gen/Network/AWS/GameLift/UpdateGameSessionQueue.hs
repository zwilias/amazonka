...
ugsqPlayerLatencyPolicies =
  lens _ugsqPlayerLatencyPolicies (\s a -> s {_ugsqPlayerLatencyPolicies = a}) .
  _Default . _Coerce

-- | Maximum time, in seconds, that a new game session placement request remains in the queue. When a request exceeds this time, the game session placement changes to a @TIMED_OUT@ status.
ugsqTimeoutInSeconds :: Lens' UpdateGameSessionQueue (Maybe Natural)
ugsqTimeoutInSeconds =
  lens _ugsqTimeoutInSeconds (\s a -> s {_ugsqTimeoutInSeconds = a}) .
  mapping _Nat
...
ugsqDestinations =
  lens _ugsqDestinations (\s a -> s {_ugsqDestinations = a}) .
  _Default . _Coerce
...
instance AWSRequest UpdateGameSessionQueue where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateGameSessionQueueResponse' <$> (x .?> "GameSessionQueue") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateGameSessionQueue where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("GameLift.UpdateGameSessionQueue" :: ByteString)
...
ugsqrsGameSessionQueue ::
     Lens' UpdateGameSessionQueueResponse (Maybe GameSessionQueue)
ugsqrsGameSessionQueue =
  lens _ugsqrsGameSessionQueue (\s a -> s {_ugsqrsGameSessionQueue = a})

-- | -- | The response status code.
ugsqrsResponseStatus :: Lens' UpdateGameSessionQueueResponse Int
ugsqrsResponseStatus =
  lens _ugsqrsResponseStatus (\s a -> s {_ugsqrsResponseStatus = a})
