...
ugsMaximumPlayerSessionCount =
  lens
    ...
    (\s a -> s {_ugsMaximumPlayerSessionCount = a}) .
  mapping _Nat

-- | Policy determining whether or not the game session accepts new players.
ugsPlayerSessionCreationPolicy ::
     Lens' UpdateGameSession (Maybe PlayerSessionCreationPolicy)
ugsPlayerSessionCreationPolicy =
  lens
    _ugsPlayerSessionCreationPolicy
    (\s a -> s {_ugsPlayerSessionCreationPolicy = a})
...
ugsProtectionPolicy =
  lens _ugsProtectionPolicy (\s a -> s {_ugsProtectionPolicy = a})
...
instance AWSRequest UpdateGameSession where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateGameSessionResponse' <$> (x .?> "GameSession") <*>
         (pure (fromEnum s)))
...
instance ToJSON UpdateGameSession where
  toJSON UpdateGameSession' {..} =
    object
      (catMaybes
         [ ("MaximumPlayerSessionCount" .=) <$> _ugsMaximumPlayerSessionCount
...
ugsrsResponseStatus =
  lens _ugsrsResponseStatus (\s a -> s {_ugsrsResponseStatus = a})
