...
dsmStateMachineARN =
  lens _dsmStateMachineARN (\s a -> s {_dsmStateMachineARN = a})
...
instance AWSRequest DescribeStateMachine where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeStateMachineResponse' <$> (x .?> "status") <*>
         (pure (fromEnum s)) <*>
         (x .:> "stateMachineArn") <*>
         (x .:> "name") <*>
         (x .:> "definition") <*>
...
instance ToJSON DescribeStateMachine where
  toJSON DescribeStateMachine' {..} =
    object (catMaybes [Just ("stateMachineArn" .= _dsmStateMachineARN)])
...
dsmrsResponseStatus =
  lens _dsmrsResponseStatus (\s a -> s {_dsmrsResponseStatus = a})

-- | The Amazon Resource Name (ARN) that identifies the state machine.
dsmrsStateMachineARN :: Lens' DescribeStateMachineResponse Text
dsmrsStateMachineARN =
  lens _dsmrsStateMachineARN (\s a -> s {_dsmrsStateMachineARN = a})
...
dsmrsCreationDate =
  lens _dsmrsCreationDate (\s a -> s {_dsmrsCreationDate = a}) . _Time
