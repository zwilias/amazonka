...
instance AWSRequest DescribeExecution where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeExecutionResponse' <$> (x .?> "stopDate") <*> (x .?> "name") <*>
         (x .?> "output") <*>
         (pure (fromEnum s)) <*>
         (x .:> "executionArn") <*>
         (x .:> "stateMachineArn") <*>
...
dersStopDate =
  lens _dersStopDate (\s a -> s {_dersStopDate = a}) . mapping _Time
...
dersStateMachineARN =
  lens _dersStateMachineARN (\s a -> s {_dersStateMachineARN = a})
