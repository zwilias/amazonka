...
instance AWSRequest DescribeActivity where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeActivityResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "activityArn") <*>
         (x .:> "name") <*>
         (x .:> "creationDate"))
...
instance ToHeaders DescribeActivity where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSStepFunctions.DescribeActivity" :: ByteString)
...
desrsCreationDate =
  lens _desrsCreationDate (\s a -> s {_desrsCreationDate = a}) . _Time
