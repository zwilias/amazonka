...
newtype LogDestination = LogDestination'{_ldCloudWatchLogsLogGroup
...
logDestination
    :: LogDestination
logDestination
  = LogDestination'{_ldCloudWatchLogsLogGroup =
                      Nothing}
...
instance FromJSON LogDestination where
        parseJSON
          = withObject "LogDestination"
              (\ x ->
                 LogDestination' <$> (x .:? "cloudWatchLogsLogGroup"))

instance Hashable LogDestination where

instance NFData LogDestination where
