...
instance AWSRequest ListExecutions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListExecutionsResponse' <$> (x .?> "nextToken") <*> (pure (fromEnum s)) <*>
         (x .?> "executions" .!@ mempty))
...
instance ToHeaders ListExecutions where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSStepFunctions.ListExecutions" :: ByteString)
...
lersExecutions =
  lens _lersExecutions (\s a -> s {_lersExecutions = a}) . _Coerce
