...
instance AWSRequest SendTaskFailure where
  ...
  response =
    receiveEmpty (\s h x -> SendTaskFailureResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders SendTaskFailure where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSStepFunctions.SendTaskFailure" :: ByteString)
...
stfrsResponseStatus =
  lens _stfrsResponseStatus (\s a -> s {_stfrsResponseStatus = a})
