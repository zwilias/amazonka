...
instance AWSRequest SendTaskSuccess where
  ...
  response =
    receiveEmpty (\s h x -> SendTaskSuccessResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders SendTaskSuccess where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSStepFunctions.SendTaskSuccess" :: ByteString)
...
instance ToJSON SendTaskSuccess where
  toJSON SendTaskSuccess' {..} =
    object
      (catMaybes
         [Just ("taskToken" .= _stsTaskToken), Just ("output" .= _stsOutput)])
