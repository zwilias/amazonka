...
instance AWSRequest StopExecution where
  ...
  response =
    receiveJSON
      (\s h x ->
         StopExecutionResponse' <$> (pure (fromEnum s)) <*> (x .:> "stopDate"))
...
instance ToHeaders StopExecution where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSStepFunctions.StopExecution" :: ByteString)
...
sersResponseStatus =
  lens _sersResponseStatus (\s a -> s {_sersResponseStatus = a})
