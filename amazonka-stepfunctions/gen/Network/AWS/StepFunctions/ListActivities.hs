...
instance AWSRequest ListActivities where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListActivitiesResponse' <$> (x .?> "nextToken") <*> (pure (fromEnum s)) <*>
         (x .?> "activities" .!@ mempty))
...
instance ToHeaders ListActivities where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSStepFunctions.ListActivities" :: ByteString)
...
larsActivities =
  lens _larsActivities (\s a -> s {_larsActivities = a}) . _Coerce
