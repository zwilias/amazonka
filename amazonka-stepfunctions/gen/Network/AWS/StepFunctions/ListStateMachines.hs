...
lsmMaxResults =
  lens _lsmMaxResults (\s a -> s {_lsmMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListStateMachines where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListStateMachinesResponse' <$> (x .?> "nextToken") <*>
         (pure (fromEnum s)) <*>
...
lsmrsResponseStatus =
  lens _lsmrsResponseStatus (\s a -> s {_lsmrsResponseStatus = a})

-- | Undocumented member.
lsmrsStateMachines :: Lens' ListStateMachinesResponse [StateMachineListItem]
lsmrsStateMachines =
  lens _lsmrsStateMachines (\s a -> s {_lsmrsStateMachines = a}) . _Coerce
