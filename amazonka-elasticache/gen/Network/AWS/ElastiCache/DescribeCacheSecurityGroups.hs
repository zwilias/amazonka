...
dcsgsCacheSecurityGroupName =
  lens
    ...
    (\s a -> s {_dcsgsCacheSecurityGroupName = a})
...
dcsgsrsCacheSecurityGroups ::
     Lens' DescribeCacheSecurityGroupsResponse [CacheSecurityGroup]
dcsgsrsCacheSecurityGroups =
  lens _dcsgsrsCacheSecurityGroups (\s a -> s {_dcsgsrsCacheSecurityGroups = a}) .
  _Default . _Coerce
