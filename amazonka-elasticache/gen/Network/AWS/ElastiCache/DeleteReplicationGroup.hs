...
drgFinalSnapshotIdentifier =
  lens _drgFinalSnapshotIdentifier (\s a -> s {_drgFinalSnapshotIdentifier = a})
...
instance AWSRequest DeleteReplicationGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DeleteReplicationGroupResponse' <$> (x .@? "ReplicationGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
delrsReplicationGroup ::
     Lens' DeleteReplicationGroupResponse (Maybe ReplicationGroup)
delrsReplicationGroup =
  lens _delrsReplicationGroup (\s a -> s {_delrsReplicationGroup = a})
