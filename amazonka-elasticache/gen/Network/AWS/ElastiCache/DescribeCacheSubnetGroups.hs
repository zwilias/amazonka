...
dcsgCacheSubnetGroupName =
  lens _dcsgCacheSubnetGroupName (\s a -> s {_dcsgCacheSubnetGroupName = a})
...
instance AWSRequest DescribeCacheSubnetGroups where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeCacheSubnetGroupsResponse' <$> (x .@? "Marker") <*>
...
dcsgrsCacheSubnetGroups ::
     Lens' DescribeCacheSubnetGroupsResponse [CacheSubnetGroup]
dcsgrsCacheSubnetGroups =
  lens _dcsgrsCacheSubnetGroups (\s a -> s {_dcsgrsCacheSubnetGroups = a}) .
  _Default . _Coerce
