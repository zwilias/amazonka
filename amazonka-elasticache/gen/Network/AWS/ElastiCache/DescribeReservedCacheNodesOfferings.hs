...
drcnoProductDescription ::
     Lens' DescribeReservedCacheNodesOfferings (Maybe Text)
drcnoProductDescription =
...
instance AWSRequest DescribeReservedCacheNodesOfferings where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeReservedCacheNodesOfferingsResponse' <$> (x .@? "Marker") <*>
...
drcnorsResponseStatus =
  lens _drcnorsResponseStatus (\s a -> s {_drcnorsResponseStatus = a})
