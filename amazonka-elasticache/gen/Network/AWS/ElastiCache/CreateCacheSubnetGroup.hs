...
ccsgCacheSubnetGroupName =
  lens _ccsgCacheSubnetGroupName (\s a -> s {_ccsgCacheSubnetGroupName = a})
...
instance AWSRequest CreateCacheSubnetGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateCacheSubnetGroupResponse' <$> (x .@? "CacheSubnetGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
crsCacheSubnetGroup ::
     Lens' CreateCacheSubnetGroupResponse (Maybe CacheSubnetGroup)
crsCacheSubnetGroup =
  lens _crsCacheSubnetGroup (\s a -> s {_crsCacheSubnetGroup = a})
