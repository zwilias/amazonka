...
mcsgSubnetIds =
  lens _mcsgSubnetIds (\s a -> s {_mcsgSubnetIds = a}) . _Default . _Coerce
...
mcsgCacheSubnetGroupName =
  lens _mcsgCacheSubnetGroupName (\s a -> s {_mcsgCacheSubnetGroupName = a})
...
instance AWSRequest ModifyCacheSubnetGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ModifyCacheSubnetGroupResponse' <$> (x .@? "CacheSubnetGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
mcsgrsCacheSubnetGroup ::
     Lens' ModifyCacheSubnetGroupResponse (Maybe CacheSubnetGroup)
mcsgrsCacheSubnetGroup =
