...
dShowCacheClustersNotInReplicationGroups ::
     Lens' DescribeCacheClusters (Maybe Bool)
dShowCacheClustersNotInReplicationGroups =
  lens
    _dShowCacheClustersNotInReplicationGroups
    (\s a -> s {_dShowCacheClustersNotInReplicationGroups = a})
...
dShowCacheNodeInfo =
  lens _dShowCacheNodeInfo (\s a -> s {_dShowCacheNodeInfo = a})
...
instance AWSRequest DescribeCacheClusters where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeCacheClustersResponse' <$>
         (x .@? "CacheClusters" .!@ mempty >>= may (parseXMLList "CacheCluster")) <*>
         (x .@? "Marker") <*>
         (pure (fromEnum s)))
...
drsCacheClusters =
  lens _drsCacheClusters (\s a -> s {_drsCacheClusters = a}) .
  _Default . _Coerce
