...
drgsReplicationGroupId =
  lens _drgsReplicationGroupId (\s a -> s {_drgsReplicationGroupId = a})
...
instance AWSRequest DescribeReplicationGroups where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeReplicationGroupsResponse' <$> (x .@? "Marker") <*>
...
drgrsReplicationGroups ::
     Lens' DescribeReplicationGroupsResponse [ReplicationGroup]
drgrsReplicationGroups =
  lens _drgrsReplicationGroups (\s a -> s {_drgrsReplicationGroups = a}) .
  _Default . _Coerce
