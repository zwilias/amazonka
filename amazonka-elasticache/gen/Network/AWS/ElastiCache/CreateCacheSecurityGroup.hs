...
ccsgCacheSecurityGroupName =
  lens _ccsgCacheSecurityGroupName (\s a -> s {_ccsgCacheSecurityGroupName = a})
...
instance AWSRequest CreateCacheSecurityGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateCacheSecurityGroupResponse' <$> (x .@? "CacheSecurityGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
ccsgrsCacheSecurityGroup ::
     Lens' CreateCacheSecurityGroupResponse (Maybe CacheSecurityGroup)
ccsgrsCacheSecurityGroup =
  lens _ccsgrsCacheSecurityGroup (\s a -> s {_ccsgrsCacheSecurityGroup = a})

-- | -- | The response status code.
ccsgrsResponseStatus :: Lens' CreateCacheSecurityGroupResponse Int
ccsgrsResponseStatus =
  lens _ccsgrsResponseStatus (\s a -> s {_ccsgrsResponseStatus = a})
