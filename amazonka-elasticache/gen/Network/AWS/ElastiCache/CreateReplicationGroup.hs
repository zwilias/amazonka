...
--
-- When a Redis (cluster mode disabled) replication group has been successfully created, you can add one or more read replicas to it, up to a total of 5 read replicas. You cannot alter a Redis (cluster mode enabled) replication group after it has been created. However, if you need to increase or decrease the number of node groups (console: shards), you can avail yourself of ElastiCache for Redis' enhanced backup and restore. For more information, see <https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/backups-restoring.html Restoring From a Backup with Cluster Resizing> in the /ElastiCache User Guide/ .
--
module Network.AWS.ElastiCache.CreateReplicationGroup
    (
    -- * Creating a Request
