...
utEnableLogFileValidation =
  lens _utEnableLogFileValidation (\s a -> s {_utEnableLogFileValidation = a})
...
utIsMultiRegionTrail =
  lens _utIsMultiRegionTrail (\s a -> s {_utIsMultiRegionTrail = a})
...
instance AWSRequest UpdateTrail where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateTrailResponse' <$> (x .?> "LogFileValidationEnabled") <*>
         (x .?> "TrailARN") <*>
         (x .?> "S3KeyPrefix") <*>
         (x .?> "SnsTopicARN") <*>
         (x .?> "SnsTopicName") <*>
...
instance ToHeaders UpdateTrail where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("com.amazonaws.cloudtrail.v20131101.CloudTrail_20131101.UpdateTrail" :: ByteString)
...
utrsLogFileValidationEnabled =
  lens
    ...
    (\s a -> s {_utrsLogFileValidationEnabled = a})
...
utrsCloudWatchLogsLogGroupARN =
  lens
    ...
    (\s a -> s {_utrsCloudWatchLogsLogGroupARN = a})
...
utrsCloudWatchLogsRoleARN =
  lens _utrsCloudWatchLogsRoleARN (\s a -> s {_utrsCloudWatchLogsRoleARN = a})
...
utrsIsMultiRegionTrail =
  lens _utrsIsMultiRegionTrail (\s a -> s {_utrsIsMultiRegionTrail = a})

-- | -- | The response status code.
utrsResponseStatus :: Lens' UpdateTrailResponse Int
utrsResponseStatus =
  lens _utrsResponseStatus (\s a -> s {_utrsResponseStatus = a})
