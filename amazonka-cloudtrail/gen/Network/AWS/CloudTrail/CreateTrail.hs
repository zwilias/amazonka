...
ctEnableLogFileValidation =
  lens _ctEnableLogFileValidation (\s a -> s {_ctEnableLogFileValidation = a})
...
ctIsMultiRegionTrail =
  lens _ctIsMultiRegionTrail (\s a -> s {_ctIsMultiRegionTrail = a})
...
instance AWSRequest CreateTrail where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateTrailResponse' <$> (x .?> "LogFileValidationEnabled") <*>
         (x .?> "TrailARN") <*>
         (x .?> "S3KeyPrefix") <*>
         (x .?> "SnsTopicARN") <*>
         (x .?> "SnsTopicName") <*>
...
instance ToHeaders CreateTrail where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("com.amazonaws.cloudtrail.v20131101.CloudTrail_20131101.CreateTrail" :: ByteString)
...
ctrsLogFileValidationEnabled =
  lens
    ...
    (\s a -> s {_ctrsLogFileValidationEnabled = a})
...
ctrsCloudWatchLogsLogGroupARN =
  lens
    _ctrsCloudWatchLogsLogGroupARN
    (\s a -> s {_ctrsCloudWatchLogsLogGroupARN = a})
...
ctrsIsMultiRegionTrail =
  lens _ctrsIsMultiRegionTrail (\s a -> s {_ctrsIsMultiRegionTrail = a})
