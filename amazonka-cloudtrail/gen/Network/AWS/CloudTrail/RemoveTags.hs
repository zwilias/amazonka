...
instance AWSRequest RemoveTags where
  ...
  response =
    receiveEmpty (\s h x -> RemoveTagsResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders RemoveTags where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("com.amazonaws.cloudtrail.v20131101.CloudTrail_20131101.RemoveTags" :: ByteString)
...
instance ToJSON RemoveTags where
  toJSON RemoveTags' {..} =
    object
      (catMaybes
         [("TagsList" .=) <$> _rtTagsList, Just ("ResourceId" .= _rtResourceId)])
