...
pesEventSelectors =
  lens _pesEventSelectors (\s a -> s {_pesEventSelectors = a}) . _Coerce
...
instance AWSRequest PutEventSelectors where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutEventSelectorsResponse' <$> (x .?> "TrailARN") <*>
...
instance ToHeaders PutEventSelectors where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("com.amazonaws.cloudtrail.v20131101.CloudTrail_20131101.PutEventSelectors" :: ByteString)
...
pesrsEventSelectors =
  lens _pesrsEventSelectors (\s a -> s {_pesrsEventSelectors = a}) .
  _Default . _Coerce

-- | -- | The response status code.
pesrsResponseStatus :: Lens' PutEventSelectorsResponse Int
pesrsResponseStatus =
  lens _pesrsResponseStatus (\s a -> s {_pesrsResponseStatus = a})
