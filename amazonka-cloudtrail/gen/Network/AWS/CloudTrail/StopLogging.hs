...
instance AWSRequest StopLogging where
  ...
  response =
    receiveEmpty (\s h x -> StopLoggingResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders StopLogging where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("com.amazonaws.cloudtrail.v20131101.CloudTrail_20131101.StopLogging" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON StopLogging where
  toJSON StopLogging' {..} = object (catMaybes [Just ("Name" .= _slName)])
...
slrsResponseStatus =
  lens _slrsResponseStatus (\s a -> s {_slrsResponseStatus = a})
