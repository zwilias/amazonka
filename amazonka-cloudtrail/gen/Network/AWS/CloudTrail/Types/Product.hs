...
newtype InsightSelector =
  InsightSelector'
    { _isInsightType :: Maybe InsightType
    }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
...
instance FromJSON InsightSelector where
        parseJSON
          = withObject "InsightSelector"
              (\ x -> InsightSelector' <$> (x .:? "InsightType"))

instance Hashable InsightSelector where

instance NFData InsightSelector where
...
trailInfo
    :: TrailInfo
trailInfo =
  TrailInfo' {_tiTrailARN = Nothing, _tiHomeRegion = Nothing, _tiName = Nothing}
...
instance FromJSON TrailInfo where
        parseJSON
          = withObject "TrailInfo"
              (\ x ->
                 TrailInfo' <$>
                   (x .:? "TrailARN") <*> (x .:? "HomeRegion") <*>
                     (x .:? "Name"))

instance Hashable TrailInfo where

instance NFData TrailInfo where
