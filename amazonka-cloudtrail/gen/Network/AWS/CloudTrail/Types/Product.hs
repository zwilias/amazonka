...
instance FromJSON DataResource where
  parseJSON =
    withObject
      "DataResource"
      (\x -> DataResource' <$> (x .:? "Values" .!= mempty) <*> (x .:? "Type"))
...
instance ToJSON DataResource where
  toJSON DataResource' {..} =
    object (catMaybes [("Values" .=) <$> _drValues, ("Type" .=) <$> _drType])
...
instance FromJSON Event where
  parseJSON =
    withObject
      "Event"
      (\x ->
         Event' <$> (x .:? "Username") <*> (x .:? "Resources" .!= mempty) <*>
         (x .:? "EventTime") <*>
         (x .:? "CloudTrailEvent") <*>
         (x .:? "EventName") <*>
         (x .:? "EventSource") <*>
...
esDataResources =
  lens _esDataResources (\s a -> s {_esDataResources = a}) . _Default . _Coerce
...
esIncludeManagementEvents =
  lens _esIncludeManagementEvents (\s a -> s {_esIncludeManagementEvents = a})
...
instance FromJSON EventSelector where
  parseJSON =
    withObject
      "EventSelector"
      (\x ->
         EventSelector' <$> (x .:? "DataResources" .!= mempty) <*>
         (x .:? "ReadWriteType") <*>
...
pkValidityEndTime =
  lens _pkValidityEndTime (\s a -> s {_pkValidityEndTime = a}) . mapping _Time
...
pkValidityStartTime =
  lens _pkValidityStartTime (\s a -> s {_pkValidityStartTime = a}) .
  mapping _Time
...
instance FromJSON PublicKey where
  parseJSON =
    withObject
      "PublicKey"
      (\x ->
         PublicKey' <$> (x .:? "Fingerprint") <*> (x .:? "ValidityEndTime") <*>
         (x .:? "Value") <*>
         (x .:? "ValidityStartTime"))
...
instance FromJSON Resource where
  parseJSON =
    withObject
      "Resource"
      (\x -> Resource' <$> (x .:? "ResourceType") <*> (x .:? "ResourceName"))
...
instance FromJSON ResourceTag where
  parseJSON =
    withObject
      "ResourceTag"
      (\x ->
         ResourceTag' <$> (x .:? "ResourceId") <*> (x .:? "TagsList" .!= mempty))
...
tLogFileValidationEnabled =
  lens _tLogFileValidationEnabled (\s a -> s {_tLogFileValidationEnabled = a})
...
tCloudWatchLogsLogGroupARN =
  lens _tCloudWatchLogsLogGroupARN (\s a -> s {_tCloudWatchLogsLogGroupARN = a})
...
tIsMultiRegionTrail =
  lens _tIsMultiRegionTrail (\s a -> s {_tIsMultiRegionTrail = a})
...
instance FromJSON Trail where
  parseJSON =
    withObject
      "Trail"
      (\x ->
         Trail' <$> (x .:? "LogFileValidationEnabled") <*> (x .:? "TrailARN") <*>
         (x .:? "S3KeyPrefix") <*>
         (x .:? "SnsTopicARN") <*>
         (x .:? "SnsTopicName") <*>
