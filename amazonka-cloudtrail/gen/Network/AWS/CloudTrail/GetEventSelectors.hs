...
instance AWSRequest GetEventSelectors where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetEventSelectorsResponse' <$> (x .?> "TrailARN") <*>
...
instance ToHeaders GetEventSelectors where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("com.amazonaws.cloudtrail.v20131101.CloudTrail_20131101.GetEventSelectors" :: ByteString)
...
gesrsEventSelectors =
  lens _gesrsEventSelectors (\s a -> s {_gesrsEventSelectors = a}) .
  _Default . _Coerce

-- | -- | The response status code.
gesrsResponseStatus :: Lens' GetEventSelectorsResponse Int
gesrsResponseStatus =
  lens _gesrsResponseStatus (\s a -> s {_gesrsResponseStatus = a})
