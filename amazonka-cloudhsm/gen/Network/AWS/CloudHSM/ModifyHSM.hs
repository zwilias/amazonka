...
instance AWSRequest ModifyHSM where
  ...
  response =
    receiveJSON
      (\s h x -> ModifyHSMResponse' <$> (x .?> "HsmArn") <*> (pure (fromEnum s)))
...
instance ToHeaders ModifyHSM where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CloudHsmFrontendService.ModifyHsm" :: ByteString)
...
mhsmrsResponseStatus =
  lens _mhsmrsResponseStatus (\s a -> s {_mhsmrsResponseStatus = a})
