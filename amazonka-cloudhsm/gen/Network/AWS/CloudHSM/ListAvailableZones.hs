...
instance AWSRequest ListAvailableZones where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAvailableZonesResponse' <$> (x .?> "AZList" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lazrsAZList =
  lens _lazrsAZList (\s a -> s {_lazrsAZList = a}) . _Default . _Coerce

-- | -- | The response status code.
lazrsResponseStatus :: Lens' ListAvailableZonesResponse Int
lazrsResponseStatus =
  lens _lazrsResponseStatus (\s a -> s {_lazrsResponseStatus = a})
