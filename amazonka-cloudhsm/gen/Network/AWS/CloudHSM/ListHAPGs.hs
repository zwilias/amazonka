...
instance AWSRequest ListHAPGs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListHAPGsResponse' <$> (x .?> "NextToken") <*> (pure (fromEnum s)) <*>
         (x .?> "HapgList" .!@ mempty))
...
instance ToHeaders ListHAPGs where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CloudHsmFrontendService.ListHapgs" :: ByteString)
...
lhrsResponseStatus =
  lens _lhrsResponseStatus (\s a -> s {_lhrsResponseStatus = a})
