...
instance AWSRequest DeleteHSM where
  ...
  response =
    receiveJSON
      (\s h x -> DeleteHSMResponse' <$> (pure (fromEnum s)) <*> (x .:> "Status"))
...
instance ToHeaders DeleteHSM where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("CloudHsmFrontendService.DeleteHsm" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON DeleteHSM where
  toJSON DeleteHSM' {..} = object (catMaybes [Just ("HsmArn" .= _dhHSMARN)])
...
dhsmrsResponseStatus =
  lens _dhsmrsResponseStatus (\s a -> s {_dhsmrsResponseStatus = a})
