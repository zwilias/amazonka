...
instance AWSRequest DeleteCostCategoryDefinition
         where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 DeleteCostCategoryDefinitionResponse' <$>
                   (x .?> "CostCategoryArn") <*> (x .?> "EffectiveEnd")
                     <*> (pure (fromEnum s)))

instance Hashable DeleteCostCategoryDefinition where

instance NFData DeleteCostCategoryDefinition where
