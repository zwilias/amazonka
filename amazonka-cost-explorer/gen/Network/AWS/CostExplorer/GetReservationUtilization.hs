...
instance AWSRequest GetReservationUtilization where
  type Rs GetReservationUtilization = GetReservationUtilizationResponse
  request = postJSON costExplorer
  response =
    receiveJSON
      (\s h x ->
         GetReservationUtilizationResponse' <$> (x .?> "NextPageToken") <*>
         (x .?> "Total") <*>
         (pure (fromEnum s)) <*>
...
instance ToHeaders GetReservationUtilization where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSInsightsIndexService.GetReservationUtilization" :: ByteString)
...
grursNextPageToken =
  lens _grursNextPageToken (\s a -> s {_grursNextPageToken = a})

-- | The total amount of time that you utilized your RIs.
grursTotal ::
     Lens' GetReservationUtilizationResponse (Maybe ReservationAggregates)
grursTotal = lens _grursTotal (\s a -> s {_grursTotal = a})

-- | -- | The response status code.
grursResponseStatus :: Lens' GetReservationUtilizationResponse Int
grursResponseStatus =
  lens _grursResponseStatus (\s a -> s {_grursResponseStatus = a})

-- | The amount of time that you utilized your RIs.
grursUtilizationsByTime ::
     Lens' GetReservationUtilizationResponse [UtilizationByTime]
grursUtilizationsByTime =
  lens _grursUtilizationsByTime (\s a -> s {_grursUtilizationsByTime = a}) .
  _Coerce
