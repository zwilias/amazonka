...
instance FromText AccountScope where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AccountScope from value: '" <>
        e <> "'. Accepted values: payer"
...
instance FromText Context where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Context from value: '" <>
        e <> "'. Accepted values: cost_and_usage, reservations"
...
instance FromText Dimension where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Dimension from value: '" <>
        e <>
...
instance FromText Granularity where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Granularity from value: '" <>
        e <> "'. Accepted values: daily, monthly"
...
instance FromText GroupDefinitionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing GroupDefinitionType from value: '" <>
        e <> "'. Accepted values: dimension, tag"
...
instance FromText LookbackPeriodInDays where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LookbackPeriodInDays from value: '" <>
        e <> "'. Accepted values: seven_days, sixty_days, thirty_days"
...
instance FromText OfferingClass where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OfferingClass from value: '" <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: all_upfront, no_upfront, partial_upfront"
...
instance FromText TermInYears where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TermInYears from value: '" <>
        e <> "'. Accepted values: one_year, three_years"
