...
instance AWSRequest GetReservationCoverage where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetReservationCoverageResponse' <$> (x .?> "NextPageToken") <*>
         (x .?> "Total") <*>
         (pure (fromEnum s)) <*>
...
grcrsNextPageToken =
  lens _grcrsNextPageToken (\s a -> s {_grcrsNextPageToken = a})
...
grcrsResponseStatus =
  lens _grcrsResponseStatus (\s a -> s {_grcrsResponseStatus = a})

-- | The amount of time that your reservations covered.
grcrsCoveragesByTime :: Lens' GetReservationCoverageResponse [CoverageByTime]
grcrsCoveragesByTime =
  lens _grcrsCoveragesByTime (\s a -> s {_grcrsCoveragesByTime = a}) . _Coerce
