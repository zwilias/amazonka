...
instance AWSRequest GetDimensionValues where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDimensionValuesResponse' <$> (x .?> "NextPageToken") <*>
         (pure (fromEnum s)) <*>
...
gdvrsNextPageToken =
  lens _gdvrsNextPageToken (\s a -> s {_gdvrsNextPageToken = a})
...
gdvrsDimensionValues ::
     Lens' GetDimensionValuesResponse [DimensionValuesWithAttributes]
gdvrsDimensionValues =
  lens _gdvrsDimensionValues (\s a -> s {_gdvrsDimensionValues = a}) . _Coerce
