...
gcauGroupBy =
  lens _gcauGroupBy (\s a -> s {_gcauGroupBy = a}) . _Default . _Coerce
...
gcauMetrics =
  lens _gcauMetrics (\s a -> s {_gcauMetrics = a}) . _Default . _Coerce
...
instance AWSRequest GetCostAndUsage where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCostAndUsageResponse' <$> (x .?> "ResultsByTime" .!@ mempty) <*>
         (x .?> "NextPageToken") <*>
...
gcaursResultsByTime =
  lens _gcaursResultsByTime (\s a -> s {_gcaursResultsByTime = a}) .
  _Default . _Coerce

-- | The token for the next set of retrievable results. AWS provides the token when the response from a previous call has more results than the maximum page size.
gcaursNextPageToken :: Lens' GetCostAndUsageResponse (Maybe Text)
gcaursNextPageToken =
  lens _gcaursNextPageToken (\s a -> s {_gcaursNextPageToken = a})
...
gcaursResponseStatus =
  lens _gcaursResponseStatus (\s a -> s {_gcaursResponseStatus = a})
