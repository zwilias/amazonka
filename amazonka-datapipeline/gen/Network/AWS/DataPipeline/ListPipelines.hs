...
instance AWSRequest ListPipelines where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPipelinesResponse' <$> (x .?> "hasMoreResults") <*>
         (x .?> "marker") <*>
         (pure (fromEnum s)) <*>
...
instance ToJSON ListPipelines where
  toJSON ListPipelines' {..} = object (catMaybes [("marker" .=) <$> _lpMarker])
...
lprsPipelineIdList =
  lens _lprsPipelineIdList (\s a -> s {_lprsPipelineIdList = a}) . _Coerce
