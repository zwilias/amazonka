...
instance AWSRequest EvaluateExpression where
  ...
  response =
    receiveJSON
      (\s h x ->
         EvaluateExpressionResponse' <$> (pure (fromEnum s)) <*>
...
instance ToHeaders EvaluateExpression where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("DataPipeline.EvaluateExpression" :: ByteString)
...
eersResponseStatus =
  lens _eersResponseStatus (\s a -> s {_eersResponseStatus = a})

-- | The evaluated expression.
eersEvaluatedExpression :: Lens' EvaluateExpressionResponse Text
eersEvaluatedExpression =
  lens _eersEvaluatedExpression (\s a -> s {_eersEvaluatedExpression = a})
