...
instance FromJSON Field where
  parseJSON =
    withObject
      "Field"
      (\x ->
         Field' <$> (x .:? "refValue") <*> (x .:? "stringValue") <*>
         (x .: "key"))
...
instance ToJSON Operator where
  toJSON Operator' {..} =
    object (catMaybes [("values" .=) <$> _oValues, ("type" .=) <$> _oType])
...
instance FromJSON ParameterAttribute where
  parseJSON =
    withObject
      "ParameterAttribute"
      (\x -> ParameterAttribute' <$> (x .: "key") <*> (x .: "stringValue"))
...
instance FromJSON ParameterObject where
  parseJSON =
    withObject
      "ParameterObject"
      (\x ->
         ParameterObject' <$> (x .: "id") <*> (x .:? "attributes" .!= mempty))
...
instance ToJSON ParameterObject where
  toJSON ParameterObject' {..} =
    object
      (catMaybes [Just ("id" .= _poId), Just ("attributes" .= _poAttributes)])
...
instance ToJSON ParameterValue where
  toJSON ParameterValue' {..} =
    object
      (catMaybes [Just ("id" .= _pvId), Just ("stringValue" .= _pvStringValue)])
...
instance FromJSON PipelineDescription where
  parseJSON =
    withObject
      "PipelineDescription"
      (\x ->
         PipelineDescription' <$> (x .:? "description") <*>
         (x .:? "tags" .!= mempty) <*>
         (x .: "pipelineId") <*>
         (x .: "name") <*>
         (x .:? "fields" .!= mempty))
...
instance ToJSON Query where
  toJSON Query' {..} = object (catMaybes [("selectors" .=) <$> _qSelectors])
...
instance FromJSON TaskObject where
  parseJSON =
    withObject
      "TaskObject"
      (\x ->
         TaskObject' <$> (x .:? "pipelineId") <*> (x .:? "attemptId") <*>
         (x .:? "taskId") <*>
         (x .:? "objects" .!= mempty))
...
instance FromJSON ValidationError where
  parseJSON =
    withObject
      "ValidationError"
      (\x -> ValidationError' <$> (x .:? "id") <*> (x .:? "errors" .!= mempty))
...
instance FromJSON ValidationWarning where
  parseJSON =
    withObject
      "ValidationWarning"
      (\x ->
         ValidationWarning' <$> (x .:? "warnings" .!= mempty) <*> (x .:? "id"))
