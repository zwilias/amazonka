...
ssContinuationToken =
  lens _ssContinuationToken (\s a -> s {_ssContinuationToken = a})

-- | Specifies the fragment number from where you want the @GetMedia@ API to start returning the fragments.
ssAfterFragmentNumber :: Lens' StartSelector (Maybe Text)
ssAfterFragmentNumber =
  lens _ssAfterFragmentNumber (\s a -> s {_ssAfterFragmentNumber = a})
...
ssStartTimestamp =
  lens _ssStartTimestamp (\s a -> s {_ssStartTimestamp = a}) . mapping _Time
...
ssStartSelectorType =
  lens _ssStartSelectorType (\s a -> s {_ssStartSelectorType = a})
