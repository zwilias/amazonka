...
dplPrefixListIds =
  lens _dplPrefixListIds (\s a -> s {_dplPrefixListIds = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribePrefixLists where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribePrefixListsResponse' <$> (x .@? "nextToken") <*>
...
dplrsPrefixLists =
  lens _dplrsPrefixLists (\s a -> s {_dplrsPrefixLists = a}) .
  _Default . _Coerce

-- | -- | The response status code.
dplrsResponseStatus :: Lens' DescribePrefixListsResponse Int
dplrsResponseStatus =
  lens _dplrsResponseStatus (\s a -> s {_dplrsResponseStatus = a})
