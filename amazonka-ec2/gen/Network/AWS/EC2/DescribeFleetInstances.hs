...
dfisFilters =
  lens _dfisFilters (\s a -> s {_dfisFilters = a}) . _Default . _Coerce
...
instance AWSRequest DescribeFleetInstances where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeFleetInstancesResponse' <$> (x .@? "nextToken") <*>
         (x .@? "fleetId") <*>
         (x .@? "activeInstanceSet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
...
drsActiveInstances =
  lens _drsActiveInstances (\s a -> s {_drsActiveInstances = a}) .
  _Default . _Coerce
