...
dfifFilters =
  lens _dfifFilters (\s a -> s {_dfifFilters = a}) . _Default . _Coerce
...
dfifMaxResults =
  lens _dfifMaxResults (\s a -> s {_dfifMaxResults = a}) . mapping _Nat

-- | One or more AFI IDs.
dfifFpgaImageIds :: Lens' DescribeFpgaImages [Text]
dfifFpgaImageIds =
  lens _dfifFpgaImageIds (\s a -> s {_dfifFpgaImageIds = a}) .
  _Default . _Coerce
...
dfirsFpgaImages =
  lens _dfirsFpgaImages (\s a -> s {_dfirsFpgaImages = a}) . _Default . _Coerce
