...
instance AWSRequest ReplaceNetworkACLAssociation where
  ...
  response =
    receiveXML
      (\s h x ->
         ReplaceNetworkACLAssociationResponse' <$> (x .@? "newAssociationId") <*>
         (pure (fromEnum s)))
...
rnaarsNewAssociationId ::
     Lens' ReplaceNetworkACLAssociationResponse (Maybe Text)
rnaarsNewAssociationId =
...
rnaarsResponseStatus =
  lens _rnaarsResponseStatus (\s a -> s {_rnaarsResponseStatus = a})
