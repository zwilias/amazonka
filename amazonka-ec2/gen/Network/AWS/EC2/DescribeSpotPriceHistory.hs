...
dsphStartTime =
  lens _dsphStartTime (\s a -> s {_dsphStartTime = a}) . mapping _Time
...
dsphFilters =
  lens _dsphFilters (\s a -> s {_dsphFilters = a}) . _Default . _Coerce
...
instance AWSRequest DescribeSpotPriceHistory where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeSpotPriceHistoryResponse' <$> (x .@? "nextToken") <*>
         (x .@? "spotPriceHistorySet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
...
instance ToQuery DescribeSpotPriceHistory where
  toQuery DescribeSpotPriceHistory' {..} =
    mconcat
      ...
      , toQuery (toQueryList "ProductDescription" <$> _dsphProductDescriptions)
      , "DryRun" =: _dsphDryRun
...
dsphrsResponseStatus =
  lens _dsphrsResponseStatus (\s a -> s {_dsphrsResponseStatus = a})
