...
distFilters =
  lens _distFilters (\s a -> s {_distFilters = a}) . _Default . _Coerce
...
instance AWSRequest DescribeImportSnapshotTasks where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeImportSnapshotTasksResponse' <$> (x .@? "nextToken") <*>
         (x .@? "importSnapshotTaskSet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
...
distrsImportSnapshotTasks ::
     Lens' DescribeImportSnapshotTasksResponse [ImportSnapshotTask]
distrsImportSnapshotTasks =
  lens _distrsImportSnapshotTasks (\s a -> s {_distrsImportSnapshotTasks = a}) .
  _Default . _Coerce

-- | -- | The response status code.
distrsResponseStatus :: Lens' DescribeImportSnapshotTasksResponse Int
distrsResponseStatus =
  lens _distrsResponseStatus (\s a -> s {_distrsResponseStatus = a})
