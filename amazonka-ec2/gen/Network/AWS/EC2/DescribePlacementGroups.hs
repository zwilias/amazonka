...
dpgsFilters =
  lens _dpgsFilters (\s a -> s {_dpgsFilters = a}) . _Default . _Coerce
...
dpgsGroupNames =
  lens _dpgsGroupNames (\s a -> s {_dpgsGroupNames = a}) . _Default . _Coerce
...
instance AWSRequest DescribePlacementGroups where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribePlacementGroupsResponse' <$>
         (x .@? "placementGroupSet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
