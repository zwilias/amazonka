...
dssOwnerIds =
  lens _dssOwnerIds (\s a -> s {_dssOwnerIds = a}) . _Default . _Coerce
...
dssSnapshotIds =
  lens _dssSnapshotIds (\s a -> s {_dssSnapshotIds = a}) . _Default . _Coerce

-- | One or more AWS accounts IDs that can create volumes from the snapshot.
dssRestorableByUserIds :: Lens' DescribeSnapshots [Text]
dssRestorableByUserIds =
  lens _dssRestorableByUserIds (\s a -> s {_dssRestorableByUserIds = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeSnapshots where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeSnapshotsResponse' <$> (x .@? "nextToken") <*>
