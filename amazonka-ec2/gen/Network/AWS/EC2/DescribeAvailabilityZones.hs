...
dazZoneNames =
  lens _dazZoneNames (\s a -> s {_dazZoneNames = a}) . _Default . _Coerce
...
instance AWSRequest DescribeAvailabilityZones where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeAvailabilityZonesResponse' <$>
         (x .@? "availabilityZoneInfo" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
...
dazrsAvailabilityZones ::
     Lens' DescribeAvailabilityZonesResponse [AvailabilityZone]
dazrsAvailabilityZones =
...
dazrsResponseStatus =
  lens _dazrsResponseStatus (\s a -> s {_dazrsResponseStatus = a})
