...
cfExcessCapacityTerminationPolicy ::
     Lens' CreateFleet (Maybe FleetExcessCapacityTerminationPolicy)
cfExcessCapacityTerminationPolicy =
  lens
    _cfExcessCapacityTerminationPolicy
    (\s a -> s {_cfExcessCapacityTerminationPolicy = a})

-- | The tags for an EC2 Fleet resource.
cfTagSpecifications :: Lens' CreateFleet [TagSpecification]
cfTagSpecifications =
  lens _cfTagSpecifications (\s a -> s {_cfTagSpecifications = a}) .
  _Default . _Coerce
...
cfValidUntil =
  lens _cfValidUntil (\s a -> s {_cfValidUntil = a}) . mapping _Time
...
cfTerminateInstancesWithExpiration =
  lens
    ...
    (\s a -> s {_cfTerminateInstancesWithExpiration = a})
...
cfReplaceUnhealthyInstances =
  lens
    ...
    (\s a -> s {_cfReplaceUnhealthyInstances = a})
...
cfLaunchTemplateConfigs =
  lens _cfLaunchTemplateConfigs (\s a -> s {_cfLaunchTemplateConfigs = a}) .
  _Coerce

-- | The @TotalTargetCapacity@ , @OnDemandTargetCapacity@ , @SpotTargetCapacity@ , and @DefaultCapacityType@ structure.
cfTargetCapacitySpecification ::
     Lens' CreateFleet TargetCapacitySpecificationRequest
cfTargetCapacitySpecification =
  lens
    _cfTargetCapacitySpecification
    (\s a -> s {_cfTargetCapacitySpecification = a})
...
instance AWSRequest CreateFleet where
  ...
  response =
    receiveXML
      (\s h x ->
         CreateFleetResponse' <$> (x .@? "fleetId") <*> (pure (fromEnum s)))
...
instance ToQuery CreateFleet where
  toQuery CreateFleet' {..} =
    mconcat
      ...
      , "ExcessCapacityTerminationPolicy" =: _cfExcessCapacityTerminationPolicy
