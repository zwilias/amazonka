...
creLogsStorageLocation =
  lens _creLogsStorageLocation (\s a -> s {_creLogsStorageLocation = a})
...
creInputStorageLocation =
  lens _creInputStorageLocation (\s a -> s {_creInputStorageLocation = a})
...
instance AWSRequest CreateFpgaImage where
  ...
  response =
    receiveXML
      (\s h x ->
         CreateFpgaImageResponse' <$> (x .@? "fpgaImageId") <*>
         (x .@? "fpgaImageGlobalId") <*>
         (pure (fromEnum s)))
...
cfirsFpgaImageGlobalId =
  lens _cfirsFpgaImageGlobalId (\s a -> s {_cfirsFpgaImageGlobalId = a})

-- | -- | The response status code.
cfirsResponseStatus :: Lens' CreateFpgaImageResponse Int
cfirsResponseStatus =
  lens _cfirsResponseStatus (\s a -> s {_cfirsResponseStatus = a})
