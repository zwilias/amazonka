...
ghrppHostIdSet =
  lens _ghrppHostIdSet (\s a -> s {_ghrppHostIdSet = a}) . _Coerce
...
instance AWSRequest GetHostReservationPurchasePreview where
  type Rs GetHostReservationPurchasePreview = GetHostReservationPurchasePreviewResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         GetHostReservationPurchasePreviewResponse' <$> (x .@? "currencyCode") <*>
         (x .@? "totalHourlyPrice") <*>
         (x .@? "totalUpfrontPrice") <*>
...
ghrpprsCurrencyCode ::
     Lens' GetHostReservationPurchasePreviewResponse (Maybe CurrencyCodeValues)
ghrpprsCurrencyCode =
  lens _ghrpprsCurrencyCode (\s a -> s {_ghrpprsCurrencyCode = a})

-- | The potential total hourly price of the reservation per hour.
ghrpprsTotalHourlyPrice ::
     Lens' GetHostReservationPurchasePreviewResponse (Maybe Text)
ghrpprsTotalHourlyPrice =
  lens _ghrpprsTotalHourlyPrice (\s a -> s {_ghrpprsTotalHourlyPrice = a})

-- | The potential total upfront price. This is billed immediately.
ghrpprsTotalUpfrontPrice ::
     Lens' GetHostReservationPurchasePreviewResponse (Maybe Text)
ghrpprsTotalUpfrontPrice =
  lens _ghrpprsTotalUpfrontPrice (\s a -> s {_ghrpprsTotalUpfrontPrice = a})

-- | The purchase information of the Dedicated Host Reservation and the Dedicated Hosts associated with it.
ghrpprsPurchase :: Lens' GetHostReservationPurchasePreviewResponse [Purchase]
ghrpprsPurchase =
  lens _ghrpprsPurchase (\s a -> s {_ghrpprsPurchase = a}) . _Default . _Coerce

-- | -- | The response status code.
ghrpprsResponseStatus :: Lens' GetHostReservationPurchasePreviewResponse Int
ghrpprsResponseStatus =
  lens _ghrpprsResponseStatus (\s a -> s {_ghrpprsResponseStatus = a})
