...
rvpcVPCPeeringConnectionId =
  lens _rvpcVPCPeeringConnectionId (\s a -> s {_rvpcVPCPeeringConnectionId = a})
...
instance AWSRequest RejectVPCPeeringConnection where
  ...
  response =
    receiveXML
      (\s h x ->
         RejectVPCPeeringConnectionResponse' <$> (x .@? "return") <*>
         (pure (fromEnum s)))
...
rvpcrsResponseStatus =
  lens _rvpcrsResponseStatus (\s a -> s {_rvpcrsResponseStatus = a})
