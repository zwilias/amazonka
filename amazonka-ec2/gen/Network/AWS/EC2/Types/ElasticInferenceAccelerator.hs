...
data ElasticInferenceAccelerator = ElasticInferenceAccelerator'{_eiaCount
                                                                :: !(Maybe Nat),
                                                                _eiaType ::
                                                                !Text}
...
--
-- * 'eiaType' - The type of elastic inference accelerator. The possible values are @eia1.medium@ , @eia1.large@ , and @eia1.xlarge@ . 
elasticInferenceAccelerator
    :: Text -- ^ 'eiaType'
    -> ElasticInferenceAccelerator
elasticInferenceAccelerator pType_
  = ElasticInferenceAccelerator'{_eiaCount = Nothing,
                                 _eiaType = pType_}
