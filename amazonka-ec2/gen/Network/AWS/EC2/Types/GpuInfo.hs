...
data GpuInfo = GpuInfo'{_giTotalGpuMemoryInMiB ::
                        !(Maybe Int),
...
--
-- * 'giTotalGpuMemoryInMiB' - The total size of the memory for the GPU accelerators for the instance type.
--
-- * 'giGpus' - Describes the GPU accelerators for the instance type.
gpuInfo
    :: GpuInfo
gpuInfo
  = GpuInfo'{_giTotalGpuMemoryInMiB = Nothing,
             _giGpus = Nothing}
