...
data LocalGateway = LocalGateway'{_lgState ::
                                  !(Maybe Text),
...
--
-- * 'lgOwnerId' - The ID of the AWS account ID that owns the local gateway.
--
-- * 'lgTags' - The tags assigned to the local gateway.
localGateway
    :: LocalGateway
localGateway
  = LocalGateway'{_lgState = Nothing,
                  _lgLocalGatewayId = Nothing, _lgOutpostARN = Nothing,
                  _lgOwnerId = Nothing, _lgTags = Nothing}
