...
crVPCPeeringConnectionId =
  lens _crVPCPeeringConnectionId (\s a -> s {_crVPCPeeringConnectionId = a})
...
crEgressOnlyInternetGatewayId =
  lens
    ...
    (\s a -> s {_crEgressOnlyInternetGatewayId = a})

-- | The IPv6 CIDR block used for the destination match. Routing decisions are based on the most specific match.
crDestinationIPv6CidrBlock :: Lens' CreateRoute (Maybe Text)
crDestinationIPv6CidrBlock =
  lens _crDestinationIPv6CidrBlock (\s a -> s {_crDestinationIPv6CidrBlock = a})
