...
instance AWSRequest EnableVPCClassicLinkDNSSupport where
  type Rs EnableVPCClassicLinkDNSSupport = EnableVPCClassicLinkDNSSupportResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         EnableVPCClassicLinkDNSSupportResponse' <$> (x .@? "return") <*>
         (pure (fromEnum s)))
...
evcldsrsResponseStatus =
  lens _evcldsrsResponseStatus (\s a -> s {_evcldsrsResponseStatus = a})
