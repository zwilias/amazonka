...
instance AWSRequest CreateCustomerGateway where
  ...
  response =
    receiveXML
      (\s h x ->
         CreateCustomerGatewayResponse' <$> (x .@? "customerGateway") <*>
         (pure (fromEnum s)))
...
ccgrsCustomerGateway ::
     Lens' CreateCustomerGatewayResponse (Maybe CustomerGateway)
ccgrsCustomerGateway =
...
ccgrsResponseStatus =
  lens _ccgrsResponseStatus (\s a -> s {_ccgrsResponseStatus = a})
