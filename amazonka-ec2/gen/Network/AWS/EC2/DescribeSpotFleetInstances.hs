...
instance AWSRequest DescribeSpotFleetInstances where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeSpotFleetInstancesResponse' <$> (x .@? "nextToken") <*>
         (pure (fromEnum s)) <*>
         (x .@? "activeInstanceSet" .!@ mempty >>= parseXMLList "item") <*>
...
dsfirsResponseStatus =
  lens _dsfirsResponseStatus (\s a -> s {_dsfirsResponseStatus = a})

-- | The running instances. This list is refreshed periodically and might be out of date.
dsfirsActiveInstances ::
     Lens' DescribeSpotFleetInstancesResponse [ActiveInstance]
dsfirsActiveInstances =
  lens _dsfirsActiveInstances (\s a -> s {_dsfirsActiveInstances = a}) . _Coerce

-- | The ID of the Spot Fleet request.
dsfirsSpotFleetRequestId :: Lens' DescribeSpotFleetInstancesResponse Text
dsfirsSpotFleetRequestId =
  lens _dsfirsSpotFleetRequestId (\s a -> s {_dsfirsSpotFleetRequestId = a})
