...
diitFilters =
  lens _diitFilters (\s a -> s {_diitFilters = a}) . _Default . _Coerce
...
instance AWSRequest DescribeImportImageTasks where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeImportImageTasksResponse' <$> (x .@? "nextToken") <*>
         (x .@? "importImageTaskSet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
...
diitrsImportImageTasks ::
     Lens' DescribeImportImageTasksResponse [ImportImageTask]
diitrsImportImageTasks =
  lens _diitrsImportImageTasks (\s a -> s {_diitrsImportImageTasks = a}) .
  _Default . _Coerce

-- | -- | The response status code.
diitrsResponseStatus :: Lens' DescribeImportImageTasksResponse Int
diitrsResponseStatus =
  lens _diitrsResponseStatus (\s a -> s {_diitrsResponseStatus = a})
