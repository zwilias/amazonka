...
-- | The attribute.
rfiaAttribute ::
     Lens' ResetFpgaImageAttribute (Maybe ResetFpgaImageAttributeName)
...
instance AWSRequest ResetFpgaImageAttribute where
  ...
  response =
    receiveXML
      (\s h x ->
         ResetFpgaImageAttributeResponse' <$> (x .@? "return") <*>
         (pure (fromEnum s)))
...
rfiarsResponseStatus =
  lens _rfiarsResponseStatus (\s a -> s {_rfiarsResponseStatus = a})
