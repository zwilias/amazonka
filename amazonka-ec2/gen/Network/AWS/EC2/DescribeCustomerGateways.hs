...
instance AWSRequest DescribeCustomerGateways where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeCustomerGatewaysResponse' <$>
         (x .@? "customerGatewaySet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
...
instance ToQuery DescribeCustomerGateways where
  toQuery DescribeCustomerGateways' {..} =
    mconcat
      ...
      , toQuery (toQueryList "CustomerGatewayId" <$> _dcgCustomerGatewayIds)
...
dcgrsCustomerGateways ::
     Lens' DescribeCustomerGatewaysResponse [CustomerGateway]
dcgrsCustomerGateways =
...
dcgrsResponseStatus =
  lens _dcgrsResponseStatus (\s a -> s {_dcgrsResponseStatus = a})
