...
dpifResources =
  lens _dpifResources (\s a -> s {_dpifResources = a}) . _Default . _Coerce
...
instance AWSRequest DescribePrincipalIdFormat where
  type Rs DescribePrincipalIdFormat = DescribePrincipalIdFormatResponse
...
dpifrsPrincipals =
  lens _dpifrsPrincipals (\s a -> s {_dpifrsPrincipals = a}) .
  _Default . _Coerce
...
dpifrsResponseStatus =
  lens _dpifrsResponseStatus (\s a -> s {_dpifrsResponseStatus = a})
