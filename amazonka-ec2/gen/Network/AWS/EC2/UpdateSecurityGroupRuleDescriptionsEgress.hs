...
usgrdeIPPermissions ::
     Lens' UpdateSecurityGroupRuleDescriptionsEgress [IPPermission]
usgrdeIPPermissions =
  lens _usgrdeIPPermissions (\s a -> s {_usgrdeIPPermissions = a}) . _Coerce

instance AWSRequest UpdateSecurityGroupRuleDescriptionsEgress where
  type Rs UpdateSecurityGroupRuleDescriptionsEgress = UpdateSecurityGroupRuleDescriptionsEgressResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         UpdateSecurityGroupRuleDescriptionsEgressResponse' <$> (x .@? "return") <*>
         (pure (fromEnum s)))
...
instance ToQuery UpdateSecurityGroupRuleDescriptionsEgress where
  toQuery UpdateSecurityGroupRuleDescriptionsEgress' {..} =
    mconcat
      [ "Action" =: ("UpdateSecurityGroupRuleDescriptionsEgress" :: ByteString)
...
usgrdersReturn ::
     Lens' UpdateSecurityGroupRuleDescriptionsEgressResponse (Maybe Bool)
usgrdersReturn = lens _usgrdersReturn (\s a -> s {_usgrdersReturn = a})

-- | -- | The response status code.
usgrdersResponseStatus ::
     Lens' UpdateSecurityGroupRuleDescriptionsEgressResponse Int
usgrdersResponseStatus =
  lens _usgrdersResponseStatus (\s a -> s {_usgrdersResponseStatus = a})
