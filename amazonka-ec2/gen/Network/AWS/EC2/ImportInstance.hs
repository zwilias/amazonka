...
iiLaunchSpecification ::
     Lens' ImportInstance (Maybe ImportInstanceLaunchSpecification)
iiLaunchSpecification =
...
iiDiskImages =
  lens _iiDiskImages (\s a -> s {_iiDiskImages = a}) . _Default . _Coerce
...
iirsResponseStatus =
  lens _iirsResponseStatus (\s a -> s {_iirsResponseStatus = a})
