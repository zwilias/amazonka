...
dssgMaxResults =
  lens _dssgMaxResults (\s a -> s {_dssgMaxResults = a}) . mapping _Nat
...
instance AWSRequest DescribeStaleSecurityGroups where
  type Rs DescribeStaleSecurityGroups = DescribeStaleSecurityGroupsResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         DescribeStaleSecurityGroupsResponse' <$>
         (x .@? "staleSecurityGroupSet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (x .@? "nextToken") <*>
         (pure (fromEnum s)))
...
dssgrsStaleSecurityGroupSet ::
     Lens' DescribeStaleSecurityGroupsResponse [StaleSecurityGroup]
dssgrsStaleSecurityGroupSet =
  lens
    _dssgrsStaleSecurityGroupSet
    (\s a -> s {_dssgrsStaleSecurityGroupSet = a}) .
  _Default . _Coerce
...
dssgrsResponseStatus =
  lens _dssgrsResponseStatus (\s a -> s {_dssgrsResponseStatus = a})
