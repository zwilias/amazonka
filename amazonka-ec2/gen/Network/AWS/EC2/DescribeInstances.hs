...
module Network.AWS.EC2.DescribeInstances
    ...
    -- * Request Lenses
    , dscrbinstncsFilters
    , dscrbinstncsNextToken
    , dscrbinstncsInstanceIds
    , dscrbinstncsDryRun
    , dscrbinstncsMaxResults
...
data DescribeInstances = DescribeInstances'{_dscrbinstncsFilters
                                            :: !(Maybe [Filter]),
                                            _dscrbinstncsNextToken ::
                                            !(Maybe Text),
                                            _dscrbinstncsInstanceIds ::
                                            !(Maybe [Text]),
                                            _dscrbinstncsDryRun ::
                                            !(Maybe Bool),
                                            _dscrbinstncsMaxResults ::
                                            !(Maybe Int)}
...
dscrbinstncsFilters :: Lens' DescribeInstances [Filter]
dscrbinstncsFilters = lens _dscrbinstncsFilters (\ s a -> s{_dscrbinstncsFilters = a}) . _Default . _Coerce

-- | The token to request the next page of results.
dscrbinstncsNextToken :: Lens' DescribeInstances (Maybe Text)
dscrbinstncsNextToken = lens _dscrbinstncsNextToken (\ s a -> s{_dscrbinstncsNextToken = a})

-- | The instance IDs. Default: Describes all your instances.
dscrbinstncsInstanceIds :: Lens' DescribeInstances [Text]
dscrbinstncsInstanceIds = lens _dscrbinstncsInstanceIds (\ s a -> s{_dscrbinstncsInstanceIds = a}) . _Default . _Coerce

-- | Checks whether you have the required permissions for the action, without actually making the request, and provides an error response. If you have the required permissions, the error response is @DryRunOperation@ . Otherwise, it is @UnauthorizedOperation@ .
dscrbinstncsDryRun :: Lens' DescribeInstances (Maybe Bool)
dscrbinstncsDryRun = lens _dscrbinstncsDryRun (\ s a -> s{_dscrbinstncsDryRun = a})

-- | The maximum number of results to return in a single call. To retrieve the remaining results, make another call with the returned @NextToken@ value. This value can be between 5 and 1000. You cannot specify this parameter and the instance IDs parameter in the same call.
dscrbinstncsMaxResults :: Lens' DescribeInstances (Maybe Int)
dscrbinstncsMaxResults = lens _dscrbinstncsMaxResults (\ s a -> s{_dscrbinstncsMaxResults = a})
...
instance AWSPager DescribeInstances where
        page rq rs
          ...
          | otherwise =
            Just $ rq &
              dscrbinstncsNextToken .~ rs ^. dirsNextToken
        ...
        toQuery DescribeInstances'{..}
          = mconcat
              ["Action" =: ("DescribeInstances" :: ByteString),
               ...
               toQuery
                 (toQueryList "Filter" <$> _dscrbinstncsFilters),
               "NextToken" =: _dscrbinstncsNextToken,
               toQuery
                 (toQueryList "InstanceId" <$>
                    _dscrbinstncsInstanceIds),
               "DryRun" =: _dscrbinstncsDryRun,
               "MaxResults" =: _dscrbinstncsMaxResults]
