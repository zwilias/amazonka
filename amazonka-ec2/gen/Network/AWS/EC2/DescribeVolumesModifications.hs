...
dvmVolumeIds =
  lens _dvmVolumeIds (\s a -> s {_dvmVolumeIds = a}) . _Default . _Coerce
...
instance AWSRequest DescribeVolumesModifications where
  type Rs DescribeVolumesModifications = DescribeVolumesModificationsResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         DescribeVolumesModificationsResponse' <$>
         (x .@? "volumeModificationSet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (x .@? "nextToken") <*>
         (pure (fromEnum s)))
...
dvmrsVolumesModifications ::
     Lens' DescribeVolumesModificationsResponse [VolumeModification]
dvmrsVolumesModifications =
  lens _dvmrsVolumesModifications (\s a -> s {_dvmrsVolumesModifications = a}) .
  _Default . _Coerce
...
dvmrsResponseStatus =
  lens _dvmrsResponseStatus (\s a -> s {_dvmrsResponseStatus = a})
