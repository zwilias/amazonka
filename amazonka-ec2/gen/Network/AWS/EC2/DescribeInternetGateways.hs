...
dInternetGatewayIds =
  lens _dInternetGatewayIds (\s a -> s {_dInternetGatewayIds = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeInternetGateways where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeInternetGatewaysResponse' <$>
         (x .@? "internetGatewaySet" .!@ mempty >>= may (parseXMLList "item")) <*>
         (pure (fromEnum s)))
...
digrsInternetGateways ::
     Lens' DescribeInternetGatewaysResponse [InternetGateway]
digrsInternetGateways =
...
digrsResponseStatus =
  lens _digrsResponseStatus (\s a -> s {_digrsResponseStatus = a})
