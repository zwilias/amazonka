...
cniPrivateIPAddresses ::
     Lens' CreateNetworkInterface [PrivateIPAddressSpecification]
cniPrivateIPAddresses =
...
cniIPv6AddressCount =
  lens _cniIPv6AddressCount (\s a -> s {_cniIPv6AddressCount = a})
...
cniIPv6Addresses =
  lens _cniIPv6Addresses (\s a -> s {_cniIPv6Addresses = a}) .
  _Default . _Coerce
...
instance AWSRequest CreateNetworkInterface where
  ...
  response =
    receiveXML
      (\s h x ->
         CreateNetworkInterfaceResponse' <$> (x .@? "networkInterface") <*>
         (pure (fromEnum s)))
...
instance ToQuery CreateNetworkInterface where
  toQuery CreateNetworkInterface' {..} =
    mconcat
      ...
      , toQuery (toQueryList "PrivateIpAddresses" <$> _cniPrivateIPAddresses)
...
cnirsNetworkInterface ::
     Lens' CreateNetworkInterfaceResponse (Maybe NetworkInterface)
cnirsNetworkInterface =
