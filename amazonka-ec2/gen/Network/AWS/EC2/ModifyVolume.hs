...
instance AWSRequest ModifyVolume where
  ...
  response =
    receiveXML
      (\s h x ->
         ModifyVolumeResponse' <$> (x .@? "volumeModification") <*>
         (pure (fromEnum s)))
...
mvrsVolumeModification =
  lens _mvrsVolumeModification (\s a -> s {_mvrsVolumeModification = a})

-- | -- | The response status code.
mvrsResponseStatus :: Lens' ModifyVolumeResponse Int
mvrsResponseStatus =
  lens _mvrsResponseStatus (\s a -> s {_mvrsResponseStatus = a})
