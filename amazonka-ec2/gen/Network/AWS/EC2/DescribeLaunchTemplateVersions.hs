...
dltvsLaunchTemplateName =
  lens _dltvsLaunchTemplateName (\s a -> s {_dltvsLaunchTemplateName = a})

-- | The ID of the launch template. You must specify either the launch template ID or launch template name in the request.
dltvsLaunchTemplateId :: Lens' DescribeLaunchTemplateVersions (Maybe Text)
dltvsLaunchTemplateId =
  lens _dltvsLaunchTemplateId (\s a -> s {_dltvsLaunchTemplateId = a})
...
dltvsFilters =
  lens _dltvsFilters (\s a -> s {_dltvsFilters = a}) . _Default . _Coerce
...
dltvsVersions =
  lens _dltvsVersions (\s a -> s {_dltvsVersions = a}) . _Default . _Coerce
...
instance AWSRequest DescribeLaunchTemplateVersions where
  type Rs DescribeLaunchTemplateVersions = DescribeLaunchTemplateVersionsResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         DescribeLaunchTemplateVersionsResponse' <$> (x .@? "nextToken") <*>
...
dltvrsLaunchTemplateVersions ::
     Lens' DescribeLaunchTemplateVersionsResponse [LaunchTemplateVersion]
dltvrsLaunchTemplateVersions =
  lens
    _dltvrsLaunchTemplateVersions
    (\s a -> s {_dltvrsLaunchTemplateVersions = a}) .
  _Default . _Coerce

-- | -- | The response status code.
dltvrsResponseStatus :: Lens' DescribeLaunchTemplateVersionsResponse Int
dltvrsResponseStatus =
  lens _dltvrsResponseStatus (\s a -> s {_dltvrsResponseStatus = a})
