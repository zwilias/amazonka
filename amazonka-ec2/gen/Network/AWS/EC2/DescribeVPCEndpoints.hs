...
dvpceFilters =
  lens _dvpceFilters (\s a -> s {_dvpceFilters = a}) . _Default . _Coerce
...
dvpceVPCEndpointIds =
  lens _dvpceVPCEndpointIds (\s a -> s {_dvpceVPCEndpointIds = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeVPCEndpoints where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeVPCEndpointsResponse' <$> (x .@? "nextToken") <*>
...
dvpcersVPCEndpoints =
  lens _dvpcersVPCEndpoints (\s a -> s {_dvpcersVPCEndpoints = a}) .
  _Default . _Coerce

-- | -- | The response status code.
dvpcersResponseStatus :: Lens' DescribeVPCEndpointsResponse Int
dvpcersResponseStatus =
  lens _dvpcersResponseStatus (\s a -> s {_dvpcersResponseStatus = a})
