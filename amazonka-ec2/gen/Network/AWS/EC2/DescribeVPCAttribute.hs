...
instance AWSRequest DescribeVPCAttribute where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeVPCAttributeResponse' <$> (x .@? "enableDnsHostnames") <*>
         (x .@? "enableDnsSupport") <*>
         (x .@? "vpcId") <*>
         (pure (fromEnum s)))
...
dvpcarsEnableDNSHostnames ::
     Lens' DescribeVPCAttributeResponse (Maybe AttributeBooleanValue)
dvpcarsEnableDNSHostnames =
  lens _dvpcarsEnableDNSHostnames (\s a -> s {_dvpcarsEnableDNSHostnames = a})
...
dvpcarsResponseStatus =
  lens _dvpcarsResponseStatus (\s a -> s {_dvpcarsResponseStatus = a})
