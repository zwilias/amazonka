...
instance AWSRequest DisableVPCClassicLinkDNSSupport where
  type Rs DisableVPCClassicLinkDNSSupport = DisableVPCClassicLinkDNSSupportResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         DisableVPCClassicLinkDNSSupportResponse' <$> (x .@? "return") <*>
         (pure (fromEnum s)))
...
dvcldsrsResponseStatus =
  lens _dvcldsrsResponseStatus (\s a -> s {_dvcldsrsResponseStatus = a})
