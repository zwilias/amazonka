...
instance AWSRequest DescribeFleetHistory where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeFleetHistoryResponse' <$> (x .@? "startTime") <*>
         (x .@? "lastEvaluatedTime") <*>
         (x .@? "nextToken") <*>
...
dfhrsStartTime =
  lens _dfhrsStartTime (\s a -> s {_dfhrsStartTime = a}) . mapping _Time
...
dfhrsLastEvaluatedTime =
  lens _dfhrsLastEvaluatedTime (\s a -> s {_dfhrsLastEvaluatedTime = a}) .
  mapping _Time
...
dfhrsHistoryRecords =
  lens _dfhrsHistoryRecords (\s a -> s {_dfhrsHistoryRecords = a}) .
  _Default . _Coerce
...
dfhrsResponseStatus =
  lens _dfhrsResponseStatus (\s a -> s {_dfhrsResponseStatus = a})
