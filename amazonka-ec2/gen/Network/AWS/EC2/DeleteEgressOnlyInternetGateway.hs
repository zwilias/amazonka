...
deoigeEgressOnlyInternetGatewayId =
  lens
    ...
    (\s a -> s {_deoigeEgressOnlyInternetGatewayId = a})

instance AWSRequest DeleteEgressOnlyInternetGateway where
  type Rs DeleteEgressOnlyInternetGateway = DeleteEgressOnlyInternetGatewayResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         DeleteEgressOnlyInternetGatewayResponse' <$> (x .@? "returnCode") <*>
         (pure (fromEnum s)))
...
instance ToQuery DeleteEgressOnlyInternetGateway where
  toQuery DeleteEgressOnlyInternetGateway' {..} =
    mconcat
      ...
      , "EgressOnlyInternetGatewayId" =: _deoigeEgressOnlyInternetGatewayId
      ]
...
deoigersReturnCode =
  lens _deoigersReturnCode (\s a -> s {_deoigersReturnCode = a})

-- | -- | The response status code.
deoigersResponseStatus :: Lens' DeleteEgressOnlyInternetGatewayResponse Int
deoigersResponseStatus =
  lens _deoigersResponseStatus (\s a -> s {_deoigersResponseStatus = a})
