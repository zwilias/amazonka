...
riBillingProducts =
  lens _riBillingProducts (\s a -> s {_riBillingProducts = a}) .
  _Default . _Coerce
...
instance AWSRequest RegisterImage where
  ...
  response =
    receiveXML
      (\s h x ->
         RegisterImageResponse' <$> (x .@? "imageId") <*> (pure (fromEnum s)))
...
instance ToQuery RegisterImage where
  toQuery RegisterImage' {..} =
    mconcat
      ...
      , toQuery (toQueryList "BlockDeviceMapping" <$> _riBlockDeviceMappings)
      , "DryRun" =: _riDryRun
      , "Name" =: _riName
      ]
