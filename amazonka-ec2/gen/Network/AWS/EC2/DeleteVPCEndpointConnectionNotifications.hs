...
dvecnConnectionNotificationIds ::
     Lens' DeleteVPCEndpointConnectionNotifications [Text]
dvecnConnectionNotificationIds =
  lens
    _dvecnConnectionNotificationIds
    (\s a -> s {_dvecnConnectionNotificationIds = a}) .
  _Coerce

instance AWSRequest DeleteVPCEndpointConnectionNotifications where
  type Rs DeleteVPCEndpointConnectionNotifications = DeleteVPCEndpointConnectionNotificationsResponse
...
instance ToQuery DeleteVPCEndpointConnectionNotifications where
  toQuery DeleteVPCEndpointConnectionNotifications' {..} =
    mconcat
      [ "Action" =: ("DeleteVpcEndpointConnectionNotifications" :: ByteString)
      , "Version" =: ("2016-11-15" :: ByteString)
      , "DryRun" =: _dvecnDryRun
      , toQueryList "ConnectionNotificationId" _dvecnConnectionNotificationIds
      ]
...
dvecnrsUnsuccessful ::
     Lens' DeleteVPCEndpointConnectionNotificationsResponse [UnsuccessfulItem]
dvecnrsUnsuccessful =
  lens _dvecnrsUnsuccessful (\s a -> s {_dvecnrsUnsuccessful = a}) .
  _Default . _Coerce

-- | -- | The response status code.
dvecnrsResponseStatus ::
     Lens' DeleteVPCEndpointConnectionNotificationsResponse Int
dvecnrsResponseStatus =
  lens _dvecnrsResponseStatus (\s a -> s {_dvecnrsResponseStatus = a})
