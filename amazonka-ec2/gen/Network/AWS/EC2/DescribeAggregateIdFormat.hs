...
instance AWSRequest DescribeAggregateIdFormat where
  type Rs DescribeAggregateIdFormat = DescribeAggregateIdFormatResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         DescribeAggregateIdFormatResponse' <$> (x .@? "useLongIdsAggregated") <*>
...
daifrsUseLongIdsAggregated ::
     Lens' DescribeAggregateIdFormatResponse (Maybe Bool)
daifrsUseLongIdsAggregated =
  lens _daifrsUseLongIdsAggregated (\s a -> s {_daifrsUseLongIdsAggregated = a})

-- | Information about each resource's ID format.
daifrsStatuses :: Lens' DescribeAggregateIdFormatResponse [IdFormat]
daifrsStatuses =
  lens _daifrsStatuses (\s a -> s {_daifrsStatuses = a}) . _Default . _Coerce

-- | -- | The response status code.
daifrsResponseStatus :: Lens' DescribeAggregateIdFormatResponse Int
daifrsResponseStatus =
  lens _daifrsResponseStatus (\s a -> s {_daifrsResponseStatus = a})
