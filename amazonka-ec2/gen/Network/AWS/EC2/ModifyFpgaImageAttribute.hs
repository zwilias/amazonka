...
mfiaUserIds =
  lens _mfiaUserIds (\s a -> s {_mfiaUserIds = a}) . _Default . _Coerce
...
mfiaUserGroups =
  lens _mfiaUserGroups (\s a -> s {_mfiaUserGroups = a}) . _Default . _Coerce

-- | The load permission for the AFI.
mfiaLoadPermission ::
     Lens' ModifyFpgaImageAttribute (Maybe LoadPermissionModifications)
mfiaLoadPermission =
  lens _mfiaLoadPermission (\s a -> s {_mfiaLoadPermission = a})
...
mfiaProductCodes =
  lens _mfiaProductCodes (\s a -> s {_mfiaProductCodes = a}) .
  _Default . _Coerce
...
instance AWSRequest ModifyFpgaImageAttribute where
  ...
  response =
    receiveXML
      (\s h x ->
         ModifyFpgaImageAttributeResponse' <$> (x .@? "fpgaImageAttribute") <*>
         (pure (fromEnum s)))
...
mfiarsFpgaImageAttribute ::
     Lens' ModifyFpgaImageAttributeResponse (Maybe FpgaImageAttribute)
mfiarsFpgaImageAttribute =
  lens _mfiarsFpgaImageAttribute (\s a -> s {_mfiarsFpgaImageAttribute = a})

-- | -- | The response status code.
mfiarsResponseStatus :: Lens' ModifyFpgaImageAttributeResponse Int
mfiarsResponseStatus =
  lens _mfiarsResponseStatus (\s a -> s {_mfiarsResponseStatus = a})
