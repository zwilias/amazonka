...
instance AWSRequest ReplaceRouteTableAssociation where
  ...
  response =
    receiveXML
      (\s h x ->
         ReplaceRouteTableAssociationResponse' <$> (x .@? "newAssociationId") <*>
         (pure (fromEnum s)))
...
rrtarsNewAssociationId ::
     Lens' ReplaceRouteTableAssociationResponse (Maybe Text)
rrtarsNewAssociationId =
...
rrtarsResponseStatus =
  lens _rrtarsResponseStatus (\s a -> s {_rrtarsResponseStatus = a})
