...
dsgsFilters =
  lens _dsgsFilters (\s a -> s {_dsgsFilters = a}) . _Default . _Coerce
...
dsgsGroupNames =
  lens _dsgsGroupNames (\s a -> s {_dsgsGroupNames = a}) . _Default . _Coerce
...
dsgsGroupIds =
  lens _dsgsGroupIds (\s a -> s {_dsgsGroupIds = a}) . _Default . _Coerce
...
instance AWSRequest DescribeSecurityGroups where
  ...
  response =
    receiveXML
      (\s h x ->
         DescribeSecurityGroupsResponse' <$>
         (x .@? "securityGroupInfo" .!@ mempty >>= may (parseXMLList "item")) <*>
         (x .@? "nextToken") <*>
         (pure (fromEnum s)))
...
dsgrsResponseStatus =
  lens _dsgrsResponseStatus (\s a -> s {_dsgrsResponseStatus = a})
