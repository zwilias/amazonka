...
dnipNetworkInterfacePermissionId =
  lens
    ...
    (\s a -> s {_dnipNetworkInterfacePermissionId = a})

instance AWSRequest DeleteNetworkInterfacePermission where
  type Rs DeleteNetworkInterfacePermission = DeleteNetworkInterfacePermissionResponse
  request = postQuery ec2
  response =
    receiveXML
      (\s h x ->
         DeleteNetworkInterfacePermissionResponse' <$> (x .@? "return") <*>
         (pure (fromEnum s)))
...
instance ToQuery DeleteNetworkInterfacePermission where
  toQuery DeleteNetworkInterfacePermission' {..} =
    mconcat
      ...
      , "Force" =: _dnipForce
      , "DryRun" =: _dnipDryRun
      , "NetworkInterfacePermissionId" =: _dnipNetworkInterfacePermissionId
      ]
...
dniprsResponseStatus =
  lens _dniprsResponseStatus (\s a -> s {_dniprsResponseStatus = a})
