...
instance AWSRequest GetCampaign where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCampaignResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
instance ToPath GetCampaign where
  toPath GetCampaign' {..} =
    mconcat
      ["/v1/apps/", toBS _getApplicationId, "/campaigns/", toBS _getCampaignId]
...
gcrsCampaignResponse =
  lens _gcrsCampaignResponse (\s a -> s {_gcrsCampaignResponse = a})
