...
instance AWSRequest GetExportJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetExportJobsResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
gejrsResponseStatus =
  lens _gejrsResponseStatus (\s a -> s {_gejrsResponseStatus = a})

-- | Undocumented member.
gejrsExportJobsResponse :: Lens' GetExportJobsResponse ExportJobsResponse
gejrsExportJobsResponse =
  lens _gejrsExportJobsResponse (\s a -> s {_gejrsExportJobsResponse = a})
