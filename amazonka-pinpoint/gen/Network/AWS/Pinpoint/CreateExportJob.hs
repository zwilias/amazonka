...
cejExportJobRequest =
  lens _cejExportJobRequest (\s a -> s {_cejExportJobRequest = a})
...
instance AWSRequest CreateExportJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateExportJobResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON CreateExportJob where
  toJSON CreateExportJob' {..} =
    object (catMaybes [Just ("ExportJobRequest" .= _cejExportJobRequest)])
...
cejrsResponseStatus =
  lens _cejrsResponseStatus (\s a -> s {_cejrsResponseStatus = a})

-- | Undocumented member.
cejrsExportJobResponse :: Lens' CreateExportJobResponse ExportJobResponse
cejrsExportJobResponse =
  lens _cejrsExportJobResponse (\s a -> s {_cejrsExportJobResponse = a})
