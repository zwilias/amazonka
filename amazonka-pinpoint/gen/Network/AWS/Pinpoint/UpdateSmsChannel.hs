...
uscSMSChannelRequest =
  lens _uscSMSChannelRequest (\s a -> s {_uscSMSChannelRequest = a})
...
instance AWSRequest UpdateSmsChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateSmsChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON UpdateSmsChannel where
  toJSON UpdateSmsChannel' {..} =
    object (catMaybes [Just ("SMSChannelRequest" .= _uscSMSChannelRequest)])
...
uscrsSMSChannelResponse =
  lens _uscrsSMSChannelResponse (\s a -> s {_uscrsSMSChannelResponse = a})
