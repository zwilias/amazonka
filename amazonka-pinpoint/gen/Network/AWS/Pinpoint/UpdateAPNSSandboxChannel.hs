...
-- | Undocumented member.
uascAPNSSandboxChannelRequest ::
     Lens' UpdateAPNSSandboxChannel APNSSandboxChannelRequest
uascAPNSSandboxChannelRequest =
  lens
    _uascAPNSSandboxChannelRequest
    (\s a -> s {_uascAPNSSandboxChannelRequest = a})
...
instance AWSRequest UpdateAPNSSandboxChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateAPNSSandboxChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON UpdateAPNSSandboxChannel where
  toJSON UpdateAPNSSandboxChannel' {..} =
    object
      (catMaybes
         [Just ("APNSSandboxChannelRequest" .= _uascAPNSSandboxChannelRequest)])

instance ToPath UpdateAPNSSandboxChannel where
  toPath UpdateAPNSSandboxChannel' {..} =
    mconcat ["/v1/apps/", toBS _uascApplicationId, "/channels/apns_sandbox"]
...
uascrsResponseStatus =
  lens _uascrsResponseStatus (\s a -> s {_uascrsResponseStatus = a})

-- | Undocumented member.
uascrsAPNSSandboxChannelResponse ::
     Lens' UpdateAPNSSandboxChannelResponse APNSSandboxChannelResponse
uascrsAPNSSandboxChannelResponse =
  lens
    _uascrsAPNSSandboxChannelResponse
    (\s a -> s {_uascrsAPNSSandboxChannelResponse = a})
