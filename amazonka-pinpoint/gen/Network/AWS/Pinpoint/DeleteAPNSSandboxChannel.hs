...
instance AWSRequest DeleteAPNSSandboxChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteAPNSSandboxChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToPath DeleteAPNSSandboxChannel where
  toPath DeleteAPNSSandboxChannel' {..} =
    mconcat ["/v1/apps/", toBS _dascApplicationId, "/channels/apns_sandbox"]
...
dascrsResponseStatus =
  lens _dascrsResponseStatus (\s a -> s {_dascrsResponseStatus = a})

-- | Undocumented member.
dascrsAPNSSandboxChannelResponse ::
     Lens' DeleteAPNSSandboxChannelResponse APNSSandboxChannelResponse
dascrsAPNSSandboxChannelResponse =
  lens
    _dascrsAPNSSandboxChannelResponse
    (\s a -> s {_dascrsAPNSSandboxChannelResponse = a})
