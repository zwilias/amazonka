...
uacAPNSChannelRequest =
  lens _uacAPNSChannelRequest (\s a -> s {_uacAPNSChannelRequest = a})
...
instance AWSRequest UpdateAPNSChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateAPNSChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON UpdateAPNSChannel where
  toJSON UpdateAPNSChannel' {..} =
    object (catMaybes [Just ("APNSChannelRequest" .= _uacAPNSChannelRequest)])
...
uacrsResponseStatus =
  lens _uacrsResponseStatus (\s a -> s {_uacrsResponseStatus = a})

-- | Undocumented member.
uacrsAPNSChannelResponse :: Lens' UpdateAPNSChannelResponse APNSChannelResponse
uacrsAPNSChannelResponse =
  lens _uacrsAPNSChannelResponse (\s a -> s {_uacrsAPNSChannelResponse = a})
