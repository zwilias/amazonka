...
instance AWSRequest GetImportJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetImportJobResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
instance ToPath GetImportJob where
  toPath GetImportJob' {..} =
    mconcat
      ["/v1/apps/", toBS _gijApplicationId, "/jobs/import/", toBS _gijJobId]
...
gijrsResponseStatus =
  lens _gijrsResponseStatus (\s a -> s {_gijrsResponseStatus = a})

-- | Undocumented member.
gijrsImportJobResponse :: Lens' GetImportJobResponse ImportJobResponse
gijrsImportJobResponse =
  lens _gijrsImportJobResponse (\s a -> s {_gijrsImportJobResponse = a})
