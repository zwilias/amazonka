...
instance AWSRequest GetCampaigns where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCampaignsResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
instance ToPath GetCampaigns where
  toPath GetCampaigns' {..} =
    mconcat ["/v1/apps/", toBS _gcsApplicationId, "/campaigns"]

instance ToQuery GetCampaigns where
  toQuery GetCampaigns' {..} =
    mconcat ["token" =: _gcsToken, "page-size" =: _gcsPageSize]
...
gcsrsCampaignsResponse =
  lens _gcsrsCampaignsResponse (\s a -> s {_gcsrsCampaignsResponse = a})
