...
instance AWSRequest DeleteAPNSChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteAPNSChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
dacrsResponseStatus =
  lens _dacrsResponseStatus (\s a -> s {_dacrsResponseStatus = a})

-- | Undocumented member.
dacrsAPNSChannelResponse :: Lens' DeleteAPNSChannelResponse APNSChannelResponse
dacrsAPNSChannelResponse =
  lens _dacrsAPNSChannelResponse (\s a -> s {_dacrsAPNSChannelResponse = a})
