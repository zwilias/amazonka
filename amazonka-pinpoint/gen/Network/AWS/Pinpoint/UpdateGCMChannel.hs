...
ugcGCMChannelRequest =
  lens _ugcGCMChannelRequest (\s a -> s {_ugcGCMChannelRequest = a})
...
instance AWSRequest UpdateGCMChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateGCMChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON UpdateGCMChannel where
  toJSON UpdateGCMChannel' {..} =
    object (catMaybes [Just ("GCMChannelRequest" .= _ugcGCMChannelRequest)])
...
ugcrsResponseStatus =
  lens _ugcrsResponseStatus (\s a -> s {_ugcrsResponseStatus = a})

-- | Undocumented member.
ugcrsGCMChannelResponse :: Lens' UpdateGCMChannelResponse GCMChannelResponse
ugcrsGCMChannelResponse =
  lens _ugcrsGCMChannelResponse (\s a -> s {_ugcrsGCMChannelResponse = a})
