...
cijImportJobRequest =
  lens _cijImportJobRequest (\s a -> s {_cijImportJobRequest = a})
...
instance AWSRequest CreateImportJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateImportJobResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON CreateImportJob where
  toJSON CreateImportJob' {..} =
    object (catMaybes [Just ("ImportJobRequest" .= _cijImportJobRequest)])
...
cijrsResponseStatus =
  lens _cijrsResponseStatus (\s a -> s {_cijrsResponseStatus = a})

-- | Undocumented member.
cijrsImportJobResponse :: Lens' CreateImportJobResponse ImportJobResponse
cijrsImportJobResponse =
  lens _cijrsImportJobResponse (\s a -> s {_cijrsImportJobResponse = a})
