...
instance AWSRequest DeleteEmailChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteEmailChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
-- | Undocumented member.
decrsEmailChannelResponse ::
     Lens' DeleteEmailChannelResponse EmailChannelResponse
decrsEmailChannelResponse =
  lens _decrsEmailChannelResponse (\s a -> s {_decrsEmailChannelResponse = a})
