...
gadmcApplicationId =
  lens _gadmcApplicationId (\s a -> s {_gadmcApplicationId = a})
...
instance AWSRequest GetADMChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetADMChannelResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
gadmcrsResponseStatus =
  lens _gadmcrsResponseStatus (\s a -> s {_gadmcrsResponseStatus = a})

-- | Undocumented member.
gadmcrsADMChannelResponse :: Lens' GetADMChannelResponse ADMChannelResponse
gadmcrsADMChannelResponse =
  lens _gadmcrsADMChannelResponse (\s a -> s {_gadmcrsADMChannelResponse = a})
