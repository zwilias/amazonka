...
instance AWSRequest GetGCMChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetGCMChannelResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
ggcrsResponseStatus =
  lens _ggcrsResponseStatus (\s a -> s {_ggcrsResponseStatus = a})

-- | Undocumented member.
ggcrsGCMChannelResponse :: Lens' GetGCMChannelResponse GCMChannelResponse
ggcrsGCMChannelResponse =
  lens _ggcrsGCMChannelResponse (\s a -> s {_ggcrsGCMChannelResponse = a})
