...
instance AWSRequest GetCampaignVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCampaignVersionsResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
gcvrsResponseStatus =
  lens _gcvrsResponseStatus (\s a -> s {_gcvrsResponseStatus = a})

-- | Undocumented member.
gcvrsCampaignsResponse :: Lens' GetCampaignVersionsResponse CampaignsResponse
gcvrsCampaignsResponse =
  lens _gcvrsCampaignsResponse (\s a -> s {_gcvrsCampaignsResponse = a})
