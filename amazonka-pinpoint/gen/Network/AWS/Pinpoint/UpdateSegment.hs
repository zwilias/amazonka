...
usWriteSegmentRequest =
  lens _usWriteSegmentRequest (\s a -> s {_usWriteSegmentRequest = a})
...
instance AWSRequest UpdateSegment where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateSegmentResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
instance ToJSON UpdateSegment where
  toJSON UpdateSegment' {..} =
    object (catMaybes [Just ("WriteSegmentRequest" .= _usWriteSegmentRequest)])

instance ToPath UpdateSegment where
  toPath UpdateSegment' {..} =
    mconcat
      ["/v1/apps/", toBS _usApplicationId, "/segments/", toBS _usSegmentId]
...
usrsSegmentResponse =
  lens _usrsSegmentResponse (\s a -> s {_usrsSegmentResponse = a})
