...
instance AWSRequest DeleteGCMChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteGCMChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
dgcrsResponseStatus =
  lens _dgcrsResponseStatus (\s a -> s {_dgcrsResponseStatus = a})

-- | Undocumented member.
dgcrsGCMChannelResponse :: Lens' DeleteGCMChannelResponse GCMChannelResponse
dgcrsGCMChannelResponse =
  lens _dgcrsGCMChannelResponse (\s a -> s {_dgcrsGCMChannelResponse = a})
