...
uadmcApplicationId =
  lens _uadmcApplicationId (\s a -> s {_uadmcApplicationId = a})

-- | Undocumented member.
uadmcADMChannelRequest :: Lens' UpdateADMChannel ADMChannelRequest
uadmcADMChannelRequest =
  lens _uadmcADMChannelRequest (\s a -> s {_uadmcADMChannelRequest = a})
...
instance AWSRequest UpdateADMChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateADMChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON UpdateADMChannel where
  toJSON UpdateADMChannel' {..} =
    object (catMaybes [Just ("ADMChannelRequest" .= _uadmcADMChannelRequest)])
...
uadmcrsResponseStatus =
  lens _uadmcrsResponseStatus (\s a -> s {_uadmcrsResponseStatus = a})

-- | Undocumented member.
uadmcrsADMChannelResponse :: Lens' UpdateADMChannelResponse ADMChannelResponse
uadmcrsADMChannelResponse =
  lens _uadmcrsADMChannelResponse (\s a -> s {_uadmcrsADMChannelResponse = a})
