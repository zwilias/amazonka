...
instance AWSRequest GetSmsChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetSmsChannelResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
gscrsResponseStatus =
  lens _gscrsResponseStatus (\s a -> s {_gscrsResponseStatus = a})

-- | Undocumented member.
gscrsSMSChannelResponse :: Lens' GetSmsChannelResponse SMSChannelResponse
gscrsSMSChannelResponse =
  lens _gscrsSMSChannelResponse (\s a -> s {_gscrsSMSChannelResponse = a})
