...
data ResultRowValue = ResultRowValue'{_rrvType ::
                                      !Text,
                                      _rrvValue :: !Text, _rrvKey :: !Text}
...
resultRowValue pType_ pValue_ pKey_
  = ResultRowValue'{_rrvType = pType_,
                    _rrvValue = pValue_, _rrvKey = pKey_}
...
instance FromJSON ResultRowValue where
        parseJSON
          = withObject "ResultRowValue"
              (\ x ->
                 ResultRowValue' <$>
                   (x .: "Type") <*> (x .: "Value") <*> (x .: "Key"))

instance Hashable ResultRowValue where

instance NFData ResultRowValue where
