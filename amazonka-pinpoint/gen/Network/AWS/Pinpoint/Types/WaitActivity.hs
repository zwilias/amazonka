...
data WaitActivity = WaitActivity'{_waNextActivity ::
                                  !(Maybe Text),
...
waitActivity
    :: WaitActivity
waitActivity
  = WaitActivity'{_waNextActivity = Nothing,
                  _waWaitTime = Nothing}
...
instance FromJSON WaitActivity where
        parseJSON
          = withObject "WaitActivity"
              (\ x ->
                 WaitActivity' <$>
                   (x .:? "NextActivity") <*> (x .:? "WaitTime"))

instance Hashable WaitActivity where

instance NFData WaitActivity where
