...
newtype JourneyEmailMessage = JourneyEmailMessage'{_jemFromAddress
                                                   :: Maybe Text}
...
instance FromJSON JourneyEmailMessage where
        parseJSON
          = withObject "JourneyEmailMessage"
              (\ x ->
                 JourneyEmailMessage' <$> (x .:? "FromAddress"))

instance Hashable JourneyEmailMessage where

instance NFData JourneyEmailMessage where
