...
data CreateTemplateMessageBody = CreateTemplateMessageBody'{_ctmbRequestId
                                                            :: !(Maybe Text),
                                                            _ctmbARN ::
                                                            !(Maybe Text),
                                                            _ctmbMessage ::
...
createTemplateMessageBody
    :: CreateTemplateMessageBody
createTemplateMessageBody
  = CreateTemplateMessageBody'{_ctmbRequestId =
                                 Nothing,
                               _ctmbARN = Nothing, _ctmbMessage = Nothing}
...
instance FromJSON CreateTemplateMessageBody where
        parseJSON
          = withObject "CreateTemplateMessageBody"
              (\ x ->
                 CreateTemplateMessageBody' <$>
                   (x .:? "RequestID") <*> (x .:? "Arn") <*>
                     (x .:? "Message"))

instance Hashable CreateTemplateMessageBody where

instance NFData CreateTemplateMessageBody where
