...
instance FromText Action where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Action from value: '" <>
        e <> "'. Accepted values: deep_link, open_app, url"
...
instance FromText AttributeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AttributeType from value: '" <>
        e <> "'. Accepted values: exclusive, inclusive"
...
instance FromText CampaignStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CampaignStatus from value: '" <>
        e <>
...
instance FromText DefinitionFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DefinitionFormat from value: '" <>
        e <> "'. Accepted values: csv, json"
...
instance FromText DimensionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DimensionType from value: '" <>
        e <> "'. Accepted values: exclusive, inclusive"
...
instance FromText Duration where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Duration from value: '" <>
        e <> "'. Accepted values: day_14, day_30, day_7, hr_24"
...
instance FromText Frequency where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Frequency from value: '" <>
        e <> "'. Accepted values: daily, hourly, monthly, once, weekly"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: promotional, transactional"
...
instance FromText Mode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Mode from value: '" <>
        e <> "'. Accepted values: delivery, filter"
...
instance FromText RecencyType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RecencyType from value: '" <>
...
instance FromText SegmentType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SegmentType from value: '" <>
        e <> "'. Accepted values: dimensional, import"
