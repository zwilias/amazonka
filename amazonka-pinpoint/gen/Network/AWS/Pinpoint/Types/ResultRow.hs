...
data ResultRow = ResultRow'{_rrGroupedBys ::
                            ![ResultRowValue],
...
resultRow
    :: ResultRow
resultRow
  = ResultRow'{_rrGroupedBys = mempty,
               _rrValues = mempty}
...
instance FromJSON ResultRow where
        parseJSON
          = withObject "ResultRow"
              (\ x ->
                 ResultRow' <$>
                   (x .:? "GroupedBys" .!= mempty) <*>
                     (x .:? "Values" .!= mempty))

instance Hashable ResultRow where

instance NFData ResultRow where
