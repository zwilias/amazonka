...
data ApplicationDateRangeKpiResponse = ApplicationDateRangeKpiResponse'{_adrkNextToken
                                                                        ::
                                                                        !(Maybe
                                                                            Text),
                                                                        _adrkKpiResult
...
applicationDateRangeKpiResponse
    :: BaseKpiResult -- ^ 'adrkKpiResult'
    -> Text -- ^ 'adrkKpiName'
    -> UTCTime -- ^ 'adrkEndTime'
    -> UTCTime -- ^ 'adrkStartTime'
    -> Text -- ^ 'adrkApplicationId'
    -> ApplicationDateRangeKpiResponse
applicationDateRangeKpiResponse pKpiResult_ pKpiName_
  pEndTime_ pStartTime_ pApplicationId_
  = ApplicationDateRangeKpiResponse'{_adrkNextToken =
                                       Nothing,
...
instance FromJSON ApplicationDateRangeKpiResponse
         where
        parseJSON
          = withObject "ApplicationDateRangeKpiResponse"
              (\ x ->
                 ApplicationDateRangeKpiResponse' <$>
                   (x .:? "NextToken") <*> (x .: "KpiResult") <*>
                     (x .: "KpiName")
                     <*> (x .: "EndTime")
                     <*> (x .: "StartTime")
                     <*> (x .: "ApplicationId"))
...
instance NFData ApplicationDateRangeKpiResponse where
