...
data TemplateConfiguration = TemplateConfiguration'{_tcSMSTemplate
                                                    :: !(Maybe Template),
                                                    _tcVoiceTemplate ::
                                                    !(Maybe Template),
                                                    _tcPushTemplate ::
...
templateConfiguration
    :: TemplateConfiguration
templateConfiguration
  = TemplateConfiguration'{_tcSMSTemplate = Nothing,
...
instance FromJSON TemplateConfiguration where
        parseJSON
          = withObject "TemplateConfiguration"
              (\ x ->
                 TemplateConfiguration' <$>
                   (x .:? "SMSTemplate") <*> (x .:? "VoiceTemplate") <*>
                     (x .:? "PushTemplate")
                     <*> (x .:? "EmailTemplate"))

instance Hashable TemplateConfiguration where

instance NFData TemplateConfiguration where
