...
data EventCondition = EventCondition'{_ecDimensions
...
eventCondition
    :: EventCondition
eventCondition
  = EventCondition'{_ecDimensions = Nothing,
...
instance FromJSON EventCondition where
        parseJSON
          = withObject "EventCondition"
              (\ x ->
                 EventCondition' <$>
                   (x .:? "Dimensions") <*> (x .:? "MessageActivity"))

instance Hashable EventCondition where

instance NFData EventCondition where
