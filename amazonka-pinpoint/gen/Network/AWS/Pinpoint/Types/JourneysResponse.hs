...
data JourneysResponse = JourneysResponse'{_jNextToken
                                          :: !(Maybe Text),
...
journeysResponse
    :: JourneysResponse
journeysResponse
  = JourneysResponse'{_jNextToken = Nothing,
                      _jItem = mempty}
...
instance FromJSON JourneysResponse where
        parseJSON
          = withObject "JourneysResponse"
              (\ x ->
                 JourneysResponse' <$>
                   (x .:? "NextToken") <*> (x .:? "Item" .!= mempty))

instance Hashable JourneysResponse where

instance NFData JourneysResponse where
