...
data TemplatesResponse = TemplatesResponse'{_tNextToken
                                            :: !(Maybe Text),
...
templatesResponse
    :: TemplatesResponse
templatesResponse
  = TemplatesResponse'{_tNextToken = Nothing,
                       _tItem = mempty}
...
instance FromJSON TemplatesResponse where
        parseJSON
          = withObject "TemplatesResponse"
              (\ x ->
                 TemplatesResponse' <$>
                   (x .:? "NextToken") <*> (x .:? "Item" .!= mempty))

instance Hashable TemplatesResponse where

instance NFData TemplatesResponse where
