...
newtype SegmentCondition = SegmentCondition'{_scSegmentId
                                             :: Text}
...
instance FromJSON SegmentCondition where
        parseJSON
          = withObject "SegmentCondition"
              (\ x -> SegmentCondition' <$> (x .: "SegmentId"))

instance Hashable SegmentCondition where

instance NFData SegmentCondition where
