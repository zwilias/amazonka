...
data TemplateVersionsResponse = TemplateVersionsResponse'{_tvRequestId
                                                          :: !(Maybe Text),
                                                          _tvNextToken ::
                                                          !(Maybe Text),
                                                          _tvMessage ::
...
templateVersionsResponse
    :: TemplateVersionsResponse
templateVersionsResponse
  = TemplateVersionsResponse'{_tvRequestId = Nothing,
                              _tvNextToken = Nothing, _tvMessage = Nothing,
                              _tvItem = mempty}
...
instance FromJSON TemplateVersionsResponse where
        parseJSON
          = withObject "TemplateVersionsResponse"
              (\ x ->
                 TemplateVersionsResponse' <$>
                   (x .:? "RequestID") <*> (x .:? "NextToken") <*>
                     (x .:? "Message")
                     <*> (x .:? "Item" .!= mempty))

instance Hashable TemplateVersionsResponse where

instance NFData TemplateVersionsResponse where
