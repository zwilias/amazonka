...
newtype RandomSplitActivity = RandomSplitActivity'{_rsaBranches
...
instance FromJSON RandomSplitActivity where
        parseJSON
          = withObject "RandomSplitActivity"
              (\ x ->
                 RandomSplitActivity' <$>
                   (x .:? "Branches" .!= mempty))

instance Hashable RandomSplitActivity where

instance NFData RandomSplitActivity where
