...
instance AWSRequest DeleteEndpoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteEndpointResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
instance ToPath DeleteEndpoint where
  toPath DeleteEndpoint' {..} =
    mconcat
      ["/v1/apps/", toBS _deApplicationId, "/endpoints/", toBS _deEndpointId]
...
dersEndpointResponse =
  lens _dersEndpointResponse (\s a -> s {_dersEndpointResponse = a})
