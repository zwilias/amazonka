...
dadmcApplicationId =
  lens _dadmcApplicationId (\s a -> s {_dadmcApplicationId = a})
...
instance AWSRequest DeleteADMChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteADMChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
dadmcrsResponseStatus =
  lens _dadmcrsResponseStatus (\s a -> s {_dadmcrsResponseStatus = a})

-- | Undocumented member.
dadmcrsADMChannelResponse :: Lens' DeleteADMChannelResponse ADMChannelResponse
dadmcrsADMChannelResponse =
  lens _dadmcrsADMChannelResponse (\s a -> s {_dadmcrsADMChannelResponse = a})
