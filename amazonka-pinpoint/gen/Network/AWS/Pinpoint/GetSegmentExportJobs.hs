...
instance AWSRequest GetSegmentExportJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetSegmentExportJobsResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
gsejrsResponseStatus =
  lens _gsejrsResponseStatus (\s a -> s {_gsejrsResponseStatus = a})

-- | Undocumented member.
gsejrsExportJobsResponse ::
     Lens' GetSegmentExportJobsResponse ExportJobsResponse
gsejrsExportJobsResponse =
  lens _gsejrsExportJobsResponse (\s a -> s {_gsejrsExportJobsResponse = a})
