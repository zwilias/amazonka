...
instance AWSRequest GetImportJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetImportJobsResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
gijsrsResponseStatus =
  lens _gijsrsResponseStatus (\s a -> s {_gijsrsResponseStatus = a})

-- | Undocumented member.
gijsrsImportJobsResponse :: Lens' GetImportJobsResponse ImportJobsResponse
gijsrsImportJobsResponse =
  lens _gijsrsImportJobsResponse (\s a -> s {_gijsrsImportJobsResponse = a})
