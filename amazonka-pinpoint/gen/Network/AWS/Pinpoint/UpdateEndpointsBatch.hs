...
uebEndpointBatchRequest =
  lens _uebEndpointBatchRequest (\s a -> s {_uebEndpointBatchRequest = a})
...
instance AWSRequest UpdateEndpointsBatch where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateEndpointsBatchResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToJSON UpdateEndpointsBatch where
  toJSON UpdateEndpointsBatch' {..} =
    object
      (catMaybes [Just ("EndpointBatchRequest" .= _uebEndpointBatchRequest)])
...
uebrsResponseStatus =
  lens _uebrsResponseStatus (\s a -> s {_uebrsResponseStatus = a})
