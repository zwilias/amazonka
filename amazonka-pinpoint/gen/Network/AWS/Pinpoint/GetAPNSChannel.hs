...
instance AWSRequest GetAPNSChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetAPNSChannelResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
gacrsResponseStatus =
  lens _gacrsResponseStatus (\s a -> s {_gacrsResponseStatus = a})

-- | Undocumented member.
gacrsAPNSChannelResponse :: Lens' GetAPNSChannelResponse APNSChannelResponse
gacrsAPNSChannelResponse =
  lens _gacrsAPNSChannelResponse (\s a -> s {_gacrsAPNSChannelResponse = a})
