...
sumsSendUsersMessageRequest =
  lens
    ...
    (\s a -> s {_sumsSendUsersMessageRequest = a})
...
instance AWSRequest SendUsersMessages where
  ...
  response =
    receiveJSON
      (\s h x ->
         SendUsersMessagesResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
sumrsResponseStatus =
  lens _sumrsResponseStatus (\s a -> s {_sumrsResponseStatus = a})

-- | Undocumented member.
sumrsSendUsersMessageResponse ::
     Lens' SendUsersMessagesResponse SendUsersMessageResponse
sumrsSendUsersMessageResponse =
  lens
    _sumrsSendUsersMessageResponse
    (\s a -> s {_sumrsSendUsersMessageResponse = a})
