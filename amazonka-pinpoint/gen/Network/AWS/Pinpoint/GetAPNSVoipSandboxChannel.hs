...
gavscApplicationId =
  lens _gavscApplicationId (\s a -> s {_gavscApplicationId = a})

instance AWSRequest GetAPNSVoipSandboxChannel where
  type Rs GetAPNSVoipSandboxChannel = GetAPNSVoipSandboxChannelResponse
  request = get pinpoint
  response =
    receiveJSON
      (\s h x ->
         GetAPNSVoipSandboxChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToPath GetAPNSVoipSandboxChannel where
  toPath GetAPNSVoipSandboxChannel' {..} =
    mconcat
      ["/v1/apps/", toBS _gavscApplicationId, "/channels/apns_voip_sandbox"]
...
gavscrsResponseStatus =
  lens _gavscrsResponseStatus (\s a -> s {_gavscrsResponseStatus = a})

-- | Undocumented member.
gavscrsAPNSVoipSandboxChannelResponse ::
     Lens' GetAPNSVoipSandboxChannelResponse APNSVoipSandboxChannelResponse
gavscrsAPNSVoipSandboxChannelResponse =
  lens
    _gavscrsAPNSVoipSandboxChannelResponse
    (\s a -> s {_gavscrsAPNSVoipSandboxChannelResponse = a})
