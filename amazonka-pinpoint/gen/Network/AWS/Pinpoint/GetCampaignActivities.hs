...
instance AWSRequest GetCampaignActivities where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCampaignActivitiesResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
gcarsResponseStatus =
  lens _gcarsResponseStatus (\s a -> s {_gcarsResponseStatus = a})

-- | Undocumented member.
gcarsActivitiesResponse ::
     Lens' GetCampaignActivitiesResponse ActivitiesResponse
gcarsActivitiesResponse =
  lens _gcarsActivitiesResponse (\s a -> s {_gcarsActivitiesResponse = a})
