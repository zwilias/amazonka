...
instance AWSRequest DeleteAPNSVoipChannel where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteAPNSVoipChannelResponse' <$> (pure (fromEnum s)) <*>
         (eitherParseJSON x))
...
instance ToPath DeleteAPNSVoipChannel where
  toPath DeleteAPNSVoipChannel' {..} =
    mconcat ["/v1/apps/", toBS _davcApplicationId, "/channels/apns_voip"]
...
davcrsResponseStatus =
  lens _davcrsResponseStatus (\s a -> s {_davcrsResponseStatus = a})

-- | Undocumented member.
davcrsAPNSVoipChannelResponse ::
     Lens' DeleteAPNSVoipChannelResponse APNSVoipChannelResponse
davcrsAPNSVoipChannelResponse =
  lens
    _davcrsAPNSVoipChannelResponse
    (\s a -> s {_davcrsAPNSVoipChannelResponse = a})
