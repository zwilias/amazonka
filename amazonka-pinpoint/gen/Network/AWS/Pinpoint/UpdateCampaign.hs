...
ucWriteCampaignRequest =
  lens _ucWriteCampaignRequest (\s a -> s {_ucWriteCampaignRequest = a})
...
instance AWSRequest UpdateCampaign where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateCampaignResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
instance ToJSON UpdateCampaign where
  toJSON UpdateCampaign' {..} =
    object
      (catMaybes [Just ("WriteCampaignRequest" .= _ucWriteCampaignRequest)])

instance ToPath UpdateCampaign where
  toPath UpdateCampaign' {..} =
    mconcat
      ["/v1/apps/", toBS _ucApplicationId, "/campaigns/", toBS _ucCampaignId]
...
ucrsCampaignResponse =
  lens _ucrsCampaignResponse (\s a -> s {_ucrsCampaignResponse = a})
