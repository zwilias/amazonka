...
instance AWSRequest GetEndpoint where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetEndpointResponse' <$> (pure (fromEnum s)) <*> (eitherParseJSON x))
...
instance ToPath GetEndpoint where
  toPath GetEndpoint' {..} =
    mconcat
      ["/v1/apps/", toBS _geApplicationId, "/endpoints/", toBS _geEndpointId]
...
gersEndpointResponse =
  lens _gersEndpointResponse (\s a -> s {_gersEndpointResponse = a})
