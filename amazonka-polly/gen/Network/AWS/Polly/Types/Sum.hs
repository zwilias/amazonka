...
instance FromText Gender where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Gender from value: '" <>
...
instance FromText OutputFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OutputFormat from value: '" <>
        e <> "'. Accepted values: json, mp3, ogg_vorbis, pcm"
...
instance FromText SpeechMarkType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SpeechMarkType from value: '" <>
        e <> "'. Accepted values: sentence, ssml, viseme, word"
...
instance FromText TextType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TextType from value: '" <>
        e <> "'. Accepted values: ssml, text"
...
instance FromText VoiceId where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VoiceId from value: '" <>
        e <>
