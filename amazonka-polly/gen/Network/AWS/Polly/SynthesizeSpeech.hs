...
ssSpeechMarkTypes =
  lens _ssSpeechMarkTypes (\s a -> s {_ssSpeechMarkTypes = a}) .
  _Default . _Coerce
...
ssLexiconNames =
  lens _ssLexiconNames (\s a -> s {_ssLexiconNames = a}) . _Default . _Coerce
...
instance AWSRequest SynthesizeSpeech where
  ...
  response =
    receiveBody
      (\s h x ->
         SynthesizeSpeechResponse' <$> (h .#? "x-amzn-RequestCharacters") <*>
         (h .#? "Content-Type") <*>
         (pure (fromEnum s)) <*>
         (pure x))
...
ssrsRequestCharacters =
  lens _ssrsRequestCharacters (\s a -> s {_ssrsRequestCharacters = a})
...
ssrsResponseStatus =
  lens _ssrsResponseStatus (\s a -> s {_ssrsResponseStatus = a})
