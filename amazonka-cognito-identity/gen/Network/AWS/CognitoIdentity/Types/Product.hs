...
cipServerSideTokenCheck =
  lens _cipServerSideTokenCheck (\s a -> s {_cipServerSideTokenCheck = a})
...
instance FromJSON CognitoIdentityProvider where
  parseJSON =
    withObject
      "CognitoIdentityProvider"
      (\x ->
         CognitoIdentityProvider' <$> (x .:? "ClientId") <*>
...
instance FromJSON Credentials where
  parseJSON =
    withObject
      "Credentials"
      (\x ->
         Credentials' <$> (x .:? "SessionToken") <*> (x .:? "Expiration") <*>
         (x .:? "SecretKey") <*>
         (x .:? "AccessKeyId"))
...
idLastModifiedDate =
  lens _idLastModifiedDate (\s a -> s {_idLastModifiedDate = a}) . mapping _Time

-- | Date on which the identity was created.
idCreationDate :: Lens' IdentityDescription (Maybe UTCTime)
idCreationDate =
  lens _idCreationDate (\s a -> s {_idCreationDate = a}) . mapping _Time
...
instance FromJSON IdentityDescription where
  parseJSON =
    withObject
      "IdentityDescription"
      (\x ->
         IdentityDescription' <$> (x .:? "LastModifiedDate") <*>
         (x .:? "CreationDate") <*>
         (x .:? "Logins" .!= mempty) <*>
         (x .:? "IdentityId"))
...
ipSamlProviderARNs =
  lens _ipSamlProviderARNs (\s a -> s {_ipSamlProviderARNs = a}) .
  _Default . _Coerce

-- | Optional key:value pairs mapping provider names to provider app IDs.
ipSupportedLoginProviders :: Lens' IdentityPool (HashMap Text Text)
ipSupportedLoginProviders =
  lens _ipSupportedLoginProviders (\s a -> s {_ipSupportedLoginProviders = a}) .
  _Default . _Map
...
instance FromJSON IdentityPool where
  parseJSON =
    withObject
      "IdentityPool"
      (\x ->
         IdentityPool' <$> (x .:? "SamlProviderARNs" .!= mempty) <*>
...
instance FromJSON IdentityPoolShortDescription where
  parseJSON =
    withObject
      "IdentityPoolShortDescription"
      (\x ->
         IdentityPoolShortDescription' <$> (x .:? "IdentityPoolId") <*>
         (x .:? "IdentityPoolName"))
...
instance FromJSON MappingRule where
  parseJSON =
    withObject
      "MappingRule"
      (\x ->
         MappingRule' <$> (x .: "Claim") <*> (x .: "MatchType") <*>
         (x .: "Value") <*>
         (x .: "RoleARN"))
...
rmRulesConfiguration =
  lens _rmRulesConfiguration (\s a -> s {_rmRulesConfiguration = a})
...
rmAmbiguousRoleResolution ::
     Lens' RoleMapping (Maybe AmbiguousRoleResolutionType)
rmAmbiguousRoleResolution =
  lens _rmAmbiguousRoleResolution (\s a -> s {_rmAmbiguousRoleResolution = a})
...
instance FromJSON RoleMapping where
  parseJSON =
    withObject
      "RoleMapping"
      (\x ->
         RoleMapping' <$> (x .:? "RulesConfiguration") <*>
...
instance FromJSON UnprocessedIdentityId where
  parseJSON =
    withObject
      "UnprocessedIdentityId"
      (\x ->
         UnprocessedIdentityId' <$> (x .:? "ErrorCode") <*> (x .:? "IdentityId"))
