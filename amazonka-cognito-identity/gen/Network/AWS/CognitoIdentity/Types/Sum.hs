...
instance FromText AmbiguousRoleResolutionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AmbiguousRoleResolutionType from value: '" <>
        e <> "'. Accepted values: authenticatedrole, deny"
...
instance FromText CognitoErrorCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CognitoErrorCode from value: '" <>
        e <> "'. Accepted values: accessdenied, internalservererror"
...
instance FromText MappingRuleMatchType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MappingRuleMatchType from value: '" <>
        e <> "'. Accepted values: contains, equals, notequal, startswith"
...
instance FromText RoleMappingType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RoleMappingType from value: '" <>
        e <> "'. Accepted values: rules, token"
