...
ldiDeveloperUserIdentifier =
  lens _ldiDeveloperUserIdentifier (\s a -> s {_ldiDeveloperUserIdentifier = a})
...
ldiMaxResults =
  lens _ldiMaxResults (\s a -> s {_ldiMaxResults = a}) . mapping _Nat
...
instance AWSRequest LookupDeveloperIdentity where
  ...
  response =
    receiveJSON
      (\s h x ->
         LookupDeveloperIdentityResponse' <$> (x .?> "NextToken") <*>
         (x .?> "IdentityId") <*>
...
instance ToHeaders LookupDeveloperIdentity where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityService.LookupDeveloperIdentity" :: ByteString)
...
ldirsDeveloperUserIdentifierList =
  lens
    ...
    (\s a -> s {_ldirsDeveloperUserIdentifierList = a}) .
  _Default . _Coerce
