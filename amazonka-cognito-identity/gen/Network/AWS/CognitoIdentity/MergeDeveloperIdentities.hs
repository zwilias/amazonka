...
mdiDeveloperProviderName =
  lens _mdiDeveloperProviderName (\s a -> s {_mdiDeveloperProviderName = a})
...
instance AWSRequest MergeDeveloperIdentities where
  ...
  response =
    receiveJSON
      (\s h x ->
         MergeDeveloperIdentitiesResponse' <$> (x .?> "IdentityId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders MergeDeveloperIdentities where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCognitoIdentityService.MergeDeveloperIdentities" :: ByteString)
  ...
  toJSON MergeDeveloperIdentities' {..} =
    object
      (catMaybes
         ...
         , Just ("DestinationUserIdentifier" .= _mdiDestinationUserIdentifier)
