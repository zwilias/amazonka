...
cspScalingPlanName =
  lens _cspScalingPlanName (\s a -> s {_cspScalingPlanName = a})

-- | A CloudFormation stack or set of tags. You can create one scaling plan per application source.
cspApplicationSource :: Lens' CreateScalingPlan ApplicationSource
cspApplicationSource =
  lens _cspApplicationSource (\s a -> s {_cspApplicationSource = a})

-- | The scaling instructions.
cspScalingInstructions :: Lens' CreateScalingPlan [ScalingInstruction]
cspScalingInstructions =
  lens _cspScalingInstructions (\s a -> s {_cspScalingInstructions = a}) .
  _Coerce
...
instance AWSRequest CreateScalingPlan where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateScalingPlanResponse' <$> (pure (fromEnum s)) <*>
...
instance ToHeaders CreateScalingPlan where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AnyScaleScalingPlannerFrontendService.CreateScalingPlan" :: ByteString)
...
csprsResponseStatus =
  lens _csprsResponseStatus (\s a -> s {_csprsResponseStatus = a})

-- | The version of the scaling plan. This value is always 1.
csprsScalingPlanVersion :: Lens' CreateScalingPlanResponse Integer
csprsScalingPlanVersion =
  lens _csprsScalingPlanVersion (\s a -> s {_csprsScalingPlanVersion = a})
