...
dScalingPlanVersion =
  lens _dScalingPlanVersion (\s a -> s {_dScalingPlanVersion = a})

-- | The names of the scaling plans (up to 10). If you specify application sources, you cannot specify scaling plan names.
dScalingPlanNames :: Lens' DescribeScalingPlans [Text]
dScalingPlanNames =
  lens _dScalingPlanNames (\s a -> s {_dScalingPlanNames = a}) .
  _Default . _Coerce
...
dApplicationSources =
  lens _dApplicationSources (\s a -> s {_dApplicationSources = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeScalingPlans where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeScalingPlansResponse' <$> (x .?> "ScalingPlans" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeScalingPlans where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AnyScaleScalingPlannerFrontendService.DescribeScalingPlans" :: ByteString)
...
drsScalingPlans =
  lens _drsScalingPlans (\s a -> s {_drsScalingPlans = a}) . _Default . _Coerce
