...
dsprScalingPlanName =
  lens _dsprScalingPlanName (\s a -> s {_dsprScalingPlanName = a})

-- | The version of the scaling plan.
dsprScalingPlanVersion :: Lens' DescribeScalingPlanResources Integer
dsprScalingPlanVersion =
  lens _dsprScalingPlanVersion (\s a -> s {_dsprScalingPlanVersion = a})

instance AWSRequest DescribeScalingPlanResources where
  type Rs DescribeScalingPlanResources = DescribeScalingPlanResourcesResponse
  request = postJSON autoScalingPlans
  response =
    receiveJSON
      (\s h x ->
         DescribeScalingPlanResourcesResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders DescribeScalingPlanResources where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AnyScaleScalingPlannerFrontendService.DescribeScalingPlanResources" :: ByteString)
...
dsprrsScalingPlanResources ::
     Lens' DescribeScalingPlanResourcesResponse [ScalingPlanResource]
dsprrsScalingPlanResources =
  lens _dsprrsScalingPlanResources (\s a -> s {_dsprrsScalingPlanResources = a}) .
  _Default . _Coerce

-- | -- | The response status code.
dsprrsResponseStatus :: Lens' DescribeScalingPlanResourcesResponse Int
dsprrsResponseStatus =
  lens _dsprrsResponseStatus (\s a -> s {_dsprrsResponseStatus = a})
