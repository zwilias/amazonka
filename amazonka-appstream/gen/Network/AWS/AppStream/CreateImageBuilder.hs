...
cibEnableDefaultInternetAccess =
  lens
    ...
    (\s a -> s {_cibEnableDefaultInternetAccess = a})
...
cibAppstreamAgentVersion =
  lens _cibAppstreamAgentVersion (\s a -> s {_cibAppstreamAgentVersion = a})
...
instance AWSRequest CreateImageBuilder where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateImageBuilderResponse' <$> (x .?> "ImageBuilder") <*>
         (pure (fromEnum s)))
...
cibrsResponseStatus =
  lens _cibrsResponseStatus (\s a -> s {_cibrsResponseStatus = a})
