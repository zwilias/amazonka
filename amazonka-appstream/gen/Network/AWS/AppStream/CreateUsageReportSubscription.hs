...
instance AWSRequest CreateUsageReportSubscription
        ...
        response
          = receiveJSON
              (\ s h x ->
                 CreateUsageReportSubscriptionResponse' <$>
                   (x .?> "Schedule") <*> (x .?> "S3BucketName") <*>
                     (pure (fromEnum s)))

instance Hashable CreateUsageReportSubscription where

instance NFData CreateUsageReportSubscription where

instance ToHeaders CreateUsageReportSubscription
         where
        toHeaders
          = const
              (mconcat
