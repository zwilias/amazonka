...
udcServiceAccountCredentials =
  lens
    ...
    (\s a -> s {_udcServiceAccountCredentials = a})
...
udcOrganizationalUnitDistinguishedNames =
  lens
    ...
    (\s a -> s {_udcOrganizationalUnitDistinguishedNames = a}) .
  _Default . _Coerce
...
instance AWSRequest UpdateDirectoryConfig where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateDirectoryConfigResponse' <$> (x .?> "DirectoryConfig") <*>
         (pure (fromEnum s)))
...
instance ToJSON UpdateDirectoryConfig where
  toJSON UpdateDirectoryConfig' {..} =
    object
      (catMaybes
         [ ("ServiceAccountCredentials" .=) <$> _udcServiceAccountCredentials
...
udcrsDirectoryConfig =
  lens _udcrsDirectoryConfig (\s a -> s {_udcrsDirectoryConfig = a})

-- | -- | The response status code.
udcrsResponseStatus :: Lens' UpdateDirectoryConfigResponse Int
udcrsResponseStatus =
  lens _udcrsResponseStatus (\s a -> s {_udcrsResponseStatus = a})
