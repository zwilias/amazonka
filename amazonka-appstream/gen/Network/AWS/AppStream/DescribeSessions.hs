...
dsAuthenticationType =
  lens _dsAuthenticationType (\s a -> s {_dsAuthenticationType = a})
...
instance AWSRequest DescribeSessions where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeSessionsResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Sessions" .!@ mempty) <*>
         (pure (fromEnum s)))
...
dssrsSessions =
  lens _dssrsSessions (\s a -> s {_dssrsSessions = a}) . _Default . _Coerce
