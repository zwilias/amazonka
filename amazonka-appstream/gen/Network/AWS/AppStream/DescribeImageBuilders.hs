...
instance AWSRequest DescribeImageBuilders where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeImageBuildersResponse' <$> (x .?> "ImageBuilders" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
dibsrsImageBuilders =
  lens _dibsrsImageBuilders (\s a -> s {_dibsrsImageBuilders = a}) .
  _Default . _Coerce
...
dibsrsResponseStatus =
  lens _dibsrsResponseStatus (\s a -> s {_dibsrsResponseStatus = a})
