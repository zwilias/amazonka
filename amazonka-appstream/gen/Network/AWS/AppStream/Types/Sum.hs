...
instance FromText AuthenticationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuthenticationType from value: '" <>
        e <> "'. Accepted values: api, saml, userpool"
  ...
  parser =
    takeLowerText >>= \case
      ...
      "vpc_configuration_security_group_ids" ->
        pure VPCConfigurationSecurityGroupIds
      e ->
        fromTextError $
        "Failure parsing FleetAttribute from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      "domain_join_error_ds_machine_account_quota_exceeded" ->
        pure DomainJoinErrorDsMachineAccountQuotaExceeded
      "domain_join_error_file_not_found" -> pure DomainJoinErrorFileNotFound
      "domain_join_error_invalid_parameter" ->
        pure DomainJoinErrorInvalidParameter
      ...
      "domain_join_internal_service_error" ->
        pure DomainJoinInternalServiceError
      "domain_join_nerr_invalid_workgroup_name" ->
        pure DomainJoinNerrInvalidWorkgroupName
      "domain_join_nerr_password_expired" -> pure DomainJoinNerrPasswordExpired
      "domain_join_nerr_workstation_not_started" ->
        pure DomainJoinNerrWorkstationNotStarted
      "iam_service_role_is_missing" -> pure IAMServiceRoleIsMissing
      "iam_service_role_missing_describe_security_groups_action" ->
        pure IAMServiceRoleMissingDescribeSecurityGroupsAction
      "iam_service_role_missing_describe_subnet_action" ->
        pure IAMServiceRoleMissingDescribeSubnetAction
      "iam_service_role_missing_eni_create_action" ->
        pure IAMServiceRoleMissingEniCreateAction
      "iam_service_role_missing_eni_delete_action" ->
        pure IAMServiceRoleMissingEniDeleteAction
      "iam_service_role_missing_eni_describe_action" ->
        pure IAMServiceRoleMissingEniDescribeAction
      ...
      "subnet_has_insufficient_ip_addresses" ->
        pure SubnetHasInsufficientIPAddresses
      "subnet_not_found" -> pure SubnetNotFound
      e ->
        fromTextError $
        "Failure parsing FleetErrorCode from value: '" <>
        e <>
...
instance FromText FleetState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FleetState from value: '" <>
        e <> "'. Accepted values: running, starting, stopped, stopping"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: always_on, on_demand"
...
instance FromText ImageBuilderState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ImageBuilderState from value: '" <>
        e <>
...
instance FromText ImageBuilderStateChangeReasonCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ImageBuilderStateChangeReasonCode from value: '" <>
        e <> "'. Accepted values: image_unavailable, internal_error"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: available, copying, deleting, failed, pending"
...
instance FromText ImageStateChangeReasonCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ImageStateChangeReasonCode from value: '" <>
        e <>
...
instance FromText SessionState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SessionState from value: '" <>
        e <> "'. Accepted values: active, expired, pending"
...
instance FromText StackAttribute where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StackAttribute from value: '" <>
        e <>
...
instance FromText StackErrorCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StackErrorCode from value: '" <>
        e <>
...
instance FromText StorageConnectorType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StorageConnectorType from value: '" <>
        e <> "'. Accepted values: homefolders"
...
instance FromText VisibilityType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VisibilityType from value: '" <>
