...
instance AWSRequest StopImageBuilder where
  ...
  response =
    receiveJSON
      (\s h x ->
         StopImageBuilderResponse' <$> (x .?> "ImageBuilder") <*>
         (pure (fromEnum s)))
...
instance ToJSON StopImageBuilder where
  toJSON StopImageBuilder' {..} = object (catMaybes [Just ("Name" .= _stoName)])
...
sibrsResponseStatus =
  lens _sibrsResponseStatus (\s a -> s {_sibrsResponseStatus = a})
