...
liRangesOnIndexedValues =
  lens _liRangesOnIndexedValues (\s a -> s {_liRangesOnIndexedValues = a}) .
  _Default . _Coerce

-- | The consistency level to execute the request at.
liConsistencyLevel :: Lens' ListIndex (Maybe ConsistencyLevel)
liConsistencyLevel =
  lens _liConsistencyLevel (\s a -> s {_liConsistencyLevel = a})
...
instance AWSRequest ListIndex where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListIndexResponse' <$> (x .?> "IndexAttachments" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
lirsIndexAttachments =
  lens _lirsIndexAttachments (\s a -> s {_lirsIndexAttachments = a}) .
  _Default . _Coerce
