...
lfnMaxResults =
  lens _lfnMaxResults (\s a -> s {_lfnMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListFacetNames where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListFacetNamesResponse' <$> (x .?> "NextToken") <*>
         (x .?> "FacetNames" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lfnrsFacetNames =
  lens _lfnrsFacetNames (\s a -> s {_lfnrsFacetNames = a}) . _Default . _Coerce

-- | -- | The response status code.
lfnrsResponseStatus :: Lens' ListFacetNamesResponse Int
lfnrsResponseStatus =
  lens _lfnrsResponseStatus (\s a -> s {_lfnrsResponseStatus = a})
