...
lasaMaxResults =
  lens _lasaMaxResults (\s a -> s {_lasaMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListAppliedSchemaARNs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAppliedSchemaARNsResponse' <$> (x .?> "SchemaArns" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
lasarsSchemaARNs =
  lens _lasarsSchemaARNs (\s a -> s {_lasarsSchemaARNs = a}) .
  _Default . _Coerce
...
lasarsResponseStatus =
  lens _lasarsResponseStatus (\s a -> s {_lasarsResponseStatus = a})
