...
lopConsistencyLevel =
  lens _lopConsistencyLevel (\s a -> s {_lopConsistencyLevel = a})
...
lopMaxResults =
  lens _lopMaxResults (\s a -> s {_lopMaxResults = a}) . mapping _Nat
...
lopObjectReference =
  lens _lopObjectReference (\s a -> s {_lopObjectReference = a})
...
instance AWSRequest ListObjectParents where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListObjectParentsResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Parents" .!@ mempty) <*>
         (pure (fromEnum s)))
