...
atiTargetReference =
  lens _atiTargetReference (\s a -> s {_atiTargetReference = a})
...
instance AWSRequest AttachToIndex where
  ...
  response =
    receiveJSON
      (\s h x ->
         AttachToIndexResponse' <$> (x .?> "AttachedObjectIdentifier") <*>
         (pure (fromEnum s)))
...
atirsAttachedObjectIdentifier =
  lens
    ...
    (\s a -> s {_atirsAttachedObjectIdentifier = a})

-- | -- | The response status code.
atirsResponseStatus :: Lens' AttachToIndexResponse Int
atirsResponseStatus =
  lens _atirsResponseStatus (\s a -> s {_atirsResponseStatus = a})
