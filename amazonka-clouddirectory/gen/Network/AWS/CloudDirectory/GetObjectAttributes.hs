...
goaConsistencyLevel =
  lens _goaConsistencyLevel (\s a -> s {_goaConsistencyLevel = a})
...
goaObjectReference =
  lens _goaObjectReference (\s a -> s {_goaObjectReference = a})
...
goaAttributeNames =
  lens _goaAttributeNames (\s a -> s {_goaAttributeNames = a}) . _Coerce
...
instance AWSRequest GetObjectAttributes where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetObjectAttributesResponse' <$> (x .?> "Attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToPath GetObjectAttributes where
  toPath = const "/amazonclouddirectory/2017-01-11/object/attributes/get"
...
goarsAttributes =
  lens _goarsAttributes (\s a -> s {_goarsAttributes = a}) . _Default . _Coerce
