...
lpaConsistencyLevel =
  lens _lpaConsistencyLevel (\s a -> s {_lpaConsistencyLevel = a})
...
lpaMaxResults =
  lens _lpaMaxResults (\s a -> s {_lpaMaxResults = a}) . mapping _Nat
...
lpaPolicyReference =
  lens _lpaPolicyReference (\s a -> s {_lpaPolicyReference = a})
...
lparsObjectIdentifiers =
  lens _lparsObjectIdentifiers (\s a -> s {_lparsObjectIdentifiers = a}) .
  _Default . _Coerce
