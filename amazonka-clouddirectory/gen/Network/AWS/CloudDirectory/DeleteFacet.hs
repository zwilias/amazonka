...
instance AWSRequest DeleteFacet where
  ...
  response =
    receiveEmpty (\s h x -> DeleteFacetResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteFacet where
  toHeaders DeleteFacet' {..} = mconcat ["x-amz-data-partition" =# _dfSchemaARN]

instance ToJSON DeleteFacet where
  toJSON DeleteFacet' {..} = object (catMaybes [Just ("Name" .= _dfName)])
