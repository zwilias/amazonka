...
lfaMaxResults =
  lens _lfaMaxResults (\s a -> s {_lfaMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListFacetAttributes where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListFacetAttributesResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lfarsAttributes =
  lens _lfarsAttributes (\s a -> s {_lfarsAttributes = a}) . _Default . _Coerce

-- | -- | The response status code.
lfarsResponseStatus :: Lens' ListFacetAttributesResponse Int
lfarsResponseStatus =
  lens _lfarsResponseStatus (\s a -> s {_lfarsResponseStatus = a})
