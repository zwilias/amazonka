...
atlSourceObjectReference =
  lens _atlSourceObjectReference (\s a -> s {_atlSourceObjectReference = a})

-- | Identifies the target object that the typed link will attach to.
atlTargetObjectReference :: Lens' AttachTypedLink ObjectReference
atlTargetObjectReference =
  lens _atlTargetObjectReference (\s a -> s {_atlTargetObjectReference = a})
...
instance AWSRequest AttachTypedLink where
  ...
  response =
    receiveJSON
      (\s h x ->
         AttachTypedLinkResponse' <$> (x .?> "TypedLinkSpecifier") <*>
         (pure (fromEnum s)))
...
atlrsTypedLinkSpecifier ::
     Lens' AttachTypedLinkResponse (Maybe TypedLinkSpecifier)
atlrsTypedLinkSpecifier =
  lens _atlrsTypedLinkSpecifier (\s a -> s {_atlrsTypedLinkSpecifier = a})

-- | -- | The response status code.
atlrsResponseStatus :: Lens' AttachTypedLinkResponse Int
atlrsResponseStatus =
  lens _atlrsResponseStatus (\s a -> s {_atlrsResponseStatus = a})
