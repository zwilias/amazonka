...
ltlfaMaxResults =
  lens _ltlfaMaxResults (\s a -> s {_ltlfaMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListTypedLinkFacetAttributes where
  type Rs ListTypedLinkFacetAttributes = ListTypedLinkFacetAttributesResponse
  request = postJSON cloudDirectory
  response =
    receiveJSON
      (\s h x ->
         ListTypedLinkFacetAttributesResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToPath ListTypedLinkFacetAttributes where
  toPath = const "/amazonclouddirectory/2017-01-11/typedlink/facet/attributes"
...
ltlfarsAttributes ::
     Lens' ListTypedLinkFacetAttributesResponse [TypedLinkAttributeDefinition]
ltlfarsAttributes =
  lens _ltlfarsAttributes (\s a -> s {_ltlfarsAttributes = a}) .
  _Default . _Coerce

-- | -- | The response status code.
ltlfarsResponseStatus :: Lens' ListTypedLinkFacetAttributesResponse Int
ltlfarsResponseStatus =
  lens _ltlfarsResponseStatus (\s a -> s {_ltlfarsResponseStatus = a})
