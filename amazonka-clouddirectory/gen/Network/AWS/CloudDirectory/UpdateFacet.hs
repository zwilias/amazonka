...
ufAttributeUpdates =
  lens _ufAttributeUpdates (\s a -> s {_ufAttributeUpdates = a}) .
  _Default . _Coerce
...
instance AWSRequest UpdateFacet where
  ...
  response =
    receiveEmpty (\s h x -> UpdateFacetResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateFacet where
  toHeaders UpdateFacet' {..} = mconcat ["x-amz-data-partition" =# _ufSchemaARN]
