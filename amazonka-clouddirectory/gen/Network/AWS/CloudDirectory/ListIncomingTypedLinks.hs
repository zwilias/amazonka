...
litlFilterAttributeRanges ::
     Lens' ListIncomingTypedLinks [TypedLinkAttributeRange]
litlFilterAttributeRanges =
  lens _litlFilterAttributeRanges (\s a -> s {_litlFilterAttributeRanges = a}) .
  _Default . _Coerce

-- | The consistency level to execute the request at.
litlConsistencyLevel :: Lens' ListIncomingTypedLinks (Maybe ConsistencyLevel)
litlConsistencyLevel =
  lens _litlConsistencyLevel (\s a -> s {_litlConsistencyLevel = a})
...
litlFilterTypedLink ::
     Lens' ListIncomingTypedLinks (Maybe TypedLinkSchemaAndFacetName)
litlFilterTypedLink =
  lens _litlFilterTypedLink (\s a -> s {_litlFilterTypedLink = a})

-- | The maximum number of results to retrieve.
litlMaxResults :: Lens' ListIncomingTypedLinks (Maybe Natural)
litlMaxResults =
  lens _litlMaxResults (\s a -> s {_litlMaxResults = a}) . mapping _Nat
...
litlObjectReference =
  lens _litlObjectReference (\s a -> s {_litlObjectReference = a})
...
instance AWSRequest ListIncomingTypedLinks where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListIncomingTypedLinksResponse' <$> (x .?> "LinkSpecifiers" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToPath ListIncomingTypedLinks where
  toPath = const "/amazonclouddirectory/2017-01-11/typedlink/incoming"
...
litlrsLinkSpecifiers ::
     Lens' ListIncomingTypedLinksResponse [TypedLinkSpecifier]
litlrsLinkSpecifiers =
  lens _litlrsLinkSpecifiers (\s a -> s {_litlrsLinkSpecifiers = a}) .
  _Default . _Coerce
...
litlrsResponseStatus =
  lens _litlrsResponseStatus (\s a -> s {_litlrsResponseStatus = a})
