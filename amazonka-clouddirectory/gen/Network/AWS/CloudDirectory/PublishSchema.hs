...
psDevelopmentSchemaARN =
  lens _psDevelopmentSchemaARN (\s a -> s {_psDevelopmentSchemaARN = a})
...
instance AWSRequest PublishSchema where
  ...
  response =
    receiveJSON
      (\s h x ->
         PublishSchemaResponse' <$> (x .?> "PublishedSchemaArn") <*>
         (pure (fromEnum s)))
...
psrsPublishedSchemaARN =
  lens _psrsPublishedSchemaARN (\s a -> s {_psrsPublishedSchemaARN = a})

-- | -- | The response status code.
psrsResponseStatus :: Lens' PublishSchemaResponse Int
psrsResponseStatus =
  lens _psrsResponseStatus (\s a -> s {_psrsResponseStatus = a})
