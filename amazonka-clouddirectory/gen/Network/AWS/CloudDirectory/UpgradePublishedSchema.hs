...
upsDevelopmentSchemaARN =
  lens _upsDevelopmentSchemaARN (\s a -> s {_upsDevelopmentSchemaARN = a})

-- | The ARN of the published schema to be upgraded.
upsPublishedSchemaARN :: Lens' UpgradePublishedSchema Text
upsPublishedSchemaARN =
  lens _upsPublishedSchemaARN (\s a -> s {_upsPublishedSchemaARN = a})
...
instance AWSRequest UpgradePublishedSchema where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpgradePublishedSchemaResponse' <$> (x .?> "UpgradedSchemaArn") <*>
         (pure (fromEnum s)))
...
instance ToPath UpgradePublishedSchema where
  toPath = const "/amazonclouddirectory/2017-01-11/schema/upgradepublished"
...
upsrsUpgradedSchemaARN =
  lens _upsrsUpgradedSchemaARN (\s a -> s {_upsrsUpgradedSchemaARN = a})

-- | -- | The response status code.
upsrsResponseStatus :: Lens' UpgradePublishedSchemaResponse Int
upsrsResponseStatus =
  lens _upsrsResponseStatus (\s a -> s {_upsrsResponseStatus = a})
