...
loaConsistencyLevel =
  lens _loaConsistencyLevel (\s a -> s {_loaConsistencyLevel = a})
...
loaMaxResults =
  lens _loaMaxResults (\s a -> s {_loaMaxResults = a}) . mapping _Nat
...
loaObjectReference =
  lens _loaObjectReference (\s a -> s {_loaObjectReference = a})
...
instance AWSRequest ListObjectAttributes where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListObjectAttributesResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Attributes" .!@ mempty) <*>
         (pure (fromEnum s)))
...
loarsAttributes =
  lens _loarsAttributes (\s a -> s {_loarsAttributes = a}) . _Default . _Coerce

-- | -- | The response status code.
loarsResponseStatus :: Lens' ListObjectAttributesResponse Int
loarsResponseStatus =
  lens _loarsResponseStatus (\s a -> s {_loarsResponseStatus = a})
