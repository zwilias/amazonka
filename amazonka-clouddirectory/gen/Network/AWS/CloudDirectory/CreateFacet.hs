...
cfAttributes =
  lens _cfAttributes (\s a -> s {_cfAttributes = a}) . _Default . _Coerce
...
instance AWSRequest CreateFacet where
  ...
  response =
    receiveEmpty (\s h x -> CreateFacetResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders CreateFacet where
  toHeaders CreateFacet' {..} = mconcat ["x-amz-data-partition" =# _cfSchemaARN]
