...
ldsaMaxResults =
  lens _ldsaMaxResults (\s a -> s {_ldsaMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListDevelopmentSchemaARNs where
  type Rs ListDevelopmentSchemaARNs = ListDevelopmentSchemaARNsResponse
  request = postJSON cloudDirectory
  response =
    receiveJSON
      (\s h x ->
         ListDevelopmentSchemaARNsResponse' <$> (x .?> "SchemaArns" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToPath ListDevelopmentSchemaARNs where
  toPath = const "/amazonclouddirectory/2017-01-11/schema/development"
...
ldsarsSchemaARNs =
  lens _ldsarsSchemaARNs (\s a -> s {_ldsarsSchemaARNs = a}) .
  _Default . _Coerce
...
ldsarsResponseStatus =
  lens _ldsarsResponseStatus (\s a -> s {_ldsarsResponseStatus = a})
