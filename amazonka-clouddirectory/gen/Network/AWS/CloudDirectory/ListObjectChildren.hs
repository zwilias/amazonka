...
locConsistencyLevel =
  lens _locConsistencyLevel (\s a -> s {_locConsistencyLevel = a})
...
locMaxResults =
  lens _locMaxResults (\s a -> s {_locMaxResults = a}) . mapping _Nat
...
locObjectReference =
  lens _locObjectReference (\s a -> s {_locObjectReference = a})
...
instance AWSRequest ListObjectChildren where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListObjectChildrenResponse' <$> (x .?> "Children" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
locrsChildren =
  lens _locrsChildren (\s a -> s {_locrsChildren = a}) . _Default . _Map
...
locrsResponseStatus =
  lens _locrsResponseStatus (\s a -> s {_locrsResponseStatus = a})
