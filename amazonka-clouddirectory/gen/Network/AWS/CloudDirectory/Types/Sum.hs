...
instance FromText BatchReadExceptionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BatchReadExceptionType from value: '" <>
        e <>
...
instance FromText ConsistencyLevel where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConsistencyLevel from value: '" <>
        e <> "'. Accepted values: eventual, serializable"
...
instance FromText DirectoryState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DirectoryState from value: '" <>
...
instance FromText FacetAttributeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FacetAttributeType from value: '" <>
        e <> "'. Accepted values: binary, boolean, datetime, number, string"
...
instance FromText ObjectType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ObjectType from value: '" <>
        e <> "'. Accepted values: index, leaf_node, node, policy"
...
instance FromText RangeMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RangeMode from value: '" <>
        e <>
...
instance FromText RequiredAttributeBehavior where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RequiredAttributeBehavior from value: '" <>
        e <> "'. Accepted values: not_required, required_always"
...
instance FromText RuleType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RuleType from value: '" <>
        e <>
...
instance FromText UpdateActionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UpdateActionType from value: '" <>
        e <> "'. Accepted values: create_or_update, delete"
