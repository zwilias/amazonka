...
instance AWSRequest GetAppliedSchemaVersion where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetAppliedSchemaVersionResponse' <$> (x .?> "AppliedSchemaArn") <*>
         (pure (fromEnum s)))
...
instance ToPath GetAppliedSchemaVersion where
  toPath = const "/amazonclouddirectory/2017-01-11/schema/getappliedschema"
...
gasvrsAppliedSchemaARN =
  lens _gasvrsAppliedSchemaARN (\s a -> s {_gasvrsAppliedSchemaARN = a})

-- | -- | The response status code.
gasvrsResponseStatus :: Lens' GetAppliedSchemaVersionResponse Int
gasvrsResponseStatus =
  lens _gasvrsResponseStatus (\s a -> s {_gasvrsResponseStatus = a})
