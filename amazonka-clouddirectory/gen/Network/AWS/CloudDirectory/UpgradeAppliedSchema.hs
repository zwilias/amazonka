...
uasPublishedSchemaARN =
  lens _uasPublishedSchemaARN (\s a -> s {_uasPublishedSchemaARN = a})
...
instance AWSRequest UpgradeAppliedSchema where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpgradeAppliedSchemaResponse' <$> (x .?> "DirectoryArn") <*>
         (x .?> "UpgradedSchemaArn") <*>
         (pure (fromEnum s)))
...
instance ToPath UpgradeAppliedSchema where
  toPath = const "/amazonclouddirectory/2017-01-11/schema/upgradeapplied"
...
uasrsUpgradedSchemaARN =
  lens _uasrsUpgradedSchemaARN (\s a -> s {_uasrsUpgradedSchemaARN = a})
