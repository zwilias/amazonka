...
instance FromText CompressionFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CompressionFormat from value: '" <>
        e <> "'. Accepted values: gzip, snappy, uncompressed, zip"
...
instance FromText DeliveryStreamStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeliveryStreamStatus from value: '" <>
        e <> "'. Accepted values: active, creating, deleting"
...
instance FromText DeliveryStreamType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeliveryStreamType from value: '" <>
        e <> "'. Accepted values: directput, kinesisstreamassource"
...
instance FromText ElasticsearchIndexRotationPeriod where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ElasticsearchIndexRotationPeriod from value: '" <>
        e <>
...
instance FromText ElasticsearchS3BackupMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ElasticsearchS3BackupMode from value: '" <>
        e <> "'. Accepted values: alldocuments, faileddocumentsonly"
...
instance FromText HECEndpointType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HECEndpointType from value: '" <>
        e <> "'. Accepted values: event, raw"
...
instance FromText NoEncryptionConfig where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NoEncryptionConfig from value: '" <>
        e <> "'. Accepted values: noencryption"
...
instance FromText OrcCompression where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrcCompression from value: '" <>
        e <> "'. Accepted values: none, snappy, zlib"
...
instance FromText OrcFormatVersion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrcFormatVersion from value: '" <>
        e <> "'. Accepted values: v0_11, v0_12"
...
instance FromText ParquetCompression where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ParquetCompression from value: '" <>
        e <> "'. Accepted values: gzip, snappy, uncompressed"
...
instance FromText ParquetWriterVersion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ParquetWriterVersion from value: '" <>
        e <> "'. Accepted values: v1, v2"
...
instance FromText ProcessorParameterName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProcessorParameterName from value: '" <>
        e <>
...
instance FromText ProcessorType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ProcessorType from value: '" <>
        e <> "'. Accepted values: lambda"
...
instance FromText RedshiftS3BackupMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RedshiftS3BackupMode from value: '" <>
...
instance FromText S3BackupMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing S3BackupMode from value: '" <>
        e <> "'. Accepted values: disabled, enabled"
...
instance FromText SplunkS3BackupMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SplunkS3BackupMode from value: '" <>
        e <> "'. Accepted values: allevents, failedeventsonly"
