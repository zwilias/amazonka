...
ldsDeliveryStreamType =
  lens _ldsDeliveryStreamType (\s a -> s {_ldsDeliveryStreamType = a})
...
ldsExclusiveStartDeliveryStreamName =
  lens
    ...
    (\s a -> s {_ldsExclusiveStartDeliveryStreamName = a})
...
instance AWSRequest ListDeliveryStreams where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDeliveryStreamsResponse' <$> (pure (fromEnum s)) <*>
...
ldsrsResponseStatus =
  lens _ldsrsResponseStatus (\s a -> s {_ldsrsResponseStatus = a})

-- | The names of the delivery streams.
ldsrsDeliveryStreamNames :: Lens' ListDeliveryStreamsResponse [Text]
ldsrsDeliveryStreamNames =
  lens _ldsrsDeliveryStreamNames (\s a -> s {_ldsrsDeliveryStreamNames = a}) .
  _Coerce
...
ldsrsHasMoreDeliveryStreams =
  lens
    ...
    (\s a -> s {_ldsrsHasMoreDeliveryStreams = a})
