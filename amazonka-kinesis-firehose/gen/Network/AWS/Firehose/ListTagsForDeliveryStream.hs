...
ltfdsExclusiveStartTagKey =
  lens _ltfdsExclusiveStartTagKey (\s a -> s {_ltfdsExclusiveStartTagKey = a})

-- | The name of the delivery stream whose tags you want to list.
ltfdsDeliveryStreamName :: Lens' ListTagsForDeliveryStream Text
ltfdsDeliveryStreamName =
  lens _ltfdsDeliveryStreamName (\s a -> s {_ltfdsDeliveryStreamName = a})

instance AWSRequest ListTagsForDeliveryStream where
  type Rs ListTagsForDeliveryStream = ListTagsForDeliveryStreamResponse
  request = postJSON firehose
  response =
    receiveJSON
      (\s h x ->
         ListTagsForDeliveryStreamResponse' <$> (pure (fromEnum s)) <*>
         (x .?> "Tags" .!@ mempty) <*>
         (x .:> "HasMoreTags"))
...
ltfdsrsResponseStatus =
  lens _ltfdsrsResponseStatus (\s a -> s {_ltfdsrsResponseStatus = a})
...
ltfdsrsHasMoreTags =
  lens _ltfdsrsHasMoreTags (\s a -> s {_ltfdsrsHasMoreTags = a})
