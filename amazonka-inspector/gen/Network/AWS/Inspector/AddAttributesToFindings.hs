...
aatfFindingARNs =
  lens _aatfFindingARNs (\s a -> s {_aatfFindingARNs = a}) . _List1

-- | The array of attributes that you want to assign to specified findings.
aatfAttributes :: Lens' AddAttributesToFindings [Attribute]
aatfAttributes =
  lens _aatfAttributes (\s a -> s {_aatfAttributes = a}) . _Coerce
...
instance AWSRequest AddAttributesToFindings where
  ...
  response =
    receiveJSON
      (\s h x ->
         AddAttributesToFindingsResponse' <$> (pure (fromEnum s)) <*>
...
aatfrsResponseStatus =
  lens _aatfrsResponseStatus (\s a -> s {_aatfrsResponseStatus = a})

-- | Attribute details that cannot be described. An error code is provided for each failed item.
aatfrsFailedItems ::
     Lens' AddAttributesToFindingsResponse (HashMap Text FailedItemDetails)
aatfrsFailedItems =
  lens _aatfrsFailedItems (\s a -> s {_aatfrsFailedItems = a}) . _Map
