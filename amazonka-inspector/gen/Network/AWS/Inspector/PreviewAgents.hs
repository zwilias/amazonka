...
paPreviewAgentsARN =
  lens _paPreviewAgentsARN (\s a -> s {_paPreviewAgentsARN = a})
...
instance AWSRequest PreviewAgents where
  ...
  response =
    receiveJSON
      (\s h x ->
         PreviewAgentsResponse' <$> (x .?> "nextToken") <*> (pure (fromEnum s)) <*>
...
instance ToHeaders PreviewAgents where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("InspectorService.PreviewAgents" :: ByteString)
...
parsResponseStatus =
  lens _parsResponseStatus (\s a -> s {_parsResponseStatus = a})

-- | The resulting list of agents.
parsAgentPreviews :: Lens' PreviewAgentsResponse [AgentPreview]
parsAgentPreviews =
  lens _parsAgentPreviews (\s a -> s {_parsAgentPreviews = a}) . _Coerce
