...
instance FromText AgentHealth where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AgentHealth from value: '" <>
...
instance FromText AgentHealthCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AgentHealthCode from value: '" <>
        e <>
...
instance FromText AssessmentRunNotificationSNSStatusCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AssessmentRunNotificationSNSStatusCode from value: '" <>
        e <>
...
instance FromText AssessmentRunState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AssessmentRunState from value: '" <>
        e <>
...
instance FromText AssetType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AssetType from value: '" <>
        e <> "'. Accepted values: ec2-instance"
...
instance FromText FailedItemErrorCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FailedItemErrorCode from value: '" <>
        e <>
...
instance FromText InspectorEvent where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing InspectorEvent from value: '" <>
        e <>
...
instance FromText ReportFileFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReportFileFormat from value: '" <>
        e <> "'. Accepted values: html, pdf"
...
instance FromText ReportStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReportStatus from value: '" <>
        e <> "'. Accepted values: completed, failed, work_in_progress"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: finding, full"
...
instance FromText Severity where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Severity from value: '" <>
        e <> "'. Accepted values: high, informational, low, medium, undefined"
...
instance FromText StopAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StopAction from value: '" <>
        e <> "'. Accepted values: skip_evaluation, start_evaluation"
