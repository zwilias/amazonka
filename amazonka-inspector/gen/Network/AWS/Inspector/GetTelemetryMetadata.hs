...
gtmAssessmentRunARN =
  lens _gtmAssessmentRunARN (\s a -> s {_gtmAssessmentRunARN = a})
...
instance AWSRequest GetTelemetryMetadata where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetTelemetryMetadataResponse' <$> (pure (fromEnum s)) <*>
...
instance ToJSON GetTelemetryMetadata where
  toJSON GetTelemetryMetadata' {..} =
    object (catMaybes [Just ("assessmentRunArn" .= _gtmAssessmentRunARN)])
...
gtmrsResponseStatus =
  lens _gtmrsResponseStatus (\s a -> s {_gtmrsResponseStatus = a})

-- | Telemetry details.
gtmrsTelemetryMetadata :: Lens' GetTelemetryMetadataResponse [TelemetryMetadata]
gtmrsTelemetryMetadata =
  lens _gtmrsTelemetryMetadata (\s a -> s {_gtmrsTelemetryMetadata = a}) .
  _Coerce
