...
crgResourceGroupTags =
  lens _crgResourceGroupTags (\s a -> s {_crgResourceGroupTags = a}) . _List1
...
instance AWSRequest CreateResourceGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateResourceGroupResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "resourceGroupArn"))
...
instance ToJSON CreateResourceGroup where
  toJSON CreateResourceGroup' {..} =
    object (catMaybes [Just ("resourceGroupTags" .= _crgResourceGroupTags)])
...
crgrsResourceGroupARN =
  lens _crgrsResourceGroupARN (\s a -> s {_crgrsResourceGroupARN = a})
