...
darAssessmentRunARNs =
  lens _darAssessmentRunARNs (\s a -> s {_darAssessmentRunARNs = a}) . _List1
...
instance AWSRequest DescribeAssessmentRuns where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeAssessmentRunsResponse' <$> (pure (fromEnum s)) <*>
...
instance ToJSON DescribeAssessmentRuns where
  toJSON DescribeAssessmentRuns' {..} =
    object (catMaybes [Just ("assessmentRunArns" .= _darAssessmentRunARNs)])
...
darrsResponseStatus =
  lens _darrsResponseStatus (\s a -> s {_darrsResponseStatus = a})

-- | Information about the assessment run.
darrsAssessmentRuns :: Lens' DescribeAssessmentRunsResponse [AssessmentRun]
darrsAssessmentRuns =
  lens _darrsAssessmentRuns (\s a -> s {_darrsAssessmentRuns = a}) . _Coerce

-- | Assessment run details that cannot be described. An error code is provided for each failed item.
darrsFailedItems ::
     Lens' DescribeAssessmentRunsResponse (HashMap Text FailedItemDetails)
darrsFailedItems =
