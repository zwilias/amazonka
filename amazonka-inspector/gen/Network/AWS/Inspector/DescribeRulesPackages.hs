...
drpRulesPackageARNs =
  lens _drpRulesPackageARNs (\s a -> s {_drpRulesPackageARNs = a}) . _List1
...
instance AWSRequest DescribeRulesPackages where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeRulesPackagesResponse' <$> (pure (fromEnum s)) <*>
...
drprsResponseStatus =
  lens _drprsResponseStatus (\s a -> s {_drprsResponseStatus = a})

-- | Information about the rules package.
drprsRulesPackages :: Lens' DescribeRulesPackagesResponse [RulesPackage]
drprsRulesPackages =
  lens _drprsRulesPackages (\s a -> s {_drprsRulesPackages = a}) . _Coerce

-- | Rules package details that cannot be described. An error code is provided for each failed item.
drprsFailedItems ::
     Lens' DescribeRulesPackagesResponse (HashMap Text FailedItemDetails)
drprsFailedItems =
  lens _drprsFailedItems (\s a -> s {_drprsFailedItems = a}) . _Map
