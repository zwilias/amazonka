...
lfAssessmentRunARNs =
  lens _lfAssessmentRunARNs (\s a -> s {_lfAssessmentRunARNs = a}) .
  _Default . _Coerce
...
instance AWSRequest ListFindings where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListFindingsResponse' <$> (x .?> "nextToken") <*> (pure (fromEnum s)) <*>
...
lfrsFindingARNs =
  lens _lfrsFindingARNs (\s a -> s {_lfrsFindingARNs = a}) . _Coerce
