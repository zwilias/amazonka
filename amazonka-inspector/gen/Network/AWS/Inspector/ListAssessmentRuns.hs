...
larAssessmentTemplateARNs =
  lens _larAssessmentTemplateARNs (\s a -> s {_larAssessmentTemplateARNs = a}) .
  _Default . _Coerce
...
instance AWSRequest ListAssessmentRuns where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAssessmentRunsResponse' <$> (x .?> "nextToken") <*>
         (pure (fromEnum s)) <*>
...
larrsResponseStatus =
  lens _larrsResponseStatus (\s a -> s {_larrsResponseStatus = a})

-- | A list of ARNs that specifies the assessment runs that are returned by the action.
larrsAssessmentRunARNs :: Lens' ListAssessmentRunsResponse [Text]
larrsAssessmentRunARNs =
  lens _larrsAssessmentRunARNs (\s a -> s {_larrsAssessmentRunARNs = a}) .
  _Coerce
