...
instance AWSRequest ListRulesPackages where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListRulesPackagesResponse' <$> (x .?> "nextToken") <*>
         (pure (fromEnum s)) <*>
...
lrprsResponseStatus =
  lens _lrprsResponseStatus (\s a -> s {_lrprsResponseStatus = a})

-- | The list of ARNs that specifies the rules packages returned by the action.
lrprsRulesPackageARNs :: Lens' ListRulesPackagesResponse [Text]
lrprsRulesPackageARNs =
  lens _lrprsRulesPackageARNs (\s a -> s {_lrprsRulesPackageARNs = a}) . _Coerce
