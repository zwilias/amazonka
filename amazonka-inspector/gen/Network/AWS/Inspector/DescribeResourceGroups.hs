...
drgResourceGroupARNs =
  lens _drgResourceGroupARNs (\s a -> s {_drgResourceGroupARNs = a}) . _List1
...
instance AWSRequest DescribeResourceGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeResourceGroupsResponse' <$> (pure (fromEnum s)) <*>
...
instance ToJSON DescribeResourceGroups where
  toJSON DescribeResourceGroups' {..} =
    object (catMaybes [Just ("resourceGroupArns" .= _drgResourceGroupARNs)])
...
drgrsResponseStatus =
  lens _drgrsResponseStatus (\s a -> s {_drgrsResponseStatus = a})

-- | Information about a resource group.
drgrsResourceGroups :: Lens' DescribeResourceGroupsResponse [ResourceGroup]
drgrsResourceGroups =
  lens _drgrsResourceGroups (\s a -> s {_drgrsResourceGroups = a}) . _Coerce

-- | Resource group details that cannot be described. An error code is provided for each failed item.
drgrsFailedItems ::
     Lens' DescribeResourceGroupsResponse (HashMap Text FailedItemDetails)
drgrsFailedItems =
  lens _drgrsFailedItems (\s a -> s {_drgrsFailedItems = a}) . _Map
