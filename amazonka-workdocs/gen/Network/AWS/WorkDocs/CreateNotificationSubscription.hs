...
cnsSubscriptionType =
  lens _cnsSubscriptionType (\s a -> s {_cnsSubscriptionType = a})

instance AWSRequest CreateNotificationSubscription where
  type Rs CreateNotificationSubscription = CreateNotificationSubscriptionResponse
  request = postJSON workDocs
  response =
    receiveJSON
      (\s h x ->
         CreateNotificationSubscriptionResponse' <$> (x .?> "Subscription") <*>
         (pure (fromEnum s)))
...
instance ToPath CreateNotificationSubscription where
  toPath CreateNotificationSubscription' {..} =
    mconcat
      ["/api/v1/organizations/", toBS _cnsOrganizationId, "/subscriptions"]
...
-- | The subscription.
cnsrsSubscription ::
     Lens' CreateNotificationSubscriptionResponse (Maybe Subscription)
...
cnsrsResponseStatus =
  lens _cnsrsResponseStatus (\s a -> s {_cnsrsResponseStatus = a})
