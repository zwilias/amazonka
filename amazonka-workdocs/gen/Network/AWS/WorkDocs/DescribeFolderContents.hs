...
dfcsAuthenticationToken =
  lens _dfcsAuthenticationToken (\s a -> s {_dfcsAuthenticationToken = a}) .
  mapping _Sensitive
...
instance AWSRequest DescribeFolderContents where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeFolderContentsResponse' <$> (x .?> "Folders" .!@ mempty) <*>
         (x .?> "Documents" .!@ mempty) <*>
         (x .?> "Marker") <*>
         (pure (fromEnum s)))
...
dfcrsFolders =
  lens _dfcrsFolders (\s a -> s {_dfcrsFolders = a}) . _Default . _Coerce

-- | The documents in the specified folder.
dfcrsDocuments :: Lens' DescribeFolderContentsResponse [DocumentMetadata]
dfcrsDocuments =
  lens _dfcrsDocuments (\s a -> s {_dfcrsDocuments = a}) . _Default . _Coerce
