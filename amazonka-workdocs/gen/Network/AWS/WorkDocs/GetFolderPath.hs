...
gfpAuthenticationToken =
  lens _gfpAuthenticationToken (\s a -> s {_gfpAuthenticationToken = a}) .
  mapping _Sensitive
...
instance ToQuery GetFolderPath where
  toQuery GetFolderPath' {..} =
    mconcat
      ["marker" =: _gfpMarker, "limit" =: _gfpLimit, "fields" =: _gfpFields]
...
gfprsResponseStatus =
  lens _gfprsResponseStatus (\s a -> s {_gfprsResponseStatus = a})
