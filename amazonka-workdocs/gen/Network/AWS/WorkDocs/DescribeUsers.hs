...
duAuthenticationToken =
  lens _duAuthenticationToken (\s a -> s {_duAuthenticationToken = a}) .
  mapping _Sensitive
...
instance AWSRequest DescribeUsers where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeUsersResponse' <$> (x .?> "Users" .!@ mempty) <*>
...
dursTotalNumberOfUsers =
  lens _dursTotalNumberOfUsers (\s a -> s {_dursTotalNumberOfUsers = a})
