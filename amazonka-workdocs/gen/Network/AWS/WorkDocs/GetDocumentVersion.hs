...
gdvAuthenticationToken =
  lens _gdvAuthenticationToken (\s a -> s {_gdvAuthenticationToken = a}) .
  mapping _Sensitive

-- | Set this to TRUE to include custom metadata in the response.
gdvIncludeCustomMetadata :: Lens' GetDocumentVersion (Maybe Bool)
gdvIncludeCustomMetadata =
  lens _gdvIncludeCustomMetadata (\s a -> s {_gdvIncludeCustomMetadata = a})
...
instance AWSRequest GetDocumentVersion where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDocumentVersionResponse' <$> (x .?> "CustomMetadata" .!@ mempty) <*>
         (x .?> "Metadata") <*>
         (pure (fromEnum s)))
...
gdvrsCustomMetadata =
  lens _gdvrsCustomMetadata (\s a -> s {_gdvrsCustomMetadata = a}) .
  _Default . _Map

-- | The version metadata.
gdvrsMetadata ::
     Lens' GetDocumentVersionResponse (Maybe DocumentVersionMetadata)
gdvrsMetadata = lens _gdvrsMetadata (\s a -> s {_gdvrsMetadata = a})

-- | -- | The response status code.
gdvrsResponseStatus :: Lens' GetDocumentVersionResponse Int
gdvrsResponseStatus =
  lens _gdvrsResponseStatus (\s a -> s {_gdvrsResponseStatus = a})
