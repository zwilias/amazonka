...
cfAuthenticationToken =
  lens _cfAuthenticationToken (\s a -> s {_cfAuthenticationToken = a}) .
  mapping _Sensitive
...
instance AWSRequest CreateFolder where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateFolderResponse' <$> (x .?> "Metadata") <*> (pure (fromEnum s)))
...
instance ToJSON CreateFolder where
  toJSON CreateFolder' {..} =
    object
      (catMaybes
         [("Name" .=) <$> _cfName, Just ("ParentFolderId" .= _cfParentFolderId)])
