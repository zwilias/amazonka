...
rarpAuthenticationToken =
  lens _rarpAuthenticationToken (\s a -> s {_rarpAuthenticationToken = a}) .
  mapping _Sensitive
...
instance AWSRequest RemoveAllResourcePermissions where
  type Rs RemoveAllResourcePermissions = RemoveAllResourcePermissionsResponse
...
instance ToPath RemoveAllResourcePermissions where
  toPath RemoveAllResourcePermissions' {..} =
    mconcat ["/api/v1/resources/", toBS _rarpResourceId, "/permissions"]
