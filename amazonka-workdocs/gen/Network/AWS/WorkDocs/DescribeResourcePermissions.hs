...
drpAuthenticationToken =
  lens _drpAuthenticationToken (\s a -> s {_drpAuthenticationToken = a}) .
  mapping _Sensitive
...
instance AWSRequest DescribeResourcePermissions where
  type Rs DescribeResourcePermissions = DescribeResourcePermissionsResponse
...
instance ToPath DescribeResourcePermissions where
  toPath DescribeResourcePermissions' {..} =
    mconcat ["/api/v1/resources/", toBS _drpResourceId, "/permissions"]
...
drprsPrincipals =
  lens _drprsPrincipals (\s a -> s {_drprsPrincipals = a}) . _Default . _Coerce
