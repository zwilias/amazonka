...
ccmAuthenticationToken =
  lens _ccmAuthenticationToken (\s a -> s {_ccmAuthenticationToken = a}) .
  mapping _Sensitive
...
ccmCustomMetadata =
  lens _ccmCustomMetadata (\s a -> s {_ccmCustomMetadata = a}) . _Map
...
instance ToJSON CreateCustomMetadata where
  toJSON CreateCustomMetadata' {..} =
    object (catMaybes [Just ("CustomMetadata" .= _ccmCustomMetadata)])

instance ToPath CreateCustomMetadata where
  toPath CreateCustomMetadata' {..} =
    mconcat ["/api/v1/resources/", toBS _ccmResourceId, "/customMetadata"]

instance ToQuery CreateCustomMetadata where
  toQuery CreateCustomMetadata' {..} = mconcat ["versionid" =: _ccmVersionId]
...
ccmrsResponseStatus =
  lens _ccmrsResponseStatus (\s a -> s {_ccmrsResponseStatus = a})
