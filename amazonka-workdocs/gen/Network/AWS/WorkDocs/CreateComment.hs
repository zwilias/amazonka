...
ccNotifyCollaborators =
  lens _ccNotifyCollaborators (\s a -> s {_ccNotifyCollaborators = a})

-- | Amazon WorkDocs authentication token. Do not set this field when using administrative API actions, as in accessing the API using AWS credentials.
ccAuthenticationToken :: Lens' CreateComment (Maybe Text)
ccAuthenticationToken =
  lens _ccAuthenticationToken (\s a -> s {_ccAuthenticationToken = a}) .
  mapping _Sensitive
...
instance AWSRequest CreateComment where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateCommentResponse' <$> (x .?> "Comment") <*> (pure (fromEnum s)))
