...
instance FromXML AccessLog where
  parseXML x =
    AccessLog' <$> (x .@? "EmitInterval") <*> (x .@? "S3BucketPrefix") <*>
    (x .@? "S3BucketName") <*>
    (x .@ "Enabled")
...
instance FromXML AdditionalAttribute where
  parseXML x = AdditionalAttribute' <$> (x .@? "Value") <*> (x .@? "Key")
...
instance FromXML AppCookieStickinessPolicy where
  parseXML x =
    AppCookieStickinessPolicy' <$> (x .@? "PolicyName") <*> (x .@? "CookieName")
...
bsdPolicyNames =
  lens _bsdPolicyNames (\s a -> s {_bsdPolicyNames = a}) . _Default . _Coerce

-- | The port on which the EC2 instance is listening.
bsdInstancePort :: Lens' BackendServerDescription (Maybe Natural)
bsdInstancePort =
  lens _bsdInstancePort (\s a -> s {_bsdInstancePort = a}) . mapping _Nat
...
instance FromXML ConnectionDraining where
  parseXML x = ConnectionDraining' <$> (x .@? "Timeout") <*> (x .@ "Enabled")
...
instance ToQuery ConnectionSettings where
  toQuery ConnectionSettings' {..} = mconcat ["IdleTimeout" =: _csIdleTimeout]
...
instance ToQuery CrossZoneLoadBalancing where
  toQuery CrossZoneLoadBalancing' {..} = mconcat ["Enabled" =: _czlbEnabled]
...
hcUnhealthyThreshold =
  lens _hcUnhealthyThreshold (\s a -> s {_hcUnhealthyThreshold = a}) . _Nat

-- | The number of consecutive health checks successes required before moving the instance to the @Healthy@ state.
hcHealthyThreshold :: Lens' HealthCheck Natural
hcHealthyThreshold =
  lens _hcHealthyThreshold (\s a -> s {_hcHealthyThreshold = a}) . _Nat

instance FromXML HealthCheck where
  parseXML x =
    HealthCheck' <$> (x .@ "Target") <*> (x .@ "Interval") <*> (x .@ "Timeout") <*>
...
instance FromXML InstanceState where
  parseXML x =
    InstanceState' <$> (x .@? "InstanceId") <*> (x .@? "State") <*>
    (x .@? "ReasonCode") <*>
    (x .@? "Description")
...
instance FromXML LBCookieStickinessPolicy where
  parseXML x =
    LBCookieStickinessPolicy' <$> (x .@? "PolicyName") <*>
...
instance FromXML Listener where
  parseXML x =
    Listener' <$> (x .@? "InstanceProtocol") <*> (x .@? "SSLCertificateId") <*>
    (x .@ "Protocol") <*>
    (x .@ "LoadBalancerPort") <*>
    (x .@ "InstancePort")
...
ldPolicyNames =
  lens _ldPolicyNames (\s a -> s {_ldPolicyNames = a}) . _Default . _Coerce
...
lbaCrossZoneLoadBalancing ::
     Lens' LoadBalancerAttributes (Maybe CrossZoneLoadBalancing)
lbaCrossZoneLoadBalancing =
  lens _lbaCrossZoneLoadBalancing (\s a -> s {_lbaCrossZoneLoadBalancing = a})
...
instance FromXML LoadBalancerAttributes where
  parseXML x =
    LoadBalancerAttributes' <$> (x .@? "CrossZoneLoadBalancing") <*>
    (x .@? "AccessLog") <*>
    (x .@? "AdditionalAttributes" .!@ mempty >>= may (parseXMLList "member")) <*>
...
lbdSourceSecurityGroup ::
     Lens' LoadBalancerDescription (Maybe SourceSecurityGroup)
lbdSourceSecurityGroup =
...
lbdCanonicalHostedZoneName =
  lens _lbdCanonicalHostedZoneName (\s a -> s {_lbdCanonicalHostedZoneName = a})
...
lbdCreatedTime =
  lens _lbdCreatedTime (\s a -> s {_lbdCreatedTime = a}) . mapping _Time
...
lbdBackendServerDescriptions ::
     Lens' LoadBalancerDescription [BackendServerDescription]
lbdBackendServerDescriptions =
  lens
...
lbdInstances =
  lens _lbdInstances (\s a -> s {_lbdInstances = a}) . _Default . _Coerce
...
instance FromXML LoadBalancerDescription where
  parseXML x =
    LoadBalancerDescription' <$> (x .@? "SourceSecurityGroup") <*>
    ...
    (x .@? "Scheme") <*>
    (x .@? "ListenerDescriptions" .!@ mempty >>= may (parseXMLList "member")) <*>
    (x .@? "DNSName") <*>
    (x .@? "Policies")
...
pOtherPolicies =
  lens _pOtherPolicies (\s a -> s {_pOtherPolicies = a}) . _Default . _Coerce
...
instance FromXML PolicyAttributeDescription where
  parseXML x =
    PolicyAttributeDescription' <$> (x .@? "AttributeValue") <*>
    (x .@? "AttributeName")
...
instance FromXML PolicyAttributeTypeDescription where
  parseXML x =
    PolicyAttributeTypeDescription' <$> (x .@? "AttributeType") <*>
    (x .@? "Cardinality") <*>
    (x .@? "DefaultValue") <*>
    (x .@? "AttributeName") <*>
    (x .@? "Description")
...
pdPolicyAttributeDescriptions ::
     Lens' PolicyDescription [PolicyAttributeDescription]
pdPolicyAttributeDescriptions =
  lens
...
instance FromXML PolicyDescription where
  parseXML x =
    PolicyDescription' <$> (x .@? "PolicyName") <*>
...
ptdPolicyAttributeTypeDescriptions ::
     Lens' PolicyTypeDescription [PolicyAttributeTypeDescription]
ptdPolicyAttributeTypeDescriptions =
  lens
...
instance FromXML PolicyTypeDescription where
  parseXML x =
    PolicyTypeDescription' <$> (x .@? "PolicyTypeName") <*>
    (x .@? "Description") <*>
...
instance FromXML SourceSecurityGroup where
  parseXML x =
    SourceSecurityGroup' <$> (x .@? "OwnerAlias") <*> (x .@? "GroupName")
...
instance FromXML TagDescription where
  parseXML x =
    TagDescription' <$> (x .@? "LoadBalancerName") <*>
