...
clbcspCookieExpirationPeriod ::
     Lens' CreateLBCookieStickinessPolicy (Maybe Integer)
clbcspCookieExpirationPeriod =
  lens
...
instance AWSRequest CreateLBCookieStickinessPolicy where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x -> CreateLBCookieStickinessPolicyResponse' <$> (pure (fromEnum s)))
...
clbcsprsResponseStatus =
  lens _clbcsprsResponseStatus (\s a -> s {_clbcsprsResponseStatus = a})
