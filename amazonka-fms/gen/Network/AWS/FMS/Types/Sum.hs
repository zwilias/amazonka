...
instance FromText PolicyComplianceStatusType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PolicyComplianceStatusType from value: '" <>
...
instance FromText SecurityServiceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SecurityServiceType from value: '" <>
        e <> "'. Accepted values: waf"
...
instance FromText ViolationReason where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ViolationReason from value: '" <>
        e <>
