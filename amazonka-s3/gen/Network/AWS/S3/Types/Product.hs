...
instance FromXML AbortIncompleteMultipartUpload where
  parseXML x = AbortIncompleteMultipartUpload' <$> (x .@? "DaysAfterInitiation")
...
instance FromXML AnalyticsConfiguration where
  parseXML x =
    AnalyticsConfiguration' <$> (x .@? "Filter") <*> (x .@ "Id") <*>
...
instance FromXML AnalyticsS3BucketDestination where
  parseXML x =
    AnalyticsS3BucketDestination' <$> (x .@? "BucketAccountId") <*>
    (x .@? "Prefix") <*>
    (x .@ "Format") <*>
    (x .@ "Bucket")
...
crExposeHeaders =
  lens _crExposeHeaders (\s a -> s {_crExposeHeaders = a}) . _Default . _Coerce
...
instance FromXML DeleteMarkerEntry where
  parseXML x =
    DeleteMarkerEntry' <$> (x .@? "VersionId") <*> (x .@? "IsLatest") <*>
    (x .@? "Owner") <*>
    (x .@? "Key") <*>
    (x .@? "LastModified")
...
instance FromXML DeletedObject where
  parseXML x =
    DeletedObject' <$> (x .@? "VersionId") <*> (x .@? "DeleteMarker") <*>
...
instance FromXML Destination where
  parseXML x =
    Destination' <$> (x .@? "AccessControlTranslation") <*> (x .@? "Account") <*>
    (x .@? "StorageClass") <*>
...
instance FromXML InventoryS3BucketDestination where
  parseXML x =
    InventoryS3BucketDestination' <$> (x .@? "Prefix") <*> (x .@? "AccountId") <*>
    (x .@? "Encryption") <*>
    (x .@ "Bucket") <*>
    (x .@ "Format")
...
instance FromXML LambdaFunctionConfiguration where
  parseXML x =
    LambdaFunctionConfiguration' <$> (x .@? "Id") <*> (x .@? "Filter") <*>
    (x .@ "CloudFunction") <*>
...
instance FromXML LifecycleExpiration where
  parseXML x =
    LifecycleExpiration' <$> (x .@? "Days") <*> (x .@? "Date") <*>
...
instance FromXML LifecycleRule where
  parseXML x =
    LifecycleRule' <$> (may (parseXMLList "Transition") x) <*>
...
instance FromXML LifecycleRuleAndOperator where
  parseXML x =
    LifecycleRuleAndOperator' <$> (x .@? "Prefix") <*>
    (x .@? "Tag" .!@ mempty >>= may (parseXMLList "Tag"))
...
instance FromXML LifecycleRuleFilter where
  parseXML x =
    LifecycleRuleFilter' <$> (x .@? "Tag") <*> (x .@? "Prefix") <*>
    (x .@? "And")
...
instance FromXML MultipartUpload where
  parseXML x =
    MultipartUpload' <$> (x .@? "Initiated") <*> (x .@? "Initiator") <*>
    (x .@? "Owner") <*>
    (x .@? "Key") <*>
    (x .@? "StorageClass") <*>
    (x .@? "UploadId")
...
instance FromXML NoncurrentVersionTransition where
  parseXML x =
    NoncurrentVersionTransition' <$> (x .@ "NoncurrentDays") <*>
    (x .@ "StorageClass")
...
instance FromXML NotificationConfiguration where
  parseXML x =
    NotificationConfiguration' <$> (may (parseXMLList "QueueConfiguration") x) <*>
...
instance FromXML ObjectVersion where
  parseXML x =
    ObjectVersion' <$> (x .@? "ETag") <*> (x .@? "VersionId") <*> (x .@? "Size") <*>
    (x .@? "IsLatest") <*>
    (x .@? "Owner") <*>
    (x .@? "Key") <*>
    (x .@? "StorageClass") <*>
...
instance ToXML OutputSerialization where
  toXML OutputSerialization' {..} = mconcat ["JSON" @= _osJSON, "CSV" @= _osCSV]
...
instance FromXML Part where
  parseXML x =
    Part' <$> (x .@? "ETag") <*> (x .@? "Size") <*> (x .@? "PartNumber") <*>
    (x .@? "LastModified")
...
instance FromXML Progress where
  parseXML x =
    Progress' <$> (x .@? "BytesReturned") <*> (x .@? "BytesScanned") <*>
    (x .@? "BytesProcessed")
...
instance FromXML QueueConfiguration where
  parseXML x =
    QueueConfiguration' <$> (x .@? "Id") <*> (x .@? "Filter") <*> (x .@ "Queue") <*>
    (parseXMLList "Event" x)
...
instance FromXML Redirect where
  parseXML x =
    Redirect' <$> (x .@? "HostName") <*> (x .@? "Protocol") <*>
    (x .@? "HttpRedirectCode") <*>
    (x .@? "ReplaceKeyWith") <*>
...
instance FromXML ReplicationRule where
  parseXML x =
    ReplicationRule' <$> (x .@? "ID") <*> (x .@? "SourceSelectionCriteria") <*>
    (x .@ "Prefix") <*>
    (x .@ "Status") <*>
    (x .@ "Destination")
...
instance FromXML S3ServiceError where
  parseXML x =
    S3ServiceError' <$> (x .@? "VersionId") <*> (x .@? "Key") <*> (x .@? "Code") <*>
    (x .@? "Message")
...
instance FromXML SelectObjectContentEventStream where
  parseXML x =
    SelectObjectContentEventStream' <$> (x .@? "Progress") <*> (x .@? "Records") <*>
    (x .@? "Cont") <*>
    (x .@? "Stats") <*>
    (x .@? "End")
...
instance FromXML ServerSideEncryptionByDefault where
  parseXML x =
    ServerSideEncryptionByDefault' <$> (x .@? "KMSMasterKeyID") <*>
    (x .@ "SSEAlgorithm")
...
instance FromXML Stats where
  parseXML x =
    Stats' <$> (x .@? "BytesReturned") <*> (x .@? "BytesScanned") <*>
    (x .@? "BytesProcessed")
...
instance FromXML StorageClassAnalysisDataExport where
  parseXML x =
    StorageClassAnalysisDataExport' <$> (x .@ "OutputSchemaVersion") <*>
    (x .@ "Destination")
...
instance FromXML TopicConfiguration where
  parseXML x =
    TopicConfiguration' <$> (x .@? "Id") <*> (x .@? "Filter") <*> (x .@ "Topic") <*>
    (parseXMLList "Event" x)
