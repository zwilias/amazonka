...
cmuWebsiteRedirectLocation =
  lens _cmuWebsiteRedirectLocation (\s a -> s {_cmuWebsiteRedirectLocation = a})
...
instance AWSRequest CreateMultipartUpload where
  ...
  response =
    receiveXML
      (\s h x ->
         CreateMultipartUploadResponse' <$> (h .#? "x-amz-request-charged") <*>
         (x .@? "Bucket") <*>
...
cmursSSEKMSKeyId =
  lens _cmursSSEKMSKeyId (\s a -> s {_cmursSSEKMSKeyId = a}) .
  mapping _Sensitive
