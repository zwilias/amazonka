...
instance AWSRequest GetBucketWebsite where
  ...
  response =
    receiveXML
      (\s h x ->
         GetBucketWebsiteResponse' <$> (x .@? "RedirectAllRequestsTo") <*>
         (x .@? "ErrorDocument") <*>
         (x .@? "IndexDocument") <*>
         (x .@? "RoutingRules" .!@ mempty >>= may (parseXMLList "RoutingRule")) <*>
         (pure (fromEnum s)))
...
gbwrsRedirectAllRequestsTo =
  lens _gbwrsRedirectAllRequestsTo (\s a -> s {_gbwrsRedirectAllRequestsTo = a})
