...
instance AWSRequest GetBucketAccelerateConfiguration where
  type Rs GetBucketAccelerateConfiguration = GetBucketAccelerateConfigurationResponse
  request = get s3
  response =
    receiveXML
      (\s h x ->
         GetBucketAccelerateConfigurationResponse' <$> (x .@? "Status") <*>
         (pure (fromEnum s)))
...
grsStatus ::
     Lens' GetBucketAccelerateConfigurationResponse (Maybe BucketAccelerateStatus)
