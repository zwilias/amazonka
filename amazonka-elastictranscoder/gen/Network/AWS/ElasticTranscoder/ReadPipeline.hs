...
instance AWSRequest ReadPipeline where
  ...
  response =
    receiveJSON
      (\s h x ->
         ReadPipelineResponse' <$> (x .?> "Warnings" .!@ mempty) <*>
         (x .?> "Pipeline") <*>
         (pure (fromEnum s)))
...
instance ToPath ReadPipeline where
  toPath ReadPipeline' {..} = mconcat ["/2012-09-25/pipelines/", toBS _rId]
...
rrsWarnings =
  lens _rrsWarnings (\s a -> s {_rrsWarnings = a}) . _Default . _Coerce
