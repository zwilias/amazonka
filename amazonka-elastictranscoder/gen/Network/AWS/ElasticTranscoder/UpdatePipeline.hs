...
instance AWSRequest UpdatePipeline where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdatePipelineResponse' <$> (x .?> "Warnings" .!@ mempty) <*>
         (x .?> "Pipeline") <*>
         (pure (fromEnum s)))
...
instance ToPath UpdatePipeline where
  toPath UpdatePipeline' {..} = mconcat ["/2012-09-25/pipelines/", toBS _upId]
...
uprsWarnings =
  lens _uprsWarnings (\s a -> s {_uprsWarnings = a}) . _Default . _Coerce
