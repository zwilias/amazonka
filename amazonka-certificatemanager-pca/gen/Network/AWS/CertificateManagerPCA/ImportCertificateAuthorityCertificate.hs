...
icacCertificateAuthorityARN =
  lens
    ...
    (\s a -> s {_icacCertificateAuthorityARN = a})
...
icacCertificate =
  lens _icacCertificate (\s a -> s {_icacCertificate = a}) . _Base64
...
icacCertificateChain =
  lens _icacCertificateChain (\s a -> s {_icacCertificateChain = a}) . _Base64

instance AWSRequest ImportCertificateAuthorityCertificate where
  type Rs ImportCertificateAuthorityCertificate = ImportCertificateAuthorityCertificateResponse
  request = postJSON certificateManagerPCA
  response = receiveNull ImportCertificateAuthorityCertificateResponse'
...
instance ToHeaders ImportCertificateAuthorityCertificate where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("ACMPrivateCA.ImportCertificateAuthorityCertificate" :: ByteString)
