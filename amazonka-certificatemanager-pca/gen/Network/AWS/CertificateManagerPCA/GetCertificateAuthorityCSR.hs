...
gcacsrCertificateAuthorityARN =
  lens
    ...
    (\s a -> s {_gcacsrCertificateAuthorityARN = a})

instance AWSRequest GetCertificateAuthorityCSR where
  type Rs GetCertificateAuthorityCSR = GetCertificateAuthorityCSRResponse
  request = postJSON certificateManagerPCA
  response =
    receiveJSON
      (\s h x ->
         GetCertificateAuthorityCSRResponse' <$> (x .?> "Csr") <*>
         (pure (fromEnum s)))
...
instance ToJSON GetCertificateAuthorityCSR where
  toJSON GetCertificateAuthorityCSR' {..} =
    object
      (catMaybes
         [Just ("CertificateAuthorityArn" .= _gcacsrCertificateAuthorityARN)])
...
gcacsrrsResponseStatus =
  lens _gcacsrrsResponseStatus (\s a -> s {_gcacsrrsResponseStatus = a})
