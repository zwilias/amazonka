...
icIdempotencyToken =
  lens _icIdempotencyToken (\s a -> s {_icIdempotencyToken = a})
...
icCertificateAuthorityARN =
  lens _icCertificateAuthorityARN (\s a -> s {_icCertificateAuthorityARN = a})
...
icSigningAlgorithm =
  lens _icSigningAlgorithm (\s a -> s {_icSigningAlgorithm = a})
...
instance AWSRequest IssueCertificate where
  ...
  response =
    receiveJSON
      (\s h x ->
         IssueCertificateResponse' <$> (x .?> "CertificateArn") <*>
         (pure (fromEnum s)))
...
icrsCertificateARN =
  lens _icrsCertificateARN (\s a -> s {_icrsCertificateARN = a})

-- | -- | The response status code.
icrsResponseStatus :: Lens' IssueCertificateResponse Int
icrsResponseStatus =
  lens _icrsResponseStatus (\s a -> s {_icrsResponseStatus = a})
