...
asOrganizationalUnit =
  lens _asOrganizationalUnit (\s a -> s {_asOrganizationalUnit = a})
...
asGenerationQualifier =
  lens _asGenerationQualifier (\s a -> s {_asGenerationQualifier = a})
...
asDistinguishedNameQualifier =
  lens
    ...
    (\s a -> s {_asDistinguishedNameQualifier = a})
...
instance FromJSON ASN1Subject where
  parseJSON =
    withObject
      "ASN1Subject"
      (\x ->
         ASN1Subject' <$> (x .:? "GivenName") <*> (x .:? "State") <*>
         (x .:? "CommonName") <*>
  ...
  toJSON ASN1Subject' {..} =
    object
      (catMaybes
         ...
         , ("Surname" .=) <$> _asSurname
         , ("DistinguishedNameQualifier" .=) <$> _asDistinguishedNameQualifier
         ])
...
caCertificateAuthorityConfiguration ::
     Lens' CertificateAuthority (Maybe CertificateAuthorityConfiguration)
caCertificateAuthorityConfiguration =
  lens
    _caCertificateAuthorityConfiguration
    (\s a -> s {_caCertificateAuthorityConfiguration = a})
...
caRevocationConfiguration ::
     Lens' CertificateAuthority (Maybe RevocationConfiguration)
caRevocationConfiguration =
  lens _caRevocationConfiguration (\s a -> s {_caRevocationConfiguration = a})

-- | Date and time at which your private CA was last updated.
caLastStateChangeAt :: Lens' CertificateAuthority (Maybe UTCTime)
caLastStateChangeAt =
  lens _caLastStateChangeAt (\s a -> s {_caLastStateChangeAt = a}) .
  mapping _Time
...
instance FromJSON CertificateAuthority where
  parseJSON =
    withObject
      "CertificateAuthority"
      (\x ->
         CertificateAuthority' <$> (x .:? "Status") <*> (x .:? "FailureReason") <*>
...
cacSigningAlgorithm =
  lens _cacSigningAlgorithm (\s a -> s {_cacSigningAlgorithm = a})
...
instance FromJSON CertificateAuthorityConfiguration where
  parseJSON =
    withObject
      "CertificateAuthorityConfiguration"
      (\x ->
         CertificateAuthorityConfiguration' <$> (x .: "KeyAlgorithm") <*>
         (x .: "SigningAlgorithm") <*>
         (x .: "Subject"))
...
ccExpirationInDays =
  lens _ccExpirationInDays (\s a -> s {_ccExpirationInDays = a}) . mapping _Nat
...
instance FromJSON CrlConfiguration where
  parseJSON =
    withObject
      "CrlConfiguration"
      (\x ->
         CrlConfiguration' <$> (x .:? "CustomCname") <*>
         (x .:? "ExpirationInDays") <*>
         (x .:? "S3BucketName") <*>
         (x .: "Enabled"))
...
rcCrlConfiguration =
  lens _rcCrlConfiguration (\s a -> s {_rcCrlConfiguration = a})
...
instance ToJSON RevocationConfiguration where
  toJSON RevocationConfiguration' {..} =
    object (catMaybes [("CrlConfiguration" .=) <$> _rcCrlConfiguration])
...
instance ToJSON Validity where
  toJSON Validity' {..} =
    object (catMaybes [Just ("Value" .= _vValue), Just ("Type" .= _vType)])
