...
instance FromText AuditReportResponseFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuditReportResponseFormat from value: '" <>
...
instance FromText AuditReportStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuditReportStatus from value: '" <>
        e <> "'. Accepted values: creating, failed, success"
...
instance FromText CertificateAuthorityStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CertificateAuthorityStatus from value: '" <>
        e <>
...
instance FromText CertificateAuthorityType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CertificateAuthorityType from value: '" <>
        e <> "'. Accepted values: subordinate"
...
instance FromText SigningAlgorithm where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SigningAlgorithm from value: '" <>
        e <>
...
instance FromText ValidityPeriodType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ValidityPeriodType from value: '" <>
        e <> "'. Accepted values: absolute, days, end_date, months, years"
