...
gcacCertificateAuthorityARN =
  lens
    ...
    (\s a -> s {_gcacCertificateAuthorityARN = a})

instance AWSRequest GetCertificateAuthorityCertificate where
  type Rs GetCertificateAuthorityCertificate = GetCertificateAuthorityCertificateResponse
  request = postJSON certificateManagerPCA
  response =
    receiveJSON
      (\s h x ->
         GetCertificateAuthorityCertificateResponse' <$> (x .?> "Certificate") <*>
         (x .?> "CertificateChain") <*>
         (pure (fromEnum s)))
...
gcacrsCertificate ::
     Lens' GetCertificateAuthorityCertificateResponse (Maybe Text)
...
gcacrsCertificateChain ::
     Lens' GetCertificateAuthorityCertificateResponse (Maybe Text)
gcacrsCertificateChain =
  lens _gcacrsCertificateChain (\s a -> s {_gcacrsCertificateChain = a})

-- | -- | The response status code.
gcacrsResponseStatus :: Lens' GetCertificateAuthorityCertificateResponse Int
gcacrsResponseStatus =
  lens _gcacrsResponseStatus (\s a -> s {_gcacrsResponseStatus = a})
