...
rcCertificateSerial =
  lens _rcCertificateSerial (\s a -> s {_rcCertificateSerial = a})

-- | Specifies why you revoked the certificate.
rcRevocationReason :: Lens' RevokeCertificate RevocationReason
rcRevocationReason =
  lens _rcRevocationReason (\s a -> s {_rcRevocationReason = a})
...
instance ToHeaders RevokeCertificate where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("ACMPrivateCA.RevokeCertificate" :: ByteString)
