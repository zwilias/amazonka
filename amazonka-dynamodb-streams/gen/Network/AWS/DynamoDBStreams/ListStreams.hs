...
lsExclusiveStartStreamARN =
  lens _lsExclusiveStartStreamARN (\s a -> s {_lsExclusiveStartStreamARN = a})
...
instance AWSRequest ListStreams where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListStreamsResponse' <$> (x .?> "LastEvaluatedStreamArn") <*>
         (x .?> "Streams" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lsrsLastEvaluatedStreamARN =
  lens _lsrsLastEvaluatedStreamARN (\s a -> s {_lsrsLastEvaluatedStreamARN = a})

-- | A list of stream descriptors associated with the current account and endpoint.
lsrsStreams :: Lens' ListStreamsResponse [Stream]
lsrsStreams =
  lens _lsrsStreams (\s a -> s {_lsrsStreams = a}) . _Default . _Coerce
