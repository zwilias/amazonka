...
gftDurationSeconds =
  lens _gftDurationSeconds (\s a -> s {_gftDurationSeconds = a}) . mapping _Nat
...
instance AWSRequest GetFederationToken where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         GetFederationTokenResponse' <$> (x .@? "PackedPolicySize") <*>
         (x .@? "Credentials") <*>
         (x .@? "FederatedUser") <*>
         (pure (fromEnum s)))
...
gftrsResponseStatus =
  lens _gftrsResponseStatus (\s a -> s {_gftrsResponseStatus = a})
