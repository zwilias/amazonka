...
grNumberOfBytes =
  lens _grNumberOfBytes (\s a -> s {_grNumberOfBytes = a}) . mapping _Nat
...
instance AWSRequest GenerateRandom where
  ...
  response =
    receiveJSON
      (\s h x ->
         GenerateRandomResponse' <$> (x .?> "Plaintext") <*> (pure (fromEnum s)))
...
instance ToJSON GenerateRandom where
  toJSON GenerateRandom' {..} =
    object (catMaybes [("NumberOfBytes" .=) <$> _grNumberOfBytes])
...
grrsPlaintext =
  lens _grrsPlaintext (\s a -> s {_grrsPlaintext = a}) .
