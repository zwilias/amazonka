...
--
-- * 'ckKeyUsage' - Determines the cryptographic operations for which you can use the CMK. The default value is @ENCRYPT_DECRYPT@ . This parameter is required only for asymmetric CMKs. You can't change the @KeyUsage@ value after the CMK is created. Select only one valid value.     * For symmetric CMKs, omit the parameter or specify @ENCRYPT_DECRYPT@ .     * For asymmetric CMKs with RSA key material, specify @ENCRYPT_DECRYPT@ or @SIGN_VERIFY@ .     * For asymmetric CMKs with ECC key material, specify @SIGN_VERIFY@ .
--
