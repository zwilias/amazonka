...
instance AWSRequest GetKeyRotationStatus where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetKeyRotationStatusResponse' <$> (x .?> "KeyRotationEnabled") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetKeyRotationStatus where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("TrentService.GetKeyRotationStatus" :: ByteString)
...
gkrsrsKeyRotationEnabled =
  lens _gkrsrsKeyRotationEnabled (\s a -> s {_gkrsrsKeyRotationEnabled = a})

-- | -- | The response status code.
gkrsrsResponseStatus :: Lens' GetKeyRotationStatusResponse Int
gkrsrsResponseStatus =
  lens _gkrsrsResponseStatus (\s a -> s {_gkrsrsResponseStatus = a})
