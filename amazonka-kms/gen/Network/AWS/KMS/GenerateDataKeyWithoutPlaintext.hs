...
gdkwpEncryptionContext ::
     Lens' GenerateDataKeyWithoutPlaintext (HashMap Text Text)
gdkwpEncryptionContext =
...
gdkwpNumberOfBytes =
  lens _gdkwpNumberOfBytes (\s a -> s {_gdkwpNumberOfBytes = a}) . mapping _Nat
...
gdkwpGrantTokens =
  lens _gdkwpGrantTokens (\s a -> s {_gdkwpGrantTokens = a}) .
  _Default . _Coerce
...
instance AWSRequest GenerateDataKeyWithoutPlaintext where
  ...
  response =
    receiveJSON
      (\s h x ->
         GenerateDataKeyWithoutPlaintextResponse' <$> (x .?> "KeyId") <*>
         (x .?> "CiphertextBlob") <*>
         (pure (fromEnum s)))
...
gdkwprsCiphertextBlob ::
     Lens' GenerateDataKeyWithoutPlaintextResponse (Maybe ByteString)
gdkwprsCiphertextBlob =
  lens _gdkwprsCiphertextBlob (\s a -> s {_gdkwprsCiphertextBlob = a}) .
  mapping _Base64

-- | -- | The response status code.
gdkwprsResponseStatus :: Lens' GenerateDataKeyWithoutPlaintextResponse Int
gdkwprsResponseStatus =
  lens _gdkwprsResponseStatus (\s a -> s {_gdkwprsResponseStatus = a})
