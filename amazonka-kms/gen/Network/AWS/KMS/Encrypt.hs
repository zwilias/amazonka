...
eEncryptionContext =
  lens _eEncryptionContext (\s a -> s {_eEncryptionContext = a}) . _Default .
  _Map
...
eGrantTokens =
  lens _eGrantTokens (\s a -> s {_eGrantTokens = a}) . _Default . _Coerce
...
ePlaintext =
  lens _ePlaintext (\s a -> s {_ePlaintext = a}) . _Sensitive . _Base64
...
instance AWSRequest Encrypt where
  ...
  response =
    receiveJSON
      (\s h x ->
         EncryptResponse' <$> (x .?> "KeyId") <*> (x .?> "CiphertextBlob") <*>
         (pure (fromEnum s)))
...
ersCiphertextBlob =
  lens _ersCiphertextBlob (\s a -> s {_ersCiphertextBlob = a}) . mapping _Base64
