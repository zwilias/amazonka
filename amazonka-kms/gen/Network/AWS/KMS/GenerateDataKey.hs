...
gdkEncryptionContext =
  lens _gdkEncryptionContext (\s a -> s {_gdkEncryptionContext = a}) . _Default .
  _Map
...
gdkNumberOfBytes =
  lens _gdkNumberOfBytes (\s a -> s {_gdkNumberOfBytes = a}) . mapping _Nat
...
gdkGrantTokens =
  lens _gdkGrantTokens (\s a -> s {_gdkGrantTokens = a}) . _Default . _Coerce
...
instance AWSRequest GenerateDataKey where
  ...
  response =
    receiveJSON
      (\s h x ->
         GenerateDataKeyResponse' <$> (pure (fromEnum s)) <*> (x .:> "KeyId") <*>
         (x .:> "Plaintext") <*>
         (x .:> "CiphertextBlob"))
...
gdkrsResponseStatus =
  lens _gdkrsResponseStatus (\s a -> s {_gdkrsResponseStatus = a})
...
gdkrsPlaintext =
  lens _gdkrsPlaintext (\s a -> s {_gdkrsPlaintext = a}) . _Sensitive . _Base64
