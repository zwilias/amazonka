...
reSourceEncryptionContext =
  lens _reSourceEncryptionContext (\s a -> s {_reSourceEncryptionContext = a}) .
  _Default . _Map
...
reGrantTokens =
  lens _reGrantTokens (\s a -> s {_reGrantTokens = a}) . _Default . _Coerce
...
reCiphertextBlob =
  lens _reCiphertextBlob (\s a -> s {_reCiphertextBlob = a}) . _Base64
...
instance AWSRequest ReEncrypt where
  ...
  response =
    receiveJSON
      (\s h x ->
         ReEncryptResponse' <$> (x .?> "SourceKeyId") <*> (x .?> "KeyId") <*>
         (x .?> "CiphertextBlob") <*>
         (pure (fromEnum s)))
...
rersCiphertextBlob =
  lens _rersCiphertextBlob (\s a -> s {_rersCiphertextBlob = a}) .
  mapping _Base64

-- | -- | The response status code.
rersResponseStatus :: Lens' ReEncryptResponse Int
rersResponseStatus =
  lens _rersResponseStatus (\s a -> s {_rersResponseStatus = a})
