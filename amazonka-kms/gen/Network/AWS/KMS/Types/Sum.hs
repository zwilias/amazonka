...
instance FromText AlgorithmSpec where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AlgorithmSpec from value: '" <>
        e <>
...
instance FromText DataKeySpec where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DataKeySpec from value: '" <>
        e <> "'. Accepted values: aes_128, aes_256"
...
instance FromText ExpirationModelType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExpirationModelType from value: '" <>
        e <>
...
instance FromText GrantOperation where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing GrantOperation from value: '" <>
        e <>
...
instance FromText KeyManagerType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing KeyManagerType from value: '" <>
        e <> "'. Accepted values: aws, customer"
...
instance FromText KeyState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing KeyState from value: '" <>
        e <>
...
instance FromText KeyUsageType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing KeyUsageType from value: '" <>
        e <> "'. Accepted values: encrypt_decrypt"
...
instance FromText OriginType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OriginType from value: '" <>
        e <> "'. Accepted values: aws_kms, external"
...
instance FromText WrappingKeySpec where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing WrappingKeySpec from value: '" <>
        e <> "'. Accepted values: rsa_2048"
