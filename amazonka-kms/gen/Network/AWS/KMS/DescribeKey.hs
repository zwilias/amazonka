...
dGrantTokens =
  lens _dGrantTokens (\s a -> s {_dGrantTokens = a}) . _Default . _Coerce
...
instance AWSRequest DescribeKey where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeKeyResponse' <$> (x .?> "KeyMetadata") <*> (pure (fromEnum s)))
...
dkrsResponseStatus =
  lens _dkrsResponseStatus (\s a -> s {_dkrsResponseStatus = a})
