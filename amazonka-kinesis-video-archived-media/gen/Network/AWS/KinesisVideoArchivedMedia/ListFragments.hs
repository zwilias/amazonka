...
lfFragmentSelector =
  lens _lfFragmentSelector (\s a -> s {_lfFragmentSelector = a})
...
instance AWSRequest ListFragments where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListFragmentsResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Fragments" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lfrsFragments =
  lens _lfrsFragments (\s a -> s {_lfrsFragments = a}) . _Default . _Coerce

-- | -- | The response status code.
lfrsResponseStatus :: Lens' ListFragmentsResponse Int
lfrsResponseStatus =
  lens _lfrsResponseStatus (\s a -> s {_lfrsResponseStatus = a})
