...
gmfflFragments =
  lens _gmfflFragments (\s a -> s {_gmfflFragments = a}) . _Coerce
...
instance AWSRequest GetMediaForFragmentList where
  ...
  response =
    receiveBody
      (\s h x ->
         GetMediaForFragmentListResponse' <$> (h .#? "Content-Type") <*>
         (pure (fromEnum s)) <*>
         (pure x))
...
gmfflrsContentType =
  lens _gmfflrsContentType (\s a -> s {_gmfflrsContentType = a})

-- | -- | The response status code.
gmfflrsResponseStatus :: Lens' GetMediaForFragmentListResponse Int
gmfflrsResponseStatus =
  lens _gmfflrsResponseStatus (\s a -> s {_gmfflrsResponseStatus = a})
