...
fFragmentLengthInMilliseconds =
  lens
    ...
    (\s a -> s {_fFragmentLengthInMilliseconds = a})

-- | The time stamp from the AWS server corresponding to the fragment.
fServerTimestamp :: Lens' Fragment (Maybe UTCTime)
fServerTimestamp =
  lens _fServerTimestamp (\s a -> s {_fServerTimestamp = a}) . mapping _Time

-- | The total fragment size, including information about the fragment and contained media data.
fFragmentSizeInBytes :: Lens' Fragment (Maybe Integer)
fFragmentSizeInBytes =
  lens _fFragmentSizeInBytes (\s a -> s {_fFragmentSizeInBytes = a})
...
fProducerTimestamp =
  lens _fProducerTimestamp (\s a -> s {_fProducerTimestamp = a}) . mapping _Time
...
instance FromJSON Fragment where
  parseJSON =
    withObject
      "Fragment"
      (\x ->
         Fragment' <$> (x .:? "FragmentLengthInMilliseconds") <*>
         (x .:? "ServerTimestamp") <*>
...
fsFragmentSelectorType =
  lens _fsFragmentSelectorType (\s a -> s {_fsFragmentSelectorType = a})
...
trStartTimestamp =
  lens _trStartTimestamp (\s a -> s {_trStartTimestamp = a}) . _Time
