...
instance AWSRequest DescribeEnvironmentMemberships where
  type Rs DescribeEnvironmentMemberships = DescribeEnvironmentMembershipsResponse
  request = postJSON cloud9
  response =
    receiveJSON
      (\s h x ->
         DescribeEnvironmentMembershipsResponse' <$> (x .?> "nextToken") <*>
...
instance ToHeaders DescribeEnvironmentMemberships where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCloud9WorkspaceManagementService.DescribeEnvironmentMemberships" :: ByteString)
...
drsMemberships ::
     Lens' DescribeEnvironmentMembershipsResponse [EnvironmentMember]
drsMemberships =
  lens _drsMemberships (\s a -> s {_drsMemberships = a}) . _Default . _Coerce
