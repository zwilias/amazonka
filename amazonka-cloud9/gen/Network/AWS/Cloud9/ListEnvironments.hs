...
instance AWSRequest ListEnvironments where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListEnvironmentsResponse' <$> (x .?> "environmentIds" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListEnvironments where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCloud9WorkspaceManagementService.ListEnvironments" :: ByteString)
...
lersEnvironmentIds =
  lens _lersEnvironmentIds (\s a -> s {_lersEnvironmentIds = a}) .
  _Default . _Coerce
