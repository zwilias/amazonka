...
instance AWSRequest UpdateEnvironmentMembership where
  type Rs UpdateEnvironmentMembership = UpdateEnvironmentMembershipResponse
  request = postJSON cloud9
  response =
    receiveJSON
      (\s h x ->
         UpdateEnvironmentMembershipResponse' <$> (x .?> "membership") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateEnvironmentMembership where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCloud9WorkspaceManagementService.UpdateEnvironmentMembership" :: ByteString)
...
uemrsMembership ::
     Lens' UpdateEnvironmentMembershipResponse (Maybe EnvironmentMember)
...
uemrsResponseStatus =
  lens _uemrsResponseStatus (\s a -> s {_uemrsResponseStatus = a})
