...
instance FromText MemberPermissions where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MemberPermissions from value: '" <>
        e <> "'. Accepted values: read-only, read-write"
...
instance FromText Permissions where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Permissions from value: '" <>
        e <> "'. Accepted values: owner, read-only, read-write"
