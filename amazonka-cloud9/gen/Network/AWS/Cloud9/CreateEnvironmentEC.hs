...
ceecAutomaticStopTimeMinutes =
  lens
    ...
    (\s a -> s {_ceecAutomaticStopTimeMinutes = a})
...
ceecClientRequestToken =
  lens _ceecClientRequestToken (\s a -> s {_ceecClientRequestToken = a})
...
instance AWSRequest CreateEnvironmentEC where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateEnvironmentECResponse' <$> (x .?> "environmentId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders CreateEnvironmentEC where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCloud9WorkspaceManagementService.CreateEnvironmentEC" :: ByteString)
...
ceecrsEnvironmentId =
  lens _ceecrsEnvironmentId (\s a -> s {_ceecrsEnvironmentId = a})

-- | -- | The response status code.
ceecrsResponseStatus :: Lens' CreateEnvironmentECResponse Int
ceecrsResponseStatus =
  lens _ceecrsResponseStatus (\s a -> s {_ceecrsResponseStatus = a})
