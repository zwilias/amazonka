...
instance AWSRequest DeleteEnvironmentMembership where
  type Rs DeleteEnvironmentMembership = DeleteEnvironmentMembershipResponse
  request = postJSON cloud9
  response =
    receiveEmpty
      (\s h x -> DeleteEnvironmentMembershipResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteEnvironmentMembership where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSCloud9WorkspaceManagementService.DeleteEnvironmentMembership" :: ByteString)
...
demrsResponseStatus =
  lens _demrsResponseStatus (\s a -> s {_demrsResponseStatus = a})
