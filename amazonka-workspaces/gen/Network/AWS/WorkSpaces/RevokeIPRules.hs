...
instance AWSRequest RevokeIPRules where
  ...
  response =
    receiveEmpty (\s h x -> RevokeIPRulesResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders RevokeIPRules where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("WorkspacesService.RevokeIpRules" :: ByteString)
...
instance ToJSON RevokeIPRules where
  toJSON RevokeIPRules' {..} =
    object
      (catMaybes
         [Just ("GroupId" .= _rirGroupId), Just ("UserRules" .= _rirUserRules)])
...
rirrsResponseStatus =
  lens _rirrsResponseStatus (\s a -> s {_rirrsResponseStatus = a})
