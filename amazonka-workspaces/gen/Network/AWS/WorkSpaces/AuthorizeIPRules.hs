...
instance AWSRequest AuthorizeIPRules where
  ...
  response =
    receiveEmpty (\s h x -> AuthorizeIPRulesResponse' <$> (pure (fromEnum s)))
...
instance ToJSON AuthorizeIPRules where
  toJSON AuthorizeIPRules' {..} =
    object
      (catMaybes
         [Just ("GroupId" .= _airGroupId), Just ("UserRules" .= _airUserRules)])
...
airrsResponseStatus =
  lens _airrsResponseStatus (\s a -> s {_airrsResponseStatus = a})
