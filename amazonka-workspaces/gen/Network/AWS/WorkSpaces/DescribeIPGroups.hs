...
dipgGroupIds =
  lens _dipgGroupIds (\s a -> s {_dipgGroupIds = a}) . _Default . _Coerce
...
dipgMaxResults =
  lens _dipgMaxResults (\s a -> s {_dipgMaxResults = a}) . mapping _Nat
...
instance AWSRequest DescribeIPGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeIPGroupsResponse' <$> (x .?> "Result" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
digsrsResult =
  lens _digsrsResult (\s a -> s {_digsrsResult = a}) . _Default . _Coerce
...
digsrsResponseStatus =
  lens _digsrsResponseStatus (\s a -> s {_digsrsResponseStatus = a})
