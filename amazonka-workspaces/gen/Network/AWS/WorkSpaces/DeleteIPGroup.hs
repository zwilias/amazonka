...
instance AWSRequest DeleteIPGroup where
  ...
  response =
    receiveEmpty (\s h x -> DeleteIPGroupResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteIPGroup where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("WorkspacesService.DeleteIpGroup" :: ByteString)
...
dipgrsResponseStatus =
  lens _dipgrsResponseStatus (\s a -> s {_dipgrsResponseStatus = a})
