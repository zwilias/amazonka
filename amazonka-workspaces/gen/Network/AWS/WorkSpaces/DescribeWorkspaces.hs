...
dwWorkspaceIds =
  lens _dwWorkspaceIds (\s a -> s {_dwWorkspaceIds = a}) . mapping _List1
...
instance AWSRequest DescribeWorkspaces where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeWorkspacesResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Workspaces" .!@ mempty) <*>
         (pure (fromEnum s)))
...
dwrsWorkspaces =
  lens _dwrsWorkspaces (\s a -> s {_dwrsWorkspaces = a}) . _Default . _Coerce

-- | -- | The response status code.
dwrsResponseStatus :: Lens' DescribeWorkspacesResponse Int
dwrsResponseStatus =
  lens _dwrsResponseStatus (\s a -> s {_dwrsResponseStatus = a})
