...
swStopWorkspaceRequests =
  lens _swStopWorkspaceRequests (\s a -> s {_swStopWorkspaceRequests = a}) .
  _List1
...
instance AWSRequest StopWorkspaces where
  ...
  response =
    receiveJSON
      (\s h x ->
         StopWorkspacesResponse' <$> (x .?> "FailedRequests" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders StopWorkspaces where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("WorkspacesService.StopWorkspaces" :: ByteString)
...
instance ToJSON StopWorkspaces where
  toJSON StopWorkspaces' {..} =
    object
      (catMaybes [Just ("StopWorkspaceRequests" .= _swStopWorkspaceRequests)])
...
srsFailedRequests =
  lens _srsFailedRequests (\s a -> s {_srsFailedRequests = a}) .
  _Default . _Coerce
