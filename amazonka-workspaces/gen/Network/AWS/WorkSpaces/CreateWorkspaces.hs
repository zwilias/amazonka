...
instance AWSRequest CreateWorkspaces where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateWorkspacesResponse' <$> (x .?> "FailedRequests" .!@ mempty) <*>
...
cwrsFailedRequests ::
     Lens' CreateWorkspacesResponse [FailedCreateWorkspaceRequest]
cwrsFailedRequests =
  lens _cwrsFailedRequests (\s a -> s {_cwrsFailedRequests = a}) .
  _Default . _Coerce
...
cwrsPendingRequests =
  lens _cwrsPendingRequests (\s a -> s {_cwrsPendingRequests = a}) .
  _Default . _Coerce

-- | -- | The response status code.
cwrsResponseStatus :: Lens' CreateWorkspacesResponse Int
cwrsResponseStatus =
  lens _cwrsResponseStatus (\s a -> s {_cwrsResponseStatus = a})
