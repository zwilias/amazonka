...
mwpWorkspaceProperties =
  lens _mwpWorkspaceProperties (\s a -> s {_mwpWorkspaceProperties = a})

instance AWSRequest ModifyWorkspaceProperties where
  type Rs ModifyWorkspaceProperties = ModifyWorkspacePropertiesResponse
  request = postJSON workSpaces
  response =
    receiveEmpty
      (\s h x -> ModifyWorkspacePropertiesResponse' <$> (pure (fromEnum s)))
...
mwprsResponseStatus =
  lens _mwprsResponseStatus (\s a -> s {_mwprsResponseStatus = a})
