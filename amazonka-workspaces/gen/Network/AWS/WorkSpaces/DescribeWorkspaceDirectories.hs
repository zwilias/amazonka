...
dwdDirectoryIds =
  lens _dwdDirectoryIds (\s a -> s {_dwdDirectoryIds = a}) . mapping _List1
...
dwdrsDirectories ::
     Lens' DescribeWorkspaceDirectoriesResponse [WorkspaceDirectory]
dwdrsDirectories =
  lens _dwdrsDirectories (\s a -> s {_dwdrsDirectories = a}) .
  _Default . _Coerce
...
dwdrsResponseStatus =
  lens _dwdrsResponseStatus (\s a -> s {_dwdrsResponseStatus = a})
