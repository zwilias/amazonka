...
instance FromText Compute where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Compute from value: '" <>
        e <> "'. Accepted values: graphics, performance, power, standard, value"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: connected, disconnected, unknown"
...
instance FromText ModificationResourceEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ModificationResourceEnum from value: '" <>
        e <> "'. Accepted values: compute_type, root_volume, user_volume"
...
instance FromText ModificationStateEnum where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ModificationStateEnum from value: '" <>
        e <> "'. Accepted values: update_in_progress, update_initiated"
...
instance FromText RunningMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RunningMode from value: '" <>
        e <> "'. Accepted values: always_on, auto_stop"
...
instance FromText TargetWorkspaceState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TargetWorkspaceState from value: '" <>
        e <> "'. Accepted values: admin_maintenance, available"
...
instance FromText WorkspaceDirectoryState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing WorkspaceDirectoryState from value: '" <>
        e <>
...
instance FromText WorkspaceDirectoryType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing WorkspaceDirectoryType from value: '" <>
        e <> "'. Accepted values: ad_connector, simple_ad"
...
instance FromText WorkspaceState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing WorkspaceState from value: '" <>
        e <>
