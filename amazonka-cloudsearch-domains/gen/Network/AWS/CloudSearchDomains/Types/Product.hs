...
instance FromJSON Bucket where
  parseJSON =
    withObject "Bucket" (\x -> Bucket' <$> (x .:? "value") <*> (x .:? "count"))
...
instance FromJSON BucketInfo where
  parseJSON =
    withObject "BucketInfo" (\x -> BucketInfo' <$> (x .:? "buckets" .!= mempty))
...
instance FromJSON FieldStats where
  parseJSON =
    withObject
      "FieldStats"
      (\x ->
         FieldStats' <$> (x .:? "max") <*> (x .:? "mean") <*> (x .:? "count") <*>
         (x .:? "missing") <*>
         (x .:? "stddev") <*>
         (x .:? "min") <*>
         (x .:? "sumOfSquares") <*>
...
hitHighlights =
  lens _hitHighlights (\s a -> s {_hitHighlights = a}) . _Default . _Map
...
smSuggestions =
  lens _smSuggestions (\s a -> s {_smSuggestions = a}) . _Default . _Coerce
...
instance FromJSON SuggestModel where
  parseJSON =
    withObject
      "SuggestModel"
      (\x ->
         SuggestModel' <$> (x .:? "found") <*> (x .:? "suggestions" .!= mempty) <*>
         (x .:? "query"))
...
instance FromJSON SuggestionMatch where
  parseJSON =
    withObject
      "SuggestionMatch"
      (\x ->
         SuggestionMatch' <$> (x .:? "suggestion") <*> (x .:? "score") <*>
         (x .:? "id"))
