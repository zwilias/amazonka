...
dmAccountIds =
  lens _dmAccountIds (\s a -> s {_dmAccountIds = a}) . _Default . _Coerce
...
instance AWSRequest DeleteMembers where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteMembersResponse' <$> (x .?> "unprocessedAccounts" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteMembers where
  toJSON DeleteMembers' {..} =
    object (catMaybes [("accountIds" .=) <$> _dmAccountIds])
...
drsUnprocessedAccounts =
  lens _drsUnprocessedAccounts (\s a -> s {_drsUnprocessedAccounts = a}) .
  _Default . _Coerce
