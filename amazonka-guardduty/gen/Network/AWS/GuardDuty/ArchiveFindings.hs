...
afFindingIds =
  lens _afFindingIds (\s a -> s {_afFindingIds = a}) . _Default . _Coerce
...
instance AWSRequest ArchiveFindings where
  ...
  response =
    receiveEmpty (\s h x -> ArchiveFindingsResponse' <$> (pure (fromEnum s)))
...
instance ToJSON ArchiveFindings where
  toJSON ArchiveFindings' {..} =
    object (catMaybes [("findingIds" .=) <$> _afFindingIds])
