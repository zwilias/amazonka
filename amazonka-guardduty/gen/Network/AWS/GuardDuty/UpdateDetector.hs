...
instance AWSRequest UpdateDetector where
  ...
  response =
    receiveEmpty (\s h x -> UpdateDetectorResponse' <$> (pure (fromEnum s)))
...
instance ToJSON UpdateDetector where
  toJSON UpdateDetector' {..} = object (catMaybes [("enable" .=) <$> _udEnable])

instance ToPath UpdateDetector where
  toPath UpdateDetector' {..} = mconcat ["/detector/", toBS _udDetectorId]
