...
smmAccountIds =
  lens _smmAccountIds (\s a -> s {_smmAccountIds = a}) . _Default . _Coerce
...
instance ToJSON StopMonitoringMembers where
  toJSON StopMonitoringMembers' {..} =
    object (catMaybes [("accountIds" .=) <$> _smmAccountIds])
...
smmrsUnprocessedAccounts ::
     Lens' StopMonitoringMembersResponse [UnprocessedAccount]
smmrsUnprocessedAccounts =
  lens _smmrsUnprocessedAccounts (\s a -> s {_smmrsUnprocessedAccounts = a}) .
  _Default . _Coerce

-- | -- | The response status code.
smmrsResponseStatus :: Lens' StopMonitoringMembersResponse Int
smmrsResponseStatus =
  lens _smmrsResponseStatus (\s a -> s {_smmrsResponseStatus = a})
