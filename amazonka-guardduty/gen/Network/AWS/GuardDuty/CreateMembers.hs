...
cmAccountDetails =
  lens _cmAccountDetails (\s a -> s {_cmAccountDetails = a}) .
  _Default . _Coerce
...
instance AWSRequest CreateMembers where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateMembersResponse' <$> (x .?> "unprocessedAccounts" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON CreateMembers where
  toJSON CreateMembers' {..} =
    object (catMaybes [("accountDetails" .=) <$> _cmAccountDetails])
...
cmrsUnprocessedAccounts =
  lens _cmrsUnprocessedAccounts (\s a -> s {_cmrsUnprocessedAccounts = a}) .
  _Default . _Coerce
