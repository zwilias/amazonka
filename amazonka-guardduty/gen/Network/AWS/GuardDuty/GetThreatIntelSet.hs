...
gtisThreatIntelSetId =
  lens _gtisThreatIntelSetId (\s a -> s {_gtisThreatIntelSetId = a})
...
instance AWSRequest GetThreatIntelSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetThreatIntelSetResponse' <$> (x .?> "status") <*> (x .?> "location") <*>
         (x .?> "format") <*>
         (x .?> "name") <*>
         (pure (fromEnum s)))
...
gtisrsResponseStatus =
  lens _gtisrsResponseStatus (\s a -> s {_gtisrsResponseStatus = a})
