...
gfsFindingStatisticTypes =
  lens _gfsFindingStatisticTypes (\s a -> s {_gfsFindingStatisticTypes = a}) .
  _Default . _Coerce

-- | Represents the criteria used for querying findings.
gfsFindingCriteria :: Lens' GetFindingsStatistics (Maybe FindingCriteria)
gfsFindingCriteria =
  lens _gfsFindingCriteria (\s a -> s {_gfsFindingCriteria = a})
...
instance AWSRequest GetFindingsStatistics where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetFindingsStatisticsResponse' <$> (x .?> "findingStatistics") <*>
         (pure (fromEnum s)))
...
gfsrsFindingStatistics ::
     Lens' GetFindingsStatisticsResponse (Maybe FindingStatistics)
gfsrsFindingStatistics =
  lens _gfsrsFindingStatistics (\s a -> s {_gfsrsFindingStatistics = a})

-- | -- | The response status code.
gfsrsResponseStatus :: Lens' GetFindingsStatisticsResponse Int
gfsrsResponseStatus =
  lens _gfsrsResponseStatus (\s a -> s {_gfsrsResponseStatus = a})
