...
lisMaxResults =
  lens _lisMaxResults (\s a -> s {_lisMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListIPSets where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListIPSetsResponse' <$> (x .?> "nextToken") <*>
         (x .?> "ipSetIds" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToQuery ListIPSets where
  toQuery ListIPSets' {..} =
    mconcat ["nextToken" =: _lisNextToken, "maxResults" =: _lisMaxResults]
...
lisrsIPSetIds =
  lens _lisrsIPSetIds (\s a -> s {_lisrsIPSetIds = a}) . _Default . _Coerce
