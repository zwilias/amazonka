...
gmAccountIds =
  lens _gmAccountIds (\s a -> s {_gmAccountIds = a}) . _Default . _Coerce
...
instance AWSRequest GetMembers where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetMembersResponse' <$> (x .?> "members" .!@ mempty) <*>
...
instance ToJSON GetMembers where
  toJSON GetMembers' {..} =
    object (catMaybes [("accountIds" .=) <$> _gmAccountIds])
...
gmrsMembers =
  lens _gmrsMembers (\s a -> s {_gmrsMembers = a}) . _Default . _Coerce

-- | A list of objects containing the unprocessed account and a result string explaining why it was unprocessed.
gmrsUnprocessedAccounts :: Lens' GetMembersResponse [UnprocessedAccount]
gmrsUnprocessedAccounts =
  lens _gmrsUnprocessedAccounts (\s a -> s {_gmrsUnprocessedAccounts = a}) .
  _Default . _Coerce
