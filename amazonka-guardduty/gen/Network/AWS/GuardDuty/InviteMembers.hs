...
imAccountIds =
  lens _imAccountIds (\s a -> s {_imAccountIds = a}) . _Default . _Coerce
...
imDisableEmailNotification =
  lens _imDisableEmailNotification (\s a -> s {_imDisableEmailNotification = a})
...
instance AWSRequest InviteMembers where
  ...
  response =
    receiveJSON
      (\s h x ->
         InviteMembersResponse' <$> (x .?> "unprocessedAccounts" .!@ mempty) <*>
         (pure (fromEnum s)))
...
imrsUnprocessedAccounts =
  lens _imrsUnprocessedAccounts (\s a -> s {_imrsUnprocessedAccounts = a}) .
  _Default . _Coerce

-- | -- | The response status code.
imrsResponseStatus :: Lens' InviteMembersResponse Int
imrsResponseStatus =
  lens _imrsResponseStatus (\s a -> s {_imrsResponseStatus = a})
