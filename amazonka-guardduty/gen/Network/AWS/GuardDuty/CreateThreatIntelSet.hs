...
instance AWSRequest CreateThreatIntelSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateThreatIntelSetResponse' <$> (x .?> "threatIntelSetId") <*>
         (pure (fromEnum s)))
...
ctisrsThreatIntelSetId =
  lens _ctisrsThreatIntelSetId (\s a -> s {_ctisrsThreatIntelSetId = a})

-- | -- | The response status code.
ctisrsResponseStatus :: Lens' CreateThreatIntelSetResponse Int
ctisrsResponseStatus =
  lens _ctisrsResponseStatus (\s a -> s {_ctisrsResponseStatus = a})
