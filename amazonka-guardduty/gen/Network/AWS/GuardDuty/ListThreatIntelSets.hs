...
ltisMaxResults =
  lens _ltisMaxResults (\s a -> s {_ltisMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListThreatIntelSets where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListThreatIntelSetsResponse' <$> (x .?> "threatIntelSetIds" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListThreatIntelSets where
  toQuery ListThreatIntelSets' {..} =
    mconcat ["nextToken" =: _ltisNextToken, "maxResults" =: _ltisMaxResults]
...
ltisrsThreatIntelSetIds =
  lens _ltisrsThreatIntelSetIds (\s a -> s {_ltisrsThreatIntelSetIds = a}) .
  _Default . _Coerce
...
ltisrsResponseStatus =
  lens _ltisrsResponseStatus (\s a -> s {_ltisrsResponseStatus = a})
