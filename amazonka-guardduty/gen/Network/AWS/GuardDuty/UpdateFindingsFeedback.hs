...
uffFindingIds =
  lens _uffFindingIds (\s a -> s {_uffFindingIds = a}) . _Default . _Coerce
...
instance AWSRequest UpdateFindingsFeedback where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateFindingsFeedbackResponse' <$> (pure (fromEnum s)))
...
uffrsResponseStatus =
  lens _uffrsResponseStatus (\s a -> s {_uffrsResponseStatus = a})
