...
instance AWSRequest GetFilter where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetFilterResponse' <$> (x .?> "findingCriteria") <*> (x .?> "action") <*>
         (x .?> "name") <*>
         (x .?> "description") <*>
         (x .?> "rank") <*>
         (pure (fromEnum s)))
...
instance ToPath GetFilter where
  toPath GetFilter' {..} =
    mconcat ["/detector/", toBS _gDetectorId, "/filter/", toBS _gFilterName]
...
gfrsFindingCriteria =
  lens _gfrsFindingCriteria (\s a -> s {_gfrsFindingCriteria = a})
