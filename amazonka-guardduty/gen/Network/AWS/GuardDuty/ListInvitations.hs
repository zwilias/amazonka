...
instance AWSRequest ListInvitations where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListInvitationsResponse' <$> (x .?> "invitations" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListInvitations where
  toQuery ListInvitations' {..} =
    mconcat ["nextToken" =: _liNextToken, "maxResults" =: _liMaxResults]
...
lirsInvitations =
  lens _lirsInvitations (\s a -> s {_lirsInvitations = a}) . _Default . _Coerce
