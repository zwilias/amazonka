...
diAccountIds =
  lens _diAccountIds (\s a -> s {_diAccountIds = a}) . _Default . _Coerce
...
instance AWSRequest DeleteInvitations where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteInvitationsResponse' <$> (x .?> "unprocessedAccounts" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteInvitations where
  toJSON DeleteInvitations' {..} =
    object (catMaybes [("accountIds" .=) <$> _diAccountIds])
...
dirsUnprocessedAccounts =
  lens _dirsUnprocessedAccounts (\s a -> s {_dirsUnprocessedAccounts = a}) .
  _Default . _Coerce
