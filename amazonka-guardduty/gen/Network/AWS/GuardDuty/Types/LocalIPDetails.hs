...
newtype LocalIPDetails = LocalIPDetails'{_lidIPAddressV4
                                         :: Maybe Text}
...
instance FromJSON LocalIPDetails where
        parseJSON
          = withObject "LocalIPDetails"
              (\ x -> LocalIPDetails' <$> (x .:? "ipAddressV4"))

instance Hashable LocalIPDetails where

instance NFData LocalIPDetails where
