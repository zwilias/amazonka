...
data Destination = Destination'{_dDestinationId ::
                                !Text,
...
destination
    :: Text -- ^ 'dDestinationId'
    -> DestinationType -- ^ 'dDestinationType'
    -> PublishingStatus -- ^ 'dStatus'
    -> Destination
destination pDestinationId_ pDestinationType_
  pStatus_
  = Destination'{_dDestinationId = pDestinationId_,
...
instance FromJSON Destination where
        parseJSON
          = withObject "Destination"
              (\ x ->
                 Destination' <$>
                   (x .: "destinationId") <*> (x .: "destinationType")
                     <*> (x .: "status"))
