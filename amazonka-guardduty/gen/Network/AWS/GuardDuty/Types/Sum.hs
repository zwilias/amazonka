...
instance FromText DetectorStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DetectorStatus from value: '" <>
...
instance FromText Feedback where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Feedback from value: '" <>
        e <> "'. Accepted values: not_useful, useful"
...
instance FromText FilterAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FilterAction from value: '" <>
        e <> "'. Accepted values: archive, noop"
...
instance FromText FindingStatisticType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FindingStatisticType from value: '" <>
        e <> "'. Accepted values: count_by_severity"
...
instance FromText IPSetFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IPSetFormat from value: '" <>
        e <>
...
instance FromText IPSetStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IPSetStatus from value: '" <>
        e <>
...
instance FromText OrderBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OrderBy from value: '" <>
...
instance FromText ThreatIntelSetFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ThreatIntelSetFormat from value: '" <>
        e <>
        "'. Accepted values: alien_vault, fire_eye, otx_csv, proof_point, stix, txt"
...
instance FromText ThreatIntelSetStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ThreatIntelSetStatus from value: '" <>
        e <>
        "'. Accepted values: activating, active, deactivating, delete_pending, deleted, error, inactive"
