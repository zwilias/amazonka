...
data AdminAccount = AdminAccount'{_aaAdminAccountId
                                  :: !(Maybe Text),
...
adminAccount
    :: AdminAccount
adminAccount
  = AdminAccount'{_aaAdminAccountId = Nothing,
...
instance FromJSON AdminAccount where
        parseJSON
          = withObject "AdminAccount"
              (\ x ->
                 AdminAccount' <$>
                   (x .:? "adminAccountId") <*> (x .:? "adminStatus"))

instance Hashable AdminAccount where

instance NFData AdminAccount where
