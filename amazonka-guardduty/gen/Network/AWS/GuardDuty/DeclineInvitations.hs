...
dAccountIds =
  lens _dAccountIds (\s a -> s {_dAccountIds = a}) . _Default . _Coerce
...
instance ToJSON DeclineInvitations where
  toJSON DeclineInvitations' {..} =
    object (catMaybes [("accountIds" .=) <$> _dAccountIds])
...
disrsUnprocessedAccounts ::
     Lens' DeclineInvitationsResponse [UnprocessedAccount]
disrsUnprocessedAccounts =
  lens _disrsUnprocessedAccounts (\s a -> s {_disrsUnprocessedAccounts = a}) .
  _Default . _Coerce
