...
icCertificateChain =
  lens _icCertificateChain (\s a -> s {_icCertificateChain = a}) .
  mapping _Base64
...
icPrivateKey =
  lens _icPrivateKey (\s a -> s {_icPrivateKey = a}) . _Sensitive . _Base64
...
instance AWSRequest ImportCertificate where
  ...
  response =
    receiveJSON
      (\s h x ->
         ImportCertificateResponse' <$> (x .?> "CertificateArn") <*>
         (pure (fromEnum s)))
