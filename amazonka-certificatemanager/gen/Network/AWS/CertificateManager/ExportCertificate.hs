...
ecPassphrase =
  lens _ecPassphrase (\s a -> s {_ecPassphrase = a}) . _Sensitive . _Base64
...
instance AWSRequest ExportCertificate where
  ...
  response =
    receiveJSON
      (\s h x ->
         ExportCertificateResponse' <$> (x .?> "PrivateKey") <*>
         (x .?> "Certificate") <*>
         (x .?> "CertificateChain") <*>
         (pure (fromEnum s)))
...
ecrsPrivateKey =
  lens _ecrsPrivateKey (\s a -> s {_ecrsPrivateKey = a}) . mapping _Sensitive
...
ecrsCertificateChain =
  lens _ecrsCertificateChain (\s a -> s {_ecrsCertificateChain = a})

-- | -- | The response status code.
ecrsResponseStatus :: Lens' ExportCertificateResponse Int
ecrsResponseStatus =
  lens _ecrsResponseStatus (\s a -> s {_ecrsResponseStatus = a})
