...
ltfcCertificateARN =
  lens _ltfcCertificateARN (\s a -> s {_ltfcCertificateARN = a})
...
instance AWSRequest ListTagsForCertificate where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListTagsForCertificateResponse' <$> (x .?> "Tags") <*>
         (pure (fromEnum s)))
...
instance ToJSON ListTagsForCertificate where
  toJSON ListTagsForCertificate' {..} =
    object (catMaybes [Just ("CertificateArn" .= _ltfcCertificateARN)])
...
ltfcrsResponseStatus =
  lens _ltfcrsResponseStatus (\s a -> s {_ltfcrsResponseStatus = a})
