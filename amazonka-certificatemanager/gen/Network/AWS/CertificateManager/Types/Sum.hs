...
instance FromText CertificateTransparencyLoggingPreference where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CertificateTransparencyLoggingPreference from value: '" <>
...
instance FromText CertificateType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CertificateType from value: '" <>
        e <> "'. Accepted values: amazon_issued, imported, private"
...
instance FromText DomainStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DomainStatus from value: '" <>
...
instance FromText ExtendedKeyUsageName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ExtendedKeyUsageName from value: '" <>
        e <>
...
instance FromText FailureReason where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FailureReason from value: '" <>
        e <>
...
instance FromText KeyAlgorithm where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing KeyAlgorithm from value: '" <>
        e <>
...
instance FromText KeyUsageName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing KeyUsageName from value: '" <>
        e <>
...
instance FromText RenewalEligibility where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RenewalEligibility from value: '" <>
        e <> "'. Accepted values: eligible, ineligible"
...
instance FromText RenewalStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RenewalStatus from value: '" <>
        e <>
...
instance FromText RevocationReason where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RevocationReason from value: '" <>
        e <>
...
instance FromText ValidationMethod where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ValidationMethod from value: '" <>
        e <> "'. Accepted values: dns, email"
