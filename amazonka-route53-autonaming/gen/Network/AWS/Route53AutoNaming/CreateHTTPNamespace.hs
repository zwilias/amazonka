...
--
--
-- For the current limit on the number of namespaces that you can create using the same AWS account, see <https://docs.aws.amazon.com/cloud-map/latest/dg/cloud-map-limits.html AWS Cloud Map Limits> in the /AWS Cloud Map Developer Guide/ .
--
module Network.AWS.Route53AutoNaming.CreateHTTPNamespace
    (
    -- * Creating a Request
...
createHTTPNamespace pName_
  = CreateHTTPNamespace'{_chttpnCreatorRequestId =
                           Nothing,
                         _chttpnDescription = Nothing, _chttpnName = pName_}
