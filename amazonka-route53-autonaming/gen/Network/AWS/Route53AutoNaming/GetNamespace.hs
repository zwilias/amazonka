...
instance AWSRequest GetNamespace where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetNamespaceResponse' <$> (x .?> "Namespace") <*> (pure (fromEnum s)))
...
instance ToJSON GetNamespace where
  toJSON GetNamespace' {..} = object (catMaybes [Just ("Id" .= _gnId)])
...
gnrsResponseStatus =
  lens _gnrsResponseStatus (\s a -> s {_gnrsResponseStatus = a})
