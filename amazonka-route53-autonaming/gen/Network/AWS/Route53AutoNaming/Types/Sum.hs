...
instance FromText CustomHealthStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CustomHealthStatus from value: '" <>
...
instance FromText FilterCondition where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FilterCondition from value: '" <>
        e <> "'. Accepted values: between, eq, in"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: http, https, tcp"
...
instance FromText NamespaceFilterName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NamespaceFilterName from value: '" <>
        e <> "'. Accepted values: type"
...
instance FromText NamespaceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NamespaceType from value: '" <>
        e <> "'. Accepted values: dns_private, dns_public"
...
instance FromText OperationFilterName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OperationFilterName from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: fail, pending, submitted, success"
...
instance FromText OperationTargetType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OperationTargetType from value: '" <>
        e <> "'. Accepted values: instance, namespace, service"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: a, aaaa, cname, srv"
...
instance FromText RoutingPolicy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RoutingPolicy from value: '" <>
        e <> "'. Accepted values: multivalue, weighted"
...
instance FromText ServiceFilterName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ServiceFilterName from value: '" <>
        e <> "'. Accepted values: namespace_id"
