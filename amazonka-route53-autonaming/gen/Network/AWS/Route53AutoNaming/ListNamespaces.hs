...
instance AWSRequest ListNamespaces where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListNamespacesResponse' <$> (x .?> "Namespaces" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
lnrsNamespaces =
  lens _lnrsNamespaces (\s a -> s {_lnrsNamespaces = a}) . _Default . _Coerce
...
lnrsResponseStatus =
  lens _lnrsResponseStatus (\s a -> s {_lnrsResponseStatus = a})
