...
gihsInstances =
  lens _gihsInstances (\s a -> s {_gihsInstances = a}) . mapping _List1
...
gihsMaxResults =
  lens _gihsMaxResults (\s a -> s {_gihsMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetInstancesHealthStatus where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetInstancesHealthStatusResponse' <$> (x .?> "Status" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetInstancesHealthStatus where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("Route53AutoNaming_v20170314.GetInstancesHealthStatus" :: ByteString)
...
gihsrsStatus ::
     Lens' GetInstancesHealthStatusResponse (HashMap Text HealthStatus)
gihsrsStatus =
  lens _gihsrsStatus (\s a -> s {_gihsrsStatus = a}) . _Default . _Map
...
gihsrsResponseStatus =
  lens _gihsrsResponseStatus (\s a -> s {_gihsrsResponseStatus = a})
