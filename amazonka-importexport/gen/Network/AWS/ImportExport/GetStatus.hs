...
instance AWSRequest GetStatus where
  ...
  response =
    receiveXMLWrapper
      "GetStatusResult"
      (\s h x ->
         GetStatusResponse' <$> (x .@? "Carrier") <*> (x .@? "TrackingNumber") <*>
         (x .@? "Signature") <*>
         (x .@? "JobType") <*>
         (x .@? "JobId") <*>
...
gsrsSignatureFileContents =
  lens _gsrsSignatureFileContents (\s a -> s {_gsrsSignatureFileContents = a})
...
gsrsArtifactList =
  lens _gsrsArtifactList (\s a -> s {_gsrsArtifactList = a}) .
  _Default . _Coerce
...
gsrsCreationDate =
  lens _gsrsCreationDate (\s a -> s {_gsrsCreationDate = a}) . mapping _Time
