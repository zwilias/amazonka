...
plehesStatus ::
     Lens' PutLifecycleEventHookExecutionStatus (Maybe LifecycleEventStatus)
...
plehesDeploymentId =
  lens _plehesDeploymentId (\s a -> s {_plehesDeploymentId = a})

-- | The execution ID of a deployment's lifecycle hook. A deployment lifecycle hook is specified in the @hooks@ section of the AppSpec file.
plehesLifecycleEventHookExecutionId ::
     Lens' PutLifecycleEventHookExecutionStatus (Maybe Text)
plehesLifecycleEventHookExecutionId =
  lens
    _plehesLifecycleEventHookExecutionId
    (\s a -> s {_plehesLifecycleEventHookExecutionId = a})

instance AWSRequest PutLifecycleEventHookExecutionStatus where
  type Rs PutLifecycleEventHookExecutionStatus = PutLifecycleEventHookExecutionStatusResponse
...
instance ToHeaders PutLifecycleEventHookExecutionStatus where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("CodeDeploy_20141006.PutLifecycleEventHookExecutionStatus" :: ByteString)
...
plehesrsLifecycleEventHookExecutionId ::
     Lens' PutLifecycleEventHookExecutionStatusResponse (Maybe Text)
plehesrsLifecycleEventHookExecutionId =
  lens
    _plehesrsLifecycleEventHookExecutionId
    (\s a -> s {_plehesrsLifecycleEventHookExecutionId = a})

-- | -- | The response status code.
plehesrsResponseStatus :: Lens' PutLifecycleEventHookExecutionStatusResponse Int
plehesrsResponseStatus =
  lens _plehesrsResponseStatus (\s a -> s {_plehesrsResponseStatus = a})
