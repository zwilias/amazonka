...
cdcComputePlatform =
  lens _cdcComputePlatform (\s a -> s {_cdcComputePlatform = a})
...
cdcMinimumHealthyHosts ::
     Lens' CreateDeploymentConfig (Maybe MinimumHealthyHosts)
cdcMinimumHealthyHosts =
...
cdcTrafficRoutingConfig ::
     Lens' CreateDeploymentConfig (Maybe TrafficRoutingConfig)
cdcTrafficRoutingConfig =
  lens _cdcTrafficRoutingConfig (\s a -> s {_cdcTrafficRoutingConfig = a})
...
instance AWSRequest CreateDeploymentConfig where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateDeploymentConfigResponse' <$> (x .?> "deploymentConfigId") <*>
         (pure (fromEnum s)))
...
cdcrsDeploymentConfigId =
  lens _cdcrsDeploymentConfigId (\s a -> s {_cdcrsDeploymentConfigId = a})
