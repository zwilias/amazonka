...
instance AWSRequest GetDeploymentConfig where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDeploymentConfigResponse' <$> (x .?> "deploymentConfigInfo") <*>
         (pure (fromEnum s)))
...
instance ToJSON GetDeploymentConfig where
  toJSON GetDeploymentConfig' {..} =
    object
      (catMaybes [Just ("deploymentConfigName" .= _gdcDeploymentConfigName)])
...
gdcrsDeploymentConfigInfo ::
     Lens' GetDeploymentConfigResponse (Maybe DeploymentConfigInfo)
gdcrsDeploymentConfigInfo =
  lens _gdcrsDeploymentConfigInfo (\s a -> s {_gdcrsDeploymentConfigInfo = a})
