...
instance AWSRequest GetDeploymentGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDeploymentGroupResponse' <$> (x .?> "deploymentGroupInfo") <*>
         (pure (fromEnum s)))
...
gdgrsDeploymentGroupInfo ::
     Lens' GetDeploymentGroupResponse (Maybe DeploymentGroupInfo)
gdgrsDeploymentGroupInfo =
  lens _gdgrsDeploymentGroupInfo (\s a -> s {_gdgrsDeploymentGroupInfo = a})

-- | -- | The response status code.
gdgrsResponseStatus :: Lens' GetDeploymentGroupResponse Int
gdgrsResponseStatus =
  lens _gdgrsResponseStatus (\s a -> s {_gdgrsResponseStatus = a})
