...
bgdDeploymentIds =
  lens _bgdDeploymentIds (\s a -> s {_bgdDeploymentIds = a}) . _Coerce
...
instance AWSRequest BatchGetDeployments where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchGetDeploymentsResponse' <$> (x .?> "deploymentsInfo" .!@ mempty) <*>
         (pure (fromEnum s)))
...
bgdrsDeploymentsInfo =
  lens _bgdrsDeploymentsInfo (\s a -> s {_bgdrsDeploymentsInfo = a}) .
  _Default . _Coerce

-- | -- | The response status code.
bgdrsResponseStatus :: Lens' BatchGetDeploymentsResponse Int
bgdrsResponseStatus =
  lens _bgdrsResponseStatus (\s a -> s {_bgdrsResponseStatus = a})
