...
instance AWSRequest ListDeploymentConfigs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDeploymentConfigsResponse' <$> (x .?> "nextToken") <*>
...
ldcrsDeploymentConfigsList =
  lens _ldcrsDeploymentConfigsList (\s a -> s {_ldcrsDeploymentConfigsList = a}) .
  _Default . _Coerce

-- | -- | The response status code.
ldcrsResponseStatus :: Lens' ListDeploymentConfigsResponse Int
ldcrsResponseStatus =
  lens _ldcrsResponseStatus (\s a -> s {_ldcrsResponseStatus = a})
