...
data DeployErrorCode = AgentIssue
                     | AlarmActive
                     | ApplicationMissing
...
instance FromText DeployErrorCode where
    parser = takeLowerText >>= \case
        ...
        e -> fromTextError $ "Failure parsing DeployErrorCode from value: '" <> e
           <> "'. Accepted values: agent_issue, alarm_active, application_missing, auto_scaling_configuration, auto_scaling_iam_role_permissions, autoscaling_validation_error, codedeploy_resource_cannot_be_found, customer_application_unhealthy, deployment_group_missing, ecs_update_error, elastic_load_balancing_invalid, elb_invalid_instance, health_constraints, health_constraints_invalid, hook_execution_failure, iam_role_missing, iam_role_permissions, internal_error, invalid_ecs_service, invalid_lambda_configuration, invalid_lambda_function, invalid_revision, manual_stop, missing_blue_green_deployment_configuration, missing_elb_information, missing_github_token, no_ec2_subscription, no_instances, over_max_instances, resource_limit_exceeded, revision_missing, throttled, timeout"
