...
instance FromText ApplicationRevisionSortBy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ApplicationRevisionSortBy from value: '" <>
        e <> "'. Accepted values: firstusedtime, lastusedtime, registertime"
...
instance FromText AutoRollbackEvent where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AutoRollbackEvent from value: '" <>
        e <>
...
instance FromText BundleType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BundleType from value: '" <>
        e <> "'. Accepted values: json, tar, tgz, yaml, zip"
...
instance FromText ComputePlatform where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ComputePlatform from value: '" <>
        e <> "'. Accepted values: lambda, server"
  ...
  parser =
    takeLowerText >>= \case
      ...
      "missing_blue_green_deployment_configuration" ->
        pure MissingBlueGreenDeploymentConfiguration
      ...
      e ->
        fromTextError $
        "Failure parsing DeployErrorCode from value: '" <>
        e <>
...
instance FromText DeploymentCreator where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeploymentCreator from value: '" <>
        e <> "'. Accepted values: autoscaling, codedeployrollback, user"
...
instance FromText DeploymentOption where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeploymentOption from value: '" <>
        e <> "'. Accepted values: with_traffic_control, without_traffic_control"
...
instance FromText DeploymentReadyAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeploymentReadyAction from value: '" <>
        e <> "'. Accepted values: continue_deployment, stop_deployment"
...
instance FromText DeploymentStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeploymentStatus from value: '" <>
        e <>
...
instance FromText DeploymentType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeploymentType from value: '" <>
        e <> "'. Accepted values: blue_green, in_place"
...
instance FromText EC2TagFilterType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EC2TagFilterType from value: '" <>
        e <> "'. Accepted values: key_and_value, key_only, value_only"
...
instance FromText FileExistsBehavior where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FileExistsBehavior from value: '" <>
        e <> "'. Accepted values: disallow, overwrite, retain"
...
instance FromText GreenFleetProvisioningAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing GreenFleetProvisioningAction from value: '" <>
        e <> "'. Accepted values: copy_auto_scaling_group, discover_existing"
...
instance FromText InstanceAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing InstanceAction from value: '" <>
        e <> "'. Accepted values: keep_alive, terminate"
...
instance FromText InstanceStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing InstanceStatus from value: '" <>
        e <>
...
instance FromText LifecycleErrorCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LifecycleErrorCode from value: '" <>
        e <>
...
instance FromText LifecycleEventStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LifecycleEventStatus from value: '" <>
        e <>
...
instance FromText ListStateFilterAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ListStateFilterAction from value: '" <>
        e <> "'. Accepted values: exclude, ignore, include"
...
instance FromText MinimumHealthyHostsType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MinimumHealthyHostsType from value: '" <>
        e <> "'. Accepted values: fleet_percent, host_count"
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: deregistered, registered"
...
instance FromText RevisionLocationType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RevisionLocationType from value: '" <>
        e <> "'. Accepted values: github, s3, string"
...
instance FromText SortOrder where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SortOrder from value: '" <>
...
instance FromText StopStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing StopStatus from value: '" <>
        e <> "'. Accepted values: pending, succeeded"
...
instance FromText TagFilterType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TagFilterType from value: '" <>
        e <> "'. Accepted values: key_and_value, key_only, value_only"
...
instance FromText TrafficRoutingType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TrafficRoutingType from value: '" <>
        e <> "'. Accepted values: allatonce, timebasedcanary, timebasedlinear"
...
instance FromText TriggerEventType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TriggerEventType from value: '" <>
        e <>
