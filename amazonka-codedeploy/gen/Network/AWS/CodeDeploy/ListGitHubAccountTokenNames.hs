...
instance AWSRequest ListGitHubAccountTokenNames where
  type Rs ListGitHubAccountTokenNames = ListGitHubAccountTokenNamesResponse
...
lghatnrsTokenNameList =
  lens _lghatnrsTokenNameList (\s a -> s {_lghatnrsTokenNameList = a}) .
  _Default . _Coerce
...
lghatnrsResponseStatus =
  lens _lghatnrsResponseStatus (\s a -> s {_lghatnrsResponseStatus = a})
