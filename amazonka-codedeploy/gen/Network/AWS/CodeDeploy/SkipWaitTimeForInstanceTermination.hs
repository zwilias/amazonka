...
swtfitDeploymentId =
  lens _swtfitDeploymentId (\s a -> s {_swtfitDeploymentId = a})

instance AWSRequest SkipWaitTimeForInstanceTermination where
  type Rs SkipWaitTimeForInstanceTermination = SkipWaitTimeForInstanceTerminationResponse
  request = postJSON codeDeploy
  response = receiveNull SkipWaitTimeForInstanceTerminationResponse'

instance Hashable SkipWaitTimeForInstanceTermination

instance NFData SkipWaitTimeForInstanceTermination

instance ToHeaders SkipWaitTimeForInstanceTermination where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("CodeDeploy_20141006.SkipWaitTimeForInstanceTermination" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON SkipWaitTimeForInstanceTermination where
  toJSON SkipWaitTimeForInstanceTermination' {..} =
    object (catMaybes [("deploymentId" .=) <$> _swtfitDeploymentId])

instance ToPath SkipWaitTimeForInstanceTermination where
  toPath = const "/"

instance ToQuery SkipWaitTimeForInstanceTermination where
...
instance NFData SkipWaitTimeForInstanceTerminationResponse
