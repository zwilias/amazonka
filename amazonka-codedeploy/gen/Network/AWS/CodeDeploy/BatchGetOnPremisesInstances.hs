...
bgopiInstanceNames =
  lens _bgopiInstanceNames (\s a -> s {_bgopiInstanceNames = a}) . _Coerce
...
instance ToJSON BatchGetOnPremisesInstances where
  toJSON BatchGetOnPremisesInstances' {..} =
    object (catMaybes [Just ("instanceNames" .= _bgopiInstanceNames)])
...
bgopirsInstanceInfos =
  lens _bgopirsInstanceInfos (\s a -> s {_bgopirsInstanceInfos = a}) .
  _Default . _Coerce

-- | -- | The response status code.
bgopirsResponseStatus :: Lens' BatchGetOnPremisesInstancesResponse Int
bgopirsResponseStatus =
  lens _bgopirsResponseStatus (\s a -> s {_bgopirsResponseStatus = a})
