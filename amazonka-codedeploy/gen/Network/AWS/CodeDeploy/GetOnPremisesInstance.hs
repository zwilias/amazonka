...
instance AWSRequest GetOnPremisesInstance where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetOnPremisesInstanceResponse' <$> (x .?> "instanceInfo") <*>
         (pure (fromEnum s)))
...
gopirsInstanceInfo =
  lens _gopirsInstanceInfo (\s a -> s {_gopirsInstanceInfo = a})

-- | -- | The response status code.
gopirsResponseStatus :: Lens' GetOnPremisesInstanceResponse Int
gopirsResponseStatus =
  lens _gopirsResponseStatus (\s a -> s {_gopirsResponseStatus = a})
