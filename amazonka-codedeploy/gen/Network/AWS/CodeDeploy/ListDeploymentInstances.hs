...
lInstanceStatusFilter =
  lens _lInstanceStatusFilter (\s a -> s {_lInstanceStatusFilter = a}) .
  _Default . _Coerce
...
lInstanceTypeFilter =
  lens _lInstanceTypeFilter (\s a -> s {_lInstanceTypeFilter = a}) .
  _Default . _Coerce
...
instance AWSPager ListDeploymentInstances where
  page rq rs
    ...
    | otherwise = Just $ rq & lNextToken .~ rs ^. ldirsNextToken
...
instance AWSRequest ListDeploymentInstances where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDeploymentInstancesResponse' <$> (x .?> "nextToken") <*>
...
instance ToHeaders ListDeploymentInstances where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("CodeDeploy_20141006.ListDeploymentInstances" :: ByteString)
...
instance ToJSON ListDeploymentInstances where
  toJSON ListDeploymentInstances' {..} =
    object
      (catMaybes
         [ ("instanceStatusFilter" .=) <$> _lInstanceStatusFilter
...
ldirsInstancesList =
  lens _ldirsInstancesList (\s a -> s {_ldirsInstancesList = a}) .
  _Default . _Coerce

-- | -- | The response status code.
ldirsResponseStatus :: Lens' ListDeploymentInstancesResponse Int
ldirsResponseStatus =
  lens _ldirsResponseStatus (\s a -> s {_ldirsResponseStatus = a})
