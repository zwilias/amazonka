...
bdcUseEarliestTimeOnPointInTimeUnavailable ::
     Lens' BacktrackDBCluster (Maybe Bool)
bdcUseEarliestTimeOnPointInTimeUnavailable =
  lens
    _bdcUseEarliestTimeOnPointInTimeUnavailable
    (\s a -> s {_bdcUseEarliestTimeOnPointInTimeUnavailable = a})
...
bdcDBClusterIdentifier =
  lens _bdcDBClusterIdentifier (\s a -> s {_bdcDBClusterIdentifier = a})
...
instance AWSRequest BacktrackDBCluster where
  ...
  response = receiveXMLWrapper "BacktrackDBClusterResult" (\s h x -> parseXML x)
