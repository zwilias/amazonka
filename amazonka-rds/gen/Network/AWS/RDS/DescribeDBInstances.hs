...
ddbiFilters =
  lens _ddbiFilters (\s a -> s {_ddbiFilters = a}) . _Default . _Coerce
...
ddbiDBInstanceIdentifier =
  lens _ddbiDBInstanceIdentifier (\s a -> s {_ddbiDBInstanceIdentifier = a})
...
instance AWSRequest DescribeDBInstances where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeDBInstancesResponse' <$>
         (x .@? "DBInstances" .!@ mempty >>= may (parseXMLList "DBInstance")) <*>
         (x .@? "Marker") <*>
         (pure (fromEnum s)))
...
ddbirsDBInstances =
  lens _ddbirsDBInstances (\s a -> s {_ddbirsDBInstances = a}) .
  _Default . _Coerce
...
ddbirsResponseStatus =
  lens _ddbirsResponseStatus (\s a -> s {_ddbirsResponseStatus = a})
