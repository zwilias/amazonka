...
data DescribeDBProxyTargets = DescribeDBProxyTargets'{_ddptFilters
                                                      :: !(Maybe [Filter]),
                                                      _ddptMarker ::
                                                      !(Maybe Text),
                                                      _ddptMaxRecords ::
...
describeDBProxyTargets
    :: Text -- ^ 'ddptDBProxyName'
    -> DescribeDBProxyTargets
describeDBProxyTargets pDBProxyName_
  = DescribeDBProxyTargets'{_ddptFilters = Nothing,
                            _ddptMarker = Nothing, _ddptMaxRecords = Nothing,
...
instance AWSPager DescribeDBProxyTargets where
        page rq rs
          ...
          | otherwise =
            Just $ rq & ddptMarker .~ rs ^. ddbptrsMarker

instance AWSRequest DescribeDBProxyTargets where
        type Rs DescribeDBProxyTargets =
             DescribeDBProxyTargetsResponse
        request = postQuery rds
        response
          = receiveXMLWrapper "DescribeDBProxyTargetsResult"
              (\ s h x ->
                 DescribeDBProxyTargetsResponse' <$>
                   (x .@? "Targets" .!@ mempty >>=
...
instance Hashable DescribeDBProxyTargets where

instance NFData DescribeDBProxyTargets where
        ...
        toQuery DescribeDBProxyTargets'{..}
          = mconcat
              ["Action" =:
               ...
               "Filters" =:
                 toQuery (toQueryList "Filter" <$> _ddptFilters),
               "Marker" =: _ddptMarker,
...
data DescribeDBProxyTargetsResponse = DescribeDBProxyTargetsResponse'{_ddbptrsTargets
                                                                      ::
                                                                      !(Maybe
                                                                          [DBProxyTarget]),
                                                                      _ddbptrsMarker
...
describeDBProxyTargetsResponse
    :: Int -- ^ 'ddbptrsResponseStatus'
    -> DescribeDBProxyTargetsResponse
describeDBProxyTargetsResponse pResponseStatus_
  = DescribeDBProxyTargetsResponse'{_ddbptrsTargets =
                                      Nothing,
...
instance NFData DescribeDBProxyTargetsResponse where
