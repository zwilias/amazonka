...
mdsOptionGroupName =
  lens _mdsOptionGroupName (\s a -> s {_mdsOptionGroupName = a})

-- | The identifier of the DB snapshot to modify.
mdsDBSnapshotIdentifier :: Lens' ModifyDBSnapshot Text
mdsDBSnapshotIdentifier =
  lens _mdsDBSnapshotIdentifier (\s a -> s {_mdsDBSnapshotIdentifier = a})
...
instance AWSRequest ModifyDBSnapshot where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ModifyDBSnapshotResponse' <$> (x .@? "DBSnapshot") <*>
         (pure (fromEnum s)))
...
mdsrsResponseStatus =
  lens _mdsrsResponseStatus (\s a -> s {_mdsrsResponseStatus = a})
