...
dedpFilters =
  lens _dedpFilters (\s a -> s {_dedpFilters = a}) . _Default . _Coerce
...
dedpDBParameterGroupFamily =
  lens _dedpDBParameterGroupFamily (\s a -> s {_dedpDBParameterGroupFamily = a})
...
instance AWSPager DescribeEngineDefaultParameters where
  page rq rs
    ...
    | otherwise =
      Just $ rq & dedpMarker .~ rs ^? dedprsEngineDefaults . edMarker . _Just
...
instance AWSRequest DescribeEngineDefaultParameters where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeEngineDefaultParametersResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "EngineDefaults"))
...
dedprsResponseStatus =
  lens _dedprsResponseStatus (\s a -> s {_dedprsResponseStatus = a})

-- | Undocumented member.
dedprsEngineDefaults ::
     Lens' DescribeEngineDefaultParametersResponse EngineDefaults
dedprsEngineDefaults =
