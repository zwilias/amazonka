...
cesSourceIds =
  lens _cesSourceIds (\s a -> s {_cesSourceIds = a}) . _Default . _Coerce
...
instance AWSRequest CreateEventSubscription where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateEventSubscriptionResponse' <$> (x .@? "EventSubscription") <*>
         (pure (fromEnum s)))
...
cesrsResponseStatus =
  lens _cesrsResponseStatus (\s a -> s {_cesrsResponseStatus = a})
