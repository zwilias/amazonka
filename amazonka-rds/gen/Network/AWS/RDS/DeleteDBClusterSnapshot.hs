...
ddcsDBClusterSnapshotIdentifier =
  lens
    ...
    (\s a -> s {_ddcsDBClusterSnapshotIdentifier = a})
...
instance AWSRequest DeleteDBClusterSnapshot where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DeleteDBClusterSnapshotResponse' <$> (x .@? "DBClusterSnapshot") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
ddcsrsDBClusterSnapshot ::
     Lens' DeleteDBClusterSnapshotResponse (Maybe DBClusterSnapshot)
ddcsrsDBClusterSnapshot =
  lens _ddcsrsDBClusterSnapshot (\s a -> s {_ddcsrsDBClusterSnapshot = a})
