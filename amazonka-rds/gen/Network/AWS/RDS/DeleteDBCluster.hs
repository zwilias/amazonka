...
ddbcFinalDBSnapshotIdentifier =
  lens
    ...
    (\s a -> s {_ddbcFinalDBSnapshotIdentifier = a})
...
ddbcSkipFinalSnapshot =
  lens _ddbcSkipFinalSnapshot (\s a -> s {_ddbcSkipFinalSnapshot = a})

-- | The DB cluster identifier for the DB cluster to be deleted. This parameter isn't case-sensitive. Constraints:     * Must match an existing DBClusterIdentifier.
ddbcDBClusterIdentifier :: Lens' DeleteDBCluster Text
ddbcDBClusterIdentifier =
  lens _ddbcDBClusterIdentifier (\s a -> s {_ddbcDBClusterIdentifier = a})
...
instance AWSRequest DeleteDBCluster where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DeleteDBClusterResponse' <$> (x .@? "DBCluster") <*>
         (pure (fromEnum s)))
...
ddbcrsResponseStatus =
  lens _ddbcrsResponseStatus (\s a -> s {_ddbcrsResponseStatus = a})
