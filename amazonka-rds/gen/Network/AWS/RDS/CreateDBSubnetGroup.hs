...
cdbsgDBSubnetGroupName =
  lens _cdbsgDBSubnetGroupName (\s a -> s {_cdbsgDBSubnetGroupName = a})
...
cdbsgDBSubnetGroupDescription =
  lens
    ...
    (\s a -> s {_cdbsgDBSubnetGroupDescription = a})

-- | The EC2 Subnet IDs for the DB subnet group.
cdbsgSubnetIds :: Lens' CreateDBSubnetGroup [Text]
cdbsgSubnetIds =
  lens _cdbsgSubnetIds (\s a -> s {_cdbsgSubnetIds = a}) . _Coerce
...
instance AWSRequest CreateDBSubnetGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateDBSubnetGroupResponse' <$> (x .@? "DBSubnetGroup") <*>
         (pure (fromEnum s)))
...
cdsgrsDBSubnetGroup =
  lens _cdsgrsDBSubnetGroup (\s a -> s {_cdsgrsDBSubnetGroup = a})

-- | -- | The response status code.
cdsgrsResponseStatus :: Lens' CreateDBSubnetGroupResponse Int
cdsgrsResponseStatus =
  lens _cdsgrsResponseStatus (\s a -> s {_cdsgrsResponseStatus = a})
