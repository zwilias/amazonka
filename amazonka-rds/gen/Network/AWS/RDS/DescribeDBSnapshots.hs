...
ddsDBSnapshotIdentifier =
  lens _ddsDBSnapshotIdentifier (\s a -> s {_ddsDBSnapshotIdentifier = a})
...
ddsDBInstanceIdentifier =
  lens _ddsDBInstanceIdentifier (\s a -> s {_ddsDBInstanceIdentifier = a})
...
instance AWSRequest DescribeDBSnapshots where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeDBSnapshotsResponse' <$> (x .@? "Marker") <*>
         (x .@? "DBSnapshots" .!@ mempty >>= may (parseXMLList "DBSnapshot")) <*>
         (pure (fromEnum s)))
...
ddsrsDBSnapshots =
  lens _ddsrsDBSnapshots (\s a -> s {_ddsrsDBSnapshots = a}) .
  _Default . _Coerce
