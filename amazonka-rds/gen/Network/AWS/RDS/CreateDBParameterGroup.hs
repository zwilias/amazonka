...
cdbpgDBParameterGroupName =
  lens _cdbpgDBParameterGroupName (\s a -> s {_cdbpgDBParameterGroupName = a})
...
instance AWSRequest CreateDBParameterGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateDBParameterGroupResponse' <$> (x .@? "DBParameterGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
cdpgrsDBParameterGroup ::
     Lens' CreateDBParameterGroupResponse (Maybe DBParameterGroup)
cdpgrsDBParameterGroup =
  lens _cdpgrsDBParameterGroup (\s a -> s {_cdpgrsDBParameterGroup = a})

-- | -- | The response status code.
cdpgrsResponseStatus :: Lens' CreateDBParameterGroupResponse Int
cdpgrsResponseStatus =
  lens _cdpgrsResponseStatus (\s a -> s {_cdpgrsResponseStatus = a})
