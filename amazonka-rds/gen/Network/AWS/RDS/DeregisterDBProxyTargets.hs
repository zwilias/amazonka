...
data DeregisterDBProxyTargets = DeregisterDBProxyTargets'{_ddbptDBClusterIdentifiers
                                                          :: !(Maybe [Text]),
...
deregisterDBProxyTargets
    :: Text -- ^ 'ddbptDBProxyName'
    -> DeregisterDBProxyTargets
deregisterDBProxyTargets pDBProxyName_
  = DeregisterDBProxyTargets'{_ddbptDBClusterIdentifiers
                                = Nothing,
...
instance AWSRequest DeregisterDBProxyTargets where
        type Rs DeregisterDBProxyTargets =
             DeregisterDBProxyTargetsResponse
        request = postQuery rds
        response
          = receiveXMLWrapper "DeregisterDBProxyTargetsResult"
              (\ s h x ->
                 DeregisterDBProxyTargetsResponse' <$>
                   (pure (fromEnum s)))

instance Hashable DeregisterDBProxyTargets where

instance NFData DeregisterDBProxyTargets where
...
newtype DeregisterDBProxyTargetsResponse = DeregisterDBProxyTargetsResponse'{_ddptrsResponseStatus
                                                                             ::
                                                                             Int}
...
deregisterDBProxyTargetsResponse
    :: Int -- ^ 'ddptrsResponseStatus'
    -> DeregisterDBProxyTargetsResponse
deregisterDBProxyTargetsResponse pResponseStatus_
  = DeregisterDBProxyTargetsResponse'{_ddptrsResponseStatus
                                        = pResponseStatus_}
