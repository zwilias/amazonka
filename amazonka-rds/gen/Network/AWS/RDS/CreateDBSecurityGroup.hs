...
cdsgDBSecurityGroupName =
  lens _cdsgDBSecurityGroupName (\s a -> s {_cdsgDBSecurityGroupName = a})
...
cdsgDBSecurityGroupDescription =
  lens
    ...
    (\s a -> s {_cdsgDBSecurityGroupDescription = a})
...
instance AWSRequest CreateDBSecurityGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         CreateDBSecurityGroupResponse' <$> (x .@? "DBSecurityGroup") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
cdbsgrsDBSecurityGroup ::
     Lens' CreateDBSecurityGroupResponse (Maybe DBSecurityGroup)
cdbsgrsDBSecurityGroup =
...
cdbsgrsResponseStatus =
  lens _cdbsgrsResponseStatus (\s a -> s {_cdbsgrsResponseStatus = a})
