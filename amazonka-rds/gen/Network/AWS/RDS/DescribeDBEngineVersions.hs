...
ddevListSupportedTimezones =
  lens _ddevListSupportedTimezones (\s a -> s {_ddevListSupportedTimezones = a})
...
ddevFilters =
  lens _ddevFilters (\s a -> s {_ddevFilters = a}) . _Default . _Coerce
...
ddevDBParameterGroupFamily =
  lens _ddevDBParameterGroupFamily (\s a -> s {_ddevDBParameterGroupFamily = a})
...
ddevListSupportedCharacterSets =
  lens
    ...
    (\s a -> s {_ddevListSupportedCharacterSets = a})
...
instance AWSRequest DescribeDBEngineVersions where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeDBEngineVersionsResponse' <$> (x .@? "Marker") <*>
...
ddevrsDBEngineVersions ::
     Lens' DescribeDBEngineVersionsResponse [DBEngineVersion]
ddevrsDBEngineVersions =
  lens _ddevrsDBEngineVersions (\s a -> s {_ddevrsDBEngineVersions = a}) .
  _Default . _Coerce
