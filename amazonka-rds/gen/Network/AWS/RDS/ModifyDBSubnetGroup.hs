...
mdsgDBSubnetGroupDescription =
  lens
    ...
    (\s a -> s {_mdsgDBSubnetGroupDescription = a})
...
mdsgDBSubnetGroupName =
  lens _mdsgDBSubnetGroupName (\s a -> s {_mdsgDBSubnetGroupName = a})
...
instance AWSRequest ModifyDBSubnetGroup where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         ModifyDBSubnetGroupResponse' <$> (x .@? "DBSubnetGroup") <*>
         (pure (fromEnum s)))
...
mdsgrsDBSubnetGroup =
  lens _mdsgrsDBSubnetGroup (\s a -> s {_mdsgrsDBSubnetGroup = a})
