...
data VPNDetails = VPNDetails'{_vdVPNName ::
                              !(Maybe Text),
...
vpnDetails
    :: VPNDetails
vpnDetails
  = VPNDetails'{_vdVPNName = Nothing,
                _vdVPNTunnelOriginatorIP = Nothing,
                _vdVPNId = Nothing, _vdVPNState = Nothing,
                _vdVPNPSK = Nothing, _vdVPNGatewayIP = Nothing}
...
instance FromXML VPNDetails where
        parseXML x
          = VPNDetails' <$>
              (x .@? "VpnName") <*> (x .@? "VpnTunnelOriginatorIP")
                <*> (x .@? "VpnId")
                <*> (x .@? "VpnState")
                <*> (x .@? "VpnPSK")
                <*> (x .@? "VpnGatewayIp")

instance Hashable VPNDetails where

instance NFData VPNDetails where
