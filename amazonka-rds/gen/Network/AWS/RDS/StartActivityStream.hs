...
data StartActivityStream = StartActivityStream'{_sApplyImmediately
                                                :: !(Maybe Bool),
                                                _sResourceARN :: !Text,
...
startActivityStream
    :: Text -- ^ 'sResourceARN'
    -> ActivityStreamMode -- ^ 'sMode'
    -> Text -- ^ 'sKMSKeyId'
    -> StartActivityStream
startActivityStream pResourceARN_ pMode_ pKMSKeyId_
  = StartActivityStream'{_sApplyImmediately = Nothing,
                         _sResourceARN = pResourceARN_, _sMode = pMode_,
...
instance AWSRequest StartActivityStream where
        type Rs StartActivityStream =
             StartActivityStreamResponse
        request = postQuery rds
        response
          = receiveXMLWrapper "StartActivityStreamResult"
              (\ s h x ->
                 StartActivityStreamResponse' <$>
                   (x .@? "Status") <*> (x .@? "KinesisStreamName") <*>
                     (x .@? "Mode")
                     <*> (x .@? "KmsKeyId")
                     <*> (x .@? "ApplyImmediately")
                     <*> (pure (fromEnum s)))

instance Hashable StartActivityStream where

instance NFData StartActivityStream where
        ...
        toQuery StartActivityStream'{..}
          = mconcat
              ["Action" =: ("StartActivityStream" :: ByteString),
               ...
               "ResourceArn" =: _sResourceARN, "Mode" =: _sMode,
               "KmsKeyId" =: _sKMSKeyId]

-- | /See:/ 'startActivityStreamResponse' smart constructor.
data StartActivityStreamResponse = StartActivityStreamResponse'{_srsStatus
                                                                ::
                                                                !(Maybe
...
startActivityStreamResponse pResponseStatus_
  = StartActivityStreamResponse'{_srsStatus = Nothing,
                                 _srsKinesisStreamName = Nothing,
                                 _srsMode = Nothing, _srsKMSKeyId = Nothing,
...
instance NFData StartActivityStreamResponse where
