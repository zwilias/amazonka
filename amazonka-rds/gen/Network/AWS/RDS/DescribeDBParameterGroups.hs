...
ddpgFilters =
  lens _ddpgFilters (\s a -> s {_ddpgFilters = a}) . _Default . _Coerce
...
ddpgDBParameterGroupName =
  lens _ddpgDBParameterGroupName (\s a -> s {_ddpgDBParameterGroupName = a})
...
instance AWSRequest DescribeDBParameterGroups where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         DescribeDBParameterGroupsResponse' <$> (x .@? "Marker") <*>
...
ddpgrsDBParameterGroups ::
     Lens' DescribeDBParameterGroupsResponse [DBParameterGroup]
ddpgrsDBParameterGroups =
  lens _ddpgrsDBParameterGroups (\s a -> s {_ddpgrsDBParameterGroups = a}) .
  _Default . _Coerce

-- | -- | The response status code.
ddpgrsResponseStatus :: Lens' DescribeDBParameterGroupsResponse Int
ddpgrsResponseStatus =
  lens _ddpgrsResponseStatus (\s a -> s {_ddpgrsResponseStatus = a})
