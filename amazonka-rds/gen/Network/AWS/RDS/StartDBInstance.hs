...
sdbiDBInstanceIdentifier =
  lens _sdbiDBInstanceIdentifier (\s a -> s {_sdbiDBInstanceIdentifier = a})
...
instance AWSRequest StartDBInstance where
  ...
  response =
    receiveXMLWrapper
      ...
      (\s h x ->
         StartDBInstanceResponse' <$> (x .@? "DBInstance") <*>
         (pure (fromEnum s)))
...
sdbirsResponseStatus =
  lens _sdbirsResponseStatus (\s a -> s {_sdbirsResponseStatus = a})
