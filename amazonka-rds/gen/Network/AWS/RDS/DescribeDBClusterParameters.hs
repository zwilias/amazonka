...
ddcpFilters =
  lens _ddcpFilters (\s a -> s {_ddcpFilters = a}) . _Default . _Coerce
...
ddcpDBClusterParameterGroupName =
  lens
    ...
    (\s a -> s {_ddcpDBClusterParameterGroupName = a})

instance AWSRequest DescribeDBClusterParameters where
  type Rs DescribeDBClusterParameters = DescribeDBClusterParametersResponse
  request = postQuery rds
  response =
    receiveXMLWrapper
      "DescribeDBClusterParametersResult"
      (\s h x ->
         DescribeDBClusterParametersResponse' <$> (x .@? "Marker") <*>
...
ddcprsParameters =
  lens _ddcprsParameters (\s a -> s {_ddcprsParameters = a}) .
  _Default . _Coerce
