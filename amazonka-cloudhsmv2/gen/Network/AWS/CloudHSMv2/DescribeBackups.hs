...
instance AWSRequest DescribeBackups where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeBackupsResponse' <$> (x .?> "Backups" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeBackups where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("BaldrApiService.DescribeBackups" :: ByteString)
...
dbrsBackups =
  lens _dbrsBackups (\s a -> s {_dbrsBackups = a}) . _Default . _Coerce
