...
bCreateTimestamp =
  lens _bCreateTimestamp (\s a -> s {_bCreateTimestamp = a}) . mapping _Time
...
instance FromJSON Backup where
  parseJSON =
    withObject
      "Backup"
      (\x ->
         Backup' <$> (x .:? "ClusterId") <*> (x .:? "CreateTimestamp") <*>
         (x .:? "BackupState") <*>
         (x .: "BackupId"))
...
cManufacturerHardwareCertificate =
  lens
    ...
    (\s a -> s {_cManufacturerHardwareCertificate = a})
...
cClusterCertificate =
  lens _cClusterCertificate (\s a -> s {_cClusterCertificate = a})

-- | The HSM hardware certificate issued (signed) by AWS CloudHSM.
cAWSHardwareCertificate :: Lens' Certificates (Maybe Text)
cAWSHardwareCertificate =
  lens _cAWSHardwareCertificate (\s a -> s {_cAWSHardwareCertificate = a})
...
instance FromJSON Certificates where
  parseJSON =
    withObject
      "Certificates"
      (\x ->
         Certificates' <$> (x .:? "ManufacturerHardwareCertificate") <*>
         (x .:? "ClusterCsr") <*>
         (x .:? "HsmCertificate") <*>
...
cSubnetMapping =
  lens _cSubnetMapping (\s a -> s {_cSubnetMapping = a}) . _Default . _Map
...
cCreateTimestamp =
  lens _cCreateTimestamp (\s a -> s {_cCreateTimestamp = a}) . mapping _Time
...
instance FromJSON Cluster where
  parseJSON =
    withObject
      "Cluster"
      (\x ->
         Cluster' <$> (x .:? "PreCoPassword") <*> (x .:? "StateMessage") <*>
         (x .:? "State") <*>
...
hsmAvailabilityZone =
  lens _hsmAvailabilityZone (\s a -> s {_hsmAvailabilityZone = a})
...
instance FromJSON HSM where
  parseJSON =
    withObject
      "HSM"
      (\x ->
         HSM' <$> (x .:? "StateMessage") <*> (x .:? "State") <*> (x .:? "EniId") <*>
         (x .:? "SubnetId") <*>
         (x .:? "AvailabilityZone") <*>
         (x .:? "ClusterId") <*>
         (x .:? "EniIp") <*>
