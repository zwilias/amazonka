...
instance FromText BackupPolicy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BackupPolicy from value: '" <>
...
instance FromText BackupState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing BackupState from value: '" <>
        e <> "'. Accepted values: create_in_progress, deleted, ready"
...
instance FromText HSMState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HSMState from value: '" <>
        e <>
