...
cpusProgressUpdateStreamName =
  lens
    ...
    (\s a -> s {_cpusProgressUpdateStreamName = a})

instance AWSRequest CreateProgressUpdateStream where
  type Rs CreateProgressUpdateStream = CreateProgressUpdateStreamResponse
  request = postJSON migrationHub
  response =
    receiveEmpty
      (\s h x -> CreateProgressUpdateStreamResponse' <$> (pure (fromEnum s)))
...
instance ToJSON CreateProgressUpdateStream where
  toJSON CreateProgressUpdateStream' {..} =
    object
      (catMaybes
         [ ("DryRun" .=) <$> _cpusDryRun
         , Just ("ProgressUpdateStreamName" .= _cpusProgressUpdateStreamName)
         ])
...
cpusrsResponseStatus =
  lens _cpusrsResponseStatus (\s a -> s {_cpusrsResponseStatus = a})
