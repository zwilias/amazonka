...
instance FromJSON CreatedArtifact where
  parseJSON =
    withObject
      "CreatedArtifact"
      (\x -> CreatedArtifact' <$> (x .:? "Description") <*> (x .: "Name"))
...
instance FromJSON DiscoveredResource where
  parseJSON =
    withObject
      "DiscoveredResource"
      (\x ->
         DiscoveredResource' <$> (x .:? "Description") <*>
         (x .: "ConfigurationId"))
...
mtUpdateDateTime =
  lens _mtUpdateDateTime (\s a -> s {_mtUpdateDateTime = a}) . mapping _Time

-- |
mtResourceAttributeList :: Lens' MigrationTask [ResourceAttribute]
mtResourceAttributeList =
  lens _mtResourceAttributeList (\s a -> s {_mtResourceAttributeList = a}) .
  _Default . _Coerce
...
mtProgressUpdateStream =
  lens _mtProgressUpdateStream (\s a -> s {_mtProgressUpdateStream = a})

-- | Unique identifier that references the migration task.
mtMigrationTaskName :: Lens' MigrationTask (Maybe Text)
mtMigrationTaskName =
  lens _mtMigrationTaskName (\s a -> s {_mtMigrationTaskName = a})
...
instance FromJSON MigrationTask where
  parseJSON =
    withObject
      "MigrationTask"
      (\x ->
         MigrationTask' <$> (x .:? "UpdateDateTime") <*>
...
mtsUpdateDateTime =
  lens _mtsUpdateDateTime (\s a -> s {_mtsUpdateDateTime = a}) . mapping _Time

-- |
mtsProgressPercent :: Lens' MigrationTaskSummary (Maybe Natural)
mtsProgressPercent =
  lens _mtsProgressPercent (\s a -> s {_mtsProgressPercent = a}) . mapping _Nat
...
mtsProgressUpdateStream =
  lens _mtsProgressUpdateStream (\s a -> s {_mtsProgressUpdateStream = a})

-- | Unique identifier that references the migration task.
mtsMigrationTaskName :: Lens' MigrationTaskSummary (Maybe Text)
mtsMigrationTaskName =
  lens _mtsMigrationTaskName (\s a -> s {_mtsMigrationTaskName = a})
...
instance FromJSON MigrationTaskSummary where
  parseJSON =
    withObject
      "MigrationTaskSummary"
      (\x ->
         MigrationTaskSummary' <$> (x .:? "Status") <*> (x .:? "UpdateDateTime") <*>
         (x .:? "ProgressPercent") <*>
         (x .:? "StatusDetail") <*>
         (x .:? "ProgressUpdateStream") <*>
         (x .:? "MigrationTaskName"))
...
pussProgressUpdateStreamName =
  lens
    ...
    (\s a -> s {_pussProgressUpdateStreamName = a})

instance FromJSON ProgressUpdateStreamSummary where
  parseJSON =
    withObject
      "ProgressUpdateStreamSummary"
      (\x -> ProgressUpdateStreamSummary' <$> (x .:? "ProgressUpdateStreamName"))
...
instance ToJSON ResourceAttribute where
  toJSON ResourceAttribute' {..} =
    object (catMaybes [Just ("Type" .= _raType), Just ("Value" .= _raValue)])
...
tProgressPercent =
  lens _tProgressPercent (\s a -> s {_tProgressPercent = a}) . mapping _Nat
...
instance FromJSON Task where
  parseJSON =
    withObject
      "Task"
      (\x ->
         Task' <$> (x .:? "ProgressPercent") <*> (x .:? "StatusDetail") <*>
         (x .: "Status"))
