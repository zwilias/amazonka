...
instance FromText ApplicationStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ApplicationStatus from value: '" <>
        e <> "'. Accepted values: completed, in_progress, not_started"
...
instance FromText MigrationStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MigrationStatus from value: '" <>
        e <> "'. Accepted values: completed, failed, in_progress, not_started"
...
instance FromText ResourceAttributeType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ResourceAttributeType from value: '" <>
        e <>
