...
data ApplicationState = ApplicationState'{_asLastUpdatedTime
                                          :: !(Maybe POSIX),
...
applicationState
    :: ApplicationState
applicationState
  = ApplicationState'{_asLastUpdatedTime = Nothing,
...
instance FromJSON ApplicationState where
        parseJSON
          = withObject "ApplicationState"
              (\ x ->
                 ApplicationState' <$>
                   (x .:? "LastUpdatedTime") <*> (x .:? "ApplicationId")
                     <*> (x .:? "ApplicationStatus"))

instance Hashable ApplicationState where

instance NFData ApplicationState where
