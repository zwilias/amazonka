...
acaProgressUpdateStream =
  lens _acaProgressUpdateStream (\s a -> s {_acaProgressUpdateStream = a})

-- | Unique identifier that references the migration task.
acaMigrationTaskName :: Lens' AssociateCreatedArtifact Text
acaMigrationTaskName =
  lens _acaMigrationTaskName (\s a -> s {_acaMigrationTaskName = a})

-- | An ARN of the AWS resource related to the migration (e.g., AMI, EC2 instance, RDS instance, etc.)
acaCreatedArtifact :: Lens' AssociateCreatedArtifact CreatedArtifact
acaCreatedArtifact =
  lens _acaCreatedArtifact (\s a -> s {_acaCreatedArtifact = a})
...
instance AWSRequest AssociateCreatedArtifact where
  ...
  response =
    receiveEmpty
      (\s h x -> AssociateCreatedArtifactResponse' <$> (pure (fromEnum s)))
...
acarsResponseStatus =
  lens _acarsResponseStatus (\s a -> s {_acarsResponseStatus = a})
