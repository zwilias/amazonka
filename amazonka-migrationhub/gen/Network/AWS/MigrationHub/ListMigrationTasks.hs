...
lmtMaxResults =
  lens _lmtMaxResults (\s a -> s {_lmtMaxResults = a}) . mapping _Nat
...
lmtrsMigrationTaskSummaryList ::
     Lens' ListMigrationTasksResponse [MigrationTaskSummary]
lmtrsMigrationTaskSummaryList =
  lens
    _lmtrsMigrationTaskSummaryList
    (\s a -> s {_lmtrsMigrationTaskSummaryList = a}) .
  _Default . _Coerce
...
lmtrsResponseStatus =
  lens _lmtrsResponseStatus (\s a -> s {_lmtrsResponseStatus = a})
