...
dmtProgressUpdateStream =
  lens _dmtProgressUpdateStream (\s a -> s {_dmtProgressUpdateStream = a})

-- | The identifier given to the MigrationTask.
dmtMigrationTaskName :: Lens' DescribeMigrationTask Text
dmtMigrationTaskName =
  lens _dmtMigrationTaskName (\s a -> s {_dmtMigrationTaskName = a})
...
instance AWSRequest DescribeMigrationTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeMigrationTaskResponse' <$> (x .?> "MigrationTask") <*>
         (pure (fromEnum s)))
...
dmtrsMigrationTask =
  lens _dmtrsMigrationTask (\s a -> s {_dmtrsMigrationTask = a})

-- | -- | The response status code.
dmtrsResponseStatus :: Lens' DescribeMigrationTaskResponse Int
dmtrsResponseStatus =
  lens _dmtrsResponseStatus (\s a -> s {_dmtrsResponseStatus = a})
