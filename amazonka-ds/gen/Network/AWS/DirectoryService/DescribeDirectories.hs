...
ddDirectoryIds =
  lens _ddDirectoryIds (\s a -> s {_ddDirectoryIds = a}) . _Default . _Coerce
...
ddrsDirectoryDescriptions ::
     Lens' DescribeDirectoriesResponse [DirectoryDescription]
ddrsDirectoryDescriptions =
  lens _ddrsDirectoryDescriptions (\s a -> s {_ddrsDirectoryDescriptions = a}) .
  _Default . _Coerce
