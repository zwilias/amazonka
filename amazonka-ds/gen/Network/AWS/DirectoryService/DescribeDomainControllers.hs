...
ddcDomainControllerIds =
  lens _ddcDomainControllerIds (\s a -> s {_ddcDomainControllerIds = a}) .
  _Default . _Coerce
...
instance AWSRequest DescribeDomainControllers where
  type Rs DescribeDomainControllers = DescribeDomainControllersResponse
  request = postJSON directoryService
  response =
    receiveJSON
      (\s h x ->
         DescribeDomainControllersResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders DescribeDomainControllers where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("DirectoryService_20150416.DescribeDomainControllers" :: ByteString)
...
ddcrsDomainControllers ::
     Lens' DescribeDomainControllersResponse [DomainController]
ddcrsDomainControllers =
  lens _ddcrsDomainControllers (\s a -> s {_ddcrsDomainControllers = a}) .
  _Default . _Coerce

-- | -- | The response status code.
ddcrsResponseStatus :: Lens' DescribeDomainControllersResponse Int
ddcrsResponseStatus =
  lens _ddcrsResponseStatus (\s a -> s {_ddcrsResponseStatus = a})
