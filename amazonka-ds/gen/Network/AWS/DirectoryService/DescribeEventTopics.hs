...
dTopicNames =
  lens _dTopicNames (\s a -> s {_dTopicNames = a}) . _Default . _Coerce
...
instance AWSRequest DescribeEventTopics where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeEventTopicsResponse' <$> (x .?> "EventTopics" .!@ mempty) <*>
         (pure (fromEnum s)))
...
detrsEventTopics =
  lens _detrsEventTopics (\s a -> s {_detrsEventTopics = a}) .
  _Default . _Coerce
