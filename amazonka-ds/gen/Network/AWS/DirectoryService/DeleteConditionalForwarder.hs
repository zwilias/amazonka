...
delRemoteDomainName =
  lens _delRemoteDomainName (\s a -> s {_delRemoteDomainName = a})

instance AWSRequest DeleteConditionalForwarder where
  type Rs DeleteConditionalForwarder = DeleteConditionalForwarderResponse
  request = postJSON directoryService
  response =
    receiveEmpty
      (\s h x -> DeleteConditionalForwarderResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteConditionalForwarder where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("DirectoryService_20150416.DeleteConditionalForwarder" :: ByteString)
