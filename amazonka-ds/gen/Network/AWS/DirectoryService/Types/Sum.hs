...
instance FromText DirectoryEdition where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DirectoryEdition from value: '" <>
        e <> "'. Accepted values: enterprise, standard"
...
instance FromText DirectorySize where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DirectorySize from value: '" <>
        e <> "'. Accepted values: large, small"
...
instance FromText DirectoryStage where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DirectoryStage from value: '" <>
        e <>
...
instance FromText DirectoryType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DirectoryType from value: '" <>
        e <> "'. Accepted values: adconnector, microsoftad, simplead"
...
instance FromText DomainControllerStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DomainControllerStatus from value: '" <>
        e <>
...
instance FromText IPRouteStatusMsg where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IPRouteStatusMsg from value: '" <>
        e <>
...
instance FromText RadiusAuthenticationProtocol where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RadiusAuthenticationProtocol from value: '" <>
        e <> "'. Accepted values: chap, ms-chapv1, ms-chapv2, pap"
...
instance FromText RadiusStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RadiusStatus from value: '" <>
        e <> "'. Accepted values: completed, creating, failed"
...
instance FromText ReplicationScope where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReplicationScope from value: '" <>
        e <> "'. Accepted values: domain"
...
instance FromText SchemaExtensionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SchemaExtensionStatus from value: '" <>
        e <>
...
instance FromText SnapshotStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SnapshotStatus from value: '" <>
        e <> "'. Accepted values: completed, creating, failed"
...
instance FromText SnapshotType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SnapshotType from value: '" <>
        e <> "'. Accepted values: auto, manual"
...
instance FromText TopicStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TopicStatus from value: '" <>
        e <> "'. Accepted values: deleted, failed, registered, topic not found"
...
instance FromText TrustDirection where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TrustDirection from value: '" <>
        e <> "'. Accepted values: one-way: incoming, one-way: outgoing, two-way"
...
instance FromText TrustState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TrustState from value: '" <>
        e <>
...
instance FromText TrustType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TrustType from value: '" <>
        e <> "'. Accepted values: forest"
