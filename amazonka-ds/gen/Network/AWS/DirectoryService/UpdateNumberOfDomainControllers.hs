...
unodcDesiredNumber =
  lens _unodcDesiredNumber (\s a -> s {_unodcDesiredNumber = a}) . _Nat

instance AWSRequest UpdateNumberOfDomainControllers where
  type Rs UpdateNumberOfDomainControllers = UpdateNumberOfDomainControllersResponse
...
instance ToHeaders UpdateNumberOfDomainControllers where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("DirectoryService_20150416.UpdateNumberOfDomainControllers" :: ByteString)
...
unodcrsResponseStatus =
  lens _unodcrsResponseStatus (\s a -> s {_unodcrsResponseStatus = a})
