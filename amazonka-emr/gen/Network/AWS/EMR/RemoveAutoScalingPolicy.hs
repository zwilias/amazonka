...
raspInstanceGroupId =
  lens _raspInstanceGroupId (\s a -> s {_raspInstanceGroupId = a})
...
instance AWSRequest RemoveAutoScalingPolicy where
  ...
  response =
    receiveEmpty
      (\s h x -> RemoveAutoScalingPolicyResponse' <$> (pure (fromEnum s)))
...
rasprsResponseStatus =
  lens _rasprsResponseStatus (\s a -> s {_rasprsResponseStatus = a})
