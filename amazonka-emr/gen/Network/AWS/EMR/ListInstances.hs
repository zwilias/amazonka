...
liInstanceFleetType =
  lens _liInstanceFleetType (\s a -> s {_liInstanceFleetType = a})
...
liInstanceStates =
  lens _liInstanceStates (\s a -> s {_liInstanceStates = a}) .
  _Default . _Coerce
...
instance AWSRequest ListInstances where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListInstancesResponse' <$> (x .?> "Marker") <*>
         (x .?> "Instances" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListInstances where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("ElasticMapReduce.ListInstances" :: ByteString)
...
lirsInstances =
  lens _lirsInstances (\s a -> s {_lirsInstances = a}) . _Default . _Coerce
