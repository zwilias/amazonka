...
rjfEBSRootVolumeSize =
  lens _rjfEBSRootVolumeSize (\s a -> s {_rjfEBSRootVolumeSize = a})
...
rjfConfigurations =
  lens _rjfConfigurations (\s a -> s {_rjfConfigurations = a}) .
  _Default . _Coerce
...
rjfAutoScalingRole =
  lens _rjfAutoScalingRole (\s a -> s {_rjfAutoScalingRole = a})

-- | The name of a security configuration to apply to the cluster.
rjfSecurityConfiguration :: Lens' RunJobFlow (Maybe Text)
rjfSecurityConfiguration =
  lens _rjfSecurityConfiguration (\s a -> s {_rjfSecurityConfiguration = a})

-- | Specifies the way that individual Amazon EC2 instances terminate when an automatic scale-in activity occurs or an instance group is resized. @TERMINATE_AT_INSTANCE_HOUR@ indicates that Amazon EMR terminates nodes at the instance-hour boundary, regardless of when the request to terminate the instance was submitted. This option is only available with Amazon EMR 5.1.0 and later and is the default for clusters created using that version. @TERMINATE_AT_TASK_COMPLETION@ indicates that Amazon EMR blacklists and drains tasks from nodes before terminating the Amazon EC2 instances, regardless of the instance-hour boundary. With either behavior, Amazon EMR removes the least active nodes first and blocks instance termination if it could lead to HDFS corruption. @TERMINATE_AT_TASK_COMPLETION@ available only in Amazon EMR version 4.1.0 and later, and is the default for versions of Amazon EMR earlier than 5.1.0.
rjfScaleDownBehavior :: Lens' RunJobFlow (Maybe ScaleDownBehavior)
rjfScaleDownBehavior =
  lens _rjfScaleDownBehavior (\s a -> s {_rjfScaleDownBehavior = a})
...
rjfRepoUpgradeOnBoot =
  lens _rjfRepoUpgradeOnBoot (\s a -> s {_rjfRepoUpgradeOnBoot = a})
...
rjfKerberosAttributes =
  lens _rjfKerberosAttributes (\s a -> s {_rjfKerberosAttributes = a})
...
rjfApplications =
  lens _rjfApplications (\s a -> s {_rjfApplications = a}) . _Default . _Coerce
...
instance AWSRequest RunJobFlow where
  ...
  response =
    receiveJSON
      (\s h x ->
         RunJobFlowResponse' <$> (x .?> "JobFlowId") <*> (pure (fromEnum s)))
...
rjfrsResponseStatus =
  lens _rjfrsResponseStatus (\s a -> s {_rjfrsResponseStatus = a})
