...
instance AWSRequest ListBootstrapActions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListBootstrapActionsResponse' <$> (x .?> "BootstrapActions" .!@ mempty) <*>
         (x .?> "Marker") <*>
         (pure (fromEnum s)))
...
instance ToJSON ListBootstrapActions where
  toJSON ListBootstrapActions' {..} =
    object
      (catMaybes
         [("Marker" .=) <$> _lbaMarker, Just ("ClusterId" .= _lbaClusterId)])
...
lbarsResponseStatus =
  lens _lbarsResponseStatus (\s a -> s {_lbarsResponseStatus = a})
