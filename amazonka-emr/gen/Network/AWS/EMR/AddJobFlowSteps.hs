...
instance AWSRequest AddJobFlowSteps where
  ...
  response =
    receiveJSON
      (\s h x ->
         AddJobFlowStepsResponse' <$> (x .?> "StepIds" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders AddJobFlowSteps where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("ElasticMapReduce.AddJobFlowSteps" :: ByteString)
...
instance ToJSON AddJobFlowSteps where
  toJSON AddJobFlowSteps' {..} =
    object
      (catMaybes
         [Just ("JobFlowId" .= _ajfsJobFlowId), Just ("Steps" .= _ajfsSteps)])
...
ajfsrsStepIds =
  lens _ajfsrsStepIds (\s a -> s {_ajfsrsStepIds = a}) . _Default . _Coerce

-- | -- | The response status code.
ajfsrsResponseStatus :: Lens' AddJobFlowStepsResponse Int
ajfsrsResponseStatus =
  lens _ajfsrsResponseStatus (\s a -> s {_ajfsrsResponseStatus = a})
