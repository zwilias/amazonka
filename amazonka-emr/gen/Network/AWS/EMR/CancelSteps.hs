...
instance AWSRequest CancelSteps where
  ...
  response =
    receiveJSON
      (\s h x ->
         CancelStepsResponse' <$> (x .?> "CancelStepsInfoList" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToJSON CancelSteps where
  toJSON CancelSteps' {..} =
    object
      (catMaybes
         [("StepIds" .=) <$> _csStepIds, ("ClusterId" .=) <$> _csClusterId])
...
csrsCancelStepsInfoList =
  lens _csrsCancelStepsInfoList (\s a -> s {_csrsCancelStepsInfoList = a}) .
  _Default . _Coerce
