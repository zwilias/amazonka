...
paspInstanceGroupId =
  lens _paspInstanceGroupId (\s a -> s {_paspInstanceGroupId = a})

-- | Specifies the definition of the automatic scaling policy.
paspAutoScalingPolicy :: Lens' PutAutoScalingPolicy AutoScalingPolicy
paspAutoScalingPolicy =
  lens _paspAutoScalingPolicy (\s a -> s {_paspAutoScalingPolicy = a})
...
instance AWSRequest PutAutoScalingPolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         PutAutoScalingPolicyResponse' <$> (x .?> "ClusterId") <*>
         (x .?> "AutoScalingPolicy") <*>
         (x .?> "InstanceGroupId") <*>
         (pure (fromEnum s)))
...
pasprsAutoScalingPolicy ::
     Lens' PutAutoScalingPolicyResponse (Maybe AutoScalingPolicyDescription)
pasprsAutoScalingPolicy =
  lens _pasprsAutoScalingPolicy (\s a -> s {_pasprsAutoScalingPolicy = a})

-- | Specifies the ID of the instance group to which the scaling policy is applied.
pasprsInstanceGroupId :: Lens' PutAutoScalingPolicyResponse (Maybe Text)
pasprsInstanceGroupId =
  lens _pasprsInstanceGroupId (\s a -> s {_pasprsInstanceGroupId = a})

-- | -- | The response status code.
pasprsResponseStatus :: Lens' PutAutoScalingPolicyResponse Int
pasprsResponseStatus =
  lens _pasprsResponseStatus (\s a -> s {_pasprsResponseStatus = a})
