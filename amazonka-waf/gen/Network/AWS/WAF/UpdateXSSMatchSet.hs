...
instance AWSRequest UpdateXSSMatchSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateXSSMatchSetResponse' <$> (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateXSSMatchSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.UpdateXssMatchSet" :: ByteString)
...
uxmsrsResponseStatus =
  lens _uxmsrsResponseStatus (\s a -> s {_uxmsrsResponseStatus = a})
