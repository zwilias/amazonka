...
instance AWSRequest CreateXSSMatchSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateXSSMatchSetResponse' <$> (x .?> "XssMatchSet") <*>
         (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders CreateXSSMatchSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.CreateXssMatchSet" :: ByteString)
...
instance ToJSON CreateXSSMatchSet where
  toJSON CreateXSSMatchSet' {..} =
    object
      (catMaybes
         [Just ("Name" .= _cxmsName), Just ("ChangeToken" .= _cxmsChangeToken)])
...
cxmsrsResponseStatus =
  lens _cxmsrsResponseStatus (\s a -> s {_cxmsrsResponseStatus = a})
