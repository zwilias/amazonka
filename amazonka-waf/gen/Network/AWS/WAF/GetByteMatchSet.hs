...
gbmsByteMatchSetId =
  lens _gbmsByteMatchSetId (\s a -> s {_gbmsByteMatchSetId = a})
...
instance AWSRequest GetByteMatchSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetByteMatchSetResponse' <$> (x .?> "ByteMatchSet") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetByteMatchSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.GetByteMatchSet" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON GetByteMatchSet where
  toJSON GetByteMatchSet' {..} =
    object (catMaybes [Just ("ByteMatchSetId" .= _gbmsByteMatchSetId)])
...
gbmsrsByteMatchSet =
  lens _gbmsrsByteMatchSet (\s a -> s {_gbmsrsByteMatchSet = a})

-- | -- | The response status code.
gbmsrsResponseStatus :: Lens' GetByteMatchSetResponse Int
gbmsrsResponseStatus =
  lens _gbmsrsResponseStatus (\s a -> s {_gbmsrsResponseStatus = a})
