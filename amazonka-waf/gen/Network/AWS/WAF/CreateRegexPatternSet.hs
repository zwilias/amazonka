...
instance AWSRequest CreateRegexPatternSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateRegexPatternSetResponse' <$> (x .?> "RegexPatternSet") <*>
         (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
instance ToJSON CreateRegexPatternSet where
  toJSON CreateRegexPatternSet' {..} =
    object
      (catMaybes
         [Just ("Name" .= _crpsName), Just ("ChangeToken" .= _crpsChangeToken)])
...
crpsrsRegexPatternSet ::
     Lens' CreateRegexPatternSetResponse (Maybe RegexPatternSet)
crpsrsRegexPatternSet =
  lens _crpsrsRegexPatternSet (\s a -> s {_crpsrsRegexPatternSet = a})
...
crpsrsResponseStatus =
  lens _crpsrsResponseStatus (\s a -> s {_crpsrsResponseStatus = a})
