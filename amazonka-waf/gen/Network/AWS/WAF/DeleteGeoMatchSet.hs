...
instance AWSRequest DeleteGeoMatchSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteGeoMatchSetResponse' <$> (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DeleteGeoMatchSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.DeleteGeoMatchSet" :: ByteString)
...
dgmsrsResponseStatus =
  lens _dgmsrsResponseStatus (\s a -> s {_dgmsrsResponseStatus = a})
