...
instance AWSRequest ListRuleGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListRuleGroupsResponse' <$> (x .?> "RuleGroups" .!@ mempty) <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListRuleGroups where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.ListRuleGroups" :: ByteString)
...
lrgrsRuleGroups =
  lens _lrgrsRuleGroups (\s a -> s {_lrgrsRuleGroups = a}) . _Default . _Coerce
...
lrgrsResponseStatus =
  lens _lrgrsResponseStatus (\s a -> s {_lrgrsResponseStatus = a})
