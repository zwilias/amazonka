...
dscsSizeConstraintSetId =
  lens _dscsSizeConstraintSetId (\s a -> s {_dscsSizeConstraintSetId = a})
...
instance AWSRequest DeleteSizeConstraintSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteSizeConstraintSetResponse' <$> (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
dscsrsResponseStatus =
  lens _dscsrsResponseStatus (\s a -> s {_dscsrsResponseStatus = a})
