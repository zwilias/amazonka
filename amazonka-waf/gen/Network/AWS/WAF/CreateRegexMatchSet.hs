...
instance AWSRequest CreateRegexMatchSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateRegexMatchSetResponse' <$> (x .?> "RegexMatchSet") <*>
         (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
instance ToJSON CreateRegexMatchSet where
  toJSON CreateRegexMatchSet' {..} =
    object
      (catMaybes
         [Just ("Name" .= _crmsName), Just ("ChangeToken" .= _crmsChangeToken)])
...
crmsrsRegexMatchSet =
  lens _crmsrsRegexMatchSet (\s a -> s {_crmsrsRegexMatchSet = a})
...
crmsrsResponseStatus =
  lens _crmsrsResponseStatus (\s a -> s {_crmsrsResponseStatus = a})
