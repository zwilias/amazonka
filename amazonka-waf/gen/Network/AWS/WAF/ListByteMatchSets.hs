...
instance AWSRequest ListByteMatchSets where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListByteMatchSetsResponse' <$> (x .?> "ByteMatchSets" .!@ mempty) <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListByteMatchSets where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.ListByteMatchSets" :: ByteString)
...
instance ToJSON ListByteMatchSets where
  toJSON ListByteMatchSets' {..} =
    object
      (catMaybes
         [("NextMarker" .=) <$> _lbmsNextMarker, ("Limit" .=) <$> _lbmsLimit])
...
lbmsrsByteMatchSets =
  lens _lbmsrsByteMatchSets (\s a -> s {_lbmsrsByteMatchSets = a}) .
  _Default . _Coerce
...
lbmsrsResponseStatus =
  lens _lbmsrsResponseStatus (\s a -> s {_lbmsrsResponseStatus = a})
