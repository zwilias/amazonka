...
drpsRegexPatternSetId =
  lens _drpsRegexPatternSetId (\s a -> s {_drpsRegexPatternSetId = a})
...
instance AWSRequest DeleteRegexPatternSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteRegexPatternSetResponse' <$> (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
drpsrsResponseStatus =
  lens _drpsrsResponseStatus (\s a -> s {_drpsrsResponseStatus = a})
