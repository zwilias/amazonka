...
drmsRegexMatchSetId =
  lens _drmsRegexMatchSetId (\s a -> s {_drmsRegexMatchSetId = a})
...
instance AWSRequest DeleteRegexMatchSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteRegexMatchSetResponse' <$> (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
drmsrsResponseStatus =
  lens _drmsrsResponseStatus (\s a -> s {_drmsrsResponseStatus = a})
