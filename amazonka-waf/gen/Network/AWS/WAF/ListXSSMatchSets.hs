...
instance AWSRequest ListXSSMatchSets where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListXSSMatchSetsResponse' <$> (x .?> "XssMatchSets" .!@ mempty) <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListXSSMatchSets where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.ListXssMatchSets" :: ByteString)
...
instance ToJSON ListXSSMatchSets where
  toJSON ListXSSMatchSets' {..} =
    object
      (catMaybes
         [("NextMarker" .=) <$> _lxmsNextMarker, ("Limit" .=) <$> _lxmsLimit])
...
lxmsrsXSSMatchSets =
  lens _lxmsrsXSSMatchSets (\s a -> s {_lxmsrsXSSMatchSets = a}) .
  _Default . _Coerce
...
lxmsrsResponseStatus =
  lens _lxmsrsResponseStatus (\s a -> s {_lxmsrsResponseStatus = a})
