...
instance AWSRequest ListGeoMatchSets where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListGeoMatchSetsResponse' <$> (x .?> "GeoMatchSets" .!@ mempty) <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListGeoMatchSets where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.ListGeoMatchSets" :: ByteString)
...
instance ToJSON ListGeoMatchSets where
  toJSON ListGeoMatchSets' {..} =
    object
      (catMaybes
         [("NextMarker" .=) <$> _lgmsNextMarker, ("Limit" .=) <$> _lgmsLimit])
...
lgmsrsGeoMatchSets =
  lens _lgmsrsGeoMatchSets (\s a -> s {_lgmsrsGeoMatchSets = a}) .
  _Default . _Coerce
...
lgmsrsResponseStatus =
  lens _lgmsrsResponseStatus (\s a -> s {_lgmsrsResponseStatus = a})
