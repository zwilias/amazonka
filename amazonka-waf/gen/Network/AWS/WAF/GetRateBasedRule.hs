...
instance AWSRequest GetRateBasedRule where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetRateBasedRuleResponse' <$> (x .?> "Rule") <*> (pure (fromEnum s)))
...
instance ToHeaders GetRateBasedRule where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.GetRateBasedRule" :: ByteString)
...
grbrrsResponseStatus =
  lens _grbrrsResponseStatus (\s a -> s {_grbrrsResponseStatus = a})
