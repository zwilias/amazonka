...
instance AWSRequest GetSampledRequests where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetSampledRequestsResponse' <$> (x .?> "SampledRequests" .!@ mempty) <*>
         (x .?> "PopulationSize") <*>
         (x .?> "TimeWindow") <*>
         (pure (fromEnum s)))
...
gsrrsResponseStatus =
  lens _gsrrsResponseStatus (\s a -> s {_gsrrsResponseStatus = a})
