...
instance AWSRequest UpdateGeoMatchSet where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateGeoMatchSetResponse' <$> (x .?> "ChangeToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateGeoMatchSet where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSWAF_20150824.UpdateGeoMatchSet" :: ByteString)
...
ugmsrsResponseStatus =
  lens _ugmsrsResponseStatus (\s a -> s {_ugmsrsResponseStatus = a})
