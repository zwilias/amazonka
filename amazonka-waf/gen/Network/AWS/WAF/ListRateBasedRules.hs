...
instance AWSRequest ListRateBasedRules where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListRateBasedRulesResponse' <$> (x .?> "Rules" .!@ mempty) <*>
         (x .?> "NextMarker") <*>
         (pure (fromEnum s)))
...
instance ToJSON ListRateBasedRules where
  toJSON ListRateBasedRules' {..} =
    object
      (catMaybes
         [("NextMarker" .=) <$> _lrbrNextMarker, ("Limit" .=) <$> _lrbrLimit])
...
lrbrrsRules =
  lens _lrbrrsRules (\s a -> s {_lrbrrsRules = a}) . _Default . _Coerce
...
lrbrrsResponseStatus =
  lens _lrbrrsResponseStatus (\s a -> s {_lrbrrsResponseStatus = a})
