...
cgaOpenIdConnectConfig =
  lens _cgaOpenIdConnectConfig (\s a -> s {_cgaOpenIdConnectConfig = a})
...
cgaAuthenticationType =
  lens _cgaAuthenticationType (\s a -> s {_cgaAuthenticationType = a})
...
instance AWSRequest CreateGraphqlAPI where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateGraphqlAPIResponse' <$> (x .?> "graphqlApi") <*>
         (pure (fromEnum s)))
...
cgarsResponseStatus =
  lens _cgarsResponseStatus (\s a -> s {_cgarsResponseStatus = a})
