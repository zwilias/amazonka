...
ldsMaxResults =
  lens _ldsMaxResults (\s a -> s {_ldsMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListDataSources where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDataSourcesResponse' <$> (x .?> "dataSources" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListDataSources where
  toQuery ListDataSources' {..} =
    mconcat ["nextToken" =: _ldsNextToken, "maxResults" =: _ldsMaxResults]
...
ldsrsDataSources =
  lens _ldsrsDataSources (\s a -> s {_ldsrsDataSources = a}) .
  _Default . _Coerce
