...
crResponseMappingTemplate =
  lens _crResponseMappingTemplate (\s a -> s {_crResponseMappingTemplate = a})
...
crRequestMappingTemplate =
  lens _crRequestMappingTemplate (\s a -> s {_crRequestMappingTemplate = a})
...
instance AWSRequest CreateResolver where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateResolverResponse' <$> (x .?> "resolver") <*> (pure (fromEnum s)))
  ...
  toJSON CreateResolver' {..} =
    object
      (catMaybes
         ...
         , Just ("requestMappingTemplate" .= _crRequestMappingTemplate)
         ])

instance ToPath CreateResolver where
  toPath CreateResolver' {..} =
    mconcat
      ["/v1/apis/", toBS _crApiId, "/types/", toBS _crTypeName, "/resolvers"]
