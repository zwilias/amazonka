...
instance FromText DataSourceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DataSourceType from value: '" <>
        e <>
...
instance FromText DefaultAction where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DefaultAction from value: '" <>
...
instance FromText FieldLogLevel where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing FieldLogLevel from value: '" <>
        e <> "'. Accepted values: all, error, none"
...
instance FromText OutputType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OutputType from value: '" <>
        e <> "'. Accepted values: json, sdl"
...
instance FromText SchemaStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SchemaStatus from value: '" <>
        e <> "'. Accepted values: active, deleting, processing"
...
instance FromText TypeDefinitionFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TypeDefinitionFormat from value: '" <>
        e <> "'. Accepted values: json, sdl"
