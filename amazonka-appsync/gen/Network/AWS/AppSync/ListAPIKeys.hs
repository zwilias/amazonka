...
lakMaxResults =
  lens _lakMaxResults (\s a -> s {_lakMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListAPIKeys where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAPIKeysResponse' <$> (x .?> "apiKeys" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToPath ListAPIKeys where
  toPath ListAPIKeys' {..} = mconcat ["/v1/apis/", toBS _lakApiId, "/apikeys"]

instance ToQuery ListAPIKeys where
  toQuery ListAPIKeys' {..} =
    mconcat ["nextToken" =: _lakNextToken, "maxResults" =: _lakMaxResults]
...
lakrsApiKeys =
  lens _lakrsApiKeys (\s a -> s {_lakrsApiKeys = a}) . _Default . _Coerce
...
lakrsResponseStatus =
  lens _lakrsResponseStatus (\s a -> s {_lakrsResponseStatus = a})
