...
instance AWSRequest CreateType where
  ...
  response =
    receiveJSON
      (\s h x -> CreateTypeResponse' <$> (x .?> "type") <*> (pure (fromEnum s)))
...
instance ToJSON CreateType where
  toJSON CreateType' {..} =
    object
      (catMaybes
         [Just ("definition" .= _ctDefinition), Just ("format" .= _ctFormat)])

instance ToPath CreateType where
  toPath CreateType' {..} = mconcat ["/v1/apis/", toBS _ctApiId, "/types"]
