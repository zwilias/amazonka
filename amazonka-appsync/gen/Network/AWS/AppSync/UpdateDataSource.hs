...
udsElasticsearchConfig ::
     Lens' UpdateDataSource (Maybe ElasticsearchDataSourceConfig)
udsElasticsearchConfig =
  lens _udsElasticsearchConfig (\s a -> s {_udsElasticsearchConfig = a})
...
instance AWSRequest UpdateDataSource where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateDataSourceResponse' <$> (x .?> "dataSource") <*>
         (pure (fromEnum s)))
...
instance ToPath UpdateDataSource where
  toPath UpdateDataSource' {..} =
    mconcat ["/v1/apis/", toBS _udsApiId, "/datasources/", toBS _udsName]
