...
instance AWSRequest ListResolvers where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListResolversResponse' <$> (x .?> "nextToken") <*>
         (x .?> "resolvers" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToPath ListResolvers where
  toPath ListResolvers' {..} =
    mconcat
      ["/v1/apis/", toBS _lrApiId, "/types/", toBS _lrTypeName, "/resolvers"]

instance ToQuery ListResolvers where
  toQuery ListResolvers' {..} =
    mconcat ["nextToken" =: _lrNextToken, "maxResults" =: _lrMaxResults]
...
lrrsResolvers =
  lens _lrrsResolvers (\s a -> s {_lrrsResolvers = a}) . _Default . _Coerce
