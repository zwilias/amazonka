...
ugaOpenIdConnectConfig =
  lens _ugaOpenIdConnectConfig (\s a -> s {_ugaOpenIdConnectConfig = a})
...
ugaAuthenticationType =
  lens _ugaAuthenticationType (\s a -> s {_ugaAuthenticationType = a})
...
instance AWSRequest UpdateGraphqlAPI where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateGraphqlAPIResponse' <$> (x .?> "graphqlApi") <*>
         (pure (fromEnum s)))
...
instance ToPath UpdateGraphqlAPI where
  toPath UpdateGraphqlAPI' {..} = mconcat ["/v1/apis/", toBS _ugaApiId]
...
ugarsResponseStatus =
  lens _ugarsResponseStatus (\s a -> s {_ugarsResponseStatus = a})
