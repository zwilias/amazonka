...
instance AWSRequest DeleteGraphqlAPI where
  ...
  response =
    receiveEmpty (\s h x -> DeleteGraphqlAPIResponse' <$> (pure (fromEnum s)))
...
instance ToPath DeleteGraphqlAPI where
  toPath DeleteGraphqlAPI' {..} = mconcat ["/v1/apis/", toBS _dgaApiId]
...
dgarsResponseStatus =
  lens _dgarsResponseStatus (\s a -> s {_dgarsResponseStatus = a})
