...
cbpVirtualInterfaceId =
  lens _cbpVirtualInterfaceId (\s a -> s {_cbpVirtualInterfaceId = a})
...
instance AWSRequest CreateBGPPeer where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateBGPPeerResponse' <$> (x .?> "virtualInterface") <*>
         (pure (fromEnum s)))
...
cbprsVirtualInterface =
  lens _cbprsVirtualInterface (\s a -> s {_cbprsVirtualInterface = a})

-- | -- | The response status code.
cbprsResponseStatus :: Lens' CreateBGPPeerResponse Int
cbprsResponseStatus =
  lens _cbprsResponseStatus (\s a -> s {_cbprsResponseStatus = a})
