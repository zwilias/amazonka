...
dbpCustomerAddress =
  lens _dbpCustomerAddress (\s a -> s {_dbpCustomerAddress = a})
...
dbpVirtualInterfaceId =
  lens _dbpVirtualInterfaceId (\s a -> s {_dbpVirtualInterfaceId = a})
...
instance AWSRequest DeleteBGPPeer where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteBGPPeerResponse' <$> (x .?> "virtualInterface") <*>
         (pure (fromEnum s)))
...
dbprsVirtualInterface =
  lens _dbprsVirtualInterface (\s a -> s {_dbprsVirtualInterface = a})
