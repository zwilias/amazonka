...
delVirtualInterfaceId =
  lens _delVirtualInterfaceId (\s a -> s {_delVirtualInterfaceId = a})
...
instance AWSRequest DeleteVirtualInterface where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteVirtualInterfaceResponse' <$> (x .?> "virtualInterfaceState") <*>
         (pure (fromEnum s)))
...
instance ToJSON DeleteVirtualInterface where
  toJSON DeleteVirtualInterface' {..} =
    object (catMaybes [Just ("virtualInterfaceId" .= _delVirtualInterfaceId)])
...
-- | Undocumented member.
dvirsVirtualInterfaceState ::
     Lens' DeleteVirtualInterfaceResponse (Maybe VirtualInterfaceState)
dvirsVirtualInterfaceState =
  lens _dvirsVirtualInterfaceState (\s a -> s {_dvirsVirtualInterfaceState = a})

-- | -- | The response status code.
dvirsResponseStatus :: Lens' DeleteVirtualInterfaceResponse Int
dvirsResponseStatus =
  lens _dvirsResponseStatus (\s a -> s {_dvirsResponseStatus = a})
