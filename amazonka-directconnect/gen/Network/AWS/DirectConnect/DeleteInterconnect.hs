...
instance AWSRequest DeleteInterconnect where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteInterconnectResponse' <$> (x .?> "interconnectState") <*>
         (pure (fromEnum s)))
...
-- | Undocumented member.
drsInterconnectState ::
     Lens' DeleteInterconnectResponse (Maybe InterconnectState)
drsInterconnectState =
  lens _drsInterconnectState (\s a -> s {_drsInterconnectState = a})
