...
instance AWSRequest DescribeLocations where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeLocationsResponse' <$> (x .?> "locations" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeLocations where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("OvertureService.DescribeLocations" :: ByteString)
...
dlsrsLocations =
  lens _dlsrsLocations (\s a -> s {_dlsrsLocations = a}) . _Default . _Coerce
