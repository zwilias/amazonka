...
cdcgDirectConnectGatewayName =
  lens
    ...
    (\s a -> s {_cdcgDirectConnectGatewayName = a})

instance AWSRequest CreateDirectConnectGateway where
  type Rs CreateDirectConnectGateway = CreateDirectConnectGatewayResponse
  request = postJSON directConnect
  response =
    receiveJSON
      (\s h x ->
         CreateDirectConnectGatewayResponse' <$> (x .?> "directConnectGateway") <*>
         (pure (fromEnum s)))
  ...
  toJSON CreateDirectConnectGateway' {..} =
    object
      (catMaybes
         ...
         , Just ("directConnectGatewayName" .= _cdcgDirectConnectGatewayName)
         ])
...
cdcgrsDirectConnectGateway ::
     Lens' CreateDirectConnectGatewayResponse (Maybe DirectConnectGateway)
cdcgrsDirectConnectGateway =
  lens _cdcgrsDirectConnectGateway (\s a -> s {_cdcgrsDirectConnectGateway = a})

-- | -- | The response status code.
cdcgrsResponseStatus :: Lens' CreateDirectConnectGatewayResponse Int
cdcgrsResponseStatus =
  lens _cdcgrsResponseStatus (\s a -> s {_cdcgrsResponseStatus = a})
