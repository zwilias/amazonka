...
cpSetupModeDisabled =
  lens _cpSetupModeDisabled (\s a -> s {_cpSetupModeDisabled = a})
...
instance AWSRequest CreateProfile where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateProfileResponse' <$> (x .?> "ProfileArn") <*> (pure (fromEnum s)))
...
instance ToHeaders CreateProfile where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AlexaForBusiness.CreateProfile" :: ByteString)
