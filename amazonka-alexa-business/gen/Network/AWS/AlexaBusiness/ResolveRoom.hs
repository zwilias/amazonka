...
instance AWSRequest ResolveRoom where
  ...
  response =
    receiveJSON
      (\s h x ->
         ResolveRoomResponse' <$> (x .?> "RoomSkillParameters" .!@ mempty) <*>
         (x .?> "RoomArn") <*>
         (x .?> "RoomName") <*>
         (pure (fromEnum s)))
...
instance ToJSON ResolveRoom where
  toJSON ResolveRoom' {..} =
    object
      (catMaybes [Just ("UserId" .= _rrUserId), Just ("SkillId" .= _rrSkillId)])
...
rrrsRoomSkillParameters =
  lens _rrrsRoomSkillParameters (\s a -> s {_rrrsRoomSkillParameters = a}) .
  _Default . _Coerce
...
rrrsResponseStatus =
  lens _rrrsResponseStatus (\s a -> s {_rrrsResponseStatus = a})
