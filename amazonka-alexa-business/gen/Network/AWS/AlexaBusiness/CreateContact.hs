...
ccClientRequestToken =
  lens _ccClientRequestToken (\s a -> s {_ccClientRequestToken = a})
...
instance AWSRequest CreateContact where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateContactResponse' <$> (x .?> "ContactArn") <*> (pure (fromEnum s)))
...
instance ToHeaders CreateContact where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AlexaForBusiness.CreateContact" :: ByteString)
