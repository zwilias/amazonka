...
ldeMaxResults =
  lens _ldeMaxResults (\s a -> s {_ldeMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListDeviceEvents where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDeviceEventsResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders ListDeviceEvents where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AlexaForBusiness.ListDeviceEvents" :: ByteString)
...
ldersDeviceEvents =
  lens _ldersDeviceEvents (\s a -> s {_ldersDeviceEvents = a}) .
  _Default . _Coerce

-- | -- | The response status code.
ldersResponseStatus :: Lens' ListDeviceEventsResponse Int
ldersResponseStatus =
  lens _ldersResponseStatus (\s a -> s {_ldersResponseStatus = a})
