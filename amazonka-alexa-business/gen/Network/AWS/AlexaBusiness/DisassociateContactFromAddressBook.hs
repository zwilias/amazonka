...
dcfabAddressBookARN =
  lens _dcfabAddressBookARN (\s a -> s {_dcfabAddressBookARN = a})

instance AWSRequest DisassociateContactFromAddressBook where
  type Rs DisassociateContactFromAddressBook = DisassociateContactFromAddressBookResponse
  request = postJSON alexaBusiness
  response =
    receiveEmpty
      (\s h x ->
         DisassociateContactFromAddressBookResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DisassociateContactFromAddressBook where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AlexaForBusiness.DisassociateContactFromAddressBook" :: ByteString)
...
dcfabrsResponseStatus =
  lens _dcfabrsResponseStatus (\s a -> s {_dcfabrsResponseStatus = a})
