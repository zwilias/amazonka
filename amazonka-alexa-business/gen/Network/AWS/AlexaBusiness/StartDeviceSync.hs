...
instance AWSRequest StartDeviceSync where
  ...
  response =
    receiveEmpty (\s h x -> StartDeviceSyncResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders StartDeviceSync where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AlexaForBusiness.StartDeviceSync" :: ByteString)
...
sdsrsResponseStatus =
  lens _sdsrsResponseStatus (\s a -> s {_sdsrsResponseStatus = a})
