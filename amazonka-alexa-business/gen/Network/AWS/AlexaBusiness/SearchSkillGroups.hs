...
ssgSortCriteria =
  lens _ssgSortCriteria (\s a -> s {_ssgSortCriteria = a}) . _Default . _Coerce
...
ssgMaxResults =
  lens _ssgMaxResults (\s a -> s {_ssgMaxResults = a}) . mapping _Nat
...
instance AWSRequest SearchSkillGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         SearchSkillGroupsResponse' <$> (x .?> "NextToken") <*>
...
ssgrsSkillGroups =
  lens _ssgrsSkillGroups (\s a -> s {_ssgrsSkillGroups = a}) .
  _Default . _Coerce
