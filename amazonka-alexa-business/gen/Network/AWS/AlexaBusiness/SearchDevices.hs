...
sdSortCriteria =
  lens _sdSortCriteria (\s a -> s {_sdSortCriteria = a}) . _Default . _Coerce
...
instance AWSRequest SearchDevices where
  ...
  response =
    receiveJSON
      (\s h x ->
         SearchDevicesResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Devices" .!@ mempty) <*>
         (x .?> "TotalCount") <*>
         (pure (fromEnum s)))
...
instance ToHeaders SearchDevices where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AlexaForBusiness.SearchDevices" :: ByteString)
...
sdrsDevices =
  lens _sdrsDevices (\s a -> s {_sdrsDevices = a}) . _Default . _Coerce
...
sdrsResponseStatus =
  lens _sdrsResponseStatus (\s a -> s {_sdrsResponseStatus = a})
