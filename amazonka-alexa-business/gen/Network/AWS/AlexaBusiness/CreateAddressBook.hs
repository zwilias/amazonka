...
cabClientRequestToken =
  lens _cabClientRequestToken (\s a -> s {_cabClientRequestToken = a})
...
instance AWSRequest CreateAddressBook where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateAddressBookResponse' <$> (x .?> "AddressBookArn") <*>
         (pure (fromEnum s)))
...
cabrsAddressBookARN =
  lens _cabrsAddressBookARN (\s a -> s {_cabrsAddressBookARN = a})
