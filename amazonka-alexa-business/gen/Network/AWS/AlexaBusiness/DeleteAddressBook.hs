...
instance AWSRequest DeleteAddressBook where
  ...
  response =
    receiveEmpty (\s h x -> DeleteAddressBookResponse' <$> (pure (fromEnum s)))
...
instance ToJSON DeleteAddressBook where
  toJSON DeleteAddressBook' {..} =
    object (catMaybes [Just ("AddressBookArn" .= _dabAddressBookARN)])
...
dabrsResponseStatus =
  lens _dabrsResponseStatus (\s a -> s {_dabrsResponseStatus = a})
