...
data InstantBooking = InstantBooking'{_ibEnabled ::
                                      !(Maybe Bool),
...
instantBooking
    :: InstantBooking
instantBooking
  = InstantBooking'{_ibEnabled = Nothing,
...
instance FromJSON InstantBooking where
        parseJSON
          = withObject "InstantBooking"
              (\ x ->
                 InstantBooking' <$>
                   (x .:? "Enabled") <*> (x .:? "DurationInMinutes"))

instance Hashable InstantBooking where

instance NFData InstantBooking where
