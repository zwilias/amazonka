...
data GatewayGroup = GatewayGroup'{_ggARN ::
                                  !(Maybe Text),
                                  _ggName :: !(Maybe Text),
...
gatewayGroup
    :: GatewayGroup
gatewayGroup
  = GatewayGroup'{_ggARN = Nothing, _ggName = Nothing,
...
instance FromJSON GatewayGroup where
        parseJSON
          = withObject "GatewayGroup"
              (\ x ->
                 GatewayGroup' <$>
                   (x .:? "Arn") <*> (x .:? "Name") <*>
                     (x .:? "Description"))

instance Hashable GatewayGroup where

instance NFData GatewayGroup where
