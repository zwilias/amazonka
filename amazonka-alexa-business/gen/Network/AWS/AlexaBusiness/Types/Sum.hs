...
instance FromText ConnectionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConnectionStatus from value: '" <>
        e <> "'. Accepted values: offline, online"
...
instance FromText DeviceEventType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeviceEventType from value: '" <>
        e <> "'. Accepted values: connection_status, device_status"
...
instance FromText DeviceStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeviceStatus from value: '" <>
        e <> "'. Accepted values: deregistered, pending, ready, was_offline"
...
instance FromText DeviceStatusDetailCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DeviceStatusDetailCode from value: '" <>
        e <>
...
instance FromText DistanceUnit where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DistanceUnit from value: '" <>
        e <> "'. Accepted values: imperial, metric"
...
instance FromText EnrollmentStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing EnrollmentStatus from value: '" <>
        e <>
...
instance FromText Feature where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Feature from value: '" <>
        e <>
...
instance FromText SortValue where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SortValue from value: '" <>
...
instance FromText TemperatureUnit where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TemperatureUnit from value: '" <>
        e <> "'. Accepted values: celsius, fahrenheit"
...
instance FromText WakeWord where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing WakeWord from value: '" <>
        e <> "'. Accepted values: alexa, amazon, computer, echo"
