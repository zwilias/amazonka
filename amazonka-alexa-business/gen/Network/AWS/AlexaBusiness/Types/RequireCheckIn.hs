...
data RequireCheckIn = RequireCheckIn'{_rciEnabled ::
                                      !(Maybe Bool),
...
requireCheckIn
    :: RequireCheckIn
requireCheckIn
  = RequireCheckIn'{_rciEnabled = Nothing,
...
instance FromJSON RequireCheckIn where
        parseJSON
          = withObject "RequireCheckIn"
              (\ x ->
                 RequireCheckIn' <$>
                   (x .:? "Enabled") <*> (x .:? "ReleaseAfterMinutes"))

instance Hashable RequireCheckIn where

instance NFData RequireCheckIn where
