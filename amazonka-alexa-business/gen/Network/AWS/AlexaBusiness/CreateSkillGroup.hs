...
csgClientRequestToken =
  lens _csgClientRequestToken (\s a -> s {_csgClientRequestToken = a})
...
instance AWSRequest CreateSkillGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateSkillGroupResponse' <$> (x .?> "SkillGroupArn") <*>
         (pure (fromEnum s)))
...
instance ToHeaders CreateSkillGroup where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AlexaForBusiness.CreateSkillGroup" :: ByteString)
...
csgrsSkillGroupARN =
  lens _csgrsSkillGroupARN (\s a -> s {_csgrsSkillGroupARN = a})
