...
dsgfrSkillGroupARN =
  lens _dsgfrSkillGroupARN (\s a -> s {_dsgfrSkillGroupARN = a})
...
instance AWSRequest DisassociateSkillGroupFromRoom where
  type Rs DisassociateSkillGroupFromRoom = DisassociateSkillGroupFromRoomResponse
  request = postJSON alexaBusiness
  response =
    receiveEmpty
      (\s h x -> DisassociateSkillGroupFromRoomResponse' <$> (pure (fromEnum s)))
...
dsgfrrsResponseStatus =
  lens _dsgfrrsResponseStatus (\s a -> s {_dsgfrrsResponseStatus = a})
