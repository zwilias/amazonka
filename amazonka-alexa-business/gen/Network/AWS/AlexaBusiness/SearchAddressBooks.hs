...
sabSortCriteria =
  lens _sabSortCriteria (\s a -> s {_sabSortCriteria = a}) . _Default . _Coerce
...
sabMaxResults =
  lens _sabMaxResults (\s a -> s {_sabMaxResults = a}) . mapping _Nat
...
instance AWSRequest SearchAddressBooks where
  ...
  response =
    receiveJSON
      (\s h x ->
         SearchAddressBooksResponse' <$> (x .?> "NextToken") <*>
...
sabrsAddressBooks =
  lens _sabrsAddressBooks (\s a -> s {_sabrsAddressBooks = a}) .
  _Default . _Coerce
...
sabrsResponseStatus =
  lens _sabrsResponseStatus (\s a -> s {_sabrsResponseStatus = a})
