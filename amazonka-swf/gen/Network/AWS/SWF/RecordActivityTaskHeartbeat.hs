...
instance AWSRequest RecordActivityTaskHeartbeat where
  ...
  response =
    receiveJSON
      (\s h x ->
         RecordActivityTaskHeartbeatResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "cancelRequested"))
...
instance ToHeaders RecordActivityTaskHeartbeat where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("SimpleWorkflowService.RecordActivityTaskHeartbeat" :: ByteString)
...
instance ToJSON RecordActivityTaskHeartbeat where
  toJSON RecordActivityTaskHeartbeat' {..} =
    object
      (catMaybes
         [("details" .=) <$> _rathDetails, Just ("taskToken" .= _rathTaskToken)])
...
rathrsResponseStatus =
  lens _rathrsResponseStatus (\s a -> s {_rathrsResponseStatus = a})
