...
instance AWSPager PollForDecisionTask where
  page rq rs
    ...
    | otherwise = Just $ rq & pfdtNextPageToken .~ rs ^. pfdtrsNextPageToken
...
instance AWSRequest PollForDecisionTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         PollForDecisionTaskResponse' <$> (x .?> "nextPageToken") <*>
         (x .?> "workflowType") <*>
...
pfdtrsPreviousStartedEventId ::
     Lens' PollForDecisionTaskResponse (Maybe Integer)
pfdtrsPreviousStartedEventId =
  lens
...
pfdtrsEvents =
  lens _pfdtrsEvents (\s a -> s {_pfdtrsEvents = a}) . _Default . _Coerce
...
pfdtrsWorkflowExecution ::
     Lens' PollForDecisionTaskResponse (Maybe WorkflowExecution)
pfdtrsWorkflowExecution =
  lens _pfdtrsWorkflowExecution (\s a -> s {_pfdtrsWorkflowExecution = a})

-- | -- | The response status code.
pfdtrsResponseStatus :: Lens' PollForDecisionTaskResponse Int
pfdtrsResponseStatus =
  lens _pfdtrsResponseStatus (\s a -> s {_pfdtrsResponseStatus = a})
