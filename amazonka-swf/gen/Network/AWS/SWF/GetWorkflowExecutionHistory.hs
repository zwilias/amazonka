...
instance AWSPager GetWorkflowExecutionHistory where
  page rq rs
    ...
    | otherwise = Just $ rq & gwehNextPageToken .~ rs ^. gwehrsNextPageToken
...
instance AWSRequest GetWorkflowExecutionHistory where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetWorkflowExecutionHistoryResponse' <$> (x .?> "nextPageToken") <*>
         (pure (fromEnum s)) <*>
         (x .?> "events" .!@ mempty))
...
instance ToHeaders GetWorkflowExecutionHistory where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("SimpleWorkflowService.GetWorkflowExecutionHistory" :: ByteString)
...
gwehrsResponseStatus =
  lens _gwehrsResponseStatus (\s a -> s {_gwehrsResponseStatus = a})
