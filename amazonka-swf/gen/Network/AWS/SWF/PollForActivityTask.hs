...
instance AWSRequest PollForActivityTask where
  ...
  response =
    receiveJSON
      (\s h x ->
         PollForActivityTaskResponse' <$> (x .?> "activityType") <*>
         (x .?> "activityId") <*>
         (x .?> "input") <*>
         (x .?> "taskToken") <*>
         (x .?> "workflowExecution") <*>
...
pfatrsWorkflowExecution ::
     Lens' PollForActivityTaskResponse (Maybe WorkflowExecution)
pfatrsWorkflowExecution =
  lens _pfatrsWorkflowExecution (\s a -> s {_pfatrsWorkflowExecution = a})

-- | -- | The response status code.
pfatrsResponseStatus :: Lens' PollForActivityTaskResponse Int
pfatrsResponseStatus =
  lens _pfatrsResponseStatus (\s a -> s {_pfatrsResponseStatus = a})
