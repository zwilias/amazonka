...
lcweExecutionFilter ::
     Lens' ListClosedWorkflowExecutions (Maybe WorkflowExecutionFilter)
lcweExecutionFilter =
  lens _lcweExecutionFilter (\s a -> s {_lcweExecutionFilter = a})

-- | If specified, only workflow executions that match this /close status/ are listed. For example, if TERMINATED is specified, then only TERMINATED workflow executions are listed.
lcweCloseStatusFilter ::
     Lens' ListClosedWorkflowExecutions (Maybe CloseStatusFilter)
lcweCloseStatusFilter =
...
lcweCloseTimeFilter ::
     Lens' ListClosedWorkflowExecutions (Maybe ExecutionTimeFilter)
lcweCloseTimeFilter =
...
instance AWSPager ListClosedWorkflowExecutions where
  page rq rs
    ...
    | otherwise = Just $ rq & lcweNextPageToken .~ rs ^. weiNextPageToken
...
instance ToHeaders ListClosedWorkflowExecutions where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("SimpleWorkflowService.ListClosedWorkflowExecutions" :: ByteString)
