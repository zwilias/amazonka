...
lwtMaximumPageSize =
  lens _lwtMaximumPageSize (\s a -> s {_lwtMaximumPageSize = a}) . mapping _Nat
...
instance AWSPager ListWorkflowTypes where
  page rq rs
    ...
    | otherwise = Just $ rq & lwtNextPageToken .~ rs ^. lwtrsNextPageToken
...
instance AWSRequest ListWorkflowTypes where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListWorkflowTypesResponse' <$> (x .?> "nextPageToken") <*>
         (pure (fromEnum s)) <*>
         (x .?> "typeInfos" .!@ mempty))
...
lwtrsTypeInfos =
  lens _lwtrsTypeInfos (\s a -> s {_lwtrsTypeInfos = a}) . _Coerce
