...
instance AWSRequest DescribeWorkflowType where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeWorkflowTypeResponse' <$> (pure (fromEnum s)) <*>
         (x .:> "typeInfo") <*>
         (x .:> "configuration"))
...
dwtrsResponseStatus =
  lens _dwtrsResponseStatus (\s a -> s {_dwtrsResponseStatus = a})
...
dwtrsConfiguration ::
     Lens' DescribeWorkflowTypeResponse WorkflowTypeConfiguration
dwtrsConfiguration =
