...
usdStreamingDistributionConfig ::
     Lens' UpdateStreamingDistribution StreamingDistributionConfig
usdStreamingDistributionConfig =
  lens
...
instance AWSRequest UpdateStreamingDistribution where
  ...
  response =
    receiveXML
      (\s h x ->
         UpdateStreamingDistributionResponse' <$> (h .#? "ETag") <*>
         (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath UpdateStreamingDistribution where
  toPath UpdateStreamingDistribution' {..} =
    mconcat ["/2017-10-30/streaming-distribution/", toBS _usdId, "/config"]
...
usdrsStreamingDistribution ::
     Lens' UpdateStreamingDistributionResponse (Maybe StreamingDistribution)
usdrsStreamingDistribution =
  lens _usdrsStreamingDistribution (\s a -> s {_usdrsStreamingDistribution = a})

-- | -- | The response status code.
usdrsResponseStatus :: Lens' UpdateStreamingDistributionResponse Int
usdrsResponseStatus =
  lens _usdrsResponseStatus (\s a -> s {_usdrsResponseStatus = a})
