...
instance AWSRequest GetPublicKey where
  ...
  response =
    receiveXML
      (\s h x ->
         GetPublicKeyResponse' <$> (h .#? "ETag") <*> (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath GetPublicKey where
  toPath GetPublicKey' {..} = mconcat ["/2017-10-30/public-key/", toBS _gpkId]
...
gpkrsResponseStatus =
  lens _gpkrsResponseStatus (\s a -> s {_gpkrsResponseStatus = a})
