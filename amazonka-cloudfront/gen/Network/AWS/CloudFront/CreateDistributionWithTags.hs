...
cdwtDistributionConfigWithTags =
  lens
    ...
    (\s a -> s {_cdwtDistributionConfigWithTags = a})

instance AWSRequest CreateDistributionWithTags where
  type Rs CreateDistributionWithTags = CreateDistributionWithTagsResponse
  request = postXML cloudFront
  response =
    receiveXML
      (\s h x ->
         CreateDistributionWithTagsResponse' <$> (h .#? "ETag") <*> (parseXML x) <*>
         (h .#? "Location") <*>
         (pure (fromEnum s)))
...
cdwtrsDistribution ::
     Lens' CreateDistributionWithTagsResponse (Maybe Distribution)
cdwtrsDistribution =
  lens _cdwtrsDistribution (\s a -> s {_cdwtrsDistribution = a})
...
cdwtrsResponseStatus =
  lens _cdwtrsResponseStatus (\s a -> s {_cdwtrsResponseStatus = a})
