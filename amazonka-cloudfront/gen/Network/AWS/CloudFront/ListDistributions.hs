...
instance AWSPager ListDistributions where
  page rq rs
    ...
    | isNothing (rs ^? ldrsDistributionList . dlNextMarker . _Just) = Nothing
    | otherwise =
      Just $ rq & ldMarker .~ rs ^? ldrsDistributionList . dlNextMarker . _Just
...
instance AWSRequest ListDistributions where
  ...
  response =
    receiveXML
      (\s h x ->
         ListDistributionsResponse' <$> (pure (fromEnum s)) <*> (parseXML x))
