...
upkPublicKeyConfig =
  lens _upkPublicKeyConfig (\s a -> s {_upkPublicKeyConfig = a})
...
instance AWSRequest UpdatePublicKey where
  ...
  response =
    receiveXML
      (\s h x ->
         UpdatePublicKeyResponse' <$> (h .#? "ETag") <*> (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdatePublicKey where
  toHeaders UpdatePublicKey' {..} = mconcat ["If-Match" =# _upkIfMatch]

instance ToPath UpdatePublicKey where
  toPath UpdatePublicKey' {..} =
    mconcat ["/2017-10-30/public-key/", toBS _upkId, "/config"]
...
upkrsResponseStatus =
  lens _upkrsResponseStatus (\s a -> s {_upkrsResponseStatus = a})
