...
instance AWSRequest GetFieldLevelEncryptionProfile where
  type Rs GetFieldLevelEncryptionProfile = GetFieldLevelEncryptionProfileResponse
  request = get cloudFront
  response =
    receiveXML
      (\s h x ->
         GetFieldLevelEncryptionProfileResponse' <$> (h .#? "ETag") <*>
         (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath GetFieldLevelEncryptionProfile where
  toPath GetFieldLevelEncryptionProfile' {..} =
    mconcat ["/2017-10-30/field-level-encryption-profile/", toBS _gflepId]
...
gfleprsFieldLevelEncryptionProfile ::
     Lens' GetFieldLevelEncryptionProfileResponse (Maybe FieldLevelEncryptionProfile)
gfleprsFieldLevelEncryptionProfile =
  lens
    _gfleprsFieldLevelEncryptionProfile
    (\s a -> s {_gfleprsFieldLevelEncryptionProfile = a})

-- | -- | The response status code.
gfleprsResponseStatus :: Lens' GetFieldLevelEncryptionProfileResponse Int
gfleprsResponseStatus =
  lens _gfleprsResponseStatus (\s a -> s {_gfleprsResponseStatus = a})
