...
instance AWSRequest GetFieldLevelEncryptionConfig where
  type Rs GetFieldLevelEncryptionConfig = GetFieldLevelEncryptionConfigResponse
  request = get cloudFront
  response =
    receiveXML
      (\s h x ->
         GetFieldLevelEncryptionConfigResponse' <$> (h .#? "ETag") <*>
         (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath GetFieldLevelEncryptionConfig where
  toPath GetFieldLevelEncryptionConfig' {..} =
    mconcat ["/2017-10-30/field-level-encryption/", toBS _gflecId, "/config"]
...
gflecrsFieldLevelEncryptionConfig ::
     Lens' GetFieldLevelEncryptionConfigResponse (Maybe FieldLevelEncryptionConfig)
gflecrsFieldLevelEncryptionConfig =
  lens
    _gflecrsFieldLevelEncryptionConfig
    (\s a -> s {_gflecrsFieldLevelEncryptionConfig = a})

-- | -- | The response status code.
gflecrsResponseStatus :: Lens' GetFieldLevelEncryptionConfigResponse Int
gflecrsResponseStatus =
  lens _gflecrsResponseStatus (\s a -> s {_gflecrsResponseStatus = a})
