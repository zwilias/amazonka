...
instance AWSRequest GetFieldLevelEncryption where
  ...
  response =
    receiveXML
      (\s h x ->
         GetFieldLevelEncryptionResponse' <$> (h .#? "ETag") <*> (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath GetFieldLevelEncryption where
  toPath GetFieldLevelEncryption' {..} =
    mconcat ["/2017-10-30/field-level-encryption/", toBS _gfleId]
...
gflersFieldLevelEncryption ::
     Lens' GetFieldLevelEncryptionResponse (Maybe FieldLevelEncryption)
gflersFieldLevelEncryption =
  lens _gflersFieldLevelEncryption (\s a -> s {_gflersFieldLevelEncryption = a})

-- | -- | The response status code.
gflersResponseStatus :: Lens' GetFieldLevelEncryptionResponse Int
gflersResponseStatus =
  lens _gflersResponseStatus (\s a -> s {_gflersResponseStatus = a})
