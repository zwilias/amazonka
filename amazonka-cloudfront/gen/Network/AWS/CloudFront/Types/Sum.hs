...
instance FromText CertificateSource where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CertificateSource from value: '" <>
        e <> "'. Accepted values: acm, cloudfront, iam"
...
instance FromText Format where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Format from value: '" <>
        e <> "'. Accepted values: urlencoded"
...
instance FromText GeoRestrictionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing GeoRestrictionType from value: '" <>
        e <> "'. Accepted values: blacklist, none, whitelist"
...
instance FromText HTTPVersion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HTTPVersion from value: '" <>
        e <> "'. Accepted values: http1.1, http2"
...
instance FromText ItemSelection where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ItemSelection from value: '" <>
        e <> "'. Accepted values: all, none, whitelist"
...
instance FromText Method where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Method from value: '" <>
        e <> "'. Accepted values: delete, get, head, options, patch, post, put"
...
instance FromText MinimumProtocolVersion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MinimumProtocolVersion from value: '" <>
        e <>
...
instance FromText OriginProtocolPolicy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OriginProtocolPolicy from value: '" <>
        e <> "'. Accepted values: http-only, https-only, match-viewer"
...
instance FromText PriceClass where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing PriceClass from value: '" <>
        e <>
...
instance FromText SSLProtocol where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SSLProtocol from value: '" <>
        e <> "'. Accepted values: sslv3, tlsv1, tlsv1.1, tlsv1.2"
...
instance FromText SSLSupportMethod where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SSLSupportMethod from value: '" <>
        e <> "'. Accepted values: sni-only, vip"
...
instance FromText ViewerProtocolPolicy where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ViewerProtocolPolicy from value: '" <>
        e <> "'. Accepted values: allow-all, https-only, redirect-to-https"
