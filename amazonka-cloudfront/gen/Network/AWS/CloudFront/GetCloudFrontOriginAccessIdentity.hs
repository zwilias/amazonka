...
instance AWSRequest GetCloudFrontOriginAccessIdentity where
  ...
  response =
    receiveXML
      (\s h x ->
         GetCloudFrontOriginAccessIdentityResponse' <$> (h .#? "ETag") <*>
         (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath GetCloudFrontOriginAccessIdentity where
  toPath GetCloudFrontOriginAccessIdentity' {..} =
    mconcat ["/2017-10-30/origin-access-identity/cloudfront/", toBS _gcfoaiId]
...
gcfoairsCloudFrontOriginAccessIdentity ::
     Lens' GetCloudFrontOriginAccessIdentityResponse (Maybe CloudFrontOriginAccessIdentity)
gcfoairsCloudFrontOriginAccessIdentity =
  lens
...
gcfoairsResponseStatus =
  lens _gcfoairsResponseStatus (\s a -> s {_gcfoairsResponseStatus = a})
