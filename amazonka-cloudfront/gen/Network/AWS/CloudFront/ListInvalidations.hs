...
instance AWSPager ListInvalidations where
  page rq rs
    ...
    | isNothing (rs ^? lirsInvalidationList . ilNextMarker . _Just) = Nothing
    | otherwise =
      Just $ rq & liMarker .~ rs ^? lirsInvalidationList . ilNextMarker . _Just
...
instance AWSRequest ListInvalidations where
  ...
  response =
    receiveXML
      (\s h x ->
         ListInvalidationsResponse' <$> (pure (fromEnum s)) <*> (parseXML x))
...
instance ToPath ListInvalidations where
  toPath ListInvalidations' {..} =
    mconcat
      ["/2017-10-30/distribution/", toBS _liDistributionId, "/invalidation"]
