...
instance AWSRequest GetDistributionConfig where
  ...
  response =
    receiveXML
      (\s h x ->
         GetDistributionConfigResponse' <$> (h .#? "ETag") <*> (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath GetDistributionConfig where
  toPath GetDistributionConfig' {..} =
    mconcat ["/2017-10-30/distribution/", toBS _gdcId, "/config"]
...
gdcrsDistributionConfig ::
     Lens' GetDistributionConfigResponse (Maybe DistributionConfig)
gdcrsDistributionConfig =
  lens _gdcrsDistributionConfig (\s a -> s {_gdcrsDistributionConfig = a})
