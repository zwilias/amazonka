...
instance AWSRequest GetPublicKeyConfig where
  ...
  response =
    receiveXML
      (\s h x ->
         GetPublicKeyConfigResponse' <$> (h .#? "ETag") <*> (parseXML x) <*>
         (pure (fromEnum s)))
...
instance ToPath GetPublicKeyConfig where
  toPath GetPublicKeyConfig' {..} =
    mconcat ["/2017-10-30/public-key/", toBS _gpkcId, "/config"]
...
gpkcrsPublicKeyConfig ::
     Lens' GetPublicKeyConfigResponse (Maybe PublicKeyConfig)
gpkcrsPublicKeyConfig =
  lens _gpkcrsPublicKeyConfig (\s a -> s {_gpkcrsPublicKeyConfig = a})

-- | -- | The response status code.
gpkcrsResponseStatus :: Lens' GetPublicKeyConfigResponse Int
gpkcrsResponseStatus =
  lens _gpkcrsResponseStatus (\s a -> s {_gpkcrsResponseStatus = a})
