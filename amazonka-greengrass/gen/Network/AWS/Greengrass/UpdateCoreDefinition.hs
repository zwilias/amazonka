...
ucdCoreDefinitionId =
  lens _ucdCoreDefinitionId (\s a -> s {_ucdCoreDefinitionId = a})
...
instance ToPath UpdateCoreDefinition where
  toPath UpdateCoreDefinition' {..} =
    mconcat ["/greengrass/definition/cores/", toBS _ucdCoreDefinitionId]
...
ucdrsResponseStatus =
  lens _ucdrsResponseStatus (\s a -> s {_ucdrsResponseStatus = a})
