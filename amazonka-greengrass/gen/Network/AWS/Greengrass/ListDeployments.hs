...
instance AWSRequest ListDeployments where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDeploymentsResponse' <$> (x .?> "NextToken") <*>
...
instance ToPath ListDeployments where
  toPath ListDeployments' {..} =
    mconcat ["/greengrass/groups/", toBS _lGroupId, "/deployments"]
...
ldrsDeployments =
  lens _ldrsDeployments (\s a -> s {_ldrsDeployments = a}) . _Default . _Coerce
