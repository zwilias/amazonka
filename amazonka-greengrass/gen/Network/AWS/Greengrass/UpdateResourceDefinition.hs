...
urdResourceDefinitionId =
  lens _urdResourceDefinitionId (\s a -> s {_urdResourceDefinitionId = a})
...
instance AWSRequest UpdateResourceDefinition where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateResourceDefinitionResponse' <$> (pure (fromEnum s)))
...
instance ToPath UpdateResourceDefinition where
  toPath UpdateResourceDefinition' {..} =
    mconcat ["/greengrass/definition/resources/", toBS _urdResourceDefinitionId]
