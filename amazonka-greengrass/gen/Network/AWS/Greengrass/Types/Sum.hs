...
instance FromText LoggerComponent where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LoggerComponent from value: '" <>
        e <> "'. Accepted values: greengrasssystem, lambda"
...
instance FromText LoggerLevel where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LoggerLevel from value: '" <>
        e <> "'. Accepted values: debug, error, fatal, info, warn"
...
instance FromText LoggerType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LoggerType from value: '" <>
        e <> "'. Accepted values: awscloudwatch, filesystem"
...
instance FromText SoftwareToUpdate where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SoftwareToUpdate from value: '" <>
        e <> "'. Accepted values: core, ota_agent"
...
instance FromText UpdateAgentLogLevel where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UpdateAgentLogLevel from value: '" <>
        e <>
...
instance FromText UpdateTargetsArchitecture where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UpdateTargetsArchitecture from value: '" <>
        e <> "'. Accepted values: aarch64, armv7l, x86_64"
...
instance FromText UpdateTargetsOperatingSystem where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing UpdateTargetsOperatingSystem from value: '" <>
        e <> "'. Accepted values: amazon_linux, raspbian, ubuntu"
