...
dsdSubscriptionDefinitionId =
  lens
    ...
    (\s a -> s {_dsdSubscriptionDefinitionId = a})

instance AWSRequest DeleteSubscriptionDefinition where
  type Rs DeleteSubscriptionDefinition = DeleteSubscriptionDefinitionResponse
  request = delete greengrass
  response =
    receiveEmpty
      (\s h x -> DeleteSubscriptionDefinitionResponse' <$> (pure (fromEnum s)))
...
dsdrsResponseStatus =
  lens _dsdrsResponseStatus (\s a -> s {_dsdrsResponseStatus = a})
