...
instance AWSRequest AssociateRoleToGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         AssociateRoleToGroupResponse' <$> (x .?> "AssociatedAt") <*>
         (pure (fromEnum s)))
...
instance ToJSON AssociateRoleToGroup where
  toJSON AssociateRoleToGroup' {..} =
    object (catMaybes [("RoleArn" .=) <$> _artgRoleARN])
...
artgrsAssociatedAt =
  lens _artgrsAssociatedAt (\s a -> s {_artgrsAssociatedAt = a})

-- | -- | The response status code.
artgrsResponseStatus :: Lens' AssociateRoleToGroupResponse Int
artgrsResponseStatus =
  lens _artgrsResponseStatus (\s a -> s {_artgrsResponseStatus = a})
