...
instance AWSRequest GetGroupVersion where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetGroupVersionResponse' <$> (x .?> "Definition") <*> (x .?> "Arn") <*>
         (x .?> "CreationTimestamp") <*>
         (x .?> "Version") <*>
         (x .?> "Id") <*>
         (pure (fromEnum s)))
...
ggvrsCreationTimestamp =
  lens _ggvrsCreationTimestamp (\s a -> s {_ggvrsCreationTimestamp = a})
...
ggvrsResponseStatus =
  lens _ggvrsResponseStatus (\s a -> s {_ggvrsResponseStatus = a})
