...
uddDeviceDefinitionId =
  lens _uddDeviceDefinitionId (\s a -> s {_uddDeviceDefinitionId = a})
...
instance AWSRequest UpdateDeviceDefinition where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateDeviceDefinitionResponse' <$> (pure (fromEnum s)))
...
instance ToPath UpdateDeviceDefinition where
  toPath UpdateDeviceDefinition' {..} =
    mconcat ["/greengrass/definition/devices/", toBS _uddDeviceDefinitionId]
...
uddrsResponseStatus =
  lens _uddrsResponseStatus (\s a -> s {_uddrsResponseStatus = a})
