...
instance AWSRequest GetGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetGroupResponse' <$> (x .?> "LatestVersionArn") <*> (x .?> "Arn") <*>
         (x .?> "Name") <*>
         (x .?> "CreationTimestamp") <*>
         (x .?> "Id") <*>
         (x .?> "LatestVersion") <*>
...
instance ToPath GetGroup where
  toPath GetGroup' {..} = mconcat ["/greengrass/groups/", toBS _ggGroupId]
...
ggrsLatestVersionARN =
  lens _ggrsLatestVersionARN (\s a -> s {_ggrsLatestVersionARN = a})
...
ggrsCreationTimestamp =
  lens _ggrsCreationTimestamp (\s a -> s {_ggrsCreationTimestamp = a})
...
ggrsLastUpdatedTimestamp =
  lens _ggrsLastUpdatedTimestamp (\s a -> s {_ggrsLastUpdatedTimestamp = a})
