...
lldvLoggerDefinitionId =
  lens _lldvLoggerDefinitionId (\s a -> s {_lldvLoggerDefinitionId = a})

instance AWSRequest ListLoggerDefinitionVersions where
  type Rs ListLoggerDefinitionVersions = ListLoggerDefinitionVersionsResponse
  request = get greengrass
  response =
    receiveJSON
      (\s h x ->
         ListLoggerDefinitionVersionsResponse' <$> (x .?> "Versions" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListLoggerDefinitionVersions where
  toQuery ListLoggerDefinitionVersions' {..} =
    mconcat ["NextToken" =: _lldvNextToken, "MaxResults" =: _lldvMaxResults]
...
lldvrsVersions ::
     Lens' ListLoggerDefinitionVersionsResponse [VersionInformation]
lldvrsVersions =
  lens _lldvrsVersions (\s a -> s {_lldvrsVersions = a}) . _Default . _Coerce
...
lldvrsResponseStatus =
  lens _lldvrsResponseStatus (\s a -> s {_lldvrsResponseStatus = a})
