...
instance AWSRequest ResetDeployments where
  ...
  response =
    receiveJSON
      (\s h x ->
         ResetDeploymentsResponse' <$> (x .?> "DeploymentId") <*>
         (x .?> "DeploymentArn") <*>
         (pure (fromEnum s)))
...
instance ToJSON ResetDeployments where
  toJSON ResetDeployments' {..} = object (catMaybes [("Force" .=) <$> _rdForce])

instance ToPath ResetDeployments where
  toPath ResetDeployments' {..} =
    mconcat ["/greengrass/groups/", toBS _rdGroupId, "/deployments/$reset"]
