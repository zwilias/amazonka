...
instance AWSRequest ListLoggerDefinitions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListLoggerDefinitionsResponse' <$> (x .?> "NextToken") <*>
...
instance ToQuery ListLoggerDefinitions where
  toQuery ListLoggerDefinitions' {..} =
    mconcat ["NextToken" =: _lldNextToken, "MaxResults" =: _lldMaxResults]
...
lldrsDefinitions =
  lens _lldrsDefinitions (\s a -> s {_lldrsDefinitions = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lldrsResponseStatus :: Lens' ListLoggerDefinitionsResponse Int
lldrsResponseStatus =
  lens _lldrsResponseStatus (\s a -> s {_lldrsResponseStatus = a})
