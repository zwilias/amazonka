...
drdResourceDefinitionId =
  lens _drdResourceDefinitionId (\s a -> s {_drdResourceDefinitionId = a})
...
instance AWSRequest DeleteResourceDefinition where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteResourceDefinitionResponse' <$> (pure (fromEnum s)))
...
instance ToPath DeleteResourceDefinition where
  toPath DeleteResourceDefinition' {..} =
    mconcat ["/greengrass/definition/resources/", toBS _drdResourceDefinitionId]
