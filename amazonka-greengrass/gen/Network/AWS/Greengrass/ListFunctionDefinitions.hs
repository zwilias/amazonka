...
instance AWSRequest ListFunctionDefinitions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListFunctionDefinitionsResponse' <$> (x .?> "NextToken") <*>
...
instance ToQuery ListFunctionDefinitions where
  toQuery ListFunctionDefinitions' {..} =
    mconcat ["NextToken" =: _lfdNextToken, "MaxResults" =: _lfdMaxResults]
...
lfdrsDefinitions ::
     Lens' ListFunctionDefinitionsResponse [DefinitionInformation]
lfdrsDefinitions =
  lens _lfdrsDefinitions (\s a -> s {_lfdrsDefinitions = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lfdrsResponseStatus :: Lens' ListFunctionDefinitionsResponse Int
lfdrsResponseStatus =
  lens _lfdrsResponseStatus (\s a -> s {_lfdrsResponseStatus = a})
