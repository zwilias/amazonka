...
dfdFunctionDefinitionId =
  lens _dfdFunctionDefinitionId (\s a -> s {_dfdFunctionDefinitionId = a})
...
instance AWSRequest DeleteFunctionDefinition where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteFunctionDefinitionResponse' <$> (pure (fromEnum s)))
...
instance ToPath DeleteFunctionDefinition where
  toPath DeleteFunctionDefinition' {..} =
    mconcat ["/greengrass/definition/functions/", toBS _dfdFunctionDefinitionId]
...
dfdrsResponseStatus =
  lens _dfdrsResponseStatus (\s a -> s {_dfdrsResponseStatus = a})
