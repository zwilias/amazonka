...
instance AWSRequest ListSubscriptionDefinitions where
  type Rs ListSubscriptionDefinitions = ListSubscriptionDefinitionsResponse
  request = get greengrass
  response =
    receiveJSON
      (\s h x ->
         ListSubscriptionDefinitionsResponse' <$> (x .?> "NextToken") <*>
...
instance ToQuery ListSubscriptionDefinitions where
  toQuery ListSubscriptionDefinitions' {..} =
    mconcat ["NextToken" =: _lsdNextToken, "MaxResults" =: _lsdMaxResults]
...
lsdrsDefinitions ::
     Lens' ListSubscriptionDefinitionsResponse [DefinitionInformation]
lsdrsDefinitions =
  lens _lsdrsDefinitions (\s a -> s {_lsdrsDefinitions = a}) .
  _Default . _Coerce
