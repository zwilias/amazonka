...
dddDeviceDefinitionId =
  lens _dddDeviceDefinitionId (\s a -> s {_dddDeviceDefinitionId = a})
...
instance AWSRequest DeleteDeviceDefinition where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteDeviceDefinitionResponse' <$> (pure (fromEnum s)))
...
instance ToPath DeleteDeviceDefinition where
  toPath DeleteDeviceDefinition' {..} =
    mconcat ["/greengrass/definition/devices/", toBS _dddDeviceDefinitionId]
...
dddrsResponseStatus =
  lens _dddrsResponseStatus (\s a -> s {_dddrsResponseStatus = a})
