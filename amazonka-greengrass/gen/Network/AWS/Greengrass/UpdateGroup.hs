...
instance AWSRequest UpdateGroup where
  ...
  response =
    receiveEmpty (\s h x -> UpdateGroupResponse' <$> (pure (fromEnum s)))
...
instance ToJSON UpdateGroup where
  toJSON UpdateGroup' {..} = object (catMaybes [("Name" .=) <$> _ugName])

instance ToPath UpdateGroup where
  toPath UpdateGroup' {..} = mconcat ["/greengrass/groups/", toBS _ugGroupId]
