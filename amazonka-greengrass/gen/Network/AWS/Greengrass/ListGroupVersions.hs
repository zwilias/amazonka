...
instance AWSRequest ListGroupVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListGroupVersionsResponse' <$> (x .?> "Versions" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListGroupVersions where
  toQuery ListGroupVersions' {..} =
    mconcat ["NextToken" =: _lgvNextToken, "MaxResults" =: _lgvMaxResults]
...
lgvrsVersions =
  lens _lgvrsVersions (\s a -> s {_lgvrsVersions = a}) . _Default . _Coerce
...
lgvrsResponseStatus =
  lens _lgvrsResponseStatus (\s a -> s {_lgvrsResponseStatus = a})
