...
gcdCoreDefinitionId =
  lens _gcdCoreDefinitionId (\s a -> s {_gcdCoreDefinitionId = a})
...
instance AWSRequest GetCoreDefinition where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetCoreDefinitionResponse' <$> (x .?> "LatestVersionArn") <*>
         (x .?> "Arn") <*>
         (x .?> "Name") <*>
         (x .?> "CreationTimestamp") <*>
         (x .?> "Id") <*>
...
instance ToPath GetCoreDefinition where
  toPath GetCoreDefinition' {..} =
    mconcat ["/greengrass/definition/cores/", toBS _gcdCoreDefinitionId]
...
gcdrsLatestVersionARN =
  lens _gcdrsLatestVersionARN (\s a -> s {_gcdrsLatestVersionARN = a})
...
gcdrsCreationTimestamp =
  lens _gcdrsCreationTimestamp (\s a -> s {_gcdrsCreationTimestamp = a})
...
gcdrsLatestVersion =
  lens _gcdrsLatestVersion (\s a -> s {_gcdrsLatestVersion = a})

-- | The time, in milliseconds since the epoch, when the definition was last updated.
gcdrsLastUpdatedTimestamp :: Lens' GetCoreDefinitionResponse (Maybe Text)
gcdrsLastUpdatedTimestamp =
  lens _gcdrsLastUpdatedTimestamp (\s a -> s {_gcdrsLastUpdatedTimestamp = a})

-- | -- | The response status code.
gcdrsResponseStatus :: Lens' GetCoreDefinitionResponse Int
gcdrsResponseStatus =
  lens _gcdrsResponseStatus (\s a -> s {_gcdrsResponseStatus = a})
