...
grdResourceDefinitionId =
  lens _grdResourceDefinitionId (\s a -> s {_grdResourceDefinitionId = a})
...
instance AWSRequest GetResourceDefinition where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetResourceDefinitionResponse' <$> (x .?> "LatestVersionArn") <*>
         (x .?> "Arn") <*>
         (x .?> "Name") <*>
         (x .?> "CreationTimestamp") <*>
         (x .?> "Id") <*>
...
instance ToPath GetResourceDefinition where
  toPath GetResourceDefinition' {..} =
    mconcat ["/greengrass/definition/resources/", toBS _grdResourceDefinitionId]
...
grdrsLatestVersionARN =
  lens _grdrsLatestVersionARN (\s a -> s {_grdrsLatestVersionARN = a})
...
grdrsCreationTimestamp =
  lens _grdrsCreationTimestamp (\s a -> s {_grdrsCreationTimestamp = a})
...
grdrsLatestVersion =
  lens _grdrsLatestVersion (\s a -> s {_grdrsLatestVersion = a})

-- | The time, in milliseconds since the epoch, when the definition was last updated.
grdrsLastUpdatedTimestamp :: Lens' GetResourceDefinitionResponse (Maybe Text)
grdrsLastUpdatedTimestamp =
  lens _grdrsLastUpdatedTimestamp (\s a -> s {_grdrsLastUpdatedTimestamp = a})

-- | -- | The response status code.
grdrsResponseStatus :: Lens' GetResourceDefinitionResponse Int
grdrsResponseStatus =
  lens _grdrsResponseStatus (\s a -> s {_grdrsResponseStatus = a})
