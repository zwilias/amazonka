...
instance AWSRequest ListDeviceDefinitions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDeviceDefinitionsResponse' <$> (x .?> "NextToken") <*>
...
instance ToQuery ListDeviceDefinitions where
  toQuery ListDeviceDefinitions' {..} =
    mconcat ["NextToken" =: _lddNextToken, "MaxResults" =: _lddMaxResults]
...
lddrsDefinitions =
  lens _lddrsDefinitions (\s a -> s {_lddrsDefinitions = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lddrsResponseStatus :: Lens' ListDeviceDefinitionsResponse Int
lddrsResponseStatus =
  lens _lddrsResponseStatus (\s a -> s {_lddrsResponseStatus = a})
