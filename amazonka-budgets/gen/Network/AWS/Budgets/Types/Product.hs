...
bCostFilters =
  lens _bCostFilters (\s a -> s {_bCostFilters = a}) . _Default . _Map
...
instance FromJSON Budget where
  parseJSON =
    withObject
      "Budget"
      (\x ->
         Budget' <$> (x .:? "CalculatedSpend") <*> (x .:? "BudgetLimit") <*>
         (x .:? "TimePeriod") <*>
         (x .:? "CostTypes") <*>
...
instance FromJSON CalculatedSpend where
  parseJSON =
    withObject
      "CalculatedSpend"
      (\x ->
         CalculatedSpend' <$> (x .:? "ForecastedSpend") <*> (x .: "ActualSpend"))
...
ctIncludeRecurring =
  lens _ctIncludeRecurring (\s a -> s {_ctIncludeRecurring = a})
...
ctIncludeSubscription =
  lens _ctIncludeSubscription (\s a -> s {_ctIncludeSubscription = a})
...
ctIncludeOtherSubscription =
  lens _ctIncludeOtherSubscription (\s a -> s {_ctIncludeOtherSubscription = a})
...
instance FromJSON CostTypes where
  parseJSON =
    withObject
      "CostTypes"
      (\x ->
         CostTypes' <$> (x .:? "UseAmortized") <*> (x .:? "IncludeRecurring") <*>
         (x .:? "UseBlended") <*>
         (x .:? "IncludeSupport") <*>
         (x .:? "IncludeDiscount") <*>
...
nComparisonOperator =
  lens _nComparisonOperator (\s a -> s {_nComparisonOperator = a})
...
instance FromJSON Notification where
  parseJSON =
    withObject
      "Notification"
      (\x ->
         Notification' <$> (x .:? "ThresholdType") <*> (x .: "NotificationType") <*>
...
instance FromJSON Spend where
  parseJSON =
    withObject "Spend" (\x -> Spend' <$> (x .: "Amount") <*> (x .: "Unit"))
...
instance ToJSON Spend where
  toJSON Spend' {..} =
    object (catMaybes [Just ("Amount" .= _sAmount), Just ("Unit" .= _sUnit)])
...
instance FromJSON Subscriber where
  parseJSON =
    withObject
      "Subscriber"
      (\x -> Subscriber' <$> (x .: "SubscriptionType") <*> (x .: "Address"))
...
instance ToJSON TimePeriod where
  toJSON TimePeriod' {..} =
    object (catMaybes [("Start" .=) <$> _tpStart, ("End" .=) <$> _tpEnd])
