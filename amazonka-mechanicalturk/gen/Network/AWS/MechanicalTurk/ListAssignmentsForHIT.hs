...
lafhitAssignmentStatuses =
  lens _lafhitAssignmentStatuses (\s a -> s {_lafhitAssignmentStatuses = a}) .
  _Default . _Coerce
...
lafhitMaxResults =
  lens _lafhitMaxResults (\s a -> s {_lafhitMaxResults = a}) . mapping _Nat
...
instance AWSPager ListAssignmentsForHIT where
  page rq rs
    ...
    | otherwise = Just $ rq & lafhitNextToken .~ rs ^. lafhitrsNextToken
...
instance AWSRequest ListAssignmentsForHIT where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAssignmentsForHITResponse' <$> (x .?> "NextToken") <*>
         (x .?> "NumResults") <*>
...
instance ToHeaders ListAssignmentsForHIT where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.ListAssignmentsForHIT" :: ByteString)
...
lafhitrsNumResults =
  lens _lafhitrsNumResults (\s a -> s {_lafhitrsNumResults = a})

-- | The collection of Assignment data structures returned by this call.
lafhitrsAssignments :: Lens' ListAssignmentsForHITResponse [Assignment]
lafhitrsAssignments =
  lens _lafhitrsAssignments (\s a -> s {_lafhitrsAssignments = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lafhitrsResponseStatus :: Lens' ListAssignmentsForHITResponse Int
lafhitrsResponseStatus =
  lens _lafhitrsResponseStatus (\s a -> s {_lafhitrsResponseStatus = a})
