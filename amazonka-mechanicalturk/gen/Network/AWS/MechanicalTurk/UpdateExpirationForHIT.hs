...
instance AWSRequest UpdateExpirationForHIT where
  ...
  response =
    receiveEmpty
      (\s h x -> UpdateExpirationForHITResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders UpdateExpirationForHIT where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.UpdateExpirationForHIT" :: ByteString)
...
instance ToJSON UpdateExpirationForHIT where
  toJSON UpdateExpirationForHIT' {..} =
    object
      (catMaybes
         [Just ("HITId" .= _uefhitHITId), Just ("ExpireAt" .= _uefhitExpireAt)])
...
uefhitrsResponseStatus =
  lens _uefhitrsResponseStatus (\s a -> s {_uefhitrsResponseStatus = a})
