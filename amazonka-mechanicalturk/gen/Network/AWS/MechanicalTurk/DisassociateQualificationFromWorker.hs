...
dqfwQualificationTypeId =
  lens _dqfwQualificationTypeId (\s a -> s {_dqfwQualificationTypeId = a})

instance AWSRequest DisassociateQualificationFromWorker where
  type Rs DisassociateQualificationFromWorker = DisassociateQualificationFromWorkerResponse
  request = postJSON mechanicalTurk
  response =
    receiveEmpty
      (\s h x ->
         DisassociateQualificationFromWorkerResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DisassociateQualificationFromWorker where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.DisassociateQualificationFromWorker" :: ByteString)
...
dqfwrsResponseStatus =
  lens _dqfwrsResponseStatus (\s a -> s {_dqfwrsResponseStatus = a})
