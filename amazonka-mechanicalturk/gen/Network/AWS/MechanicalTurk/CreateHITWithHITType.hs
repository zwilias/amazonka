...
chitwhittHITReviewPolicy =
  lens _chitwhittHITReviewPolicy (\s a -> s {_chitwhittHITReviewPolicy = a})
...
chitwhittUniqueRequestToken =
  lens
    ...
    (\s a -> s {_chitwhittUniqueRequestToken = a})
...
chitwhittRequesterAnnotation =
  lens
    ...
    (\s a -> s {_chitwhittRequesterAnnotation = a})

-- | The number of times the HIT can be accepted and completed before the HIT becomes unavailable.
chitwhittMaxAssignments :: Lens' CreateHITWithHITType (Maybe Int)
chitwhittMaxAssignments =
  lens _chitwhittMaxAssignments (\s a -> s {_chitwhittMaxAssignments = a})
...
chitwhittHITLayoutId =
  lens _chitwhittHITLayoutId (\s a -> s {_chitwhittHITLayoutId = a})
...
chitwhittHITLayoutParameters =
  lens
    ...
    (\s a -> s {_chitwhittHITLayoutParameters = a}) .
  _Default . _Coerce
...
chitwhittAssignmentReviewPolicy ::
     Lens' CreateHITWithHITType (Maybe ReviewPolicy)
chitwhittAssignmentReviewPolicy =
  lens
    _chitwhittAssignmentReviewPolicy
    (\s a -> s {_chitwhittAssignmentReviewPolicy = a})

-- | The HIT type ID you want to create this HIT with.
chitwhittHITTypeId :: Lens' CreateHITWithHITType Text
chitwhittHITTypeId =
  lens _chitwhittHITTypeId (\s a -> s {_chitwhittHITTypeId = a})

-- | An amount of time, in seconds, after which the HIT is no longer available for users to accept. After the lifetime of the HIT elapses, the HIT no longer appears in HIT searches, even if not all of the assignments for the HIT have been accepted.
chitwhittLifetimeInSeconds :: Lens' CreateHITWithHITType Integer
chitwhittLifetimeInSeconds =
  lens _chitwhittLifetimeInSeconds (\s a -> s {_chitwhittLifetimeInSeconds = a})
...
instance AWSRequest CreateHITWithHITType where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateHITWithHITTypeResponse' <$> (x .?> "HIT") <*> (pure (fromEnum s)))
...
instance ToHeaders CreateHITWithHITType where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.CreateHITWithHITType" :: ByteString)
...
chitwhittrsResponseStatus =
  lens _chitwhittrsResponseStatus (\s a -> s {_chitwhittrsResponseStatus = a})
