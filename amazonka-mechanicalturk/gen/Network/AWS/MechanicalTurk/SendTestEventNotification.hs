...
instance AWSRequest SendTestEventNotification where
  type Rs SendTestEventNotification = SendTestEventNotificationResponse
  request = postJSON mechanicalTurk
  response =
    receiveEmpty
      (\s h x -> SendTestEventNotificationResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders SendTestEventNotification where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.SendTestEventNotification" :: ByteString)
...
stenrsResponseStatus =
  lens _stenrsResponseStatus (\s a -> s {_stenrsResponseStatus = a})
