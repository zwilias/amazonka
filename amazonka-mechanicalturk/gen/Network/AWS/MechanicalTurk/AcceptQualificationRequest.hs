...
aqrQualificationRequestId =
  lens _aqrQualificationRequestId (\s a -> s {_aqrQualificationRequestId = a})

instance AWSRequest AcceptQualificationRequest where
  type Rs AcceptQualificationRequest = AcceptQualificationRequestResponse
  request = postJSON mechanicalTurk
  response =
    receiveEmpty
      (\s h x -> AcceptQualificationRequestResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders AcceptQualificationRequest where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.AcceptQualificationRequest" :: ByteString)
...
aqrrsResponseStatus =
  lens _aqrrsResponseStatus (\s a -> s {_aqrrsResponseStatus = a})
