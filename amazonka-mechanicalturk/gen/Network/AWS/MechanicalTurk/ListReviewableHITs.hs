...
lrhitMaxResults =
  lens _lrhitMaxResults (\s a -> s {_lrhitMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListReviewableHITs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListReviewableHITsResponse' <$> (x .?> "NextToken") <*>
         (x .?> "NumResults") <*>
         (x .?> "HITs" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListReviewableHITs where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.ListReviewableHITs" :: ByteString)
...
lrhitrsHITs =
  lens _lrhitrsHITs (\s a -> s {_lrhitrsHITs = a}) . _Default . _Coerce

-- | -- | The response status code.
lrhitrsResponseStatus :: Lens' ListReviewableHITsResponse Int
lrhitrsResponseStatus =
  lens _lrhitrsResponseStatus (\s a -> s {_lrhitrsResponseStatus = a})
