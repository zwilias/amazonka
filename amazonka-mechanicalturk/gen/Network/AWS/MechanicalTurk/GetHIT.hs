...
instance AWSRequest GetHIT where
  ...
  response =
    receiveJSON
      (\s h x -> GetHITResponse' <$> (x .?> "HIT") <*> (pure (fromEnum s)))
...
instance ToJSON GetHIT where
  toJSON GetHIT' {..} = object (catMaybes [Just ("HITId" .= _ghitHITId)])
...
ghitrsResponseStatus =
  lens _ghitrsResponseStatus (\s a -> s {_ghitrsResponseStatus = a})
