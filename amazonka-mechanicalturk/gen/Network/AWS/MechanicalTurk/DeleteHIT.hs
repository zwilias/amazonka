...
instance AWSRequest DeleteHIT where
  ...
  response = receiveEmpty (\s h x -> DeleteHITResponse' <$> (pure (fromEnum s)))
...
instance ToJSON DeleteHIT where
  toJSON DeleteHIT' {..} = object (catMaybes [Just ("HITId" .= _dhitHITId)])
...
dhitrsResponseStatus =
  lens _dhitrsResponseStatus (\s a -> s {_dhitrsResponseStatus = a})
