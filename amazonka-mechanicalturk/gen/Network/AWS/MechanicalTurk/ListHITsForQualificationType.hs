...
lhitfqtMaxResults =
  lens _lhitfqtMaxResults (\s a -> s {_lhitfqtMaxResults = a}) . mapping _Nat

-- | The ID of the Qualification type to use when querying HITs.
lhitfqtQualificationTypeId :: Lens' ListHITsForQualificationType Text
lhitfqtQualificationTypeId =
  lens _lhitfqtQualificationTypeId (\s a -> s {_lhitfqtQualificationTypeId = a})

instance AWSPager ListHITsForQualificationType where
  page rq rs
    | stop (rs ^. lhitfqtrsNextToken) = Nothing
    | stop (rs ^. lhitfqtrsHITs) = Nothing
    | otherwise = Just $ rq & lhitfqtNextToken .~ rs ^. lhitfqtrsNextToken

instance AWSRequest ListHITsForQualificationType where
  type Rs ListHITsForQualificationType = ListHITsForQualificationTypeResponse
  request = postJSON mechanicalTurk
  response =
    receiveJSON
      (\s h x ->
         ListHITsForQualificationTypeResponse' <$> (x .?> "NextToken") <*>
         (x .?> "NumResults") <*>
         (x .?> "HITs" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListHITsForQualificationType where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.ListHITsForQualificationType" :: ByteString)
