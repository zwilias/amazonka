...
gqsQualificationTypeId =
  lens _gqsQualificationTypeId (\s a -> s {_gqsQualificationTypeId = a})
...
instance AWSRequest GetQualificationScore where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetQualificationScoreResponse' <$> (x .?> "Qualification") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetQualificationScore where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.GetQualificationScore" :: ByteString)
...
gqsrsQualification =
  lens _gqsrsQualification (\s a -> s {_gqsrsQualification = a})

-- | -- | The response status code.
gqsrsResponseStatus :: Lens' GetQualificationScoreResponse Int
gqsrsResponseStatus =
  lens _gqsrsResponseStatus (\s a -> s {_gqsrsResponseStatus = a})
