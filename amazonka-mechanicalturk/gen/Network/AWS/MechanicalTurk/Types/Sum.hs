...
instance FromText AssignmentStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AssignmentStatus from value: '" <>
        e <> "'. Accepted values: approved, rejected, submitted"
...
instance FromText Comparator where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Comparator from value: '" <>
        e <>
...
instance FromText HITAccessActions where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HITAccessActions from value: '" <>
        e <>
...
instance FromText HITReviewStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HITReviewStatus from value: '" <>
        e <>
...
instance FromText HITStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HITStatus from value: '" <>
        e <>
...
instance FromText NotificationTransport where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NotificationTransport from value: '" <>
        e <> "'. Accepted values: email, sns, sqs"
...
instance FromText NotifyWorkersFailureCode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing NotifyWorkersFailureCode from value: '" <>
        e <> "'. Accepted values: hardfailure, softfailure"
...
instance FromText QualificationStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing QualificationStatus from value: '" <>
        e <> "'. Accepted values: granted, revoked"
...
instance FromText QualificationTypeStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing QualificationTypeStatus from value: '" <>
...
instance FromText ReviewActionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReviewActionStatus from value: '" <>
        e <> "'. Accepted values: cancelled, failed, intended, succeeded"
...
instance FromText ReviewPolicyLevel where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReviewPolicyLevel from value: '" <>
        e <> "'. Accepted values: assignment, hit"
...
instance FromText ReviewableHITStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReviewableHITStatus from value: '" <>
        e <> "'. Accepted values: reviewable, reviewing"
