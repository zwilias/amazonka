...
lwwqtStatus ::
     Lens' ListWorkersWithQualificationType (Maybe QualificationStatus)
...
lwwqtMaxResults =
  lens _lwwqtMaxResults (\s a -> s {_lwwqtMaxResults = a}) . mapping _Nat

-- | The ID of the Qualification type of the Qualifications to return.
lwwqtQualificationTypeId :: Lens' ListWorkersWithQualificationType Text
lwwqtQualificationTypeId =
  lens _lwwqtQualificationTypeId (\s a -> s {_lwwqtQualificationTypeId = a})
...
instance AWSRequest ListWorkersWithQualificationType where
  type Rs ListWorkersWithQualificationType = ListWorkersWithQualificationTypeResponse
  request = postJSON mechanicalTurk
  response =
    receiveJSON
      (\s h x ->
         ListWorkersWithQualificationTypeResponse' <$> (x .?> "NextToken") <*>
         (x .?> "NumResults") <*>
...
instance ToHeaders ListWorkersWithQualificationType where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.ListWorkersWithQualificationType" :: ByteString)
...
lwwqtrsQualifications ::
     Lens' ListWorkersWithQualificationTypeResponse [Qualification]
lwwqtrsQualifications =
  lens _lwwqtrsQualifications (\s a -> s {_lwwqtrsQualifications = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lwwqtrsResponseStatus :: Lens' ListWorkersWithQualificationTypeResponse Int
lwwqtrsResponseStatus =
  lens _lwwqtrsResponseStatus (\s a -> s {_lwwqtrsResponseStatus = a})
