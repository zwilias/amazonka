...
lhitMaxResults =
  lens _lhitMaxResults (\s a -> s {_lhitMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListHITs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListHITsResponse' <$> (x .?> "NextToken") <*> (x .?> "NumResults") <*>
         (x .?> "HITs" .!@ mempty) <*>
         (pure (fromEnum s)))
...
lhitrsResponseStatus =
  lens _lhitrsResponseStatus (\s a -> s {_lhitrsResponseStatus = a})
