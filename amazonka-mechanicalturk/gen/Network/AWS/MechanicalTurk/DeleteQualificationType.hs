...
dqtQualificationTypeId =
  lens _dqtQualificationTypeId (\s a -> s {_dqtQualificationTypeId = a})
...
instance AWSRequest DeleteQualificationType where
  ...
  response =
    receiveEmpty
      (\s h x -> DeleteQualificationTypeResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders DeleteQualificationType where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.DeleteQualificationType" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON DeleteQualificationType where
  toJSON DeleteQualificationType' {..} =
    object (catMaybes [Just ("QualificationTypeId" .= _dqtQualificationTypeId)])
...
dqtrsResponseStatus =
  lens _dqtrsResponseStatus (\s a -> s {_dqtrsResponseStatus = a})
