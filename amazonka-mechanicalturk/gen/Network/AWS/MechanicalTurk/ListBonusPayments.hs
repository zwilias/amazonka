...
lbpMaxResults =
  lens _lbpMaxResults (\s a -> s {_lbpMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListBonusPayments where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListBonusPaymentsResponse' <$> (x .?> "BonusPayments" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (x .?> "NumResults") <*>
         (pure (fromEnum s)))
...
lbprsBonusPayments =
  lens _lbprsBonusPayments (\s a -> s {_lbprsBonusPayments = a}) .
  _Default . _Coerce
...
lbprsResponseStatus =
  lens _lbprsResponseStatus (\s a -> s {_lbprsResponseStatus = a})
