...
uqtTestDurationInSeconds =
  lens _uqtTestDurationInSeconds (\s a -> s {_uqtTestDurationInSeconds = a})

-- | The new status of the Qualification type - Active | Inactive
uqtQualificationTypeStatus ::
     Lens' UpdateQualificationType (Maybe QualificationTypeStatus)
uqtQualificationTypeStatus =
  lens _uqtQualificationTypeStatus (\s a -> s {_uqtQualificationTypeStatus = a})
...
uqtAutoGrantedValue =
  lens _uqtAutoGrantedValue (\s a -> s {_uqtAutoGrantedValue = a})
...
uqtRetryDelayInSeconds =
  lens _uqtRetryDelayInSeconds (\s a -> s {_uqtRetryDelayInSeconds = a})

-- | The ID of the Qualification type to update.
uqtQualificationTypeId :: Lens' UpdateQualificationType Text
uqtQualificationTypeId =
  lens _uqtQualificationTypeId (\s a -> s {_uqtQualificationTypeId = a})
...
instance AWSRequest UpdateQualificationType where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateQualificationTypeResponse' <$> (x .?> "QualificationType") <*>
         (pure (fromEnum s)))
...
instance ToHeaders UpdateQualificationType where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.UpdateQualificationType" :: ByteString)
...
uqtrsQualificationType ::
     Lens' UpdateQualificationTypeResponse (Maybe QualificationType)
uqtrsQualificationType =
  lens _uqtrsQualificationType (\s a -> s {_uqtrsQualificationType = a})

-- | -- | The response status code.
uqtrsResponseStatus :: Lens' UpdateQualificationTypeResponse Int
uqtrsResponseStatus =
  lens _uqtrsResponseStatus (\s a -> s {_uqtrsResponseStatus = a})
