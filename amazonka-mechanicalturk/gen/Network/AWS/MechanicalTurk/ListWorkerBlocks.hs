...
lwbMaxResults =
  lens _lwbMaxResults (\s a -> s {_lwbMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListWorkerBlocks where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListWorkerBlocksResponse' <$> (x .?> "WorkerBlocks" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (x .?> "NumResults") <*>
         (pure (fromEnum s)))
...
lwbrsWorkerBlocks =
  lens _lwbrsWorkerBlocks (\s a -> s {_lwbrsWorkerBlocks = a}) .
  _Default . _Coerce
...
lwbrsResponseStatus =
  lens _lwbrsResponseStatus (\s a -> s {_lwbrsResponseStatus = a})
