...
cqtTestDurationInSeconds =
  lens _cqtTestDurationInSeconds (\s a -> s {_cqtTestDurationInSeconds = a})
...
cqtAutoGrantedValue =
  lens _cqtAutoGrantedValue (\s a -> s {_cqtAutoGrantedValue = a})
...
cqtRetryDelayInSeconds =
  lens _cqtRetryDelayInSeconds (\s a -> s {_cqtRetryDelayInSeconds = a})
...
cqtQualificationTypeStatus ::
     Lens' CreateQualificationType QualificationTypeStatus
cqtQualificationTypeStatus =
  lens _cqtQualificationTypeStatus (\s a -> s {_cqtQualificationTypeStatus = a})
...
instance AWSRequest CreateQualificationType where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateQualificationTypeResponse' <$> (x .?> "QualificationType") <*>
         (pure (fromEnum s)))
...
instance ToHeaders CreateQualificationType where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("MTurkRequesterServiceV20170117.CreateQualificationType" :: ByteString)
...
cqtrsQualificationType ::
     Lens' CreateQualificationTypeResponse (Maybe QualificationType)
cqtrsQualificationType =
  lens _cqtrsQualificationType (\s a -> s {_cqtrsQualificationType = a})

-- | -- | The response status code.
cqtrsResponseStatus :: Lens' CreateQualificationTypeResponse Int
cqtrsResponseStatus =
  lens _cqtrsResponseStatus (\s a -> s {_cqtrsResponseStatus = a})
