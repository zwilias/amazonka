...
instance AWSRequest DescribeAddresses where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeAddressesResponse' <$> (x .?> "Addresses" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeAddresses where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSIESnowballJobManagementService.DescribeAddresses" :: ByteString)
...
drsAddresses =
  lens _drsAddresses (\s a -> s {_drsAddresses = a}) . _Default . _Coerce
