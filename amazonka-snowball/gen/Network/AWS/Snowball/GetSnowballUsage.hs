...
instance AWSRequest GetSnowballUsage where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetSnowballUsageResponse' <$> (x .?> "SnowballsInUse") <*>
         (x .?> "SnowballLimit") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetSnowballUsage where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSIESnowballJobManagementService.GetSnowballUsage" :: ByteString)
...
gsursSnowballsInUse =
  lens _gsursSnowballsInUse (\s a -> s {_gsursSnowballsInUse = a})
...
gsursSnowballLimit =
  lens _gsursSnowballLimit (\s a -> s {_gsursSnowballLimit = a})

-- | -- | The response status code.
gsursResponseStatus :: Lens' GetSnowballUsageResponse Int
gsursResponseStatus =
  lens _gsursResponseStatus (\s a -> s {_gsursResponseStatus = a})
