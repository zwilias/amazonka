...
instance FromText JobState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing JobState from value: '" <>
        e <>
...
instance FromText JobType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing JobType from value: '" <>
        e <> "'. Accepted values: export, import, local_use"
...
instance FromText ShippingOption where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ShippingOption from value: '" <>
        e <> "'. Accepted values: express, next_day, second_day, standard"
...
instance FromText SnowballCapacity where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SnowballCapacity from value: '" <>
        e <> "'. Accepted values: nopreference, t100, t50, t80"
...
instance FromText SnowballType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing SnowballType from value: '" <>
        e <> "'. Accepted values: edge, standard"
