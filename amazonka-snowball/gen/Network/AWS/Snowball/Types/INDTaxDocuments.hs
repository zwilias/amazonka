...
newtype INDTaxDocuments = INDTaxDocuments'{_indtdGSTIN
                                           :: Maybe Text}
...
instance FromJSON INDTaxDocuments where
        parseJSON
          = withObject "INDTaxDocuments"
              (\ x -> INDTaxDocuments' <$> (x .:? "GSTIN"))

instance Hashable INDTaxDocuments where

instance NFData INDTaxDocuments where
