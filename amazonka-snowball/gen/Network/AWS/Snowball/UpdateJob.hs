...
ujForwardingAddressId =
  lens _ujForwardingAddressId (\s a -> s {_ujForwardingAddressId = a})
...
ujSnowballCapacityPreference =
  lens
    ...
    (\s a -> s {_ujSnowballCapacityPreference = a})
...
instance AWSRequest UpdateJob where
  ...
  response = receiveEmpty (\s h x -> UpdateJobResponse' <$> (pure (fromEnum s)))
  ...
  toJSON UpdateJob' {..} =
    object
      (catMaybes
         ...
         , ("RoleARN" .=) <$> _ujRoleARN
         , ("SnowballCapacityPreference" .=) <$> _ujSnowballCapacityPreference
         , Just ("JobId" .= _ujJobId)
         ])
