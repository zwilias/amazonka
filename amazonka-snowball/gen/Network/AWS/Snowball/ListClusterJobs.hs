...
lcjMaxResults =
  lens _lcjMaxResults (\s a -> s {_lcjMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListClusterJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListClusterJobsResponse' <$> (x .?> "JobListEntries" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListClusterJobs where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSIESnowballJobManagementService.ListClusterJobs" :: ByteString)
...
lcjrsJobListEntries =
  lens _lcjrsJobListEntries (\s a -> s {_lcjrsJobListEntries = a}) .
  _Default . _Coerce
...
lcjrsResponseStatus =
  lens _lcjrsResponseStatus (\s a -> s {_lcjrsResponseStatus = a})
