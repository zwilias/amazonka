...
cjForwardingAddressId =
  lens _cjForwardingAddressId (\s a -> s {_cjForwardingAddressId = a})
...
cjSnowballCapacityPreference =
  lens
    ...
    (\s a -> s {_cjSnowballCapacityPreference = a})
...
instance AWSRequest CreateJob where
  ...
  response =
    receiveJSON
      (\s h x -> CreateJobResponse' <$> (x .?> "JobId") <*> (pure (fromEnum s)))
  ...
  toJSON CreateJob' {..} =
    object
      (catMaybes
         ...
         , ("RoleARN" .=) <$> _cjRoleARN
         , ("SnowballCapacityPreference" .=) <$> _cjSnowballCapacityPreference
         ])
