...
instance AWSRequest GetJobUnlockCode where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetJobUnlockCodeResponse' <$> (x .?> "UnlockCode") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetJobUnlockCode where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSIESnowballJobManagementService.GetJobUnlockCode" :: ByteString)
...
gjucrsResponseStatus =
  lens _gjucrsResponseStatus (\s a -> s {_gjucrsResponseStatus = a})
