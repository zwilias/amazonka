...
instance AWSRequest ListChannels where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListChannelsResponse' <$> (x .?> "channelSummaries" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListChannels where
  toQuery ListChannels' {..} =
    mconcat ["nextToken" =: _lcNextToken, "maxResults" =: _lcMaxResults]
...
lcrsChannelSummaries =
  lens _lcrsChannelSummaries (\s a -> s {_lcrsChannelSummaries = a}) .
  _Default . _Coerce
