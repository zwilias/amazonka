...
rpaPipelineActivity =
  lens _rpaPipelineActivity (\s a -> s {_rpaPipelineActivity = a})
...
instance AWSRequest RunPipelineActivity where
  ...
  response =
    receiveJSON
      (\s h x ->
         RunPipelineActivityResponse' <$> (x .?> "logResult") <*>
         (x .?> "payloads") <*>
         (pure (fromEnum s)))
...
rparsPayloads =
  lens _rparsPayloads (\s a -> s {_rparsPayloads = a}) . mapping _List1

-- | -- | The response status code.
rparsResponseStatus :: Lens' RunPipelineActivityResponse Int
rparsResponseStatus =
  lens _rparsResponseStatus (\s a -> s {_rparsResponseStatus = a})
