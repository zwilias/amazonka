...
instance AWSRequest ListPipelines where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPipelinesResponse' <$> (x .?> "pipelineSummaries" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListPipelines where
  toQuery ListPipelines' {..} =
    mconcat ["nextToken" =: _lpNextToken, "maxResults" =: _lpMaxResults]
...
lprsPipelineSummaries =
  lens _lprsPipelineSummaries (\s a -> s {_lprsPipelineSummaries = a}) .
  _Default . _Coerce
