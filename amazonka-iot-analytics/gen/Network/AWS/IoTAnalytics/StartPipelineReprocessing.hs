...
sprStartTime =
  lens _sprStartTime (\s a -> s {_sprStartTime = a}) . mapping _Time
...
instance AWSRequest StartPipelineReprocessing where
  type Rs StartPipelineReprocessing = StartPipelineReprocessingResponse
  request = postJSON ioTAnalytics
  response =
    receiveJSON
      (\s h x ->
         StartPipelineReprocessingResponse' <$> (x .?> "reprocessingId") <*>
         (pure (fromEnum s)))
...
instance ToJSON StartPipelineReprocessing where
  toJSON StartPipelineReprocessing' {..} =
    object
      (catMaybes
         [("startTime" .=) <$> _sprStartTime, ("endTime" .=) <$> _sprEndTime])
...
sprrsReprocessingId =
  lens _sprrsReprocessingId (\s a -> s {_sprrsReprocessingId = a})

-- | -- | The response status code.
sprrsResponseStatus :: Lens' StartPipelineReprocessingResponse Int
sprrsResponseStatus =
  lens _sprrsResponseStatus (\s a -> s {_sprrsResponseStatus = a})
