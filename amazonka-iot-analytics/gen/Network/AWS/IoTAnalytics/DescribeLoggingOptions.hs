...
instance AWSRequest DescribeLoggingOptions where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeLoggingOptionsResponse' <$> (x .?> "loggingOptions") <*>
         (pure (fromEnum s)))
...
dlorsLoggingOptions ::
     Lens' DescribeLoggingOptionsResponse (Maybe LoggingOptions)
dlorsLoggingOptions =
  lens _dlorsLoggingOptions (\s a -> s {_dlorsLoggingOptions = a})

-- | -- | The response status code.
dlorsResponseStatus :: Lens' DescribeLoggingOptionsResponse Int
dlorsResponseStatus =
  lens _dlorsResponseStatus (\s a -> s {_dlorsResponseStatus = a})
