...
instance AWSRequest GetDatasetContent where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDatasetContentResponse' <$> (x .?> "status") <*>
         (x .?> "entries" .!@ mempty) <*>
         (x .?> "timestamp") <*>
         (pure (fromEnum s)))
...
instance ToQuery GetDatasetContent where
  toQuery GetDatasetContent' {..} = mconcat ["versionId" =: _gdcVersionId]
...
gdcrsEntries =
  lens _gdcrsEntries (\s a -> s {_gdcrsEntries = a}) . _Default . _Coerce

-- | The time when the request was made.
gdcrsTimestamp :: Lens' GetDatasetContentResponse (Maybe UTCTime)
gdcrsTimestamp =
  lens _gdcrsTimestamp (\s a -> s {_gdcrsTimestamp = a}) . mapping _Time
