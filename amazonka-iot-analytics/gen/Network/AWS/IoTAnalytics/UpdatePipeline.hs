...
upPipelineActivities =
  lens _upPipelineActivities (\s a -> s {_upPipelineActivities = a}) . _List1
...
instance ToJSON UpdatePipeline where
  toJSON UpdatePipeline' {..} =
    object (catMaybes [Just ("pipelineActivities" .= _upPipelineActivities)])

instance ToPath UpdatePipeline where
  toPath UpdatePipeline' {..} = mconcat ["/pipelines/", toBS _upPipelineName]
