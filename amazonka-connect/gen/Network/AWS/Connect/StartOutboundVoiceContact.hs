...
sovcAttributes =
  lens _sovcAttributes (\s a -> s {_sovcAttributes = a}) . _Default . _Map
...
sovcSourcePhoneNumber =
  lens _sovcSourcePhoneNumber (\s a -> s {_sovcSourcePhoneNumber = a})
...
sovcDestinationPhoneNumber =
  lens _sovcDestinationPhoneNumber (\s a -> s {_sovcDestinationPhoneNumber = a})
...
instance AWSRequest StartOutboundVoiceContact where
  type Rs StartOutboundVoiceContact = StartOutboundVoiceContactResponse
  request = putJSON connect
  response =
    receiveJSON
      (\s h x ->
         StartOutboundVoiceContactResponse' <$> (x .?> "ContactId") <*>
         (pure (fromEnum s)))
...
sovcrsResponseStatus =
  lens _sovcrsResponseStatus (\s a -> s {_sovcrsResponseStatus = a})
