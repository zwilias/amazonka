...
--
--
-- For more information, see <https://docs.aws.amazon.com/connect/latest/adminguide/real-time-metrics-reports.html Real-time Metrics Reports> in the /Amazon Connect Administrator Guide/ .
--
module Network.AWS.Connect.GetCurrentMetricData
    (
    -- * Creating a Request
...
--
-- * 'gcmdFilters' - The queues, up to 100, or channels, to use to filter the metrics returned. Metric data is retrieved only for the resources associated with the queues or channels included in the filter. You can include both queue IDs and queue ARNs in the same request. The only supported channel is @VOICE@ .
--
