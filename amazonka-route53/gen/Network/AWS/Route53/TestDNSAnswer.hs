...
tdaEDNS0ClientSubnetIP =
  lens _tdaEDNS0ClientSubnetIP (\s a -> s {_tdaEDNS0ClientSubnetIP = a})

-- | If you specify an IP address for @edns0clientsubnetip@ , you can optionally specify the number of bits of the IP address that you want the checking tool to include in the DNS query. For example, if you specify @192.0.2.44@ for @edns0clientsubnetip@ and @24@ for @edns0clientsubnetmask@ , the checking tool will simulate a request from 192.0.2.0/24. The default value is 24 bits for IPv4 addresses and 64 bits for IPv6 addresses.
tdaEDNS0ClientSubnetMask :: Lens' TestDNSAnswer (Maybe Text)
tdaEDNS0ClientSubnetMask =
  lens _tdaEDNS0ClientSubnetMask (\s a -> s {_tdaEDNS0ClientSubnetMask = a})
...
instance AWSRequest TestDNSAnswer where
  ...
  response =
    receiveXML
      (\s h x ->
         TestDNSAnswerResponse' <$> (pure (fromEnum s)) <*> (x .@ "Nameserver") <*>
         (x .@ "RecordName") <*>
         (x .@ "RecordType") <*>
...
tdarsResponseStatus =
  lens _tdarsResponseStatus (\s a -> s {_tdarsResponseStatus = a})
...
tdarsRecordData =
  lens _tdarsRecordData (\s a -> s {_tdarsRecordData = a}) . _Coerce
