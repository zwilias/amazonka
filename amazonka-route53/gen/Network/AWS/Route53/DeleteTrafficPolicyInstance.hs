...
instance AWSRequest DeleteTrafficPolicyInstance where
  type Rs DeleteTrafficPolicyInstance = DeleteTrafficPolicyInstanceResponse
  request = delete route53
  response =
    receiveEmpty
      (\s h x -> DeleteTrafficPolicyInstanceResponse' <$> (pure (fromEnum s)))
...
dtpirsResponseStatus =
  lens _dtpirsResponseStatus (\s a -> s {_dtpirsResponseStatus = a})
