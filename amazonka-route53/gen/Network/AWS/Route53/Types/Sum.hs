  ...
  parser =
    takeLowerText >>= \case
      ...
      "max_reusable_delegation_sets_by_owner" ->
        pure MaxReusableDelegationSetsByOwner
      "max_traffic_policies_by_owner" -> pure MaxTrafficPoliciesByOwner
      "max_traffic_policy_instances_by_owner" ->
        pure MaxTrafficPolicyInstancesByOwner
      e ->
        fromTextError $
        "Failure parsing AccountLimitType from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: create, delete, upsert"
...
instance FromText ChangeStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ChangeStatus from value: '" <>
        e <> "'. Accepted values: insync, pending"
...
instance FromText CloudWatchRegion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CloudWatchRegion from value: '" <>
        e <>
...
instance FromText Failover where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing Failover from value: '" <>
        e <> "'. Accepted values: primary, secondary"
...
instance FromText HealthCheckRegion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HealthCheckRegion from value: '" <>
        e <>
...
instance FromText HealthCheckType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HealthCheckType from value: '" <>
        e <>
...
instance FromText HostedZoneLimitType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing HostedZoneLimitType from value: '" <>
        e <>
...
instance FromText InsufficientDataHealthStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing InsufficientDataHealthStatus from value: '" <>
        e <> "'. Accepted values: healthy, lastknownstatus, unhealthy"
...
instance FromText RecordType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing RecordType from value: '" <>
        e <>
...
instance FromText ResettableElementName where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ResettableElementName from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      "max_zones_by_reusable_delegation_set" ->
        pure MaxZonesByReusableDelegationSet
      e ->
        fromTextError $
        "Failure parsing ReusableDelegationSetLimitType from value: '" <>
        e <> "'. Accepted values: max_zones_by_reusable_delegation_set"
...
instance FromText TagResourceType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TagResourceType from value: '" <>
        e <> "'. Accepted values: healthcheck, hostedzone"
...
instance FromText VPCRegion where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VPCRegion from value: '" <>
        e <>
