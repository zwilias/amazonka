...
instance FromXML AliasTarget where
  parseXML x =
    AliasTarget' <$> (x .@ "HostedZoneId") <*> (x .@ "DNSName") <*>
...
instance ToXML Change where
  toXML Change' {..} =
    mconcat ["Action" @= _cAction, "ResourceRecordSet" @= _cResourceRecordSet]
...
instance ToXML ChangeBatch where
  toXML ChangeBatch' {..} =
    mconcat
      ["Comment" @= _cbComment, "Changes" @= toXMLList "Change" _cbChanges]
...
instance FromXML ChangeInfo where
  parseXML x =
    ChangeInfo' <$> (x .@? "Comment") <*> (x .@ "Id") <*> (x .@ "Status") <*>
    (x .@ "SubmittedAt")
...
cwacDimensions =
  lens _cwacDimensions (\s a -> s {_cwacDimensions = a}) . _Default . _Coerce

-- | For the metric that the CloudWatch alarm is associated with, the number of periods that the metric is compared to the threshold.
cwacEvaluationPeriods :: Lens' CloudWatchAlarmConfiguration Natural
cwacEvaluationPeriods =
  lens _cwacEvaluationPeriods (\s a -> s {_cwacEvaluationPeriods = a}) . _Nat
...
cwacComparisonOperator =
  lens _cwacComparisonOperator (\s a -> s {_cwacComparisonOperator = a})
...
instance FromXML DelegationSet where
  parseXML x =
    DelegationSet' <$> (x .@? "Id") <*> (x .@? "CallerReference") <*>
...
instance FromXML GeoLocation where
  parseXML x =
    GeoLocation' <$> (x .@? "SubdivisionCode") <*> (x .@? "CountryCode") <*>
    (x .@? "ContinentCode")
...
instance FromXML GeoLocationDetails where
  parseXML x =
    GeoLocationDetails' <$> (x .@? "SubdivisionName") <*>
    (x .@? "SubdivisionCode") <*>
    (x .@? "CountryName") <*>
    (x .@? "CountryCode") <*>
    (x .@? "ContinentCode") <*>
...
hcCloudWatchAlarmConfiguration ::
     Lens' HealthCheck (Maybe CloudWatchAlarmConfiguration)
hcCloudWatchAlarmConfiguration =
  lens
    _hcCloudWatchAlarmConfiguration
    (\s a -> s {_hcCloudWatchAlarmConfiguration = a})
...
hcHealthCheckVersion =
  lens _hcHealthCheckVersion (\s a -> s {_hcHealthCheckVersion = a}) . _Nat

instance FromXML HealthCheck where
  parseXML x =
    HealthCheck' <$> (x .@? "LinkedService") <*>
...
hccHealthThreshold =
  lens _hccHealthThreshold (\s a -> s {_hccHealthThreshold = a}) . mapping _Nat
...
hccInsufficientDataHealthStatus ::
     Lens' HealthCheckConfig (Maybe InsufficientDataHealthStatus)
hccInsufficientDataHealthStatus =
  lens
    _hccInsufficientDataHealthStatus
    (\s a -> s {_hccInsufficientDataHealthStatus = a})

-- | A complex type that identifies the CloudWatch alarm that you want Amazon Route 53 health checkers to use to determine whether this health check is healthy.
hccAlarmIdentifier :: Lens' HealthCheckConfig (Maybe AlarmIdentifier)
hccAlarmIdentifier =
  lens _hccAlarmIdentifier (\s a -> s {_hccAlarmIdentifier = a})
...
hccChildHealthChecks =
  lens _hccChildHealthChecks (\s a -> s {_hccChildHealthChecks = a}) .
  _Default . _Coerce
...
hccRequestInterval =
  lens _hccRequestInterval (\s a -> s {_hccRequestInterval = a}) . mapping _Nat
...
instance FromXML HealthCheckConfig where
  parseXML x =
    HealthCheckConfig' <$> (x .@? "FailureThreshold") <*> (x .@? "IPAddress") <*>
    (x .@? "EnableSNI") <*>
    (x .@? "SearchString") <*>
    (x .@? "HealthThreshold") <*>
...
instance FromXML HealthCheckObservation where
  parseXML x =
    HealthCheckObservation' <$> (x .@? "IPAddress") <*> (x .@? "StatusReport") <*>
    (x .@? "Region")
...
hzResourceRecordSetCount =
  lens _hzResourceRecordSetCount (\s a -> s {_hzResourceRecordSetCount = a})
...
instance FromXML HostedZone where
  parseXML x =
    HostedZone' <$> (x .@? "LinkedService") <*> (x .@? "Config") <*>
...
instance FromXML HostedZoneConfig where
  parseXML x = HostedZoneConfig' <$> (x .@? "PrivateZone") <*> (x .@? "Comment")
...
instance ToXML HostedZoneConfig where
  toXML HostedZoneConfig' {..} =
    mconcat ["PrivateZone" @= _hzcPrivateZone, "Comment" @= _hzcComment]
...
lsServicePrincipal =
  lens _lsServicePrincipal (\s a -> s {_lsServicePrincipal = a})
...
instance FromXML LinkedService where
  parseXML x =
    LinkedService' <$> (x .@? "ServicePrincipal") <*> (x .@? "Description")
...
qlcCloudWatchLogsLogGroupARN =
  lens
    ...
    (\s a -> s {_qlcCloudWatchLogsLogGroupARN = a})

instance FromXML QueryLoggingConfig where
  parseXML x =
    QueryLoggingConfig' <$> (x .@ "Id") <*> (x .@ "HostedZoneId") <*>
...
rrsTrafficPolicyInstanceId =
  lens _rrsTrafficPolicyInstanceId (\s a -> s {_rrsTrafficPolicyInstanceId = a})
...
rrsMultiValueAnswer =
  lens _rrsMultiValueAnswer (\s a -> s {_rrsMultiValueAnswer = a})
...
instance FromXML ResourceRecordSet where
  parseXML x =
    ResourceRecordSet' <$> (x .@? "TTL") <*>
    (x .@? "ResourceRecords" .!@ mempty >>= may (parseXMLList1 "ResourceRecord")) <*>
    (x .@? "AliasTarget") <*>
    (x .@? "Weight") <*>
...
instance ToXML ResourceRecordSet where
  toXML ResourceRecordSet' {..} =
    mconcat
      [ "TTL" @= _rrsTTL
      , "ResourceRecords" @=
        toXML (toXMLList "ResourceRecord" <$> _rrsResourceRecords)
      , "AliasTarget" @= _rrsAliasTarget
      , "Weight" @= _rrsWeight
      , "TrafficPolicyInstanceId" @= _rrsTrafficPolicyInstanceId
...
instance FromXML ResourceTagSet where
  parseXML x =
    ResourceTagSet' <$> (x .@? "ResourceId") <*> (x .@? "ResourceType") <*>
...
instance FromXML ReusableDelegationSetLimit where
  parseXML x = ReusableDelegationSetLimit' <$> (x .@ "Type") <*> (x .@ "Value")
...
srCheckedTime =
  lens _srCheckedTime (\s a -> s {_srCheckedTime = a}) . mapping _Time

instance FromXML StatusReport where
  parseXML x = StatusReport' <$> (x .@? "Status") <*> (x .@? "CheckedTime")
...
instance FromXML TrafficPolicy where
  parseXML x =
    TrafficPolicy' <$> (x .@? "Comment") <*> (x .@ "Id") <*> (x .@ "Version") <*>
    (x .@ "Name") <*>
    (x .@ "Type") <*>
    (x .@ "Document")
...
tpsLatestVersion =
  lens _tpsLatestVersion (\s a -> s {_tpsLatestVersion = a}) . _Nat

-- | The number of traffic policies that are associated with the current AWS account.
tpsTrafficPolicyCount :: Lens' TrafficPolicySummary Natural
tpsTrafficPolicyCount =
  lens _tpsTrafficPolicyCount (\s a -> s {_tpsTrafficPolicyCount = a}) . _Nat

instance FromXML TrafficPolicySummary where
  parseXML x =
    TrafficPolicySummary' <$> (x .@ "Id") <*> (x .@ "Name") <*> (x .@ "Type") <*>
    (x .@ "LatestVersion") <*>
...
instance ToXML VPC where
  toXML VPC' {..} = mconcat ["VPCRegion" @= _vpcVPCRegion, "VPCId" @= _vpcVPCId]
