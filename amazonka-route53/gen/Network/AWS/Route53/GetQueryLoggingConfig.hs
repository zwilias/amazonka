...
instance AWSRequest GetQueryLoggingConfig where
  ...
  response =
    receiveXML
      (\s h x ->
         GetQueryLoggingConfigResponse' <$> (pure (fromEnum s)) <*>
...
gqlcrsResponseStatus =
  lens _gqlcrsResponseStatus (\s a -> s {_gqlcrsResponseStatus = a})

-- | A complex type that contains information about the query logging configuration that you specified in a 'GetQueryLoggingConfig' request.
gqlcrsQueryLoggingConfig ::
     Lens' GetQueryLoggingConfigResponse QueryLoggingConfig
gqlcrsQueryLoggingConfig =
  lens _gqlcrsQueryLoggingConfig (\s a -> s {_gqlcrsQueryLoggingConfig = a})
