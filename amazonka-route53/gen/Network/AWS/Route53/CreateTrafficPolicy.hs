...
instance AWSRequest CreateTrafficPolicy where
  ...
  response =
    receiveXML
      (\s h x ->
         CreateTrafficPolicyResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "TrafficPolicy") <*>
         (h .# "Location"))
...
instance ToXML CreateTrafficPolicy where
  toXML CreateTrafficPolicy' {..} =
    mconcat
      ["Comment" @= _ctpComment, "Name" @= _ctpName, "Document" @= _ctpDocument]
...
ctprsResponseStatus =
  lens _ctprsResponseStatus (\s a -> s {_ctprsResponseStatus = a})

-- | A complex type that contains settings for the new traffic policy.
ctprsTrafficPolicy :: Lens' CreateTrafficPolicyResponse TrafficPolicy
ctprsTrafficPolicy =
  lens _ctprsTrafficPolicy (\s a -> s {_ctprsTrafficPolicy = a})
