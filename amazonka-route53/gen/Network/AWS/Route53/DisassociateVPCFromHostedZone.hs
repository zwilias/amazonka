...
instance AWSRequest DisassociateVPCFromHostedZone where
  ...
  response =
    receiveXML
      (\s h x ->
         DisassociateVPCFromHostedZoneResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "ChangeInfo"))
...
instance ToPath DisassociateVPCFromHostedZone where
  toPath DisassociateVPCFromHostedZone' {..} =
    mconcat
      ["/2013-04-01/hostedzone/", toBS _dvfhzHostedZoneId, "/disassociatevpc"]
...
dvfhzrsResponseStatus =
  lens _dvfhzrsResponseStatus (\s a -> s {_dvfhzrsResponseStatus = a})
