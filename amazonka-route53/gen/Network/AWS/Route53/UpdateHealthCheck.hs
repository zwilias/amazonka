...
uhcResetElements =
  lens _uhcResetElements (\s a -> s {_uhcResetElements = a}) .
  _Default . _Coerce
...
uhcHealthThreshold =
  lens _uhcHealthThreshold (\s a -> s {_uhcHealthThreshold = a}) . mapping _Nat
...
uhcInsufficientDataHealthStatus ::
     Lens' UpdateHealthCheck (Maybe InsufficientDataHealthStatus)
uhcInsufficientDataHealthStatus =
  lens
    _uhcInsufficientDataHealthStatus
    (\s a -> s {_uhcInsufficientDataHealthStatus = a})
...
uhcAlarmIdentifier =
  lens _uhcAlarmIdentifier (\s a -> s {_uhcAlarmIdentifier = a})
...
uhcChildHealthChecks =
  lens _uhcChildHealthChecks (\s a -> s {_uhcChildHealthChecks = a}) .
  _Default . _Coerce
...
instance AWSRequest UpdateHealthCheck where
  ...
  response =
    receiveXML
      (\s h x ->
         UpdateHealthCheckResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "HealthCheck"))
...
uhcrsResponseStatus =
  lens _uhcrsResponseStatus (\s a -> s {_uhcrsResponseStatus = a})
