...
instance AWSRequest ListGeoLocations where
  ...
  response =
    receiveXML
      (\s h x ->
         ListGeoLocationsResponse' <$> (x .@? "NextContinentCode") <*>
         (x .@? "NextCountryCode") <*>
...
lglrsNextSubdivisionCode =
  lens _lglrsNextSubdivisionCode (\s a -> s {_lglrsNextSubdivisionCode = a})

-- | -- | The response status code.
lglrsResponseStatus :: Lens' ListGeoLocationsResponse Int
lglrsResponseStatus =
  lens _lglrsResponseStatus (\s a -> s {_lglrsResponseStatus = a})

-- | A complex type that contains one @GeoLocationDetails@ element for each location that Amazon Route 53 supports for geolocation.
lglrsGeoLocationDetailsList ::
     Lens' ListGeoLocationsResponse [GeoLocationDetails]
lglrsGeoLocationDetailsList =
  lens
