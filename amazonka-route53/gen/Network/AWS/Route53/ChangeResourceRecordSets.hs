...
instance AWSRequest ChangeResourceRecordSets where
  ...
  response =
    receiveXML
      (\s h x ->
         ChangeResourceRecordSetsResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "ChangeInfo"))
...
instance ToPath ChangeResourceRecordSets where
  toPath ChangeResourceRecordSets' {..} =
    mconcat ["/2013-04-01/hostedzone/", toBS _crrsHostedZoneId, "/rrset/"]
...
crrsrsResponseStatus =
  lens _crrsrsResponseStatus (\s a -> s {_crrsrsResponseStatus = a})
