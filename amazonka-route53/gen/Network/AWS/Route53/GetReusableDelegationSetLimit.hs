...
grdslDelegationSetId =
  lens _grdslDelegationSetId (\s a -> s {_grdslDelegationSetId = a})

instance AWSRequest GetReusableDelegationSetLimit where
  type Rs GetReusableDelegationSetLimit = GetReusableDelegationSetLimitResponse
  request = get route53
  response =
    receiveXML
      (\s h x ->
         GetReusableDelegationSetLimitResponse' <$> (pure (fromEnum s)) <*>
         (x .@ "Limit") <*>
         (x .@ "Count"))
...
grdslrsResponseStatus =
  lens _grdslrsResponseStatus (\s a -> s {_grdslrsResponseStatus = a})

-- | The current setting for the limit on hosted zones that you can associate with the specified reusable delegation set.
grdslrsLimit ::
     Lens' GetReusableDelegationSetLimitResponse ReusableDelegationSetLimit
