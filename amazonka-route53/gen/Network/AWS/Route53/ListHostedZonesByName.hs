...
instance AWSRequest ListHostedZonesByName where
  ...
  response =
    receiveXML
      (\s h x ->
         ListHostedZonesByNameResponse' <$> (x .@? "HostedZoneId") <*>
         (x .@? "NextHostedZoneId") <*>
         (x .@? "DNSName") <*>
         (x .@? "NextDNSName") <*>
         (pure (fromEnum s)) <*>
...
lhzbnrsHostedZoneId =
  lens _lhzbnrsHostedZoneId (\s a -> s {_lhzbnrsHostedZoneId = a})
...
lhzbnrsNextHostedZoneId ::
     Lens' ListHostedZonesByNameResponse (Maybe ResourceId)
lhzbnrsNextHostedZoneId =
  lens _lhzbnrsNextHostedZoneId (\s a -> s {_lhzbnrsNextHostedZoneId = a})
...
lhzbnrsNextDNSName =
  lens _lhzbnrsNextDNSName (\s a -> s {_lhzbnrsNextDNSName = a})

-- | -- | The response status code.
lhzbnrsResponseStatus :: Lens' ListHostedZonesByNameResponse Int
lhzbnrsResponseStatus =
  lens _lhzbnrsResponseStatus (\s a -> s {_lhzbnrsResponseStatus = a})

-- | A complex type that contains general information about the hosted zone.
lhzbnrsHostedZones :: Lens' ListHostedZonesByNameResponse [HostedZone]
lhzbnrsHostedZones =
  lens _lhzbnrsHostedZones (\s a -> s {_lhzbnrsHostedZones = a}) . _Coerce
...
lhzbnrsIsTruncated =
  lens _lhzbnrsIsTruncated (\s a -> s {_lhzbnrsIsTruncated = a})
