...
instance AWSRequest ListQueryLoggingConfigs where
  ...
  response =
    receiveXML
      (\s h x ->
         ListQueryLoggingConfigsResponse' <$> (x .@? "NextToken") <*>
         (pure (fromEnum s)) <*>
...
lqlcrsResponseStatus =
  lens _lqlcrsResponseStatus (\s a -> s {_lqlcrsResponseStatus = a})
...
lqlcrsQueryLoggingConfigs =
  lens _lqlcrsQueryLoggingConfigs (\s a -> s {_lqlcrsQueryLoggingConfigs = a}) .
  _Coerce
