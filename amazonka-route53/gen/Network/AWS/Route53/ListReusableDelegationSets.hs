...
instance AWSRequest ListReusableDelegationSets where
  ...
  response =
    receiveXML
      (\s h x ->
         ListReusableDelegationSetsResponse' <$> (x .@? "NextMarker") <*>
         (pure (fromEnum s)) <*>
         (x .@? "DelegationSets" .!@ mempty >>= parseXMLList "DelegationSet") <*>
         (x .@ "Marker") <*>
         (x .@ "IsTruncated") <*>
         (x .@ "MaxItems"))
...
lrdsrsDelegationSets =
  lens _lrdsrsDelegationSets (\s a -> s {_lrdsrsDelegationSets = a}) . _Coerce
