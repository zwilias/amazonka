...
instance AWSRequest ListTagsForResources where
  ...
  response =
    receiveXML
      (\s h x ->
         ListTagsForResourcesResponse' <$> (pure (fromEnum s)) <*>
         (x .@? "ResourceTagSets" .!@ mempty >>= parseXMLList "ResourceTagSet"))
...
lrsResourceTagSets =
  lens _lrsResourceTagSets (\s a -> s {_lrsResourceTagSets = a}) . _Coerce
