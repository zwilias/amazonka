...
utpiTrafficPolicyId =
  lens _utpiTrafficPolicyId (\s a -> s {_utpiTrafficPolicyId = a})

-- | The version of the traffic policy that you want Amazon Route 53 to use to update resource record sets for the specified traffic policy instance.
utpiTrafficPolicyVersion :: Lens' UpdateTrafficPolicyInstance Natural
utpiTrafficPolicyVersion =
  lens _utpiTrafficPolicyVersion (\s a -> s {_utpiTrafficPolicyVersion = a}) .
  _Nat

instance AWSRequest UpdateTrafficPolicyInstance where
  type Rs UpdateTrafficPolicyInstance = UpdateTrafficPolicyInstanceResponse
  request = postXML route53
  response =
    receiveXML
      (\s h x ->
         UpdateTrafficPolicyInstanceResponse' <$> (pure (fromEnum s)) <*>
...
utpirsResponseStatus =
  lens _utpirsResponseStatus (\s a -> s {_utpirsResponseStatus = a})

-- | A complex type that contains settings for the updated traffic policy instance.
utpirsTrafficPolicyInstance ::
     Lens' UpdateTrafficPolicyInstanceResponse TrafficPolicyInstance
utpirsTrafficPolicyInstance =
  lens
    _utpirsTrafficPolicyInstance
    (\s a -> s {_utpirsTrafficPolicyInstance = a})
