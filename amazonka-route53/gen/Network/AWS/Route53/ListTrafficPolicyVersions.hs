...
ltpvTrafficPolicyVersionMarker =
  lens
    ...
    (\s a -> s {_ltpvTrafficPolicyVersionMarker = a})
...
instance AWSRequest ListTrafficPolicyVersions where
  type Rs ListTrafficPolicyVersions = ListTrafficPolicyVersionsResponse
  request = get route53
  response =
    receiveXML
      (\s h x ->
         ListTrafficPolicyVersionsResponse' <$> (pure (fromEnum s)) <*>
         (x .@? "TrafficPolicies" .!@ mempty >>= parseXMLList "TrafficPolicy") <*>
         (x .@ "IsTruncated") <*>
...
instance ToPath ListTrafficPolicyVersions where
  toPath ListTrafficPolicyVersions' {..} =
    mconcat ["/2013-04-01/trafficpolicies/", toBS _ltpvId, "/versions"]
...
ltpvrsResponseStatus =
  lens _ltpvrsResponseStatus (\s a -> s {_ltpvrsResponseStatus = a})

-- | A list that contains one @TrafficPolicy@ element for each traffic policy version that is associated with the specified traffic policy.
ltpvrsTrafficPolicies :: Lens' ListTrafficPolicyVersionsResponse [TrafficPolicy]
ltpvrsTrafficPolicies =
  lens _ltpvrsTrafficPolicies (\s a -> s {_ltpvrsTrafficPolicies = a}) . _Coerce
...
ltpvrsTrafficPolicyVersionMarker =
  lens
    ...
    (\s a -> s {_ltpvrsTrafficPolicyVersionMarker = a})
