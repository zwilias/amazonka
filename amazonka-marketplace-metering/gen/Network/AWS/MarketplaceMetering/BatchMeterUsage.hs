...
bmuUsageRecords =
  lens _bmuUsageRecords (\s a -> s {_bmuUsageRecords = a}) . _Coerce
...
instance AWSRequest BatchMeterUsage where
  ...
  response =
    receiveJSON
      (\s h x ->
         BatchMeterUsageResponse' <$> (x .?> "Results" .!@ mempty) <*>
...
bmursResults =
  lens _bmursResults (\s a -> s {_bmursResults = a}) . _Default . _Coerce

-- | Contains all UsageRecords that were not processed by BatchMeterUsage. This is a list of UsageRecords. You can retry the failed request by making another BatchMeterUsage call with this list as input in the BatchMeterUsageRequest.
bmursUnprocessedRecords :: Lens' BatchMeterUsageResponse [UsageRecord]
bmursUnprocessedRecords =
  lens _bmursUnprocessedRecords (\s a -> s {_bmursUnprocessedRecords = a}) .
  _Default . _Coerce

-- | -- | The response status code.
bmursResponseStatus :: Lens' BatchMeterUsageResponse Int
bmursResponseStatus =
  lens _bmursResponseStatus (\s a -> s {_bmursResponseStatus = a})
