...
urCustomerIdentifier =
  lens _urCustomerIdentifier (\s a -> s {_urCustomerIdentifier = a})
...
instance FromJSON UsageRecord where
  parseJSON =
    withObject
      "UsageRecord"
      (\x ->
         UsageRecord' <$> (x .: "Timestamp") <*> (x .: "CustomerIdentifier") <*>
         (x .: "Dimension") <*>
         (x .: "Quantity"))
...
urrMeteringRecordId =
  lens _urrMeteringRecordId (\s a -> s {_urrMeteringRecordId = a})
...
instance FromJSON UsageRecordResult where
  parseJSON =
    withObject
      "UsageRecordResult"
      (\x ->
         UsageRecordResult' <$> (x .:? "Status") <*> (x .:? "UsageRecord") <*>
         (x .:? "MeteringRecordId"))
