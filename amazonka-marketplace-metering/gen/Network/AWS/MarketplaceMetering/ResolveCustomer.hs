...
rcRegistrationToken =
  lens _rcRegistrationToken (\s a -> s {_rcRegistrationToken = a})
...
instance AWSRequest ResolveCustomer where
  ...
  response =
    receiveJSON
      (\s h x ->
         ResolveCustomerResponse' <$> (x .?> "CustomerIdentifier") <*>
         (x .?> "ProductCode") <*>
         (pure (fromEnum s)))
...
instance ToJSON ResolveCustomer where
  toJSON ResolveCustomer' {..} =
    object (catMaybes [Just ("RegistrationToken" .= _rcRegistrationToken)])
...
rcrsCustomerIdentifier =
  lens _rcrsCustomerIdentifier (\s a -> s {_rcrsCustomerIdentifier = a})
