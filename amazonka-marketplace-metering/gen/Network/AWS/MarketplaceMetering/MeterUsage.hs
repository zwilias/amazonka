...
muUsageQuantity =
  lens _muUsageQuantity (\s a -> s {_muUsageQuantity = a}) . _Nat
...
instance AWSRequest MeterUsage where
  ...
  response =
    receiveJSON
      (\s h x ->
         MeterUsageResponse' <$> (x .?> "MeteringRecordId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders MeterUsage where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AWSMPMeteringService.MeterUsage" :: ByteString)
...
mursMeteringRecordId =
  lens _mursMeteringRecordId (\s a -> s {_mursMeteringRecordId = a})

-- | -- | The response status code.
mursResponseStatus :: Lens' MeterUsageResponse Int
mursResponseStatus =
  lens _mursResponseStatus (\s a -> s {_mursResponseStatus = a})
