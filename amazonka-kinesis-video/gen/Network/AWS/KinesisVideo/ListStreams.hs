...
lsStreamNameCondition =
  lens _lsStreamNameCondition (\s a -> s {_lsStreamNameCondition = a})
...
instance AWSRequest ListStreams where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListStreamsResponse' <$> (x .?> "StreamInfoList" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
lsrsStreamInfoList =
  lens _lsrsStreamInfoList (\s a -> s {_lsrsStreamInfoList = a}) .
  _Default . _Coerce
