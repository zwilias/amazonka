...
siCreationTime =
  lens _siCreationTime (\s a -> s {_siCreationTime = a}) . mapping _Time
...
siDataRetentionInHours =
  lens _siDataRetentionInHours (\s a -> s {_siDataRetentionInHours = a}) .
  mapping _Nat
...
instance FromJSON StreamInfo where
  parseJSON =
    withObject
      "StreamInfo"
      (\x ->
         StreamInfo' <$> (x .:? "CreationTime") <*> (x .:? "Status") <*>
         (x .:? "MediaType") <*>
...
sncComparisonOperator =
  lens _sncComparisonOperator (\s a -> s {_sncComparisonOperator = a})

-- | A value to compare.
sncComparisonValue :: Lens' StreamNameCondition (Maybe Text)
sncComparisonValue =
  lens _sncComparisonValue (\s a -> s {_sncComparisonValue = a})
