...
instance AWSRequest CreateKeysAndCertificate where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateKeysAndCertificateResponse' <$> (x .?> "keyPair") <*>
         (x .?> "certificatePem") <*>
         (x .?> "certificateArn") <*>
         (x .?> "certificateId") <*>
         (pure (fromEnum s)))
