...
data CreateProvisioningTemplate = CreateProvisioningTemplate'{_cptPreProvisioningHook
                                                              ::
                                                              !(Maybe
                                                                  ProvisioningHook),
                                                              _cptEnabled ::
...
createProvisioningTemplate
    ...
    -> Text -- ^ 'cptTemplateBody'
    -> Text -- ^ 'cptProvisioningRoleARN'
    -> CreateProvisioningTemplate
createProvisioningTemplate pTemplateName_
  pTemplateBody_ pProvisioningRoleARN_
  = CreateProvisioningTemplate'{_cptPreProvisioningHook
                                  = Nothing,
                                _cptEnabled = Nothing,
                                _cptDescription = Nothing, _cptTags = Nothing,
...
instance AWSRequest CreateProvisioningTemplate where
        ...
        response
          = receiveJSON
              (\ s h x ->
                 CreateProvisioningTemplateResponse' <$>
                   (x .?> "templateName") <*> (x .?> "defaultVersionId")
                     <*> (x .?> "templateArn")
                     <*> (pure (fromEnum s)))

instance Hashable CreateProvisioningTemplate where

instance NFData CreateProvisioningTemplate where
...
data CreateProvisioningTemplateResponse = CreateProvisioningTemplateResponse'{_cptrsTemplateName
                                                                              ::
                                                                              !(Maybe
                                                                                  Text),
...
createProvisioningTemplateResponse pResponseStatus_
  = CreateProvisioningTemplateResponse'{_cptrsTemplateName
                                          = Nothing,
