...
utgExpectedVersion =
  lens _utgExpectedVersion (\s a -> s {_utgExpectedVersion = a})
...
utgThingGroupProperties =
  lens _utgThingGroupProperties (\s a -> s {_utgThingGroupProperties = a})
...
instance AWSRequest UpdateThingGroup where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateThingGroupResponse' <$> (x .?> "version") <*> (pure (fromEnum s)))
...
utgrsResponseStatus =
  lens _utgrsResponseStatus (\s a -> s {_utgrsResponseStatus = a})
