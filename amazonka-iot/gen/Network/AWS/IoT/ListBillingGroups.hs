...
data ListBillingGroups = ListBillingGroups'{_lbgNamePrefixFilter
                                            :: !(Maybe Text),
...
--
-- * 'lbgNextToken' - The token to retrieve the next set of results.
--
-- * 'lbgMaxResults' - The maximum number of results to return per request.
listBillingGroups
    :: ListBillingGroups
listBillingGroups
  = ListBillingGroups'{_lbgNamePrefixFilter = Nothing,
                       _lbgNextToken = Nothing, _lbgMaxResults = Nothing}
...
instance AWSPager ListBillingGroups where
        page rq rs
          ...
          | otherwise =
            Just $ rq & lbgNextToken .~ rs ^. lbgrsNextToken
...
instance Hashable ListBillingGroups where

instance NFData ListBillingGroups where
...
data ListBillingGroupsResponse = ListBillingGroupsResponse'{_lbgrsNextToken
                                                            :: !(Maybe Text),
                                                            _lbgrsBillingGroups
                                                            ::
                                                            !(Maybe
                                                                [GroupNameAndARN]),
                                                            _lbgrsResponseStatus
                                                            :: !Int}
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'lbgrsNextToken' - The token used to get the next set of results, or __null__ if there are no additional results.
--
-- * 'lbgrsBillingGroups' - The list of billing groups.
--
-- * 'lbgrsResponseStatus' - -- | The response status code.
listBillingGroupsResponse
    :: Int -- ^ 'lbgrsResponseStatus'
    -> ListBillingGroupsResponse
listBillingGroupsResponse pResponseStatus_
  = ListBillingGroupsResponse'{_lbgrsNextToken =
                                 Nothing,
...
instance NFData ListBillingGroupsResponse where
