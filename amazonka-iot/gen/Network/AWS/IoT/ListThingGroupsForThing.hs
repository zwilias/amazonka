...
ltgftMaxResults =
  lens _ltgftMaxResults (\s a -> s {_ltgftMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListThingGroupsForThing where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListThingGroupsForThingResponse' <$> (x .?> "thingGroups" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToQuery ListThingGroupsForThing where
  toQuery ListThingGroupsForThing' {..} =
    mconcat ["nextToken" =: _ltgftNextToken, "maxResults" =: _ltgftMaxResults]
...
ltgftrsThingGroups =
  lens _ltgftrsThingGroups (\s a -> s {_ltgftrsThingGroups = a}) .
  _Default . _Coerce
...
ltgftrsResponseStatus =
  lens _ltgftrsResponseStatus (\s a -> s {_ltgftrsResponseStatus = a})
