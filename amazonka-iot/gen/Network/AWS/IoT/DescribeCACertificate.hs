...
instance AWSRequest DescribeCACertificate where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeCACertificateResponse' <$> (x .?> "certificateDescription") <*>
...
dcaccrsCertificateDescription ::
     Lens' DescribeCACertificateResponse (Maybe CACertificateDescription)
dcaccrsCertificateDescription =
  lens
    _dcaccrsCertificateDescription
    (\s a -> s {_dcaccrsCertificateDescription = a})

-- | Information about the registration configuration.
dcaccrsRegistrationConfig ::
     Lens' DescribeCACertificateResponse (Maybe RegistrationConfig)
dcaccrsRegistrationConfig =
  lens _dcaccrsRegistrationConfig (\s a -> s {_dcaccrsRegistrationConfig = a})

-- | -- | The response status code.
dcaccrsResponseStatus :: Lens' DescribeCACertificateResponse Int
dcaccrsResponseStatus =
  lens _dcaccrsResponseStatus (\s a -> s {_dcaccrsResponseStatus = a})
