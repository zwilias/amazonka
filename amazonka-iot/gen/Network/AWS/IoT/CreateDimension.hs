...
data CreateDimension = CreateDimension'{_cdTags ::
                                        !(Maybe [Tag]),
                                        _cdName :: !Text,
...
createDimension
    :: Text -- ^ 'cdName'
    -> DimensionType -- ^ 'cdType'
    -> NonEmpty Text -- ^ 'cdStringValues'
    -> Text -- ^ 'cdClientRequestToken'
    -> CreateDimension
createDimension pName_ pType_ pStringValues_
  pClientRequestToken_
  = CreateDimension'{_cdTags = Nothing,
                     _cdName = pName_, _cdType = pType_,
...
instance Hashable CreateDimension where

instance NFData CreateDimension where
...
instance ToJSON CreateDimension where
        toJSON CreateDimension'{..}
          = object
              (catMaybes
                 [("tags" .=) <$> _cdTags, Just ("type" .= _cdType),
...
data CreateDimensionResponse = CreateDimensionResponse'{_cdrsArn
                                                        :: !(Maybe Text),
                                                        _cdrsName ::
                                                        !(Maybe Text),
                                                        _cdrsResponseStatus ::
...
createDimensionResponse pResponseStatus_
  = CreateDimensionResponse'{_cdrsArn = Nothing,
                             _cdrsName = Nothing,
...
instance NFData CreateDimensionResponse where
