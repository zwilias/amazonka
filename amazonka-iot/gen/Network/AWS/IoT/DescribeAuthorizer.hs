...
instance AWSRequest DescribeAuthorizer where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeAuthorizerResponse' <$> (x .?> "authorizerDescription") <*>
         (pure (fromEnum s)))
...
darsAuthorizerDescription ::
     Lens' DescribeAuthorizerResponse (Maybe AuthorizerDescription)
darsAuthorizerDescription =
  lens _darsAuthorizerDescription (\s a -> s {_darsAuthorizerDescription = a})
