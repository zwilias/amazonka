...
lptMaxResults =
  lens _lptMaxResults (\s a -> s {_lptMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListPrincipalThings where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListPrincipalThingsResponse' <$> (x .?> "nextToken") <*>
         (x .?> "things" .!@ mempty) <*>
         (pure (fromEnum s)))
...
instance ToQuery ListPrincipalThings where
  toQuery ListPrincipalThings' {..} =
    mconcat ["nextToken" =: _lptNextToken, "maxResults" =: _lptMaxResults]
...
lptrsThings =
  lens _lptrsThings (\s a -> s {_lptrsThings = a}) . _Default . _Coerce
