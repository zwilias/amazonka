...
ljeftMaxResults =
  lens _ljeftMaxResults (\s a -> s {_ljeftMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListJobExecutionsForThing where
  type Rs ListJobExecutionsForThing = ListJobExecutionsForThingResponse
  request = get ioT
  response =
    receiveJSON
      (\s h x ->
...
ljeftrsExecutionSummaries ::
     Lens' ListJobExecutionsForThingResponse [JobExecutionSummaryForThing]
ljeftrsExecutionSummaries =
  lens _ljeftrsExecutionSummaries (\s a -> s {_ljeftrsExecutionSummaries = a}) .
  _Default . _Coerce
...
ljeftrsResponseStatus =
  lens _ljeftrsResponseStatus (\s a -> s {_ljeftrsResponseStatus = a})
