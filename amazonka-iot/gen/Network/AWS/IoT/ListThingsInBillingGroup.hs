...
data ListThingsInBillingGroup = ListThingsInBillingGroup'{_ltibgNextToken
                                                          :: !(Maybe Text),
                                                          _ltibgMaxResults ::
                                                          !(Maybe Nat),
...
instance AWSPager ListThingsInBillingGroup where
        page rq rs
          ...
          | otherwise =
            Just $ rq & ltibgNextToken .~ rs ^. ltibgrsNextToken

instance AWSRequest ListThingsInBillingGroup where
        type Rs ListThingsInBillingGroup =
             ListThingsInBillingGroupResponse
        request = get ioT
        response
          = receiveJSON
...
instance Hashable ListThingsInBillingGroup where

instance NFData ListThingsInBillingGroup where
...
instance ToPath ListThingsInBillingGroup where
        toPath ListThingsInBillingGroup'{..}
          = mconcat
              ["/billing-groups/", toBS _ltibgBillingGroupName,
               "/things"]
...
data ListThingsInBillingGroupResponse = ListThingsInBillingGroupResponse'{_ltibgrsNextToken
                                                                          ::
                                                                          !(Maybe
                                                                              Text),
                                                                          _ltibgrsThings
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ltibgrsNextToken' - The token used to get the next set of results, or __null__ if there are no additional results.
--
-- * 'ltibgrsThings' - A list of things in the billing group.
--
-- * 'ltibgrsResponseStatus' - -- | The response status code.
listThingsInBillingGroupResponse
    :: Int -- ^ 'ltibgrsResponseStatus'
    -> ListThingsInBillingGroupResponse
listThingsInBillingGroupResponse pResponseStatus_
  = ListThingsInBillingGroupResponse'{_ltibgrsNextToken
                                        = Nothing,
