...
ltrtrMaxResults =
  lens _ltrtrMaxResults (\s a -> s {_ltrtrMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListThingRegistrationTaskReports where
  type Rs ListThingRegistrationTaskReports = ListThingRegistrationTaskReportsResponse
  request = get ioT
  response =
    receiveJSON
      (\s h x ->
...
instance ToPath ListThingRegistrationTaskReports where
  toPath ListThingRegistrationTaskReports' {..} =
    mconcat ["/thing-registration-tasks/", toBS _ltrtrTaskId, "/reports"]
...
ltrtrrsResourceLinks =
  lens _ltrtrrsResourceLinks (\s a -> s {_ltrtrrsResourceLinks = a}) .
  _Default . _Coerce
...
-- | The type of task report.
ltrtrrsReportType ::
     Lens' ListThingRegistrationTaskReportsResponse (Maybe ReportType)
...
ltrtrrsResponseStatus =
  lens _ltrtrrsResponseStatus (\s a -> s {_ltrtrrsResponseStatus = a})
