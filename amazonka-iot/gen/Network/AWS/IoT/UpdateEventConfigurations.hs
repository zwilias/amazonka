...
uecEventConfigurations ::
     Lens' UpdateEventConfigurations (HashMap EventType Configuration)
uecEventConfigurations =
  lens _uecEventConfigurations (\s a -> s {_uecEventConfigurations = a}) .
  _Default . _Map

instance AWSRequest UpdateEventConfigurations where
  type Rs UpdateEventConfigurations = UpdateEventConfigurationsResponse
  request = patchJSON ioT
  response =
    receiveEmpty
      (\s h x -> UpdateEventConfigurationsResponse' <$> (pure (fromEnum s)))
...
instance ToJSON UpdateEventConfigurations where
  toJSON UpdateEventConfigurations' {..} =
    object (catMaybes [("eventConfigurations" .=) <$> _uecEventConfigurations])
...
uecrsResponseStatus =
  lens _uecrsResponseStatus (\s a -> s {_uecrsResponseStatus = a})
