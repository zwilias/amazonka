...
cotauAdditionalParameters =
  lens _cotauAdditionalParameters (\s a -> s {_cotauAdditionalParameters = a}) .
  _Default . _Map
...
cotauTargetSelection =
  lens _cotauTargetSelection (\s a -> s {_cotauTargetSelection = a})
...
instance AWSRequest CreateOTAUpdate where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateOTAUpdateResponse' <$> (x .?> "awsIotJobId") <*>
         (x .?> "otaUpdateStatus") <*>
         (x .?> "awsIotJobArn") <*>
         (x .?> "otaUpdateId") <*>
         (x .?> "otaUpdateArn") <*>
...
cotaursAwsIotJobId =
  lens _cotaursAwsIotJobId (\s a -> s {_cotaursAwsIotJobId = a})

-- | The OTA update status.
cotaursOtaUpdateStatus :: Lens' CreateOTAUpdateResponse (Maybe OTAUpdateStatus)
cotaursOtaUpdateStatus =
  lens _cotaursOtaUpdateStatus (\s a -> s {_cotaursOtaUpdateStatus = a})

-- | The AWS IoT job ARN associated with the OTA update.
cotaursAwsIotJobARN :: Lens' CreateOTAUpdateResponse (Maybe Text)
cotaursAwsIotJobARN =
  lens _cotaursAwsIotJobARN (\s a -> s {_cotaursAwsIotJobARN = a})

-- | The OTA update ID.
cotaursOtaUpdateId :: Lens' CreateOTAUpdateResponse (Maybe Text)
cotaursOtaUpdateId =
  lens _cotaursOtaUpdateId (\s a -> s {_cotaursOtaUpdateId = a})

-- | The OTA update ARN.
cotaursOtaUpdateARN :: Lens' CreateOTAUpdateResponse (Maybe Text)
cotaursOtaUpdateARN =
  lens _cotaursOtaUpdateARN (\s a -> s {_cotaursOtaUpdateARN = a})

-- | -- | The response status code.
cotaursResponseStatus :: Lens' CreateOTAUpdateResponse Int
cotaursResponseStatus =
  lens _cotaursResponseStatus (\s a -> s {_cotaursResponseStatus = a})
