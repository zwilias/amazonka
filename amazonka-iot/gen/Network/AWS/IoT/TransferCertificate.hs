...
tcTargetAWSAccount =
  lens _tcTargetAWSAccount (\s a -> s {_tcTargetAWSAccount = a})
...
instance AWSRequest TransferCertificate where
  ...
  response =
    receiveJSON
      (\s h x ->
         TransferCertificateResponse' <$> (x .?> "transferredCertificateArn") <*>
         (pure (fromEnum s)))
...
instance ToJSON TransferCertificate where
  toJSON TransferCertificate' {..} =
    object (catMaybes [("transferMessage" .=) <$> _tcTransferMessage])
...
tcrsTransferredCertificateARN =
  lens
    ...
    (\s a -> s {_tcrsTransferredCertificateARN = a})

-- | -- | The response status code.
tcrsResponseStatus :: Lens' TransferCertificateResponse Int
tcrsResponseStatus =
  lens _tcrsResponseStatus (\s a -> s {_tcrsResponseStatus = a})
