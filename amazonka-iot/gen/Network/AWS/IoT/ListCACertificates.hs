...
lcacAscendingOrder =
  lens _lcacAscendingOrder (\s a -> s {_lcacAscendingOrder = a})
...
instance AWSRequest ListCACertificates where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListCACertificatesResponse' <$> (x .?> "certificates" .!@ mempty) <*>
         (x .?> "nextMarker") <*>
         (pure (fromEnum s)))
...
lcacrsCertificates =
  lens _lcacrsCertificates (\s a -> s {_lcacrsCertificates = a}) .
  _Default . _Coerce
...
lcacrsResponseStatus =
  lens _lcacrsResponseStatus (\s a -> s {_lcacrsResponseStatus = a})
