...
instance AWSRequest DescribeDefaultAuthorizer where
  type Rs DescribeDefaultAuthorizer = DescribeDefaultAuthorizerResponse
  request = get ioT
  response =
    receiveJSON
      (\s h x ->
         DescribeDefaultAuthorizerResponse' <$> (x .?> "authorizerDescription") <*>
         (pure (fromEnum s)))
...
ddarsAuthorizerDescription ::
     Lens' DescribeDefaultAuthorizerResponse (Maybe AuthorizerDescription)
ddarsAuthorizerDescription =
  lens _ddarsAuthorizerDescription (\s a -> s {_ddarsAuthorizerDescription = a})

-- | -- | The response status code.
ddarsResponseStatus :: Lens' DescribeDefaultAuthorizerResponse Int
ddarsResponseStatus =
  lens _ddarsResponseStatus (\s a -> s {_ddarsResponseStatus = a})
