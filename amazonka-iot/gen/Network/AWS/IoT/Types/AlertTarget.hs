...
data AlertTarget = AlertTarget'{_atAlertTargetARN ::
                                !Text,
                                _atRoleARN :: !Text}
...
alertTarget
    :: Text -- ^ 'atAlertTargetARN'
    -> Text -- ^ 'atRoleARN'
    -> AlertTarget
alertTarget pAlertTargetARN_ pRoleARN_
  = AlertTarget'{_atAlertTargetARN = pAlertTargetARN_,
...
instance FromJSON AlertTarget where
        parseJSON
          = withObject "AlertTarget"
              (\ x ->
                 AlertTarget' <$>
                   (x .: "alertTargetArn") <*> (x .: "roleArn"))

instance Hashable AlertTarget where

instance NFData AlertTarget where
