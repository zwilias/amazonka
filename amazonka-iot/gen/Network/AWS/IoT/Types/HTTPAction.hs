...
data HTTPAction = HTTPAction'{_httpaConfirmationURL
                              :: !(Maybe Text),
...
hTTPAction
    ...
    -> HTTPAction
hTTPAction pUrl_
  = HTTPAction'{_httpaConfirmationURL = Nothing,
                _httpaAuth = Nothing, _httpaHeaders = Nothing,
                _httpaUrl = pUrl_}
...
instance FromJSON HTTPAction where
        parseJSON
          = withObject "HTTPAction"
              (\ x ->
                 HTTPAction' <$>
                   (x .:? "confirmationUrl") <*> (x .:? "auth") <*>
                     (x .:? "headers" .!= mempty)
                     <*> (x .: "url"))

instance Hashable HTTPAction where

instance NFData HTTPAction where
