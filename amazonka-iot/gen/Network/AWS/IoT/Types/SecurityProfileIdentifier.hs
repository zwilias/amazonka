...
data SecurityProfileIdentifier = SecurityProfileIdentifier'{_spiName
                                                            :: !Text,
                                                            _spiArn :: !Text}
...
securityProfileIdentifier pName_ pArn_
  = SecurityProfileIdentifier'{_spiName = pName_,
                               _spiArn = pArn_}
...
instance FromJSON SecurityProfileIdentifier where
        parseJSON
          = withObject "SecurityProfileIdentifier"
              (\ x ->
                 SecurityProfileIdentifier' <$>
                   (x .: "name") <*> (x .: "arn"))

instance Hashable SecurityProfileIdentifier where

instance NFData SecurityProfileIdentifier where
