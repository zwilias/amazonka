...
data MitigationAction = MitigationAction'{_maActionParams
...
mitigationAction
    :: MitigationAction
mitigationAction
  = MitigationAction'{_maActionParams = Nothing,
                      _maName = Nothing, _maId = Nothing,
                      _maRoleARN = Nothing}
...
instance FromJSON MitigationAction where
        parseJSON
          = withObject "MitigationAction"
              (\ x ->
                 MitigationAction' <$>
                   (x .:? "actionParams") <*> (x .:? "name") <*>
                     (x .:? "id")
                     <*> (x .:? "roleArn"))

instance Hashable MitigationAction where

instance NFData MitigationAction where
