...
newtype SecurityProfileTarget = SecurityProfileTarget'{_sptArn
                                                       :: Text}
...
instance FromJSON SecurityProfileTarget where
        parseJSON
          = withObject "SecurityProfileTarget"
              (\ x -> SecurityProfileTarget' <$> (x .: "arn"))

instance Hashable SecurityProfileTarget where

instance NFData SecurityProfileTarget where
