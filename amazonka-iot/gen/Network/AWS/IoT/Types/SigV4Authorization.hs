...
data SigV4Authorization = SigV4Authorization'{_svaSigningRegion
                                              :: !Text,
...
sigV4Authorization
    :: Text -- ^ 'svaSigningRegion'
    -> Text -- ^ 'svaServiceName'
    -> Text -- ^ 'svaRoleARN'
    -> SigV4Authorization
sigV4Authorization pSigningRegion_ pServiceName_
  pRoleARN_
  = SigV4Authorization'{_svaSigningRegion =
                          pSigningRegion_,
...
instance FromJSON SigV4Authorization where
        parseJSON
          = withObject "SigV4Authorization"
              (\ x ->
                 SigV4Authorization' <$>
                   (x .: "signingRegion") <*> (x .: "serviceName") <*>
                     (x .: "roleArn"))

instance Hashable SigV4Authorization where

instance NFData SigV4Authorization where
