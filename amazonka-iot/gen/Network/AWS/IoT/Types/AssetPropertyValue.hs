...
data AssetPropertyValue = AssetPropertyValue'{_apvQuality
                                              :: !(Maybe Text),
                                              _apvValue ::
...
assetPropertyValue
    :: AssetPropertyVariant -- ^ 'apvValue'
    -> AssetPropertyTimestamp -- ^ 'apvTimestamp'
    -> AssetPropertyValue
assetPropertyValue pValue_ pTimestamp_
  = AssetPropertyValue'{_apvQuality = Nothing,
                        _apvValue = pValue_, _apvTimestamp = pTimestamp_}
...
instance FromJSON AssetPropertyValue where
        parseJSON
          = withObject "AssetPropertyValue"
              (\ x ->
                 AssetPropertyValue' <$>
                   (x .:? "quality") <*> (x .: "value") <*>
                     (x .: "timestamp"))

instance Hashable AssetPropertyValue where

instance NFData AssetPropertyValue where
