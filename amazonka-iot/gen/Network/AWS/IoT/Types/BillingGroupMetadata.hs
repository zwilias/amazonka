...
newtype BillingGroupMetadata = BillingGroupMetadata'{_bgmCreationDate
                                                     :: Maybe POSIX}
...
instance FromJSON BillingGroupMetadata where
        parseJSON
          = withObject "BillingGroupMetadata"
              (\ x ->
                 BillingGroupMetadata' <$> (x .:? "creationDate"))

instance Hashable BillingGroupMetadata where

instance NFData BillingGroupMetadata where
