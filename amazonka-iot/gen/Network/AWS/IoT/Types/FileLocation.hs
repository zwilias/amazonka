...
data FileLocation = FileLocation'{_flStream ::
                                  !(Maybe Stream),
...
fileLocation
    :: FileLocation
fileLocation
  = FileLocation'{_flStream = Nothing,
...
instance FromJSON FileLocation where
        parseJSON
          = withObject "FileLocation"
              (\ x ->
                 FileLocation' <$>
                   (x .:? "stream") <*> (x .:? "s3Location"))

instance Hashable FileLocation where

instance NFData FileLocation where
