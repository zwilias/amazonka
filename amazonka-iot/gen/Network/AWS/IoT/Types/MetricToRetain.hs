...
data MetricToRetain = MetricToRetain'{_mtrMetricDimension
...
metricToRetain pMetric_
  = MetricToRetain'{_mtrMetricDimension = Nothing,
                    _mtrMetric = pMetric_}
...
instance FromJSON MetricToRetain where
        parseJSON
          = withObject "MetricToRetain"
              (\ x ->
                 MetricToRetain' <$>
                   (x .:? "metricDimension") <*> (x .: "metric"))

instance Hashable MetricToRetain where

instance NFData MetricToRetain where
