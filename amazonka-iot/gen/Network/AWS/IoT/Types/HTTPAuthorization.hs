...
newtype HTTPAuthorization = HTTPAuthorization'{_httpaSigv4
...
instance FromJSON HTTPAuthorization where
        parseJSON
          = withObject "HTTPAuthorization"
              (\ x -> HTTPAuthorization' <$> (x .:? "sigv4"))

instance Hashable HTTPAuthorization where

instance NFData HTTPAuthorization where
