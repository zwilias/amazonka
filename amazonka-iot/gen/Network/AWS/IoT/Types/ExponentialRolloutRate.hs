...
data ExponentialRolloutRate = ExponentialRolloutRate'{_errBaseRatePerMinute
                                                      :: !Nat,
                                                      _errIncrementFactor ::
                                                      !Double,
...
--
-- * 'errIncrementFactor' - The exponential factor to increase the rate of rollout for a job.
--
-- * 'errRateIncreaseCriteria' - The criteria to initiate the increase in rate of rollout for a job. AWS IoT supports up to one digit after the decimal (for example, 1.5, but not 1.55).
exponentialRolloutRate
    :: Natural -- ^ 'errBaseRatePerMinute'
    -> Double -- ^ 'errIncrementFactor'
    -> RateIncreaseCriteria -- ^ 'errRateIncreaseCriteria'
    -> ExponentialRolloutRate
exponentialRolloutRate pBaseRatePerMinute_
  pIncrementFactor_ pRateIncreaseCriteria_
  = ExponentialRolloutRate'{_errBaseRatePerMinute =
...
instance FromJSON ExponentialRolloutRate where
        parseJSON
          = withObject "ExponentialRolloutRate"
              (\ x ->
                 ExponentialRolloutRate' <$>
                   (x .: "baseRatePerMinute") <*>
                     (x .: "incrementFactor")
...
instance Hashable ExponentialRolloutRate where

instance NFData ExponentialRolloutRate where
