...
newtype AuditCheckConfiguration = AuditCheckConfiguration'{_accEnabled
                                                           :: Maybe Bool}
...
instance FromJSON AuditCheckConfiguration where
        parseJSON
          = withObject "AuditCheckConfiguration"
              (\ x ->
                 AuditCheckConfiguration' <$> (x .:? "enabled"))

instance Hashable AuditCheckConfiguration where

instance NFData AuditCheckConfiguration where
