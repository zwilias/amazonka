...
data PercentPair = PercentPair'{_ppValue ::
                                !(Maybe Double),
...
instance FromJSON PercentPair where
        parseJSON
          = withObject "PercentPair"
              (\ x ->
                 PercentPair' <$>
                   (x .:? "value") <*> (x .:? "percent"))

instance Hashable PercentPair where

instance NFData PercentPair where
