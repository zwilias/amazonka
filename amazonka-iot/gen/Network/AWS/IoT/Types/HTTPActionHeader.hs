...
data HTTPActionHeader = HTTPActionHeader'{_httpahKey
                                          :: !Text,
                                          _httpahValue :: !Text}
...
hTTPActionHeader pKey_ pValue_
  = HTTPActionHeader'{_httpahKey = pKey_,
...
instance FromJSON HTTPActionHeader where
        parseJSON
          = withObject "HTTPActionHeader"
              (\ x ->
                 HTTPActionHeader' <$>
                   (x .: "key") <*> (x .: "value"))

instance Hashable HTTPActionHeader where

instance NFData HTTPActionHeader where
