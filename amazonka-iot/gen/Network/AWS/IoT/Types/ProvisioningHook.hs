...
data ProvisioningHook = ProvisioningHook'{_phPayloadVersion
                                          :: !(Maybe Text),
                                          _phTargetARN :: !Text}
...
provisioningHook pTargetARN_
  = ProvisioningHook'{_phPayloadVersion = Nothing,
...
instance FromJSON ProvisioningHook where
        parseJSON
          = withObject "ProvisioningHook"
              (\ x ->
                 ProvisioningHook' <$>
                   (x .:? "payloadVersion") <*> (x .: "targetArn"))

instance Hashable ProvisioningHook where

instance NFData ProvisioningHook where
