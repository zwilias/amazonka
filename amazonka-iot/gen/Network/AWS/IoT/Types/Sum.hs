...
instance FromText ActionType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ActionType from value: '" <>
        e <> "'. Accepted values: connect, publish, receive, subscribe"
...
instance FromText AuthDecision where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuthDecision from value: '" <>
        e <> "'. Accepted values: allowed, explicit_deny, implicit_deny"
...
instance FromText AuthorizerStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AuthorizerStatus from value: '" <>
...
instance FromText AutoRegistrationStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing AutoRegistrationStatus from value: '" <>
        e <> "'. Accepted values: disable, enable"
...
instance FromText CACertificateStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CACertificateStatus from value: '" <>
        e <> "'. Accepted values: active, inactive"
...
instance FromText CannedAccessControlList where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CannedAccessControlList from value: '" <>
        e <>
...
instance FromText CertificateStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing CertificateStatus from value: '" <>
        e <>
...
instance FromText DynamoKeyType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing DynamoKeyType from value: '" <>
...
instance FromText IndexStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing IndexStatus from value: '" <>
        e <> "'. Accepted values: active, building, rebuilding"
...
instance FromText JobExecutionStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing JobExecutionStatus from value: '" <>
        e <>
  ...
  parser =
    takeLowerText >>= \case
      ...
      e ->
        ...
        e <> "'. Accepted values: canceled, completed, in_progress"
...
instance FromText LogLevel where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LogLevel from value: '" <>
        e <> "'. Accepted values: debug, disabled, error, info, warn"
...
instance FromText LogTargetType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LogTargetType from value: '" <>
        e <> "'. Accepted values: default, thing_group"
...
instance FromText MessageFormat where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing MessageFormat from value: '" <>
        e <> "'. Accepted values: json, raw"
...
instance FromText OTAUpdateStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing OTAUpdateStatus from value: '" <>
        e <>
...
instance FromText ReportType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReportType from value: '" <>
        e <> "'. Accepted values: errors, results"
...
instance FromText TargetSelection where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TargetSelection from value: '" <>
        e <> "'. Accepted values: continuous, snapshot"
...
instance FromText TaskStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing TaskStatus from value: '" <>
        e <>
...
instance FromText ThingIndexingMode where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ThingIndexingMode from value: '" <>
        e <> "'. Accepted values: off, registry, registry_and_shadow"
