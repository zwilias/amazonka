...
instance AWSRequest GetV2LoggingOptions where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetV2LoggingOptionsResponse' <$> (x .?> "disableAllLogs") <*>
         (x .?> "defaultLogLevel") <*>
         (x .?> "roleArn") <*>
         (pure (fromEnum s)))
...
gvlorsDisableAllLogs =
  lens _gvlorsDisableAllLogs (\s a -> s {_gvlorsDisableAllLogs = a})

-- | The default log level.
gvlorsDefaultLogLevel :: Lens' GetV2LoggingOptionsResponse (Maybe LogLevel)
gvlorsDefaultLogLevel =
  lens _gvlorsDefaultLogLevel (\s a -> s {_gvlorsDefaultLogLevel = a})
...
gvlorsResponseStatus =
  lens _gvlorsResponseStatus (\s a -> s {_gvlorsResponseStatus = a})
