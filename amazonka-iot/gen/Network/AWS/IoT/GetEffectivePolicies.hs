...
gepCognitoIdentityPoolId =
  lens _gepCognitoIdentityPoolId (\s a -> s {_gepCognitoIdentityPoolId = a})
...
instance ToQuery GetEffectivePolicies where
  toQuery GetEffectivePolicies' {..} = mconcat ["thingName" =: _gepThingName]
...
geprsEffectivePolicies =
  lens _geprsEffectivePolicies (\s a -> s {_geprsEffectivePolicies = a}) .
  _Default . _Coerce

-- | -- | The response status code.
geprsResponseStatus :: Lens' GetEffectivePoliciesResponse Int
geprsResponseStatus =
  lens _geprsResponseStatus (\s a -> s {_geprsResponseStatus = a})
