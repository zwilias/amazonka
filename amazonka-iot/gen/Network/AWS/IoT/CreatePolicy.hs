...
instance AWSRequest CreatePolicy where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreatePolicyResponse' <$> (x .?> "policyName") <*>
         (x .?> "policyDocument") <*>
         (x .?> "policyVersionId") <*>
         (x .?> "policyArn") <*>
         (pure (fromEnum s)))
...
instance ToJSON CreatePolicy where
  toJSON CreatePolicy' {..} =
    object (catMaybes [Just ("policyDocument" .= _cpPolicyDocument)])

instance ToPath CreatePolicy where
  toPath CreatePolicy' {..} = mconcat ["/policies/", toBS _cpPolicyName]
...
cprsPolicyDocument =
  lens _cprsPolicyDocument (\s a -> s {_cprsPolicyDocument = a})

-- | The policy version ID.
cprsPolicyVersionId :: Lens' CreatePolicyResponse (Maybe Text)
cprsPolicyVersionId =
  lens _cprsPolicyVersionId (\s a -> s {_cprsPolicyVersionId = a})
