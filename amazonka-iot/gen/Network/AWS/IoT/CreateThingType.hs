...
cttThingTypeProperties =
  lens _cttThingTypeProperties (\s a -> s {_cttThingTypeProperties = a})
...
instance AWSRequest CreateThingType where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateThingTypeResponse' <$> (x .?> "thingTypeName") <*>
         (x .?> "thingTypeId") <*>
         (x .?> "thingTypeArn") <*>
         (pure (fromEnum s)))
...
instance ToJSON CreateThingType where
  toJSON CreateThingType' {..} =
    object (catMaybes [("thingTypeProperties" .=) <$> _cttThingTypeProperties])
...
cttrsThingTypeName =
  lens _cttrsThingTypeName (\s a -> s {_cttrsThingTypeName = a})
...
cttrsResponseStatus =
  lens _cttrsResponseStatus (\s a -> s {_cttrsResponseStatus = a})
