...
uaAuthorizerFunctionARN =
  lens _uaAuthorizerFunctionARN (\s a -> s {_uaAuthorizerFunctionARN = a})

-- | The public keys used to verify the token signature.
uaTokenSigningPublicKeys :: Lens' UpdateAuthorizer (HashMap Text Text)
uaTokenSigningPublicKeys =
  lens _uaTokenSigningPublicKeys (\s a -> s {_uaTokenSigningPublicKeys = a}) .
  _Default . _Map
...
instance AWSRequest UpdateAuthorizer where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateAuthorizerResponse' <$> (x .?> "authorizerName") <*>
         (x .?> "authorizerArn") <*>
         (pure (fromEnum s)))
...
uarsAuthorizerName =
  lens _uarsAuthorizerName (\s a -> s {_uarsAuthorizerName = a})
