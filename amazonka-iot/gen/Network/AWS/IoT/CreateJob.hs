...
cjJobExecutionsRolloutConfig ::
     Lens' CreateJob (Maybe JobExecutionsRolloutConfig)
cjJobExecutionsRolloutConfig =
  lens
    _cjJobExecutionsRolloutConfig
    (\s a -> s {_cjJobExecutionsRolloutConfig = a})
...
cjDocumentParameters =
  lens _cjDocumentParameters (\s a -> s {_cjDocumentParameters = a}) .
  _Default . _Map

-- | Configuration information for pre-signed S3 URLs.
cjPresignedURLConfig :: Lens' CreateJob (Maybe PresignedURLConfig)
cjPresignedURLConfig =
  lens _cjPresignedURLConfig (\s a -> s {_cjPresignedURLConfig = a})
...
instance AWSRequest CreateJob where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateJobResponse' <$> (x .?> "jobId") <*> (x .?> "jobArn") <*>
         (x .?> "description") <*>
         (pure (fromEnum s)))
...
instance ToJSON CreateJob where
  toJSON CreateJob' {..} =
    object
      (catMaybes
         [ ("jobExecutionsRolloutConfig" .=) <$> _cjJobExecutionsRolloutConfig
