...
lrdsMaxResults =
  lens _lrdsMaxResults (\s a -> s {_lrdsMaxResults = a}) . mapping _Nat
...
instance ToHeaders ListResourceDataSync where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.ListResourceDataSync" :: ByteString)
...
lrdsrsResourceDataSyncItems ::
     Lens' ListResourceDataSyncResponse [ResourceDataSyncItem]
lrdsrsResourceDataSyncItems =
  lens
    _lrdsrsResourceDataSyncItems
    (\s a -> s {_lrdsrsResourceDataSyncItems = a}) .
  _Default . _Coerce
...
lrdsrsResponseStatus =
  lens _lrdsrsResponseStatus (\s a -> s {_lrdsrsResponseStatus = a})
