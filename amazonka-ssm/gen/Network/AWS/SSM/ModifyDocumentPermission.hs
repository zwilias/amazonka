...
instance AWSRequest ModifyDocumentPermission where
  ...
  response =
    receiveEmpty
      (\s h x -> ModifyDocumentPermissionResponse' <$> (pure (fromEnum s)))
...
mdprsResponseStatus =
  lens _mdprsResponseStatus (\s a -> s {_mdprsResponseStatus = a})
