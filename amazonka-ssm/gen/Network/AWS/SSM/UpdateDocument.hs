...
instance AWSRequest UpdateDocument where
  ...
  response =
    receiveJSON
      (\s h x ->
         UpdateDocumentResponse' <$> (x .?> "DocumentDescription") <*>
         (pure (fromEnum s)))
...
udrsDocumentDescription ::
     Lens' UpdateDocumentResponse (Maybe DocumentDescription)
udrsDocumentDescription =
  lens _udrsDocumentDescription (\s a -> s {_udrsDocumentDescription = a})
