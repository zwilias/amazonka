...
uddvDocumentVersion =
  lens _uddvDocumentVersion (\s a -> s {_uddvDocumentVersion = a})

instance AWSRequest UpdateDocumentDefaultVersion where
  type Rs UpdateDocumentDefaultVersion = UpdateDocumentDefaultVersionResponse
  request = postJSON ssm
  response =
    receiveJSON
      (\s h x ->
         UpdateDocumentDefaultVersionResponse' <$> (x .?> "Description") <*>
         (pure (fromEnum s)))
...
uddvrsDescription ::
     Lens' UpdateDocumentDefaultVersionResponse (Maybe DocumentDefaultVersionDescription)
...
uddvrsResponseStatus =
  lens _uddvrsResponseStatus (\s a -> s {_uddvrsResponseStatus = a})
