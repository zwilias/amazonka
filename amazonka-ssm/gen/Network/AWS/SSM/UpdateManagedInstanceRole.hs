...
instance AWSRequest UpdateManagedInstanceRole where
  type Rs UpdateManagedInstanceRole = UpdateManagedInstanceRoleResponse
  request = postJSON ssm
  response =
    receiveEmpty
      (\s h x -> UpdateManagedInstanceRoleResponse' <$> (pure (fromEnum s)))
...
umirrsResponseStatus =
  lens _umirrsResponseStatus (\s a -> s {_umirrsResponseStatus = a})
