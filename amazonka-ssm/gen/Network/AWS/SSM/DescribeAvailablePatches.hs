...
dapMaxResults =
  lens _dapMaxResults (\s a -> s {_dapMaxResults = a}) . mapping _Nat
...
instance AWSRequest DescribeAvailablePatches where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeAvailablePatchesResponse' <$> (x .?> "Patches" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
daprsPatches =
  lens _daprsPatches (\s a -> s {_daprsPatches = a}) . _Default . _Coerce
...
daprsResponseStatus =
  lens _daprsResponseStatus (\s a -> s {_daprsResponseStatus = a})
