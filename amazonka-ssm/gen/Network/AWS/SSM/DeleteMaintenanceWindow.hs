...
instance AWSRequest DeleteMaintenanceWindow where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteMaintenanceWindowResponse' <$> (x .?> "WindowId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DeleteMaintenanceWindow where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.DeleteMaintenanceWindow" :: ByteString)
...
dmwrsResponseStatus =
  lens _dmwrsResponseStatus (\s a -> s {_dmwrsResponseStatus = a})
