...
cmwDescription =
  lens _cmwDescription (\s a -> s {_cmwDescription = a}) . mapping _Sensitive
...
cmwAllowUnassociatedTargets =
  lens
    ...
    (\s a -> s {_cmwAllowUnassociatedTargets = a})
...
instance AWSRequest CreateMaintenanceWindow where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateMaintenanceWindowResponse' <$> (x .?> "WindowId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders CreateMaintenanceWindow where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.CreateMaintenanceWindow" :: ByteString)
...
cmwrsResponseStatus =
  lens _cmwrsResponseStatus (\s a -> s {_cmwrsResponseStatus = a})
