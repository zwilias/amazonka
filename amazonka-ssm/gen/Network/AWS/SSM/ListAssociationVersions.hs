...
lavMaxResults =
  lens _lavMaxResults (\s a -> s {_lavMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListAssociationVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListAssociationVersionsResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders ListAssociationVersions where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.ListAssociationVersions" :: ByteString)
...
lavrsAssociationVersions ::
     Lens' ListAssociationVersionsResponse (Maybe (NonEmpty AssociationVersionInfo))
lavrsAssociationVersions =
  lens _lavrsAssociationVersions (\s a -> s {_lavrsAssociationVersions = a}) .
  mapping _List1

-- | -- | The response status code.
lavrsResponseStatus :: Lens' ListAssociationVersionsResponse Int
lavrsResponseStatus =
  lens _lavrsResponseStatus (\s a -> s {_lavrsResponseStatus = a})
