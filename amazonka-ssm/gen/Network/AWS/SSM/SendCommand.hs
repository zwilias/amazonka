...
scNotificationConfig =
  lens _scNotificationConfig (\s a -> s {_scNotificationConfig = a})

-- | Sha256 or Sha1.
scDocumentHashType :: Lens' SendCommand (Maybe DocumentHashType)
scDocumentHashType =
  lens _scDocumentHashType (\s a -> s {_scDocumentHashType = a})

-- | The directory structure within the S3 bucket where the responses should be stored.
scOutputS3KeyPrefix :: Lens' SendCommand (Maybe Text)
scOutputS3KeyPrefix =
  lens _scOutputS3KeyPrefix (\s a -> s {_scOutputS3KeyPrefix = a})
...
scInstanceIds =
  lens _scInstanceIds (\s a -> s {_scInstanceIds = a}) . _Default . _Coerce
...
scParameters =
  lens _scParameters (\s a -> s {_scParameters = a}) . _Default . _Map
...
scTimeoutSeconds =
  lens _scTimeoutSeconds (\s a -> s {_scTimeoutSeconds = a}) . mapping _Nat
...
scOutputS3BucketName =
  lens _scOutputS3BucketName (\s a -> s {_scOutputS3BucketName = a})
