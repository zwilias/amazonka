...
lResourceIds =
  lens _lResourceIds (\s a -> s {_lResourceIds = a}) . mapping _List1
...
lResourceTypes =
  lens _lResourceTypes (\s a -> s {_lResourceTypes = a}) . mapping _List1
...
instance AWSRequest ListComplianceItems where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListComplianceItemsResponse' <$> (x .?> "ComplianceItems" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
lcirsComplianceItems =
  lens _lcirsComplianceItems (\s a -> s {_lcirsComplianceItems = a}) .
  _Default . _Coerce
...
lcirsResponseStatus =
  lens _lcirsResponseStatus (\s a -> s {_lcirsResponseStatus = a})
