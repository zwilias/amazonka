...
caScheduleExpression =
  lens _caScheduleExpression (\s a -> s {_caScheduleExpression = a})

-- | An Amazon S3 bucket where you want to store the output details of the request.
caOutputLocation ::
     Lens' CreateAssociation (Maybe InstanceAssociationOutputLocation)
...
caParameters =
  lens _caParameters (\s a -> s {_caParameters = a}) . _Default . _Map
...
instance AWSRequest CreateAssociation where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreateAssociationResponse' <$> (x .?> "AssociationDescription") <*>
         (pure (fromEnum s)))
...
crsAssociationDescription ::
     Lens' CreateAssociationResponse (Maybe AssociationDescription)
crsAssociationDescription =
  lens _crsAssociationDescription (\s a -> s {_crsAssociationDescription = a})
