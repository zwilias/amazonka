...
instance AWSRequest GetParameters where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetParametersResponse' <$> (x .?> "Parameters" .!@ mempty) <*>
...
grsParameters =
  lens _grsParameters (\s a -> s {_grsParameters = a}) . _Default . _Coerce
...
grsInvalidParameters =
  lens _grsInvalidParameters (\s a -> s {_grsInvalidParameters = a}) .
  _Default . _Coerce
