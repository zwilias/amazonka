...
cpbOperatingSystem =
  lens _cpbOperatingSystem (\s a -> s {_cpbOperatingSystem = a})
...
cpbApprovedPatchesComplianceLevel ::
     Lens' CreatePatchBaseline (Maybe PatchComplianceLevel)
cpbApprovedPatchesComplianceLevel =
  lens
    _cpbApprovedPatchesComplianceLevel
    (\s a -> s {_cpbApprovedPatchesComplianceLevel = a})
...
cpbApprovedPatches =
  lens _cpbApprovedPatches (\s a -> s {_cpbApprovedPatches = a}) .
  _Default . _Coerce
...
cpbApprovedPatchesEnableNonSecurity =
  lens
    ...
    (\s a -> s {_cpbApprovedPatchesEnableNonSecurity = a})
...
cpbRejectedPatches =
  lens _cpbRejectedPatches (\s a -> s {_cpbRejectedPatches = a}) .
  _Default . _Coerce
...
instance AWSRequest CreatePatchBaseline where
  ...
  response =
    receiveJSON
      (\s h x ->
         CreatePatchBaselineResponse' <$> (x .?> "BaselineId") <*>
         (pure (fromEnum s)))
...
cpbrsResponseStatus =
  lens _cpbrsResponseStatus (\s a -> s {_cpbrsResponseStatus = a})
