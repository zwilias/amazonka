...
gpbpWithDecryption =
  lens _gpbpWithDecryption (\s a -> s {_gpbpWithDecryption = a})

-- | Filters to limit the request results.
gpbpParameterFilters :: Lens' GetParametersByPath [ParameterStringFilter]
gpbpParameterFilters =
  lens _gpbpParameterFilters (\s a -> s {_gpbpParameterFilters = a}) .
  _Default . _Coerce
...
gpbpMaxResults =
  lens _gpbpMaxResults (\s a -> s {_gpbpMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetParametersByPath where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetParametersByPathResponse' <$> (x .?> "NextToken") <*>
         (x .?> "Parameters" .!@ mempty) <*>
         (pure (fromEnum s)))
...
gpbprsParameters =
  lens _gpbprsParameters (\s a -> s {_gpbprsParameters = a}) .
  _Default . _Coerce

-- | -- | The response status code.
gpbprsResponseStatus :: Lens' GetParametersByPathResponse Int
gpbprsResponseStatus =
  lens _gpbprsResponseStatus (\s a -> s {_gpbprsResponseStatus = a})
