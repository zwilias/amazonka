...
instance ToHeaders DescribePatchGroupState where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.DescribePatchGroupState" :: ByteString)
...
dpgsrsInstancesWithMissingPatches ::
     Lens' DescribePatchGroupStateResponse (Maybe Int)
dpgsrsInstancesWithMissingPatches =
  lens
    _dpgsrsInstancesWithMissingPatches
    (\s a -> s {_dpgsrsInstancesWithMissingPatches = a})
...
dpgsrsInstancesWithInstalledOtherPatches =
  lens
    ...
    (\s a -> s {_dpgsrsInstancesWithInstalledOtherPatches = a})
...
dpgsrsInstancesWithNotApplicablePatches =
  lens
    ...
    (\s a -> s {_dpgsrsInstancesWithNotApplicablePatches = a})
...
dpgsrsInstancesWithInstalledPatches =
  lens
    ...
    (\s a -> s {_dpgsrsInstancesWithInstalledPatches = a})
...
dpgsrsInstancesWithFailedPatches =
  lens
    ...
    (\s a -> s {_dpgsrsInstancesWithFailedPatches = a})
