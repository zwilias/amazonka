...
dpbMaxResults =
  lens _dpbMaxResults (\s a -> s {_dpbMaxResults = a}) . mapping _Nat
...
instance ToHeaders DescribePatchBaselines where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.DescribePatchBaselines" :: ByteString)
...
dpbsrsBaselineIdentities ::
     Lens' DescribePatchBaselinesResponse [PatchBaselineIdentity]
dpbsrsBaselineIdentities =
  lens _dpbsrsBaselineIdentities (\s a -> s {_dpbsrsBaselineIdentities = a}) .
  _Default . _Coerce
...
dpbsrsResponseStatus =
  lens _dpbsrsResponseStatus (\s a -> s {_dpbsrsResponseStatus = a})
