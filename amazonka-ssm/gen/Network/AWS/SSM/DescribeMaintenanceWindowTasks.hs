...
instance AWSRequest DescribeMaintenanceWindowTasks where
  type Rs DescribeMaintenanceWindowTasks = DescribeMaintenanceWindowTasksResponse
  request = postJSON ssm
  response =
    receiveJSON
      (\s h x ->
         DescribeMaintenanceWindowTasksResponse' <$> (x .?> "Tasks" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
dmwtsrsTasks ::
     Lens' DescribeMaintenanceWindowTasksResponse [MaintenanceWindowTask]
dmwtsrsTasks =
  lens _dmwtsrsTasks (\s a -> s {_dmwtsrsTasks = a}) . _Default . _Coerce
...
dmwtsrsResponseStatus =
  lens _dmwtsrsResponseStatus (\s a -> s {_dmwtsrsResponseStatus = a})
