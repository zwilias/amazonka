...
dpgMaxResults =
  lens _dpgMaxResults (\s a -> s {_dpgMaxResults = a}) . mapping _Nat
...
instance AWSRequest DescribePatchGroups where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribePatchGroupsResponse' <$> (x .?> "Mappings" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
dpgrsMappings ::
     Lens' DescribePatchGroupsResponse [PatchGroupPatchBaselineMapping]
dpgrsMappings =
  lens _dpgrsMappings (\s a -> s {_dpgrsMappings = a}) . _Default . _Coerce
