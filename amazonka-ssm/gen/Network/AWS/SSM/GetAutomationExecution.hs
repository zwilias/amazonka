...
gaeAutomationExecutionId =
  lens _gaeAutomationExecutionId (\s a -> s {_gaeAutomationExecutionId = a})
...
instance AWSRequest GetAutomationExecution where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetAutomationExecutionResponse' <$> (x .?> "AutomationExecution") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetAutomationExecution where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.GetAutomationExecution" :: ByteString)
...
instance ToJSON GetAutomationExecution where
  toJSON GetAutomationExecution' {..} =
    object
      (catMaybes [Just ("AutomationExecutionId" .= _gaeAutomationExecutionId)])
...
gaersAutomationExecution ::
     Lens' GetAutomationExecutionResponse (Maybe AutomationExecution)
gaersAutomationExecution =
  lens _gaersAutomationExecution (\s a -> s {_gaersAutomationExecution = a})

-- | -- | The response status code.
gaersResponseStatus :: Lens' GetAutomationExecutionResponse Int
gaersResponseStatus =
  lens _gaersResponseStatus (\s a -> s {_gaersResponseStatus = a})
