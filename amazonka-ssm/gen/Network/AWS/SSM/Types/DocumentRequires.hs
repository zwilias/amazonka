...
data DocumentRequires = DocumentRequires'{_drVersion
                                          :: !(Maybe Text),
                                          _drName :: !Text}
...
documentRequires pName_
  = DocumentRequires'{_drVersion = Nothing,
                      _drName = pName_}
...
instance FromJSON DocumentRequires where
        parseJSON
          = withObject "DocumentRequires"
              (\ x ->
                 DocumentRequires' <$>
                   (x .:? "Version") <*> (x .: "Name"))

instance Hashable DocumentRequires where

instance NFData DocumentRequires where
