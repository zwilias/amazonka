...
newtype RelatedOpsItem = RelatedOpsItem'{_roiOpsItemId
                                         :: Text}
...
instance FromJSON RelatedOpsItem where
        parseJSON
          = withObject "RelatedOpsItem"
              (\ x -> RelatedOpsItem' <$> (x .: "OpsItemId"))

instance Hashable RelatedOpsItem where

instance NFData RelatedOpsItem where
