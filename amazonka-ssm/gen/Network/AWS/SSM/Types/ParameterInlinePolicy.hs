...
data ParameterInlinePolicy = ParameterInlinePolicy'{_pipPolicyType
                                                    :: !(Maybe Text),
                                                    _pipPolicyStatus ::
                                                    !(Maybe Text),
                                                    _pipPolicyText ::
...
parameterInlinePolicy
    :: ParameterInlinePolicy
parameterInlinePolicy
  = ParameterInlinePolicy'{_pipPolicyType = Nothing,
                           _pipPolicyStatus = Nothing, _pipPolicyText = Nothing}
...
instance FromJSON ParameterInlinePolicy where
        parseJSON
          = withObject "ParameterInlinePolicy"
              (\ x ->
                 ParameterInlinePolicy' <$>
                   (x .:? "PolicyType") <*> (x .:? "PolicyStatus") <*>
                     (x .:? "PolicyText"))

instance Hashable ParameterInlinePolicy where

instance NFData ParameterInlinePolicy where
