...
data OpsEntityItem = OpsEntityItem'{_oeiContent ::
                                    !(Maybe [Map Text Text]),
...
opsEntityItem
    :: OpsEntityItem
opsEntityItem
  = OpsEntityItem'{_oeiContent = Nothing,
...
instance FromJSON OpsEntityItem where
        parseJSON
          = withObject "OpsEntityItem"
              (\ x ->
                 OpsEntityItem' <$>
                   (x .:? "Content" .!= mempty) <*>
                     (x .:? "CaptureTime"))

instance Hashable OpsEntityItem where

instance NFData OpsEntityItem where
