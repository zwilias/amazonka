...
newtype OpsItemNotification = OpsItemNotification'{_oinARN
                                                   :: Maybe Text}
...
instance FromJSON OpsItemNotification where
        parseJSON
          = withObject "OpsItemNotification"
              (\ x -> OpsItemNotification' <$> (x .:? "Arn"))

instance Hashable OpsItemNotification where

instance NFData OpsItemNotification where
