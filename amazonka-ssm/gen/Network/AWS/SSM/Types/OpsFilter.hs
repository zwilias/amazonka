...
data OpsFilter = OpsFilter'{_ofType ::
                            !(Maybe OpsFilterOperatorType),
                            _ofKey :: !Text, _ofValues :: !(List1 Text)}
...
opsFilter
    :: Text -- ^ 'ofKey'
    -> NonEmpty Text -- ^ 'ofValues'
    -> OpsFilter
opsFilter pKey_ pValues_
  = OpsFilter'{_ofType = Nothing, _ofKey = pKey_,
...
instance Hashable OpsFilter where

instance NFData OpsFilter where

instance ToJSON OpsFilter where
        toJSON OpsFilter'{..}
          = object
              (catMaybes
                 [("Type" .=) <$> _ofType, Just ("Key" .= _ofKey),
