...
giAggregators =
  lens _giAggregators (\s a -> s {_giAggregators = a}) . mapping _List1
...
giResultAttributes =
  lens _giResultAttributes (\s a -> s {_giResultAttributes = a}) .
  mapping _List1
...
instance AWSRequest GetInventory where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetInventoryResponse' <$> (x .?> "Entities" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
girsEntities =
  lens _girsEntities (\s a -> s {_girsEntities = a}) . _Default . _Coerce
