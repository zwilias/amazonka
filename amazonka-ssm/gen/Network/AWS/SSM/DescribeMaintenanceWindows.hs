...
dmwMaxResults =
  lens _dmwMaxResults (\s a -> s {_dmwMaxResults = a}) . mapping _Nat

instance AWSRequest DescribeMaintenanceWindows where
  type Rs DescribeMaintenanceWindows = DescribeMaintenanceWindowsResponse
...
dmwsrsWindowIdentities ::
     Lens' DescribeMaintenanceWindowsResponse [MaintenanceWindowIdentity]
dmwsrsWindowIdentities =
  lens _dmwsrsWindowIdentities (\s a -> s {_dmwsrsWindowIdentities = a}) .
  _Default . _Coerce
...
dmwsrsResponseStatus =
  lens _dmwsrsResponseStatus (\s a -> s {_dmwsrsResponseStatus = a})
