...
saeAutomationExecutionId =
  lens _saeAutomationExecutionId (\s a -> s {_saeAutomationExecutionId = a})
...
instance AWSRequest StopAutomationExecution where
  ...
  response =
    receiveEmpty
      (\s h x -> StopAutomationExecutionResponse' <$> (pure (fromEnum s)))
...
instance ToHeaders StopAutomationExecution where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.StopAutomationExecution" :: ByteString)
...
saersResponseStatus =
  lens _saersResponseStatus (\s a -> s {_saersResponseStatus = a})
