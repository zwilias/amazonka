...
lcsMaxResults =
  lens _lcsMaxResults (\s a -> s {_lcsMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListComplianceSummaries where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListComplianceSummariesResponse' <$> (x .?> "NextToken") <*>
...
instance ToHeaders ListComplianceSummaries where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.ListComplianceSummaries" :: ByteString)
...
lcsrsComplianceSummaryItems ::
     Lens' ListComplianceSummariesResponse [ComplianceSummaryItem]
lcsrsComplianceSummaryItems =
  lens
    _lcsrsComplianceSummaryItems
    (\s a -> s {_lcsrsComplianceSummaryItems = a}) .
  _Default . _Coerce

-- | -- | The response status code.
lcsrsResponseStatus :: Lens' ListComplianceSummariesResponse Int
lcsrsResponseStatus =
  lens _lcsrsResponseStatus (\s a -> s {_lcsrsResponseStatus = a})
