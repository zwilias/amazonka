...
gisMaxResults =
  lens _gisMaxResults (\s a -> s {_gisMaxResults = a}) . mapping _Nat
...
instance AWSRequest GetInventorySchema where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetInventorySchemaResponse' <$> (x .?> "Schemas" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
gisrsSchemas =
  lens _gisrsSchemas (\s a -> s {_gisrsSchemas = a}) . _Default . _Coerce
