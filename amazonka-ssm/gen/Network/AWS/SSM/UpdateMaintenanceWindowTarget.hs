...
uOwnerInformation =
  lens _uOwnerInformation (\s a -> s {_uOwnerInformation = a}) .
  mapping _Sensitive
...
uDescription =
  lens _uDescription (\s a -> s {_uDescription = a}) . mapping _Sensitive
...
instance AWSRequest UpdateMaintenanceWindowTarget where
  type Rs UpdateMaintenanceWindowTarget = UpdateMaintenanceWindowTargetResponse
  request = postJSON ssm
  response =
    receiveJSON
      (\s h x ->
         UpdateMaintenanceWindowTargetResponse' <$> (x .?> "OwnerInformation") <*>
         (x .?> "WindowTargetId") <*>
         (x .?> "Name") <*>
         (x .?> "Targets" .!@ mempty) <*>
         (x .?> "Description") <*>
...
ursOwnerInformation =
  lens _ursOwnerInformation (\s a -> s {_ursOwnerInformation = a}) .
  mapping _Sensitive
...
ursDescription =
  lens _ursDescription (\s a -> s {_ursDescription = a}) . mapping _Sensitive
