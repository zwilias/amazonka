...
lieMaxResults =
  lens _lieMaxResults (\s a -> s {_lieMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListInventoryEntries where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListInventoryEntriesResponse' <$> (x .?> "InstanceId") <*>
         (x .?> "TypeName") <*>
         (x .?> "Entries" .!@ mempty) <*>
         (x .?> "SchemaVersion") <*>
         (x .?> "CaptureTime") <*>
...
instance ToHeaders ListInventoryEntries where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.ListInventoryEntries" :: ByteString)
...
liersEntries =
  lens _liersEntries (\s a -> s {_liersEntries = a}) . _Default . _Coerce

-- | The inventory schema version used by the instance(s).
liersSchemaVersion :: Lens' ListInventoryEntriesResponse (Maybe Text)
liersSchemaVersion =
  lens _liersSchemaVersion (\s a -> s {_liersSchemaVersion = a})
...
liersResponseStatus =
  lens _liersResponseStatus (\s a -> s {_liersResponseStatus = a})
