...
didMaxResults =
  lens _didMaxResults (\s a -> s {_didMaxResults = a}) . mapping _Nat
...
instance AWSRequest DescribeInventoryDeletions where
  type Rs DescribeInventoryDeletions = DescribeInventoryDeletionsResponse
...
didrsInventoryDeletions ::
     Lens' DescribeInventoryDeletionsResponse [InventoryDeletionStatusItem]
didrsInventoryDeletions =
  lens _didrsInventoryDeletions (\s a -> s {_didrsInventoryDeletions = a}) .
  _Default . _Coerce
...
didrsResponseStatus =
  lens _didrsResponseStatus (\s a -> s {_didrsResponseStatus = a})
