...
diSchemaDeleteOption ::
     Lens' DeleteInventory (Maybe InventorySchemaDeleteOption)
diSchemaDeleteOption =
  lens _diSchemaDeleteOption (\s a -> s {_diSchemaDeleteOption = a})
...
instance AWSRequest DeleteInventory where
  ...
  response =
    receiveJSON
      (\s h x ->
         DeleteInventoryResponse' <$> (x .?> "TypeName") <*>
         (x .?> "DeletionSummary") <*>
         (x .?> "DeletionId") <*>
         (pure (fromEnum s)))
...
dirsDeletionSummary ::
     Lens' DeleteInventoryResponse (Maybe InventoryDeletionSummary)
dirsDeletionSummary =
  lens _dirsDeletionSummary (\s a -> s {_dirsDeletionSummary = a})
