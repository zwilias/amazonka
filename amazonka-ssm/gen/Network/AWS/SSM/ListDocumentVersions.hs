...
ldvMaxResults =
  lens _ldvMaxResults (\s a -> s {_ldvMaxResults = a}) . mapping _Nat
...
instance AWSRequest ListDocumentVersions where
  ...
  response =
    receiveJSON
      (\s h x ->
         ListDocumentVersionsResponse' <$> (x .?> "DocumentVersions") <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders ListDocumentVersions where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.ListDocumentVersions" :: ByteString)
...
ldvrsDocumentVersions ::
     Lens' ListDocumentVersionsResponse (Maybe (NonEmpty DocumentVersionInfo))
ldvrsDocumentVersions =
  lens _ldvrsDocumentVersions (\s a -> s {_ldvrsDocumentVersions = a}) .
  mapping _List1
...
ldvrsResponseStatus =
  lens _ldvrsResponseStatus (\s a -> s {_ldvrsResponseStatus = a})
