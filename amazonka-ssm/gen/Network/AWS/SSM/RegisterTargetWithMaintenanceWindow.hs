...
rOwnerInformation =
  lens _rOwnerInformation (\s a -> s {_rOwnerInformation = a}) .
  mapping _Sensitive
...
rDescription =
  lens _rDescription (\s a -> s {_rDescription = a}) . mapping _Sensitive
...
rResourceType ::
     Lens' RegisterTargetWithMaintenanceWindow MaintenanceWindowResourceType
...
instance AWSRequest RegisterTargetWithMaintenanceWindow where
  type Rs RegisterTargetWithMaintenanceWindow = RegisterTargetWithMaintenanceWindowResponse
...
rrsWindowTargetId ::
     Lens' RegisterTargetWithMaintenanceWindowResponse (Maybe Text)
