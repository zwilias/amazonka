...
gdpbOperatingSystem =
  lens _gdpbOperatingSystem (\s a -> s {_gdpbOperatingSystem = a})
...
instance AWSRequest GetDefaultPatchBaseline where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetDefaultPatchBaselineResponse' <$> (x .?> "OperatingSystem") <*>
         (x .?> "BaselineId") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetDefaultPatchBaseline where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.GetDefaultPatchBaseline" :: ByteString)
         , "Content-Type" =# ("application/x-amz-json-1.1" :: ByteString)
         ])

instance ToJSON GetDefaultPatchBaseline where
  toJSON GetDefaultPatchBaseline' {..} =
    object (catMaybes [("OperatingSystem" .=) <$> _gdpbOperatingSystem])
...
gdpbrsOperatingSystem ::
     Lens' GetDefaultPatchBaselineResponse (Maybe OperatingSystem)
gdpbrsOperatingSystem =
  lens _gdpbrsOperatingSystem (\s a -> s {_gdpbrsOperatingSystem = a})
...
gdpbrsResponseStatus =
  lens _gdpbrsResponseStatus (\s a -> s {_gdpbrsResponseStatus = a})
