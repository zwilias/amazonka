...
dipMaxResults =
  lens _dipMaxResults (\s a -> s {_dipMaxResults = a}) . mapping _Nat
...
instance AWSRequest DescribeInstancePatches where
  ...
  response =
    receiveJSON
      (\s h x ->
         DescribeInstancePatchesResponse' <$> (x .?> "Patches" .!@ mempty) <*>
         (x .?> "NextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders DescribeInstancePatches where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =# ("AmazonSSM.DescribeInstancePatches" :: ByteString)
...
diprsPatches =
  lens _diprsPatches (\s a -> s {_diprsPatches = a}) . _Default . _Coerce
