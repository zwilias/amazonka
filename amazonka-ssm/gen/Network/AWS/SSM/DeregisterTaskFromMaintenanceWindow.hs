...
instance AWSRequest DeregisterTaskFromMaintenanceWindow where
  type Rs DeregisterTaskFromMaintenanceWindow = DeregisterTaskFromMaintenanceWindowResponse
  request = postJSON ssm
  response =
    receiveJSON
      (\s h x ->
         DeregisterTaskFromMaintenanceWindowResponse' <$> (x .?> "WindowTaskId") <*>
         (x .?> "WindowId") <*>
         (pure (fromEnum s)))
...
derrsWindowTaskId ::
     Lens' DeregisterTaskFromMaintenanceWindowResponse (Maybe Text)
