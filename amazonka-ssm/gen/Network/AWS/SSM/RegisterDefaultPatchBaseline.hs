...
instance AWSRequest RegisterDefaultPatchBaseline where
  type Rs RegisterDefaultPatchBaseline = RegisterDefaultPatchBaselineResponse
  request = postJSON ssm
  response =
    receiveJSON
      (\s h x ->
         RegisterDefaultPatchBaselineResponse' <$> (x .?> "BaselineId") <*>
         (pure (fromEnum s)))
...
rdpbrsResponseStatus =
  lens _rdpbrsResponseStatus (\s a -> s {_rdpbrsResponseStatus = a})
