...
instance FromText ConnectorCapability where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConnectorCapability from value: '" <>
        e <> "'. Accepted values: vsphere"
...
instance FromText ConnectorStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ConnectorStatus from value: '" <>
...
instance FromText LicenseType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing LicenseType from value: '" <>
        e <> "'. Accepted values: aws, byol"
...
instance FromText ReplicationJobState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReplicationJobState from value: '" <>
        e <> "'. Accepted values: active, deleted, deleting, failed, pending"
...
instance FromText ReplicationRunState where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReplicationRunState from value: '" <>
        e <>
...
instance FromText ReplicationRunType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ReplicationRunType from value: '" <>
        e <> "'. Accepted values: automatic, on_demand"
...
instance FromText ServerCatalogStatus where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ServerCatalogStatus from value: '" <>
        e <>
...
instance FromText ServerType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing ServerType from value: '" <>
        e <> "'. Accepted values: virtual_machine"
...
instance FromText VMManagerType where
  parser =
    takeLowerText >>= \case
      ...
      e ->
        fromTextError $
        "Failure parsing VMManagerType from value: '" <>
        e <> "'. Accepted values: vsphere"
