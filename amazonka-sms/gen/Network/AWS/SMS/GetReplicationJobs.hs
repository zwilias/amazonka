...
grjReplicationJobId =
  lens _grjReplicationJobId (\s a -> s {_grjReplicationJobId = a})
...
instance AWSRequest GetReplicationJobs where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetReplicationJobsResponse' <$> (x .?> "replicationJobList" .!@ mempty) <*>
         (x .?> "nextToken") <*>
         (pure (fromEnum s)))
...
instance ToHeaders GetReplicationJobs where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSServerMigrationService_V2016_10_24.GetReplicationJobs" :: ByteString)
...
grjrsReplicationJobList =
  lens _grjrsReplicationJobList (\s a -> s {_grjrsReplicationJobList = a}) .
  _Default . _Coerce
...
grjrsResponseStatus =
  lens _grjrsResponseStatus (\s a -> s {_grjrsResponseStatus = a})
