...
grrReplicationJobId =
  lens _grrReplicationJobId (\s a -> s {_grrReplicationJobId = a})
...
instance AWSRequest GetReplicationRuns where
  ...
  response =
    receiveJSON
      (\s h x ->
         GetReplicationRunsResponse' <$> (x .?> "replicationJob") <*>
         (x .?> "nextToken") <*>
...
instance ToHeaders GetReplicationRuns where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSServerMigrationService_V2016_10_24.GetReplicationRuns" :: ByteString)
...
grrrsReplicationJob =
  lens _grrrsReplicationJob (\s a -> s {_grrrsReplicationJob = a})
...
grrrsReplicationRunList =
  lens _grrrsReplicationRunList (\s a -> s {_grrrsReplicationRunList = a}) .
  _Default . _Coerce

-- | -- | The response status code.
grrrsResponseStatus :: Lens' GetReplicationRunsResponse Int
grrrsResponseStatus =
  lens _grrrsResponseStatus (\s a -> s {_grrrsResponseStatus = a})
