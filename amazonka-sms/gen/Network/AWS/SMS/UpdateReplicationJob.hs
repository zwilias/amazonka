...
urjNextReplicationRunStartTime =
  lens
    ...
    (\s a -> s {_urjNextReplicationRunStartTime = a}) .
  mapping _Time
...
urjReplicationJobId =
  lens _urjReplicationJobId (\s a -> s {_urjReplicationJobId = a})
...
instance ToHeaders UpdateReplicationJob where
  toHeaders =
    const
      (mconcat
         [ "X-Amz-Target" =#
           ("AWSServerMigrationService_V2016_10_24.UpdateReplicationJob" :: ByteString)
...
urjrsResponseStatus =
  lens _urjrsResponseStatus (\s a -> s {_urjrsResponseStatus = a})
